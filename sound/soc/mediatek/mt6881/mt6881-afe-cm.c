...
EXPORT_SYMBOL_GPL(mt6881_set_cm_mux);
...
EXPORT_SYMBOL_GPL(mt6881_get_cm_mux);

static int mt6881_convert_cm_ch(unsigned int ch)
{
	return ch - 1;
}
...
int mt6881_set_cm(struct mtk_base_afe *afe, int id,
	       unsigned int update, bool swap, unsigned int ch)
{
	...
	samplerate = mt6881_general_rate_transform_inverse(afe->dev, rate);
...
EXPORT_SYMBOL_GPL(mt6881_set_cm);
...
EXPORT_SYMBOL_GPL(mt6881_enable_cm_bypass);
...
EXPORT_SYMBOL_GPL(mt6881_enable_cm);
...
EXPORT_SYMBOL_GPL(mt6881_is_need_enable_cm);
