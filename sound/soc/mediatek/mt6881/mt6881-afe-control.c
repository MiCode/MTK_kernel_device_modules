...
unsigned int mt6881_swpm_rate_transform(struct device *dev,
		unsigned int rate)
{
	switch (rate) {
	case 32000:
				   ...
				   unsigned int rate, int aud_blk)
{
	...
	default:
		return mt6881_general_rate_transform(dev, rate);
	}
}
...
EXPORT_SYMBOL(mt6881_enable_dc_compensation);
...
EXPORT_SYMBOL(mt6881_set_lch_dc_compensation);
...
EXPORT_SYMBOL(mt6881_set_rch_dc_compensation);
...
EXPORT_SYMBOL(mt6881_set_adda_predistortion);
...
EXPORT_SYMBOL(mt6881_aud_get_power_scenario);
