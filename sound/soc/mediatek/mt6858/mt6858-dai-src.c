			    ...
			    int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		if (id == MT6858_DAI_SRC_0)
			mtk_set_src_0_param(afe, id);
		else if (id == MT6858_DAI_SRC_1)
			mtk_set_src_1_param(afe, id);
		else if (id == MT6858_DAI_SRC_2)
				  ...
				  struct snd_soc_dapm_widget *sink)
{
	...
	if (strcmp(w->name, HW_SRC_0_EN_W_NAME) == 0)
		src_priv = afe_priv->dai_priv[MT6858_DAI_SRC_0];
	else if (strcmp(w->name, HW_SRC_1_EN_W_NAME) == 0)
		src_priv = afe_priv->dai_priv[MT6858_DAI_SRC_1];
	else if (strcmp(w->name, HW_SRC_2_EN_W_NAME) == 0)
		src_priv = afe_priv->dai_priv[MT6858_DAI_SRC_2];
	else
		src_priv = afe_priv->dai_priv[MT6858_DAI_SRC_3];
				     ...
				     struct snd_soc_dapm_widget *sink)
{
	...
	need_apll = mt6858_get_apll_by_rate(afe, src_priv->ul_rate);
...
#ifdef GSRC_REG
	unsigned int rate_reg = mt6858_rate_transform(afe->dev, rate, id);
#endif
...
int mt6858_dai_src_register(struct mtk_base_afe *afe)
{
	...
	/* set dai priv */
	ret = mt6858_dai_set_priv(afe, MT6858_DAI_SRC_0,
				  sizeof(struct mtk_afe_src_priv), NULL);
	if (ret)
		return ret;

	ret = mt6858_dai_set_priv(afe, MT6858_DAI_SRC_1,
				  sizeof(struct mtk_afe_src_priv), NULL);
	if (ret)
		return ret;

	ret = mt6858_dai_set_priv(afe, MT6858_DAI_SRC_2,
				  sizeof(struct mtk_afe_src_priv), NULL);
	if (ret)
		return ret;

	ret = mt6858_dai_set_priv(afe, MT6858_DAI_SRC_3,
				  sizeof(struct mtk_afe_src_priv), NULL);
	if (ret)
		return ret;
