...
int mt6858_set_local_afe(struct mtk_base_afe *afe)
{
	local_afe = afe;
	return 0;
}

unsigned int mt6858_swpm_rate_transform(struct device *dev,
		unsigned int rate)
{
	switch (rate) {
	case 32000:
...
unsigned int mt6858_general_rate_transform(struct device *dev,
		unsigned int rate)
{
	switch (rate) {
	case 8000:
...
unsigned int mt6858_general_rate_transform_inverse(struct device *dev,
		unsigned int rate)
{
	switch (rate) {
...
unsigned int mt6858_rate_transform(struct device *dev,
				   unsigned int rate, int aud_blk)
{
	switch (aud_blk) {
	case MT6858_DAI_PCM_0:
	case MT6858_DAI_PCM_1:
		return pcm_rate_transform(dev, rate);
	default:
		return mt6858_general_rate_transform(dev, rate);
	}
}

int mt6858_dai_set_priv(struct mtk_base_afe *afe, int id,
...
int mt6858_set_adda_predistortion(int hp_impedance)
{
	int ret;
...
EXPORT_SYMBOL(mt6858_set_adda_predistortion);

int mt6858_get_output_device(struct mtk_base_afe *afe)
{
...
int mt6858_get_input_device(struct mtk_base_afe *afe)
{
	return AUDIO_INPUT_INVALID;
}

void mt6858_aud_swpm_power_off(void)
{
	memset(&local_audio_data, 0, sizeof(local_audio_data));
	local_audio_data.freq_clock = 26;
}

void mt6858_aud_update_power_scenario(void)
{
	...
	adda_priv = afe_priv->dai_priv[MT6858_DAI_ADDA];
	...
	audio_data.output_device = mt6858_get_output_device(local_afe);
	audio_data.input_device = mt6858_get_input_device(local_afe);
	if (audio_data.input_device != AUDIO_INPUT_INVALID &&
		...
		audio_data.sample_rate = mt6858_swpm_rate_transform(local_afe->dev, value);
...
void *mt6858_aud_get_power_scenario(void)
{
...
EXPORT_SYMBOL(mt6858_aud_get_power_scenario);
