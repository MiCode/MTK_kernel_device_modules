...
/*
 *  Mediatek ALSA SoC AFE platform driver for 6858
 *
...
static ssize_t mt6858_debug_read_reg(char *buffer, int size, struct mtk_base_afe *afe);

static const struct snd_pcm_hardware mt6858_afe_hardware = {
...
static int mt6858_fe_startup(struct snd_pcm_substream *substream,
...
void mt6858_fe_shutdown(struct snd_pcm_substream *substream,
			struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct mtk_base_afe *afe = snd_soc_dai_get_drvdata(dai);
...
int mt6858_fe_trigger(struct snd_pcm_substream *substream, int cmd,
		      struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	...
	if (!in_interrupt()) {
		...
		mt6858_aud_update_power_scenario();
	}
	...
	case SNDRV_PCM_TRIGGER_RESUME:
		...
		if (id == MT6858_MEMIF_DL23)
			mtk_memif_set_pbuf_size(afe, id, MT6858_MEMIF_PBUF_SIZE_32_BYTES);

		if (!strcmp(memif->data->name, "VUL_CM0")
			|| !strcmp(memif->data->name, "VUL_CM1"))
			mtk_memif_set_min_max_len(afe, id, MT6858_MEMIF_MAX_LEN_64_BYTES,
						MT6858_MEMIF_MAX_LEN_64_BYTES);
		...
		if (is_afe_need_triggered(memif)) {
			...
			if (!strcmp(memif->data->name, "VUL8") || !strcmp(memif->data->name, "VUL_CM0")) {
				if (mt6858_is_need_enable_cm(afe, CM0))
					mt6858_enable_cm(afe, CM0, 1);
			}
			if (!strcmp(memif->data->name, "VUL9") || !strcmp(memif->data->name, "VUL_CM1")) {
				if (mt6858_is_need_enable_cm(afe, CM1))
					mt6858_enable_cm(afe, CM1, 1);
			}
		}
	...
	case SNDRV_PCM_TRIGGER_SUSPEND:
		...
		if (is_afe_need_triggered(memif)) {
			...
			if (!strcmp(memif->data->name, "VUL8") || !strcmp(memif->data->name, "VUL_CM0"))
				mt6858_enable_cm(afe, CM0, 0);
			if (!strcmp(memif->data->name, "VUL9") || !strcmp(memif->data->name, "VUL_CM1"))
				mt6858_enable_cm(afe, CM1, 0);
		}
...
static int mt6858_memif_fs(struct snd_pcm_substream *substream,
			   unsigned int rate)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	...
	rate_reg = mt6858_rate_transform(afe->dev, rate, id);

	switch (id) {
	case MT6858_MEMIF_VUL8:
	case MT6858_MEMIF_VUL_CM0:
		cm = 0x0;
		break;
	case MT6858_MEMIF_VUL9:
	case MT6858_MEMIF_VUL_CM1:
		cm = 0x1;
		break;
	default:
		cm = 0x0;
	...
	mt6858_set_cm_rate(cm, rate_reg);

	return rate_reg;
}

static int mt6858_get_dai_fs(struct mtk_base_afe *afe,
			     int dai_id, unsigned int rate)
{
	return mt6858_rate_transform(afe->dev, rate, dai_id);
}

static int mt6858_irq_fs(struct snd_pcm_substream *substream, unsigned int rate)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_component *component =
		snd_soc_rtdcom_lookup(rtd, AFE_PCM_NAME);
	...
	if (!component)
		return -EINVAL;
	afe = snd_soc_component_get_drvdata(component);
	return mt6858_general_rate_transform(afe->dev, rate);
}

int mt6858_get_memif_pbuf_size(struct snd_pcm_substream *substream)
{
	struct snd_pcm_runtime *runtime = substream->runtime;

	if ((runtime->period_size * 1000) / runtime->rate > 10)
		return MT6858_MEMIF_PBUF_SIZE_256_BYTES;
	else
		return MT6858_MEMIF_PBUF_SIZE_32_BYTES;
}

/* FE DAIs */
static const struct snd_soc_dai_ops mt6858_memif_dai_ops = {
	.startup        = mt6858_fe_startup,
	.shutdown       = mt6858_fe_shutdown,
	.hw_params      = mtk_afe_fe_hw_params,
	.hw_free        = mtk_afe_fe_hw_free,
	.prepare        = mtk_afe_fe_prepare,
	.trigger        = mt6858_fe_trigger,
};
...
static struct snd_soc_dai_driver mt6858_memif_dai_driver[] = {
...
/* kcontrol */
static int mt6858_irq_cnt1_get(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	...
	ucontrol->value.integer.value[0] =
		afe_priv->irq_cnt[MT6858_PRIMARY_MEMIF];
	return 0;
}

static int mt6858_irq_cnt1_set(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6858_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6858_PRIMARY_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
	int irq_id = memif->irq_usage;
...
static int mt6858_irq_cnt2_get(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	...
	ucontrol->value.integer.value[0] =
		afe_priv->irq_cnt[MT6858_RECORD_MEMIF];
	return 0;
}

static int mt6858_irq_cnt2_set(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6858_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6858_RECORD_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
	int irq_id = memif->irq_usage;
	int irq_cnt = afe_priv->irq_cnt[memif_num];
...
static int mt6858_deep_irq_cnt_get(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	...
	ucontrol->value.integer.value[0] = afe_priv->irq_cnt[MT6858_DEEP_MEMIF];
	return 0;
}

static int mt6858_deep_irq_cnt_set(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6858_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6858_DEEP_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
	int irq_id = memif->irq_usage;
	int irq_cnt = afe_priv->irq_cnt[memif_num];
...
static int mt6858_voip_rx_irq_cnt_get(struct snd_kcontrol *kcontrol,
				      struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	...
	ucontrol->value.integer.value[0] = afe_priv->irq_cnt[MT6858_VOIP_MEMIF];
	return 0;
}

static int mt6858_voip_rx_irq_cnt_set(struct snd_kcontrol *kcontrol,
				      struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6858_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6858_VOIP_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
	int irq_id = memif->irq_usage;
	int irq_cnt = afe_priv->irq_cnt[memif_num];
...
static int mt6858_deep_scene_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6858_deep_scene_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6858_fast_scene_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6858_fast_scene_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6858_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6858_FAST_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
...
static int mt6858_primary_scene_get(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6858_primary_scene_set(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6858_voip_scene_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6858_voip_scene_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6858_record_xrun_assert_get(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6858_afe_private *afe_priv = afe->platform_priv;
	int xrun_assert = afe_priv->xrun_assert[MT6858_RECORD_MEMIF];

	ucontrol->value.integer.value[0] = xrun_assert;
	return 0;
}

static int mt6858_record_xrun_assert_set(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	...
	afe_priv->xrun_assert[MT6858_RECORD_MEMIF] = xrun_assert;
	return 0;
}

static int mt6858_echo_ref_xrun_assert_get(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6858_afe_private *afe_priv = afe->platform_priv;
	int xrun_assert = afe_priv->xrun_assert[MT6858_ECHO_REF_MEMIF];

	ucontrol->value.integer.value[0] = xrun_assert;
	return 0;
}

static int mt6858_echo_ref_xrun_assert_set(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	...
	dev_info(afe->dev, "%s(), xrun_assert %d\n", __func__, xrun_assert);
	afe_priv->xrun_assert[MT6858_ECHO_REF_MEMIF] = xrun_assert;
	return 0;
}

#if !IS_ENABLED(CONFIG_NEBULA_SND_PASSTHROUGH)
static int mt6858_sram_size_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6858_vow_barge_in_irq_id_get(struct snd_kcontrol *kcontrol,
					  struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	int memif_num = MT6858_BARGE_IN_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
	int irq_id = memif->irq_usage;
...
#if IS_ENABLED(CONFIG_SND_SOC_MTK_AUDIO_DSP)
static int mt6858_adsp_ref_mem_get(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6858_adsp_ref_mem_set(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6858_adsp_mem_get(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6858_adsp_mem_set(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
#if !IS_ENABLED(CONFIG_NEBULA_SND_PASSTHROUGH)
static int mt6858_mmap_dl_scene_get(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6858_mmap_dl_scene_set(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6858_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6858_MMAP_DL_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
...
static int mt6858_mmap_ul_scene_get(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6858_mmap_ul_scene_set(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6858_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6858_MMAP_UL_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
...
static int mt6858_mmap_ion_get(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
...
static int mt6858_mmap_ion_set(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6858_dl_mmap_fd_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6858_dl_mmap_fd_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}

static int mt6858_ul_mmap_fd_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6858_ul_mmap_fd_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}
...
#if IS_ENABLED(CONFIG_NEBULA_SND_PASSTHROUGH)
static int mt6858_set_memif_sram_mode(struct device *dev,
				      enum mtk_audio_sram_mode sram_mode);
static int mt6858_set_sram_mode(struct device *dev,
				enum mtk_audio_sram_mode sram_mode);

static int mt6858_use_dram_only_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	...
	for (i = 0; i < MT6858_MEMIF_NUM; i++)
		ucontrol->value.bytes.data[i] = afe->memif[i].use_dram_only;

	return 0;
}

static int mt6858_sram_mode_set(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	...
	rc = mt6858_set_sram_mode(afe->dev, ucontrol->value.bytes.data[0]);
...
// according to mt6858_mt6681_dai_links
// c0d15c -> capture 9 -> UL9 -> memif id 24
unsigned int virtio_id_memif_index_mapping[MT6858_MEMIF_NUM] = {
	MT6858_MEMIF_DL0,
	MT6858_MEMIF_DL1,
	MT6858_MEMIF_DL2,
	MT6858_MEMIF_DL3,
	...
	MT6858_MEMIF_VUL2,
	MT6858_MEMIF_VUL5,
	MT6858_MEMIF_VUL_CM0,
	MT6858_MEMIF_VUL_CM1,
	MT6858_MEMIF_VUL10,
	MT6858_MEMIF_VUL6,
	MT6858_MEMIF_VUL25,
	MT6858_MEMIF_VUL8,
	MT6858_MEMIF_VUL24,
	MT6858_MEMIF_ETDM_IN1,
	MT6858_MEMIF_ETDM_IN2,
	MT6858_MEMIF_ETDM_IN4,
};

static int mt6858_passthrough_shm_set(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
#ifndef SKIP_VIP
// ALPS08709395: AudioQOS, set VIP
static int mt6858_audio_vip_get(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = 0;
	return 0;
...
static int mt6858_audio_vip_set(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6858_binderthread_vip_get(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = 0;
	return 0;
...
static int mt6858_binderthread_vip_set(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6858_swcodec_vip_get(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = 0;
	return 0;
...
static int mt6858_swcodec_vip_set(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
...
#endif
static int mt6858_mmap_clean_get(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = 0;
	return 0;
...
static int mt6858_mmap_clean_set(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static const struct snd_kcontrol_new mt6858_pcm_kcontrols[] = {
	...
	SOC_SINGLE_EXT("adsp_ref_sharemem_scenario",
		       SND_SOC_NOPM, 0, 0x1, 0,
		       mt6858_adsp_ref_mem_get,
		       mt6858_adsp_ref_mem_set),
	...
	SOC_SINGLE_EXT("mmap_play_scenario", SND_SOC_NOPM, 0, 0x1, 0,
		       mt6858_mmap_dl_scene_get, mt6858_mmap_dl_scene_set),
	SOC_SINGLE_EXT("mmap_record_scenario", SND_SOC_NOPM, 0, 0x1, 0,
		       mt6858_mmap_ul_scene_get, mt6858_mmap_ul_scene_set),
	...
	SOC_SINGLE_EXT("aaudio_dl_mmap_fd",
		       SND_SOC_NOPM, 0, 0xffffffff, 0,
		       mt6858_dl_mmap_fd_get,
		       mt6858_dl_mmap_fd_set),
	SOC_SINGLE_EXT("aaudio_ul_mmap_fd",
		       SND_SOC_NOPM, 0, 0xffffffff, 0,
		       mt6858_ul_mmap_fd_get,
		       mt6858_ul_mmap_fd_set),
#endif
	SOC_ENUM_EXT("MTK_RECORD_MISO1", mt6858_pcm_type_enum[0],
		     record_miso1_en_get, record_miso1_en_set),
#ifndef SKIP_VIP
	SOC_SINGLE_EXT("audio_vip", SND_SOC_NOPM, 0, 0x3fffff, 0,
		       mt6858_audio_vip_get, mt6858_audio_vip_set),
	SOC_SINGLE_EXT("SWCODEC_VIP", SND_SOC_NOPM, 0, 0x3fffff, 0,
		       mt6858_swcodec_vip_get, mt6858_swcodec_vip_set),
	SOC_SINGLE_EXT("BinderThread_VIP", SND_SOC_NOPM, 0, 0x3fffff, 0,
		       mt6858_binderthread_vip_get, mt6858_binderthread_vip_set),
#endif
	SOC_SINGLE_EXT("aaudio_clean_buffer",
		       SND_SOC_NOPM, 0, 0xffffffff, 0,
		       mt6858_mmap_clean_get,
		       mt6858_mmap_clean_set),
#if IS_ENABLED(CONFIG_NEBULA_SND_PASSTHROUGH)
	SND_SOC_BYTES_EXT("use_dram_only", MT6858_MEMIF_NUM,
		      mt6858_use_dram_only_get, NULL),
	SND_SOC_BYTES_EXT("sram_mode", 1,
		      NULL, mt6858_sram_mode_set),
	SND_SOC_BYTES_EXT("passthrough_shm",
		      sizeof(struct virtio_passthrough_shm_msg),
		      NULL, mt6858_passthrough_shm_set),
#endif
};
			...
			int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		mt6858_enable_cm_bypass(afe, CM0, 0x0);
		mt6858_set_cm(afe, CM0, 0x1, false, channels);
		break;
	case SND_SOC_DAPM_PRE_PMD:
		mt6858_enable_cm_bypass(afe, CM0, 0x1);
		break;
	default:
		break;
	}
			...
			int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		channels = mtk_get_channel_value();
		mt6858_enable_cm_bypass(afe, CM1, 0x0);
		mt6858_set_cm(afe, CM1, 0x1, false, channels);
		break;
	case SND_SOC_DAPM_PRE_PMD:
		/* remove so we fix in normal mode */
#if IS_ENABLED(CONFIG_SND_SOC_MTK_AUTO_AUDIO)
		mt6858_enable_cm_bypass(afe, CM1, 0x1);
#endif
		break;
	default:
		break;
		      ...
		      struct snd_ctl_elem_value *ucontrol)
{
	...
	if (!strcmp(widget->name, "CM1_UL_MUX")) {
		ucontrol->value.integer.value[0] = mt6858_get_cm_mux(CM1);
	} else if (!strcmp(widget->name, "CM0_UL_MUX")) {
		ucontrol->value.integer.value[0] = mt6858_get_cm_mux(CM0);
	} else {
		      ...
		      struct snd_ctl_elem_value *ucontrol)
{
	...
	if (!strcmp(widget->name, "CM1_UL_MUX"))
		mt6858_set_cm_mux(CM1, ucontrol->value.integer.value[0]);
	else if (!strcmp(widget->name, "CM0_UL_MUX"))
		mt6858_set_cm_mux(CM0, ucontrol->value.integer.value[0]);
	else
		dev_info(afe->dev, "%s(), No match: %s\n", __func__, widget->name);
...
static const struct snd_soc_dapm_widget mt6858_memif_widgets[] = {
	/* inter-connections */
...
static const struct snd_soc_dapm_route mt6858_memif_routes[] = {
	{"UL0", NULL, "UL0_CH1"},
	{"UL0", NULL, "UL0_CH2"},
	/* Normal record */
...
static const struct mtk_base_memif_data memif_data[MT6858_MEMIF_NUM] = {
	[MT6858_MEMIF_DL0] = {
		.name = "DL0",
		.id = MT6858_MEMIF_DL0,
...
static const struct mtk_base_irq_data irq_data[MT6858_IRQ_NUM] = {
	[MT6858_IRQ_0] = {
		.id = MT6858_IRQ_0,
...
#if IS_ENABLED(CONFIG_SND_SOC_MTK_AUTO_AUDIO)
static const int memif_irq_usage[MT6858_MEMIF_NUM] = {
	/* TODO: verify each memif & irq */
	[MT6858_MEMIF_DL0] = MT6858_IRQ_4,
	[MT6858_MEMIF_DL1] = MT6858_IRQ_1,
	[MT6858_MEMIF_DL2] = MT6858_IRQ_2,
	[MT6858_MEMIF_DL3] = MT6858_IRQ_3,
	[MT6858_MEMIF_DL4] = MT6858_IRQ_3,
	[MT6858_MEMIF_DL5] = MT6858_IRQ_3,
	[MT6858_MEMIF_DL6] = MT6858_IRQ_3,
	[MT6858_MEMIF_DL7] = MT6858_IRQ_7,
	[MT6858_MEMIF_DL8] = MT6858_IRQ_8,
	[MT6858_MEMIF_DL23] = MT6858_IRQ_9,
	[MT6858_MEMIF_DL24] = MT6858_IRQ_10,
	[MT6858_MEMIF_DL25] = MT6858_IRQ_11,
	[MT6858_MEMIF_DL_24CH] = MT6858_IRQ_12,
	[MT6858_MEMIF_VUL0] = MT6858_IRQ_13,
	[MT6858_MEMIF_VUL1] = MT6858_IRQ_14,
	[MT6858_MEMIF_VUL2] = MT6858_IRQ_15,
	[MT6858_MEMIF_VUL3] = MT6858_IRQ_16,
	[MT6858_MEMIF_VUL4] = MT6858_IRQ_17,
	[MT6858_MEMIF_VUL5] = MT6858_IRQ_18,
	[MT6858_MEMIF_VUL6] = MT6858_IRQ_19,
	[MT6858_MEMIF_VUL7] = MT6858_IRQ_20,
	[MT6858_MEMIF_VUL8] = MT6858_IRQ_21,
	[MT6858_MEMIF_VUL9] = MT6858_IRQ_22,
	[MT6858_MEMIF_VUL10] = MT6858_IRQ_23,
	[MT6858_MEMIF_VUL24] = MT6858_IRQ_24,
	[MT6858_MEMIF_VUL25] = MT6858_IRQ_25,
	[MT6858_MEMIF_VUL_CM0] = MT6858_IRQ_26,
	[MT6858_MEMIF_VUL_CM1] = MT6858_IRQ_0,
	[MT6858_MEMIF_ETDM_IN1] = MT6858_IRQ_0,
	[MT6858_MEMIF_ETDM_IN2] = MT6858_IRQ_0,
	[MT6858_MEMIF_ETDM_IN4] = MT6858_IRQ_0,
};
#else
static const int memif_irq_usage[MT6858_MEMIF_NUM] = {
	/* ADSP will use IRQ 29/30 for urgent IRQ, please do not use */
	/* TODO: verify each memif & irq */
	[MT6858_MEMIF_DL0] = MT6858_IRQ_0,
	[MT6858_MEMIF_DL1] = MT6858_IRQ_1,
	[MT6858_MEMIF_DL2] = MT6858_IRQ_2,
	[MT6858_MEMIF_DL3] = MT6858_IRQ_3,
	[MT6858_MEMIF_DL4] = MT6858_IRQ_4,
	[MT6858_MEMIF_DL5] = MT6858_IRQ_5,
	[MT6858_MEMIF_DL6] = MT6858_IRQ_6,
	[MT6858_MEMIF_DL7] = MT6858_IRQ_7,
	[MT6858_MEMIF_DL8] = MT6858_IRQ_8,
	[MT6858_MEMIF_DL23] = MT6858_IRQ_9,
	[MT6858_MEMIF_DL24] = MT6858_IRQ_10,
...
static bool mt6858_is_volatile_reg(struct device *dev, unsigned int reg)
{
...
static const struct regmap_config mt6858_afe_regmap_config = {
	.reg_bits = 32,
	.reg_stride = 4,
	.val_bits = 32,

	.volatile_reg = mt6858_is_volatile_reg,
...
static irqreturn_t mt6858_afe_irq_handler(int irq_id, void *dev)
{
...
static int mt6858_afe_runtime_suspend(struct device *dev)
{
...
static int mt6858_afe_runtime_resume(struct device *dev)
{
	struct mtk_base_afe *afe = dev_get_drvdata(dev);
	int ret = 0;

	ret = mt6858_afe_enable_clock(afe);
	dev_info_ratelimited(afe->dev, "%s(), enable_clock ret %d\n", __func__, ret);

	if (ret)
		return ret;
	mt6858_afe_sram_request(afe);
...
static int mt6858_afe_pcm_copy(struct snd_pcm_substream *substream,
				...
				enum mtk_audio_sram_mode sram_mode)
{
	...
	/* set memif sram mode */
	mt6858_set_memif_sram_mode(dev, sram_mode);
...
static const struct mtk_audio_sram_ops mt6858_sram_ops = {
	.set_sram_mode = mt6858_set_sram_mode,
};
				    ...
				    char *buf, loff_t offset, size_t size)
{
	...
	read_size = mt6858_debug_read_reg(buffer, AFE_SYS_DEBUG_SIZE, afe);
...
static int mt6858_afe_component_probe(struct snd_soc_component *component)
{
	struct mtk_base_afe *afe = NULL;
	...
	if (component) {
		...
		mt6858_add_misc_control(component);
...
static const struct snd_soc_component_driver mt6858_afe_component = {
	.name = AFE_PCM_NAME,
	.probe = mt6858_afe_component_probe,
...
static ssize_t mt6858_debugfs_read(struct file *file, char __user *buf,
				   size_t count, loff_t *pos)
{
	...
	n = mt6858_debug_read_reg(buffer, size, afe);
...
static const struct mtk_afe_debug_cmd mt6858_debug_cmds[] = {
	MTK_AFE_DBG_CMD("write_reg", mtk_afe_debug_write_reg),
	{}
};

static const struct file_operations mt6858_debugfs_ops = {
	.open = mtk_afe_debugfs_open,
	.write = mtk_afe_debugfs_write,
	.read = mt6858_debugfs_read,
};
#endif

static int mt6858_dai_memif_register(struct mtk_base_afe *afe)
{
	...
	dai->dai_drivers = mt6858_memif_dai_driver;
	dai->num_dai_drivers = ARRAY_SIZE(mt6858_memif_dai_driver);

	dai->controls = mt6858_pcm_kcontrols;
	dai->num_controls = ARRAY_SIZE(mt6858_pcm_kcontrols);
	dai->dapm_widgets = mt6858_memif_widgets;
	dai->num_dapm_widgets = ARRAY_SIZE(mt6858_memif_widgets);
	dai->dapm_routes = mt6858_memif_routes;
	dai->num_dapm_routes = ARRAY_SIZE(mt6858_memif_routes);
	return 0;
}

typedef int (*dai_register_cb)(struct mtk_base_afe *);
static const dai_register_cb dai_register_cbs[] = {
	mt6858_dai_adda_register,
	mt6858_dai_i2s_register,
	mt6858_dai_hw_gain_register,
	mt6858_dai_src_register,
	mt6858_dai_pcm_register,
	mt6858_dai_hostless_register,
	mt6858_dai_memif_register,
};

static int mt6858_afe_pcm_dev_probe(struct platform_device *pdev)
{
	int ret, i;
	unsigned int tmp_reg = 0;
#if !defined(IS_FPGA_EARLY_PORTING) || defined(FORCE_FPGA_ENABLE_IRQ)
	...
	ret = mtk_audio_sram_init(dev, afe->sram, &mt6858_sram_ops);
	if (ret)
		return ret;
#endif
	/* init memif */
	...
	afe->memif_size = MT6858_MEMIF_NUM;
	...
	afe->memif[MT6858_DEEP_MEMIF].ack = mtk_sp_clean_written_buffer_ack;
	afe->memif[MT6858_FAST_MEMIF].fast_palyback = 1;

	mutex_init(&afe->irq_alloc_lock);       /* needed when dynamic irq */

	/* init irq */
	afe->irqs_size = MT6858_IRQ_NUM;
...
#endif
	ret = devm_request_irq(dev, irq_id, mt6858_afe_irq_handler,
			       IRQF_TRIGGER_NONE,
	...
	/* others */
	afe->mtk_afe_hardware = &mt6858_afe_hardware;
	afe->memif_fs = mt6858_memif_fs;
	afe->irq_fs = mt6858_irq_fs;
	afe->get_dai_fs = mt6858_get_dai_fs;
	afe->get_memif_pbuf_size = mt6858_get_memif_pbuf_size;

	afe->runtime_resume = mt6858_afe_runtime_resume;
	afe->runtime_suspend = mt6858_afe_runtime_suspend;

	afe->request_dram_resource = mt6858_afe_dram_request;
	afe->release_dram_resource = mt6858_afe_dram_release;

	afe->copy = mt6858_afe_pcm_copy;
...
#if IS_ENABLED(CONFIG_DEBUG_FS)
	/* debugfs */
	afe->debug_cmds = mt6858_debug_cmds;
	afe->debugfs = debugfs_create_file("mtksocaudio",
					   S_IFREG | 0444, NULL,
					   afe, &mt6858_debugfs_ops);
#endif
	...
	mt6858_set_cm_mux(CM0, 0);
	mt6858_set_cm_mux(CM1, 0);
err_find_pmic:
#if IS_ENABLED(CONFIG_SND_SOC_MTK_AUDIO_DSP)
...
static void mt6858_afe_pcm_dev_remove(struct platform_device *pdev)
{
	struct mtk_base_afe *afe = platform_get_drvdata(pdev);

	pm_runtime_disable(&pdev->dev);
	if (!pm_runtime_status_suspended(&pdev->dev))
		mt6858_afe_runtime_suspend(&pdev->dev);
#if !defined(SKIP_SB_USB_OFFLOAD)
...
static const struct of_device_id mt6858_afe_pcm_dt_match[] = {
	{ .compatible = "mediatek,mt6858-sound", },
	{},
};
MODULE_DEVICE_TABLE(of, mt6858_afe_pcm_dt_match);

static const struct dev_pm_ops mt6858_afe_pm_ops = {
	SET_RUNTIME_PM_OPS(mt6858_afe_runtime_suspend,
			   mt6858_afe_runtime_resume, NULL)
};

static struct platform_driver mt6858_afe_pcm_driver = {
	.driver = {
		.name = "mt6858-audio",
		.of_match_table = mt6858_afe_pcm_dt_match,
#if IS_ENABLED(CONFIG_PM)
		.pm = &mt6858_afe_pm_ops,
#endif
	},
	.probe = mt6858_afe_pcm_dev_probe,
	.remove = mt6858_afe_pcm_dev_remove,
};

module_platform_driver(mt6858_afe_pcm_driver);

MODULE_DESCRIPTION("Mediatek ALSA SoC AFE platform driver for 6858");
