		...
		const char *name)
{
	...
	if (strncmp(name, "afe_dl0_dac_hires_audio", 21) == 0 ||
		...
		dai_id = MT6858_DAI_ADDA;
	else if (strncmp(name, "aud_dl1_dac_hires_audio", 21) == 0 ||
		 strncmp(name, "afe_ul1_adc_hires_audio", 21) == 0)
		dai_id = MT6858_DAI_ADDA_CH34;
	else
		return NULL;
...
static int mtk_adda_ul_src_dmic(struct mtk_base_afe *afe, int id)
{
	...
	switch (id) {
	case MT6858_DAI_ADDA:
	case MT6858_DAI_AP_DMIC:
		reg_con0 = AFE_ADDA_UL0_SRC_CON0;
		reg_con1 = AFE_ADDA_UL0_SRC_CON1;
		break;
	case MT6858_DAI_ADDA_CH34:
	case MT6858_DAI_AP_DMIC_CH34:
			     ...
			     int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		mt6858_afe_gpio_request(afe, true, MT6858_DAI_ADDA, 1);
		...
		if (mtkaif_dmic) {
			...
			regmap_update_bits(afe->regmap, AFE_MTKAIF0_RX_CFG0,
					   ...
					   0x0);
			mtk_adda_ul_src_dmic(afe, MT6858_DAI_ADDA);
		}
		break;
	case SND_SOC_DAPM_POST_PMD:
		/* should delayed 1/fs(smallest is 8k) = 125us before afe off */
		udelay(125);
		mt6858_afe_gpio_request(afe, false, MT6858_DAI_ADDA, 1);
				  ...
				  int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		mt6858_afe_gpio_request(afe, true, MT6858_DAI_ADDA_CH34, 1);
		...
		if (mtkaif_dmic) {
			...
			regmap_update_bits(afe->regmap,
					   ...
					   0x0);
			mtk_adda_ul_src_dmic(afe, MT6858_DAI_ADDA_CH34);
		}
	...
	case SND_SOC_DAPM_POST_PMD:
		/* should delayed 1/fs(smallest is 8k) = 125us before afe off */
		udelay(125);
		mt6858_afe_gpio_request(afe, false, MT6858_DAI_ADDA_CH34, 1);
			     ...
			     int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		mt6858_afe_gpio_request(afe, true, MT6858_DAI_ADDA, 1);
		//mtk_adda_ul_src_dmic(afe, MT6858_DAI_AP_DMIC);
		mt6858_afe_gpio_request(afe, true, MT6858_DAI_AP_DMIC, 1);
		break;
	case SND_SOC_DAPM_POST_PMD:
		/* should delayed 1/fs(smallest is 8k) = 125us before afe off */
		udelay(125);
		mt6858_afe_gpio_request(afe, false, MT6858_DAI_ADDA, 1);
		mt6858_afe_gpio_request(afe, false, MT6858_DAI_AP_DMIC, 1);
				  ...
				  int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		mt6858_afe_gpio_request(afe, true, MT6858_DAI_ADDA_CH34, 1);
		mt6858_afe_gpio_request(afe, true, MT6858_DAI_AP_DMIC_CH34, 1);
		break;
	case SND_SOC_DAPM_POST_PMD:
		/* should delayed 1/fs(smallest is 8k) = 125us before afe off */
		udelay(125);
		mt6858_afe_gpio_request(afe, false, MT6858_DAI_ADDA_CH34, 1);
		mt6858_afe_gpio_request(afe, false, MT6858_DAI_AP_DMIC_CH34, 1);
		break;
	default:
		break;
	}
...
static bool is_adda_mtkaif_need_phase_delay(struct mt6858_afe_private *afe_priv)
{
	if (mt6858_afe_gpio_is_prepared(MT6858_AFE_GPIO_DAT_MISO0_ON) &&
	...
	if (mt6858_afe_gpio_is_prepared(MT6858_AFE_GPIO_DAT_MISO1_ON) &&
			     ...
			     int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		mt6858_afe_gpio_request(afe, true, MT6858_DAI_ADDA, 0);
		break;
	case SND_SOC_DAPM_POST_PMD:
		/* should delayed 1/fs(smallest is 8k) = 125us before afe off */
		udelay(125);
		mt6858_afe_gpio_request(afe, false, MT6858_DAI_ADDA, 0);
		break;
	default:
		break;
	}
...
/* mtkaif dmic */
static const char *const mt6858_adda_off_on_str[] = {
	"Off", "On"
};

static const struct soc_enum mt6858_adda_enum[] = {
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(mt6858_adda_off_on_str),
			    mt6858_adda_off_on_str),
};

static int mt6858_adda_ap_dmic_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6858_adda_ap_dmic_set(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6858_adda_dmic_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6858_adda_dmic_set(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6858_adda6_only_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6858_adda6_only_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6858_vow_enable_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6858_vow_enable_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static const struct snd_kcontrol_new mtk_adda_controls[] = {
	...
	SOC_ENUM_EXT("MTKAIF_DMIC", mt6858_adda_enum[0],
		     mt6858_adda_dmic_get, mt6858_adda_dmic_set),
	SOC_ENUM_EXT("MTKAIF_ADDA6_ONLY", mt6858_adda_enum[0],
		     mt6858_adda6_only_get, mt6858_adda6_only_set),
	SOC_SINGLE_EXT("ADDA_DL_MAX_VOL",
		       SND_SOC_NOPM, 0, 0x1, 0,
		       mt6858_adda_dl_max_vol_get,
		       mt6858_adda_dl_max_vol_set),
	SOC_SINGLE_EXT("VOW_ENABLE",
		       SND_SOC_NOPM, 0, 0x1, 0,
		       mt6858_vow_enable_get,
		       mt6858_vow_enable_set),
	SOC_ENUM_EXT("AP DMIC Used", mt6858_adda_enum[0],
		     mt6858_adda_ap_dmic_get, mt6858_adda_ap_dmic_set),
};
...
static const struct snd_soc_dapm_widget mtk_dai_adda_widgets[] = {
...
#if !defined(IS_FPGA_EARLY_PORTING)
	/* clock */
	SND_SOC_DAPM_CLOCK_SUPPLY("clk_top_audio_h_sel"),

	SND_SOC_DAPM_CLOCK_SUPPLY("afe_dl0_dac_audio"),
	...
	SND_SOC_DAPM_CLOCK_SUPPLY("afe_ul1_adc_hires_audio"),
#endif
};
...
static const struct snd_soc_dapm_route mtk_dai_adda_routes[] = {
	...
	/* clk */
	{"ADDA Playback", NULL, "afe_dl0_predis_audio"},
	{"ADDA Playback", NULL, "afe_dl0_dac_audio"},
	{"ADDA Playback", NULL, "afe_dl0_dac_hires_audio",
	...
	{"clk_top_audio_h_sel", NULL, APLL2_W_NAME},

	{"afe_dl0_dac_hires_audio", NULL, "clk_top_audio_h_sel"},
...
int mt6858_dai_adda_register(struct mtk_base_afe *afe)
{
	...
	/* set dai priv */
	ret = mt6858_dai_set_priv(afe, MT6858_DAI_ADDA,
				  sizeof(struct mtk_afe_adda_priv), NULL);
	if (ret)
		return ret;

	ret = mt6858_dai_set_priv(afe, MT6858_DAI_ADDA_CH34,
				  sizeof(struct mtk_afe_adda_priv), NULL);
	if (ret)
		return ret;
	/* get ap mic type */
	...
	afe_priv->dai_priv[MT6858_DAI_AP_DMIC] =
		afe_priv->dai_priv[MT6858_DAI_ADDA];
	afe_priv->dai_priv[MT6858_DAI_AP_DMIC_CH34] =
		afe_priv->dai_priv[MT6858_DAI_ADDA_CH34];
