...
#include "mt6858-afe-clk.h"
#include "mt6858-afe-common.h"
#include "mt6858-afe-gpio.h"
#include "mt6858-interconnection.h"
...
const struct mtk_base_etdm_data mtk_etdm_data[DAI_I2S_NUM] = {
	...
	[DAI_I2SIN4] = {
		...
		/* Shawn: (Todo) eTDM pad top setting changed
			 ...
			 struct snd_ctl_elem_value *ucontrol)
{
	...
	struct mtk_afe_i2s_priv *i2sin4_priv = afe_priv->dai_priv[MT6858_DAI_I2S_IN4];
			 ...
			 struct snd_ctl_elem_value *ucontrol)
{
	...
	struct mt6858_afe_private *afe_priv = afe->platform_priv;
	struct mtk_afe_i2s_priv *i2sin4_priv = afe_priv->dai_priv[MT6858_DAI_I2S_IN4];
	struct mtk_afe_i2s_priv *i2sout4_priv = afe_priv->dai_priv[MT6858_DAI_I2S_OUT4];
	unsigned int value = 0;
			      ...
			      const char *name)
{
	if (strncmp(name, "I2SIN1", 6) == 0)
		return MT6858_DAI_I2S_IN1;
	else if (strncmp(name, "I2SIN2", 6) == 0)
		return MT6858_DAI_I2S_IN2;
	else if (strncmp(name, "I2SIN4", 6) == 0)
		return MT6858_DAI_I2S_IN4;
	else if (strncmp(name, "I2SOUT1", 7) == 0)
		return MT6858_DAI_I2S_OUT1;
...
static int mtk_is_i2s_low_power(int i2s_num)
{
	...
	i2s_bit_shift = i2s_num - MT6858_DAI_I2S_IN1;
	if (i2s_bit_shift < 0 || i2s_bit_shift > MT6858_DAI_I2S_MAX_NUM) {
...
/* low jitter control */
static const char *const mt6858_i2s_hd_str[] = {
	"Normal", "Low_Jitter"
};

static const struct soc_enum mt6858_i2s_enum[] = {
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(mt6858_i2s_hd_str),
			    mt6858_i2s_hd_str),
};

static int mt6858_i2s_hd_get(struct snd_kcontrol *kcontrol,
			     struct snd_ctl_elem_value *ucontrol)
{
...
static int mt6858_i2s_hd_set(struct snd_kcontrol *kcontrol,
			     struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static const struct snd_kcontrol_new mtk_dai_i2s_controls[] = {
	SOC_ENUM_EXT(MTK_AFE_I2SIN1_KCONTROL_NAME, mt6858_i2s_enum[0],
		     mt6858_i2s_hd_get, mt6858_i2s_hd_set),
	SOC_ENUM_EXT(MTK_AFE_I2SIN2_KCONTROL_NAME, mt6858_i2s_enum[0],
		     mt6858_i2s_hd_get, mt6858_i2s_hd_set),
	SOC_ENUM_EXT(MTK_AFE_I2SIN4_KCONTROL_NAME, mt6858_i2s_enum[0],
		     mt6858_i2s_hd_get, mt6858_i2s_hd_set),
	SOC_ENUM_EXT(MTK_AFE_I2SOUT1_KCONTROL_NAME, mt6858_i2s_enum[0],
		     mt6858_i2s_hd_get, mt6858_i2s_hd_set),
	SOC_ENUM_EXT(MTK_AFE_I2SOUT2_KCONTROL_NAME, mt6858_i2s_enum[0],
		     mt6858_i2s_hd_get, mt6858_i2s_hd_set),
			    ...
			    int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		mt6858_afe_gpio_request(afe, true, i2s_priv->id, 0);
		break;
	case SND_SOC_DAPM_POST_PMD:
		mt6858_afe_gpio_request(afe, false, i2s_priv->id, 0);
		break;
	default:
		break;
	}
			  ...
			  int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		if (strcmp(w->name, APLL1_W_NAME) == 0)
			mt6858_apll1_enable(afe);
		else
			mt6858_apll2_enable(afe);
		break;
	case SND_SOC_DAPM_POST_PMD:
		if (strcmp(w->name, APLL1_W_NAME) == 0)
			mt6858_apll1_disable(afe);
		else
			mt6858_apll2_disable(afe);
		break;
	default:
		break;
	}
			     ...
			     int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		mt6858_mck_enable(afe, i2s_priv->mclk_id, i2s_priv->mclk_rate);
		break;
	case SND_SOC_DAPM_POST_PMD:
		i2s_priv->mclk_rate = 0;
		mt6858_mck_disable(afe, i2s_priv->mclk_id, i2s_priv->mclk_rate);
		break;
	default:
		break;
	}
				    ...
				    struct snd_soc_dapm_widget *sink)
{
	...
	i2s_need_apll = mt6858_get_apll_by_rate(afe, i2s_priv->rate);
		...
		struct snd_soc_dai *dai)
{
	...
	unsigned int rate_reg = mt6858_rate_transform(afe->dev,
				rate, dai->id);
				  ...
				  int clk_id, unsigned int freq, int dir)
{
	...
	apll = mt6858_get_apll_by_rate(afe, freq);
	apll_rate = mt6858_get_apll_rate(afe, apll);
...
static const struct mtk_afe_i2s_priv mt6858_i2s_priv[DAI_I2S_NUM] = {
	[DAI_I2SIN1] = {
		.id = MT6858_DAI_I2S_IN1,
		.mclk_id = MT6858_I2SIN1_MCK,
		.share_property_name = "i2sin1-share",
		.share_i2s_id = -1,
	},
	[DAI_I2SIN2] = {
		.id = MT6858_DAI_I2S_IN2,
		.mclk_id = MT6858_I2SIN2_MCK,
	...
	[DAI_I2SOUT1] = {
		...
		.share_i2s_id = MT6858_DAI_I2S_IN1,
	},
	[DAI_I2SOUT2] = {
		.id = MT6858_DAI_I2S_OUT2,
		.mclk_id = MT6858_I2SIN2_MCK,
		.share_property_name = "i2sout2-share",
		.share_i2s_id = MT6858_DAI_I2S_IN2,
	},
	[DAI_I2SOUT4] = {
		.id = MT6858_DAI_I2S_OUT4,
		.mclk_id = MT6858_I2SIN1_MCK,
		.share_property_name = "i2sout4-share",
		.share_i2s_id = MT6858_DAI_I2S_IN4,
	},
	/* [DAI_FMI2S_MASTER] = {
	 *	.id = MT6858_DAI_FM_I2S_MASTER,
	 *	.mclk_id = MT6858_FMI2S_MCK,
...
#define I2S_IN_NUM (DAI_I2SIN_NUM)
#define I2S_OUT_NUM (DAI_I2SOUT_NUM - DAI_I2SIN_NUM)
...
static int etdm_parse_dt(struct mtk_base_afe *afe)
{
	...
	for (i = 0; i < I2S_OUT_NUM; i++) {
		i2s_priv = afe_priv->dai_priv[MT6858_DAI_I2S_OUT1 + i];
	...
	for (i = 0; i < I2S_IN_NUM; i++) {
		i2s_priv = afe_priv->dai_priv[MT6858_DAI_I2S_IN1 + i];
...
int mt6858_dai_i2s_get_share(struct mtk_base_afe *afe)
{
	struct mt6858_afe_private *afe_priv = afe->platform_priv;
	...
	for (i = 0; i < DAI_I2S_NUM; i++) {
		i2s_priv = afe_priv->dai_priv[mt6858_i2s_priv[i].id];
		property_name = mt6858_i2s_priv[i].share_property_name;
...
int mt6858_dai_i2s_set_priv(struct mtk_base_afe *afe)
{
	int i;
	int ret;

	for (i = 0; i < DAI_I2S_NUM; i++) {
		ret = mt6858_dai_set_priv(afe, mt6858_i2s_priv[i].id,
...
static int mt6858_dai_i2s_config(struct mtk_base_afe *afe, int i2s_id,
				 bool enable)
{
	struct mt6858_afe_private *afe_priv = afe->platform_priv;
	...
	/* set share i2s */
	if (i2s_priv && i2s_priv->share_i2s_id >= 0)
		ret = mt6858_dai_i2s_config(afe, i2s_priv->share_i2s_id, true);


	return ret;
}

static int mt6858_dai_clk_always_on(struct mtk_base_afe *afe, int dai_id,
				  bool enable)
{
	dev_info(afe->dev, "%s(), id %d, enable %d\n", __func__, dai_id, enable);

	if (dai_id >= MT6858_DAI_I2S_IN1 && dai_id <= MT6858_DAI_I2S_OUT4)
		mt6858_dai_i2s_config(afe, dai_id, enable);

	return 0;
}

int mt6858_afe_set_clk_always_on(struct mtk_base_afe *afe,
					bool enable)
{
	struct mt6858_afe_private *afe_priv = afe->platform_priv;
	...
	for (id = MT6858_DAI_I2S_IN1; id <= MT6858_DAI_I2S_OUT4; id++) {
		...
		if (dai_attr->bclk_ao || dai_attr->lrck_ao)
			mt6858_dai_clk_always_on(afe, id, enable);
...
EXPORT_SYMBOL_GPL(mt6858_afe_set_clk_always_on);

int mt6858_dai_i2s_register(struct mtk_base_afe *afe)
{
	...
	ret = mt6858_dai_i2s_set_priv(afe);
	if (ret)
		return ret;

	/* parse share i2s */
	ret = mt6858_dai_i2s_get_share(afe);
	if (ret)
		return ret;
