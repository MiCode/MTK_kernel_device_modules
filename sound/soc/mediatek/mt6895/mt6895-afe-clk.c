				    ...
				    int clk_id)
{
	...
	if (clk_id < 0) {
		dev_info(afe->dev, "%s() set_audio_int_bus_parent fail %d\n",
			__func__, clk_id);
		return -EINVAL;
	}
...
int mt6895_afe_enable_clock(struct mtk_base_afe *afe)
{
	...
	dev_info(afe->dev, "%s() start successfully\n", __func__);
...
int mt6895_mck_enable(struct mtk_base_afe *afe, int mck_id, int rate)
{
	...
	if (mck_id < 0) {
		dev_info(afe->dev, "%s() mck_enable fail invalid mck_id %d\n",
			__func__, mck_id);
		return -EINVAL;
	}
	...
	if (div_clk_id <= 0) {
		dev_info(afe->dev, "%s() mck_enable fail invalid div_clk_id %d\n",
			__func__, div_clk_id);
		return -EINVAL;
	}
	/* enable div, set rate */
	...
	if (div_clk_id <= 0) {
		dev_info(afe->dev, "%s() mck_enable fail invalid div_msb_clk_id %d\n",
			__func__, div_clk_id);
		return -EINVAL;
	}
...
void mt6895_mck_disable(struct mtk_base_afe *afe, int mck_id)
{
	...
	if (mck_id < 0) {
		dev_info(afe->dev, "%s() mck_disable fail invalid mck_id %d\n",
			__func__, mck_id);
		return;
	}
	m_sel_id = mck_div[mck_id].m_sel_id;
