			    ...
			    int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		if (id == MT6991_DAI_SRC_0)
			mtk_set_src_0_param(afe, id);
		else if (id == MT6991_DAI_SRC_1)
			mtk_set_src_1_param(afe, id);
		else if (id == MT6991_DAI_SRC_2)
				  ...
				  struct snd_soc_dapm_widget *sink)
{
	...
	if (strcmp(w->name, HW_SRC_0_EN_W_NAME) == 0)
		src_priv = afe_priv->dai_priv[MT6991_DAI_SRC_0];
	else if (strcmp(w->name, HW_SRC_1_EN_W_NAME) == 0)
		src_priv = afe_priv->dai_priv[MT6991_DAI_SRC_1];
	else if (strcmp(w->name, HW_SRC_2_EN_W_NAME) == 0)
		src_priv = afe_priv->dai_priv[MT6991_DAI_SRC_2];
	else
		src_priv = afe_priv->dai_priv[MT6991_DAI_SRC_3];
				     ...
				     struct snd_soc_dapm_widget *sink)
{
	...
	need_apll = mt6991_get_apll_by_rate(afe, src_priv->ul_rate);
...
int mt6991_dai_src_register(struct mtk_base_afe *afe)
{
	...
	/* set dai priv */
	ret = mt6991_dai_set_priv(afe, MT6991_DAI_SRC_0,
				  sizeof(struct mtk_afe_src_priv), NULL);
	if (ret)
		return ret;

	ret = mt6991_dai_set_priv(afe, MT6991_DAI_SRC_1,
				  sizeof(struct mtk_afe_src_priv), NULL);
	if (ret)
		return ret;

	ret = mt6991_dai_set_priv(afe, MT6991_DAI_SRC_2,
				  sizeof(struct mtk_afe_src_priv), NULL);
	if (ret)
		return ret;

	ret = mt6991_dai_set_priv(afe, MT6991_DAI_SRC_3,
				  sizeof(struct mtk_afe_src_priv), NULL);
	if (ret)
		return ret;
