...
static int mt6991_mt6681_dev_probe(struct platform_device *pdev)
{
	...
	if (get_smartpa_type() == SMARTPA_FS19XX) {
		...
		dev_info(&pdev->dev, "%s(), spk_node is speaker-codec-fs\n",__func__);
	} else {
		spk_node = of_get_child_by_name(pdev->dev.of_node,
			"mediatek,speaker-codec");
		dev_info(&pdev->dev, "%s(), spk_node is speaker-codec\n",__func__);
	}
	...
	if (ret) {
		dev_info(&pdev->dev, "%s [TF-NOSOUND] snd_soc_register_card fail %d\n",
			__func__, ret);
	} else
