		      ...
		      struct snd_soc_dai *dai)
{
	...
	case SNDRV_PCM_TRIGGER_RESUME:
		...
		if (id == MT6991_MEMIF_DL23 || id == MT6991_MEMIF_DL_24CH) {
			mtk_memif_set_pbuf_size(afe, id, MT6991_MEMIF_PBUF_SIZE_64_BYTES);
			mtk_memif_set_min_max_len(afe, id, MT6991_MEMIF_MIN_LEN_16_BYTES, MT6991_MEMIF_MAX_LEN_64_BYTES);
		} else {
			mtk_memif_set_pbuf_size(afe, id, MT6991_MEMIF_PBUF_SIZE_256_BYTES);
			mtk_memif_set_min_max_len(afe, id, MT6991_MEMIF_MIN_LEN_64_BYTES, MT6991_MEMIF_MAX_LEN_64_BYTES);
		}
                        ...
                        struct snd_ctl_elem_value *ucontrol)
{
        ...
        pr_info("enable: %d", enable);
        if (enable) {
                mt6991_afe_gpio_request(afe, true, MT6991_GPIO_BACK_DMIC_POWER, 0);
        } else {
