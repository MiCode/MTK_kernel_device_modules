...
EXPORT_SYMBOL_GPL(mt6993_set_cm_rate);
...
EXPORT_SYMBOL_GPL(mt6993_get_cm_mux);

static int mt6993_convert_cm_ch(unsigned int ch)
{
	return ch - 1;
}
...
int mt6993_set_cm(struct mtk_base_afe *afe, int id,
	       unsigned int update, bool swap, unsigned int ch)
{
	...
	samplerate = mt6993_general_rate_transform_inverse(afe->dev, rate);
...
EXPORT_SYMBOL_GPL(mt6993_set_cm);
...
EXPORT_SYMBOL_GPL(mt6993_enable_cm_bypass);
...
EXPORT_SYMBOL_GPL(mt6993_enable_cm);
...
EXPORT_SYMBOL_GPL(mt6993_is_need_enable_cm);
