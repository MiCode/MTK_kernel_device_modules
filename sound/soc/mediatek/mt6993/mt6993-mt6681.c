...
/*
 *  mt6993-mt6681.c  --  mt6993 mt6681 ALSA SoC machine driver
 *
 ...
 * mt6993_mt6681_spk_amp_event()
 */
#define EXT_SPK_AMP_W_NAME "Ext_Speaker_Amp"
#define BYPASS_FOR_61_BRINGUP 1


static struct snd_soc_card mt6993_mt6681_soc_card;

struct mt6993_compress_info compr_info;
...
static const struct soc_enum mt6993_spk_type_enum[] = {
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(mt6993_spk_type_str),
			    mt6993_spk_type_str),
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(mt6993_spk_i2s_type_str),
			    mt6993_spk_i2s_type_str),
};

static int mt6993_spk_type_get(struct snd_kcontrol *kcontrol,
...
static int mt6993_spk_i2s_out_type_get(struct snd_kcontrol *kcontrol,
				       struct snd_ctl_elem_value *ucontrol)
{
...
static int mt6993_spk_i2s_in_type_get(struct snd_kcontrol *kcontrol,
				      struct snd_ctl_elem_value *ucontrol)
{
...
static int mt6993_ipm_info_get(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	unsigned int ipm_info = 0;
	int ret = 0;
	struct snd_soc_card *card = &mt6993_mt6681_soc_card;
...
static int mt6993_compress_info_set(struct snd_kcontrol *kcontrol,
				    const unsigned int __user *data,
				    unsigned int size)
{
	if (copy_from_user(&compr_info,
			   data,
			   sizeof(struct mt6993_compress_info))) {
...
static int mt6993_compress_info_get(struct snd_kcontrol *kcontrol,
				    unsigned int __user *data, unsigned int size)
{
	...
	if (copy_to_user(data, &compr_info, sizeof(struct mt6993_compress_info))) {
...
static int mt6993_mt6681_spk_amp_event(struct snd_soc_dapm_widget *w,
					...
					int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		/* spk amp on control */
#if IS_ENABLED(CONFIG_SND_SOC_MTK_AUTO_AUDIO)
		mt6993_afe_gpio_request(afe, true, MT6993_GPIO_EXT_HP_AMP, 0);
#endif
		break;
	case SND_SOC_DAPM_PRE_PMD:
		/* spk amp off control */
#if IS_ENABLED(CONFIG_SND_SOC_MTK_AUTO_AUDIO)
		mt6993_afe_gpio_request(afe, false, MT6993_GPIO_EXT_HP_AMP, 0);
#endif
		break;
	default:
		break;
...
static const struct snd_soc_dapm_widget mt6993_mt6681_widgets[] = {
	SND_SOC_DAPM_SPK(EXT_SPK_AMP_W_NAME, mt6993_mt6681_spk_amp_event),
};

static const struct snd_soc_dapm_route mt6993_mt6681_routes[] = {
...
static const struct snd_soc_dapm_route mt6993_mt6681_routes_dummy[] = {};

static const struct snd_kcontrol_new mt6993_mt6681_controls[] = {
	SOC_DAPM_PIN_SWITCH(EXT_SPK_AMP_W_NAME),
	SOC_ENUM_EXT("MTK_SPK_TYPE_GET", mt6993_spk_type_enum[0],
		     mt6993_spk_type_get, NULL),
	SOC_ENUM_EXT("MTK_SPK_I2S_OUT_TYPE_GET", mt6993_spk_type_enum[1],
		     mt6993_spk_i2s_out_type_get, NULL),
	SOC_ENUM_EXT("MTK_SPK_I2S_IN_TYPE_GET", mt6993_spk_type_enum[1],
		     mt6993_spk_i2s_in_type_get, NULL),
	SOC_SINGLE_EXT("MTK_IPM_INFO_GET", SND_SOC_NOPM, 0, 0x1, 0,
		       mt6993_ipm_info_get,
		       NULL),
	SND_SOC_BYTES_TLV("MTK_COMPRESS_INFO",
			  sizeof(struct mt6993_compress_info),
			  mt6993_compress_info_get, mt6993_compress_info_set),
};

/*
 * define mtk_spk_i2s_mck node in dts when need mclk,
 * BE i2s need assign snd_soc_ops = mt6993_mt6681_i2s_ops
 */
static int mt6993_mt6681_i2s_hw_params(struct snd_pcm_substream *substream,
...
static const struct snd_soc_ops mt6993_mt6681_i2s_ops = {
	.hw_params = mt6993_mt6681_i2s_hw_params,
};

static int mt6993_mt6681_mtkaif_calibration(struct snd_soc_pcm_runtime *rtd)
{
	...
	miso0_need_calib = mt6993_afe_gpio_is_prepared(MT6993_AFE_GPIO_DAT_MISO0_ON);
	miso1_need_calib = mt6993_afe_gpio_is_prepared(MT6993_AFE_GPIO_DAT_MISO1_ON);

	mt6993_afe_gpio_request(afe, true, MT6993_DAI_MTKAIF, 1);
	...
	if (!afe_priv->miso_only) {
		...
		mt6993_afe_gpio_request(afe, true, MT6993_DAI_MISO_ONLY, 0);
	}
	...
	mt6993_afe_gpio_request(afe, false, MT6993_DAI_MTKAIF, 1);
	if (afe_priv->miso_only)
		mt6993_afe_gpio_request(afe, false, MT6993_DAI_MISO_ONLY, 0);
...
static int mt6993_mt6681_init(struct snd_soc_pcm_runtime *rtd)
{
	struct snd_soc_component *component =
		snd_soc_rtdcom_lookup(rtd, AFE_PCM_NAME);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(component);
	...
	ops.enable_dc_compensation = mt6993_enable_dc_compensation;
	ops.set_lch_dc_compensation = mt6993_set_lch_dc_compensation;
	ops.set_rch_dc_compensation = mt6993_set_rch_dc_compensation;
	ops.adda_dl_gain_control = mt6993_adda_dl_gain_control;
	...
	register_get_power_scene_callback(mt6993_aud_get_power_scenario);

	/* mtkaif calibration */
	mt6993_mt6681_mtkaif_calibration(rtd);
...
#if IS_ENABLED(CONFIG_SND_SOC_MTK_AUTO_AUDIO)
	mt6993_afe_set_clk_always_on(afe, true);
#endif
	return 0;
}

static int mt6993_i2s_hw_params_fixup(struct snd_soc_pcm_runtime *rtd,
				      struct snd_pcm_hw_params *params)
{
...
static const struct snd_pcm_hardware mt6993_mt6681_vow_hardware = {
...
static int mt6993_mt6681_vow_startup(struct snd_pcm_substream *substream)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_component *component =
			snd_soc_rtdcom_lookup(rtd, AFE_PCM_NAME);
	...
	snd_soc_set_runtime_hwparams(substream, &mt6993_mt6681_vow_hardware);

	mt6993_afe_gpio_request(afe, true, MT6993_DAI_VOW, 0);
...
static void mt6993_mt6681_vow_shutdown(struct snd_pcm_substream *substream)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_component *component =
			snd_soc_rtdcom_lookup(rtd, AFE_PCM_NAME);
	...
	mt6993_afe_gpio_request(afe, false, MT6993_DAI_VOW, 0);
...
static const struct snd_soc_ops mt6993_mt6681_vow_ops = {
	.startup = mt6993_mt6681_vow_startup,
	.shutdown = mt6993_mt6681_vow_shutdown,
};
...
static struct snd_soc_dai_link mt6993_mt6681_dai_links[] = {
	/* Front End DAI links */
	{
		.name = "Playback_1",
	...
	},
	{
		...
		.ops = &mt6993_mt6681_i2s_ops,
		.no_pcm = 1,
		.dpcm_capture = 1,
		.ignore_suspend = 1,
		.ignore_pmdown_time = 1,
		.be_hw_params_fixup = mt6993_i2s_hw_params_fixup,
...
static struct snd_soc_card mt6993_mt6681_soc_card = {
	.name = "mt6993-mt6681",
	.owner = THIS_MODULE,
	.dai_link = mt6993_mt6681_dai_links,
	.num_links = ARRAY_SIZE(mt6993_mt6681_dai_links),

	.controls = mt6993_mt6681_controls,
	.num_controls = ARRAY_SIZE(mt6993_mt6681_controls),
	.dapm_widgets = mt6993_mt6681_widgets,
	.num_dapm_widgets = ARRAY_SIZE(mt6993_mt6681_widgets),
	.dapm_routes = mt6993_mt6681_routes,
	.num_dapm_routes = ARRAY_SIZE(mt6993_mt6681_routes),
};

static int mt6993_mt6681_bypass_primary_codec(struct platform_device *pdev)
{
	struct snd_soc_card *card = &mt6993_mt6681_soc_card;
	int i;
	struct snd_soc_dai_link *dai_link;
	...
	card->dapm_routes = mt6993_mt6681_routes_dummy;
	card->num_dapm_routes = ARRAY_SIZE(mt6993_mt6681_routes_dummy);
	return 0;
}

static int mt6993_mt6681_dev_probe(struct platform_device *pdev)
{
	struct snd_soc_card *card = &mt6993_mt6681_soc_card;
	...
	if (!mt6681_probe_done) {
		...
		mt6993_mt6681_bypass_primary_codec(pdev);
	}
...
static const struct of_device_id mt6993_mt6681_dt_match[] = {
	{.compatible = "mediatek,mt6993-mt6681-sound",},
	{}
};
#endif

static const struct dev_pm_ops mt6993_mt6681_pm_ops = {
	.poweroff = snd_soc_poweroff,
	.restore = snd_soc_resume,
};

static struct platform_driver mt6993_mt6681_driver = {
	.driver = {
		.name = "mt6993-mt6681",
#if IS_ENABLED(CONFIG_OF)
		.of_match_table = mt6993_mt6681_dt_match,
#endif
		.pm = &mt6993_mt6681_pm_ops,
	},
	.probe = mt6993_mt6681_dev_probe,
};

module_platform_driver(mt6993_mt6681_driver);

/* Module information */
MODULE_DESCRIPTION("MT6993 mt6681 ALSA SoC machine driver");
MODULE_AUTHOR("Shane Chien <shane.chien@mediatek.com>");
MODULE_LICENSE("GPL");
MODULE_ALIAS("mt6993 mt6681 soc card");
