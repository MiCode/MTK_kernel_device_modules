...
static ssize_t mt6993_debug_read_reg(char *buffer, int size, struct mtk_base_afe *afe);

static const struct snd_pcm_hardware mt6993_afe_hardware = {
...
static int mt6993_fe_startup(struct snd_pcm_substream *substream,
...
void mt6993_fe_shutdown(struct snd_pcm_substream *substream,
			struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct mtk_base_afe *afe = snd_soc_dai_get_drvdata(dai);
...
int mt6993_fe_trigger(struct snd_pcm_substream *substream, int cmd,
		      struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	...
	if (!in_interrupt()) {
		...
		mt6993_aud_update_power_scenario();
	}
	...
	case SNDRV_PCM_TRIGGER_RESUME:
		...
		if (is_afe_need_triggered(memif)) {
			...
			if (!strcmp(memif->data->name, "VUL8") || !strcmp(memif->data->name, "VUL_CM0")) {
				if (mt6993_is_need_enable_cm(afe, CM0))
					mt6993_enable_cm(afe, CM0, 1);
			}
			if (!strcmp(memif->data->name, "VUL9") || !strcmp(memif->data->name, "VUL_CM1")) {
				if (mt6993_is_need_enable_cm(afe, CM1))
					mt6993_enable_cm(afe, CM1, 1);
			}
			if (!strcmp(memif->data->name, "VUL10") || !strcmp(memif->data->name, "VUL_CM2")) {
				if (mt6993_is_need_enable_cm(afe, CM2))
					mt6993_enable_cm(afe, CM2, 1);
			}
		}
	...
	case SNDRV_PCM_TRIGGER_SUSPEND:
		...
		if (is_afe_need_triggered(memif)) {
			...
			if (!strcmp(memif->data->name, "VUL8") || !strcmp(memif->data->name, "VUL_CM0"))
				mt6993_enable_cm(afe, CM0, 0);
			if (!strcmp(memif->data->name, "VUL9") || !strcmp(memif->data->name, "VUL_CM1"))
				mt6993_enable_cm(afe, CM1, 0);
			if (!strcmp(memif->data->name, "VUL10") || !strcmp(memif->data->name, "VUL_CM2"))
				mt6993_enable_cm(afe, CM2, 0);
		}
...
static int mt6993_memif_fs(struct snd_pcm_substream *substream,
			   unsigned int rate)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	...
	rate_reg = mt6993_rate_transform(afe->dev, rate, id);

	switch (id) {
	case MT6993_MEMIF_VUL8:
	case MT6993_MEMIF_VUL_CM0:
		cm = 0x0;
		break;
	case MT6993_MEMIF_VUL9:
	case MT6993_MEMIF_VUL_CM1:
		cm = 0x1;
		break;
	case MT6993_MEMIF_VUL10:
	case MT6993_MEMIF_VUL_CM2:
		cm = 0x2;
		break;
	default:
		cm = 0x0;
	...
	mt6993_set_cm_rate(cm, rate_reg);

	return rate_reg;
}

static int mt6993_get_dai_fs(struct mtk_base_afe *afe,
			     int dai_id, unsigned int rate)
{
	return mt6993_rate_transform(afe->dev, rate, dai_id);
}

static int mt6993_irq_fs(struct snd_pcm_substream *substream, unsigned int rate)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_component *component =
		snd_soc_rtdcom_lookup(rtd, AFE_PCM_NAME);
	...
	if (!component)
		return -EINVAL;
	afe = snd_soc_component_get_drvdata(component);
	return mt6993_general_rate_transform(afe->dev, rate);
}

int mt6993_get_memif_pbuf_size(struct snd_pcm_substream *substream)
{
	struct snd_pcm_runtime *runtime = substream->runtime;

	if ((runtime->period_size * 1000) / runtime->rate > 10)
		return MT6993_MEMIF_PBUF_SIZE_256_BYTES;
	else
		return MT6993_MEMIF_PBUF_SIZE_32_BYTES;
}

/* FE DAIs */
static const struct snd_soc_dai_ops mt6993_memif_dai_ops = {
	.startup        = mt6993_fe_startup,
	.shutdown       = mt6993_fe_shutdown,
	.hw_params      = mtk_afe_fe_hw_params,
	.hw_free        = mtk_afe_fe_hw_free,
	.prepare        = mtk_afe_fe_prepare,
	.trigger        = mt6993_fe_trigger,
};
...
static struct snd_soc_dai_driver mt6993_memif_dai_driver[] = {
...
/* kcontrol */
static int mt6993_irq_cnt1_get(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	...
	ucontrol->value.integer.value[0] =
		afe_priv->irq_cnt[MT6993_PRIMARY_MEMIF];
	return 0;
}

static int mt6993_irq_cnt1_set(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6993_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6993_PRIMARY_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
	int irq_id = memif->irq_usage;
...
static int mt6993_irq_cnt2_get(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	...
	ucontrol->value.integer.value[0] =
		afe_priv->irq_cnt[MT6993_RECORD_MEMIF];
	return 0;
}

static int mt6993_irq_cnt2_set(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6993_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6993_RECORD_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
	int irq_id = memif->irq_usage;
	int irq_cnt = afe_priv->irq_cnt[memif_num];
...
static int mt6993_deep_irq_cnt_get(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	...
	ucontrol->value.integer.value[0] = afe_priv->irq_cnt[MT6993_DEEP_MEMIF];
	return 0;
}

static int mt6993_deep_irq_cnt_set(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6993_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6993_DEEP_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
	int irq_id = memif->irq_usage;
	int irq_cnt = afe_priv->irq_cnt[memif_num];
...
static int mt6993_voip_rx_irq_cnt_get(struct snd_kcontrol *kcontrol,
				      struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	...
	ucontrol->value.integer.value[0] = afe_priv->irq_cnt[MT6993_VOIP_MEMIF];
	return 0;
}

static int mt6993_voip_rx_irq_cnt_set(struct snd_kcontrol *kcontrol,
				      struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6993_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6993_VOIP_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
	int irq_id = memif->irq_usage;
	int irq_cnt = afe_priv->irq_cnt[memif_num];
...
static int mt6993_deep_scene_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6993_deep_scene_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6993_fast_scene_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6993_fast_scene_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6993_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6993_FAST_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
...
static int mt6993_primary_scene_get(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6993_primary_scene_set(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6993_voip_scene_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6993_voip_scene_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6993_record_xrun_assert_get(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6993_afe_private *afe_priv = afe->platform_priv;
	int xrun_assert = afe_priv->xrun_assert[MT6993_RECORD_MEMIF];

	ucontrol->value.integer.value[0] = xrun_assert;
	return 0;
}

static int mt6993_record_xrun_assert_set(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	...
	afe_priv->xrun_assert[MT6993_RECORD_MEMIF] = xrun_assert;
	return 0;
}

static int mt6993_echo_ref_xrun_assert_get(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6993_afe_private *afe_priv = afe->platform_priv;
	int xrun_assert = afe_priv->xrun_assert[MT6993_ECHO_REF_MEMIF];

	ucontrol->value.integer.value[0] = xrun_assert;
	return 0;
}

static int mt6993_echo_ref_xrun_assert_set(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	...
	dev_info(afe->dev, "%s(), xrun_assert %d\n", __func__, xrun_assert);
	afe_priv->xrun_assert[MT6993_ECHO_REF_MEMIF] = xrun_assert;
	return 0;
}

#if !IS_ENABLED(CONFIG_NEBULA_SND_PASSTHROUGH)
static int mt6993_sram_size_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6993_vow_barge_in_irq_id_get(struct snd_kcontrol *kcontrol,
					  struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	int memif_num = MT6993_BARGE_IN_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
	int irq_id = memif->irq_usage;
...
static int mt6993_adsp_ref_mem_get(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6993_adsp_ref_mem_set(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6993_adsp_mem_get(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6993_adsp_mem_set(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
#if !IS_ENABLED(CONFIG_NEBULA_SND_PASSTHROUGH)
static int mt6993_mmap_dl_scene_get(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6993_mmap_dl_scene_set(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6993_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6993_MMAP_DL_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
...
static int mt6993_mmap_ul_scene_get(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6993_mmap_ul_scene_set(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	struct mt6993_afe_private *afe_priv = afe->platform_priv;
	int memif_num = MT6993_MMAP_UL_MEMIF;
	struct mtk_base_afe_memif *memif = &afe->memif[memif_num];
...
static int mt6993_mmap_ion_get(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
...
static int mt6993_mmap_ion_set(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6993_dl_mmap_fd_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6993_dl_mmap_fd_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}

static int mt6993_ul_mmap_fd_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6993_ul_mmap_fd_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}
...
#if IS_ENABLED(CONFIG_NEBULA_SND_PASSTHROUGH)
static int mt6993_set_memif_sram_mode(struct device *dev,
				      enum mtk_audio_sram_mode sram_mode);
static int mt6993_set_sram_mode(struct device *dev,
				enum mtk_audio_sram_mode sram_mode);

static int mt6993_use_dram_only_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	...
	for (i = 0; i < MT6993_MEMIF_NUM; i++)
		ucontrol->value.bytes.data[i] = afe->memif[i].use_dram_only;

	return 0;
}

static int mt6993_sram_mode_set(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	...
	rc = mt6993_set_sram_mode(afe->dev, ucontrol->value.bytes.data[0]);
...
// according to mt6993_mt6681_dai_links
// c0d15c -> capture 9 -> UL9 -> memif id 24
unsigned int virtio_id_memif_index_mapping[MT6993_MEMIF_NUM] = {
	MT6993_MEMIF_DL0,
	MT6993_MEMIF_DL1,
	MT6993_MEMIF_DL2,
	MT6993_MEMIF_DL3,
	...
	MT6993_MEMIF_VUL2,
	MT6993_MEMIF_VUL5,
	MT6993_MEMIF_VUL_CM0,
	MT6993_MEMIF_VUL_CM1,
	MT6993_MEMIF_VUL_CM2,
	MT6993_MEMIF_VUL10,
	MT6993_MEMIF_VUL6,
	MT6993_MEMIF_VUL25,
	MT6993_MEMIF_VUL26,
	MT6993_MEMIF_VUL8,
	MT6993_MEMIF_VUL24,
	MT6993_MEMIF_ETDM_IN0,
	MT6993_MEMIF_ETDM_IN1,
	MT6993_MEMIF_ETDM_IN2,
	MT6993_MEMIF_ETDM_IN3,
	MT6993_MEMIF_ETDM_IN4,
	MT6993_MEMIF_ETDM_IN6,
	MT6993_MEMIF_HDMI
};

static int mt6993_passthrough_shm_set(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
#ifndef SKIP_VIP
// ALPS08709395: AudioQOS, set VIP
static int mt6993_audio_vip_get(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = 0;
	return 0;
...
static int mt6993_audio_vip_set(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6993_swcodec_vip_get(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = 0;
	return 0;
...
static int mt6993_swcodec_vip_set(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
...
#endif
static int mt6993_mmap_clean_get(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = 0;
	return 0;
...
static int mt6993_mmap_clean_set(struct snd_kcontrol *kcontrol,
			       struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static const struct soc_enum mt6993_pcm_type_enum[] = {
...
static const struct snd_kcontrol_new mt6993_pcm_kcontrols[] = {
	...
	SOC_SINGLE_EXT("adsp_ref_sharemem_scenario",
		       SND_SOC_NOPM, 0, 0x1, 0,
		       mt6993_adsp_ref_mem_get,
		       mt6993_adsp_ref_mem_set),
	...
	SOC_SINGLE_EXT("mmap_play_scenario", SND_SOC_NOPM, 0, 0x1, 0,
		       mt6993_mmap_dl_scene_get, mt6993_mmap_dl_scene_set),
	SOC_SINGLE_EXT("mmap_record_scenario", SND_SOC_NOPM, 0, 0x1, 0,
		       mt6993_mmap_ul_scene_get, mt6993_mmap_ul_scene_set),
	...
	SOC_SINGLE_EXT("aaudio_dl_mmap_fd",
		       SND_SOC_NOPM, 0, 0xffffffff, 0,
		       mt6993_dl_mmap_fd_get,
		       mt6993_dl_mmap_fd_set),
	SOC_SINGLE_EXT("aaudio_ul_mmap_fd",
		       SND_SOC_NOPM, 0, 0xffffffff, 0,
		       mt6993_ul_mmap_fd_get,
		       mt6993_ul_mmap_fd_set),
#endif
	SOC_ENUM_EXT("MTK_RECORD_MISO1", mt6993_pcm_type_enum[0],
	...
	SOC_SINGLE_EXT("aaudio_clean_buffer",
		       SND_SOC_NOPM, 0, 0xffffffff, 0,
		       mt6993_mmap_clean_get,
		       mt6993_mmap_clean_set),
#if IS_ENABLED(CONFIG_NEBULA_SND_PASSTHROUGH)
	SND_SOC_BYTES_EXT("use_dram_only", MT6993_MEMIF_NUM,
		      mt6993_use_dram_only_get, NULL),
	SND_SOC_BYTES_EXT("sram_mode", 1,
		      NULL, mt6993_sram_mode_set),
			...
			int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		mt6993_enable_cm_bypass(afe, CM0, 0x0);
		mt6993_set_cm(afe, CM0, 0x1, false, channels);
		break;
	case SND_SOC_DAPM_PRE_PMD:
		mt6993_enable_cm_bypass(afe, CM0, 0x1);
		break;
	default:
		break;
	}
			...
			int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		channels = mtk_get_channel_value();
		mt6993_enable_cm_bypass(afe, CM1, 0x0);
		mt6993_set_cm(afe, CM1, 0x1, false, channels);
		break;
	case SND_SOC_DAPM_PRE_PMD:
		/* remove so we fix in normal mode */
#if IS_ENABLED(CONFIG_SND_SOC_MTK_AUTO_AUDIO)
		mt6993_enable_cm_bypass(afe, CM1, 0x1);
#endif
		break;
	default:
		break;
			...
			int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		channels = mtk_get_channel_value();
		mt6993_enable_cm_bypass(afe, CM2, 0x0);
		mt6993_set_cm(afe, CM2, 0x1, false, channels);
		break;
	case SND_SOC_DAPM_PRE_PMD:
		/* remove so we fix in normal mode */
#if IS_ENABLED(CONFIG_SND_SOC_MTK_AUTO_AUDIO)
		mt6993_enable_cm_bypass(afe, CM2, 0x1);
#endif
		break;
	default:
		break;
		      ...
		      struct snd_ctl_elem_value *ucontrol)
{
	...
	if (!strcmp(widget->name, "CM2_UL_MUX")) {
		ucontrol->value.integer.value[0] = mt6993_get_cm_mux(CM2);
	} else if (!strcmp(widget->name, "CM1_UL_MUX")) {
		ucontrol->value.integer.value[0] = mt6993_get_cm_mux(CM1);
	} else if (!strcmp(widget->name, "CM0_UL_MUX")) {
		ucontrol->value.integer.value[0] = mt6993_get_cm_mux(CM0);
	} else {
		      ...
		      struct snd_ctl_elem_value *ucontrol)
{
	...
	if (!strcmp(widget->name, "CM2_UL_MUX"))
		...
		mt6993_set_cm_mux(CM1, ucontrol->value.integer.value[0]);
	else if (!strcmp(widget->name, "CM0_UL_MUX"))
		mt6993_set_cm_mux(CM0, ucontrol->value.integer.value[0]);
	else
		dev_info(afe->dev, "%s(), No match: %s\n", __func__, widget->name);
...
static const struct snd_soc_dapm_widget mt6993_memif_widgets[] = {
	/* inter-connections */
...
static const struct snd_soc_dapm_route mt6993_memif_routes[] = {
	{"UL0", NULL, "UL0_CH1"},
	{"UL0", NULL, "UL0_CH2"},
	/* Normal record */
...
static const struct mtk_base_memif_data memif_data[MT6993_MEMIF_NUM] = {
	[MT6993_MEMIF_DL0] = {
		.name = "DL0",
		.id = MT6993_MEMIF_DL0,
...
static const struct mtk_base_irq_data irq_data[MT6993_IRQ_NUM] = {
	[MT6993_IRQ_0] = {
		.id = MT6993_IRQ_0,
...
#if IS_ENABLED(CONFIG_SND_SOC_MTK_AUTO_AUDIO)
static const int memif_irq_usage[MT6993_MEMIF_NUM] = {
	/* TODO: verify each memif & irq */
	[MT6993_MEMIF_DL0] = MT6993_IRQ_4,
	[MT6993_MEMIF_DL1] = MT6993_IRQ_1,
	[MT6993_MEMIF_DL2] = MT6993_IRQ_2,
	[MT6993_MEMIF_DL3] = MT6993_IRQ_3,
	[MT6993_MEMIF_DL4] = MT6993_IRQ_3,
	[MT6993_MEMIF_DL5] = MT6993_IRQ_3,
	[MT6993_MEMIF_DL6] = MT6993_IRQ_3,
	[MT6993_MEMIF_DL7] = MT6993_IRQ_7,
	[MT6993_MEMIF_DL8] = MT6993_IRQ_8,
	[MT6993_MEMIF_DL23] = MT6993_IRQ_9,
	[MT6993_MEMIF_DL24] = MT6993_IRQ_10,
	[MT6993_MEMIF_DL25] = MT6993_IRQ_11,
	[MT6993_MEMIF_DL26] = MT6993_IRQ_0,
	[MT6993_MEMIF_DL_4CH] = MT6993_IRQ_5,
	[MT6993_MEMIF_DL_24CH] = MT6993_IRQ_12,
	[MT6993_MEMIF_VUL0] = MT6993_IRQ_13,
	[MT6993_MEMIF_VUL1] = MT6993_IRQ_14,
	[MT6993_MEMIF_VUL2] = MT6993_IRQ_15,
	[MT6993_MEMIF_VUL3] = MT6993_IRQ_16,
	[MT6993_MEMIF_VUL4] = MT6993_IRQ_17,
	[MT6993_MEMIF_VUL5] = MT6993_IRQ_18,
	[MT6993_MEMIF_VUL6] = MT6993_IRQ_19,
	[MT6993_MEMIF_VUL7] = MT6993_IRQ_20,
	[MT6993_MEMIF_VUL8] = MT6993_IRQ_21,
	[MT6993_MEMIF_VUL9] = MT6993_IRQ_22,
	[MT6993_MEMIF_VUL10] = MT6993_IRQ_23,
	[MT6993_MEMIF_VUL24] = MT6993_IRQ_24,
	[MT6993_MEMIF_VUL25] = MT6993_IRQ_25,
	[MT6993_MEMIF_VUL26] = MT6993_IRQ_0,
	[MT6993_MEMIF_VUL_CM0] = MT6993_IRQ_26,
	[MT6993_MEMIF_VUL_CM1] = MT6993_IRQ_0,
	[MT6993_MEMIF_VUL_CM2] = MT6993_IRQ_0,
	[MT6993_MEMIF_ETDM_IN0] = MT6993_IRQ_0,
	[MT6993_MEMIF_ETDM_IN1] = MT6993_IRQ_0,
	[MT6993_MEMIF_ETDM_IN2] = MT6993_IRQ_0,
	[MT6993_MEMIF_ETDM_IN3] = MT6993_IRQ_0,
	[MT6993_MEMIF_ETDM_IN4] = MT6993_IRQ_0,
	[MT6993_MEMIF_ETDM_IN6] = MT6993_IRQ_0,
	[MT6993_MEMIF_HDMI] = MT6993_IRQ_31
};
#else
static const int memif_irq_usage[MT6993_MEMIF_NUM] = {
	/* ADSP will use IRQ 29/30 for urgent IRQ, please do not use */
	/* TODO: verify each memif & irq */
	[MT6993_MEMIF_DL0] = MT6993_IRQ_0,
	[MT6993_MEMIF_DL1] = MT6993_IRQ_1,
	[MT6993_MEMIF_DL2] = MT6993_IRQ_2,
	[MT6993_MEMIF_DL3] = MT6993_IRQ_3,
	[MT6993_MEMIF_DL4] = MT6993_IRQ_4,
	[MT6993_MEMIF_DL5] = MT6993_IRQ_5,
	[MT6993_MEMIF_DL6] = MT6993_IRQ_6,
	[MT6993_MEMIF_DL7] = MT6993_IRQ_7,
	[MT6993_MEMIF_DL8] = MT6993_IRQ_8,
	[MT6993_MEMIF_DL23] = MT6993_IRQ_9,
	[MT6993_MEMIF_DL24] = MT6993_IRQ_10,
	[MT6993_MEMIF_DL25] = MT6993_IRQ_11,
	[MT6993_MEMIF_DL26] = MT6993_IRQ_0,
	[MT6993_MEMIF_DL_4CH] = MT6993_IRQ_0,
	[MT6993_MEMIF_DL_24CH] = MT6993_IRQ_12,
	[MT6993_MEMIF_VUL0] = MT6993_IRQ_13,
	[MT6993_MEMIF_VUL1] = MT6993_IRQ_14,
	[MT6993_MEMIF_VUL2] = MT6993_IRQ_15,
...
static bool mt6993_is_volatile_reg(struct device *dev, unsigned int reg)
{
...
static const struct regmap_config mt6993_afe_regmap_config = {
	.reg_bits = 32,
	.reg_stride = 4,
	.val_bits = 32,

	.volatile_reg = mt6993_is_volatile_reg,
...
static irqreturn_t mt6993_afe_irq_handler(int irq_id, void *dev)
{
...
#if !IS_ENABLED(CONFIG_NEBULA_SND_PASSTHROUGH)
	for (i = 0; i < MT6993_IRQ_NUM; ++i) {
		if (((cus_status_mcu & (0x1 << irq_data[i].id)) && i == MT6993_IRQ_31) ||
		    ((status_mcu & (0x1 << irq_data[i].id)) && i != MT6993_IRQ_31)) {
			regmap_read(afe->regmap, irq_data[i].irq_clr_reg, &tmp_reg);
			regmap_update_bits(afe->regmap, irq_data[i].irq_clr_reg,
					0xc0000000,
					tmp_reg^0xc0000000);
...
static int mt6993_afe_runtime_suspend(struct device *dev)
{
...
skip_regmap:
	mt6993_afe_sram_release(afe);
	mt6993_afe_disable_clock(afe);
	return 0;
}

static int mt6993_afe_runtime_resume(struct device *dev)
{
	struct mtk_base_afe *afe = dev_get_drvdata(dev);
	int ret = 0;

	ret = mt6993_afe_enable_clock(afe);
	dev_info_ratelimited(afe->dev, "%s(), enable_clock ret %d\n", __func__, ret);

	if (ret)
		return ret;
	mt6993_afe_sram_request(afe);
...
static int mt6993_afe_pcm_copy(struct snd_pcm_substream *substream,
				...
				enum mtk_audio_sram_mode sram_mode)
{
	...
	/* set memif sram mode */
	mt6993_set_memif_sram_mode(dev, sram_mode);
...
static const struct mtk_audio_sram_ops mt6993_sram_ops = {
	.set_sram_mode = mt6993_set_sram_mode,
};
				    ...
				    char *buf, loff_t offset, size_t size)
{
	...
	read_size = mt6993_debug_read_reg(buffer, AFE_SYS_DEBUG_SIZE, afe);
...
static int mt6993_afe_component_probe(struct snd_soc_component *component)
{
	struct mtk_base_afe *afe = NULL;
	...
	if (component) {
		...
		mt6993_add_misc_control(component);
...
static const struct snd_soc_component_driver mt6993_afe_component = {
	.name = AFE_PCM_NAME,
	.probe = mt6993_afe_component_probe,
...
static ssize_t mt6993_debug_read_reg(char *buffer, int size, struct mtk_base_afe *afe)
{
	...
	if (afe_priv->cksys_ck) {
		regmap_read(afe_priv->cksys_ck, CLK_CFG_9, &value);
		n += scnprintf(buffer + n, size - n,
				"CLK_CFG_9 = 0x%x\n", value);
	...
	if (afe_priv->vlp_ck) {
		regmap_read(afe_priv->vlp_ck, VLP_CLK_CFG_7, &value);
		n += scnprintf(buffer + n, size - n,
				"VLP_CLK_CFG_7 = 0x%x\n", value);
		regmap_read(afe_priv->vlp_ck, VLP_CLK_CFG_8, &value);
		n += scnprintf(buffer + n, size - n,
				"VLP_CLK_CFG_8 = 0x%x\n", value);
...
static ssize_t mt6993_debugfs_read(struct file *file, char __user *buf,
				   size_t count, loff_t *pos)
{
	...
	n = mt6993_debug_read_reg(buffer, size, afe);
...
static const struct mtk_afe_debug_cmd mt6993_debug_cmds[] = {
	MTK_AFE_DBG_CMD("write_reg", mtk_afe_debug_write_reg),
	{}
};

static const struct file_operations mt6993_debugfs_ops = {
	.open = mtk_afe_debugfs_open,
	.write = mtk_afe_debugfs_write,
	.read = mt6993_debugfs_read,
};
#endif

static int mt6993_dai_memif_register(struct mtk_base_afe *afe)
{
	...
	dai->dai_drivers = mt6993_memif_dai_driver;
	dai->num_dai_drivers = ARRAY_SIZE(mt6993_memif_dai_driver);

	dai->controls = mt6993_pcm_kcontrols;
	dai->num_controls = ARRAY_SIZE(mt6993_pcm_kcontrols);
	dai->dapm_widgets = mt6993_memif_widgets;
	dai->num_dapm_widgets = ARRAY_SIZE(mt6993_memif_widgets);
	dai->dapm_routes = mt6993_memif_routes;
	dai->num_dapm_routes = ARRAY_SIZE(mt6993_memif_routes);
	return 0;
}

typedef int (*dai_register_cb)(struct mtk_base_afe *);
static const dai_register_cb dai_register_cbs[] = {
	mt6993_dai_adda_register,
	mt6993_dai_i2s_register,
	mt6993_dai_hw_gain_register,
	mt6993_dai_src_register,
	mt6993_dai_pcm_register,
	mt6993_dai_tdm_register,
	mt6993_dai_hostless_register,
	mt6993_dai_memif_register,
};

static int mt6993_afe_pcm_dev_probe(struct platform_device *pdev)
{
	int ret, i;
	unsigned int tmp_reg = 0;
#if !defined(IS_FPGA_EARLY_PORTING) || defined(FORCE_FPGA_ENABLE_IRQ)
	...
	struct mt6993_afe_private *afe_priv;
	struct resource *res;
	struct device *dev;
	...
	ret = mtk_audio_sram_init(dev, afe->sram, &mt6993_sram_ops);
	if (ret)
		return ret;
#endif
	/* init memif */
	...
	afe->memif_size = MT6993_MEMIF_NUM;
	...
	afe->memif[MT6993_DEEP_MEMIF].ack = mtk_sp_clean_written_buffer_ack;
	afe->memif[MT6993_FAST_MEMIF].fast_palyback = 1;

	mutex_init(&afe->irq_alloc_lock);       /* needed when dynamic irq */

	/* init irq */
	afe->irqs_size = MT6993_IRQ_NUM;
...
#endif
	ret = devm_request_irq(dev, irq_id, mt6993_afe_irq_handler,
			       IRQF_TRIGGER_NONE,
	...
	/* others */
	afe->mtk_afe_hardware = &mt6993_afe_hardware;
	afe->memif_fs = mt6993_memif_fs;
	afe->irq_fs = mt6993_irq_fs;
	afe->get_dai_fs = mt6993_get_dai_fs;
	afe->get_memif_pbuf_size = mt6993_get_memif_pbuf_size;

	afe->runtime_resume = mt6993_afe_runtime_resume;
	afe->runtime_suspend = mt6993_afe_runtime_suspend;

	afe->request_dram_resource = mt6993_afe_dram_request;
	afe->release_dram_resource = mt6993_afe_dram_release;

	afe->copy = mt6993_afe_pcm_copy;
...
#if IS_ENABLED(CONFIG_DEBUG_FS)
	/* debugfs */
	afe->debug_cmds = mt6993_debug_cmds;
	afe->debugfs = debugfs_create_file("mtksocaudio",
					   S_IFREG | 0444, NULL,
					   afe, &mt6993_debugfs_ops);
#endif
	...
	mt6993_set_cm_mux(CM0, 0);
	mt6993_set_cm_mux(CM1, 0);
	mt6993_set_cm_mux(CM2, 0);
err_find_pmic:
#if IS_ENABLED(CONFIG_SND_SOC_MTK_AUDIO_DSP)
...
static void mt6993_afe_pcm_dev_remove(struct platform_device *pdev)
{
	struct mtk_base_afe *afe = platform_get_drvdata(pdev);

	pm_runtime_disable(&pdev->dev);
	if (!pm_runtime_status_suspended(&pdev->dev))
		mt6993_afe_runtime_suspend(&pdev->dev);
...
static const struct of_device_id mt6993_afe_pcm_dt_match[] = {
	{ .compatible = "mediatek,mt6993-sound", },
	{},
};
MODULE_DEVICE_TABLE(of, mt6993_afe_pcm_dt_match);

static const struct dev_pm_ops mt6993_afe_pm_ops = {
	SET_RUNTIME_PM_OPS(mt6993_afe_runtime_suspend,
			   mt6993_afe_runtime_resume, NULL)
};

static struct platform_driver mt6993_afe_pcm_driver = {
	.driver = {
		.name = "mt6993-audio",
		.of_match_table = mt6993_afe_pcm_dt_match,
#if IS_ENABLED(CONFIG_PM)
		.pm = &mt6993_afe_pm_ops,
#endif
	},
	.probe = mt6993_afe_pcm_dev_probe,
	.remove = mt6993_afe_pcm_dev_remove,
};

module_platform_driver(mt6993_afe_pcm_driver);
