			     ...
			     int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		mt6993_afe_gpio_request(afe, true, MT6993_DAI_ADDA, 1);
				  ...
				  int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		if (mtkaif_dmic) {
			...
			regmap_update_bits(afe->regmap,
					   ...
					   0x0);
			mtk_adda_ul_src_dmic(afe, MT6993_DAI_ADDA_CH34);
		}
	...
	case SND_SOC_DAPM_POST_PMD:
		/* should delayed 1/fs(smallest is 8k) = 125us before afe off */
		udelay(125);
		mt6993_afe_gpio_request(afe, false, MT6993_DAI_ADDA_CH34, 1);
			     ...
			     int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		mt6993_afe_gpio_request(afe, true, MT6993_DAI_ADDA_CH56, 1);
		break;
	case SND_SOC_DAPM_POST_PMD:
		/* should delayed 1/fs(smallest is 8k) = 125us before afe off */
		udelay(125);
		mt6993_afe_gpio_request(afe, false, MT6993_DAI_ADDA_CH56, 1);
			     ...
			     int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		mt6993_afe_gpio_request(afe, true, MT6993_DAI_ADDA, 1);
		//mtk_adda_ul_src_dmic(afe, MT6993_DAI_AP_DMIC);
		mt6993_afe_gpio_request(afe, true, MT6993_DAI_AP_DMIC, 1);
		break;
	case SND_SOC_DAPM_POST_PMD:
		/* should delayed 1/fs(smallest is 8k) = 125us before afe off */
		udelay(125);
		mt6993_afe_gpio_request(afe, false, MT6993_DAI_ADDA, 1);
		mt6993_afe_gpio_request(afe, false, MT6993_DAI_AP_DMIC, 1);
				  ...
				  int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		mt6993_afe_gpio_request(afe, true, MT6993_DAI_ADDA_CH34, 1);
		mt6993_afe_gpio_request(afe, true, MT6993_DAI_AP_DMIC_CH34, 1);
		break;
	case SND_SOC_DAPM_POST_PMD:
		/* should delayed 1/fs(smallest is 8k) = 125us before afe off */
		udelay(125);
		mt6993_afe_gpio_request(afe, false, MT6993_DAI_ADDA_CH34, 1);
		mt6993_afe_gpio_request(afe, false, MT6993_DAI_AP_DMIC_CH34, 1);
		break;
	default:
		break;
	}
				  ...
				  int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		mt6993_afe_gpio_request(afe, true, MT6993_DAI_ADDA_CH56, 1);
		mt6993_afe_gpio_request(afe, true, MT6993_DAI_AP_DMIC_CH56, 1);
		break;
	case SND_SOC_DAPM_POST_PMD:
		/* should delayed 1/fs(smallest is 8k) = 125us before afe off */
		udelay(125);
		mt6993_afe_gpio_request(afe, false, MT6993_DAI_ADDA_CH56, 1);
		mt6993_afe_gpio_request(afe, false, MT6993_DAI_AP_DMIC_CH56, 1);
		break;
	default:
		break;
	}
...
static bool is_adda_mtkaif_need_phase_delay(struct mt6993_afe_private *afe_priv)
{
	if (mt6993_afe_gpio_is_prepared(MT6993_AFE_GPIO_DAT_MISO0_ON) &&
	...
	if (mt6993_afe_gpio_is_prepared(MT6993_AFE_GPIO_DAT_MISO1_ON) &&
			     ...
			     int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		mt6993_afe_gpio_request(afe, true, MT6993_DAI_ADDA, 0);
		break;
	case SND_SOC_DAPM_POST_PMD:
		/* should delayed 1/fs(smallest is 8k) = 125us before afe off */
		udelay(125);
				  ...
				  int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		mt6993_afe_gpio_request(afe, true, MT6993_DAI_ADDA_CH34, 0);
		break;
	case SND_SOC_DAPM_POST_PMD:
		/* should delayed 1/fs(smallest is 8k) = 125us before afe off */
		udelay(125);
		mt6993_afe_gpio_request(afe, false, MT6993_DAI_ADDA_CH34, 0);
		break;
	default:
		break;
	}
...
/* mtkaif dmic */
static const char *const mt6993_adda_off_on_str[] = {
	"Off", "On"
};

static const struct soc_enum mt6993_adda_enum[] = {
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(mt6993_adda_off_on_str),
			    mt6993_adda_off_on_str),
};

static int mt6993_adda_ap_dmic_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6993_adda_ap_dmic_set(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6993_adda_dmic_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6993_adda_dmic_set(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6993_adda6_only_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6993_adda6_only_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6993_adda_dl_max_vol_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
#ifdef MT6681_NO_VS1_VOTE
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
...
static int mt6993_adda_dl_max_vol_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
#ifdef MT6681_NO_VS1_VOTE
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
...
static int mt6993_vow_enable_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6993_vow_enable_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static const struct snd_kcontrol_new mtk_adda_controls[] = {
	...
	SOC_ENUM_EXT("MTKAIF_DMIC", mt6993_adda_enum[0],
		     mt6993_adda_dmic_get, mt6993_adda_dmic_set),
	SOC_ENUM_EXT("MTKAIF_ADDA6_ONLY", mt6993_adda_enum[0],
		     mt6993_adda6_only_get, mt6993_adda6_only_set),
	SOC_SINGLE_EXT("ADDA_DL_MAX_VOL",
		       SND_SOC_NOPM, 0, 0x1, 0,
		       mt6993_adda_dl_max_vol_get,
		       mt6993_adda_dl_max_vol_set),
	SOC_SINGLE_EXT("VOW_ENABLE",
		       SND_SOC_NOPM, 0, 0x1, 0,
		       mt6993_vow_enable_get,
		       mt6993_vow_enable_set),
	SOC_ENUM_EXT("AP DMIC Used", mt6993_adda_enum[0],
		     mt6993_adda_ap_dmic_get, mt6993_adda_ap_dmic_set),
};
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
...
#endif
			/* UL0 bypass soundwired */
			regmap_update_bits(afe->regmap,
					AFE_SOUNDWIRE_ADDA_PDI2_CON0,
					PDM2PCM_PATH_MASK_SFT,
					0x1 << PDM2PCM_PATH_SFT);
...
#endif
			/* UL1 bypass soundwired */
			regmap_update_bits(afe->regmap,
					AFE_SOUNDWIRE_ADDA_PDI3_CON0,
					PDM2PCM_PATH_MASK_SFT,
					0x1 << PDM2PCM_PATH_SFT);
		...
		case MT6993_DAI_AP_DMIC_CH56:
			...
			regmap_update_bits(afe->regmap,
					...
					0x0 << RG_MTKAIF1_RXIF_BYPASS_SRC_SFT);
#endif
			/* UL2 bypass soundwired */
			regmap_update_bits(afe->regmap,
					AFE_SOUNDWIRE_ADDA_PDI4_CON0,
					PDM2PCM_PATH_MASK_SFT,
					0x1 << PDM2PCM_PATH_SFT);
...
int mt6993_dai_adda_register(struct mtk_base_afe *afe)
{
	...
	/* set dai priv */
	ret = mt6993_dai_set_priv(afe, MT6993_DAI_ADDA,
				  sizeof(struct mtk_afe_adda_priv), NULL);
	if (ret)
		return ret;

	ret = mt6993_dai_set_priv(afe, MT6993_DAI_ADDA_CH34,
				  sizeof(struct mtk_afe_adda_priv), NULL);
	if (ret)
		return ret;

	ret = mt6993_dai_set_priv(afe, MT6993_DAI_ADDA_CH56,
				  sizeof(struct mtk_afe_adda_priv), NULL);
	if (ret)
		return ret;
	...
	afe_priv->dai_priv[MT6993_DAI_AP_DMIC] =
		afe_priv->dai_priv[MT6993_DAI_ADDA];
	afe_priv->dai_priv[MT6993_DAI_AP_DMIC_CH34] =
		afe_priv->dai_priv[MT6993_DAI_ADDA_CH34];
	afe_priv->dai_priv[MT6993_DAI_AP_DMIC_CH56] =
		afe_priv->dai_priv[MT6993_DAI_ADDA_CH56];
