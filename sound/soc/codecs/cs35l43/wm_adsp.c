		      ...
		      unsigned int alg, void *buf, size_t len)
{
	...
	//adsp_info(dsp, "wm_adsp_write_ctl control '%s' with data: 0x%x(ctl sts:%d, cache=0x%x), ret= %d\n",
	//		name, *((int*)buf), cs_ctl->enabled, *((int*)cs_ctl->cache), ret);
	if (ret < 0)
		return ret;
		     ...
		     unsigned int alg, void *buf, size_t len)
{
	...
	ret = cs_dsp_coeff_read_ctrl(cs_dsp_get_ctl(&dsp->cs_dsp, name, type, alg),
				     0, buf, len);
	mutex_unlock(&dsp->cs_dsp.pwr_lock);
	//adsp_info(dsp, "wm_adsp_read_ctl control '%s' with data: 0x%x, ret= %d\n", name, *((int*)buf), ret);
	return ret;
}
					 ...
					 const char *filetype)
{
	...
	if (system_name && asoc_component_prefix)
		...
		*filename = kasprintf(GFP_KERNEL, "%s-%s-%s-%s.%s", asoc_component_prefix, dsp->part,
					  dsp->fwf_name, wm_adsp_fw[dsp->fw].file, filetype);
	else
					  ...
					  char **coeff_filename)
{
	...
	if (!wm_adsp_request_firmware_file(dsp, wmfw_firmware, wmfw_filename,
					   "", NULL, NULL, "wmfw")) {
		if (asoc_component_prefix)
		//different tuning with name prefix
			wm_adsp_request_firmware_file(dsp, coeff_firmware, coeff_filename,
					      "", NULL, asoc_component_prefix, "bin");
		else
			wm_adsp_request_firmware_file(dsp, coeff_firmware, coeff_filename,
					      "", NULL, NULL, "bin");
		return 0;
