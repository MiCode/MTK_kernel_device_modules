...
#if 0
static int cs35l43_rw_registers_get(struct snd_kcontrol *kcontrol,
        unsigned int __user *data, unsigned int size)
{
    struct snd_soc_component *component = snd_soc_kcontrol_component(kcontrol);
    struct cs35l43_private *cs35l43 = snd_soc_component_get_drvdata(component);
    int *buff = cs35l43->ctlbuf;
    int ret = 0;

    dev_dbg(cs35l43->dev, "Get: size = %d, %08x %08x %08x %08x %08x %08x %08x\n",
...
static int cs35l43_rw_registers_put(struct snd_kcontrol *kcontrol,
        const unsigned int __user *data, unsigned int size)
{
    ...
    memset(cs35l43->ctlbuf, 0x00, REG_VALUE_SIZE);
    if (copy_from_user(cs35l43->ctlbuf, data, size)) {
        ret = -EFAULT;
        goto exit;
    }
    ...
    dev_dbg(cs35l43->dev, "Put: size = %d, %08x %08x %08x %08x %08x %08x %08x\n",
            size, buff[0], buff[1], buff[2], buff[3],
            buff[4], buff[5], buff[6]);
    ...
    if(rw == 0x80) { /* 0x80 stand for write */
        for(i = 0; i < num; i ++) {
            ...
            dev_info(cs35l43->dev, "ctl write: %x <== %x, %s\n",
    ...
    if(rw == 0x81) { /* 0x81 stand for read */
        for(i = 0; i < num; i ++) {
            ...
            dev_info(cs35l43->dev, "ctl read: %x ==> %x, ret = %d\n",
                    reg, val, ret);
