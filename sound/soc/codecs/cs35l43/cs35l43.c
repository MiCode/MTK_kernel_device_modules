...
({ \
	...
	if (__ret) \
		pr_info("I2C retry : regmap_update_bits: %d, (0x%x, 0x%x, 0x%x), ret = %d\n", __i, reg, mask, val, __ret); \
	__ret; \
})
...
({ \
	...
	if (__ret) \
		pr_info("I2C retry : regmap_write: %d, (0x%x, 0x%x), ret = %d\n", __i, reg, val, __ret); \
	__ret; \
})
...
({ \
	...
	if (__ret) \
		pr_info("I2C retry : regmap_read: %d, (0x%x, 0x%x), ret = %d\n", __i, reg, *val, __ret); \
	__ret; \
})
...
({ \
	...
	if (__ret) \
		pr_info("I2C retry : regmap_raw_write: %d, (0x%x, 0x%x, %d), ret = %d\n", __i, reg, *val, len, __ret); \
	__ret; \
})
...
({ \
	...
	if (__ret) \
		pr_info("I2C retry : regmap_raw_write_async: %d, (0x%x, 0x%x, %d), ret = %d\n", __i, reg, *val, len, __ret); \
	__ret; \
})
...
({ \
	...
	if (__ret) \
		pr_info("I2C retry : regmap_raw_read: %d, (0x%x, 0x%x, %d), ret = %d\n", __i, reg, *val, len, __ret); \
	__ret; \
})
...
#if defined(CONFIG_TARGET_PRODUCT_HAOTIAN) || defined(CONFIG_TARGET_PRODUCT_DADA)
static const struct snd_kcontrol_new amc_enable_ctrl =
	SOC_DAPM_SINGLE("Switch", SND_SOC_NOPM, 0, 1, 0);
#endif
...
static const char * const delta_enum_text[] = {"null", "music", "voice", "movie", "voip", "video", "game", "handsfree", "handset"};
				 ...
				 struct snd_ctl_elem_value *ucontrol)
{
	...
	int val;
	bool booted = cs35l43->dsp.cs_dsp.booted;
				    ...
				    struct snd_ctl_elem_value *ucontrol)
{
	...
	struct cs35l43_private *cs35l43 = snd_soc_component_get_drvdata(component);
	struct wm_adsp *dsp = &cs35l43->dsp;
	struct snd_card *card = dsp->component->card->snd_card;
	...
	mutex_lock(&cs35l43->calib_mode_lock);
	...
	if (!kctl) {
		...
		mutex_unlock(&cs35l43->calib_mode_lock);
		return -EINVAL;
	}
...
static int cs35l43_rcv_switch_pinctrl_get(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_soc_kcontrol_component(kcontrol);
	struct cs35l43_private	*cs35l43 = snd_soc_component_get_drvdata(component);
	ucontrol->value.integer.value[0] = 0;
	if (cs35l43->cs35l43_rcv_pinctrl && cs35l43->enable_rcv_pin_control)
		ucontrol->value.integer.value[0] = cs35l43->enable_rcv_switch;
	return 0;
}

static int cs35l43_rcv_switch_pinctrl_put(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
    struct snd_soc_component *component = snd_soc_kcontrol_component(kcontrol);
    struct cs35l43_private	*cs35l43 = snd_soc_component_get_drvdata(component);

    cs35l43->enable_rcv_switch = ucontrol->value.integer.value[0];

    if (cs35l43->cs35l43_rcv_pinctrl && cs35l43->enable_rcv_pin_control) {
        if (cs35l43->enable_rcv_switch) {
            dev_info(cs35l43->dev, "%s: enable_rcv_switch %d, rcv switch gpio pull up\n", __func__,
					cs35l43->enable_rcv_switch);
            pinctrl_select_state(cs35l43->cs35l43_rcv_pinctrl,
					cs35l43->pinctrl_state[0]);
        } else {
            dev_info(cs35l43->dev, "%s: enable_rcv_switch %d, rcv switch gpio pull down\n", __func__,
					cs35l43->enable_rcv_switch);
            pinctrl_select_state(cs35l43->cs35l43_rcv_pinctrl,
					cs35l43->pinctrl_state[1]);
        }
    }
...
static int cs35l43_apply_delta_tuning(struct cs35l43_private *cs35l43)
{
	...
	if ((cs35l43->delta_requested < 0) || (cs35l43->delta_requested > ARRAY_SIZE(delta_enum_text))) {
		dev_err(cs35l43->dev, "Applying delta file index %d is illegal\n", cs35l43->delta_requested);
		return 0;
	}

	if (dsp->tuning_has_prefix || dsp->component->name_prefix)
		snprintf(filename, NAME_MAX, "%s-%s-delta-%s-spk-prot.bin",
			 dsp->component->name_prefix,
			 dsp->part,
			 delta_enum_text[cs35l43->delta_requested]);
	else
		snprintf(filename, NAME_MAX, "%s-delta-%s-spk-prot.bin",
			 dsp->part,
			 delta_enum_text[cs35l43->delta_requested]);
	...
	//load and then read back to check the consistency of every block
	ret = cs_dsp_load_then_verify_coeff(&dsp->cs_dsp, firmware, filename, &success);
	if (!success) {
		cs35l43->err_cnt_delta++;
	} else {
		cs35l43->delta_applied = cs35l43->delta_requested;
	}
	dev_info(cs35l43->dev, "[TF-STABILITY] applying delta file %s: %d, err_count: %d\n",
				filename, success, cs35l43->err_cnt_delta);
			   ...
			   struct snd_ctl_elem_value *ucontrol)
{
	...
	if (!cs35l43->dsp.cs_dsp.running) {
		dev_err(cs35l43->dev, "Error applying delta file enum(%d) due to DSP not run\n",
			cs35l43->delta_requested);
		return -EINVAL;
	}
	...
	if (cs35l43->low_pwr_mode == CS35L43_LOW_PWR_MODE_HIBERNATE) {
			if (cs35l43->hibernate_state == CS35L43_HIBERNATE_AWAKE)
				ret = cs35l43_apply_delta_tuning(cs35l43);
	} else {
		ret = cs35l43_apply_delta_tuning(cs35l43);
	}
...
static const int cs35l43_pcm_sftramp_ms_double[] = {0, 1, 2, 4, 8, 16, 32,64};
static int cs35l43_mute_amplifier(struct cs35l43_private *cs35l43, bool mute)
{

	int reg, vol, soft_ramp_val;
	int soft_ramp_ms;
	dev_info(cs35l43->dev, "%s: mute status = %d\n", __func__, mute);

	regmap_read(cs35l43->regmap, CS35L43_AMP_CTRL, &reg);
	vol = (reg & CS35L43_AMP_VOL_PCM_MASK) >> CS35L43_AMP_VOL_PCM_SHIFT;
	dev_info(cs35l43->dev, "%s: reg = 0x%x, vol = 0x%x\n", __func__, reg, vol);
	if (mute) {
		// unmute status
		if (vol > 0x4CF || vol < 0x400) {
			regmap_update_bits(cs35l43->regmap, CS35L43_AMP_CTRL,
				CS35L43_AMP_VOL_PCM_MASK, 0x400 << CS35L43_AMP_VOL_PCM_SHIFT);
			soft_ramp_val = (reg >> CS35L43_AMP_RAMP_PCM_SHIFT) & CS35L43_AMP_RAMP_PCM_MASK;
			soft_ramp_ms = (cs35l43_pcm_sftramp_ms_double[soft_ramp_val] * 18 + 1) / 2;
			dev_info(cs35l43->dev, "%s: soft_ramp_ms = %d\n", __func__, soft_ramp_ms);
			if (soft_ramp_ms)
				msleep(soft_ramp_ms);
		}
	}else {
		if (vol >= 0x400 && vol <= 0x4CF)
			regmap_update_bits(cs35l43->regmap, CS35L43_AMP_CTRL,
				CS35L43_AMP_VOL_PCM_MASK, 0x0 << CS35L43_AMP_VOL_PCM_SHIFT);
	}
	return 0;
}

static int cs35l43_delta_rt_put(struct snd_kcontrol *kcontrol,
			   struct snd_ctl_elem_value *ucontrol)
	{
		struct snd_soc_component *component =
					 snd_soc_kcontrol_component(kcontrol);
		...
		if (!mutex_is_locked(&cs35l43->delta_rt_lock))
			queue_work(cs35l43->delta_rt_wq, &cs35l43->delta_rt_work);
...
#if defined(AMP_REGISTER_DEBUG)
static int cs35l43_amp_register_dump_get(struct snd_kcontrol *kcontrol,
			 struct snd_ctl_elem_value *ucontrol)
{
   ucontrol->value.integer.value[0] = 0;
...
static int cs35l43_amp_register_dump_put(struct snd_kcontrol *kcontrol,
			 struct snd_ctl_elem_value *ucontrol)
{
   struct snd_soc_component *component =
		 snd_soc_kcontrol_component(kcontrol);
   struct cs35l43_private *cs35l43 =
		 snd_soc_component_get_drvdata(component);
   struct regmap *map = cs35l43->regmap;
   unsigned int val;
   int i, reg_max, reg_stride;
   int ret;

   dev_dbg(cs35l43->dev, "%s: register_dump = %lu\n", __func__, ucontrol->value.integer.value[0]);

   if (ucontrol->value.integer.value[0]) {
   		// Dump registers and dsp memory with single read
	   reg_max = (2 == ucontrol->value.integer.value[0]) ?  CS35L43_DSP1_PMEM_5114 : CS35L43_MIXER_NGATE_CH2_CFG;
	   reg_stride = 4;
		   
	   for (i = 0; reg_max >= i; i += reg_stride) {
		   if (!cs35l43_readable_reg(cs35l43->dev, i))
			   continue;
	   
		   ret = regmap_read(map, i, &val);
		   if (ret) {
			   dev_err(cs35l43->dev, "%s: Failed to read reg = 0x%08x, ret = %d\n", __func__, i, ret);
			   return ret;
		   } else {
			   dev_err(cs35l43->dev, "%08x: %08x\n", i, val);
		   }
	   	}

	   // Dump registers and dsp memory with single or burst read
	 if (3 == ucontrol->value.integer.value[0]) {
		reg_stride = 4*3;
	 	unsigned int buf[3];

		for (i = CS35L43_DSP1_XMEM_PACKED_0; CS35L43_DSP1_XMEM_PACKED_6143 >= i; i += reg_stride) {
			ret = regmap_raw_read(map, i, buf, reg_stride);
			if (ret) {
				dev_err(cs35l43->dev, "%s: Failed to read reg = 0x%08x, ret = %d\n", __func__, i, ret);
				return ret;
			} else {
				for (int j = 0; sizeof(buf)/sizeof(unsigned int) > j; j++)
					dev_err(cs35l43->dev, "%08x: %08x\n", i+4*j, buf[j]);
			}
		}
	 }
	 ...
	 if (3 == ucontrol->value.integer.value[0]) {
		...
		for (i = CS35L43_DSP1_XMEM_UNPACKED32_0; CS35L43_DSP1_XMEM_UNPACKED32_4095 >= i; i += reg_stride) {
			ret = regmap_read(map, i, &val);
			if (ret) {
				dev_err(cs35l43->dev, "%s: Failed to read reg = 0x%08x, ret = %d\n", __func__, i, ret);
				return ret;
	 ...
	 if (3 == ucontrol->value.integer.value[0]) {
		...
		for (i = CS35L43_DSP1_XMEM_UNPACKED24_0; CS35L43_DSP1_XMEM_UNPACKED24_8191 >= i; i += reg_stride) {
			ret = regmap_read(map, i, &val);
			if (ret) {
				dev_err(cs35l43->dev, "%s: Failed to read reg = 0x%08x, ret = %d\n", __func__, i, ret);
				return ret;
	 ...
	 if (3 == ucontrol->value.integer.value[0]) {
		...
		for (i = CS35L43_DSP1_XROM_UNPACKED24_0; CS35L43_DSP1_XROM_UNPACKED24_6141 >= i; i += reg_stride) {
			ret = regmap_read(map, i, &val);
			if (ret) {
				dev_err(cs35l43->dev, "%s: Failed to read reg = 0x%08x, ret = %d\n", __func__, i, ret);
				return ret;
	 ...
	 if (3 == ucontrol->value.integer.value[0]) {
		...
		for (i = CS35L43_DSP1_YMEM_PACKED_0; CS35L43_DSP1_YMEM_PACKED_1532 >= i; i += reg_stride) {
			ret = regmap_raw_read(map, i, buf, reg_stride);
			if (ret) {
				dev_err(cs35l43->dev, "%s: Failed to read reg = 0x%08x, ret = %d\n", __func__, i, ret);
				return ret;
	 ...
	 if (3 == ucontrol->value.integer.value[0]) {
		...
		for (i = CS35L43_DSP1_YMEM_UNPACKED32_0; CS35L43_DSP1_YMEM_UNPACKED32_1022 >= i; i += reg_stride) {
			ret = regmap_read(map, i, &val);
			if (ret) {
				dev_err(cs35l43->dev, "%s: Failed to read reg = 0x%08x, ret = %d\n", __func__, i, ret);
				return ret;
	 ...
	 if (3 == ucontrol->value.integer.value[0]) {
		...
		for (i = CS35L43_DSP1_YMEM_UNPACKED24_0; CS35L43_DSP1_YMEM_UNPACKED24_2045 >= i; i += reg_stride) {
			ret = regmap_read(map, i, &val);
			if (ret) {
				dev_err(cs35l43->dev, "%s: Failed to read reg = 0x%08x, ret = %d\n", __func__, i, ret);
				return ret;
	...
	if (3 == ucontrol->value.integer.value[0]) {
		...
		for (i = CS35L43_DSP1_PMEM_0; CS35L43_DSP1_PMEM_5114 >= i; i += reg_stride) {
			ret = regmap_raw_read(map, i, buf, reg_stride);
			if (ret) {
				dev_err(cs35l43->dev, "%s: Failed to read reg = 0x%08x, ret = %d\n", __func__, i, ret);
				return ret;
...
static int cs35l43_reg_addr_get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
	   snd_soc_kcontrol_component(kcontrol);
	struct cs35l43_private *cs35l43 =
	...
	ucontrol->value.integer.value[0] = cs35l43->reg_addr;
	dev_dbg(cs35l43->dev, "%s: get reg_addr = 0x%08lx\n", __func__, ucontrol->value.integer.value[0]);

	return 0;
}

static int cs35l43_reg_addr_put(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
	   snd_soc_kcontrol_component(kcontrol);
	struct cs35l43_private *cs35l43 =
	...
	cs35l43->reg_addr = ucontrol->value.integer.value[0];
	dev_dbg(cs35l43->dev, "%s: set reg_addr = 0x%08x\n", __func__, cs35l43->reg_addr);

	return 0;
}

static int cs35l43_reg_val_get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
	   snd_soc_kcontrol_component(kcontrol);
	struct cs35l43_private *cs35l43 =
	...
	ret = regmap_read(cs35l43->regmap, cs35l43->reg_addr, &val);
	if (ret) {
		dev_err(cs35l43->dev, "%s: Failed to read register value\n", __func__);
		return ret;
	}
...
static int cs35l43_reg_val_put(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
	   snd_soc_kcontrol_component(kcontrol);
	struct cs35l43_private *cs35l43 =
	...
	ret = regmap_write(cs35l43->regmap, cs35l43->reg_addr, val);
	if (ret) {
		dev_err(cs35l43->dev, "%s: Failed to write register value\n", __func__);
		return ret;
	}
...
static const struct snd_kcontrol_new cs35l43_aud_controls[] = {
#if defined(AMP_REGISTER_DEBUG)
	SOC_SINGLE_EXT("AMP Register Dump", SND_SOC_NOPM, 0, 3, 0,
				cs35l43_amp_register_dump_get, cs35l43_amp_register_dump_put),
	SOC_SINGLE_EXT("REG ADDR", 0, 0, 0x7fffffff, 0, cs35l43_reg_addr_get, cs35l43_reg_addr_put),
	SOC_SINGLE_EXT("REG VAL", 0, 0, 0x7fffffff, 0, cs35l43_reg_val_get, cs35l43_reg_val_put),
#endif
	...
	SOC_SINGLE_SX_TLV("Digital PCM Volume", CS35L43_AMP_CTRL,
			CS35L43_AMP_VOL_PCM_SHIFT, 0x4CF, 0x391,
			dig_vol_tlv),
	...
	SOC_ENUM_EXT("Delta Select", delta_enum, cs35l43_delta_select_get, cs35l43_delta_select_put),
	SOC_ENUM_EXT("Delta Select RT", delta_enum, cs35l43_delta_select_get, cs35l43_delta_rt_put),
...
static void cs35l43_delta_rt_work(struct work_struct *wk)
{
	struct cs35l43_private *cs35l43;

	cs35l43 = container_of(wk, struct cs35l43_private, delta_rt_work);

	mutex_lock(&cs35l43->delta_rt_lock);
	if (cs35l43->low_pwr_mode == CS35L43_LOW_PWR_MODE_HIBERNATE) {
		if (cs35l43->hibernate_state == CS35L43_HIBERNATE_AWAKE) {
			cs35l43_mute_amplifier(cs35l43, true);
			cs35l43_apply_delta_tuning(cs35l43);
			cs35l43_mute_amplifier(cs35l43, false);
		}
	} else {
		cs35l43_mute_amplifier(cs35l43, true);
		cs35l43_apply_delta_tuning(cs35l43);
	...
	mutex_unlock(&cs35l43->delta_rt_lock);
}
		       ...
		       struct snd_kcontrol *kcontrol, int event)
{
	...
	dev_dbg(cs35l43->dev, "%s: event = 0x%x, dsp preloaded = %d, dsp running = %d\n",
		__func__, event, cs35l43->dsp.preloaded, cs35l43->dsp.cs_dsp.running);
		       ...
		       struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		dev_info(cs35l43->dev, "PMU audio state post: 0x%x, err_cnt_delta: %d\n", audio_state, cs35l43->err_cnt_delta);
...
static int cs35l43_exit_hibernate(struct cs35l43_private *cs35l43)
{
	...
	dev_info(cs35l43->dev, "%s, sync all cached registers\n", __func__);
		...
		struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMD:
		...
		regmap_write(cs35l43->regmap, CS35L43_GLOBAL_ENABLES, 0);
		//}
		regmap_write(cs35l43->regmap, CS35L43_ASP_ENABLES1, 0);
		usleep_range(1000, 1100);
			/*
			*wait for power down finish
			*/
			pdn = false;
			for (i = 0; i < 100; i++) {
				regmap_read(cs35l43->regmap,
					CS35L43_IRQ1_EINT_1,
					&val);
				if (val & CS35L43_MSM_PDN_DONE_EINT1_MASK) {
					pdn = true;
					break;
				}
				usleep_range(1000, 1100);
			...
			dev_dbg(cs35l43->dev, "PDN Done Status : %d, %s\n", pdn, __func__);
...
#if defined(CONFIG_TARGET_PRODUCT_HAOTIAN) || defined(CONFIG_TARGET_PRODUCT_DADA)
	SND_SOC_DAPM_SWITCH("AMC Enable", SND_SOC_NOPM, 0, 0, &amc_enable_ctrl),
#endif
...
#if defined(CONFIG_TARGET_PRODUCT_HAOTIAN) || defined(CONFIG_TARGET_PRODUCT_DADA)
	{"AMC Enable", "Switch", "ASP TX1 Source"},
	{"ASPTX1", NULL, "AMC Enable"},
#else
				 ...
				 struct snd_soc_dai *dai)
{
	...
	if ((cs35l43->Global_FS != 0) && (rate != cs35l43->Global_FS)) {
		dev_err(cs35l43->dev, "%s: Unmatched sample rate with previous\n", __func__);
		return -EINVAL;
	}
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		cs35l43_component_set_sysclk(dai->component, 0, 0, TDM_MAX_SLOTS * params_rate(params) * asp_width, 0);
...
static int cs35lxx_dai_drv_append_suffix(struct cs35l43_private *cs35l43,
				...
				int num_dai)
{
	...
	dev_dbg(cs35l43->dev, "cs35lxx_dai_drv_append_suffix addr:%d \n", cs35l43->addr);
	if ((dai_drv != NULL) && (num_dai > 0)) {
		for (i = 0; i < num_dai; i++) {
			snprintf(buf, MAX_NAME_LEN, "%s-rx-00%d",
					dai_drv[i].name, cs35l43->addr);
			dai_drv[i].name = cs35lxx_devm_kstrdup(cs35l43->dev, buf);
		}
	}
...
//#if defined(CONFIG_TARGET_PRODUCT_HAOTIAN)
static int cs35l43_rcv_handset_switch_init(struct cs35l43_private *cs35l43)
{
	int ret = 0, i =0;

	cs35l43->enable_rcv_pin_control = 0;
	cs35l43->cs35l43_rcv_pinctrl = devm_pinctrl_get(cs35l43->dev);
	if (IS_ERR(cs35l43->cs35l43_rcv_pinctrl)) {
		if (PTR_ERR(cs35l43->cs35l43_rcv_pinctrl) == -EPROBE_DEFER) {
			dev_err(cs35l43->dev, "pinctrl not ready\n");
			ret = -EPROBE_DEFER;
			return ret;
		}
		dev_info(cs35l43->dev, "Target does not use pinctrl\n");
		cs35l43->cs35l43_rcv_pinctrl = NULL;
		ret = -EINVAL;
		return ret;
	}
	for (i = 0; i < ARRAY_SIZE(rcv_pctl_names); i++) {
		...
		struct pinctrl_state *state =
			pinctrl_lookup_state(cs35l43->cs35l43_rcv_pinctrl, n);
		if (IS_ERR(state)) {
			dev_err(cs35l43->dev, "cannot find '%s'\n", n);
			ret = -EINVAL;
			//goto exit;
			return ret;
		}
		dev_info(cs35l43->dev, "found pin control %s\n", n);
		cs35l43->pinctrl_state[i] = state;
		cs35l43->enable_rcv_pin_control = 1;
	}
	return ret;
}
//#endif
				...
				struct cs35l43_platform_data *pdata)
{
	...
	dev_err(cs35l43->dev, "DEBUG cs35l43 probe E\n");
...
//#if defined(CONFIG_TARGET_PRODUCT_HAOTIAN)
	/* Add for rcv/spk switch */
	cs35l43_rcv_handset_switch_init(cs35l43);
//#endif
	...
	cs35l43->delta_rt_wq = create_singlethread_workqueue("cs35l43_delta_rt");
	INIT_WORK(&cs35l43->delta_rt_work, cs35l43_delta_rt_work);
	...
	cs35l43->dai_driver = devm_kzalloc(cs35l43->dev,
				sizeof(struct snd_soc_dai_driver) * ARRAY_SIZE(cs35l43_dai), GFP_KERNEL);
	if (cs35l43->dai_driver == NULL) {
		dev_err(cs35l43->dev, "%s: dai_driver malloc failed \n",
			__func__);
		ret = -ENOMEM;
		goto err_mem;
	}

	memcpy(cs35l43->dai_driver, cs35l43_dai, sizeof(struct snd_soc_dai_driver) * ARRAY_SIZE(cs35l43_dai));
	cs35lxx_dai_drv_append_suffix(cs35l43, cs35l43->dai_driver, ARRAY_SIZE(cs35l43_dai));

	set_smartpa_type("cs35l43", sizeof("cs35l43"));
	...
	mtk_spk_set_type(MTK_SPK_CS_CS35L43);
...
if (probe_err_count >= probe_max_count) {
	dev_warn(cs35l43->dev, "%s: Register dummy codec\n", __func__);
#if IS_ENABLED(CONFIG_MIEV)
	mievent_report(906001351,"PA internal exception",cs35l43->dev);
#endif
	memset(&dummy_codec, 0, sizeof(struct snd_soc_component_driver));
	ret =  snd_soc_register_component(cs35l43->dev, &dummy_codec,
	&dummy_dai, 1);
	if (ret < 0) {
		dev_err(cs35l43->dev, "%s: Register dummy codec failed\n", __func__);
	}
	probe_err_count = 0;
	return 0;
}
