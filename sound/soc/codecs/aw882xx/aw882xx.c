...
static void aw882xx_request_firmware(struct work_struct *work)
{
	...
	case COROT:
		...
		break;
	case ZIRCON:
		acf_bin_name = ACF_BIN_NAME_N16U;
		aw_dev_info(aw882xx->dev, "load N16U[ZIRCON] firmware! -> [%s]",acf_bin_name);
		break;
	default:
...
{
#if defined (AW_QCOM_PLATFORM) || defined (AW_SYNC_LOAD)
...
static struct i2c_driver aw882xx_i2c_driver = {
	.driver = {
		.name = "speaker_amp",//AW882XX_I2C_NAME,
		.owner = THIS_MODULE,
...
static int __init aw882xx_i2c_init(void)
{
	...
	aw_pr_info("aw882xx i2c init done %s", AW882XX_DRIVER_VERSION);
	return ret;
}
