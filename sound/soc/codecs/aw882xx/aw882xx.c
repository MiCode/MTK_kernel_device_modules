			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	prof_name[0] = '\0';
	aw88xx_dev_get_profile_name(aw882xx->aw_pa, prof_name, ucontrol->value.integer.value[0]);
	if (!strncmp(prof_name, "Receiver", strlen(prof_name))) {
		if (gpio_is_valid(aw882xx->spk_sw_gpio)) {
...
#endif
{
	...
	aw_dev_err(&i2c->dev, "mtk_spk_set_type MTK_SPK_AW_AW882XX");
	/*codec register*/
