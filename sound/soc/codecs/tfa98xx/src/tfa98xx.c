...
static void tfa98xx_dsp_init(struct tfa98xx *tfa98xx)
{
	...
	if (tfa98xx->init_count < TF98XX_MAX_DSP_START_TRY_COUNT) {
		...
		dev_err(&tfa98xx->i2c->dev,
			"[TF-NOSOUND] Failed starting device (%d)\n",
			tfa98xx->init_count);
...
static int tfa98xx_i2c_probe(struct i2c_client *i2c) //modify for kernel6.X
{
	...
	if (IS_ERR(tfa98xx->regmap)) {
		...
		dev_err(&i2c->dev, "[TF-NOSOUND] Failed to allocate register map: %d\n",
			ret);
...
#endif
	if (ret < 0) {
		dev_err(&i2c->dev, "[TF-NOSOUND] Failed to register TFA98xx: %d\n", ret);
