...
static int tfa98xx_cali_l = 484966;
static int tfa98xx_cali_r = 484966;
static uint8_t tfadsp_volume = 0;//default value is 0db in tfadsp algo
...
static int tfa98xx_create_controls(struct tfa98xx *tfa98xx)
{
	...
	nr_controls = 3; /* Profile and stop control */
...
static void tfa98xx_dsp_init(struct tfa98xx *tfa98xx)
{
	...
	if (tfa98xx->init_count < TF98XX_MAX_DSP_START_TRY_COUNT) {
		...
		dev_err(&tfa98xx->i2c->dev,
			...
			tfa98xx->init_count);
		mievent_report(906001354,"PA data exception",&tfa98xx->i2c->dev);
		failed = true;
	}
	if (reschedule) {
...
static int tfa98xx_send_volume(uint8_t volume_l, uint8_t volume_r)//step is 0.5db, ig : if decrease 6db, set 0x0c
{
	...
	pr_info("send volume 0x%x to host DSP.\n",  volume_r);
...
#ifdef TFA_NON_DSP_SOLUTION
		if (tfa98xx->tfa->is_probus_device){
			pr_info("send_calib_values from HAL\n");
	...
	char *buf, loff_t off, size_t count)
{
	...
	pr_info("P cal data is: %d\n", tfa98xx_cali_l);
	pr_info("S cal data is: %d\n", tfa98xx_cali_r);
...
static int tfa98xx_i2c_probe(struct i2c_client *i2c) //modify for kernel6.X
{
	...
	ret = devm_snd_soc_register_component(&i2c->dev,
				...
				ARRAY_SIZE(tfa98xx_dai));
	if(0 == ret){
		printk(KERN_ERR"%s:devm_snd_soc_register_component ret=0 setSpkType \n",__func__);//for test
