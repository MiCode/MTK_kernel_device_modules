...
void tfa98xx_set_exttemp(struct tfa_device *tfa, short ext_temp)
{
	if ((-256 <= ext_temp) && (ext_temp <= 255)) {
		...
		pr_info("Using ext temp %d C\n", twos(ext_temp));
		TFA_SET_BF(tfa, TROS, 1);
		TFA_SET_BF(tfa, EXTTS, twos(ext_temp));
	}
	else {
		pr_info("Clearing ext temp settings\n");
...
enum Tfa98xx_Error tfa_dsp_msg(struct tfa_device *tfa, int length24, const char *buf24)
{
	...
	if (tfa->ext_dsp == 1) {
		...
		if (error == Tfa98xx_Error_Buffer_too_small) {
			...
			if (tfa->verbose) {
				pr_info("Multi-message buffer full. Sending multi-message, length=%d \n", len);
			}
		...
		if (lastmessage == 1) {
			...
			if (tfa->verbose)
				pr_info("Last message for the multi-message received. Multi-message length=%d \n", length);
	...
	if ((tfa->verbose & 0x04) != 0) {
		pr_info("DSP w [%d]: ", length);
		for (i = 0; i < length; i++)
			pr_info("0x%02x ", (uint8_t)buf[i]);
		pr_info("\n");
	}
...
enum Tfa98xx_Error tfa_dsp_msg_read(struct tfa_device *tfa, int length24, unsigned char *bytes24)
{
	...
	/* DSP verbose has argument 0x04 */
	if ((tfa->verbose & 0x04) != 0) {
		pr_info("DSP R [%d]: ", length);
		for (i = 0; i < length; i++)
			pr_info("0x%02x ", (uint8_t)bytes[i]);
		pr_info("\n");
	}
	...
	unsigned char data[])
{
	...
	if (num_bytes <= 0) {
		pr_info("Error: The number of READ bytes is smaller or equal to 0! \n");
		return Tfa98xx_Error_Fail;
	}
...
enum Tfa98xx_Error tfa_show_current_state(struct tfa_device *tfa)
{
	...
	if (tfa->tfa_family == 2) { // && tfa->verbose
		...
		switch (manstate) {
		case 0: pr_info("power_down_state \n");
			break;
		case 1: pr_info("wait_for_source_settings_state \n");
			break;
		case 2: pr_info("connnect_pll_input_state \n");
			break;
		case 3: pr_info("disconnect_pll_input_state \n");
			break;
		case 4: pr_info("enable_pll_state \n");
			break;
		case 5: pr_info("enable_cgu_state \n");
			break;
		case 6: pr_info("init_cf_state \n");
			break;
		case 7: pr_info("enable_amplifier_state \n");
			break;
		case 8: pr_info("alarm_state \n");
			break;
		case 9: pr_info("operating_state \n");
			break;
		case 10: pr_info("mute_audio_state \n");
			break;
		case 11: pr_info("disable_cgu_pll_state \n");
			break;
		default:
...
enum Tfa98xx_Error tfaGetFwApiVersion(struct tfa_device *tfa, unsigned char *pFirmwareVersion)
{
	...
	if (!tfa->is_probus_device)
	{
		...
		if (err) {
			pr_info("%s Error: Unable to get API Version from DSP \n", __FUNCTION__);
			return err;
		}
...
enum Tfa98xx_Error tfaRunSpeakerStartup(struct tfa_device *tfa, int force, int profile)
{
	...
	if (err) {
		pr_info("[%s] cannot get FWAPI error = %d \n", __FUNCTION__, err);
		return err;
	}*/
	/* write all the files from the device list */
	err = tfaContWriteFiles(tfa);
	if (err) {
		pr_info("[%s] tfaContWriteFiles error = %d \n", __FUNCTION__, err);
		return err;
	}
	...
	if (err) {
		pr_info("[%s] tfaContWriteFilesProf error = %d \n", __FUNCTION__, err);
		return err;
	}
...
static void individual_calibration_results(struct tfa_device *tfa)
{
	...
	if (value_P != 1 && value_S != 1)
		pr_info("Calibration failed on both channels! \n");
	else if (value_P != 1) {
		pr_info("Calibration failed on Primary (Left) channel! \n");
		TFA_SET_BF_VOLATILE(tfa, SSLEFTE, 0); /* Disable the sound for the left speaker */
	}
	else if (value_S != 1) {
		pr_info("Calibration failed on Secondary (Right) channel! \n");
...
enum Tfa98xx_Error tfaRunWaitCalibration(struct tfa_device *tfa, int *calibrateDone)
{
	...
	*calibrateDone = 0;
	...
	if (*calibrateDone != 1) {
		...
		pr_info("Calibration has timedout! \n");
...
enum tfa_error tfa_dev_start(struct tfa_device *tfa, int next_profile, int vstep)
{
	...
	if (strstr(tfaContProfileName(tfa->cnt, tfa->dev_idx, next_profile), ".standby") != NULL) {
		...
		tfa_dev_stop(tfa);                                          //modify for kernel6.x
		goto error_exit;
	}
...
error_exit:
	...
	if ((tfa->revid & 0xffff) == 0x3a65 || (tfa->revid & 0xffff) == 0x3a64){
		...
		pr_info("\nInfo: 0xf090 = %d line:%d \n", tfa_get_bf(tfa, 0xf090), __LINE__);
	}
	if ((tfa->rev & 0xff) == 0x66 &&  (strstr(tfaContProfileName(tfa->cnt, tfa->dev_idx, next_profile), ".standby") == NULL)) {
			err = tfa_set_bf(tfa, 0xc050, 0);
			pr_info("\nInfo: 0xc050 = %d line:%d \n", tfa_get_bf(tfa, 0xc050), __LINE__);
	}
	...
	return tfa_error_ok;                                          //modify kernel6.x
}
...
enum Tfa98xx_Error tfa_dsp_partial_coefficients(struct tfa_device *tfa, uint8_t *prev, uint8_t *next)
{
	...
	for (eq = 0; eq < NR_EQ; eq++) {
		...
		pr_info("eq_biquad_mask[%d] = 0x%.8x\n", eq, eq_biquad_mask[eq]);

		eq_offset += eq_biquads[eq];
	}

	pr_info("cost for writing all coefficients     = %d\n", old_cost);
	pr_info("cost for writing changed coefficients = %d\n", new_cost);
...
{
	enum Tfa98xx_Error error = Tfa98xx_Error_Ok;                             //modify kernel6.x
	int loop = 50, ready = 0;
	int count;

	pr_info("tfa_dev_set_state()   state=%d    is_calibration=%d\n", state, is_calibration);
	/* Base states */
	...
	case TFA_STATE_INIT_CF:      /* coolflux HW access possible (~initcf) */
		...
		do {
			error = tfa98xx_dsp_system_stable(tfa, &ready);
			if (error != Tfa98xx_Error_Ok)
				return tfa_error_max;                                        //modify kernel6.x
			if (ready)
				break;
		} while (loop--);
		...
		if (((!tfa->is_probus_device) && (is_calibration)) || ((tfa->rev & 0xff) == 0x13))
		{
			...
			error = tfa98xx_faim_protect(tfa, 1);                           //modify kernel6.x
			if (tfa->verbose) {
				pr_info("FAIM enabled (err:%d).\n", error);
			}
		}
	...
	case TFA_STATE_OPERATING:    /* Amp and Algo running */
		...
		if (((!tfa->is_probus_device) && (is_calibration)) || ((tfa->rev & 0xff) == 0x13))
		{
			error = tfa98xx_faim_protect(tfa, 0);                         //modify kernel6.x
			if (tfa->verbose) {
				pr_info("FAIM disabled (err:%d).\n", error);
			}
		}
		...
		if (tfa->sync_iv_delay) {
			if (tfa->verbose)
				pr_info("syncing I/V delay for %x\n",
				(tfa->rev & 0xff));
...
int tfa_dev_mtp_get(struct tfa_device *tfa, enum tfa_mtp item)
{
	...
	case TFA_MTP_RE25_SEC:
		if ((tfa->rev & 0xFF) == 0x88) {
			...
			pr_info("Error: Current device has no secondary Re25 channel \n");
		}
		break;
	case TFA_MTP_LOCK:
		break;
...
enum tfa_error tfa_dev_mtp_set(struct tfa_device *tfa, enum tfa_mtp item, int value)
{
	...
	switch (item) {
	case TFA_MTP_OTC:
		error = tfa98xx_set_mtp(tfa, (uint16_t)value, TFA98XX_KEY2_PROTECTED_MTP0_MTPOTC_MSK);
		break;
	case TFA_MTP_EX:
		error = tfa98xx_set_mtp(tfa, (uint16_t)value, TFA98XX_KEY2_PROTECTED_MTP0_MTPEX_MSK);
		break;
	case TFA_MTP_RE25:
	case TFA_MTP_RE25_PRIM:
		if (tfa->tfa_family == 2) {
	...
	case TFA_MTP_RE25_SEC:
		if ((tfa->rev & 0xFF) == 0x88) {
			...
			pr_info("Error: Current device has no secondary Re25 channel \n");
			error = Tfa98xx_Error_Bad_Parameter;                                //modify kernel6.x
		}
		break;
	case TFA_MTP_LOCK:
		break;
...
void tfa_adapt_noisemode(struct tfa_device *tfa)
{
	...
	{
		if (tfa->verbose)
			pr_info("Adapting low noise mode is not needed, condition not fulfilled!\n");
		return;
	}
	if (tfa->verbose)
		pr_info("Adapting low noise mode\n");
