		...
		struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
	...
	ret = cs40l26_component_set_sysclk(dai->component, 0, 0, 6144000, 0);
#else
...
static struct snd_soc_dai_driver cs40l26_dummy_dai[] = {
		{
			.name = "cs40l26-pcm",
			.id = 0,
			.playback = {
	...
	static const struct snd_soc_component_driver soc_dummy_codec_dev_cs40l26 = {
		.probe = NULL,
	};
...
static int cs40l26_codec_driver_probe(struct platform_device *pdev)
{
	...
	if (!cs40l26->vibe_init_success) {
		ret = snd_soc_register_component(&pdev->dev, &soc_dummy_codec_dev_cs40l26,cs40l26_dummy_dai, ARRAY_SIZE(cs40l26_dummy_dai));
		if (ret) {
			dev_err(&pdev->dev, "%s: Failed to register dummy codec: %d\n", __func__, ret);
		}
		return ret;
	}
