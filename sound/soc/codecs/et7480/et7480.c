...
static int uart_printk_disable = 1;
...
extern bool is_dp_sw_connect(void);
	...
	if (et_priv && et_priv->tcpc_dev) {
		...
		if (TYPEC_ATTACHED_AUDIO == tcpm_inquire_typec_attach_state(et_priv->tcpc_dev)) {
			...
			if (et_priv->dio4480) {
				regmap_write(et_priv->regmap, ET7480_RESET, 0x01);
				mdelay(1);
			        dev_info(et_priv->dev,"%s: is_dp_sw_connect:%d\n", __func__, is_dp_sw_connect());
				if (uart_printk_disable && !is_dp_sw_connect()) {
					regmap_write(et_priv->regmap, ET7480_SWITCH_SETTINGS, 0x98);
					regmap_write(et_priv->regmap, ET7480_SWITCH_CONTROL, 0x18);
				} else {
					if (is_dp_sw_connect() && (mtk_dp_aux_swap_enable_get() == 1) &&
						(et_priv == global_et7480_data)) {
						regmap_write(et_priv->regmap, ET7480_SWITCH_CONTROL, 0x18);
						regmap_write(et_priv->regmap, ET7480_SWITCH_SETTINGS, 0xF8);
					} else if (is_dp_sw_connect() && (mtk_dp_aux_swap_enable_get() == 0) &&
						(et_priv == global_et7480_data)) {
						regmap_write(et_priv->regmap, ET7480_SWITCH_CONTROL, 0x78);
						regmap_write(et_priv->regmap, ET7480_SWITCH_SETTINGS, 0xF8);
					} else {
						regmap_write(et_priv->regmap, ET7480_SWITCH_SETTINGS, 0xf8);
						regmap_write(et_priv->regmap, ET7480_SWITCH_CONTROL, 0x18);
...
static int dio4485_dp_aux_pn_swap(struct dio4485_dp_aux *ps)
{
	...
	if (ps->orientation_usb == TYPEC_ORIENTATION_REVERSE) {
		...
		if (uart_printk_disable) {
			ret |= regmap_write(global_et7480_data->regmap, ET7480_SWITCH_SETTINGS, 0x98);
		} else {
			ret |= regmap_write(global_et7480_data->regmap, ET7480_SWITCH_SETTINGS, 0xF8);
		}
	}
...
static char __et7480_cmdline[COMMAND_LINE_SIZE];
static char *et7480_cmdline = __et7480_cmdline;

const char *et7480_get_cmd(void)
{
	struct device_node *of_chosen = NULL;
	char *bootargs = NULL;

	if (__et7480_cmdline[0] != 0)
		return et7480_cmdline;
	...
	if (of_chosen) {
		...
		if (!bootargs)
			...
			strlcpy(__et7480_cmdline, bootargs, COMMAND_LINE_SIZE);
...
static int et7480_probe(struct i2c_client *i2c)
{
	...
	char *et7480_cmdline_ptr = NULL;

	et7480_cmdline_ptr = strstr(et7480_get_cmd(), "et7480.disable_uart=1");
	if (et7480_cmdline_ptr == 0) {
		uart_printk_disable = false;
	}
	dev_info(&i2c->dev,"%s: et7480.disable_uart=%d \n", __func__, uart_printk_disable);
