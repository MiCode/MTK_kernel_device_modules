/*
* oca72xxx_bin_parse.c
*
* Copyright (c) 2020 OCS Technology CO., LTD
*
...
#define OCS_CODE_VERSION "V0.0.7-V1.0.4"	/* "code version"-"excel version" */

#define DEBUG_LOG_LEVEL
#ifdef DEBUG_LOG_LEVEL
#define DBG(fmt, arg...)   do {\
printk("OCS_BIN %s,line= %d,"fmt, __func__, __LINE__, ##arg);\
} while (0)
#define DBG_ERR(fmt, arg...)   do {\
printk("OCS_BIN_ERR %s,line= %d,"fmt, __func__, __LINE__, ##arg);\
} while (0)
#else
...
typedef unsigned short int oca_uint16;
typedef unsigned long int oca_uint32;

#define BigLittleSwap16(A)	((((oca_uint16)(A) & 0xff00) >> 8) | \
				 (((oca_uint16)(A) & 0x00ff) << 8))

#define BigLittleSwap32(A)	((((oca_uint32)(A) & 0xff000000) >> 24) | \
				(((oca_uint32)(A) & 0x00ff0000) >> 8) | \
				(((oca_uint32)(A) & 0x0000ff00) << 8) | \
				(((oca_uint32)(A) & 0x000000ff) << 24))


static int oca_parse_bin_header_1_0_0(struct oca_bin *bin);
...
*
********************************************************/
static int oca_check_sum(struct oca_bin *bin, int bin_num)
{
	unsigned int i = 0;
	...
	DBG("oca_bin_parse p_check_sum = %p\n", p_check_sum);
	...
	DBG("oca_bin_parse bin_num=%d, check_sum = 0x%x, sum_data = 0x%x\n",
		bin_num, check_sum, sum_data);
	if (sum_data != check_sum) {
		p_check_sum = NULL;
		DBG_ERR("oca_bin_parse check sum or check bin data len error\n");
		DBG_ERR("oca_bin_parse bin_num=%d, check_sum = 0x%x, sum_data = 0x%x\n", bin_num, check_sum, sum_data);
		return -3;
	}
	p_check_sum = NULL;
...
static int oca_check_data_version(struct oca_bin *bin, int bin_num)
{
	int i = 0;
	DBG("enter\n");
	...
	DBG_ERR("oca_bin_parse Unrecognized this bin data version\n");
	return -4;
}

static int oca_check_register_num_v1(struct oca_bin *bin, int bin_num)
{
	...
	DBG
	    ("oca_bin_parse bin_num=%d, parse_register_num = 0x%x, check_register_num = 0x%x\n",
	     bin_num, parse_register_num, check_register_num);
	if (parse_register_num != check_register_num) {
		p_check_sum = NULL;
		DBG_ERR("oca_bin_parse register num is error\n");
		DBG_ERR("oca_bin_parse bin_num=%d, parse_register_num = 0x%x, check_register_num = 0x%x\n", bin_num, parse_register_num, check_register_num);
		return -5;
	}
...
static int oca_check_dsp_reg_num_v1(struct oca_bin *bin, int bin_num)
{
	...
	DBG
	    ("oca_bin_parse bin_num=%d, parse_dsp_reg_num = 0x%x, check_dsp_reg_num = 0x%x\n",
	     bin_num, parse_dsp_reg_num, check_dsp_reg_num);
	if (parse_dsp_reg_num != check_dsp_reg_num) {
		p_check_sum = NULL;
		DBG_ERR("oca_bin_parse dsp reg num is error\n");
		DBG_ERR("oca_bin_parse bin_num=%d, parse_dsp_reg_num = 0x%x, check_dsp_reg_num = 0x%x\n", bin_num, parse_dsp_reg_num, check_dsp_reg_num);
		return -6;
	}
...
static int oca_check_soc_app_num_v1(struct oca_bin *bin, int bin_num)
{
	...
	DBG
	    ("oca_bin_parse bin_num=%d, parse_soc_app_num = 0x%x, check_soc_app_num = 0x%x\n",
	     bin_num, parse_soc_app_num, check_soc_app_num);
	if (parse_soc_app_num != check_soc_app_num) {
		p_check_sum = NULL;
		DBG_ERR("oca_bin_parse soc app num is error\n");
		DBG_ERR("oca_bin_parse bin_num=%d, parse_soc_app_num = 0x%x, check_soc_app_num = 0x%x\n", bin_num, parse_soc_app_num, check_soc_app_num);
		return -7;
	}
...
***
************************/
static void oca_get_single_bin_header_1_0_0(struct oca_bin *bin)
{
	int i;
	DBG("enter %s\n", __func__);
...
static int oca_parse_each_of_multi_bins_1_0_0(unsigned int bin_num, int bin_serial_num,
				      struct oca_bin *bin)
{
	int ret = 0;
	unsigned int bin_start_addr = 0;
	unsigned int valid_data_len = 0;
	DBG("oca_bin_parse enter multi bin branch -- %s\n", __func__);
	if (!bin_serial_num) {
	...
	ret = oca_parse_bin_header_1_0_0(bin);
	return ret;
}

/* Get the number of bins in multi bins, and set a for loop, loop processing each bin data */
static int oca_get_multi_bin_header_1_0_0(struct oca_bin *bin)
{
	int i = 0;
	int ret = 0;
	...
	oca_get_single_bin_header_1_0_0(bin);

	for (i = 0; i < bin_num; i++) {
		DBG("oca_bin_parse enter multi bin for is %d\n", i);
		ret = oca_parse_each_of_multi_bins_1_0_0(bin_num, i, bin);
		if (ret < 0) {
			return ret;
		}
	}
...
static int oca_parse_bin_header_1_0_0(struct oca_bin *bin)
{
	...
	bin_data_type = GET_32_DATA(*(bin->p_addr + 11),
				    *(bin->p_addr + 10),
				    *(bin->p_addr + 9), *(bin->p_addr + 8));
	DBG("oca_bin_parse bin_data_type 0x%x\n", bin_data_type);
	...
	case DATA_TYPE_SOC_APP:
		...
		DBG("oca_bin_parse enter single bin branch\n");
	...
	case DATA_TYPE_MULTI_BINS:
		...
		DBG("oca_bin_parse enter multi bin branch\n");
		bin->multi_bin_parse_num += 1;
		DBG("%s bin->multi_bin_parse_num is %d\n", __func__,
			bin->multi_bin_parse_num);
		ret = oca_get_multi_bin_header_1_0_0(bin);
		if (ret < 0) {
			return ret;
		}
		break;
	default:
		DBG_ERR("oca_bin_parse Unrecognized this bin data type\n");
		return -2;
	}
	return 0;
}
...
static int oca_check_bin_header_version(struct oca_bin *bin)
{
	int ret = 0;
	...
	DBG("oca_bin_parse header_version 0x%x\n", header_version);
	...
	case HEADER_VERSION_1_0_0:
		...
		return ret;
	default:
		DBG_ERR("oca_bin_parse Unrecognized this bin header version \n");
		return -1;
	}
}

int oca72xxx_parsing_bin_file(struct oca_bin *bin)
{
	int i = 0;
	int ret = 0;

	DBG("oca_bin_parse code version:%s\n", OCS_CODE_VERSION);
	if (!bin) {
		DBG_ERR("oca_bin_parse bin is NULL\n");
		return -8;
	}
	...
	ret = oca_check_bin_header_version(bin);
	if (ret < 0) {
		DBG_ERR("oca_bin_parse check bin header version error\n");
		return ret;
	}
	bin->p_addr = NULL;
	...
	for (i = 0; i < bin->all_bin_parse_num; i++) {
		/* check sum */
		ret = oca_check_sum(bin, i);
		if (ret < 0) {
			DBG_ERR("oca_bin_parse check sum data error\n");
			return ret;
		}
		/* check bin data version */
		ret = oca_check_data_version(bin, i);
		if (ret < 0) {
			DBG_ERR("oca_bin_parse check data version error\n");
			return ret;
		}
		/* check valid data */
		if (bin->header_info[i].bin_data_ver == DATA_VERSION_V1) {
			...
			if (bin->header_info[i].bin_data_type ==
			    DATA_TYPE_REGISTER) {
				ret = oca_check_register_num_v1(bin, i);
				if (ret < 0) {
					DBG_ERR
					    ("oca_bin_parse check register num error\n");
					return ret;
				}
				/* check dsp reg num */
			} else if (bin->header_info[i].bin_data_type ==
				   DATA_TYPE_DSP_REG) {
				ret = oca_check_dsp_reg_num_v1(bin, i);
				if (ret < 0) {
					DBG_ERR
					    ("oca_bin_parse check dsp reg num error\n");
					return ret;
				}
				/* check soc app num */
			} else if (bin->header_info[i].bin_data_type ==
				   DATA_TYPE_SOC_APP) {
				ret = oca_check_soc_app_num_v1(bin, i);
				if (ret < 0) {
					DBG_ERR
					    ("oca_bin_parse check soc app num error\n");
					return ret;
				}
			} else {
	...
	DBG("oca_bin_parse parsing success\n");
