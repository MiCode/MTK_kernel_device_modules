...
#define OCA_MONITOT_BIN_PARSE_VERSION	"V0.1.0"
 ...
 *
 ***************************************************************************/
static int oca_chip_status_recover(struct oca72xxx *oca72xxx)
{
	int ret = -1;
	struct oca_monitor *monitor = &oca72xxx->monitor;
	char *profile = oca72xxx->current_profile;

	OCA_DEV_LOGD(oca72xxx->dev, "enter");

	ret = oca72xxx_update_profile_esd(oca72xxx, profile);
	if (ret < 0) {
	...
	OCA_DEV_LOGI(oca72xxx->dev, "current prof[%s], dev_index[%d] ",
			profile, oca72xxx->dev_index);

	monitor->pre_vmax = OCA_VMAX_INIT_VAL;
	monitor->first_entry = OCA_FIRST_ENTRY;
	monitor->timer_cnt = 0;
	monitor->vbat_sum = 0;
...
static int oca_monitor_chip_esd_check_work(struct oca72xxx *oca72xxx)
{
	int ret = 0;
	int i = 0;

	for (i = 0; i < REG_STATUS_CHECK_MAX; i++) {
		OCA_DEV_LOGD(oca72xxx->dev, "reg_status_check[%d]", i);

		ret = oca72xxx_dev_esd_reg_status_check(&oca72xxx->oca_dev);
		if (ret < 0) {
			oca_chip_status_recover(oca72xxx);
		} else {
			OCA_DEV_LOGD(oca72xxx->dev, "chip status check succeed");
			break;
		}
		msleep(OCA_ESD_CHECK_DELAY);
	}

	if (ret < 0) {
		OCA_DEV_LOGE(oca72xxx->dev, "chip status recover failed,chip off");
		oca72xxx_update_profile_esd(oca72xxx, oca72xxx->prof_off_name);
		return ret;
	}
...
/***************************************************************************
 *
 * oca72xxx monitor work with dsp
 *
 ***************************************************************************/
static int oca_monitor_update_vmax_to_dsp(struct device *dev,
				struct oca_monitor *monitor, int vmax_set)
{
	int ret = -1;
	uint32_t enable = 0;

	if (monitor->pre_vmax != vmax_set) {
		ret = oca72xxx_dsp_get_rx_module_enable(&enable);
		if (!enable || ret < 0) {
			OCA_DEV_LOGE(dev, "get rx failed or rx disable, ret=%d, enable=%d",
				ret, enable);
			return -EPERM;
		}

		ret = oca72xxx_dsp_set_vmax(vmax_set, monitor->dev_index);
		if (ret) {
			OCA_DEV_LOGE(dev, "set dsp msg fail, ret=%d", ret);
			return ret;
		}

		OCA_DEV_LOGI(dev, "set dsp vmax=0x%x sucess", vmax_set);
		monitor->pre_vmax = vmax_set;
	} else {
		OCA_DEV_LOGI(dev, "vmax=0x%x no change", vmax_set);
	}

	return 0;
}

static void oca_monitor_with_dsp_vmax_work(struct device *dev,
					struct oca_monitor *monitor)
{
	int ret = -1;
	int vmax_set = 0;
	...
	OCA_DEV_LOGD(dev, "enter with dsp monitor");
	...
	if (monitor->timer_cnt < monitor_hdr->monitor_count) {
		...
		monitor->vbat_sum += vbat_capacity;
			OCA_DEV_LOGI(dev, "timer_cnt = %d",
			monitor->timer_cnt);
	}
	if ((monitor->timer_cnt >= monitor_hdr->monitor_count) ||
	    (monitor->first_entry == OCA_FIRST_ENTRY)) {
		if (monitor->first_entry == OCA_FIRST_ENTRY)
			monitor->first_entry = OCA_NOT_FIRST_ENTRY;
		ave_capacity = monitor->vbat_sum / monitor->timer_cnt;

		if (monitor->custom_capacity)
			ave_capacity = monitor->custom_capacity;

		OCA_DEV_LOGI(dev, "get average capacity = %d", ave_capacity);
		...
		if (ret < 0)
			...
			oca_monitor_update_vmax_to_dsp(dev, monitor, vmax_set);
...
static void oca_monitor_work_func(struct work_struct *work)
{
	int ret = 0;
	struct oca72xxx *oca72xxx = container_of(work,
				struct oca72xxx, monitor.with_dsp_work.work);
	...
	if (monitor->esd_enable) {
		ret = oca_monitor_chip_esd_check_work(oca72xxx);
		if (ret < 0)
			return;
	}

	if (monitor_hdr->monitor_switch && !(oca72xxx->oca_dev.is_rec_mode) &&
		monitor->open_dsp_en && monitor->bin_status == OCA_ACF_UPDATE) {
		OCA_DEV_LOGD(dev, "start low power protection");
		oca_monitor_with_dsp_vmax_work(dev, monitor);
	}

	if (monitor->esd_enable || (monitor_hdr->monitor_switch &&
		!(oca72xxx->oca_dev.is_rec_mode) && monitor->open_dsp_en &&
		monitor->bin_status == OCA_ACF_UPDATE)) {
...
void oca72xxx_monitor_start(struct oca_monitor *monitor)
{
	...
	ret = oca72xxx_dev_check_reg_is_rec_mode(&oca72xxx->oca_dev);
	if (ret < 0) {
		OCA_DEV_LOGE(oca72xxx->dev, "get reg current mode failed");
		return;
	}

	OCA_DEV_LOGI(oca72xxx->dev,
		"esd_enable:%d, monitor_switch:%d, monitor_open_dsp_en:%d, is_rec_mode:%d",
			monitor->esd_enable, monitor->monitor_hdr.monitor_switch,
			monitor->open_dsp_en, oca72xxx->oca_dev.is_rec_mode);
 ...
 *
 ***************************************************************************/
static ssize_t oca_attr_get_esd_enable(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	...
	if (monitor->esd_enable) {
		OCA_DEV_LOGI(oca72xxx->dev, "esd-enable=true");
		len += snprintf(buf + len, PAGE_SIZE - len,
			"esd-enable=true\n");
	} else {
		OCA_DEV_LOGI(oca72xxx->dev, "esd-enable=false");
		len += snprintf(buf + len, PAGE_SIZE - len,
			"esd-enable=false\n");
	}
...
static ssize_t oca_attr_set_esd_enable(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t len)
{
	char esd_enable[OCA_ESD_ENABLE_STRLEN] = {0};
	struct oca72xxx *oca72xxx = dev_get_drvdata(dev);
	struct oca_monitor *monitor = &oca72xxx->monitor;

	if (strlen(buf) > OCA_ESD_ENABLE_STRLEN) {
		OCA_DEV_LOGE(oca72xxx->dev, "input esd_enable_str_len is out of max[%d]",
				OCA_ESD_ENABLE_STRLEN);
		return -EINVAL;
	}

	if (sscanf(buf, "%s", esd_enable) == 1) {
		OCA_DEV_LOGD(oca72xxx->dev, "input esd-enable=[%s]", esd_enable);
		if (!strcmp(esd_enable, "true"))
			monitor->esd_enable = OCA_ESD_ENABLE;
		else
			monitor->esd_enable = OCA_ESD_DISABLE;
		OCA_DEV_LOGI(dev, "set esd-enable=[%s]",
				monitor->esd_enable ? "true" : "false");
	} else {
		OCA_DEV_LOGE(oca72xxx->dev, "input esd-enable error");
		return -EINVAL;
	}
			...
			struct device_attribute *attr, char *buf)
{
	...
	if (monitor->open_dsp_en) {
		ret = oca72xxx_dsp_get_vmax(&vmax_get, oca72xxx->dev_index);
		if (ret < 0) {
			OCA_DEV_LOGE(oca72xxx->dev,
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (monitor->open_dsp_en) {
		ret = oca72xxx_dsp_set_vmax(vmax_set, oca72xxx->dev_index);
		if (ret < 0) {
			OCA_DEV_LOGE(oca72xxx->dev, "send dsp_msg error, ret = %d",
				ret);
			return ret;
		}
		msleep(2);
...
static ssize_t oca_attr_get_monitor_switch(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	ssize_t len = 0;
	struct oca72xxx *oca72xxx = dev_get_drvdata(dev);
...
static ssize_t oca_attr_set_monitor_switch(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	uint32_t enable = 0;
	int ret = -1;
	...
	ret = oca72xxx_dev_monitor_switch_set(monitor, enable);
	if (ret)
		return ret;
...
static ssize_t oca_attr_get_monitor_time(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	ssize_t len = 0;
	struct oca72xxx *oca72xxx = dev_get_drvdata(dev);
	...
	len += snprintf(buf + len, PAGE_SIZE - len,
			"oca_monitor_timer = %u(ms)\n",
			monitor_hdr->monitor_time);
	return len;
}

static ssize_t oca_attr_set_monitor_time(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	OCA_DEV_LOGI(oca72xxx->dev, "input monitor timer=%d(ms)", timer_val);
	...
	if (timer_val != monitor_hdr->monitor_time)
		...
		OCA_DEV_LOGI(oca72xxx->dev, "no_change monitor_time");

	return count;
}

static ssize_t oca_attr_get_monitor_count(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	ssize_t len = 0;
	struct oca72xxx *oca72xxx = dev_get_drvdata(dev);
...
static ssize_t oca_attr_set_monitor_count(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (ret < 0)
		return ret;
	OCA_DEV_LOGI(oca72xxx->dev, "input monitor count=%d", monitor_count);
	...
	if (monitor_count != monitor_hdr->monitor_count)
		...
		OCA_DEV_LOGI(oca72xxx->dev, "no_change monitor_count");

	return count;
}


static ssize_t oca_attr_get_rx(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	struct oca72xxx *oca72xxx = dev_get_drvdata(dev);
	struct oca_monitor *monitor = &oca72xxx->monitor;
	...
	if (monitor->open_dsp_en) {
		ret = oca72xxx_dsp_get_rx_module_enable(&enable);
		if (ret) {
			OCA_DEV_LOGE(oca72xxx->dev, "dsp_msg error, ret=%d", ret);
			return ret;
		}
		len += snprintf(buf + len, PAGE_SIZE - len,
			"oca72xxx rx: %u\n", enable);
...
static ssize_t oca_attr_set_rx(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct oca72xxx *oca72xxx = dev_get_drvdata(dev);
	struct oca_monitor *monitor = &oca72xxx->monitor;
	...
	if (monitor->open_dsp_en) {
		OCA_DEV_LOGI(oca72xxx->dev, "set rx enable=%d", enable);

		ret = oca72xxx_dsp_set_rx_module_enable(enable);
		if (ret < 0) {
			OCA_DEV_LOGE(oca72xxx->dev, "dsp_msg error, ret=%d",
				ret);
			return ret;
		}
	} else {
		OCA_DEV_LOGE(oca72xxx->dev, "command is invalid");
		return -EINVAL;
	}
...
static DEVICE_ATTR(esd_enable, S_IWUSR | S_IRUGO,
	oca_attr_get_esd_enable, oca_attr_set_esd_enable);
...
static DEVICE_ATTR(monitor, S_IWUSR | S_IRUGO,
	oca_attr_get_monitor_switch, oca_attr_set_monitor_switch);
static DEVICE_ATTR(monitor_time, S_IWUSR | S_IRUGO,
	oca_attr_get_monitor_time, oca_attr_set_monitor_time);
static DEVICE_ATTR(monitor_count, S_IWUSR | S_IRUGO,
	oca_attr_get_monitor_count, oca_attr_set_monitor_count);
static DEVICE_ATTR(rx, S_IWUSR | S_IRUGO,
	oca_attr_get_rx, oca_attr_set_rx);
...
static struct attribute *oca_monitor_control[] = {
...
static struct attribute_group oca_monitor_control_group = {
	.attrs = oca_monitor_control,
};
 ...
 *
 ***************************************************************************/
static void oca_monitor_dtsi_parse(struct device *dev,
				struct oca_monitor *monitor,
				struct device_node *dev_node)
{
	...
	if (ret < 0) {
		OCA_DEV_LOGI(dev, "esd_enable parse failed, user default[enable]");
		monitor->esd_enable = OCA_ESD_ENABLE;
	} else {
		...
		OCA_DEV_LOGI(dev, "parse esd-enable=[%s]",
				monitor->esd_enable ? "true" : "false");
	}
}
				...
				struct device_node *dev_node)
{
	...
	oca_monitor_dtsi_parse(dev, monitor, dev_node);

	/* get platform open dsp type */
	monitor->open_dsp_en = oca72xxx_dsp_isEnable();
	...
	INIT_DELAYED_WORK(&monitor->with_dsp_work, oca_monitor_work_func);

	if (monitor->open_dsp_en) {
		ret = sysfs_create_group(&dev->kobj, &oca_monitor_control_group);
		if (ret < 0)
			OCA_DEV_LOGE(dev, "failed to create monitor dsp control sysfs nodes");
	}
...
void oca72xxx_monitor_exit(struct oca_monitor *monitor)
{
	...
	oca72xxx_monitor_stop(monitor);

	if (monitor->open_dsp_en) {
		sysfs_remove_group(&oca72xxx->dev->kobj,
				&oca_monitor_control_group);
	}
}
