		...
		struct snd_soc_dai *dai)
{
	...
	dev_info(component->dev, "%s, mclk = %d, lrck = %d\n", __func__,
			es8311->mclk_rate, params_rate(params));

	coeff = get_coeff(es8311->mclk_rate, params_rate(params));
	if (coeff < 0) {
		dev_info(component->dev, "Unable to configure sample rate %dHz with %dHz MCLK\n",
			...
			enum snd_soc_bias_level level)
{
	...
	dev_info(component->dev, "Enter into %s(), level = %d\n", __func__, level);
...
static int es8311_set_tristate(struct snd_soc_dai *dai, int tristate)
{
	...
	dev_info(component->dev, "Enter into %s(), tristate = %d\n", __func__, tristate);
...
static int es8311_mute(struct snd_soc_dai *dai, int mute, int direction)
{
	...
	dev_info(component->dev, "Enter into %s(), mute = %d\n", __func__, mute);
...
static int es8311_probe(struct snd_soc_component *component)
{
	...
	if (component == NULL) {
		dev_info(component->dev, "Codec device not registered\n");
		return -ENODEV;
	}
	...
	if (es8311->mclk_src == ES8311_MCLK_PIN) {
		...
		if (IS_ERR(es8311->mclk)) {
			dev_info(component->dev, "%s,unable to get mclk\n", __func__);
			return PTR_ERR(es8311->mclk);
		}
		if (!es8311->mclk)
			dev_info(component->dev, "%s, assuming static mclk\n", __func__);

		ret = clk_prepare_enable(es8311->mclk);
		if (ret) {
			dev_info(component->dev, "%s, unable to enable mclk\n", __func__);
			return ret;
		}
	}
	snd_soc_component_write(component, ES8311_GP_REG45, 0x00);
...
static int es8311_i2c_probe(struct i2c_client *i2c)
{
	struct es8311_private *es8311;
	int ret = 0;
	unsigned int val;

	dev_info(&i2c->dev, "Enter into %s\n", __func__);

	es8311 = devm_kzalloc(&i2c->dev,
	...
	i2c_set_clientdata(i2c, es8311);
	es8311->regmap = devm_regmap_init_i2c(i2c, &es8311_regmap);
	if (IS_ERR(es8311->regmap)) {
		ret = PTR_ERR(es8311->regmap);
		dev_info(&i2c->dev, "regmap_init() failed: %d\n", ret);
		return ret;
	}
	/* verify that we have an es8311 */
	ret = regmap_read(es8311->regmap, ES8311_CHD1_REGFD, &val);
	if (ret < 0) {
		dev_info(&i2c->dev, "failed to read i2c at addr %X\n",
		       i2c->addr);
		return ret;
	}
	/* The first ID should be 0x83 */
	if (val != 0x83) {
		dev_info(&i2c->dev, "device at addr %X is not an es8311\n",
		       i2c->addr);
		return -ENODEV;
	}
	...
	if (ret)
		dev_info(&i2c->dev, "failed to create attr group\n");
