...
static void sia91xx_interrupt(struct work_struct *work)
{
	...
	pr_info("[ info][%s] %s: ch(%d): IntReg(0x%x) IntEn(0x%x) \n", 
		LOG_FLAG, __func__, si_pa->channel_num, IntReg, IntEn);
	...
	pr_info("[ info][%s] %s: ch(%d): Int_Stat(0x%x) \n", 
		LOG_FLAG, __func__, si_pa->channel_num, regVal);
	...
	struct snd_soc_dai *dai)
{
	...
	if (si_pa->mute == SIPA_DEVICE_MUTE_ON) {
		pr_info("[debug][%s] %s: pa is mute on, direct return!",LOG_FLAG, __func__);
		return 0;
	}
...
static int sia91xx_smartpa_soft_mute(sipa_dev_t *si_pa)
{
...
static int sia91xx_smartpa_start(sipa_dev_t *si_pa, int stream)
{
...
int sia91xx_component_probe(struct snd_soc_component *component)
{
	...
	pr_err("[  err][%s] %s: mtk_spk_set_type MTK_SPK_SIA_SIA9306 1\n", LOG_FLAG, __func__);
...
int sia91xx_codec_probe(struct snd_soc_codec *codec)
{
	...
	sia91xx_append_i2c_address(&si_pa->client->dev,
			...
			0);
	mtk_spk_set_type(MTK_SPK_SIA_SIA9306);
	pr_err("[  err][%s] %s: mtk_spk_set_type MTK_SPK_SIA_SIA9306 2\n", LOG_FLAG, __func__);		
	snd_soc_dapm_new_controls(dapm, widgets,
			ARRAY_SIZE(sia91xx_dapm_widgets_common));
	snd_soc_dapm_add_routes(dapm, sia91xx_dapm_routes_common,
			ARRAY_SIZE(sia91xx_dapm_routes_common));
