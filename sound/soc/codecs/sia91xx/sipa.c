	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	pr_debug("[debug][%s] %s: ucontrol = %ld, rst = %d, channel = %d \n",
		LOG_FLAG, __func__, (long)scene, si_pa->rst_pin, si_pa->channel_num);
...
int sipa_vol_config_regmap(sipa_dev_t *si_pa)
{
	int total_gain = 0, signle_gain[3], pwmgain[4] = {0, -1, -3, -7}, inplev_gain[2] = {0, -6}, spkgain;
	unsigned int value, dcin_spkg, pwmconf, inplev, inplev_site;
	int ret, i, j;
	bool increment;

	if (si_pa->chip_type == CHIP_TYPE_SIA9187) {
		ret = regmap_read(si_pa->regmap, SIA9187_REG_SPKGAIN, &value);
		dcin_spkg = value;

		if (0 != ret)
			pr_err("[  err][%s] %s: 0x%02x read err \r\n", LOG_FLAG, __func__, SIA9187_REG_SPKGAIN);

		signle_gain[0] = (value & 0xf) + SIA9187_SPKGAIN_MIX;
		ret = regmap_read(si_pa->regmap, SIA9187_REG_PWMGAIN, &value);
		pwmconf = value;

		if (0 != ret)
			pr_err("[  err][%s] %s: 0x%02x read err \r\n", LOG_FLAG, __func__, SIA9187_REG_PWMGAIN);

		signle_gain[1] = pwmgain[((value & 0x300) >> 8)];
		ret = regmap_read(si_pa->regmap, SIA9187_REG_INPUTGIN, &value);
		inplev = value;

		if (0 != ret)
			pr_err("[  err][%s] %s: 0x%02x read err \r\n", LOG_FLAG, __func__, SIA9187_REG_INPUTGIN);

		signle_gain[2] = ((value & 0x800) >> 11) * (-6);
		total_gain = si_pa->gain;
		pr_debug("[debug][%s] %s: gain0 = %d  gain1 = %d  gain3 = %d\r\n", LOG_FLAG, __func__, signle_gain[0], signle_gain[1],
				 signle_gain[2]);
		spkgain = total_gain - signle_gain[1] - signle_gain[2];

		if ((spkgain < SIA9187_SPKGAIN_MIX) || (spkgain > SIA9187_SPKGAIN_MAX)) {
			if (spkgain > SIA9187_SPKGAIN_MAX)
				increment = true;
			else
				increment = false;

			for (j = 0; j < ARRAY_SIZE(inplev_gain); j++) {
				if (signle_gain[2] == inplev_gain[0]) {
					if (j == 0)
						inplev_site = 0;
					else
						inplev_site = 1;
				...
				spkgain = total_gain - inplev_gain[inplev_site];

				if (increment) {
					for (i = ARRAY_SIZE(pwmgain) -1; i >= 0; i--) {
						if ((spkgain >= (SIA9187_SPKGAIN_MIX + pwmgain[i])) && (spkgain <= (SIA9187_SPKGAIN_MAX + pwmgain[i]))) {
							spkgain -= pwmgain[i];
							pr_debug("[debug][%s] %s: set spkgain: %d pwmgain %d inplev %d total_gain: %d\r\n", LOG_FLAG, __func__, spkgain,
									 pwmgain[i], inplev_gain[inplev_site], total_gain);
							dcin_spkg = (dcin_spkg & 0xFF00) | ((spkgain - SIA9187_SPKGAIN_MIX) << 4) | (spkgain - SIA9187_SPKGAIN_MIX);
							ret = regmap_write(si_pa->regmap, SIA9187_REG_SPKGAIN, dcin_spkg);

							if (0 != ret)
								pr_err("[  err][%s] %s: SIA9187_REG_SPKGAIN regmap_write err \r\n", LOG_FLAG, __func__);

							pwmconf = (pwmconf & 0xFCFF) | (i << 8);
							ret = regmap_write(si_pa->regmap, SIA9187_REG_PWMGAIN, pwmconf);

							if (0 != ret)
								pr_err("[  err][%s] %s: SIA9187_REG_PWMGAIN regmap_write err \r\n", LOG_FLAG, __func__);

							inplev = (inplev & 0xF7FF) | (inplev_site << 11);
							ret = regmap_write(si_pa->regmap, SIA9187_REG_INPUTGIN, inplev);

							if (0 != ret)
								pr_err("[  err][%s] %s: SIA9187_REG_INPUTGIN regmap_write err \r\n", LOG_FLAG, __func__);

							pr_debug("[debug][%s] %s: set spkgain: 0x%02x pwmgain 0x%02x inplev 0x%02x\r\n", LOG_FLAG, __func__, dcin_spkg, pwmconf,
									 inplev);
							return 0;
						}
					}
				} else {
					for (i = 0; i < ARRAY_SIZE(pwmgain); i++) {
						if ((spkgain >= (SIA9187_SPKGAIN_MIX + pwmgain[i])) && (spkgain <= (SIA9187_SPKGAIN_MAX + pwmgain[i]))) {
							spkgain -= pwmgain[i];
							pr_debug("[debug][%s] %s: set spkgain: %d pwmgain %d inplev %d total_gain: %d\r\n", LOG_FLAG, __func__, spkgain,
									 pwmgain[i], inplev_gain[inplev_site], total_gain);
			...
			pr_err("[  err][%s] %s: total_gain set err , range of total_gain should be from %d to %d\r\n", LOG_FLAG, __func__, SIA9187_GAIN_MIX, SIA9187_GAIN_MAX);
		} else {
			pr_debug("[debug][%s] %s: set spkgain: %d  total_gain: %d\r\n", LOG_FLAG, __func__, spkgain, total_gain);
			dcin_spkg = (dcin_spkg & 0xFF00) | ((spkgain - SIA9187_SPKGAIN_MIX) << 4) | (spkgain - SIA9187_SPKGAIN_MIX);
			ret = regmap_write(si_pa->regmap, SIA9187_REG_SPKGAIN, dcin_spkg);
			pr_debug("[debug][%s] %s: set spkgain: 0x%02x\r\n", LOG_FLAG, __func__, dcin_spkg);
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	sipa_dev_t *si_pa = snd_soc_codec_get_drvdata(codec);
#endif
	int total_gain = 0, signle_gain[3], pwmgain[4] = {0, -1, -3, -7};
	unsigned int value;
	int ret;
	...
	if (si_pa->chip_type == CHIP_TYPE_SIA9187) {
		...
		signle_gain[2] = ((value & 0x800) >> 11) * (-6);
		total_gain = signle_gain[0] + signle_gain[1] + signle_gain[2];

		pr_debug("[debug][%s] %s: gain0 = %d  gain1 = %d  gain3 = %d\r\n", LOG_FLAG, __func__, signle_gain[0], signle_gain[1],
				signle_gain[2]);
	}

	ucontrol->value.integer.value[0] = total_gain;
	pr_debug("[debug][%s] %s: total_gain = %ld, channel_num = %d \r\n",
			 LOG_FLAG, __func__, ucontrol->value.integer.value[0], si_pa->channel_num);
	return 0;
}

static int sipa_vol_step_set(
	struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
#if (LINUX_VERSION_CODE > KERNEL_VERSION(4, 16, 28))
	...
	si_pa->ftm_mode = true;
	si_pa->gain = ucontrol->value.integer.value[0];

	if (si_pa->gain < SIA9187_GAIN_MIX || si_pa->gain > SIA9187_GAIN_MAX)
	{
		si_pa->ftm_mode = false;
		pr_err("[  err][%s] %s: gain out of range, it should be from %d to %d\r\n", LOG_FLAG, __func__, SIA9187_GAIN_MIX, SIA9187_GAIN_MAX);
		return 0;
	}

	if (true == sipa_regmap_get_chip_en(si_pa))
	{
		sipa_vol_config_regmap(si_pa);
		si_pa->ftm_mode = false;
	}
...
static int sipa_dynamic_create_controls(sipa_dev_t *si_pa)
{
	...
	snprintf(kctl_name, SIPA_NAME_BUF_MAX, "Spkr_ch_%d Sipa Reg", si_pa->channel_num);

	sipa_dev_control[KCTL_TYPE_REG].name   = kctl_name;
	sipa_dev_control[KCTL_TYPE_REG].iface  = SNDRV_CTL_ELEM_IFACE_MIXER;
	sipa_dev_control[KCTL_TYPE_REG].info   = snd_soc_bytes_info_ext;
	sipa_dev_control[KCTL_TYPE_REG].access = SNDRV_CTL_ELEM_ACCESS_TLV_READWRITE | SNDRV_CTL_ELEM_ACCESS_TLV_CALLBACK;
	sipa_dev_control[KCTL_TYPE_REG].tlv.c  = snd_soc_bytes_tlv_callback;
	sipa_dev_bytes_ext->max = REG_DATA_LEN;
	sipa_dev_bytes_ext->get = sipa_reg_get;
	sipa_dev_bytes_ext->put = sipa_reg_set;
	sipa_dev_control[KCTL_TYPE_REG].private_value = &sipa_dev_bytes_ext;
#endif
...
static const struct snd_kcontrol_new sipa_controls[] = {
	...
	SOC_SINGLE_EXT("Sipa Vol", SND_SOC_NOPM, 0, 100, 0,
		sipa_vol_step_get, sipa_vol_step_set),
};
...
#endif
{
	...
	if (IS_DIGITAL_PA_TYPE(si_pa->chip_type)) {
		...
		set_smartpa_type("sia93xx", sizeof("sia93xx"));
...
static int sipa_probe(struct platform_device *pdev)
{
	...
	mtk_spk_set_type(MTK_SPK_SIA_SIA9306);
	return 0;
...
MODULE_SOFTDEP("pre: snd-soc-aw882xx");
#endif
