	...
	if (is_chip_type_supported(si_pa->chip_type) &&
		...
		if (0 == si_pa->disable_pin) {
			...
			if (IS_DIGITAL_PA_TYPE(si_pa->chip_type) && !IS_DIGITAL_PA_PULL_RST_TYPE(si_pa->chip_type)) {
				...
				pr_debug("[debug][%s] %s: si_pa->chip_type = %d, set rst to low \r\n",
						LOG_FLAG, __func__, si_pa->chip_type);
			} else {
				gpio_set_value(si_pa->rst_pin, SIA81XX_ENABLE_LEVEL);
				pr_debug("[debug][%s] %s: si_pa->chip_type = %d, set rst to high \r\n",
						LOG_FLAG, __func__, si_pa->chip_type);
			}
...
static int sipa_scene_set(struct sipa_dev_s *si_pa, unsigned int scene)
{
	...
	if (IS_DIGITAL_PA_TYPE(si_pa->chip_type)) {
#if PROTECT_ULTRASONIC
		if ((si_pa->power_mode == true) && (last_scene != si_pa->scene)) {
...
#endif
{
	...
	mtk_spk_set_type(MTK_SPK_SIPA_SIA9197);
	...
	struct i2c_client *client)
{
	...
	if (IS_DIGITAL_PA_TYPE(si_pa->chip_type) && gpio_is_valid(si_pa->irq_pin)) {
