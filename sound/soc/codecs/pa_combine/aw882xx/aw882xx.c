...
#include "../fs19xx/fsm_public.h"
#include "../fs19xx/fsm-dev.h"
	...
	unsigned char reg_addr, unsigned char *buf, unsigned int len)
{
	...
	if (data == NULL) {
		aw_dev_err(&i2c->dev, "can not allocate memory");
		return -ENOMEM;
	}
	...
	unsigned char reg_addr, unsigned int reg_data)
{
	...
	if(ret < 0){
		aw_dev_err(&i2c->dev, "retray 5 times,still error,try usleep 3s, i2c_write cnt=%d error=%d",
					cnt, ret);
	...
	unsigned char reg_addr, unsigned int *reg_data)
{
	...
	if(ret < 0){
		aw_dev_err(&i2c->dev, "retray 5 times,still error,try usleep 3s, i2c_read cnt=%d error=%d",
					cnt, ret);
	        usleep_range(AW882XX_RETRY_WAIT_TIME,AW882XX_RETRY_WAIT_TIME + 100);
		ret = aw882xx_i2c_reads(i2c, reg_addr, buf, 2);
		if(ret < 0){
			aw_dev_err(&i2c->dev, "usleep 3s still ereror, i2c_read cnt=%d error=%d",
					cnt, ret);
		}
	}
...
static void aw882xx_request_firmware(struct work_struct *work)
{
	...
	if ((ret) || (!cont)) {
		aw_dev_info(aw882xx->dev, "[TF-FUNC] load [%s] failed!", acf_bin_name);
...
static int fs19xx_read_chipid(struct aw882xx *aw882xx)
{
	...
	while (cnt < AW_READ_CHIPID_RETRIES) {
		ret = aw882xx_i2c_read(aw882xx->i2c, FS19XX_CHIP_ID_REG, &reg_value);
		if (ret < 0) {
