	...
	unsigned char reg_addr, unsigned int *reg_data)
{
	...
	if(ret < 0){
		aw_dev_err(aw882xx->dev, "retray 5 times,still error,try usleep 3s, i2c_read cnt=%d error=%d",
					cnt, ret);
	        usleep_range(AW882XX_RETRY_WAIT_TIME,AW882XX_RETRY_WAIT_TIME + 100);
		ret = aw882xx_i2c_reads(aw882xx, reg_addr, buf, 2);
		if(ret < 0){
			aw_dev_err(aw882xx->dev, "usleep 3s still ereror, i2c_read cnt=%d error=%d",
					cnt, ret);
		}
	}
