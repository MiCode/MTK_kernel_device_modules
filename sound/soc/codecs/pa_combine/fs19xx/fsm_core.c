...
int fsm_parse_preset(const void *data, uint32_t size)
{
	...
	if (data == NULL || size == 0) {
		pr_err("bad parameter, size:%u", size);
		return -EINVAL;
	}
	pfile = fsm_get_presets();
	if (pfile) {
	...
	pr_info("date    : %4u%02u%02u-%02u%02u", hdr->date.year, \
	...
	if (hdr->size == 0 || hdr->size != size) {
		pr_err("invalid size: hdr:%u, fw:%u", hdr->size, size);
...
int fsm_config_vol(fsm_dev_t *fsm_dev)
{
	...
	if (fsm_dev->is19xx) {
		volctrl = 0x16;
		if (HIGH8(fsm_dev->version) == FS1958_DEV_ID){
        	volume = ((cfg->volume  << 1) + 1) << 7;
        } else {
            //for Amazon series, 3FFh(bit 15:6) as maximum
            volume = ((cfg->volume  << 8) | 0x00C0);
        }
	} else {
		volctrl = 0x06;
...
void fsm_set_scene(int scene)
{
	...
	if ((cfg->next_scene == FSM_SCENE_MUSIC) && cfg->ultrasound_on && !cfg->force_calib) {
...
void fsm_speaker_onn(void)
{
	...
	if (cfg->speaker_on && (cfg->next_scene == cfg->cur_scene)) {
		pr_debug("no need repeat to speaker on");
		return;
	}
	...
	if (ret) { // no device or firmware
		...
		fsm_mievent_report(906001354,"PA data exception",NULL);
#endif
		fsm_mutex_unlock();
		return;
	}
...
void fsm_speaker_off(void)
{
	...
	if (!cfg->speaker_on) {
		pr_debug("no need repeat to speaker off");
		return;
	}
