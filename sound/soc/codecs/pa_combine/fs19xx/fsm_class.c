...
static ssize_t fsm_re25_show(const struct class *class,
...
static ssize_t fsm_f0_show(const struct class *class,
				const struct class_attribute *attr, char *buf)
{
...
static ssize_t fsm_num_show(const struct class *class,
				const struct class_attribute *attr, char *buf)
{
	int len = 0;
	len = scnprintf(buf, PAGE_SIZE, "device count: %d\n", fsm_dev_count());
	return len;
}
...
static struct class_attribute g_fsm_class_attrs[] = {
	...
	__ATTR(fsm_num, S_IRUGO, fsm_num_show, NULL),
	__ATTR_NULL
};
