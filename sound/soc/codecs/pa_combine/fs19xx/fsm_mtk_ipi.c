...
static int fsm_afe_send_dtype(int dev_count)
{
	struct fsadsp_cmd_dtype params;
	...
	for (i = 0; i < dev_count; i++) {
		...
		if (HIGH8(fsm_dev->version) != FS19XX_DEV_ID)
			continue;
		idx = fsm_get_index_by_position(fsm_dev->pos_mask);
		if (idx < 0 || idx >= FSM_DEV_MAX)
			continue;
		params.type[idx] = fsm_dev->dtype;
		if (type == 0) {
			type = params.type[idx];
			continue;
		}
		if (type != params.type[idx])
			flags = true;
	}
	...
	afe.module_id = AFE_MODULE_ID_FSADSP_RX;
	afe.port_id = fsm_afe_get_rx_port();
	afe.param_id = CAPI_V2_PARAM_FSADSP_DTYPE;
	afe.op_set = true;

	size = sizeof(struct fsadsp_cmd_dtype);
	ret = fsm_afe_send_apr(&afe, (uint8_t *)&params, size);
	if (ret) {
		pr_err("send apr failed: %d", ret);
		return ret;
	}
...
static int fsm_afe_check_dsp_status(void)
{
	fsm_algo_calib_data_t calib_info[FSM_DEV_MAX];
	...
	ret = fsm_afe_send_apr(&afe, calib_info, FSM_DEV_MAX*sizeof(fsm_algo_calib_data_t));
	if (ret) {
		pr_err("send apr failed:%d", ret);
		return ret;
	}
	...
	for (i = 0; i < pfile->hdr.ndev; i++) {
		pr_info("get f0:%d, r0:%d", calib_info[i].f0, calib_info[i].r0);
		if ((calib_info[i].r0 == -1) || (calib_info[i].f0 == -1) ) {
			return -1;
		}
	}
...
int fsm_afe_mod_ctrl(bool enable)
{
	...
	if (fsm_afe_check_dsp_status()) {
		pr_err("check dsp status failed");
		return -1;
	}
	...
	if (preset->hdr.ndev > 1)
		fsm_afe_send_dtype(preset->hdr.ndev);
