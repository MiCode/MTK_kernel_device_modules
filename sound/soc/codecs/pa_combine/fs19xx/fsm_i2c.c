...
static int __maybe_unused fsm_i2c_probe(struct i2c_client *i2c)
{
	fsm_config_t *cfg = fsm_get_config();
	fsm_dev_t *fsm_dev;
	int ret = 0;
...
static void __maybe_unused fsm_i2c_remove(struct i2c_client *i2c)
{
...
{
#if IS_ENABLED(CONFIG_SND_SOC_FRSM)
	// skipping probe b/c we are
	// using newer version of foursemi driver
	dev_info(&i2c->dev, "%s: skipping probe\n", __func__);
	return -EINVAL;
#else
...
{
#if IS_ENABLED(CONFIG_SND_SOC_FRSM)
	// do nothing b/c we are
	// using newer version of foursemi driver
	dev_info(&i2c->dev, "%s: skipping remove\n", __func__);
#else
	fsm_i2c_remove(i2c);
#endif
}
