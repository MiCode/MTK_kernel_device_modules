...
static int frsm_i2c_set_batt(struct frsm_dev *frsm_dev, struct frsm_argv *argv)
{
	...
	if (frsm_dev->pdata->bsg_volume_v2)
		new_volume = 0; // default

	if (!smask && frsm_dev->batt_vol == new_volume)
		return 0;

	batt = (struct frsm_batt *)argv->buf;
	if (frsm_dev->pdata->bsg_type == 0)
		ret = frsm_get_batt_volume(frsm_dev, FRSM_PROT_BSG,
				batt->cap, &new_volume);
	else
		ret = frsm_get_batt_volume(frsm_dev, FRSM_PROT_BSG,
				batt->batv, &new_volume);
...
static int frsm_i2c_amp_parse_dts(struct frsm_dev *frsm_dev)
{
	...
	ret = of_property_read_u32(frsm_dev->dev->of_node, "bsg-type",
			&frsm_dev->pdata->bsg_type);
	if (ret)
		frsm_dev->pdata->bsg_type = 0;
