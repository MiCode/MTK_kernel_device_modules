...
#if IS_ENABLED(CONFIG_MIEV)
		if (ret) {
			mievent_report(906001354, "PA data exception", frsm_dev->dev);
		}
#endif
...
static int frsm_i2c_dev_init(struct frsm_dev *frsm_dev)
{
	...
	if (ret) {
#if IS_ENABLED(CONFIG_MIEV)
		mievent_report(906001353, "PA i2c exception", frsm_dev->dev);
#endif
		return ret;
	}
	...
	if (ret) {
		...
		if (frsm_dev->pdata->retry_detect) {
			...
			if (ret) {
// not reporting for p10 b/c will try aw882xx later
#if 0 //IS_ENABLED(CONFIG_MIEV)
				mievent_report(906001352, "PA detection exception", frsm_dev->dev);
#endif
				return ret;
			}
		} else {
	...
	if (ret) {
#if IS_ENABLED(CONFIG_MIEV)
		mievent_report(906001351, "PA internal exception", frsm_dev->dev);
#endif
		return ret;
	}
