...
static int frsm_amp_send_bsg_params(struct frsm_amp *frsm_amp)
{
	int payload[FRSM_PAYLOAD_MAX];
	struct frsm_adsp_pkg *pkg;
	...
	if (!test_bit(FRSM_HAS_RX_BSG, &frsm_amp->func))
		return -ENOTSUPP;
...
static int frsm_amp_smooth_batt(struct frsm_amp *frsm_amp)
{
	...
	static struct frsm_batt batt_old;
	...
	if (batt_old.batv == frsm_amp->batt.batv && batt_old.cap == frsm_amp->batt.cap && batt_old.tempr == frsm_amp->batt.tempr)
		return -EAGAIN;

	dev_info(frsm_amp->dev, "smooth batv:%d cap:%d tempr:%d\n",
			frsm_amp->batt.batv,
			frsm_amp->batt.cap,
			frsm_amp->batt.tempr);

	batt_old.batv = frsm_amp->batt.batv;
	batt_old.cap = frsm_amp->batt.cap;
	batt_old.tempr = frsm_amp->batt.tempr;
...
int frsm_amp_prot_battery(struct frsm_amp *frsm_amp, bool restore)
{
	...
	if (test_bit(FRSM_HAS_RX_BSG, &frsm_amp->func))
		ret = frsm_amp_send_bsg_params(frsm_amp);
	else
...
static int frsm_amp_parse_dts(struct frsm_amp *frsm_amp)
{
	...
	if (of_property_read_bool(np, "frsm,has-rx-bsg"))
		set_bit(FRSM_HAS_RX_BSG, &frsm_amp->func);
