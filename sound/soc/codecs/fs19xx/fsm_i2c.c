			...
			const struct i2c_device_id *id)
{
	...
	if(fsm_dev->id == 0) {
		...
		dev_set_name(&i2c->dev, "fs16xx_smartpa_0");
#endif
	...
	mtk_spk_set_type(MTK_SPK_FSM_FS19XX);

	/*add hardware info*/
	hardwareinfo_set_prop(HARDWARE_SMARTPA, "FS1945F");
	fsm_mutex_unlock();
	//mtk_spk_set_type(MTK_SPK_FOURSEMI_FS19XX);
