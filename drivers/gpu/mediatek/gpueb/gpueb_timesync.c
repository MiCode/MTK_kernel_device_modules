...
static void timesync_sync_base_internal(unsigned int flag)
{
	...
	gpueb_pr_info(GPUEB_TAG, "%s update base: ts=%llu, tick=0x%llx, fz=%d, ver=%d",
...
unsigned int gpueb_timesync_init(void)
{
	...
	gpueb_pr_info(GPUEB_TAG, "g_gpueb_ts_mbox = %d", g_gpueb_ts_mbox);
	g_gpueb_ts_mbox_offset_base = gpueb_get_send_PIN_offset_by_name("IPI_ID_TIMER");
	gpueb_pr_info(GPUEB_TAG, "g_gpueb_ts_mbox_offset_base = %d", g_gpueb_ts_mbox_offset_base);

	timesync_workqueue = create_workqueue("gpueb_ts_wq");
	if (!timesync_workqueue) {
		gpueb_pr_info(GPUEB_TAG, "workqueue create failed");
