...
static void __iomem *g_mfg_rpc_grst_base;
...
static void __iomem *g_clk_cfg_update;
static void __iomem *g_cksta_reg;
...
static int __trigger_ghpm_on(void)
{
	...
	/* Turn on CLK_MFG_EB */
	writel(PDN_SDF_BIT, CLK_CKFG_6_CLR);
	writel(PDN_MFG_EB_BIT, CLK_CKFG_6_CLR);
	writel(readl(CLK_CKFG_6_CLR) | MFG_EB_CLK_STATE, CLK_CKFG_6_CLR);
	writel(readl(CLK_CKFG_6_SET) | CLK_MFG_EB_SEL, CLK_CKFG_6_SET);
	writel(readl(CLK_CFG_UPDATE) | MFG_EB_CK_UPDATE_BIT, CLK_CFG_UPDATE);

	i = 0;
	while (readl(CKSTA_REG) & CHG_MFG_EB) {
		udelay(1);
		if (++i > GPUEB_WAIT_TIMEOUT) {
			gpueb_log_e(GHPM_TAG, "Wait CKSTA_REG timeout");
			return GHPM_CLK_EB_CK_ERR;
		}
	}
...
static int __wait_gpueb(enum gpueb_low_power_event event)
{
	...
	if (event == SUSPEND_POWER_ON) {
		...
		if (atomic_read(&g_progress_status) == POWER_OFF_IN_PROGRESS) {
			...
			/* set MFG_EB clk src to 26M */
			gpueb_log_d(GHPM_TAG, "GPUEB start set MFG_EB clk src to 26M");
			writel(readl(CLK_CKFG_6_CLR) | MFG_EB_CLK_STATE, CLK_CKFG_6_CLR);
			writel(readl(CLK_CKFG_6_SET) & ~CLK_MFG_EB_SEL, CLK_CKFG_6_SET);
			writel(readl(CLK_CFG_UPDATE) | MFG_EB_CK_UPDATE_BIT, CLK_CFG_UPDATE);
			gpueb_log_d(GHPM_TAG, "GPUEB start polling CKSTA_REG");
			i = 0;
			while (readl(CKSTA_REG) & CHG_MFG_EB) {
				udelay(1);
				if (++i > GPUEB_WAIT_TIMEOUT) {
					gpueb_log_e(GHPM_TAG, "Wait CKSTA_REG timeout, CKSTA_REG = 0x%x", readl(CKSTA_REG));
					goto wait_err;
				}
			}
			/* Turn off CLK_MFG_EB */
...
static int __ghpm_pdrv_probe(struct platform_device *pdev)
{
	...
	/* get mfg_rpc_grst base address */
	res = platform_get_resource_byname(pdev, IORESOURCE_MEM, "mfg_rpc_grst");
	if (unlikely(!res)) {
		gpueb_log_e(GHPM_TAG, "fail to get resource MFG_VCORE_AO_CONFIG");
		goto done;
	}
	g_mfg_rpc_grst_base = devm_ioremap(ghpm_dev, res->start, resource_size(res));
	if (unlikely(!g_mfg_rpc_grst_base)) {
		gpueb_log_e(GHPM_TAG, "fail to ioremap MFG_VCORE_AO_CONFIG: 0x%llx",
			(unsigned long long) res->start);
		goto done;
	}

	/* get clk_cfg_update base address */
	res = platform_get_resource_byname(pdev, IORESOURCE_MEM, "clk_cfg_update");
	if (unlikely(!res)) {
		gpueb_log_e(GHPM_TAG, "fail to get resource CLK_CFG_UPDATE");
		goto done;
	}
	g_clk_cfg_update = devm_ioremap(ghpm_dev, res->start, resource_size(res));
	if (unlikely(!g_clk_cfg_update)) {
		gpueb_log_e(GHPM_TAG, "fail to ioremap CLK_CFG_UPDATE: 0x%llx",
			(unsigned long long) res->start);
		goto done;
	}

	/* get ck_sta_reg base address */
	res = platform_get_resource_byname(pdev, IORESOURCE_MEM, "ck_sta_reg");
	if (unlikely(!res)) {
		gpueb_log_e(GHPM_TAG, "fail to get resource CKSTA_REG");
		goto done;
	}
	g_cksta_reg = devm_ioremap(ghpm_dev, res->start, resource_size(res));
	if (unlikely(!g_cksta_reg)) {
		gpueb_log_e(GHPM_TAG, "fail to ioremap CKSTA_REG: 0x%llx",
			(unsigned long long) res->start);
		goto done;
	}
