...
static void __mt_gpufreq_vgpu_set_mode(unsigned int mode)
{
	...
	if (ret == 0) {
		GPUFREQ_LOGI("set AUTO_MODE(%d) or PWM_MODE(%d) to PMIC(VGPU), mode = %d\n",
...
static void __mt_gpufreq_volt_switch_without_vsram_volt(unsigned int volt_old, unsigned int volt_new)
{
	...
	GPUFREQ_LOGI("volt_new = %d, volt_old = %d\n", volt_new, volt_old);
 ...
 */
static void _gpufreq_volt_switch_without_vsram_gpu(
		unsigned int vgpu_old, unsigned int vgpu_new)
{
	...
	vsram_gpu_new = __gpufreq_get_vsram_by_vgpu(vgpu_new);
	vsram_gpu_old = __gpufreq_get_vsram_by_vgpu(vgpu_old);

	__gpufreq_volt_scale_gpu(
			vgpu_old,
			vgpu_new,
			vsram_gpu_old,
			vsram_gpu_new);
...
unsigned int mt_gpufreq_update_volt(unsigned int pmic_volt[], unsigned int array_size)
{
	...
	GPUFREQ_LOGI("%s : update OPP table to given voltage\n",__func__);
	...
	if (__gpufreq_get_power_state() && !g_fixed_freq_volt_state) {
		_gpufreq_volt_switch_without_vsram_gpu(
			g_gpu.cur_volt,
			signed_table[g_gpu.cur_oppidx].volt);

		g_gpu.cur_volt = signed_table[g_gpu.cur_oppidx].volt;
		g_gpu.cur_vsram = signed_table[g_gpu.cur_oppidx].vsram;
	}
			...
			unsigned int volt, unsigned int temp)
{
	...
	GPUFREQ_LOGI("idx = %d, p_dynamic = %d, p_leakage = %d, p_total = %d, temp = %d\n",
...
static void __mt_update_gpufreqs_power_table(void)
{
	...
	if ((temp >= -20) && (temp <= 125)) {
		for (i = 0; i < g_gpu.signed_opp_num; i++) {
			...
			GPUFREQ_LOGI("[%d] freq_khz = %d, volt = %d, power = %d\n",
	...
	unsigned int target_volt, enum gpufreq_dvfs_state key)
{
	...
	GPUFREQ_LOGI("begin to custom commit freq: (%d->%d), volt: (%d->%d)",
...
static enum gpufreq_posdiv __gpufreq_get_posdiv_by_freq(unsigned int freq)
{
	 ...
	 */
	enum gpufreq_posdiv posdiv_power = POSDIV_POWER_4;
	...
	unsigned int vsram_old, unsigned int vsram_new)
{
	...
	GPUFREQ_LOGI("begin to scale Vgpu: (%d->%d), Vsram_gpu: (%d->%d)",
		vgpu_old, vgpu_new, vsram_old, vsram_new);
	...
	GPUFREQ_LOGI("Vgpu: %d, Vsram: %d, udelay: %d",
...
static void __gpufreq_apply_adjust(struct gpufreq_adj_info *adj_table, int adj_num)
{
	...
	for (i = 0; i < adj_num; i++) {
		...
		GPUFREQ_LOGI("[%02d*] Freq: %d, Volt: %d, Vsram: %d",
...
static void __gpufreq_update_gpu_working_table(void)
{
	...
	for (i = 0; i < g_gpu.opp_num; i++) {
		...
		GPUFREQ_LOGI("GPU[%02d] Freq: %d, Volt: %d, Vsram: %d, Margin: %d",
...
static unsigned int __calculate_vgpu_sfchg_rate(bool isRising)
{
	...
	GPUFREQ_LOGI("isRising = %d, sfchg_rate_vgpu = %d\n",
...
static unsigned int __calculate_vsram_sfchg_rate(bool isRising)
{
	...
	GPUFREQ_LOGI("isRising = %d, sfchg_rate_vsram = %d\n",
...
static unsigned int __gpufreq_get_real_fgpu(void)
{
	...
	GPUFREQ_LOGI("MFGPLL_CON1 = 0x%x", mfgpll);
...
static unsigned int __gpufreq_get_real_vgpu(void)
{
	...
	GPUFREQ_LOGI("@%s: VGPU curr voltage = %d uV\n",__func__,volt);
...
static unsigned int __gpufreq_get_real_vsram(void)
{
	...
	GPUFREQ_LOGI("@%s: VSRAM curr voltage = %d uV\n",__func__,volt);
...
static void __gpufreq_kick_pbm(int enable)
{
	...
	if (enable) {
		for (i = 0; i < g_gpu.opp_num; i++) {
			if (working_table[i].freq == cur_freq) {
				...
				if (working_table[i].volt == cur_volt) {
					...
					GPUFREQ_LOGI("request GPU power = %d,
						cur_volt = %d uV,
						cur_freq = %d KHz\n",
		...
		if (!found) {
			GPUFREQ_LOGI("tmp_idx = %d\n", tmp_idx);
			if (tmp_idx != -1 && tmp_idx < g_gpu.opp_num) {
				/* use freq to found corresponding power budget */
				power = working_table[tmp_idx].power;
				kicker_pbm_by_gpu(true, power, cur_volt / 100);
				GPUFREQ_LOGI("request GPU power = %d, cur_volt = %d uV,
					cur_freq = %d KHz\n", power, cur_volt * 10, cur_freq);
			} else {
				GPUFREQ_LOGI("Cannot found request power in power table,
...
int __gpufreq_generic_commit_gpu(int target_oppidx, enum gpufreq_dvfs_state key)
{
	...
	GPUFREQ_TRACE_START("target_oppidx=%d, key=%d",
		target_oppidx, key);
	GPUFREQ_LOGI("target_oppidx=%d, key=%d",
		target_oppidx, key);
	...
	GPUFREQ_LOGI("begin to commit GPU OPP index: (%d->%d) freq change: (%d->%d)",
		cur_oppidx, target_oppidx, cur_freq, target_freq);
	GPUFREQ_LOGI("begin to commit volt change: (%d->%d) vsram change: (%d->%d)",
		cur_volt, target_volt, cur_vsram, target_vsram);
...
static void __gpufreq_measure_power(void)
{
	...
	for (i = 0; i < opp_num; i++) {
		...
		GPUFREQ_LOGI("GPU[%02d] power: %d (dynamic: %d, leakage: %d)",
...
static void _gpufreq_setup_opp_power_table(int num)
{
	...
	for (i = 0; i < num; i++) {
		...
		GPUFREQ_LOGI("[%d], freq_khz = %u, volt = %u, power = %u\n",
				i, g_power_table[i].gpufreq_khz,
				g_power_table[i].gpufreq_volt,
				g_power_table[i].gpufreq_power);
	}
...
static void __gpufreq_set_springboard(void)
{
	...
	for (src_idx = 0; src_idx < g_gpu.opp_num; src_idx++) {
		...
		GPUFREQ_LOGI("springboard_up[%02d]: %d",
	...
	for (src_idx = 0; src_idx < g_gpu.opp_num; src_idx++) {
		...
		GPUFREQ_LOGI("springboard_down[%02d]: %d",
...
static int __gpufreq_init_opp_table(struct platform_device *pdev)
{
	...
	GPUFREQ_LOGI("number of signed GPU OPP: %d, upper and lower bound: [%d, %d]",
...
void __gpufreq_check_bus_idle(void)
{
	...
	GPUFREQ_LOGI("check bus idle [Done]\n");
}
...
static int __gpufreq_buck_control(enum gpufreq_power_state power)
{
	...
	GPUFREQ_LOGI("buck control start power=%d", power);
	...
	if (power == GPU_PWR_ON) {
		...
		if (unlikely(ret)) {
			...
			GPUFREQ_LOGI("enable VGPU [SUCCESS]");
		}
		...
		if (unlikely(ret)) {
			...
			GPUFREQ_LOGI("enable VSRAM_GPU [SUCCESS]");
		}
		g_gpu.buck_count++;
	/* power off */
	} else {
		...
		if (unlikely(ret)) {
			...
			GPUFREQ_LOGI("disable VSRAM_GPU [SUCCESS]");
		}
		...
		if (unlikely(ret)) {
			...
			GPUFREQ_LOGI("disable VGPU [SUCCESS]");
		}
		g_gpu.buck_count--;
	}
...
static int __gpufreq_clock_control(enum gpufreq_power_state power)
{
	...
	if (power == GPU_PWR_ON) {
		...
		if (unlikely(ret)) {
			...
			GPUFREQ_LOGI("subsys_mfg_cg enable [SUCCESS]");
		}
...
static int __gpufreq_mtcmos_control(enum gpufreq_power_state power)
{
	...
	GPUFREQ_TRACE_START("power=%d", power);
	GPUFREQ_LOGI("mtcmos control start power=%d", power);
	...
	if (power == GPU_PWR_ON) {
		...
		if (ret) {
			...
			GPUFREQ_LOGI("mtcmos_mfg_async enable [SUCCESS]");
		}
		...
		if (ret) {
			...
			GPUFREQ_LOGI("mtcmos_mfg enable [SUCCESS]");
		}
		if (shader_present & MFG2_SHADER_STACK0) {
			...
			if (ret) {
				...
				GPUFREQ_LOGI("mtcmos_mfg_core0 enable [SUCCESS]");
			}
		}
		if (shader_present & MFG3_SHADER_STACK2) {
			ret = clk_prepare_enable(g_clk->mtcmos_mfg_core1);
			if (ret) {
				__gpufreq_abort("fail to enable mtcmos_mfg_core1 (%d)", ret);
				goto done;
			} else {
				GPUFREQ_LOGI("mtcmos_mfg_core1 enable [SUCCESS]");
			}
...
#endif
		GPUFREQ_LOGI("enable MTCMOS done\n");
...
#endif
		GPUFREQ_LOGI("disable MTCMOS done\n");
		g_gpu.mtcmos_count--;
...
int __gpufreq_power_control(enum gpufreq_power_state power)
{
	...
	GPUFREQ_LOGI("switch power[START]: %s (Power: %d, Buck: %d, MTCMOS: %d, CG: %d)",
			power ? "On" : "Off",
...
done_unlock:
	GPUFREQ_LOGI("PWR_STATUS DONE");

	mutex_unlock(&gpufreq_lock);

	GPUFREQ_TRACE_END();

	GPUFREQ_LOGI("switch power[END]: %s (Power: %d, Buck: %d, MTCMOS: %d, CG: %d)",
			power ? "On" : "Off",
			g_gpu.power_count, g_gpu.buck_count,
			g_gpu.mtcmos_count, g_gpu.cg_count);
...
static int __gpufreq_init_segment_id(struct platform_device *pdev)
{
	...
	// MT6781 has only one segment, check segment code if need to enable this in the future
...
#endif
	GPUFREQ_LOGI("efuse_id = 0x%08X, segment_id = %d\n", efuse_id, segment_id);
...
static int __gpufreq_init_clk(struct platform_device *pdev)
{
	...
	if (IS_ERR(g_clk->mtcmos_mfg_core0)) {
		GPUFREQ_LOGE("cannot get mtcmos_mfg_core0\n");
	...
	if (IS_ERR(g_clk->mtcmos_mfg_core1)) {
		GPUFREQ_LOGE("cannot get mtcmos_mfg_core1\n");
...
static int __gpufreq_init_platform_info(struct platform_device *pdev)
{
	...
	GPUFREQ_LOGI("gpufreq init platform info SUCCESS");
	ret = GPUFREQ_SUCCESS;
...
static void __gpufreq_apply_aging(unsigned int apply_aging)
{
	...
	for (i = 0; i < opp_num; i++) {
		...
		GPUFREQ_LOGI("apply Vaging: %d, GPU[%02d] Volt: %d, Vsram: %d",
...
#if IS_BUILTIN(CONFIG_MTK_GPU_MT6781_SUPPORT)
