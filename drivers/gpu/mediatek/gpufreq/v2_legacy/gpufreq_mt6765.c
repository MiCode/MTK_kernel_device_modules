...
void mt_gpufreq_disable_by_ptpod(void)
{
	...
	__gpufreq_power_control(GPU_PWR_ON);
...
static int __gpufreq_mtcmos_control(enum gpufreq_power_state power)
{
	...
	if (power == GPU_PWR_ON) {
		...
		if (unlikely(ret < 0)) {
			__gpufreq_abort("fail to disable pd_mfg (%d)", ret);
			goto done;
		}
...
static int __gpufreq_buck_control(enum gpufreq_power_state power)
{
	...
	if (power == GPU_PWR_ON) {
		...
		if (unlikely(ret)) {
			__gpufreq_abort("fail to enable VCORE (%d)",
			ret);
			goto done;
		}
		g_gpu.buck_count++;
