...
/* Enable dl limit compose size mode, which reserve compose top and bottom lines. */
int mml_dl_reserve_top = 48;
module_param(mml_dl_reserve_top, int, 0644);
int mml_dl_reserve_bottom = 16;
module_param(mml_dl_reserve_bottom, int, 0644);
	...
	u32 *reason, u32 panel_width, u32 panel_height, struct mml_frame_info_cache *info_cache)
{
	...
	if (mml_dl_vdo(mml) && info->fps >= 120) {
		if (dest->compose.top < mml_dl_reserve_top) {
			*reason = mml_query_dl_top;
			goto decouple;
		}

		if (panel_height - (dest->compose.top + dest->compose.height) <
			mml_dl_reserve_bottom) {
			*reason = mml_query_dl_bottom;
			goto decouple;
		}
	}
	...
	u32 *reason, u32 panel_width, u32 panel_height, struct mml_frame_info_cache *info_cache)
{
	struct mml_topology_cache *tp = mml_topology_get_cache(mml);
	enum mml_mode mode;
	static int dl_debounce_times = 30;
