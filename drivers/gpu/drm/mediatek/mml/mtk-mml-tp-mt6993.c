	...
	u32 panel_width, u32 panel_height, bool *dual, struct mml_frame_info_cache *info_cache)
{
	...
	*dual = mml_rrot_single == 1 ? false : true;
	...
	if (mml_rrot_single != 2 && tputw * tputh <= MML_DL_RROT_S_PX &&
		tput < tp->qos[mml_sys_dma].opp_speeds[1]) {
		*dual = false;
		if (info_cache)
			info_cache->dl_opp = 1;
		goto check_hrt;
	...
	tput = pixel / (info->act_time / 1000);
	if (tput < tp->qos[mml_sys_dma].opp_speeds[tp->qos[mml_sys_dma].opp_cnt - 1]) {
		*dual = mml_rrot_single == 1 ? false : true;
		goto find_opp;
	}
...
find_opp:
	if (info_cache) {
		for (i = 0; i < tp->qos[mml_sys_dma].opp_cnt; i++) {
			if (tput <= tp->qos[mml_sys_dma].opp_speeds[i])
				break;
		}
		info_cache->dl_opp = i;
	}
	...
	u32 panel_width, u32 panel_height, struct mml_frame_info_cache *info_cache)
{
	...
	if (!tp || !tp->qos[mml_sys_dma].opp_cnt) {
		mml_err("no opp table support");
		return false;
	}
	 ...
	 */
	pixel = max(tputw, destw) * max(tputh, desth) * 11 / 10;
	max_clock = tp->qos[mml_sys_dma].opp_speeds[tp->qos[mml_sys_dma].opp_cnt - 1];
	...
	if (info->act_time) {
		...
		if (!opp_pixel_table[0]) {
			for (i = 0; i < ARRAY_SIZE(opp_pixel_table); i++) {
				opp_pixel_table[i] = tp->qos[mml_sys_dma].opp_speeds[i] * MML_DC_ACT_DUR;
		...
		for (i = 0; i < tp->qos[mml_sys_dma].opp_cnt; i++)
			if (ir_freq < tp->qos[mml_sys_dma].opp_speeds[i])
				break;
