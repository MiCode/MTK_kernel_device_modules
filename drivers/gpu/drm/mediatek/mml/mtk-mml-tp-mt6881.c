...
#include <dt-bindings/mml/mml-mt6881.h>
...
#define TOPOLOGY_PLATFORM	"mt6881"
...
static const struct path_node path_map[PATH_MML_MAX][MML_MAX_PATH_NODES] = {
	...
	[PATH_MMLT_PQ] = {
		...
		{MML_FG1, MML_HDR1,},
		{MML_HDR1, MML_RSZ1,},
		{MML_RSZ1, MML_WROT1_SEL,},
		{MML_WROT1_SEL, MML_WROT1,},
		{MML_WROT1,},
	},
};
...
{
	return id == MML_RDMA0 || id == MML_RDMA1;
}
...
static const u8 grp_dispatch[PATH_MML_MAX] = {
	...
	[PATH_MMLF_PQ] = MUX_SOF_GRP1,
	[PATH_MMLT_PQ] = MUX_SOF_GRP1,
};
	...
	struct mml_topology_path **path)
{
	...
	if (cfg->info.mode == MML_MODE_MML_DECOUPLE2 &&
	    cfg->info.dest[0].rotate == MML_ROT_0 && !cfg->info.dest[0].flip) {
		scene = PATH_MMLT_PQ;
	} else if (!en_pq) {
		/* rdma to wrot */
		scene = PATH_MML_NOPQ;
	...
	u32 *reason)
{
	...
	if (info->mode == MML_MODE_MML_DECOUPLE ||
		...
		info->mode == MML_MODE_MML_DECOUPLE2) {
...
static const struct mml_topology_ops tp_ops_mt6881 = {
...
static __init int mml_topology_ip_init(void)
{
	...
	return mml_topology_register_ip(TOPOLOGY_PLATFORM, &tp_ops_mt6881);
}
...
MODULE_DESCRIPTION("MediaTek SoC display MML for MT6881");
MODULE_LICENSE("GPL");
