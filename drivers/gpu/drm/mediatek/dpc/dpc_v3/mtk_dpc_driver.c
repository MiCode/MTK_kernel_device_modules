...
irqreturn_t mt6993_irq_handler(int irq, void *dev_id)
{
		...
		if (__ratelimit(&err_rate)) {
			if (unlikely(irq_aee)) {
				...
				DPCERR("irq err(%#x) psel(%#x=%#x) mtcmos(%#x) intf(%#x)",
					...
					mtcmos_sta,
					readl(dpc_base + DISP_DPC_INTSTA_INTF_PWR_RDY_STATE));
				BUG_ON(1);
			} else
				DPCERR("irq err(%#x) psel(%#x=%#x) mtcmos(%#x) intf(%#x)",
					err_sta,
	...
	// if (mtcmos_sta & BIT(28))
		// dpc_mmp(mtcmos_disp1, MMPROFILE_FLAG_PULSE, 4, 0);
	// if (mtcmos_sta & BIT(31))
		// dpc_mmp(mtcmos_disp1, MMPROFILE_FLAG_PULSE, 7, 0);
	if (mtcmos_sta & BIT(2))
		dpc_mmp(mtcmos_disp1, MMPROFILE_FLAG_START, 0, 1);
	if (mtcmos_sta & BIT(14))
		dpc_mmp(mtcmos_disp1, MMPROFILE_FLAG_END, 0, 0);

	// if (mtcmos_sta & BIT(24))
		// dpc_mmp(mtcmos_ovl0, MMPROFILE_FLAG_PULSE, 0, 0);
	// if (mtcmos_sta & BIT(27))
		// dpc_mmp(mtcmos_ovl0, MMPROFILE_FLAG_PULSE, 3, 0);
	if (mtcmos_sta & BIT(4))
		dpc_mmp(mtcmos_ovl0, MMPROFILE_FLAG_START, 0, 1);
	if (mtcmos_sta & BIT(16))
		dpc_mmp(mtcmos_ovl0, MMPROFILE_FLAG_END, 0, 0);

	// if (mtcmos_sta & BIT(27))
		// dpc_mmp(prete, MMPROFILE_FLAG_PULSE, 0x3, 0);
	// if (mtcmos_sta & BIT(29))
		// dpc_mmp(prete, MMPROFILE_FLAG_PULSE, 0x5, 1);
	// if (mtcmos_sta & BIT(31))
		// dpc_mmp(prete, MMPROFILE_FLAG_PULSE, 0x7, 0);
...
static void dpc_ap_ref_cnt(bool add, const enum mtk_vidle_voter_user user)
{
	...
	if (add && cnt == 1) {
		...
		dpc_hwccf_vote(VOTE_SET, NULL, user, false);
	} else if (!add && cnt == 0) {
		dpc_hwccf_vote(VOTE_CLR, NULL, user, false);
		dpc_mmp(hwccf_vote, MMPROFILE_FLAG_START, BIT(29) | user, cnt);
	} else if (cnt < 0) {
		atomic_set_release(&hwccf_ref, 0);
		dpc_mmp(folder, MMPROFILE_FLAG_PULSE, (add ? BIT(28) : BIT(29)) | user, U32_MAX);
	} else {
...
static int dpc_vidle_power_keep_v3(const enum mtk_vidle_voter_user _user)
{
	...
	dpc_hwccf_vote(true, NULL, DISP_VIDLE_USER_DISP_DPC_CFG, true);
	mtk_disp_vlp_vote_v2(VOTE_SET, user);
	...
	case DISP_VIDLE_FORCE_KEEP:
		break;
	default:
		dpc_hwccf_vote(false, NULL, DISP_VIDLE_USER_DISP_DPC_CFG, true);
					   ...
					   const u16 gpr, void *reuse)
{
	...
	dpc_hwccf_vote(true, pkt, user, true);
	cmdq_pkt_write(pkt, NULL, g_priv->voter_set_pa, BIT(user), U32_MAX);
	dpc_hwccf_vote(false, pkt, user, true);
}
