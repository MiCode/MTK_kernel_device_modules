...
int debug_vlp_backtrace;
module_param(debug_vlp_backtrace, int, 0644);
...
void dpc_config(const enum mtk_dpc_subsys subsys, bool en)
{
	...
	spin_lock_irqsave(&dpc_lock, flags);
	if (en && dpc_vidle_is_available() == 0) {
		DPCERR("in-available, cap:0x%x, dur:%u-%u, panel:%d, en:%d, subsys:%d",
			g_priv->vidle_mask, g_te_duration, g_vb_duration,
			g_panel_type, en, subsys);
		goto out;
	}

	if (en && atomic_read(&g_priv->dpc_en_cnt) == 0) {
		DPCERR("enable DT before config dpc group, en:%d, cnt:%d, subsys:%d",
			en, atomic_read(&g_priv->dpc_en_cnt), subsys);
		goto out;
	}

	if (MTK_DPC_OF_DISP_SUBSYS(subsys) && en &&
		atomic_read(&g_disp_mode) == DPC_VIDLE_SW_MANUAL_MODE) {
		DPCDUMP("ignore disp config, disp_mode:%d, en:%d, subsys:%d", atomic_read(&g_disp_mode), en, subsys);
		...
		DPCDUMP("ignore mml config, mml_mode:%d, en:%d, subsys:%d", atomic_read(&g_mml_mode), en, subsys);
