...
static atomic_t g_mmdvfs_dead = ATOMIC_INIT(0);
#endif
...
static struct mtk_dpc mt6878_dpc_driver_data = {
	...
	.mmdvfs_settings_count = 0, //not support mmdvfs in VCP
...
static void dpc_mtcmos_vote_v1(const u32 subsys, const u8 thread, const bool en);
...
static unsigned int mtk_dpc_get_vidle_mask(const u32 subsys, bool en)
{
...
static unsigned int mt6858_get_sys_status(enum dpc_sys_status_id id, unsigned int *status)
{
	unsigned int mask = 0, value = 0;
	void __iomem *addr = NULL;
	...
	switch (id) {
	case SYS_POWER_ACK_MMINFRA:
		mask = SPM_PWR_FLD_MMINFRA_MASK_MT6858;
		if (g_priv->sys_va[SPM_BASE])
			addr = g_priv->sys_va[SPM_BASE] + SPM_PWR_STATUS_MT6858;
		break;
	case SYS_POWER_ACK_DISP1_SUBSYS:
	case SYS_POWER_ACK_MML1_SUBSYS:
	case SYS_POWER_ACK_DPC:
		mask = SPM_PWR_FLD_DISP_VCORE_MASK_MT6858;
		if (g_priv->sys_va[SPM_BASE])
			addr = g_priv->sys_va[SPM_BASE] + SPM_PWR_STATUS_MT6858;
		break;
	case SYS_STATE_MMINFRA:
		mask = SPM_REQ_INFRA_STATE_MT6858;
		if (g_priv->sys_va[SPM_BASE])
			addr = g_priv->sys_va[SPM_BASE] + SPM_REQ_STA_4_MT6858;
		break;
	case SYS_STATE_APSRC:
		mask = SPM_REQ_APSRC_STATE_MT6858;
		if (g_priv->sys_va[SPM_BASE])
			addr = g_priv->sys_va[SPM_BASE] + SPM_REQ_STA_4_MT6858;
		break;
	case SYS_STATE_EMI:
...
static void dpc_update_group_auto_state(const u32 subsys, const bool en, unsigned int flag)
{
	if (MTK_DPC_OF_DISP_SUBSYS(subsys)) {
		if (en)
...
static void dpc_update_mtcmos_auto_state(const u32 subsys, const bool en, unsigned int flag)
{
	if (MTK_DPC_OF_DISP_SUBSYS(subsys)) {
		if (en)
...
static void dpc_dt_en_all(const u32 subsys, u32 dt_en)
{
	...
	if (subsys == DPC_SUBSYS_DISP) {
		...
		usage = g_disp_dt_usage;
		dpc_mmp(disp_dt, MMPROFILE_FLAG_PULSE, subsys, dt_en);
	} else if (subsys == DPC_SUBSYS_MML) {
		writel(dt_en, dpc_base + DISP_REG_DPC_MML_DT_EN);
		usage = g_mml_dt_usage;
		dpc_mmp(mml_dt, MMPROFILE_FLAG_PULSE, subsys, dt_en);
	}
}
...
static void dpc_irq_enable(const u32 subsys, bool en, bool manual)
{
	u32 mask = 0;
...
static unsigned int dpc_polling_dvfsrc_ack_done(void)
{
	...
	if (atomic_read(&g_mmdvfs_available) && g_panel_type == PANEL_TYPE_VDO &&
		mtk_dpc_support_cap(DPC_VIDLE_LOWER_VDISP_DVFS)) {
		ret = readl_poll_timeout_atomic(g_priv->sys_va[VDISP_DVFSRC_DEBUG],
					value, (value & BIT(0)) == 0, 1, 200);
		if (ret < 0)
			DPCERR("wait vidle HW ops done timeout, ret:%d, req:0x%x",
				ret, value);
	}
...
static void dpc_disp_group_enable_func(const enum mtk_dpc_disp_vidle group, bool en, bool dt_ctrl)
{
	...
	u32 dt_mask = (g_panel_type == PANEL_TYPE_VDO) ?
				MTK_VIDLE_VDO_DISP_DT_MASK : MTK_VIDLE_CMD_DISP_DT_MASK;
	...
	case DPC_DISP_VIDLE_VDISP_DVFS:
		...
		/*make sure DPC HW ack done, before sw force on*/
...
static void dpc_mml_group_enable_func(const enum mtk_dpc_mml_vidle group, bool en, bool dt_ctrl)
{
	...
	u32 value = 0, value1 = 0;
	u32 dt_mask = (g_panel_type == PANEL_TYPE_VDO) ?
				MTK_VIDLE_VDO_MML_DT_MASK : MTK_VIDLE_CMD_MML_MASK;
	...
	for (i = 0; i < DPC_MML_DT_CNT; ++i) {
		if (group == g_mml_dt_usage[i].group) {
			if (BIT(g_mml_dt_usage[i].index) & dt_mask) {
...
static void dpc_ddr_force_enable_v1(const u32 subsys, const bool en)
{
	u32 addr = 0;
...
static void dpc_infra_force_enable_v1(const u32 subsys, const bool en)
{
	u32 addr = 0;
	...
	if (en) {
		if (atomic_read(&g_mml_mode) == DPC_VIDLE_INACTIVE_MODE) {
			...
			if (dbg_runtime_ctrl) {
				...
				dbg_irq = 1;
				dbg_mtcmos_off = 1;
				if (g_panel_type == PANEL_TYPE_VDO && mtk_dpc_support_cap(DPC_VIDLE_LOWER_VDISP_DVFS))
					dbg_vdisp_level = 1;
			}
...
static int dpc_enable_vcp(bool en, const u32 subsys)
{
	...
	/* if display has enabled vcp, then mmdvfs_power_sync should be disabled */
	if (!g_priv->mmdvfs_power_sync)
		return en && atomic_read(&g_mmdvfs_dead) ? -EFAULT : 0;
	...
	if (ret < 0) {
		DPCERR("failed to enable vcp, user:%d, ret:%d", subsys, ret);
		return ret;
	}
	...
	if (en) {
		...
		while (!atomic_read(&g_priv->vcp_is_alive)) {
			...
			udelay(50); //vcp power on latency < 40ms
		}
	}
#endif

	return atomic_read(&g_mmdvfs_dead) ? -EFAULT : 0;
}
...
static void dpc_enable_v1(const u8 en)
{
	...
	unsigned long flags = 0;
	static bool vcp_state;

	if (g_panel_type >= PANEL_TYPE_COUNT)
		return;

	if (en && atomic_read(&g_priv->dpc_en_cnt))
		return;

	if (dpc_pm_ctrl(true, __func__)) {
		DPCERR("failed when power on");
		return;
	}
	...
	if (en && !vcp_state) {
		if (dpc_enable_vcp(true, DPC_SUBSYS_DISP) < 0) {
			DPCERR("failed to enable vcp, en:%d, mmdvfs_dead:%d",
				en, atomic_read(&g_mmdvfs_dead));
			goto out;
		}
		vcp_state = true;
	}
	...
	if (en) {
		 ...
		 */
		if (g_panel_type == PANEL_TYPE_CMD)
			dt_mask = MTK_VIDLE_CMD_MML_MASK;
		else
...
static void dpc_srt_bw_set_v1(const u32 subsys, const u32 bw_in_mb, bool force)
{
	u32 addr1 = 0, addr2 = 0, avail = 0;
	static u32 disp_bw, mml_bw;
	unsigned long flags = 0;
...
static int mtk_dpc_mmdvfs_notifier(const bool enable, const bool wdt)
{
	...
	if (!enable) {
		...
		if (!atomic_read(&g_mmdvfs_dead) && atomic_read(&g_mmdvfs_available)) {
			dpc_mmp(mmdvfs_dead, MMPROFILE_FLAG_START, enable, wdt);
			atomic_set(&g_mmdvfs_dead, 1);
		} else
...
static int vdisp_level_set_vcp(const u32 subsys, const u8 level, bool vcp_state)
{
	u32 addr = 0;
	...
	if ((g_priv->mmdvfs_power_sync && !vcp_state) || atomic_read(&g_mmdvfs_dead)) {
		DPCERR("mmdvfs is not ready, subsys:%u addr:0x%x val:0x%x vcp:%d mmdvfs_dead:%d",
			subsys, addr, level, vcp_state, atomic_read(&g_mmdvfs_dead));
...
static u8 dpc_max_dvfs_level(const u32 subsys)
{
	u8 max_level = 0;
	u32 addr;
...
static void dpc_dvfs_set_v1(const u32 subsys, const u8 level, bool force)
{
	...
	if (dpc_enable_vcp(true, subsys) < 0) {
		DPCERR("subsys:%u failed to enable vcp,level:%u,mmdvfs_dead:%d",
			subsys, max_level, atomic_read(&g_mmdvfs_dead));
		vcp_state = false;
	}

	spin_lock_irqsave(&dpc_lock, flags);
	vdisp_level_set_vcp(subsys, max_level, vcp_state);
...
static void dpc_channel_bw_set_by_idx(const u32 subsys, const u8 idx, const u32 bw_in_mb)
{
	...
	if (dpc_enable_vcp(true, subsys) < 0) {
		DPCERR("subsys:%u failed to enable vcp,level:%u,mmdvfs_dead:%d",
			subsys, max_bw_level, atomic_read(&g_mmdvfs_dead));
		vcp_state = false;
	}

	spin_lock_irqsave(&dpc_lock, flags);
	vdisp_level_set_vcp(DPC_SUBSYS_VDISP_LEVEL_OF_BW, max_bw_level, vcp_state);
...
void dpc_group_enable_func(const u16 group, bool en, bool lock)
{
	...
	if (group <= DPC_DISP_VIDLE_RESERVED)
		dpc_disp_group_enable_func((enum mtk_dpc_disp_vidle)group, en, false);
	else if (group <= DPC_MML_VIDLE_RESERVED)
		dpc_mml_group_enable_func((enum mtk_dpc_mml_vidle)group, en, false);
	else
...
#ifdef MTK_DPC_PAUSE_SUPPORT
static void dpc_pause_v1(const u32 subsys, bool en)
{
	unsigned long flags = 0;
...
static void dpc_mtcmos_auto_v1(const u32 subsys, const enum mtk_dpc_mtcmos_mode mode)
{
	unsigned long flags = 0;
...
static int dpc_config_v1(const u32 subsys, bool en)
{
	unsigned int mask = 0x0;
	unsigned long flags = 0;
	int ret __maybe_unused = 0;
...
out:
	...
	DPCFUNC("subsys:%d,en:%d,mminfra_ctrl_by_dpc:%d,ret:%d",
		subsys, en, atomic_read(&is_mminfra_ctrl_by_dpc), ret);
...
irqreturn_t mtk_dpc_disp_irq_handler(int irq, void *dev_id)
{
	...
	u32 status, merge_status, vdisp_state = 0;
	...
	if (likely(dbg_mmp)) {
		if (status & DISP_DPC_INT_DT6) {
			if (g_priv->get_sys_status && g_panel_type == PANEL_TYPE_VDO &&
				mtk_dpc_support_cap(DPC_VIDLE_LOWER_VDISP_DVFS))
				vdisp_state = g_priv->get_sys_status(SYS_VALUE_VDISP_DVFS_LEVEL, NULL);
			dpc_mmp(prete, MMPROFILE_FLAG_PULSE, vdisp_state, DISP_DPC_INT_DT6);
...
static void _dpc_analysis(bool detail)
{
	...
	DPCDUMP("hrt bw: disp(%#010x %#06x), mml(%#010x %#06x), dvfs_sta:0x%x",
...
out:
	...
	if (atomic_read(&g_mmdvfs_available) && g_panel_type == PANEL_TYPE_VDO &&
		...
		if (value > 0) {
			...
			writel(value + 1, dpc_base + DISP_REG_DPC_DISP_VDISP_DVFS_VAL);
			udelay(100);
...
static void process_dbg_opt(const char *opt)
{
	...
	if (strncmp(opt, "en:", 3) == 0) {
	...
	} else if (strncmp(opt, "underrun_trace", 14) == 0) {
		...
		if (atomic_read(&g_mmdvfs_available) && g_panel_type == PANEL_TYPE_VDO &&
			mtk_dpc_support_cap(DPC_VIDLE_LOWER_VDISP_DVFS)) {
			state = g_priv->get_sys_status(SYS_VALUE_VDISP_DVFS_LEVEL, &value);
			dpc_mmp(vdisp_off, MMPROFILE_FLAG_PULSE, state, value);
		}

		if (mtk_dpc_support_cap(DPC_VIDLE_APSRC_OFF)) {
			state = g_priv->get_sys_status(SYS_STATE_APSRC, &value);
			dpc_mmp(apsrc_off, MMPROFILE_FLAG_PULSE, state, value);
		}

		if (mtk_dpc_support_cap(DPC_VIDLE_ZERO_HRT_BW)) {
			state = g_priv->get_sys_status(SYS_STATE_HRT_BW, &value);
			dpc_mmp(dvfs_off, MMPROFILE_FLAG_PULSE, state, value);
		}
...
static int mtk_dpc_probe_v1(struct platform_device *pdev)
{
	...
	if (priv->mmdvfs_settings_count > 0) {
		DPCFUNC("enable mmdvfs notifier");
	...
	DPCFUNC("- mmsys:0x%x, panel:%d, vidle:0x%x, ret:%d",
		priv->mmsys_id, g_panel_type, priv->vidle_mask, ret);
	return ret;
}
