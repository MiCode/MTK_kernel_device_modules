...
extern void vis_display_ops_send_mipi_cmd_mtk_notify(int (*send_mipi_cmd)(struct LCM_setting_table *table, unsigned int count, unsigned int format));
#endif
#include "mtk_debug.h"
...
#ifdef CONFIG_MI_DISP_DFS_EVENT
struct timer_list underrun_report_timer;
#endif
...
CONFIG_REG:
	...
	if (dsi->ext && !dsi->ext->params->vdo_keep_hs_perline &&
		...
		dsi->hfp_minimum_dphy = DIV_ROUND_UP(2 + 7 + 4, dsi_buf_bpp);
		dsi->hfp_minimum_wc_dphy = 1;
	}

	if (dsi->driver_data->support_frame_tb_v5 &&
...
/**
 * Dphy timconfig calc for platforms whose process <= N6
 * and which use frame rate v5 table to calc FPS
 * The param of this calc is the same as param provided by designer
 *
 * This function is not support bridge ic platform yet
 */
static void mtk_dsi_dphy_timconfig_v1_2(struct mtk_dsi *dsi, void *handle)
{
	struct mtk_dsi_phy_timcon *phy_timcon = NULL;
	u32 lpx;
	u32 clk_zero = 0, clk_trail = 0;
	...
	da_hs_prep = (dsi->data_rate * 59 + 4000) / 8000 + 1;
	da_hs_zero = (dsi->data_rate * 163 + 11000) / 8000 + 1 - da_hs_prep;
	da_hs_trail = (dsi->data_rate * 78 + 7000) / 8000 + 1;
	da_hs_exit = (dsi->data_rate * 118) / 8000 + 1;

	clk_hs_prep = (dsi->data_rate * 57) / 8000 + 1;
	clk_hs_zero = (dsi->data_rate * 330) / 8000 + 1 - clk_hs_prep;
	clk_hs_trail = (dsi->data_rate * 78 + 7000) / 8000 + 1;
	clk_hs_post = (dsi->data_rate * 65 + 53000) / 8000 + 1;
	clk_hs_exit = (dsi->data_rate * 118) / 8000 + 1;
...
CONFIG_REG:
	...
	if (dsi->ext && !dsi->ext->params->vdo_keep_hs_perline &&
		!(dsi->mode_flags & MIPI_DSI_CLOCK_NON_CONTINUOUS)) {
		dsi->hfp_minimum_dphy = DIV_ROUND_UP(2 + 4 + (da_hs_trail + 1 +
						dsi->data_phy_cycle) * dsi->lanes + 4, dsi_buf_bpp);
		dsi->hfp_minimum_wc_dphy = (da_hs_trail + 1) * dsi->lanes - 2;
	} else if (dsi->ext && !dsi->ext->params->vdo_keep_hs_perline &&
		(dsi->mode_flags & MIPI_DSI_CLOCK_NON_CONTINUOUS)) {
		dsi->hfp_minimum_dphy = DIV_ROUND_UP(2 + 4 + (da_hs_trail + 1 + clk_hs_post + 1 +
					clk_hs_trail + clk_hs_exit + 1 + lpx + clk_hs_prep +
					clk_hs_zero + dsi->data_phy_cycle) * dsi->lanes + 4, dsi_buf_bpp);
		dsi->hfp_minimum_wc_dphy = (da_hs_trail + 1 + clk_hs_post + 1 + clk_hs_trail + clk_hs_exit + 1 +
						lpx + clk_hs_prep + clk_hs_zero) * dsi->lanes - 2;
	} else if (dsi->ext && dsi->ext->params->vdo_keep_hs_perline &&
		(dsi->mode_flags & MIPI_DSI_CLOCK_NON_CONTINUOUS)) {
...
static void mtk_dsi_cphy_timconfig_v1_2(struct mtk_dsi *dsi, void *handle)
{
	struct mtk_dsi_phy_timcon *phy_timcon = NULL;
	u32 lpx;
	u32 da_hs_prep, da_hs_zero, da_hs_trail, da_hs_exit;
...
static void mtk_dsi_dphy_timconfig(struct mtk_dsi *dsi, void *handle)
{
	if (dsi && dsi->driver_data) {
		...
		else if (dsi->driver_data->n_verion <= VER_N6 &&
			dsi->driver_data->support_frame_tb_v5)
			mtk_dsi_dphy_timconfig_v1_2(dsi, handle);
		else
...
static void mtk_dsi_cphy_timconfig(struct mtk_dsi *dsi, void *handle)
{
	if (dsi && dsi->driver_data) {
		if (dsi->driver_data->n_verion <= VER_N6 &&
			...
			dsi->driver_data->support_frame_tb_v5)
			mtk_dsi_cphy_timconfig_v1_2(dsi, handle);
		else
...
static void mtk_dsi_set_mode(struct mtk_dsi *dsi, const int line)
{
	...
	CRTC_MMP_MARK(index, dsi_mode_change, line, vid_mode);
...
static void mtk_dsi_tx_buf_rw(struct mtk_dsi *dsi)
{
	...
	if (dsi->driver_data->calc_golden_by_pct) {
		...
		ultra_hi_fifo_us = ultra_hi_fifo_pct ?
				...
				ultra_lo_fifo_us + 1;
		urgent_lo_fifo_us = DIV_ROUND_UP(buf_con * urgent_lo_fifo_pct, consume_rate * 100);
		urgent_hi_fifo_us = urgent_hi_fifo_pct ?
				DIV_ROUND_UP(buf_con * urgent_hi_fifo_pct, consume_rate * 100) :
				urgent_lo_fifo_us + 1;
		...
		if (mtk_crtc_is_frame_trigger_mode(&mtk_crtc->base)) {
			if (dsi && dsi->ext && dsi->ext->params && dsi->ext->params->output_valid_us)
				output_valid_us = MIN(ultra_lo_fifo_us + 5,
						dsi->ext->params->output_valid_us);
			else
				output_valid_us = ultra_lo_fifo_us + 5;
			output_valid_us = dbg_output_valid ? dbg_output_valid : output_valid_us;
			DDPINFO("%s output_valid_us=%d\n", __func__, output_valid_us);
	...
	if (!mtk_crtc_is_frame_trigger_mode(&mtk_crtc->base) &&
		...
		urgent_threshold = output_valid_us * dsi->data_rate / 8 / 64;
		if (priv && priv->data &&
			priv->data->mmsys_id == MMSYS_MT6989) // MT6989 MT6878 MT6899
			mtk_dsi_mask(dsi, DSI_BUF_CON0(dsi->driver_data), MT6989_BUF_VDE_BLOCK_URGENT, 0);
		else
	...
	if (dsi->ext->params->is_cphy) {
		...
		if (dsi->mode_flags & MIPI_DSI_MODE_VIDEO_SYNC_PULSE) {
			if (dsi->driver_data->support_frame_tb_v5) {
				...
				if (t_hsa * dsi_tmp_buf_bpp > 10 + 4)
					horizontal_sync_active_byte = ALIGN_TO((t_hsa * dsi_tmp_buf_bpp - 10), 4);
				else
					horizontal_sync_active_byte = 4;

				if (t_hbp * dsi_tmp_buf_bpp > 10 + 4)
					horizontal_backporch_byte = ALIGN_TO((t_hbp * dsi_tmp_buf_bpp - 10), 4);
				else
					horizontal_backporch_byte = 4;
			}
		} else {
		...
		if (dsi->driver_data->support_frame_tb_v5) {
			...
			if (t_hfp * dsi_tmp_buf_bpp > 12 + 4)
				horizontal_frontporch_byte = ALIGN_TO((t_hfp * dsi_tmp_buf_bpp - 12), 4);
			else
		...
		if (dsi->driver_data->support_frame_tb_v5) {
			if (t_hfp < dsi->hfp_minimum_dphy)
				DDPPR_ERR("%s error, HFP:%u < DPHY HFP minimum limitation:%u\n",
					__func__, t_hfp, dsi->hfp_minimum_dphy);
			if (horizontal_frontporch_byte < dsi->hfp_minimum_wc_dphy) {
				DDPPR_ERR("%s error, HFP_WC:%d < DPHY HFP_WC minimum limitation:%d\n",
			...
			if (t_hfp < dsi->hfp_minimum_dphy) {
				if (print_err == 0) {
					...
					DDPINFO("%s error, HFP:%u < DPHY HFP minimum limitation:%u\n",
						__func__, t_hfp, dsi->hfp_minimum_dphy);
			}
		}
...
void mtk_dsi_cur_pos_dump(struct mtk_ddp_comp *comp)
{
	...
	if (dsi->slave_dsi) {
		baddr= dsi->slave_dsi->ddp_comp.regs;
		if (!baddr) {
			DDPINFO("%s, DSI1 is NULL!\n", __func__);
			return;
		}
		reg_val = readl(DSI_INPUT_DBG(dsi->slave_dsi->driver_data) + baddr);
		DDPMSG("DSI1 cur_pos(%u,%u)\n",
			REG_FLD_VAL_GET(DSI_DBG_FLD_ROI_X, reg_val),
			REG_FLD_VAL_GET(DSI_DBG_FLD_ROI_Y, reg_val));
	}
#endif
				...
				if (mtk_drm_helper_get_opt(priv->helper_opt, MTK_DRM_OPT_DSI_UNDERRUN_AEE)) {
					if (aee_get_mode() == 4)
						...
						DDPAEE_TRACE_TOP("[IRQ] %s:buffer underrun. TS:0x%llx, NTS:0x%llx\n",
							...
							(u64)sched_clock());
#ifdef CONFIG_MI_DISP_DFS_EVENT
					timer_setup(&underrun_report_timer,underrun_report_hander,0);
					underrun_report_timer.expires = jiffies + 10*HZ;
					DDPPR_ERR("%s report underrun after 10s\n", __func__);
					add_timer(&underrun_report_timer);
#endif
				}
				mtk_dprec_snapshot();
...
static int mtk_dsi_stop_vdo_mode(struct mtk_dsi *dsi, void *handle, const int line);
...
static void mtk_output_en_doze_switch(struct mtk_dsi *dsi, enum dsi_transfer_mode mode)
{
	...
	if ((mtk_dsi_cmd_version() == DSI_CMD_V2) && doze_enabled && panel_funcs->doze_enable_start_v2) {
		...
		if (mode == TRANSFER_CMD_MODE) {
			...
			cmd_opt.flags |= MTK_MIPI_DSI_GCE_USE_DSI_THREAD |
							MTK_MIPI_DSI_GCE_USE_DSI_CMD_EVENT;
		panel_funcs->doze_enable_start_v2(dsi->panel, dsi,
					mtk_mipi_dsi_cmd, NULL, &cmd_opt);
#ifdef CONFIG_MI_DISP
	} else if ((mtk_dsi_cmd_version() == DSI_CMD_V2) && dsi_cmd_v2_dbg[AOD_DBG] &&
				(!doze_enabled) && panel_funcs->doze_disable_v2) {
#else
	...
	if (panel_funcs->doze_get_mode_flags) {
		if (!mtk_dsi_is_cmd_mode(&dsi->ddp_comp))
			mtk_dsi_stop_vdo_mode(dsi, NULL, __LINE__);
	...
	int force_lcm_update)
{
	...
	if (dsi->panel) {
		 ...
		 */
#ifdef CONFIG_MI_DISP
		/* only init after panel prepared */
		if ((!dsi->doze_enabled && !dsi->pending_switch) || force_lcm_update) {
#endif
		if ((mtk_dsi_cmd_version() == DSI_CMD_V2) && dsi_cmd_v2_dbg[PANEL_INIT_DBG] &&
			...
			cmd_opt.flags = MTK_MIPI_DSI_CMD_BY_CPU | MTK_DSI_FROCE_CMD_MODE_TRANSFER;
			ret = dsi->ext->funcs->panel_init_v2(dsi, dsi->panel, NULL, mtk_mipi_dsi_cmd, &cmd_opt);
			if (ret < 0)
				DDPPR_ERR("%s, panel init v2 fail\n", __func__);
			CRTC_MMP_MARK(0, dsi_resume, 1, 1);
		} else if (dsi->ext && dsi->ext->funcs && dsi->ext->funcs->panel_init) { //legacy
			ret = dsi->ext->funcs->panel_init(dsi->panel);
			if (ret < 0)
				DDPPR_ERR("%s, panel init fail\n", __func__);
		...
		if (!new_doze_state && dsi->doze_enabled) {
#ifdef CONFIG_MI_DISP
			if ((mtk_dsi_cmd_version() == DSI_CMD_V2) && dsi_cmd_v2_dbg[AOD_DBG] &&
#else
			if ((mtk_dsi_cmd_version() == DSI_CMD_V2) &&
#endif
	...
	if (dsi->slave_dsi) {
		...
		if (mtk_dsi_is_LTPO_VM_Enable(dsi))
			mtk_dsi_mask(dsi, DSI_TXRX_CTRL(dsi->driver_data), EXT_TE_EN, EXT_TE_EN);
		mtk_dsi_set_mode(dsi->slave_dsi, __LINE__);
	}
	if (is_bdg_supported()) {
				   ...
				   int force_lcm_update)
{
	...
	if (!mtk_dsi_is_cmd_mode(&dsi->ddp_comp)) {
		...
		cmdq_pkt_wait_no_clear(handle_tmp, mtk_crtc->gce_obj.event[EVENT_CMD_EOF]);
		mtk_use_cabc_event(handle_tmp, mtk_crtc, WAIT_AND_CLEAR_OPT, __LINE__);
		mtk_dsi_stop_vdo_mode(dsi, handle_tmp, __LINE__);
	...
	if (dsi->panel) {
		...
		if ((!new_doze_state && !skip_panel_switch) || force_lcm_update) {
			...
			if ((mtk_dsi_cmd_version() == DSI_CMD_V2) && dsi_cmd_v2_dbg[PANEL_INIT_DBG] &&
				...
				cmd_opt.flags = MTK_MIPI_DSI_CMD_BY_CPU | MTK_DSI_FROCE_CMD_MODE_TRANSFER;
				ret = dsi->ext->funcs->panel_deinit_v2(dsi, dsi->panel, NULL,
						mtk_mipi_dsi_cmd, &cmd_opt);
				if (ret < 0)
					DDPPR_ERR("%s, panel deinit v2 fail\n", __func__);

				CRTC_MMP_MARK(crtc_idx, dsi_suspend, 2, 1);
			} else if (dsi->ext && dsi->ext->funcs && dsi->ext->funcs->panel_deinit) { //legacy
				ret = dsi->ext->funcs->panel_deinit(dsi->panel);
				if (ret < 0)
					DDPPR_ERR("%s, panel deinit fail\n", __func__);
			...
			/* doze suspend */
				...
				mtk_crtc->doze_into_suspend = false;
				if (dsi->ext && dsi->ext->funcs) {
					panel_funcs = dsi->ext->funcs;
					if ((mtk_dsi_cmd_version() == DSI_CMD_V2) && dsi_cmd_v2_dbg[AOD_DBG] &&
						panel_funcs->fod_state_check_v2) {
						struct mtk_dsi_cmd_option cmd_opt = { 0 };

						cmd_opt.flags = MTK_MIPI_DSI_CMD_BY_CPU;
						panel_funcs->fod_state_check_v2(dsi->panel, dsi,
							mtk_mipi_dsi_cmd, NULL, &cmd_opt);
					} else if (panel_funcs->fod_state_check) {
						panel_funcs->fod_state_check(dsi, mipi_dsi_dcs_write_gce2, NULL);
					}
					if ((mtk_dsi_cmd_version() == DSI_CMD_V2) && dsi_cmd_v2_dbg[AOD_DBG] &&
						panel_funcs->doze_suspend_v2 && !fod_anim_flag) {
						struct mtk_dsi_cmd_option cmd_opt = { 0 };

						cmd_opt.flags = MTK_MIPI_DSI_CMD_BY_CPU;
						dsi->ext->funcs->doze_suspend_v2(dsi->panel, dsi,
							mtk_mipi_dsi_cmd, NULL, &cmd_opt);
					} else if (panel_funcs->doze_suspend && !fod_anim_flag) {
...
static void _mtk_dsi_set_mode(struct mtk_ddp_comp *comp, void *handle,
			      unsigned int mode, const int line)
{
	struct mtk_dsi *dsi = container_of(comp, struct mtk_dsi, ddp_comp);
	struct mtk_drm_crtc *mtk_crtc = comp->mtk_crtc;
	int index = drm_crtc_index(&mtk_crtc->base);
	...
	CRTC_MMP_MARK(index, dsi_mode_change, line, mode);
...
static int mtk_dsi_stop_vdo_mode(struct mtk_dsi *dsi, void *handle, const int line)
{
	...
	index = drm_crtc_index(&mtk_crtc->base);
	CRTC_MMP_MARK(index, dsi_mode_change, line, 0);
	...
	if (handle) {
		...
		CRTC_MMP_MARK(index, dsi_mode_change, handle, 1);
	...
	if (dsi->slave_dsi)
		_mtk_dsi_set_mode(&dsi->slave_dsi->ddp_comp, handle, CMD_MODE, __LINE__);
	cmdq_pkt_write(handle, dsi->ddp_comp.cmdq_base,
	...
	//CRTC_MMP_MARK(index, dsi_mode_change, 0, 2);
	mtk_dsi_poll_for_idle(dsi, handle);
	if (dsi->slave_dsi)
	...
	CRTC_MMP_MARK(index, dsi_mode_change, handle, 3);
	if (need_create_hnd) {
		cmdq_pkt_flush(handle);
		cmdq_pkt_destroy(handle);
	}
...
static int mtk_dsi_start_vdo_mode(struct mtk_ddp_comp *comp, void *handle, const int line)
{
	struct mtk_dsi *dsi = container_of(comp, struct mtk_dsi, ddp_comp);
	struct mtk_drm_crtc *mtk_crtc = comp->mtk_crtc;
	int index = drm_crtc_index(&mtk_crtc->base);
	...
	_mtk_dsi_set_mode(comp, handle, vid_mode, __LINE__);
	if (dsi->slave_dsi)
		_mtk_dsi_set_mode(&dsi->slave_dsi->ddp_comp, handle, vid_mode, __LINE__);

	if (handle)
		CRTC_MMP_MARK(index, dsi_mode_change, line, handle);
	else
		CRTC_MMP_MARK(index, dsi_mode_change, line, 0);
	...
	const struct mipi_dsi_msg *msg, struct cmdq_pkt *handle)
{
	...
	while (i < msg->tx_len) {
		...
		DDPDSI_CMD("%s, i=%d(%d), addr=0x%x, data=0x%x\n", __func__, i, msg->tx_len, vm_cmd_addr, vm_cmd_val);
		if (handle)
			cmdq_pkt_write(handle, dsi->ddp_comp.cmdq_base,
				  ...
				  const void *data, size_t len)
{
	...
	if (mtk_dsi_is_cmd_mode(&dsi->ddp_comp)) {
		...
		if (dsi->slave_dsi)
			mtk_dsi_vm_cmdq(dsi->slave_dsi, &msg, handle, VM_VFP_EN);
		...
		if (dsi->slave_dsi)
			mtk_dsi_cmdq_poll_block(&dsi->slave_dsi->ddp_comp, handle,
				...
				unsigned int para_size)
{
	...
	/* wait DSI idle */
	if (!mtk_dsi_is_cmd_mode(comp)) {
		_mtk_dsi_set_mode(comp, handle, CMD_MODE, __LINE__);
		if (dsi->slave_dsi)
			_mtk_dsi_set_mode(&dsi->slave_dsi->ddp_comp, handle, CMD_MODE, __LINE__);
		cmdq_pkt_write(handle, comp->cmdq_base,
			comp->regs_pa + DSI_START, 0, ~0);
			...
			struct mtk_ddic_dsi_msg *cmd_msg)
{
	...
	if (dsi_mode == 0) { /* CMD mode HS/LP */
		...
		mtk_dsi_stop_vdo_mode(dsi, handle, __LINE__);
		if (dsi->slave_dsi)
			mtk_dsi_stop_vdo_mode(dsi->slave_dsi, handle, __LINE__);
		...
		mtk_dsi_start_vdo_mode(comp, handle, __LINE__);
		if (dsi->slave_dsi)
			mtk_dsi_start_vdo_mode(&dsi->slave_dsi->ddp_comp, handle, __LINE__);
		mtk_disp_mutex_trigger(comp->mtk_crtc->mutex[0], handle);
		mtk_dsi_trigger(comp, handle);
		mtk_use_cabc_event(handle, mtk_crtc, SET_OPT, __LINE__);
	}
			...
			struct mtk_ddic_dsi_msg *cmd_msg)
{
	...
	if (dsi_mode == 0) { /* CMD mode LP */
		...
		mtk_dsi_start_vdo_mode(comp, cmdq_handle, __LINE__);
			...
			int dsi_mode)
{
	...
	if (read_ddic_chk_sta == 0xff) {
		...
		mtk_crtc_pkt_create(&handle2, &mtk_crtc->base,
			mtk_crtc->gce_obj.client[CLIENT_CFG]);
		mtk_use_cabc_event(handle2, mtk_crtc, SET_OPT, __LINE__);
...
static void mtk_mipi_dsi_cmd_cb(struct cmdq_cb_data data)
{
	...
	DDPDSI_CMD("%s done, handle=0x%lx\n", __func__, (unsigned long)cb_data->cmdq_handle);
...
static void mtk_dsi_vm_cmd_transfer(struct mtk_dsi *dsi, struct cmdq_pkt *handle,
			const struct mtk_dsi_cmd_msg *cmd_msg)
{
	int i;
	struct mipi_dsi_msg msg = { 0 };
	...
	for (i = 0; i < cmd_msg->cmd_num; i++) {
		...
		/* start to send VM cmd */
		cmdq_pkt_write(handle, dsi->ddp_comp.cmdq_base,
			dsi->ddp_comp.regs_pa + DSI_START, 0, VM_CMD_START);
		cmdq_pkt_write(handle, dsi->ddp_comp.cmdq_base,
			dsi->ddp_comp.regs_pa + DSI_START, VM_CMD_START, VM_CMD_START);

		/* poll VM cmd done */
		mtk_dsi_cmdq_poll(&dsi->ddp_comp, handle,
			dsi->ddp_comp.regs_pa + DSI_INTSTA, VM_CMD_DONE_INT_EN, VM_CMD_DONE_INT_EN);
...
static int mtk_dsi_cmd_transfer(struct mtk_dsi *mtk_dsi, struct cmdq_pkt *handle,
				u32 flags, const struct mtk_dsi_cmd_msg *cmd_msg)
{
	...
	bool panel_mode = mtk_dsi_is_cmd_mode(&mtk_dsi->ddp_comp); /* 0:vdo, 1:cmd */
	...
	CRTC_MMP_MARK(index, ddic_cmd_v2_msg, dsi_mode, (unsigned long)cmd_msg);
	DDPDSI_CMD("%s ++, cmd_num=%u, transfer_mode:%d, package=%d, rd=%d, slot=%d, scn=%d, mode:(0x%x,0x%x)\n",
			__func__, cmd_msg->cmd_num, cmd_msg->transfer_mode, cmd_msg->is_package,
			cmd_msg->is_rd, cmd_msg->rd_to_slot, cmd_msg->read_scn, panel_mode, dsi_mode);
	...
	/* vdo mode */
	if (!panel_mode && !(flags & MTK_DSI_FROCE_CMD_MODE_TRANSFER)) {
		if (!dsi_mode)
		...
		if (flags & MTK_DSI_SKIP_STOP_VDO_MODE) {
			...
			mtk_dsi_stop_vdo_mode(mtk_dsi, handle, __LINE__);

			if (mtk_dsi->slave_dsi)
				cmdq_pkt_write(handle, mtk_dsi->slave_dsi->ddp_comp.cmdq_base,
						mtk_dsi->slave_dsi->ddp_comp.regs_pa +
						DSI_CON_CTRL(mtk_dsi->driver_data), 0, DSI_DUAL_EN);
...
cmd_mode_transfer:
	...
	if (cmd_msg->is_rd) {
		...
		if (need_start_vdo_mode) {
			DDPDSI_CMD("%s, start vdo mode by read\n", __func__);
			if (mtk_dsi->slave_dsi) {
				cmdq_pkt_write(handle, mtk_dsi->slave_dsi->ddp_comp.cmdq_base,
						mtk_dsi->slave_dsi->ddp_comp.regs_pa +
						DSI_CON_CTRL(mtk_dsi->driver_data),
						DSI_DUAL_EN, DSI_DUAL_EN);
			}
			mtk_dsi_start_vdo_mode(comp, handle, __LINE__);
			mtk_disp_mutex_trigger(comp->mtk_crtc->mutex[0], handle);
	...
	if (need_start_vdo_mode) {
		DDPDSI_CMD("%s, start vdo mode by write\n", __func__);
		if (mtk_dsi->slave_dsi) {
			cmdq_pkt_write(handle, mtk_dsi->slave_dsi->ddp_comp.cmdq_base,
					mtk_dsi->slave_dsi->ddp_comp.regs_pa + DSI_CON_CTRL(mtk_dsi->driver_data),
					DSI_DUAL_EN, DSI_DUAL_EN);
		}
		mtk_dsi_start_vdo_mode(comp, handle, __LINE__);
		mtk_disp_mutex_trigger(comp->mtk_crtc->mutex[0], handle);
	...
	CRTC_MMP_MARK(index, ddic_cmd_v2_msg, dsi_mode, cmd_msg->is_package);
	DDPDSI_CMD("%s done\n", __func__);
	return 0;
}
...
transfer_cmd:
	...
	ret = mtk_dsi_cmd_transfer(dsi, handle, flags, cmd_msg);
	if (flags & MTK_MIPI_DSI_CMD_BY_CPU) {
	...
	for (i = 0; i < MAX_DYN_CMD_NUM; i++) {
		...
		if (dfps_cmd->src_fps == 0 || drm_mode_vrefresh(old_mode) == dfps_cmd->src_fps) {
			if ((mtk_dsi_cmd_version() == DSI_CMD_V2) && dsi_cmd_v2_dbg[MODE_SWITCH_DBG]) {
				...
				msg.tx_len = dfps_cmd->cmd_num;
				msg.tx_buf = dfps_cmd->para_list;
				...
				mipi_dsi_dcs_write_gce_dyn(dsi, handle, dfps_cmd->para_list, dfps_cmd->cmd_num);
		}
	}
}
	...
	struct mtk_dsi *dsi, unsigned int ps_wc, int mode_idx)
{
	...
	if (panel_params->is_cphy) {
		...
		} else if (dsi->driver_data->support_frame_tb_v5) {
			u32 cphy_progseq_cycle = 2;
		...
		if (dsi->driver_data->n_verion >= VER_N4) {
			...
			da_hs_prep = (data_rate * 59 + 4000) / 8000 + 1;
			da_hs_zero = (data_rate * 163 + 11000) / 8000 + 1 - da_hs_prep;
			da_hs_trail = (data_rate * 78 + 7000) / 8000 + 1;
			da_hs_exit = (data_rate * 118) / 8000 + 1;
			  ...
			  enum mtk_ddp_io_cmd cmd, void *params)
{
...
#ifdef CONFIG_MI_DISP_DFS_EVENT
			if (*(int *)params != 0 && dsi->mi_cfg.whether_backlight_zero &&
				(get_jiffies_64() - dsi->mi_cfg.timestamp_backlight_zero < HZ)) {
				dsi->mi_cfg.whether_backlight_zero = false;
			}
			if (*(int *)params == 0) {
				if (!dsi->mi_cfg.whether_backlight_zero) {
	...
	case ESD_RESTORE_BACKLIGHT:
	{
		...
		panel_ext = mtk_dsi_get_panel_ext(comp);
		if ((mtk_dsi_cmd_version() == DSI_CMD_V2) && dsi_cmd_v2_dbg[BACKLIGHT_DBG] &&
			panel_ext && panel_ext->funcs && panel_ext->funcs->esd_restore_backlight_v2) {
			struct mtk_dsi_cmd_option cmd_opt = { 0 };

			cmd_opt.flags = MTK_MIPI_DSI_CMD_BY_CPU;
			panel_ext->funcs->esd_restore_backlight_v2(dsi->panel, dsi, 
				mtk_mipi_dsi_cmd, NULL, &cmd_opt);
		} else if (panel_ext && panel_ext->funcs &&
...
#if defined(CONFIG_VIS_DISPLAY_V2_D2)
	if (is_mi_dev_support_nova()) {
		vis_display_ops_send_mipi_cmd_mtk_notify(mi_disp_panel_ddic_send_cmd);
	}
#endif
...
#if defined(CONFIG_VIS_DISPLAY_V2_D2)
	if (is_mi_dev_support_nova()) {
		vis_display_ops_send_mipi_cmd_mtk_notify(NULL);
	}
#endif
}
