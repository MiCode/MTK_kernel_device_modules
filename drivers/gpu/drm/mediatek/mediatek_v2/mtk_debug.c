...
void mtk_dump_mminfra_ck(void *_priv, bool print_kernel)
{
	...
	if (priv->data->mmsys_id == MMSYS_MT6989) {
		...
		if (print_kernel)
			...
			DDPINFO("FM_MMINFRA_CK:%u VLP_VOTE_DONE:%u\n",
				mt_get_fmeter_freq(28, CKGEN_CK2), readl(vlp_vote_done));
		...
		if (print_kernel)
			...
			DDPINFO("FM_MMINFRA_CK:%u FM_DISP_CK:%u FM_EMIPLL:%u\n",
				mt_get_fmeter_freq(22, CKGEN_CK2),
				mt_get_fmeter_freq(20, CKGEN_CK2),
				mt_get_fmeter_freq(9, ABIST));
		...
		if (print_kernel)
			DDPMSG("FM_MMINFRA_CK:%u VLP_VOTE_DONE:%u, VDISP_CK:%u EMI_PLL:%u,%u\n",
				mt_get_fmeter_freq(21, CKGEN_CK2), readl(vlp_vote_done),
				mt_get_fmeter_freq(19, CKGEN_CK2),
				mt_get_fmeter_freq(9, ABIST),
				mt_get_fmeter_freq(10, ABIST));
		else
			DDPINFO("FM_MMINFRA_CK:%u FM_DISP_CK:%u FM_EMIPLL:%u\n",
				mt_get_fmeter_freq(22, CKGEN_CK2),
				mt_get_fmeter_freq(20, CKGEN_CK2),
...
static void process_dbg_opt(const char *opt)
{
	...
	if (strncmp(opt, "helper", 6) == 0) {
		...
		if (strncmp(opt + 7, "on", 2) == 0)
			...
			g_mobile_log = 0;
	} else if (strncmp(opt, "bif_disable:", 12) == 0) {
		struct drm_crtc *crtc;
		struct mtk_drm_crtc *mtk_crtc;
		bool bif_disable = false;
