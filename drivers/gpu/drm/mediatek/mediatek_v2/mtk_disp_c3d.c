	...
	const struct DISP_C3D_LUT *c3d_lut)
{
	...
	if ((c3d_data->bin_num != 9) && (c3d_data->bin_num != 17)) {
		pr_notice("%s, c3d bin num: %d not support", __func__, c3d_data->bin_num);
		return -1;
	}

	copysize = c3d_data->c3dlut_size * sizeof(unsigned int);
	C3DFLOW_LOG("c3dBinNum: %d, copysize:%d", c3d_data->bin_num, copysize);
	...
	if (!c3d_data->is_right_pipe) {
		if (copy_from_user(&primary_data->c3d_reg,
			...
			for (i = 0; i < c3d_data->c3dlut_size; i += 3) {
...
int disp_c3d_act_get_irq(struct mtk_ddp_comp *comp, void *data)
{
	int ret = 0;
...
int disp_c3d_cfg_eventctl(struct mtk_ddp_comp *comp,
...
int disp_c3d_act_eventctl(struct mtk_ddp_comp *comp, void *data)
{
	struct mtk_disp_c3d *c3d_data = comp_to_c3d(comp);
	struct mtk_disp_c3d_primary *primary_data = c3d_data->primary_data;
	int ret = 0;
...
int disp_c3d_act_set_lut(struct mtk_ddp_comp *comp, void *data)
{
	struct mtk_drm_crtc *mtk_crtc = comp->mtk_crtc;
	struct drm_crtc *crtc = &mtk_crtc->base;
	struct mtk_disp_c3d *c3d_data = comp_to_c3d(comp);
...
int disp_c3d_cfg_bypass(struct mtk_ddp_comp *comp,
	struct cmdq_pkt *handle, void *data, unsigned int data_size)
{
	int ret = 0;
	...
	struct cmdq_pkt *handle, void *data, unsigned int data_size)
{
	...
	if ((atomic_read(&c3d->c3d_is_clock_on) == 1)
		...
		disp_c3d_write_3dlut_to_reg(comp, (struct DISP_C3D_LUT *)data);
		if ((comp->mtk_crtc) && (comp->mtk_crtc->is_dual_pipe)) {
			...
			if ((atomic_read(&companion_data->c3d_is_clock_on) == 1)
					&& (mtk_crtc->enabled))
				disp_c3d_write_3dlut_to_reg(comp_c3d1,
	...
	struct cmdq_pkt *handle, unsigned int cmd, void *data, unsigned int data_size)
{
	...
	case PQ_C3D_EVENTCTL:
		/*TODO check trigger ??*/
		ret = disp_c3d_cfg_eventctl(comp, handle, data, data_size);
		break;
	case PQ_C3D_BYPASS:
		ret = disp_c3d_cfg_bypass(comp, handle, data, data_size);
		break;
	case PQ_C3D_SET_LUT:
		...
		unsigned int cmd, void *data, unsigned int data_size)
{
	...
	case PQ_C3D_GET_BIN_NUM:
		...
		break;
	case PQ_C3D_GET_IRQ:
		ret = disp_c3d_act_get_irq(comp, data);
		break;
	case PQ_C3D_EVENTCTL:
		ret = disp_c3d_act_eventctl(comp, data);
		break;
	case PQ_C3D_SET_LUT:
		ret = disp_c3d_act_set_lut(comp, data);
		break;
	case PQ_C3D_BYPASS:
