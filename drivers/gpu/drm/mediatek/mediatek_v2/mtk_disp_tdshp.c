	...
	struct cmdq_pkt *handle, int lock)
{
	...
	cmdq_pkt_write(handle, comp->cmdq_base, comp->regs_pa + MDP_TDSHP_SIZE_PARA,
		...
		disp_tdshp_regs->SIZE_PARA_BIG_HUGE << 12), ~0);
	...
	cmdq_pkt_write(handle, comp->cmdq_base,
		...
		disp_tdshp_regs->Regional_en << 18), ~0);
	...
	struct mtk_ddp_config *cfg, struct cmdq_pkt *handle)
{
	...
	if (tdshp_data->data->need_bypass_shadow)
		cmdq_pkt_write(handle, comp->cmdq_base,
			comp->regs_pa + DISP_TDSHP_SHADOW_CTRL, 0x1 | 1 << 1, ~0);
	else
		cmdq_pkt_write(handle, comp->cmdq_base,
			comp->regs_pa + DISP_TDSHP_SHADOW_CTRL, 0x0 | 1 << 1, ~0);
