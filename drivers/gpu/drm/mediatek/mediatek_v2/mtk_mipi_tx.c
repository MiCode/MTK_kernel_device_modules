...
static int mtk_mipi_tx_pll_dphy_config_mt6993(struct mtk_mipi_tx *mipi_tx)
{
	unsigned int txdiv, txdiv0, tmp;
	u32 rate;
	unsigned int fbksel;
	...
	if (rate >= 2000) {
		...
		DDPPR_ERR("data rate error, data rate is too low\n");
		return -EINVAL;
	}
...
static int mtk_mipi_tx_pll_cphy_config_mt6993(struct mtk_mipi_tx *mipi_tx)
{
	unsigned int txdiv, txdiv0, tmp;
	u32 rate;
	unsigned int fbksel;
