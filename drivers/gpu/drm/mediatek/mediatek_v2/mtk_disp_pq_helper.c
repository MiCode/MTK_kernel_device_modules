...
//	{PQ_C3D_GET_IRQ_STATUS,              0,                                            0,},
	...
	{PQ_DBI_GET_PANEL_TYPE,              sizeof(int),                                  PQ_CMD_RW,},
	...
	void *data, bool user_lock, bool is_kernel_data)
{
	...
	if (priv->data->config_urgent && !mtk_crtc_is_frame_trigger_mode(crtc)) {
		if (index == 0) {
			if (user_lock)
				DDP_MUTEX_LOCK_CONDITION(&mtk_crtc->lock, __func__, __LINE__, false);
			...
			if (fps == 120)
				cmdq_pkt_write(pq_cmdq_handle, mtk_crtc->gce_obj.base,
