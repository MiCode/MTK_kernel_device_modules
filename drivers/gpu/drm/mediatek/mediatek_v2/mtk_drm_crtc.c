...
static unsigned int need_down_urgent;
#endif
...
void set_bif_enable(struct drm_crtc *crtc, bool en, int line)
{
	...
	if (!mtk_crtc->bif_info)
		return;
	if (en) {
		if (!mtk_crtc->bif_info->bif_enable)
			DDPMSG("set BIF_ENABLE(%d)\n", line);
		mtk_crtc->bif_info->bif_enable = priv->bif_support_mode;
	} else {
		mtk_crtc->bif_info->bif_enable = BIF_DISABLE;
		CRTC_MMP_MARK(0, bif_src_release, 0xFFFFFFFF, line);
		DDPMSG("set BIF_DISABLE(%d)\n", line);
	}
}
...
void mtk_crtc_bif_enable_mutex(struct mtk_drm_crtc *mtk_crtc, struct cmdq_pkt *handle, bool en)
{
	int i;
	...
	for (i = 0; i < ddp_ctx->bif_write_comp_nr; i++) {
		...
		if (handle) {
			if (en)
				mtk_disp_mutex_add_comp_with_cmdq(mtk_crtc, comp->id, false, handle, 0);
			else
				mtk_disp_mutex_remove_comp_with_cmdq(mtk_crtc, comp->id, handle, 0);
		} else {
			if (en)
...
void mtk_crtc_bif_enable_racing(struct mtk_drm_crtc *mtk_crtc, struct cmdq_pkt *handle)
{
	...
	DDPBIF("%s,w:%s,r:%s\n", __func__,
		mtk_dump_comp_str(bif_info->wb_comp), mtk_dump_comp_str(bif_info->read_comp));
	...
	mtk_crtc_bif_enable_mutex(mtk_crtc, handle, true);
...
static void mtk_drm_crtc_init_bind_comp(struct mtk_drm_crtc *mtk_crtc)
{
	...
	for_each_comp_in_crtc_path_bound(comp, mtk_crtc, i, j, 1) {
		...
		if (next_comp &&
		    ...
		    if (mtk_ddp_comp_get_type(next_comp->id) == MTK_OVL_BLENDER) {
				...
				pre_comp = comp;
				comp->bind_comp = NULL;
				DDPINFO("Remove no use bind blender comp:%d\n",comp->id);
			}
	...
	if (drm_crtc_index(&mtk_crtc->base) == 0 && mtk_crtc_is_dual_pipe(&(mtk_crtc->base))) {
		for_each_comp_in_dual_pipe(comp, mtk_crtc, i, j) {
			if (j >= (mtk_crtc->dual_pipe_ddp_ctx.ovl_comp_nr[i]))
				continue;
			next_comp = mtk_crtc_get_dual_comp(crtc, i, j + 1);
			if (pre_comp &&
			    (mtk_ddp_comp_get_type(pre_comp->id) == MTK_OVL_EXDMA)) {
				if (mtk_ddp_comp_get_type(next_comp->id) == MTK_OVL_BLENDER) {
					pre_comp->bind_comp = next_comp;
	...
	if(!mtk_crtc->last_blender) {
		...
		if (value >= 0 && config_regs_pa > 0) {
			if ((mtk_crtc->last_blender->id == comp->id ||
			    DUAL_MAPPING_BLENDER(mtk_crtc->last_blender->id) == comp->id) &&
			    !reset_flag)
				cmdq_pkt_write(cmdq_handle, mtk_crtc->gce_obj.base,
							config_regs_pa + addr, value, value);
			else
...
retrigger_blank_connect:
	...
	if (plane_state && plane_state->comp_state.blender_comp_id != 0) {
		...
		if (mtk_crtc->is_dual_pipe) {
			r_comp = priv->ddp_comp[DUAL_MAPPING_BLENDER(plane_state->comp_state.blender_comp_id)];
			if (r_comp && r_comp->funcs && r_comp->funcs->connect) {
				if (DUAL_MAPPING_BLENDER(mtk_crtc->last_blender->id) == r_comp->id)
					r_comp->funcs->connect(r_comp, cmdq_handle, r_comp->id, 0);
				else
					r_comp->funcs->connect(r_comp, cmdq_handle, r_comp->id, r_comp->id);
			}
		...
		if (plane_id == 0 && mtk_crtc->last_blender == mtk_crtc->first_blender && mtk_crtc->is_dual_pipe) {
			r_comp = priv->ddp_comp[DUAL_MAPPING_BLENDER(mtk_crtc->first_blender->id)];
			if (r_comp && r_comp->funcs && r_comp->funcs->connect) {
				if (DUAL_MAPPING_BLENDER(mtk_crtc->last_blender->id) == r_comp->id)
					r_comp->funcs->connect(r_comp, cmdq_handle, r_comp->id, 0);
				else
...
void mtk_drm_crtc_exdma_path_setting_reset_without_cmdq(struct mtk_drm_crtc *mtk_crtc)
{
	...
	void __iomem *ovl1_mutex_regs = 0;
	void __iomem * ovlsys_base = 0;
	struct mtk_ddp_comp *first_blender;
	struct mtk_ddp_comp *first_blender_map;
	enum mtk_ddp_comp_id out_proc;
	enum mtk_ddp_comp_id out_proc1;
	struct mtk_drm_private *priv = mtk_crtc->base.dev->dev_private;
	unsigned int addr = 0;
	int value = 0;
	int value1 = 0;
...
void mtk_drm_crtc_dump(struct drm_crtc *crtc)
{
	...
	if (bif_enabled(crtc)) {
		...
		for (i = 0; i < ddp_ctx->bif_write_comp_nr; i++) {
			if (mtk_ddp_comp_get_type(ddp_ctx->bif_write_comp[i]->id) == MTK_DISP_VIRTUAL)
				continue;
			mtk_dump_reg(ddp_ctx->bif_write_comp[i]);
		}
				       ...
				       struct drm_crtc_state *state)
{
	...
	DDPINFO("destroy/free crtc_state, 0x%08x, crtc->state=0x%08x\n",
				(uint32_t)(uintptr_t)state, (uint32_t)(uintptr_t)crtc->state);
	kfree(s);
}
...
void mtk_crtc_bif_comp_stop(struct mtk_drm_crtc *mtk_crtc, struct cmdq_pkt *cmdq_handle)
{
	struct mtk_ddp_comp *comp;
	struct mtk_crtc_ddp_ctx *ddp_ctx = &mtk_crtc->ddp_ctx[mtk_crtc->ddp_mode];
	struct mtk_bif_info *bif_info = mtk_crtc->bif_info;
	...
	struct cmdq_pkt *cmdq_handle)
{
	...
	for (i = 0; i < ddp_ctx->bif_read_comp_nr; i++) {
		...
		if ((mtk_ddp_comp_get_type(comp->id) == MTK_DISP_MDP_RDMA) ||
			(mtk_ddp_comp_get_type(comp->id) == MTK_DISP_GDMA))
...
void mtk_crtc_bif_path_prepare(struct mtk_drm_crtc *mtk_crtc)
{
	...
	for (i = 0; i < ddp_ctx->bif_write_comp_nr-1; i++) {
		...
		if (i == 0) {
			...
			tmp_comp = ddp_ctx->bif_write_comp[i-1];
			prev_id = tmp_comp->id;
		}
		if (i < (ddp_ctx->bif_write_comp_nr-1)) {
			tmp_comp = ddp_ctx->bif_write_comp[i+1];
...
void mtk_crtc_bif_data_init(struct mtk_drm_crtc *mtk_crtc,
	struct drm_device *drm_dev, const struct mtk_crtc_path_data *path_data)
{
	...
	mtk_crtc->ddp_ctx[i].bif_write_comp_nr = path_data->bif_write_path_len[i];
	mtk_crtc->ddp_ctx[i].bif_write_comp = devm_kmalloc_array(
		dev, path_data->bif_write_path_len[i],
	...
	for_each_bif_write_comp_id_in_path_data(comp_id, path_data, i, p_mode) {
		if (comp_id < 0) {
			DDPPR_ERR("%s: comp_id:%d error\n", __func__, comp_id);
			goto err;
		}
		...
		if (mtk_ddp_comp_get_type(comp_id) == MTK_DISP_VIRTUAL) {
			...
			if (!comp)
				goto err;
			comp->id = comp_id;
			mtk_crtc->ddp_ctx[p_mode].bif_write_comp[i] = comp;
			continue;
		}
		if (comp_id < 0) {
			DDPPR_ERR("%s: comp_id:%d error\n", __func__, comp_id);
...
void mtk_crtc_update_bif_roi(struct mtk_drm_crtc *mtk_crtc)
{
	...
	w_dsc = (w + 3 - 1)/3;// width_floor after dsc

	if (align)
		bif_info->wdma_offset = (align - (w_dsc%align)) % align;

	if ((w != bif_info->lcm_width) || (h != bif_info->lcm_height)
	...
	if (unlikely(bif_mode)) {
		...
		mtk_crtc_bif_data_init(mtk_crtc, mtk_crtc->base.dev, mtk_crtc->path_data);
		return;
	}

	bif_info->bif_enable = priv->bif_support_mode;
}
	...
	drm_for_each_plane_mask(plane, crtc->dev, plane_mask) {
		...
		if (mtk_crtc && mtk_crtc->path_data && mtk_crtc->path_data->is_exdma_dual_layer && priv) {
			...
			if (comp_idx < DDP_COMPONENT_ID_MAX) {
				mtk_ddp_comp_io_cmd(priv->ddp_comp[comp_idx], NULL, OVL_PHY_USAGE, plane_state);
				mtk_crtc_update_ovl_bwm2_phy(crtc, priv->ddp_comp[comp_idx], plane_state, true);
			}
		}
		if (mtk_crtc && mtk_crtc->is_dual_pipe && priv) {
			comp_idx = dual_pipe_comp_mapping(priv->data->mmsys_id, comp->id);
...
static struct disp_module_cksm_info dsc_cksm_info = {
	...
	.name = "dsc0",
	.start = &g_dsc_chksum_start,
	.stop = &g_dsc_chksum_stop,
	.instances = {
		{.name = "mon0",
...
struct disp_module_patgen_info {
	const bool *en;
	const bool *dis;
	const enum mtk_ddp_comp_id comp_id; // comp_id of the module
	int crtc_idx; // which crtc should this module follow and check
...
static struct disp_module_patgen_info dsi_self_pat_info = {
	.comp_id = DDP_COMPONENT_DSI0,
	.en = &g_dsi_self_pat_en,
	.dis = &g_dsi_self_pat_dis,
};

static struct disp_module_patgen_info dsc_mute_mode_info = {
	.comp_id = DDP_COMPONENT_DSC0,
	.en = &g_dsc_mute_enable,
	.dis = &g_dsc_mute_disable,
};
...
#endif
{
	...
	DDPINFO("crtc_state:0x%08x, atomic_state:%08x, crtc:%08x, pf:%u, crtc->state:0x%08x\n",
		(uint32_t)(uintptr_t)crtc_state, (uint32_t)(uintptr_t)atomic_state,
		(uint32_t)(uintptr_t)crtc, cb_data->pres_fence_idx, (uint32_t)(uintptr_t)crtc->state);
...
VDO_MODE:
	...
	if ((priv->mtk_dbgtp_sta.fifo_mon_en[0]) && (crtc_id == 0)) {
		...
		/* gce if (A || B) start */
		slot_addr = mtk_get_gce_backup_slot_pa(mtk_crtc, DISP_SLOT_BIF_CONDITION);
		/* clear result */
...
//			}
		...
		/* Clear vidle exception bit set by atomic commit pkt */
		if ((priv->data->mmsys_id == MMSYS_MT6993) && (crtc_id == 0))
			cmdq_pkt_write(cmdq_handle, NULL, 0x31350168, BIT(DISP_VIDLE_USER_FOR_FRAME), ~0);
...
void mtk_crtc_restore_plane_setting(struct mtk_drm_crtc *mtk_crtc)
{
	...
	if (priv->data->ovl_exdma_rule
		...
		if (mtk_crtc->is_dual_pipe) {
			...
			if (comp_idx < DDP_COMPONENT_ID_MAX)
				mtk_ddp_comp_io_cmd(priv->ddp_comp[cmp_id], cmdq_handle, PMQOS_UPDATE_BW, NULL);
		}
	}
	for_each_comp_in_cur_crtc_path(comp, mtk_crtc, i, j)
		mtk_ddp_comp_io_cmd(comp, cmdq_handle,
...
void mtk_crtc_all_layer_off(struct mtk_drm_crtc *mtk_crtc, struct cmdq_pkt *cmdq_handle)
{
	...
	if (mtk_crtc->is_dual_pipe) {
		...
		for_each_comp_in_dual_pipe(comp, mtk_crtc, i, j) {
			mtk_ddp_comp_io_cmd(comp, cmdq_handle,
				OVL_ALL_LAYER_OFF, &keep_first_layer);
			if (comp->id == DUAL_MAPPING_BLENDER(mtk_crtc->first_blender->id))
				keep_first_layer = false;
		}
	}
}
	...
	if (priv->data->ovl_exdma_rule) {
		...
		if (cmp_id < DDP_COMPONENT_ID_MAX)
			mtk_ddp_comp_io_cmd(priv->ddp_comp[cmp_id],
				NULL, PMQOS_UPDATE_BW, &flag);
		if (mtk_crtc->is_dual_pipe) {
			cmp_id = dual_pipe_comp_mapping(priv->data->mmsys_id, cmp_id);
			if (cmp_id < DDP_COMPONENT_ID_MAX)
				mtk_ddp_comp_io_cmd(priv->ddp_comp[cmp_id],
					NULL, PMQOS_UPDATE_BW, &flag);
		}
...
void mtk_drm_crtc_enable(struct drm_crtc *crtc, bool need_report_bw)
{
	...
	/* mtk_crtc_is_frame_trigger_mode: is cmd mode or not
	   prop_val[CRTC_PROP_DOZE_ACTIVE]: is doze mode or not
	   doze_changed: is doze mode changed or not
	   skip_frame: is skip this frame or not
	   recovery_flg: is in esd recovery or not
	   if cmd mode && not doze mode && doze mode not changed && not skip this frame
	   && in esd recovery mode, then do not set dirty when esd recovery in case of
	   a pu frame to be sent */
	if (mtk_crtc_is_frame_trigger_mode(crtc) &&
		!mtk_state->prop_val[CRTC_PROP_DOZE_ACTIVE] &&
		!mtk_state->doze_changed && !mtk_crtc->skip_frame &&
		!mtk_crtc->recovery_flg) {
...
void mtk_crtc_first_enable_ddp_config(struct mtk_drm_crtc *mtk_crtc)
{
	...
	if (priv->data->mmsys_id == MMSYS_MT6985 ||
		...
		if (mtk_crtc->sys_b_config_regs) {
			...
			/*Set BYPASS_MUX_SHADOW*/
			writel(0x00ff0001, mtk_crtc->sys_b_config_regs +
					DISP_REG_CONFIG_MMSYS_BYPASS_MUX_SHADOW);
		}
	...
	if (priv->data->ovl_exdma_rule && (cmp_id < DDP_COMPONENT_ID_MAX)
		...
		if (mtk_crtc->is_dual_pipe) {
			comp_idx = dual_pipe_comp_mapping(priv->data->mmsys_id, cmp_id);
			mtk_ddp_comp_first_cfg(priv->ddp_comp[comp_idx],
				&cfg, cmdq_handle);
		}
		if (!mtk_drm_helper_get_opt(priv->helper_opt, MTK_DRM_OPT_USE_PQ)) {
			mtk_crtc->pq_data->opt_bypass_pq = true;
			...
			if (mtk_crtc->is_dual_pipe) {
				mtk_ddp_comp_bypass(priv->ddp_comp[comp_idx],
					1, PQ_FEATURE_KRN_OPT_USE_PQ, cmdq_handle);
			}
		} else {
			if (mtk_crtc->pq_data->opt_bypass_pq)
		...
		mtk_ddp_comp_io_cmd(priv->ddp_comp[cmp_id],
			cmdq_handle, PMQOS_UPDATE_BW, NULL);
		if (mtk_crtc->is_dual_pipe) {
			mtk_ddp_comp_io_cmd(priv->ddp_comp[comp_idx],
				cmdq_handle, IRQ_LEVEL_NORMAL, NULL);
			mtk_ddp_comp_io_cmd(priv->ddp_comp[comp_idx],
				cmdq_handle, DSI_SET_TARGET_LINE, &cfg);
			mtk_ddp_comp_io_cmd(priv->ddp_comp[comp_idx],
				cmdq_handle,
				MTK_IO_CMD_RDMA_GOLDEN_SETTING, &cfg);
			cfg.source_bpc = mtk_ddp_comp_io_cmd(priv->ddp_comp[comp_idx],
				cmdq_handle, OVL_GET_SOURCE_BPC, NULL);
			/* update SRT BW */
			mtk_ddp_comp_io_cmd(priv->ddp_comp[comp_idx],
				cmdq_handle, PMQOS_UPDATE_BW, NULL);
...
static unsigned int get_ovl_layer_nr(enum mtk_ddp_comp_id comp_id)
{
	switch (comp_id) {
	case DDP_COMPONENT_OVL_EXDMA2:
	case DDP_COMPONENT_OVL_EXDMA3:
...
void mtk_drm_crtc_first_enable(struct drm_crtc *crtc)
{
	...
	for_each_comp_in_cur_crtc_path(comp, mtk_crtc, i ,j) {
		if (mtk_ddp_comp_get_type(comp->id) == MTK_OVL_EXDMA)
			real_layer_nr += get_ovl_layer_nr(comp->id);
	}
	mtk_crtc->real_layer_nr = real_layer_nr;
	...
	struct drm_crtc *crtc, struct disp_module_patgen_info *info)
{
	...
	comp = priv->ddp_comp[info->comp_id];
	if (*info->en == true)
		mtk_ddp_comp_config_trigger(comp, cmdq_handle, MTK_TRIG_FLAG_PATGEN_EN);
	if (*info->dis == true)
		mtk_ddp_comp_config_trigger(comp, cmdq_handle, MTK_TRIG_FLAG_PATGEN_DIS);
}
						...
						bool need_sync_mml)
{
	...
	/* Set vidle exception bit, cleared in trigger loop frame done,
	 * to prevent redundant power on/off flow before frame done
	 */
	if ((priv->data->mmsys_id == MMSYS_MT6993) && (crtc_id == 0))
		cmdq_pkt_write(cmdq_handle, NULL, 0x31350164, BIT(DISP_VIDLE_USER_FOR_FRAME), ~0);
	...
	if (crtc_id == 0) {
		...
		if (fps == 120 || fps == 90 || fps == 165 || mtk_crtc->mode_chg){
			cmdq_pkt_write(cmdq_handle, mtk_crtc->gce_obj.base,
	...
	unsigned int w)
{
	...
	if (mtk_crtc->path_data->is_exdma_dual_layer) {
		...
		DUAL_MAPPING_BLENDER(plane_state->comp_state.blender_comp_id);
	...
	if (!only_output) {
		...
		if (priv->data->ovl_exdma_rule && (cmp_id < DDP_COMPONENT_ID_MAX)
			...
			if (mtk_crtc->is_dual_pipe) {
				comp_idx = dual_pipe_comp_mapping(priv->data->mmsys_id, cmp_id);
				if (crtc->state->color_mgmt_changed)
					mtk_ddp_gamma_set(priv->ddp_comp[comp_idx],
					crtc->state, cmdq_handle);
				mtk_ddp_comp_io_cmd(priv->ddp_comp[comp_idx],
					cmdq_handle, COMP_ODDMR_CFG, &mtk_crtc->sec_on);
				mtk_ddp_comp_io_cmd(priv->ddp_comp[comp_idx],
...
#define CAPS_CHAR_SIZE 25
	...
	for (i = 0; i < DDP_MODE_NR; i++) {
		...
		if (priv->bif_support_mode && (pipe == 0))
			mtk_crtc_bif_data_init(mtk_crtc, drm_dev, path_data);
	}
