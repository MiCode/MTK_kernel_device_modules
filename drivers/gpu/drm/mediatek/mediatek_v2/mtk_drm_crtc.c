...
int dbg_trig_db;
module_param(dbg_trig_db, int, 0644);
...
int bif_mode;
module_param(bif_mode, int, 0644);
...
#ifdef CONFIG_MI_DISP
#ifdef CONFIG_VIS_DISPLAY_DALI
	{DRM_MODE_PROP_ATOMIC, "ASIC_EXT_MV", 0, ULONG_MAX, 0},
#endif
#endif
...
#define CMDQ_TIMER_PERIOD	10 /* 10s */
static int cmdq_timeout_cnt = 5;
module_param(cmdq_timeout_cnt, int, 0644);
static int once_value = 0;
module_param(once_value, int, 0644);
#endif

#ifdef CONFIG_MI_DISP_DFS_EVENT
struct timer_list cmdq_timeout_report_timer;
#endif
...
static void mtk_drm_crtc_reset(struct drm_crtc *crtc)
{
	...
	if (dev)
		of_red = of_property_read_u32(dev->of_node, "lk_mode_index", &lk_mode_index);
#endif
...
#ifdef CONFIG_MI_DISP
	if (of_red == 0) {
		state->prop_val[CRTC_PROP_DISP_MODE_IDX] = lk_mode_index;
		DDPMSG("init the refresh rate property to %llu\n", state->prop_val[CRTC_PROP_DISP_MODE_IDX]);
	} else {
		DDPMSG("init the refresh rate property fail!! of_property_read_u32 not found : %d\n", of_red);
	}
#endif
...
int bif_enabled(struct drm_crtc *crtc)
{
	...
	if (!mtk_crtc->bif_info)
		return en;
	if (!mtk_crtc->bif_info->wdma_offset)
		return en;
	if (!priv->data->support_bif)
		return en;
	crtc_index = drm_crtc_index(crtc);
	if (crtc_index) {
		DDPDBG("%s:invalid crtc:0x%p, index:%d\n", __func__, crtc, crtc_index);
		return en;
	}
	if (mtk_crtc_is_frame_trigger_mode(crtc))
		return en;

	return mtk_crtc->bif_info->bif_enable;
}
void mtk_crtc_bif_backup_path_mutex(struct mtk_drm_crtc *mtk_crtc)
{
	...
	bool racing_en = mtk_crtc->bif_info->racing_en;
	...
	mtk_crtc->bif_info->ovlsys_0[0] = readl(ddp->ovlsys0_regs + DISP_REG_MUTEX_MOD(0, ddp->data, mutex->id));
	mtk_crtc->bif_info->ovlsys_0[1] = readl(ddp->ovlsys0_regs + DISP_REG_MUTEX_MOD(1, ddp->data, mutex->id));
	mtk_crtc->bif_info->ovlsys_1[0] = readl(ddp->ovlsys1_regs + DISP_REG_MUTEX_MOD(0, ddp->data, mutex->id));
	mtk_crtc->bif_info->ovlsys_1[1] = readl(ddp->ovlsys1_regs + DISP_REG_MUTEX_MOD(1, ddp->data, mutex->id));
	mtk_crtc->bif_info->dispsys_0[0] = readl(ddp->regs + DISP_REG_MUTEX_MOD(0, ddp->data, mutex->id));
	mtk_crtc->bif_info->dispsys_0[1] = readl(ddp->regs + DISP_REG_MUTEX_MOD(1, ddp->data, mutex->id));
...
void mtk_crtc_bif_enable_mutex(struct mtk_drm_crtc *mtk_crtc, struct cmdq_pkt *handle)
{
	int i;
...
void mtk_crtc_bif_enable_racing(struct mtk_drm_crtc *mtk_crtc, struct cmdq_pkt *handle)
{
	...
	if (bif_info->wb_comp) {
		cmdq_pkt_write(handle, bif_info->wb_comp->cmdq_base,
				 bif_info->wb_comp->larb_cons[0], GENMASK(19, 16), GENMASK(19, 16));
		// update wdma bw
		bpp = mtk_get_format_bpp(format);
		...
		mtk_ddp_comp_io_cmd(bif_info->wb_comp, NULL, PMQOS_SET_HRT_BW, &bw_base);
		 bif_info->wb_comp->hrt_bw = bw_base;
	}

	if (bif_info->read_comp)
		cmdq_pkt_write(handle, bif_info->read_comp->cmdq_base,
			bif_info->read_comp->larb_cons[0], GENMASK(19, 16), GENMASK(19, 16));

	if ((bif_enabled(crtc) == BIF_HS_IDLE) && priv->data->bif_path_insert)
		priv->data->bif_path_insert(mtk_crtc, handle);

	if (bif_enabled(crtc)  == BIF_ALL_SCN) {
	...
	mtk_disp_mutex_remove_comp_with_cmdq(mtk_crtc, dsc_comp->id, handle, 0);
	mtk_crtc_bif_enable_mutex(mtk_crtc, handle);

	if (priv->data->bif_racing_config)
		priv->data->bif_racing_config(handle, mtk_crtc, true);
}
...
void mtk_crtc_bif_restore_path_mutex(struct mtk_drm_crtc *mtk_crtc, struct cmdq_pkt *handle)
{
	...
	cmdq_pkt_write(handle, mtk_crtc->gce_obj.base,
		...
		mtk_crtc->bif_info->ovlsys_0[0], ~0);
	cmdq_pkt_write(handle, mtk_crtc->gce_obj.base,
		ddp->ovlsys0_regs_pa + DISP_REG_MUTEX_MOD(1, ddp->data, mutex->id),
		mtk_crtc->bif_info->ovlsys_0[1], ~0);

	cmdq_pkt_write(handle, mtk_crtc->gce_obj.base,
		ddp->ovlsys1_regs_pa + DISP_REG_MUTEX_MOD(0, ddp->data, mutex->id),
		mtk_crtc->bif_info->ovlsys_1[0], ~0);
	cmdq_pkt_write(handle, mtk_crtc->gce_obj.base,
		ddp->ovlsys1_regs_pa + DISP_REG_MUTEX_MOD(1, ddp->data, mutex->id),
		mtk_crtc->bif_info->ovlsys_1[1], ~0);

	cmdq_pkt_write(handle, mtk_crtc->gce_obj.base,
		ddp->regs_pa + DISP_REG_MUTEX_MOD(0, ddp->data, mutex->id),
		mtk_crtc->bif_info->dispsys_0[0], ~0);
	cmdq_pkt_write(handle, mtk_crtc->gce_obj.base,
		ddp->regs_pa + DISP_REG_MUTEX_MOD(1, ddp->data, mutex->id),
		mtk_crtc->bif_info->dispsys_0[1], ~0);

	cmdq_pkt_write(handle, mtk_crtc->gce_obj.base,
		ddp->side_regs_pa + DISP_REG_MUTEX_MOD(0, ddp->data, mutex->id),
		mtk_crtc->bif_info->dispsys_1[0], ~0);
	cmdq_pkt_write(handle, mtk_crtc->gce_obj.base,
		ddp->side_regs_pa + DISP_REG_MUTEX_MOD(1, ddp->data, mutex->id),
		mtk_crtc->bif_info->dispsys_1[1], ~0);

	if (mtk_crtc->bif_info->read_comp)
		cmdq_pkt_write(handle, mtk_crtc->bif_info->read_comp->cmdq_base,
			mtk_crtc->bif_info->read_comp->larb_cons[0], 0x0, GENMASK(19, 16));
	if (mtk_crtc->bif_info->wb_comp)
		cmdq_pkt_write(handle, mtk_crtc->bif_info->wb_comp->cmdq_base,
			mtk_crtc->bif_info->wb_comp->larb_cons[0], 0x0, GENMASK(19, 16));
}

void mtk_crtc_bif_keep_read_path(struct drm_crtc *crtc, struct cmdq_pkt *handle, bool bw)
{
	struct mtk_drm_private *priv = crtc->dev->dev_private;
	struct mtk_drm_crtc *mtk_crtc = to_mtk_crtc(crtc);
	struct mtk_bif_info *bif_info = mtk_crtc->bif_info;
	struct mtk_ddp_comp *wb_comp = bif_info->wb_comp;
	unsigned int bw_zero;
	...
	if (priv->data->bif_racing_config)
		priv->data->bif_racing_config(handle, mtk_crtc, false);

	if (priv->data->bif_read_path_mutex)
		priv->data->bif_read_path_mutex(mtk_crtc, handle);
	...
	if (priv->power_state) {
		if (wb_comp) {
			...
			mtk_ddp_comp_io_cmd(wb_comp, NULL, PMQOS_SET_HRT_BW, &bw_zero);
			wb_comp->hrt_bw = bw_zero;
			//DDP_MUTEX_UNLOCK(&mtk_crtc->lock, __func__, __LINE__);
		}
	}
}
void mtk_crtc_bif_apsrc_ddren_control(struct mtk_drm_crtc *mtk_crtc, bool en, struct cmdq_pkt *handle)
{
	struct drm_crtc *crtc = &mtk_crtc->base;
	struct mtk_drm_private *priv = crtc->dev->dev_private;
...
void mtk_drm_crtc_dump(struct drm_crtc *crtc)
{
	...
	case MMSYS_MT6991:
		...
		mtk_crtc_dump_bif_info(mtk_crtc, mtk_crtc->side_config_regs);
	...
	if (bif_enabled(crtc)) {
		...
		for (i = 0; i < ddp_ctx->wb_comp_nr; i++) {
			if (mtk_ddp_comp_get_type(ddp_ctx->wb_comp[i]->id) == MTK_DISP_VIRTUAL)
				continue;
			mtk_dump_reg(ddp_ctx->wb_comp[i]);
		}

		for (i = 0; i < ddp_ctx->bif_read_comp_nr; i++) {
			if (mtk_ddp_comp_get_type(ddp_ctx->bif_read_comp[i]->id) == MTK_DISP_VIRTUAL)
				continue;
			mtk_dump_reg(ddp_ctx->bif_read_comp[i]);
		}
	}
	//addon from CWB
...
void mtk_crtc_extmv_notify(struct mtk_drm_crtc *mtk_crtc)
{
	...
	if (mtk_crtc == NULL) {
		pr_err("[EXTMV][%s(%d)] Invalid mtk_crtc is NULL and return\n", __func__, __LINE__);
		return;
	}

	output_comp = mtk_ddp_comp_request_output(mtk_crtc);
	mtk_dsi = container_of(output_comp, struct mtk_dsi, ddp_comp);
	if (mtk_dsi->ext_mv_enable == 0) {
		return;
	}

	if (mtk_dsi->meta_content == NULL) {
		return;
	}

	complete_all(&mtk_dsi->ext_mv_completion);
}

static void mtk_drm_crtc_d2_mv_parser(struct drm_crtc *crtc)
{
	struct mtk_crtc_state *state = NULL;
	int mv_blob_id = 0;
	struct drm_property_blob *mv_blob = NULL;
	struct mtk_drm_crtc *mtk_crtc = NULL;
	struct mtk_ddp_comp *output_comp = NULL;
	struct mtk_dsi *mtk_dsi = NULL;

	if (crtc == NULL) {
		pr_err("[EXTMV][%s(%d)] Invalid crtc is NULL and return\n", __func__, __LINE__);
		return;
	}

	state = to_mtk_crtc_state(crtc->state);
	mv_blob_id = (int)state->prop_val[CRTC_PROP_EXT_MV];
	if (mv_blob_id == 0) {
		return;
	}

	mv_blob = drm_property_lookup_blob(crtc->dev, mv_blob_id);
	if (mv_blob == NULL) {
		pr_err("[EXTMV][%s(%d)] It is failed to get mv blob(0x%08x)\n", __func__, __LINE__, mv_blob_id);
		return;
	}
	...
	mutex_lock(&mtk_dsi->extmv_lock);
	if (mtk_dsi->meta_content != NULL) {
		kfree(mtk_dsi->meta_content);
		mtk_dsi->meta_content = NULL;
		mtk_dsi->meta_length = 0;
	}
	mtk_dsi->meta_content = (unsigned int*)kzalloc(mv_blob->length * sizeof(char), GFP_KERNEL);
	if (mtk_dsi->meta_content != NULL) {
		memcpy(mtk_dsi->meta_content, mv_blob->data, mv_blob->length * sizeof(char));
		mtk_dsi->meta_length = (unsigned int)mv_blob->length;
	}
	mutex_unlock(&mtk_dsi->extmv_lock);

	drm_property_blob_put(mv_blob);
...
void mtk_drm_crtc_dump_vr_rg(struct drm_crtc *crtc)
{
	...
	if(mtk_crtc->is_dual_pipe && mtk_crtc->ovlsys1_regs)
		ovlsys_config_dump_analysis_mt6991(mtk_crtc->ovlsys1_regs, true);
...
static void bl_cmdq_cb(struct cmdq_cb_data data)
{
	...
	if(atomic_read(&mtk_crtc->dbi_set_freq_floor_cnt1)>0) {
		atomic_dec(&mtk_crtc->dbi_set_freq_floor_cnt1);
		user_dbi.rg_num = atomic_read(&mtk_crtc->dbi_bl_chg_reg_num);
		mtk_disp_set_freq_floor_by_cond(mtk_crtc, false, GCE_DBI, &user_dbi);
	}
}
	...
	unsigned int panel_ext_param, unsigned int cfg_flag)
{
	...
	struct gce_user_dbi user_dbi = { 0 };
	...
	mtk_ddp_comp_io_cmd(oddmr_comp, cmdq_handle, ODDMR_GET_DBI_BL_CHG_REG_NUM, &user_dbi.rg_num);

	if(user_dbi.rg_num && (!is_frame_mode)) {
		atomic_inc(&mtk_crtc->dbi_set_freq_floor_cnt0);
		atomic_set(&mtk_crtc->dbi_bl_chg_reg_num, user_dbi.rg_num);
		mtk_disp_set_freq_floor_by_cond(mtk_crtc, true, GCE_DBI, &user_dbi);
	}
	...
	unsigned int panel_ext_param, unsigned int cfg_flag, unsigned int lock)
{
	...
	mtk_ddp_comp_io_cmd(oddmr_comp, cmdq_handle, ODDMR_GET_DBI_BL_CHG_REG_NUM, &user_dbi.rg_num);
	if(user_dbi.rg_num && (!is_frame_mode)) {
		atomic_inc(&mtk_crtc->dbi_set_freq_floor_cnt1);
		atomic_set(&mtk_crtc->dbi_bl_chg_reg_num, user_dbi.rg_num);
		mtk_disp_set_freq_floor_by_cond(mtk_crtc, true, GCE_DBI, &user_dbi);
	}
...
void mtk_crtc_bif_comp_config(struct mtk_drm_crtc *mtk_crtc, struct mtk_ddp_config *cfg,
	struct cmdq_pkt *cmdq_handle)
{
	struct drm_crtc *crtc = &mtk_crtc->base;
	struct mtk_ddp_comp *comp;
	...
	for (i = 0; i < ddp_ctx->wb_comp_nr; i++) {
		...
		if (mtk_ddp_comp_get_type(comp->id) == MTK_DISP_WDMA) {
			...
			bif_info->wb_comp = comp;
			bif_info->wb_frame_done_event = get_comp_wait_event(mtk_crtc, comp);
		}

		mtk_ddp_comp_bif_write_config(comp, cfg, cmdq_handle);
	...
	for (i = 0; i < ddp_ctx->bif_read_comp_nr; i++) {
		...
		if (mtk_ddp_comp_get_type(comp->id) == MTK_DISP_MDP_RDMA)
			bif_info->read_comp = comp;

		mtk_ddp_comp_bif_read_config(comp, cfg, cmdq_handle);
		mtk_ddp_comp_start(comp, cmdq_handle);
	}
	DDPINFO("%s,\n", __func__);
}
...
{
	struct mtk_crtc_ddp_ctx *ddp_ctx = &mtk_crtc->ddp_ctx[mtk_crtc->ddp_mode];
	struct mtk_ddp_comp *comp, *tmp_comp;
	...
	for (i = 0; i < ddp_ctx->wb_comp_nr; i++) {
		...
		if (i == 0) {
			...
			tmp_comp = ddp_ctx->wb_comp[i-1];
			prev_id = tmp_comp->id;
		}
		if (i < (ddp_ctx->wb_comp_nr-1)) {
			tmp_comp = ddp_ctx->wb_comp[i+1];
	...
	for (i = 0; i < ddp_ctx->bif_read_comp_nr; i++) {
		...
		if (i == 0) {
			...
			tmp_comp = ddp_ctx->bif_read_comp[i-1];
			prev_id = tmp_comp->id;
		}

		if (i < (ddp_ctx->bif_read_comp_nr-1)) {
			tmp_comp = ddp_ctx->bif_read_comp[i+1];
			next_id = tmp_comp ? tmp_comp->id : DDP_COMPONENT_ID_MAX;
		} else {
			next_id = DDP_COMPONENT_ID_MAX;
...
void mtk_crtc_bif_info_init(struct mtk_drm_crtc *mtk_crtc)
{
	if (!mtk_crtc->bif_info) {
		...
		DDPINFO("%s: need allocate memory\n", __func__);
	}
...
void mtk_crtc_update_bif_roi(struct mtk_drm_crtc *mtk_crtc)
{
	...
	unsigned int align = priv->data->bif_wdma_limit;
	unsigned int w, h;

	if (!mtk_crtc->bif_info) {
		DDPPR_ERR("%s: bif_info_null fail\n", __func__);
		return;
	}
	bif_info = mtk_crtc->bif_info;
	w = crtc->state->adjusted_mode.hdisplay;
	h = crtc->state->adjusted_mode.vdisplay;

	if (align)
		bif_info->wdma_offset = (w%align== 0)?0:(align - (w%align));

	if ((w != mtk_crtc->scaling_ctx.lcm_width) || (h != mtk_crtc->scaling_ctx.lcm_height)
		|| (bif_info->wdma_offset == 0)
		|| (bif_mode < 0)) {
	...
	bif_info->src_roi.width = w/3 + bif_info->wdma_offset;
	bif_info->src_roi.height = h;

	if (unlikely(bif_mode)) {
		bif_info->bif_enable = bif_mode;
		DDPMSG("%s,force mode:%d\n", __func__, bif_info->bif_enable);
		return;
	}
	if (priv->enable_bif)
		bif_info->bif_enable = priv->enable_bif;
}
...
void mtk_crtc_mml_addon_config(struct drm_crtc *crtc, enum addon_type config_type,
	struct cmdq_pkt *pkt)
{
	const struct mtk_addon_module_data *addon_module[2] = {NULL, NULL};
	struct mtk_drm_crtc *mtk_crtc = to_mtk_crtc(crtc);
	struct mtk_crtc_state *state = to_mtk_crtc_state(crtc->state);
	struct mtk_lye_ddp_state *lye_state = &state->lye_state;
	union mtk_addon_config addon_config;
	u32 ddp_mode = mtk_crtc->ddp_mode;
	...
	if (config_type == ADDON_CONNECT) {
		mml_addon_module_connect(crtc, ddp_mode, MML_DL,
			addon_module[0], addon_module[1], &addon_config, pkt);
		CRTC_MMP_MARK(0, mml_dbg, lye_state->mml_dl_lye,
			MMP_MML_ADDON_CONFIG | MMP_ADDON_CONNECT);
	} else if (config_type == ADDON_DISCONNECT) {
		mml_addon_module_disconnect(crtc, ddp_mode, MML_DL,
			addon_module[0], addon_module[1], &addon_config, pkt);
		CRTC_MMP_MARK(0, mml_dbg, lye_state->mml_dl_lye,
			MMP_MML_ADDON_CONFIG | MMP_ADDON_DISCONNECT);
	}
}
	...
	struct cmdq_pkt *cmdq_handle)
{
	...
	for (i = 0; i < addon_data->module_num; i++) {
		...
		if ((addon_module->type == ADDON_AFTER &&
			...
			(addon_module->type == ADDON_AFTER &&
			addon_module->module == DISP_WDMA_MID_v2) ||
			(addon_module->type == ADDON_AFTER &&
			...
			if (mtk_crtc->is_dual_pipe) {
				...
				if (bif_enabled(crtc)  == BIF_ALL_SCN && (scn == WDMA_WRITE_BACK)) {
					if (priv->data->bif_path_addon_for_all_scn)
						priv->data->bif_path_addon_for_all_scn(mtk_crtc, cmdq_handle, false);
				}
				mtk_addon_disconnect_after(crtc, ddp_mode, addon_module,
								  &addon_config,
								  cmdq_handle);
...
static void mtk_crtc_update_ovl_hrt_usage(struct drm_crtc *crtc)
{
	...
	int written = 0;
	int layer_num = 0;
	struct gce_user_layer layer_data;
	...
	if (!mtk_crtc_is_frame_trigger_mode(crtc)) {
		layer_data.layer_num = layer_num;
		mtk_disp_set_freq_floor_by_cond(mtk_crtc, true, GCE_LAYER, &layer_data);
	}
...
//			drm_framebuffer_get(fb);
			...
			if (mtk_crtc->is_dual_pipe) {
				...
				if (bif_enabled(crtc)  == BIF_ALL_SCN && (scn == WDMA_WRITE_BACK)) {
					if (priv->data->bif_path_addon_for_all_scn)
						priv->data->bif_path_addon_for_all_scn(mtk_crtc, cmdq_handle, true);
				}
				mtk_addon_connect_after(crtc, ddp_mode, addon_module,
							  &addon_config, cmdq_handle);
			}
				      ...
				      bool skip_cwb)
{
	...
	if (bif_enabled(crtc)  == BIF_ALL_SCN)
		mtk_crtc_bif_enable_racing(mtk_crtc, cmdq_handle);
	...
	unsigned int overlap_num, struct mtk_drm_lyeblob_ids *lyeblob_ids)
{
	...
	if (crtc_idx == 0 && lyeblob_ids) {
		mtk_crtc->balance_compensate = lyeblob_ids->balance_compensate[crtc_idx];
	}
	return bw;
}
	...
	struct cmdq_pkt *cmdq_handle)
{
	...
	if ((mode_chg_index & MODE_DSI_RES)
		...
		for_each_comp_in_cur_crtc_path(comp, mtk_crtc, i, j)
			mtk_ddp_comp_io_cmd(comp, cmdq_handle, STASH_CONFIG, NULL);
	}
...
#if IS_ENABLED(CONFIG_MTK_LCM_DUAL_PORT_SUPPORT) || IS_ENABLED(CONFIG_MTK_DISPLAY_DUAL_PIPE_DUAL_PORT_SUPPORT)
		for (layers_i = 0; layers_i < MAX_LAYER_NR; layers_i++)
			mtk_crtc->usage_ovl_fmt[layers_i] = 4;
		pan_disp_frame_weight = pan_disp_frame_weight * 2;
#endif
...
static bool mtk_crtc_trig_loop_wait_sof_vdo(struct drm_crtc *crtc)
{
	struct mtk_drm_crtc *mtk_crtc = to_mtk_crtc(crtc);
	struct mtk_ddp_comp *comp = NULL;
	...
	if (!comp)
		return false;
	return mtk_oddmr_dbi_need_ddren_wa(comp);
}

static void mtk_crtc_config_event_loop(struct drm_crtc *crtc)
{
	struct mtk_drm_crtc *mtk_crtc = to_mtk_crtc(crtc);
	struct mtk_drm_private *priv = crtc->dev->dev_private;
	struct cmdq_pkt *cmdq_handle;
	...
	if ((priv->data->config_urgent ||
		mtk_crtc_trig_loop_wait_sof_vdo(crtc)) &&
		(mtk_crtc->gce_obj.client[CLIENT_EVENT_LOOP])) {
		cmdq_handle = cmdq_pkt_create(
			mtk_crtc->gce_obj.client[CLIENT_EVENT_LOOP]);
		cmdq_pkt_write(cmdq_handle, mtk_crtc->gce_obj.base,
		mtk_get_gce_backup_slot_pa(mtk_crtc, DISP_SLOT_FIRST_EVENT_LOOP),
		0x1, ~0);
		cmdq_pkt_flush(cmdq_handle);
		cmdq_pkt_destroy(cmdq_handle);
	}
...
bool mtk_crtc_with_event_loop(struct drm_crtc *crtc)
{
	...
	if (mtk_crtc_is_frame_trigger_mode(crtc)) {
		...
		/* For mt6991 dbi sof behavior and config urgent */
		if ((priv->data->config_urgent ||
			mtk_crtc_trig_loop_wait_sof_vdo(crtc)) &&
			(mtk_crtc->gce_obj.client[CLIENT_EVENT_LOOP]))
			return true;
			...
			struct mtk_ddp_comp *comp)
{
	...
	} else if (comp->id == DDP_COMPONENT_OVL_EXDMA0) {
		return mtk_crtc->gce_obj.event[EVENT_OVLSYS0_EXDMA0_EOF];
	}
...
#ifdef CONFIG_MI_DISP_DFS_EVENT
static void cmdq_timeout_report_hander(struct timer_list *t)
{
	struct mi_event_info mi_event = {0};
	mi_event.event_type = MI_EVENT_CMDQ_TIMEOUT;
	mi_disp_mievent_int(MI_DISP_PRIMARY, &mi_event);
	DDPPR_ERR("%s: report cmdq_timeout event\n", __func__);
}
#endif
	...
	if (!drm_crtc_index(crtc)) {
		...
		if (mi_esd_ctx && !atomic_read(&mi_esd_ctx->err_flag_event)) {
			...
			if (!once_value) {
				...
				cmdq_timer.expires = jiffies + CMDQ_TIMER_PERIOD*HZ;
...
#ifdef CONFIG_MI_DISP_DFS_EVENT
				timer_setup(&cmdq_timeout_report_timer,cmdq_timeout_report_hander,0);
				cmdq_timeout_report_timer.expires = jiffies + 10*HZ;
				DDPPR_ERR("%s report cmdq_timeout after 10s\n", __func__);
				add_timer(&cmdq_timeout_report_timer);
#endif
			}

			if (tigger_esd) {
				atomic_set(&mi_esd_ctx->err_flag_event, ESD_TRIGGERED_BY_CMDQ_TIMEOUT);
...
#ifdef CONFIG_MI_DISP_CMDQ_SWT
	if (!__ratelimit(&timeout_rate) && !trigger_dump)
		return;
#else
		...
		struct drm_atomic_state *atomic_state)
{
	...
	drm_for_each_plane_mask(plane, crtc->dev, plane_mask) {
		...
		if ((ext_lye_id) &&
			...
			if (priv->data->mmsys_id == MMSYS_MT6991 && mtk_crtc_is_frame_trigger_mode(crtc)) {
...
static ktime_t mtk_check_preset_fence_timestamp(struct drm_crtc *crtc)
{
	...
	if (priv && priv->data->mmsys_id == MMSYS_MT6781) {
		int vrefresh = 0;
		...
		if (vrefresh > 0 &&
			ktime_to_us(new_time - mtk_crtc->pf_time) >=
			(500000 / vrefresh)) {
			DDPINFO("%s:pf_time too early,reget it,pf_time = %lld, new_time = %lld, fps = %d\n",
				__func__, mtk_crtc->pf_time, new_time, vrefresh);
			cur_time = new_time;
...
#endif
{
	...
	if(atomic_read(&mtk_crtc->dbi_set_freq_floor_cnt0) > 0) {
		atomic_dec(&mtk_crtc->dbi_set_freq_floor_cnt0);
		user_dbi.rg_num = atomic_read(&mtk_crtc->dbi_bl_chg_reg_num);
		mtk_disp_set_freq_floor_by_cond(mtk_crtc,	false, GCE_DBI, &user_dbi);
	}
	...
	if (!mtk_crtc_is_frame_trigger_mode(crtc)) {
		layer_data.layer_num = 0;
		mtk_disp_set_freq_floor_by_cond(mtk_crtc, false, GCE_LAYER, &layer_data);
	}
...
VDO_MODE:
	/* For mt6991 sof behavior and urgent */
	if (crtc_id) {
		DDPDBG("%s:%d invalid crtc:%ld\n", __func__, __LINE__, crtc_id);
		return;
	}
	...
	if(mtk_crtc_trig_loop_wait_sof_vdo(crtc)) {
		rop.reg = false;
		rop.value = 1;
		cmdq_pkt_read(cmdq_handle, mtk_crtc->gce_obj.base,
			mtk_get_gce_backup_slot_pa(mtk_crtc,
			DISP_SLOT_FIRST_EVENT_LOOP), var1);
		GCE_IF(lop, R_CMDQ_EQUAL, rop);
		{
			GCE_DO(clear_event, EVENT_DISP0_MUTEX0_SOF);
			cmdq_pkt_write(cmdq_handle, mtk_crtc->gce_obj.base,
				mtk_get_gce_backup_slot_pa(mtk_crtc, DISP_SLOT_FIRST_EVENT_LOOP),
				0, 0x1);
		...
		GCE_FI;
		GCE_DO(wfe, EVENT_DISP0_MUTEX0_SOF);
		mtk_crtc_comp_trigger(mtk_crtc, cmdq_handle, MTK_TRIG_FLAG_TRIGGER_VDO);
	}
	if (priv->data->config_urgent) {
		GCE_DO(clear_event, EVENT_DSI0_TARGET_LINE);
		GCE_DO(wfe, EVENT_DSI0_TARGET_LINE);
		cmdq_pkt_write(cmdq_handle, NULL,
			mtk_crtc->side_config_regs_pa + MMSYS_EMI_REQ_CTL, 0x801, 0x801);
	}
...
//			}
		...
		if (crtc_id == 0) {
			...
			if (mtk_drm_helper_get_opt(priv->helper_opt,
				...
				if (priv->data->config_urgent) {
					cmdq_pkt_write(cmdq_handle, NULL,
						mtk_crtc->side_config_regs_pa + MMSYS_EMI_REQ_CTL, 0, 0x801);
				}
				mtk_crtc_comp_trigger(mtk_crtc, cmdq_handle, MTK_TRIG_FLAG_EOF_VDO);
			}
...
void mtk_crtc_connect_default_path(struct mtk_drm_crtc *mtk_crtc)
{
	...
	for_each_comp_in_crtc_path_bound(comp, mtk_crtc, i, j, 1) {
		...
		if (bif_enabled(crtc) == BIF_ALL_SCN) {
			if ((comp->id == DDP_COMPONENT_POSTALIGN0) &&
				mtk_ddp_comp_get_type(next_id) == MTK_DISP_VIRTUAL)
				bif_skip_add = true;
			else if ((comp->id == DDP_COMPONENT_SPLITTER0_IN_CB1) &&
				mtk_ddp_comp_get_type(next_id) == MTK_DISP_VIRTUAL)
				bif_skip_add = true;
			if (comp->id == DDP_COMPONENT_DSI0)
				bif_skip_add = false;
	...
	if (bif_enabled(crtc)) {
		...
		/* default enable bif mutex */
		if (bif_enabled(crtc) == BIF_ALL_SCN) {
			if (priv->data->bif_path_insert_for_all_scn)
				priv->data->bif_path_insert_for_all_scn(mtk_crtc, NULL);
			mtk_crtc_bif_enable_mutex(mtk_crtc, NULL);
		}
	}
...
void mtk_crtc_config_default_path(struct mtk_drm_crtc *mtk_crtc)
{
	...
	if (mtk_crtc->bif_info && mtk_crtc->bif_info->bif_enable)
		mtk_crtc_bif_comp_config(mtk_crtc, &cfg, cmdq_handle);
		...
		for_each_comp_in_cur_crtc_path(comp, mtk_crtc, i, j) {
			if (comp && (mtk_ddp_comp_get_type(comp->id) == MTK_DISP_AAL ||
				...
				if (comp->funcs && comp->funcs->bypass)
					mtk_ddp_comp_bypass(comp, 0, PQ_FEATURE_KRN_DOZE, cmdq_handle);
			}
		}
...
void mtk_crtc_trig_loop_pre_config(struct mtk_drm_crtc *mtk_crtc)
{
	struct drm_crtc *crtc = &mtk_crtc->base;
	struct cmdq_pkt *cmdq_handle;
	struct mtk_ddp_comp *comp;
	...
	if (mtk_crtc_is_frame_trigger_mode(crtc) || drm_crtc_index(crtc) != 0)
		return;
	...
	mtk_ddp_comp_io_cmd(output_comp, cmdq_handle, DSI_SET_TARGET_LINE, &cfg);
...
void mtk_drm_crtc_enable(struct drm_crtc *crtc, bool need_report_bw)
{
	...
	mtk_crtc_trig_loop_pre_config(mtk_crtc);
	if (mtk_crtc_with_trigger_loop(crtc)) {
		if (mtk_crtc_with_sodi_loop(crtc) &&
			(!mtk_crtc_is_frame_trigger_mode(crtc)))
		...
		if (mtk_crtc_with_event_loop(crtc)) {
			mtk_crtc_config_event_loop(crtc);
			mtk_crtc_start_event_loop(crtc);
		}
		mtk_crtc_start_trig_loop(crtc);
	}
...
void mml_cmdq_pkt_init(struct drm_crtc *crtc, struct cmdq_pkt *cmdq_handle)
{
	...
	case MML_DIRECT_LINKING:
		...
		mml_drm_racing_config_sync(mml_ctx, cmdq_handle, false);
		break;
						...
						bool need_sync_mml)
{
	...
	if (priv->data->config_urgent && !mtk_crtc_is_frame_trigger_mode(crtc)) {
		if (crtc_id == 0) {
			...
			if (fps == 120 || fps == 90 || mtk_crtc->mode_chg)
				cmdq_pkt_write(cmdq_handle, mtk_crtc->gce_obj.base,
					mtk_crtc->side_config_regs_pa + MMSYS_EMI_REQ_CTL, 0x801, 0x801);
		}
	}
				      ...
				      struct drm_atomic_state *atomic_state)
{
...
#if IS_ENABLED(CONFIG_MTK_LCM_DUAL_PORT_SUPPORT) || IS_ENABLED(CONFIG_MTK_DISPLAY_DUAL_PIPE_DUAL_PORT_SUPPORT)
	if (priv->data->ovl_exdma_rule && g_hrt_valid_layer &&
	...
	if (crtc_id == 0 && mtk_drm_helper_get_opt(priv->helper_opt, MTK_DRM_OPT_VIDLE_FULL_SCENARIO)) {
		...
		if (mtk_crtc_state->prop_val[CRTC_PROP_DOZE_ACTIVE])
			mtk_vidle_hint_update(VIDLE_HINT_AOD_ON);
		else
			mtk_vidle_hint_update(VIDLE_HINT_AOD_OFF);
#endif
			       ...
			       struct mtk_plane_state *plane_state)
{
	...
	if (comp)
		DDPINFO("%s plane_id:%d, comp_id:%d, comp_id:%d, lye_id %d, layer_caps 0x%x\n", __func__,
			plane->index, comp->id, plane_state->comp_state.comp_id,
			plane_state->comp_state.lye_id,plane_state->comp_state.layer_caps);
	...
	if (plane_state->pending.enable) {
		...
		if (priv->data->ovl_exdma_rule) {
			...
			DDPINFO("%s comp_id %d, mout 0x%x value=%d\n", __func__, comp_state->comp_id,
						exdma_addr, value);
			if (config_regs_pa > 0 && value >= 0)
				cmdq_pkt_write(cmdq_handle, mtk_crtc->gce_obj.base,
...
static void mtk_drm_bif_wb_cb(struct cmdq_cb_data data)
{
#ifndef DRM_CMDQ_DISABLE
	struct mtk_cmdq_cb_data *cb_data = data.data;
	struct drm_crtc *crtc = cb_data->crtc;
	...
	if (priv && priv->power_state) {
		...
		if (mtk_crtc->bif_info) {
			wb_comp = mtk_crtc->bif_info->wb_comp;
	...
	void *cb_data, struct cmdq_pkt *cmdq_handle)
{
	...
	if (bif_enabled(crtc) == BIF_ALL_SCN) {
		struct mtk_cmdq_cb_data *bif_wb_cb_data = kmalloc(sizeof(*bif_wb_cb_data), GFP_KERNEL);

		if (bif_wb_cb_data)
			bif_wb_cb_data->crtc = crtc;
		else {
			DDPPR_ERR("bif_wb_cb_data alloc fail\n");
			return -EINVAL;
		}

		event = mtk_crtc->bif_info->wb_frame_done_event;
		mtk_crtc_pkt_create(&handle, crtc, client);

		if (!handle) {
			DDPPR_ERR("%s:%d NULL handle\n", __func__, __LINE__);
			kfree(bif_wb_cb_data);
			return -EINVAL;
		}
		...
		mtk_crtc_bif_keep_read_path(crtc, handle, false);

		bif_wb_cb_data->cmdq_handle = handle;

		if (cmdq_pkt_flush_threaded(handle, mtk_drm_bif_wb_cb, bif_wb_cb_data) < 0) {
			DDPPR_ERR("failed to flush gce_cb threaded\n");
			kfree(bif_wb_cb_data);
		}
	}
				      ...
				      struct drm_atomic_state *atomic_state)
{
	...
	if (is_mi_dev_support_nova()) {
		mtk_drm_crtc_d2_mv_parser(crtc);
	}
#endif
#endif
		...
		if (IS_ERR_OR_NULL(comp)) {
			...
			if (IS_ERR_OR_NULL(comp)) {
				...
				if (IS_ERR_OR_NULL(comp)) {
					...
					if (IS_ERR_OR_NULL(comp))
						comp = mtk_ddp_comp_find_by_id(crtc, DDP_COMPONENT_OVL0_BLENDER1);
				}
			}
		}
		if (IS_ERR_OR_NULL(comp))
				    ...
				    int buf_len, int event_id)
{
	...
	case EVENT_OVLSYS_DISP_OVL0_SOF:
		len = snprintf(buf, buf_len, "disp_ovlsys_ovl_exdma3_sof%d",
					drm_crtc_index(&mtk_crtc->base));
		break;
	case EVENT_OVLSYS0_EXDMA0_EOF:
		len = snprintf(buf, buf_len, "disp_ovlsys0_exdma0_eof%d",
			drm_crtc_index(&mtk_crtc->base));
		break;
	case EVENT_MML_STREAM_CONFIG:
		len = snprintf(buf, buf_len, "mml_stream_config");
		break;
	case EVENT_DISP0_MUTEX0_SOF:
		len = snprintf(buf, buf_len, "dispsys0_mutex0_sof");
		break;
...
#define CAPS_CHAR_SIZE 25
	...
	atomic_set(&mtk_crtc->dbi_set_freq_floor_cnt0, 0);
	atomic_set(&mtk_crtc->dbi_set_freq_floor_cnt1, 0);
	atomic_set(&mtk_crtc->dbi_bl_chg_reg_num, 0);
	...
	for (i = 0; i < DDP_MODE_NR; i++) {
		...
		mtk_crtc->ddp_ctx[i].bif_read_comp_nr = path_data->bif_read_path_len[i];
		mtk_crtc->ddp_ctx[i].bif_read_comp = devm_kmalloc_array(
			dev, path_data->bif_read_path_len[i],
			sizeof(struct mtk_ddp_comp *), GFP_KERNEL | __GFP_ZERO);
	}
	...
	for_each_bif_read_comp_id_in_path_data(comp_id, path_data, i, p_mode) {
		struct mtk_ddp_comp *comp;
		struct device_node *node;

		if (comp_id < 0) {
			DDPPR_ERR("%s: error comp_id:%d\n", __func__, comp_id);
			return 0;
		}
		...
		if (mtk_ddp_comp_get_type(comp_id) == MTK_DISP_VIRTUAL) {
			...
			if (!comp)
				return -ENOMEM;
			comp->id = comp_id;
			mtk_crtc->ddp_ctx[p_mode].bif_read_comp[i] = comp;
			continue;
		}
		if (comp_id < 0) {
			DDPPR_ERR("%s: error comp_id:%d\n", __func__, comp_id);
		...
		if (!comp) {
			dev_err(dev, "error, Component %s not initialized\n",
				node->full_name);
			return -ENODEV;
		}
		mtk_crtc->ddp_ctx[p_mode].bif_read_comp[i] = comp;
...
void mtk_crtc_mml_racing_resubmit(struct drm_crtc *crtc, struct cmdq_pkt *_cmdq_handle)
{
	...
	mml_drm_racing_config_sync(mml_ctx, cmdq_handle, true);
...
void mtk_crtc_reset_ovlsys_dli(struct drm_crtc *crtc,  struct cmdq_pkt *cmdq_handle)
{
	...
	if (priv->data->mmsys_id == MMSYS_MT6991) {
		if (mtk_crtc->ovlsys0_regs_pa) {
			...
			cmdq_pkt_write(cmdq_handle, mtk_crtc->gce_obj.base,
				   mtk_crtc->ovlsys0_regs_pa + MT6991_OVLSYS_MISC,
				   OVLSYS_RESET_FLOW, OVLSYS_RESET_FLOW);
			cmdq_pkt_write(cmdq_handle, mtk_crtc->gce_obj.base,
				   mtk_crtc->ovlsys0_regs_pa + MT6991_OVLSYS_MISC, 0, OVLSYS_RESET_FLOW);
		}
	}
}
