			     ...
			     unsigned int ext_idx, struct mtk_plane_state *state, struct cmdq_pkt *handle)
{
	...
	unsigned int layer_src = 0;
	unsigned int value = DISP_OVL_L_EN;

	priv = (struct mtk_drm_private *)comp->mtk_crtc->base.dev->dev_private;

	if (ext_idx == LYE_NORMAL) {
		if (!pending->addr && pending->pq_loop_type == 0)
			layer_src |= LSRC_COLOR;
		else if ((pending->pq_loop_type == 2) && (priv->data->mmsys_id != MMSYS_MT6991 ||
			(state->comp_state.comp_id != DDP_COMPONENT_OVL_EXDMA2 &&
			state->comp_state.comp_id != DDP_COMPONENT_OVL1_EXDMA2)))
			layer_src |= LSRC_PQ;
		value |= (layer_src == LSRC_PQ) ? 0 : layer_src;
	}
	cmdq_pkt_write(handle, comp->cmdq_base,
		   comp->regs_pa + DISP_REG_OVL_BLD_L_EN(idx), value, DISP_OVL_L_EN | LSRC_PQ);
}
			  ...
			  enum mtk_ddp_io_cmd io_cmd, void *params)
{
	...
	case BACKUP_OVL_STATUS: {
		...
		cmdq_pkt_mem_move(handle, comp->cmdq_base,
			comp->regs_pa + DISP_REG_OVL_BLD_STA,
