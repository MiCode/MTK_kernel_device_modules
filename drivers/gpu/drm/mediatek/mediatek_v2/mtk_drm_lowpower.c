...
static void mtk_drm_vdo_mode_enter_idle(struct drm_crtc *crtc)
{
	...
	if (bif_enabled(crtc)== BIF_HS_IDLE) {
		...
		if (mtk_crtc->bif_info->sram_en) {
			...
			cmdq_pkt_write_value_addr(handle,
				mtk_get_gce_backup_slot_pa(mtk_crtc, DISP_SLOT_BIF_EN), 1, ~0);
		} else {
...
static void vdo_leave_idle_cb(struct cmdq_cb_data data)
{
	struct mtk_cmdq_cb_data *cb_data = data.data;
...
static void mtk_drm_vdo_mode_leave_idle(struct drm_crtc *crtc)
{
	...
	if (stage == BIF_READ_MODE) {
		...
		cmdq_pkt_write_value_addr(handle,
			mtk_get_gce_backup_slot_pa(mtk_crtc, DISP_SLOT_BIF_EN), 0, ~0);

		if (mtk_crtc->bif_info->bif_pkt_info == NULL) {
			cb_data = kmalloc(sizeof(*cb_data), GFP_KERNEL);
			if (cb_data) {
				cb_data->cmdq_handle = handle;
				cb_data->crtc = crtc;
