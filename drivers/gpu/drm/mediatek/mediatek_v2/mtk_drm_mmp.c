...
void init_drm_mmp_event(void)
{
	...
	g_DRM_MMP_Events.freq_floor =
		mmprofile_register_event(g_DRM_MMP_Events.drm, "FREQ_FLOOR");
...
void init_crtc_mmp_event(void)
{
	...
	for (i = 0; i < MMP_CRTC_NUM; i++) {
		...
		g_CRTC_MMP_Events[i].layer_nr = mmprofile_register_event(
			crtc_mmp_root, "layer_nr");
		g_CRTC_MMP_Events[i].dbi_rg_cnt = mmprofile_register_event(
			crtc_mmp_root, "dbi_rg_cnt");
		...
		g_CRTC_MMP_Events[i].trig_loop_err =
			mmprofile_register_event(crtc_mmp_root, "trig_loop_err");
