...
bool esd_panel_dead = false;
EXPORT_SYMBOL(esd_panel_dead);
...
int (*lcd_nvt_ts_esd_suspend)(void)=NULL;
EXPORT_SYMBOL(lcd_nvt_ts_esd_suspend);
bool foc_ts_esd_resume_probe = false;
EXPORT_SYMBOL(foc_ts_esd_resume_probe);
bool foc_ts_need_do_esd_recovery = false;
EXPORT_SYMBOL(foc_ts_need_do_esd_recovery);
static struct mi_esd_ctx *p_mi_esd;

void pinctrl_select_state(struct pinctrl *p, struct pinctrl_state *state);
...
static int mi_disp_request_err_flag(struct drm_crtc *crtc)
{
	...
	mi_esd_ctx->err_flag_irq_flags = 0x2002;
...
static bool mi_disp_esd_chk_panel_status(void)
{
	...
	DDPPR_ERR("esd read 0A:0x%02x\n", *(char *)(cmd_msg->rx_buf[0]));
done:
...
static int mi_esd_err_flag_irq_check_worker_kthread(void *data)
{
	...
	while (1) {
		...
		if(nvt_ts_esd_resume_probe == true){
			if(lcd_nvt_ts_esd_suspend!=NULL){
				(*lcd_nvt_ts_esd_suspend)();
			}
		}
		esd_panel_dead = true;
		...
		pin_level = gpio_get_value(mi_esd_ctx->err_flag_irq_gpio);
		pr_info("[ESD] pin_level %d\n",pin_level);
		while(((mi_disp_esd_chk_panel_status() == false) || !pin_level) && count < 10)
		{
		...
		if (count < 10) {
			pr_info("[ESD] call esd_recover_success\n");
			count = 0;
			//vis_mi_esd_recover_usecase();
			if(foc_ts_esd_resume_probe)
				foc_ts_need_do_esd_recovery = false;
		}
		else
		{

			#if 0
			DDPPR_ERR("[ESD] panel is not ready,recover again\n");
			mutex_lock(&private->commit.lock);
			DDP_MUTEX_LOCK(&mtk_crtc->lock, __func__, __LINE__);
			/* 1.esd recovery */
			//panel_dead = 1;
...
static int disp_esd_irq_enable_worker_kthread(void *data)
{
	struct sched_param param = { .sched_priority = 87 };
	struct drm_crtc *crtc = (struct drm_crtc *)data;
	//struct mtk_drm_private *private = crtc->dev->dev_private;
	...
	pr_info("esd start enable irq\n");
	while (1) {
		ret = wait_event_interruptible(mi_esd_ctx->err_irq_enable_wq,
		atomic_read(&mi_esd_ctx->mi_esd_irq_enable_flag));
		atomic_set(&mi_esd_ctx->mi_esd_irq_enable_flag, 0);
		if (ret < 0) {
			DDPINFO("[ESD]enable irq thread waked up accidently\n");
			continue;
		}
		msleep(100);	//sleep 100ms to enable because mipi err.
...
static void mi_disp_esd_err_flag_irq_init(struct mi_esd_ctx *mi_esd_ctx, struct drm_crtc *crtc)
{
	...
	mi_esd_ctx->disp_esd_irq_enable_task = kthread_create(
	disp_esd_irq_enable_worker_kthread, crtc, "err_flag_irq_enable");
	init_waitqueue_head(&mi_esd_ctx->err_irq_enable_wq);
	atomic_set(&mi_esd_ctx->mi_esd_irq_enable_flag, 0);
	wake_up_process(mi_esd_ctx->disp_esd_irq_enable_task);
}

/* foc tp fw err notify lcd do esd recovery */
void foc_tp_notify_lcd_esd(void)
{
	struct mi_esd_ctx *mi_esd_ctx = p_mi_esd;

	if (unlikely(!mi_esd_ctx)) {
		DDPINFO("%s:invalid ESD context\n",
			__func__);
		return;
	}
	pr_info("tp notify esd\n");
	atomic_set(&mi_esd_ctx->err_flag_event, 1);
	wake_up_interruptible(&mi_esd_ctx->err_flag_wq);
	return;
}
EXPORT_SYMBOL(foc_tp_notify_lcd_esd);
...
void mi_disp_err_enable_esd_irq(struct drm_crtc *crtc)
{
	...
	atomic_set(&mi_esd_ctx->mi_esd_irq_enable_flag, 1);
	wake_up_interruptible(&mi_esd_ctx->err_irq_enable_wq);
}
