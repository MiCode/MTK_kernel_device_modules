...
int debug_set_floor_by_cond = 1;
module_param(debug_set_floor_by_cond, int, 0644);
...
void mtk_disp_decision_floor_MT6991(struct gce_user_all *cond,
	int *step, int *reason)
{
	if (cond->layer.layer_num > 4) {
		*reason = 1;
		*step = 1;
	} else if ((cond->layer.layer_num == 4) && (cond->dbi.rg_num > 0)) {
		*reason = 2;
		*step = 1;
	} else {
		*reason = 0xFFFFFFFF;
...
void mtk_disp_set_freq_floor_by_cond(struct mtk_drm_crtc *mtk_crtc,
	bool en, enum GCE_USER user, void *data)
{
	struct drm_crtc *crtc = NULL;
	struct mtk_crtc_state *state = NULL;
	struct mtk_drm_private *priv = NULL;
	struct drm_display_mode *mode = NULL;
	struct gce_user_all *cond;
	unsigned int crtc_idx, mode_idx, freq = g_freq_steps[0], slb_ch_bw;
	int i, step, reason, urate = 70;

	if (IS_ERR_OR_NULL(mtk_crtc) || user == GCE_NONE)
		return;
	...
	priv = crtc->dev->dev_private;
	cond = &priv->gce_user[crtc_idx];
	if (!priv->data->gce_set_floor_rule || !debug_set_floor_by_cond ||
		mtk_crtc_is_frame_trigger_mode(crtc) || (crtc_idx != 0))
		return;

	/* TODO: use blanking time to check need or not */
	...
	// step1: update_scenario_condition
	switch (user) {
	case GCE_LAYER:
	{
		struct gce_user_layer *layer = (struct gce_user_layer *)data;

		if (en) {
			cond->layer.layer_num = layer->layer_num;
			CRTC_MMP_MARK(crtc_idx, layer_nr, layer->layer_num, 0);
		} else
			cond->layer.layer_num = 0;
	}
		break;
	case GCE_MML_DL:
	{
		struct gce_user_mml_dl *dl = (struct gce_user_mml_dl *)data;

		if (en) {
			cond->mml_dl.en = true;
			cond->mml_dl.reuse = dl->reuse;
		} else {
			cond->mml_dl.en = false;
			cond->mml_dl.reuse = false;
		}
	}
		break;
	case GCE_DBI:
	{
		struct gce_user_dbi *dbi = (struct gce_user_dbi *)data;

		if (en)
			cond->dbi.rg_num += dbi->rg_num;
		else if (!en && (cond->dbi.rg_num >= dbi->rg_num))
			cond->dbi.rg_num -= dbi->rg_num;
		else {
			DDPPR_ERR("%s dbi total rg_num(%d) < deleate rg_cnt(%d)\n",
				__func__, cond->dbi.rg_num, dbi->rg_num);
			cond->dbi.rg_num = 0;
		}
		CRTC_MMP_MARK(crtc_idx, dbi_rg_cnt, cond->dbi.rg_num, 0);
	}
		break;
	default:
		break;
	...
	// step2: decision freq
	priv->data->gce_set_floor_rule(cond, &step, &reason);

	if (step == cond->floor_step)
		return;
	DDPINFO("chg decision freq %lu->%lu\n",
		g_freq_steps[cond->floor_step] / 1000000,
		g_freq_steps[step] / 1000000);

	// step3: report channel bw effect mminfra/vdisp freq
	cond->floor_step = step;
	if (step == 0) {
		mtk_dump_mminfra_ck(priv, false);
		mtk_vidle_channel_bw_set(0, 18);
		mtk_vidle_dvfs_trigger(__func__);
		DRM_MMP_MARK(freq_floor, step, reason);
		return;
	}

	// Choose target freq between two step freq to prevent floating-point
	// error in bw calculation that could cause more rais 1 step freq
	freq = (g_freq_steps[step] + g_freq_steps[step - 1]) / 2;
	freq /= 1000000;

	// mt6991 mminfra urate is 0.6, vdisp urate is 0.7, so use avg urate 0.65
	if (priv->data->mmsys_id == MMSYS_MT6991)
		urate = 65;

	// freq to ch_bw formula: ch_bw = freq * 2 * 8 * urate(0.7)
	slb_ch_bw = freq * 2 * 8 * urate / 100;
	mtk_vidle_channel_bw_set(slb_ch_bw, 18);
	mtk_vidle_dvfs_trigger(__func__);
	DRM_MMP_MARK(freq_floor, step, reason);
}
