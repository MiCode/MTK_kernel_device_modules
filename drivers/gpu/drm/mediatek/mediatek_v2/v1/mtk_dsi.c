...
static void mtk_dsi_calc_vdo_timing(struct mtk_dsi *dsi)
{
	...
	if (dsi->ext->params->is_cphy) {
		...
		if (dsi->ext && !dsi->ext->params->vdo_keep_hs_perline) {
			...
			if (horizontal_frontporch_byte < hfp_minimum) {
				...
				"%s HFP:%d < CPHY HFP minimum limitation:%d !!\n",
					...
					(hfp_minimum / 2));
				if (dsi->driver_data->support_frame_tb_v5 &&
					!dsi->ext->params->skip_hfp_min_check)
		...
		if (dsi->driver_data->support_frame_tb_v5 &&
			dsi->ext && !dsi->ext->params->skip_hfp_min_check) {
...
static int mtk_preconfig_dsi_enable(struct mtk_dsi *dsi)
{
	...
	/*vdo ltpo*/
	mtk_dsi_set_LTPO_VM(dsi, NULL, NULL, 1);
#if !IS_ENABLED(CONFIG_DRM_PANEL_M12A_42_02_0B_DSC_CMD) \
	...
	int force_lcm_update)
{
	...
	if (dsi->panel) {
		...
		if (is_bdg_supported())
			check_stopstate(NULL);
		mtk_dsi_clk_hs_mode(dsi, 1);
		if (dsi->panel && dsi->ext->funcs->init) {
			dsi->ext->funcs->init(dsi->panel);
		}
#endif
		/*
...
static u32 mtk_dsi_recv_cnt(u8 type, u8 *read_data)
{
	...
	case MIPI_DSI_RX_ACKNOWLEDGE_AND_ERROR_REPORT:
		DDPPR_ERR("type is mipi error(0x02), try again\n");
		break;
	default:
		DDPPR_ERR("%s error, type is invalid(0x%x) cannot be non-recognite\n", __func__, type);
		break;
	}
...
static int mtk_dsi_read_data_by_gce(int crtc_id, struct mtk_dsi *dsi, struct mipi_dsi_msg *msg, u32 slot_idx)
{
	...
	if (dsi->driver_data->support_rd_cmdq &&
		...
		DDPPR_ERR("%s error, DSI CMDQ READ recv_cnt=%d over max value(%u)\n",
		...
		DDPPR_ERR("%s error, DSI RX DATA READ recv_cnt=%d over max value(%u)\n",
...
static int mtk_dsi_read_data_by_cpu(int crtc_id, struct mtk_dsi *dsi, struct mipi_dsi_msg *msg)
{
	...
	if (dsi && dsi->driver_data && dsi->driver_data->support_rd_cmdq) {
		...
		if (recv_cnt > dsi->driver_data->dsi_cmdq_rd_max_sz_cpu + DSI_LONG_PACKET_HDR_SIZE) {
			DDPPR_ERR("%s error, DSI RX recv_cnt=%d over max value(%u)\n", __func__, recv_cnt,
		...
		for (i = 0; i < ((recv_cnt + 3) / 4); i++) {
			...
			if ((0x1FF - (j + 3)) == 0) {
				cmdq_page--;
				if (cmdq_page < 0) {
					DDPPR_ERR("%s error, cmdq page is invalid\n", __func__);
					return -EINVAL;
				}
				mtk_dsi_mask(dsi, DSI_CMDQ_CON(dsi->driver_data), CMDQ_PAGE, cmdq_page << 16);
...
static void mtk_dsi_send_cmd_trigger(struct mtk_dsi *dsi, struct cmdq_pkt *handle, u32 cmdq_size)
{
	...
	if (!cmdq_size) {
		DDPPR_ERR("%s error, cmdq_size is 0\n", __func__);
		return;
	}
			...
			const struct mtk_dsi_cmd_msg *cmd_msg)
{
	...
	if ((!dsi->driver_data->support_rd_cmdq) &&
		...
		DDPPR_ERR("%s error, %d, rx_read_len is overflow:%d(%d)\n", __func__, __LINE__,
			cmd_msg->cmd_msg->rx_len, dsi->driver_data->dsi_rx_data_rd_max_sz);
		return -EINVAL;
	}
	...
	if (!(cmd_opt->flags & MTK_MIPI_DSI_CMD_BY_CPU)) {
		if (cmd_msg->read_scn >= READ_SCN_NUM) {
			DDPPR_ERR("%s error, %d, read_scn is invalid\n", __func__, __LINE__);
			return -EINVAL;
		}
		if (cmd_msg->rd_to_slot && cmd_msg->read_scn != ESD_CHECK_SCN) {
			DDPPR_ERR("%s error, rd_to_slot only support ESD_CHECK_SCN\n", __func__);
			return -EINVAL;
		}
		if ((cmd_msg->read_scn ==  READ_COMMON_SCN) &&
			!(cmd_opt->flags & MTK_MIPI_DSI_GCE_BLOCKING_FLUSH)) {
			DDPPR_ERR("%s error, READ_COMMON_SCN must be set blocking flush, flag=0x%x\n",
				__func__, cmd_opt->flags);
			return -EINVAL;
		}
		if (cmd_msg->cmd_msg->rx_len > dsi->driver_data->dsi_cmdq_rd_max_sz_gce) {
			DDPPR_ERR("%s error, cmdq_read size is overflow by gce is overflow:%d(%d)\n", __func__,
	...
	if (!(cmd_opt->flags & MTK_MIPI_DSI_CMD_BY_CPU) &&
		...
		DDPPR_ERR("%s error, cmdq_read size is overflow by gce,len=%d(%d),idx=%d(0x%x~0x%x),slot_num=%d(%d)\n",
	...
	if (cmd_opt->flags & MTK_MIPI_DSI_CMD_BY_CPU) {
		if (cmd_msg->cmd_msg->rx_len > dsi->driver_data->dsi_cmdq_rd_max_sz_cpu) {
			DDPPR_ERR("%s error, cmdq_read size is overflow by cpu, len=%d(%d)\n",
...
static int dsi_cmd_flags_check(void *dsi, u32 flags)
{
	if ((flags & MTK_MIPI_DSI_CMD_BY_CPU) &&
		...
		DDPPR_ERR("%s error, GCE and CPU control conflict error , flag=0x%x\n", __func__, flags);
		return -EINVAL;
	}

	if ((flags & MTK_MIPI_DSI_GCE_BLOCKING_FLUSH) && (flags & MTK_MIPI_DSI_GCE_NON_BLOCKING_FLUSH)) {
		DDPPR_ERR("%s error, GCE FLUSH setting error , flag=0x%x\n", __func__, flags);
		return -EINVAL;
	}

	if ((flags & MTK_MIPI_DSI_GCE_USE_CFG_THREAD) && (flags & MTK_MIPI_DSI_GCE_USE_DSI_THREAD)) {
		DDPPR_ERR("%s error, USE GCE THREAD setting error , flag=0x%x\n", __func__, flags);
		return -EINVAL;
	}

	if (flags & MTK_MIPI_DSI_CMD_EXTERNAL) {
		if (!(flags & MTK_MIPI_DSI_CRTC_ID)) {
			DDPPR_ERR("%s error, need to set CRTC_ID, flag=0x%x\n", __func__, flags);
			return -EINVAL;
		}
		if ((!(flags & MTK_MIPI_DSI_GCE_BLOCKING_FLUSH)) && (!(flags & MTK_MIPI_DSI_GCE_NON_BLOCKING_FLUSH))) {
			DDPPR_ERR("%s error, need to set FLUSH, flag=0x%x\n", __func__, flags);
			return -EINVAL;
		}
	}

	if (!(flags & MTK_MIPI_DSI_CRTC_ID) && !dsi) {
		DDPPR_ERR("%s error, dsi is null, need to set crtc id , flags:0x%x\n", __func__, flags);
		return -EINVAL;
	}
...
cmd_mode_transfer:
	...
	DDPDSI_CMD("%s done, rd=%d\n", __func__, cmd_msg->is_rd);
	return 0;
}
	...
	struct mtk_drm_crtc *mtk_crtc, unsigned int en)
{
	...
	if (to_info.is_support)
		DDPINFO("%s: crtc:%d overhead is_support:%d, width L:%d R:%d\n", __func__,
					crtc_idx, to_info.is_support, to_info.left_in_width,
...
#if IS_ENABLED(CONFIG_DRM_PANEL_M12A_36_02_0A_DSC_CMD) \
	|| IS_ENABLED(CONFIG_DRM_PANEL_M12A_42_02_0B_DSC_CMD)
	if (drm_mode_vrefresh(&old_state->mode) > drm_mode_vrefresh(&crtc->state->mode)) {
...
static void mtk_dsi_update_vfp(struct mtk_dsi *dsi,  struct mtk_drm_crtc *mtk_crtc,
	struct cmdq_pkt *handle, enum CHANGE_VFP_SOURCE source,
	unsigned int *vfp, unsigned int *mod_vfp)
{
	struct mtk_ddp_comp *comp = &dsi->ddp_comp;
	...
	case SOURCE_TIMING_CHANGE:
	{
		...
		*vfp = adjusted_mode.vsync_start - adjusted_mode.vdisplay;
		...
		if (mtk_drm_helper_get_opt(priv->helper_opt, MTK_DRM_OPT_MSYNC2_0) &&
			dsi && dsi->ext && dsi->ext->params && dsi->ext->params->msync2_enable) {
			if (state->prop_val[CRTC_PROP_MSYNC2_0_ENABLE] != 0) {
				DDPDBG("[Msync]%s,%d\n", __func__, __LINE__);
				...
				vfp_early_stop_value = REG_FLD_VAL(FLD_VFP_EARLY_STOP_EN, 1) |
					REG_FLD_VAL(VFP_EARLY_STOP_FLD_REG_MIN_NL, dsi->vm.vfront_porch);
				cmdq_pkt_write(handle, comp->cmdq_base,
					comp->regs_pa + DSI_VFP_EARLY_STOP(dsi->driver_data),
					vfp_early_stop_value, ~0);
	...
	case SOURCE_DEFAULT_MODE:
	{
		...
		*vfp = dsi->vm.vfront_porch;
		...
		if (priv && mtk_drm_helper_get_opt(priv->helper_opt, MTK_DRM_OPT_MSYNC2_0) &&
			panel_ext && panel_ext->params && panel_ext->params->msync2_enable) {
			if (state->prop_val[CRTC_PROP_MSYNC2_0_ENABLE] != 0) {
				if (dsi->mipi_hopping_sta &&
					panel_ext->params->dyn.max_vfp_for_msync_dyn)
					vfp = panel_ext->params->dyn.max_vfp_for_msync_dyn;
				else
					vfp = panel_ext->params->max_vfp_for_msync;
				DDPDBG("[Msync]%s,%d vfront_porch %d\n",
					__func__, __LINE__, vfp);
			}
		}
	}
	break;
	...
	struct cmdq_pkt *handle, unsigned int cur_mode, enum CHANGE_VFP_SOURCE source)
{
	...
	bool is_idle_mode = false, need_send_cmd = false;

	DDPINFO("%s+\n", __func__);

	if (!dsi || dsi->ext == NULL || dsi->ext->params == NULL) {
		DDPINFO("%s, %d, invalid parameter\n", __func__, __LINE__);
		return;
	}

	if (dsi->ext->params->change_fps_by_vfp_send_cmd) {
		need_send_cmd = (source == SOURCE_TIMING_CHANGE) ||
			((source == SOURCE_IDLE_MODE || source == SOURCE_DEFAULT_MODE) &&
				dsi->ext->params->low_power_fps.switch_en);
	}
	...
	mtk_dsi_update_vfp(dsi, mtk_crtc, handle, source, &vfp, &mod_vfp);
...
static const struct mtk_dsi_driver_data mt6881_dsi_driver_data = {
	.reg_cmdq0_ofs = 0xd00,
	.reg_cmdq1_ofs = 0xd04,
	.reg_vm_cmd_con_ofs = 0x200,
	.reg_vm_cmd_data0_ofs = 0x208,
...
static const struct of_device_id mtk_dsi_of_match[] = {
	...
	{.compatible = "mediatek,mt6881-dsi", .data = &mt6881_dsi_driver_data},
	{},
};
