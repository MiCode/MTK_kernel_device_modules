...
static void esd_check_done_cb(struct cmdq_cb_data data)
{
	...
	if (!mtk_crtc) {
		DDPPR_ERR("%s error, invalid mtk_crtc stop thread\n", __func__);
		return;
	}
	esd_ctx = mtk_crtc->esd_ctx;
	if (!esd_ctx) {
		DDPPR_ERR("%s error invalid ESD context, stop thread\n", __func__);
		return;
	}

	atomic_set(&esd_ctx->check_cmp, 1);
	wake_up_interruptible(&esd_ctx->cmp_wq);
...
static int mtk_drm_esd_result_check(struct mtk_drm_esd_ctx *esd_ctx)
{
	int index = 0;
	struct mtk_drm_crtc *mtk_crtc = NULL;
	struct drm_crtc *crtc = NULL;
...
check_done:
	...
	atomic_set(&esd_ctx->check_cmp, 0);
	return 0;
}
...
static int mtk_drm_esd_check_worker_kthread(void *data)
{
	...
	if (esd_ctx->crtc)
		...
		DDPPR_ERR("%s error, invalid crtc\n", __func__);
		return -EINVAL;
	}
	...
	while (1) {
		...
		if (esd_ctx->chk_sta == 1) {
			ret = wait_event_interruptible(
				esd_ctx->cmp_wq, atomic_read(&esd_ctx->check_cmp));
			mtk_drm_esd_result_check(esd_ctx);
		}
...
static void mtk_disp_esd_chk_init(struct drm_crtc *crtc)
{
	...
	init_waitqueue_head(&esd_ctx->cmp_wq);
