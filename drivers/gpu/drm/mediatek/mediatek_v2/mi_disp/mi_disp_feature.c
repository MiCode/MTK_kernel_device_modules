...
int mi_disp_feature_attach_display(void *display, int disp_id, int intf_type)
{
	...
	DISP_ERROR("mi_disp_feature_attach_display 11111\n");
	if (is_support_disp_id(disp_id) && is_support_disp_intf_type(intf_type)) {
		...
		if (IS_ERR(dd_ptr->dev)) {
			DISP_ERROR("mi_disp_feature_attach_display create device failed for disp-%s-%d\n",
		...
		if (ret < 0) {
			DISP_ERROR("mi_disp_feature_attach_display failed to create sysfs attrs\n");
			goto err;
		}
		sysfs_init = true;
#endif
		...
		if (ret) {
			DISP_ERROR("mi_disp_feature_attach_display failed to create disp_feature kthread\n");
			goto err;
		}
		...
		if (ret) {
			DISP_ERROR("mi_disp_feature_attach_display failed to create fod_thread kthread\n");
			goto err;
		}
		lhbm_inited = true;
#endif
		...
		if (ret) {
			DISP_ERROR("mi_disp_feature_attach_display failed to create procfs entry\n");
			goto err;
		}
		procfs_inited = true;
#endif
		...
		if (ret) {
			DISP_ERROR("mi_disp_feature_attach_display failed to create debugfs entry\n");
			goto err;
		}
		debugfs_inited = true ;
#endif
		...
		if (ret) {
			DISP_ERROR("mi_disp_feature_attach_display failed to init cooler brightness clone\n");
			goto err;
		}
#endif
		...
		if (ret) {
			DISP_ERROR("mi_disp_feature_attach_display failed to init input event handler\n");
			goto err;
		}
#endif
		...
		if (ret) {
			DISP_INFO("mi_disp_feature_attach_display failed to init boost, mi_disp_boost_enable() will retry.\n");
		}
#endif
		DISP_INFO("mi_disp_feature_attach_display attach %s display(%s intf) success\n", get_disp_id_name(disp_id),
				get_disp_intf_type_name(intf_type));

		atomic_set(&dd_ptr->pending_doze_cnt, 0);
	} else {
		DISP_ERROR("mi_disp_feature_attach_display Unknown display or interface, failed to bind\n");
	}

	DISP_ERROR("mi_disp_feature_attach_display SUCCESS to end\n");
	return 0;
