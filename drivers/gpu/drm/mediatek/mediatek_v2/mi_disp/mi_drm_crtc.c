...
{
	if (!dsi->mi_cfg.bl_enable && bl_lvl && !dsi->mi_cfg.is_android14_or_later_version) {
		DISP_INFO("%s true lvl= %d bl_enable= %d\n flag= %d", __func__, bl_lvl, dsi->mi_cfg.bl_enable, dsi->mi_cfg.is_android14_or_later_version);
		return true;
	}
	return false;
}
...
static int mi_drm_update_aod_status(struct mtk_dsi *dsi, bool doze_state, bool is_aod_exit)
{
	...
	if (is_aod_exit && !doze_state) {
		...
		DISP_INFO("aod_wait_completion complete\n");
	}
	DISP_INFO("%s exit\n", __func__);
...
int mi_drm_crtc_update_layer_state(struct drm_crtc *crtc)
{
	...
	cur_flags.miui_customizedprocess_flag = (mi_layer_flags & MI_LAYER_CUSTOMIZEDPROCESS) ? true : false;
	dsi->mi_cfg.is_android14_or_later_version |= cur_flags.miui_customizedprocess_flag;
...
#if IS_ENABLED(CONFIG_DRM_PANEL_O16U_42_02_0A_DSC_VDO) || IS_ENABLED(CONFIG_DRM_PANEL_O16U_36_02_0B_DSC_VDO) || IS_ENABLED(CONFIG_DRM_PANEL_O16U_44_0F_0C_DSC_VDO) || IS_ENABLED(CONFIG_DRM_PANEL_O16U_42_02_0D_DSC_VDO)
	if (!cur_flags.miui_customizedprocess_flag)
