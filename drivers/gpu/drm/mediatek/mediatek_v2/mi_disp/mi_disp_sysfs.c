...
#define LCM_CMDLINE_BUFFER_SIZE 64
char lcm_lockdown_info[LCM_CMDLINE_BUFFER_SIZE]="\0";
char lcm_white_point[LCM_CMDLINE_BUFFER_SIZE]="\0";
			...
			struct device_attribute *attr, char *buf)
{
	...
	// 		get_disp_intf_type_name(dd_ptr->intf_type));
	// }
	if(!strcasecmp(lcm_white_point,"\0")){
...
static ssize_t lockdown_info_show(struct device *device,
			struct device_attribute *attr, char *buf)
{
	if(!strcasecmp(lcm_lockdown_info,"\0")){
		struct device_node *pDeviceNode;
		const char* data = NULL;
		char *temp = NULL;
		char *temp2 = NULL;
