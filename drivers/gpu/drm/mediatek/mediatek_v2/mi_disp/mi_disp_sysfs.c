		...
		struct device_attribute *attr, char *buf)
{
	int backlight = 0;
	struct disp_display *dd_ptr = to_disp_display(device);
	mi_dsi_display_get_brightness(dd_ptr->display, &backlight);
	pr_info("backlight_show : backlight(%d)\n", backlight);
	return snprintf(buf, PAGE_SIZE, "%d\n", backlight);
}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	u32 backlight;
	struct disp_display *dd_ptr = to_disp_display(device);
	ret = kstrtoint(buf, 0, &backlight);
	if(ret)
		return ret;
	pr_info("backlight_store : backlight(%u)\n", backlight);
	if (dd_ptr->intf_type == MI_INTF_DSI) {
		ret = mi_dsi_display_set_backlight(dd_ptr->display, backlight);
	} else {
		DISP_ERROR("Unsupported display(%s intf)\n",
			get_disp_intf_type_name(dd_ptr->intf_type));
		ret = -EINVAL;
