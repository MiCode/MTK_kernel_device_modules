...
ssize_t mi_dsi_display_set_disp_count(void *display, char* buf)
{
	...
	return mi_dsi_panel_disp_count_set(ctx, buf);
#else
	return 0;
#endif
}
...
#ifdef CONFIG_MI_DISP_PANEL_COUNT
	return  mi_dsi_panel_disp_count_get(ctx, buf);
#else
	return 0;
#endif
}
			...
			struct disp_count_info *count_info)
{
	...
	ret = mi_dsi_panel_set_count_info(dsi_display, count_info);

	return ret;
}
/* P16 code for HQFEAT-94083 by p-zhangyundan at 2025/3/17 start */
int mi_dsi_display_set_backlight(void *display, int backlight)
{
	struct mtk_dsi *dsi_display = (struct mtk_dsi *)display;
	if (!dsi_display) {
		pr_err("Invalid display ptr\n");
	...
	return mi_dsi_panel_set_backlight(dsi_display, backlight);
}
