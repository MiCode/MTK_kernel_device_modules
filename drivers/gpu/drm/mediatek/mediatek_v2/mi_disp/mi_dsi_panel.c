...
ssize_t mi_dsi_panel_write_mipi_reg(char *buf)
{
	...
	DISP_INFO("samir:mi_dsi_panel_write_mipi_reg+++:input buffer:{%s}\n", buf);
	...
	DISP_INFO("samir:cmd_msg->flags= 0x%02x\n", cmd_msg->flags);
	DISP_INFO("samir:lcm_mipi_read_write.read_enable= %d\n", lcm_mipi_read_write.read_enable);
	...
	if (lcm_mipi_read_write.read_enable) {
		...
		if (lcm_mipi_read_write.read_count > 10) {
			...
			pr_err("samir:read lcm addr:%pad--dlen:%d\n",
				&(*(char *)(cmd_msg->tx_buf[0])), (int)cmd_msg->rx_len[0]);
			for (j = 0; j < cmd_msg->rx_len[0]; j++) {
				pr_err("samir:read lcm addr:%pad--byte:%d,val:%pad\n",
					&(*(char *)(cmd_msg->tx_buf[0])), j,
...
int mi_dsi_panel_set_disp_param(struct mtk_dsi *dsi, struct disp_feature_ctl *ctl)
{
	...
	case DISP_FEATURE_FOD_CALIBRATION_BRIGHTNESS:
		...
		if (panel_ext->funcs->backlight_for_calibration)
			panel_ext->funcs->backlight_for_calibration(dsi->panel, ctl->feature_val);
		    mi_cfg->in_fod_calibration = false;
		} else {
		if (ctl->feature_val >= 0 &&
			ctl->feature_val <= 4095) {
...
/* P16 code for HQFEAT-94083 by p-zhangyundan at 2025/3/17 start */
			...
			int backlight)
{
	...
	DISP_INFO("%s +, backlight = %d\n", __func__, backlight);
	mtk_crtc = dsi->ddp_comp.mtk_crtc;
	mtk_drm_setbacklight(&mtk_crtc->base, backlight, 0, (0X1<<SET_BACKLIGHT_LEVEL), 0);
	return ret;
}
/* P16 code for HQFEAT-94083 by p-zhangyundan at 2025/3/17 end */
