...
#define EXTMV_PKG_SIZE_V1	256	/* Package size for DSI_CMD_V1 */
#define EXTMV_PKG_SIZE_V2	1024	/* Package size for DSI_CMD_V2 */
#define EXTMV_PKG_HEADER_SIZE	4	/* Package header overhead in bytes */

/* Meta Size and Offsets*/
#define EXTMV_META_HEADER_LENGTH	14	/* MetaHeader data length in words */
#define EXTMV_META_END_LENGTH	4	/* MetaEnd data length in words */
#define EXTMV_OFF_MV_SIZE	13	/* MV size offset */
#define EXTMV_OFF_MV_DATA_START	14	/* MV data length offset */

/* ASCII Signatures (Little Endian) */
#define EXTMV_SIG_NVT_TYPE08	0x0854564E	/* "\0NVT" - Metadata header type */
#define EXTMV_SIG_NVT_TYPE09	0x0954564E	/* "\tNVT" - MV data header type */
#define EXTMV_SIG_META	0x4154454D	/* "META" */
#define EXTMV_IC_META_TIMESTAMP	0x10000801	/* ICMetaTimestamp identifier */

/* DSI Protocol Constants */
#define EXTMV_DSI_TYPE_DCS_WRITE	0x39	/* DCS long write */
#define EXTMV_DSI_CHANNEL	1	/* Virtual channel number */
#define EXTMV_DSI_FLAGS	0x0	/* Flags value */

/* Packet Length Constants */
#define EXTMV_METADATA_HEADER_LEN	28	/* Metadata header packet length (bytes) */
#define EXTMV_META_END_LEN	12	/* Meta end packet length (bytes) */

/* Meta Data Size Limits */
#define EXTMV_MAX_META_SIZE	40000	/* Maximum allowed meta size */

/* Bit Masks */
#define EXTMV_MASK_LOW_16BIT	0x0000FFFF	/* Extract lower 16 bits */
#define EXTMV_MASK_TEST_PATTERN	0x000F0000	/* Test pattern flag mask */

/* Metadata Field Offsets in meta_content[] */
#define EXTMV_OFF_HDR				0			/* Header field offset */
#define EXTMV_OFF_TS_HI				3			/* Timestamp high word offset */
#define EXTMV_OFF_TS_LO				4			/* Timestamp low word offset */
#define EXTMV_OFF_TS2_HI			7			/* Timestamp2 high word offset */
#define EXTMV_OFF_TS2_LO			8			/* Timestamp2 low word offset */
#define EXTMV_OFF_TS3_HI			9			/* Timestamp3 high word offset */
#define EXTMV_OFF_TS3_LO			10			/* Timestamp3 low word offset */
#define EXTMV_OFF_MV_HI				11			/* MV data high word offset */
#define EXTMV_OFF_MV_LO				12			/* MV data low word offset */

/* Meta End Data Offsets (relative to mv_size) */
#define EXTMV_OFF_END_HI(mv_size)	((mv_size) + 14)
#define EXTMV_OFF_END_LO(mv_size)	((mv_size) + 15)
#define EXTMV_OFF_END_HI2(mv_size)	((mv_size) + 16)
#define EXTMV_OFF_END_LO2(mv_size)	((mv_size) + 17)
#endif

#if defined(CONFIG_VIS_DISPLAY_V1)
...
ssize_t mi_dsi_panel_write_mipi_reg(char *buf)
{
	...
	struct mipi_dsi_msg msg[MAX_TX_CMD_NUM] = { 0 };
	struct mipi_dsi_msg rx_msg = { 0 };
	struct mtk_dsi_cmd_msg ddic_cmd = {
	...
	if (lcm_mipi_read_write.read_enable) {
		...
		if (lcm_mipi_read_write.read_count > 10) {
			...
			while (read_length > 0) {
				...
				rx_msg.rx_len = read_length > 10 ? 10 : read_length;
				rx_msg.rx_buf = temp_read_buffer;
				if ((mtk_dsi_cmd_version() == DSI_CMD_V2) && dsi_cmd_v2_dbg[DDIC_CMD_DBG]) {
					cmd_opt.flags = MTK_MIPI_DSI_CMD_EXTERNAL | MTK_MIPI_DSI_CMD_NEED_LOCK |
									MTK_MIPI_DSI_CMD_KICK_IDLE |MTK_MIPI_DSI_CRTC_ID |
				  					MTK_MIPI_DSI_GCE_BLOCKING_FLUSH | MTK_MIPI_DSI_GCE_USE_DSI_CMD_EVENT;
					rc = mtk_mipi_dsi_cmd(NULL, NULL, &cmd_opt, &ddic_cmd);
					if (rc != rx_msg.rx_len) {
						pr_err("%s error, %d, %d\n", __func__, rc, rx_msg.rx_len);
					} else {
						/* need to reset rc value since return value is rx_len when success,
						   otherwise mipi_rw will do retry */
						pr_info("%s, rc:%d\n", __func__, rc);
						rc = 0;
					}
				} else {
			...
			memset(cmd_msg->rx_buf[0], 0, lcm_mipi_read_write.read_count);
			cmd_msg->rx_len[0] = lcm_mipi_read_write.read_count;
			rx_msg.rx_len = lcm_mipi_read_write.read_count;
			rx_msg.rx_buf = lcm_mipi_read_write.read_buffer;
			if ((mtk_dsi_cmd_version() == DSI_CMD_V2) && dsi_cmd_v2_dbg[DDIC_CMD_DBG]) {
				cmd_opt.flags = MTK_MIPI_DSI_CMD_EXTERNAL | MTK_MIPI_DSI_CMD_NEED_LOCK |
								MTK_MIPI_DSI_CMD_KICK_IDLE |MTK_MIPI_DSI_CRTC_ID |
				  				MTK_MIPI_DSI_GCE_BLOCKING_FLUSH | MTK_MIPI_DSI_GCE_USE_DSI_CMD_EVENT;
		...
		for (i = 0; i < packet_count; i++) {
			...
			msg[i].tx_len = cmds[i].msg.tx_len;
			msg[i].tx_buf = cmds[i].msg.tx_buf;
	...
	unsigned int count, unsigned int format)
{
	...
	if ((mtk_dsi_cmd_version() == DSI_CMD_V2) && dsi_cmd_v2_dbg[DDIC_CMD_DBG]) {
		for (i = 0; i < count; i++) {
			msg[i].tx_len = cmd_msg.tx_len[i];
			msg[i].tx_buf = cmd_msg.tx_buf[i];
		}
		struct mtk_dsi_cmd_msg ddic_cmd = {
			.is_rd = 0, /* 0:write 1:read */
			.is_package = 0,
			.rd_to_slot = 0,
			.cmd_num = count,
			.transfer_mode = (lp_mode ? PACKET_LP_MODE : PACKET_HS_MODE),
			.cmd_msg = msg,
		};
		cmd_opt.flags = MTK_MIPI_DSI_CRTC_ID | MTK_MIPI_DSI_CMD_KICK_IDLE | MTK_MIPI_DSI_GCE_USE_DSI_CMD_EVENT |
				  (block ? MTK_MIPI_DSI_GCE_BLOCKING_FLUSH : MTK_MIPI_DSI_GCE_NON_BLOCKING_FLUSH);
		if (no_lock) {
			cmd_opt.flags |= (MTK_MIPI_DSI_GCE_CREATE_HANDLE | MTK_MIPI_DSI_GCE_USE_CFG_THREAD);
		} else {
			cmd_opt.flags |= (MTK_MIPI_DSI_CMD_EXTERNAL | MTK_MIPI_DSI_CMD_NEED_LOCK);
		}
		ret = mtk_mipi_dsi_cmd(NULL, NULL, &cmd_opt, &ddic_cmd);
	} else {
		if (wait_te_send)
...
int mi_dsi_panel_set_disp_param(struct mtk_dsi *dsi, struct disp_feature_ctl *ctl)
{
	...
	ctl_temp = kmalloc(sizeof(struct disp_feature_ctl), GFP_KERNEL);
	if (!ctl_temp) {
		pr_err("%s %d disp_feature_ctl clt_temp kmalloc failed!", __func__, __LINE__);
		return -EFAULT;
	}
	memcpy(ctl_temp, ctl, sizeof(struct disp_feature_ctl));
	...
	if (!dsi_panel_initialized(dsi) && ctl_temp->feature_id != DISP_FEATURE_LAYER_HAS_AOD) {
	...
	case DISP_FEATURE_DOZE_BRIGHTNESS:
		...
		if (is_support_doze_brightness(ctl_temp->feature_val)) {
			...
			if (private && mtk_drm_helper_get_opt(private->helper_opt, MTK_DRM_OPT_VIDLE_FULL_SCENARIO) && mtk_vidle_is_ff_enabled()) {
				DISP_INFO("disable ff before set doze brightness\n");
				mtk_vidle_config_ff(false);
			}
			if (ctl_temp->feature_val == DOZE_TO_NORMAL && mi_cfg->feature_val[DISP_FEATURE_DOZE_BRIGHTNESS] != DOZE_TO_NORMAL) {
				if ((mtk_dsi_cmd_version() == DSI_CMD_V2) && dsi_cmd_v2_dbg[AOD_DBG] && panel_ext->funcs->doze_disable_v2) {
					struct mtk_dsi_cmd_option cmd_opt = { 0 };

					cmd_opt.flags = MTK_MIPI_DSI_CMD_BY_CPU;
					panel_ext->funcs->doze_disable_v2(dsi->panel, dsi, mtk_mipi_dsi_cmd, NULL, &cmd_opt);
				} else if (panel_ext->funcs->doze_disable) {
				...
				if ((mtk_dsi_cmd_version() == DSI_CMD_V2) && dsi_cmd_v2_dbg[AOD_DBG] && panel_ext->funcs->doze_suspend_v2) {
					struct mtk_dsi_cmd_option cmd_opt = { 0 };

					cmd_opt.flags = MTK_MIPI_DSI_CMD_BY_CPU;
					panel_ext->funcs->doze_suspend_v2(dsi->panel, dsi, mtk_mipi_dsi_cmd, NULL, &cmd_opt);
				} else if (panel_ext->funcs->doze_suspend)
	...
	case DISP_FEATURE_VIDLE:
		...
		break;
#endif
  	case DISP_FEATURE_DYNAMIC_ELVSS_LEVEL:
  		DISP_INFO("DYNAMIC_ELVSS:%d\n", ctl->feature_val);

  		if (mi_cfg->feature_val[DISP_FEATURE_DYNAMIC_ELVSS_LEVEL] == ctl->feature_val) {
  			DISP_INFO("evlss level is the same, return\n");
  			break;
  		}

  		if (panel_ext->funcs->set_dynamic_elvss_level) {
  			panel_ext->funcs->set_dynamic_elvss_level(dsi->panel, ctl->feature_val);
  		}

  		mi_cfg->feature_val[DISP_FEATURE_DYNAMIC_ELVSS_LEVEL] = ctl->feature_val;
  		break;
	case DISP_FEATURE_LAYER_HAS_AOD:
		DISP_INFO("layer_has_aod:%x\n", ctl_temp->feature_val);
		if (mi_cfg->feature_val[DISP_FEATURE_LAYER_HAS_AOD] != ctl_temp->feature_val)
			mtk_crtc->aod_layer_changed = 1;
		mi_cfg->feature_val[DISP_FEATURE_LAYER_HAS_AOD] = ctl_temp->feature_val;
		mtk_crtc->has_aod_layer = ctl_temp->feature_val;
		break;
...
#ifdef CONFIG_VIS_DISPLAY_V2_D2
static int send_dsi_cmd(struct mtk_ddic_dsi_msg *cmd_msg,
	struct mtk_dsi_cmd_option *cmd_opt,
	struct mtk_dsi_cmd_msg *dsi_cmd,
	struct mipi_dsi_msg *msg)
{
	...
	if (!cmd_msg || !cmd_opt || !dsi_cmd || !msg) {
		pr_err("%s: Invalid parameters\n", __func__);
		return -EINVAL;
	}

	cmd_num = cmd_msg->tx_cmd_num;
	if (cmd_num == 0 || cmd_num > MAX_TX_CMD_NUM) {
		pr_err("%s: Invalid cmd_num(%u), max=%u\n", __func__, cmd_num, MAX_TX_CMD_NUM);
		return -EINVAL;
	}
	...
	if ((mtk_dsi_cmd_version() == DSI_CMD_V2) && dsi_cmd_v2_dbg[DDIC_CMD_DBG]) {
		...
		cmd_opt->flags = MTK_MIPI_DSI_CMD_EXTERNAL | MTK_MIPI_DSI_CMD_NEED_LOCK |
						MTK_MIPI_DSI_CMD_KICK_IDLE | MTK_MIPI_DSI_CRTC_ID |
						MTK_MIPI_DSI_GCE_BLOCKING_FLUSH | MTK_MIPI_DSI_GCE_USE_DSI_CMD_EVENT;
		dsi_cmd->is_package = 0;
		dsi_cmd->cmd_num = cmd_num;
		dsi_cmd->transfer_mode = PACKET_HS_MODE;
		dsi_cmd->cmd_msg = msg;
		ret = mtk_mipi_dsi_cmd(NULL, NULL, cmd_opt, dsi_cmd);
	} else {
		ret = mtk_ddic_dsi_send_cmd(cmd_msg, true);
	}
	return ret;
...
static void mi_extmv_info_send(struct mtk_dsi *dsi)
{
	...
	u32 send_package_size = 0;
	u32 send_payload_size;
	struct mtk_dsi_cmd_option cmd_opt = { 0 };
	struct mtk_dsi_cmd_msg dsi_cmd = { 0 };
	struct mipi_dsi_msg *msg;

	if (!dsi) {
		pr_err("[EXTMV]%s: Invalid dsi parameter\n", __func__);
		return;
	}
	...
	if ((mtk_dsi_cmd_version() == DSI_CMD_V2) && dsi_cmd_v2_dbg[DDIC_CMD_DBG]) {
		send_package_size = EXTMV_PKG_SIZE_V2;
	} else {
		send_package_size = EXTMV_PKG_SIZE_V1;
	}
	send_payload_size = (send_package_size - EXTMV_PKG_HEADER_SIZE) / 4;
	...
	if (meta_length < EXTMV_META_HEADER_LENGTH) {
		kfree(meta_content);
		pr_err("[EXTMV]%s: invalid meta_length(%u), min required %u\n", __func__, meta_length, EXTMV_META_HEADER_LENGTH);
		return;
	}

	meta_size = meta_content[EXTMV_OFF_HDR] & EXTMV_MASK_LOW_16BIT;
	test_pattern = !!(meta_content[EXTMV_OFF_HDR] & EXTMV_MASK_TEST_PATTERN);
	if (meta_size >= EXTMV_MAX_META_SIZE && test_pattern == false) {
		kfree(meta_content);
		return;
	}
	...
	if (meta_send_buffer == NULL) {
		kfree(meta_content);
		pr_err("[EXTMV][%s(%d)] Failed to allocate buffer\n", __func__, __LINE__);
		return;
	}
	...
	if (!cmd_msg) {
		...
		pr_err("[EXTMV][%s(%d)] Failed to alloc memory for cmd_msg", __FUNCTION__, __LINE__);
		return;
	}

	msg = vmalloc(MAX_TX_CMD_NUM * sizeof(struct mipi_dsi_msg));
	if (!msg) {
		kfree(meta_content);
		kfree(meta_send_buffer);
		vfree(cmd_msg);
		pr_err("[EXTMV][%s(%d)] Failed to alloc memory for msg", __FUNCTION__, __LINE__);
		return;
	}

	//Metadata + Timestamp
	meta_send_buffer[0] = EXTMV_SIG_NVT_TYPE08;	//NVT
	meta_send_buffer[1] = EXTMV_SIG_META;	//META
	meta_send_buffer[2] = (meta_content[EXTMV_OFF_TS_LO] & EXTMV_MASK_LOW_16BIT) | (meta_content[EXTMV_OFF_TS_HI] << 16);
	meta_send_buffer[3] = EXTMV_IC_META_TIMESTAMP;	//ICMetaTimestamp, payload size = 8 bytes
	meta_send_buffer[4] = (meta_content[EXTMV_OFF_TS2_LO] & EXTMV_MASK_LOW_16BIT) | (meta_content[EXTMV_OFF_TS2_HI] << 16);
	meta_send_buffer[5] = (meta_content[EXTMV_OFF_TS3_LO] & EXTMV_MASK_LOW_16BIT) | (meta_content[EXTMV_OFF_TS3_HI] << 16);
	//MetaMV
	meta_send_buffer[6] = (meta_content[EXTMV_OFF_MV_LO] & EXTMV_MASK_LOW_16BIT) | (meta_content[EXTMV_OFF_MV_HI] << 16);

	cmd_msg->type[0] = EXTMV_DSI_TYPE_DCS_WRITE;
	cmd_msg->tx_buf[0] = meta_send_buffer;
	cmd_msg->tx_len[0] = EXTMV_METADATA_HEADER_LEN;

	mv_size = meta_content[EXTMV_OFF_MV_SIZE];
	if (mv_size > (meta_length - EXTMV_META_HEADER_LENGTH)) {
		kfree(meta_content);
		kfree(meta_send_buffer);
		vfree(cmd_msg);
		vfree(msg);
	...
	while (mv_size > 0) {
		*(meta_send_buffer + (meta_send_buffer_index * (send_payload_size + 1))) = EXTMV_SIG_NVT_TYPE09;	//NVT
		...
		memcpy(meta_send_buffer + (meta_send_buffer_index * (send_payload_size + 1) + 1), meta_content + EXTMV_OFF_MV_DATA_START + (i * send_payload_size), send_payload_size * sizeof(u32));

		cmd_msg->type[meta_send_buffer_index] = EXTMV_DSI_TYPE_DCS_WRITE;
		cmd_msg->tx_buf[meta_send_buffer_index] = (meta_send_buffer + (meta_send_buffer_index * (send_payload_size + 1)));
		cmd_msg->tx_len[meta_send_buffer_index] = (send_size * 4) + EXTMV_PKG_HEADER_SIZE;
		...
		if (meta_send_buffer_index >= MAX_TX_CMD_NUM) {
			cmd_msg->channel = EXTMV_DSI_CHANNEL;
			cmd_msg->flags = EXTMV_DSI_FLAGS;
			cmd_msg->tx_cmd_num = MAX_TX_CMD_NUM;

			ret = send_dsi_cmd(cmd_msg, &cmd_opt, &dsi_cmd, msg);
			if (ret != 0) {
				kfree(meta_content);
				kfree(meta_send_buffer);
				vfree(cmd_msg);
				vfree(msg);
				pr_err("[EXTMV][%s(%d)] Failed to send meta data ret(%d)\n", __func__, __LINE__, ret);
				return;
			}

			meta_send_buffer_index = 0;
			memset(cmd_msg, 0, sizeof(struct mtk_ddic_dsi_msg));
			memset(msg, 0, MAX_TX_CMD_NUM * sizeof(struct mipi_dsi_msg));
		}
	}

	//MetaEnd
	mv_size = meta_content[EXTMV_OFF_MV_SIZE];
	if ((mv_size + EXTMV_META_HEADER_LENGTH + EXTMV_META_END_LENGTH) > meta_length) {
		vfree(msg);
		vfree(cmd_msg);
		kfree(meta_send_buffer);
		kfree(meta_content);
		pr_err("[EXTMV]%s: Invalid range mv_size(%u) + meta_length(%u)\n", __func__, mv_size, meta_length);
		return;
	}

	*(meta_send_buffer + (meta_send_buffer_index * (send_payload_size + 1))) = EXTMV_SIG_NVT_TYPE09;	//NVT
	*(meta_send_buffer + (meta_send_buffer_index * (send_payload_size + 1)) + 1) = (meta_content[EXTMV_OFF_END_LO(mv_size)] & EXTMV_MASK_LOW_16BIT) | (meta_content[EXTMV_OFF_END_HI(mv_size)] << 16);
	*(meta_send_buffer + (meta_send_buffer_index * (send_payload_size + 1)) + 2) = (meta_content[EXTMV_OFF_END_LO2(mv_size)] & EXTMV_MASK_LOW_16BIT) | (meta_content[EXTMV_OFF_END_HI2(mv_size)] << 16);
	...
	cmd_msg->type[meta_send_buffer_index] = EXTMV_DSI_TYPE_DCS_WRITE;
	cmd_msg->tx_buf[meta_send_buffer_index] = (meta_send_buffer + (meta_send_buffer_index * (send_payload_size + 1)));
	cmd_msg->tx_len[meta_send_buffer_index] = EXTMV_META_END_LEN;
	...
	if (ret != 0) {
		...
		kfree(meta_send_buffer);
		kfree(meta_content);
		pr_err("[EXTMV]%s Failed to send meta end ret(%d)\n", __func__, ret);
		return;
	}
...
#endif
	mi_cfg->aod_unset_backlight = false;
