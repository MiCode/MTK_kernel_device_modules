...
ssize_t  mi_dsi_panel_read_mipi_reg(char *buf)
{
	...
	const char help_message[] = {
		...
		"\tThird byte means LPM/HS send: 00/01 (Hexadecimal)\n"
		"\tExample:\n"
		"\t\twrite:00 00 00 39 01 00 00 00 00 03 51 07 FF\n"
			...
			int doze_brightness)
{
...
#endif
		dsi->mi_cfg.aod_en = false;
		if (panel_ext && panel_ext->funcs && panel_ext->funcs->set_only_aod_backlight)
			panel_ext->funcs->set_only_aod_backlight(dsi->panel, doze_brightness);
		mutex_unlock(&dsi->dsi_lock);
		return ret;
	} else if(dsi->mi_cfg.aod_en == false && doze_brightness == 0) {
		mutex_unlock(&dsi->dsi_lock);
		return ret;
	}
	...
	if (!(panel_ext && panel_ext->funcs && panel_ext->funcs->set_doze_brightness)) {
		...
		if (dsi->mi_cfg.fod_anim_flag && doze_brightness) {
			pr_info("fod_anim_flag exist, skip notify sf change 30hz fps!\n");
		} else {
		...
		if (!panel_ext->funcs->set_doze_brightness(dsi->panel, doze_brightness)) {
			...
			dsi->mi_cfg.aod_en = true;

		} else {
			 mi_disp_feature_event_notify_by_type(mi_get_disp_id("primary"), MI_DISP_EVENT_DOZE, sizeof(doze_brightness), DOZE_TO_NORMAL);
			 dsi->mi_cfg.feature_val[DISP_FEATURE_DOZE_BRIGHTNESS] = DOZE_TO_NORMAL;
		}
	}

	mutex_unlock(&dsi->dsi_lock);
	dsi->mi_cfg.mdoze_brightness = doze_brightness;
			...
			int brightness)
{
	...
	DISP_TIME_INFO("%s +, brightness = %d\n", __func__, brightness);
	comp = &dsi->ddp_comp;
	panel_ext = mtk_dsi_get_panel_ext(comp);
	mutex_lock(&dsi->dsi_lock);
	if (!(panel_ext && panel_ext->funcs && panel_ext->funcs->setbacklight_control)) {
		pr_info("set_backlight_control func not defined");
		ret = 0;
	} else {
			...
			int backlight)
{
	...
	mtk_crtc = dsi->ddp_comp.mtk_crtc;
	mtk_drm_setbacklight(&mtk_crtc->base, backlight, 0, (0X1<<SET_BACKLIGHT_LEVEL), 0);
	return ret;
}
...
int mi_dsi_panel_set_disp_param(struct mtk_dsi *dsi, struct disp_feature_ctl *ctl)
{
	...
	case DISP_FEATURE_DOZE_BRIGHTNESS:
...
#if (IS_ENABLED(CONFIG_DRM_PANEL_O16U_42_02_0A_DSC_VDO) || IS_ENABLED(CONFIG_DRM_PANEL_O16U_36_02_0B_DSC_VDO) || IS_ENABLED(CONFIG_DRM_PANEL_O16U_44_0F_0C_DSC_VDO) || IS_ENABLED(CONFIG_DRM_PANEL_O16U_42_02_0D_DSC_VDO)) && !IS_ENABLED(CONFIG_FACTORY_BUILD)
		...
		if (is_support_doze_brightness(ctl->feature_val)) {
			...
			if (ctl->feature_val == DOZE_TO_NORMAL && mi_cfg->feature_val[DISP_FEATURE_DOZE_BRIGHTNESS] != DOZE_TO_NORMAL) {
				if (panel_ext->funcs->doze_disable)
				...
				if (panel_ext->funcs->doze_enable)
	...
	case DISP_FEATURE_FOD_CALIBRATION_BRIGHTNESS:
		if (ctl->feature_val == -1) {
			...
			if (panel_ext->funcs->setbacklight_control) {
				...
				if (panel_ext->funcs->get_panel_info) {
					panel_ext->funcs->get_panel_info(dsi->panel, panel_name_buf);
					if(strstr(panel_name_buf, "dsi_o16u_36_02_0b_dsc_vdo")) {
		    ...
		    if (ctl->feature_val >= 0 && ctl->feature_val <= 2047) {
	...
	if (ctl->feature_val  == AUTH_STOP && (mi_cfg->user_doze_brightness ||mi_cfg->doze_state)) {
		if (panel_ext && panel_ext->funcs && panel_ext->funcs->set_only_aod_backlight)
			panel_ext->funcs->set_only_aod_backlight(dsi->panel, 0);
	}
#endif
		break;
	case DISP_FEATURE_LCD_HBM:
		...
		if(ctl->feature_val == FEATURE_ON) {
			mi_dsi_panel_count_enter(dsi->panel, PANEL_HBM, 0, 1);
		} else {
			mi_dsi_panel_count_enter(dsi->panel, PANEL_HBM, 0, 0);
		}
#endif
...
int mi_dsi_panel_set_count_info(struct mtk_dsi *dsi, struct disp_count_info *count_info)
{
	...
	DISP_TIME_INFO("panel feature: %s, value: %d\n",
		get_disp_feature_id_name(count_info->count_info_type), count_info->count_info_val);
...
#ifdef CONFIG_MI_DISP_PANEL_COUNT
		if ( !panel_ext->funcs->get_panel_initialized(dsi->panel) && count_info->count_info_val == 1)
			mi_dsi_panel_count_enter(dsi->panel, PANEL_POWERON_COST, 1, 1);
#endif
		if (dsi->mi_cfg.feature_val[DISP_FEATURE_DOZE_BRIGHTNESS] == DOZE_TO_NORMAL &&
				panel_ext->funcs->set_only_aod_backlight && count_info->count_info_val == 0) {
			panel_ext->funcs->set_only_aod_backlight(dsi->panel, 0);
		}
		break;
...
#ifdef CONFIG_MI_DISP_PANEL_COUNT
		mi_dsi_panel_set_build_version(dsi->panel, count_info->tx_ptr, count_info->tx_len);
#endif
		break;
	...
	case DISP_COUNT_INFO_SWITCH_KERNEL_FUNCTION_TIMER:
		DISP_INFO("swithc function timer:%d\n", count_info->count_info_val);
		break;
	default:
		break;
	}
...
void mi_disp_cfg_init(struct mtk_dsi *dsi)
{
	...
	mi_cfg->feature_val[DISP_FEATURE_GIR] = 1;
