			...
			char *buf)
{
	...
	pr_info("%s,%p\n",__func__,panel_ext);
	pr_info("%s,%p\n",__func__,panel_ext->funcs);
	pr_info("%s,%p\n",__func__,panel_ext->funcs->get_panel_info);
...
int mi_dsi_panel_set_disp_param(struct mtk_dsi *dsi, struct disp_feature_ctl *ctl)
{
	...
	case DISP_FEATURE_HBM:
		...
		if (panel_ext->funcs->hbm_control_lc) {
			pr_info("lcm hbm enable!\n");
			panel_ext->funcs->hbm_control_lc(dsi->panel, 1);
	...
	case DISP_FEATURE_LOCAL_HBM:
		...
		break;
	case DISP_FEATURE_CABC://DISP_FEATURE_BRIGHTNESS:
		pr_info("lcm cabc entry, %d\n",ctl->feature_val);
		mi_cfg->feature_val[DISP_FEATURE_CABC] = ctl->feature_val;
		if (panel_ext->funcs->cabc_control_lc) {
			panel_ext->funcs->cabc_control_lc(dsi->panel, ctl->feature_val);
			pr_info("lcm cabc end\n");
		}
		break;
	...
	case DISP_FEATURE_ROUND_MODE:
		...
		break;
	case DISP_FEATURE_HBM_OFF://DISP_FEATURE_SPR_RENDER:
		mi_cfg->feature_val[DISP_FEATURE_HBM] = ctl->feature_val;
		mi_disp_feature_event_notify_by_type(mi_get_disp_id("primary"), MI_DISP_EVENT_HBM, sizeof(ctl->feature_val), ctl->feature_val);
		pr_info("lcm hbm off entry!\n");
		if (panel_ext->funcs->hbm_control_lc) {
			panel_ext->funcs->hbm_control_lc(dsi->panel, 0);
			pr_info("lcm hbm disable!\n");
