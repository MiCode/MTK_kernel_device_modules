...
void hw_item_add_child(hw_item *root, const char *name, const char *value)
{
	...
	if (!it) {
		...
		if (name)
			strscpy(it->name, name, MAX_LEN_STR);

		if (value)
			strscpy(it->value, value, MAX_LEN_STR);
...
void hw_item_update_child(hw_item *child, const char *name, const char *value)
{
	...
	if (value) {
		strscpy(child->value, value, MAX_LEN_STR);
	}
}
...
char crypto_key[MAX_KEY_LEN] = INIT_KEY;
			     ...
			     size_t count)
{
	...
	strncpy(crypto_key, buf, MAX_KEY_LEN-1);
			    ...
			    struct kobj_attribute *attr, char *buf)
{
	...
	for (i = 0; i < strlen(src); i += blocksize) {
		...
		strscpy(padding, &src[i], blocksize + 1);
			   ...
			   struct kobj_attribute *attr, char *buf)
{
	...
	hw_item_dump();
	memcpy(buffer_show, print_buf, PAGE_SIZE);
