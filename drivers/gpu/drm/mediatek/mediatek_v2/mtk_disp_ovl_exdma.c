			   ...
			   struct mtk_ddp_config *cfg, struct cmdq_pkt *handle)
{
	...
	mtk_ovl_exdma_golden_setting(comp, cfg, handle);
}
			      ...
			      unsigned int ext_idx, struct cmdq_pkt *handle)
{
	...
	if (ext_idx != LYE_NORMAL)
		...
		if (idx == 0)
			cmdq_pkt_write(handle, comp->cmdq_base,
				comp->regs_pa + DISP_REG_OVL_L_EN(idx), 0, DISP_OVL_L_EN);
		else
			cmdq_pkt_write(handle, comp->cmdq_base,
				comp->regs_pa + DISP_REG_OVL_SRC_CON, 0, BIT(idx));
...
void mtk_ovl_exdma_cal_golden_setting(struct mtk_ddp_config *cfg,
				  ...
				  struct cmdq_pkt *handle)
{
	...
	mtk_ovl_exdma_cal_golden_setting(cfg, comp, gs);
