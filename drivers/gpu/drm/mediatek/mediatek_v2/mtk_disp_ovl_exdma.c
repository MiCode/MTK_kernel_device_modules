...
#define SMI_FLUSH_CMD_EN		BIT(28)
#define SMI_FLUSH_CMD_PREULTRA	BIT(29)
#define SMI_FLUSH_CMD_ULTRA		BIT(30)
	...
	*/
	...
	if ((priv->data->mmsys_id == MMSYS_MT6991)
		...
		cmdq_pkt_write(handle, comp->cmdq_base, comp->regs_pa
			+ DISP_REG_OVL_DATAPATH_CON, value, mask);
		cmdq_pkt_write(handle, comp->cmdq_base, comp->regs_pa + OVL_MOUT, 0x1, ~0);
	} else {
		cmdq_pkt_write(handle, comp->cmdq_base, comp->regs_pa
			+ DISP_REG_OVL_DATAPATH_CON, value, mask);
		cmdq_pkt_write(handle, comp->cmdq_base, comp->regs_pa + OVL_MOUT, 0x2, ~0);
	}
			     ...
			     unsigned int ext_idx, struct mtk_plane_state *state, struct cmdq_pkt *handle)
{
	...
	unsigned int value = (compress != 0) ? 0x10 : 0;
	unsigned int mask = DISP_OVL_L_FBCD_EN | LSRC_PQ | DISP_OVL_L_EN;
	...
	if (ext_idx != LYE_NORMAL) {
		...
		cmdq_pkt_write(handle, comp->cmdq_base,
			comp->regs_pa + DISP_REG_OVL_L_EN(0), value, mask);
}
				      ...
				      dma_addr_t addr, dma_addr_t hdr_addr)
{
	...
	if (ext_lye_idx != LYE_NORMAL) {
		...
		cmdq_pkt_write(handle, comp->cmdq_base,
				comp->regs_pa + DISP_REG_OVL_ADDR_MSB(ext_lye_idx),
				addr_msb, ~0);
	}
}
...
//		0, ~0);
	...
	if (ext_lye_idx != LYE_NORMAL) {
		...
		write_phy_layer_hdr_addr_cmdq(comp, handle, lye_idx, lx_hdr_addr);
		write_addr_msb_cmdq(comp, handle, ext_lye_idx, lx_addr, lx_hdr_addr);
				  ...
				  struct cmdq_pkt *handle)
{
	...
	/* last cmd force preultra/ultra */
	regval = SMI_FLUSH_CMD_EN | SMI_FLUSH_CMD_PREULTRA | SMI_FLUSH_CMD_ULTRA;
	cmdq_pkt_write(handle, comp->cmdq_base, baddr + DISP_REG_OVL_SMI_2ND_CFG,
		       regval, regval);
	cmdq_pkt_write(handle, comp->cmdq_base, baddr + DISP_REG_OVL_FUNC_DCM0,
		       BIT(3), BIT(3));
...
int mtk_ovl_exdma_dump(struct mtk_ddp_comp *comp)
{
	...
	mtk_serial_dump_reg(baddr, 0x8f0, 1);
	...
	bool en, struct cmdq_pkt *handle)
{
	...
	mtk_ddp_write_mask(comp, en, DISP_REG_OVL_DUMMY_REG, DISP_OVL_EXT_DDR_EN_OPT, handle);
}
