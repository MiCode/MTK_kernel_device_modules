...
#endif
//#include "include/panel_p12a_36_02_0a_alpha_data.h"
#include "include/panel-p12a-36-02-0a-dsc-vdo.h"
...
static void push_table(struct lcm *ctx, struct LCM_setting_table *table, unsigned int count)
{
	...
	for (i = 0; i < count; i++) {
		...
		switch (cmd) {
		case REGFLAG_DELAY:
			usleep_range(table[i].count * 1000, (table[i].count * 1000) + 20);
			break;
...
static void lcm_panel_init(struct lcm *ctx)
{
	...
	if (dsi != NULL && dsi->mi_cfg.is_tddi_flag && dsi->mi_cfg.tddi_gesture_flag) {
	...
	if (panel_build_id >= PANEL_BUILD_ID_P10 && ctx->gray_level >= csc_index[1][0]
		    && ctx->gray_level <= csc_index[1][ARRAY_SIZE(csc_index[1]) - 1] ) {
		csc_on[CSC_CMD_INDEX].para_list[0] = ctx->gray_level;
		push_table(ctx, csc_on, sizeof(csc_on) / sizeof(struct LCM_setting_table));
	}
...
static int lcm_panel_poweroff(struct drm_panel *panel)
{
	...
	if (dsi->mi_cfg.is_tddi_flag) {
		if (!dsi->mi_cfg.tddi_gesture_flag || dsi->mi_cfg.panel_dead_flag) {
			pr_info("%s set reset_gpio 0!!\n", __func__);
			gpiod_set_value(ctx->reset_gpio, 0);
			devm_gpiod_put(ctx->dev, ctx->reset_gpio);
			udelay(1000);
		} else if (dsi->mi_cfg.tddi_gesture_flag) {
			pr_info("%s set reset_gpio 1!!\n", __func__);
			devm_gpiod_put(ctx->dev, ctx->reset_gpio);
			udelay(1000);
		}
	} else {
	...
	if (dsi->mi_cfg.is_tddi_flag) {
		if(!dsi->mi_cfg.tddi_gesture_flag || dsi->mi_cfg.panel_dead_flag) {
			...
			usleep_range(35 * 1000, (35 * 1000)+20);
...
#else
		.dfps_cmd_table[0] = {0, 7, {0x51,0x03,0x28,0x00,0x00,0x07,0xFF} },
...
static int panel_set_doze_brightness(struct drm_panel *panel, int doze_brightness)
{
	...
	if (DOZE_BRIGHTNESS_LBM  == doze_brightness || DOZE_BRIGHTNESS_HBM  == doze_brightness) {
		if (DOZE_BRIGHTNESS_LBM  == doze_brightness){
			...
			ext_params_30hz.dyn_fps.dfps_cmd_table[0].para_list[5] = 0x07;
...
static int lcm_setbacklight_control(struct drm_panel *panel, unsigned int level)
{
	...
	bl_value[0].para_list[0] = (level >> 8) & 0xFF;
	bl_value[0].para_list[1]  = level & 0xFF;

	mi_disp_panel_ddic_send_cmd(bl_value, ARRAY_SIZE(bl_value), format);
	if (level > 0)
		dsi->mi_cfg.last_no_zero_bl_level = level;
	ctx = panel_to_lcm(panel);
	if (level > 8191) {
...
int get_lockdown_info(unsigned char* p_lockdown_info)
{
	...
	if (sscanf(oled_tplockdown_cmdline, "%02hhx%02hhx%02hhx%02hhx%02hhx%02hhx%02hhx%02hhx\n",
		...
		if (rx_buf[0] == 0x46 && rx_buf[1] == 0x36) {
			pr_info("%s: lockdown is ok\n", __func__);
		}else {
			pr_err("lockdown error !");
			goto err;
		}
	} else {
		pr_info("%s: get error\n", __func__);
...
static int mi_disp_panel_update_lhbm_code(struct mtk_dsi * dsi, enum lhbm_cmd_type type, int flat_mode, int bl_level)
{
	...
	case TYPE_WHITE_1200:
		temp_val = sizeof(lhbm_ratio) / sizeof(lhbm_ratio[0]);
		for(i = 0; i < temp_val; i++) {
			if(bl_level < lhbm_ratio[i][RATIO_DBV]) {
				gamma_ratio[0] = lhbm_ratio[i][RATIO1_1200NIT];
				gamma_ratio[1] = lhbm_ratio[i][RATIO2_1200NIT];
				gamma_ratio[2] = lhbm_ratio[i][RATIO3_1200NIT];
				pr_info("TYPE_WHITE_1200 ratio:%d %d %d\n", gamma_ratio[0], gamma_ratio[1], gamma_ratio[2]);
				break;
			}
		}
		...
		for(i = 0; i < 3; i++) {
			...
			lhbm_normal_white_1200nit[4].para_list[2*i] = (temp_val >> 8) & 0xFF;
			lhbm_normal_white_1200nit[4].para_list[2*i+1] = temp_val &0xFF;
		}
		break;
	case TYPE_WHITE_200:
		temp_val = sizeof(lhbm_ratio) / sizeof(lhbm_ratio[0]);
		for(i = 0; i < temp_val; i++) {
			if(bl_level < lhbm_ratio[i][RATIO_DBV]) {
				gamma_ratio[0] = lhbm_ratio[i][RATIO1_250NIT];
				gamma_ratio[1] = lhbm_ratio[i][RATIO2_250NIT];
				gamma_ratio[2] = lhbm_ratio[i][RATIO3_250NIT];
				pr_info("TYPE_WHITE_200 ratio:%d %d %d\n", gamma_ratio[0], gamma_ratio[1], gamma_ratio[2]);
				break;
			}
		}
		...
		for(i = 0; i < 3; i++) {
			...
			lhbm_normal_white_200nit[4].para_list[2*i] = (temp_val >> 8) & 0xFF;
			lhbm_normal_white_200nit[4].para_list[2*i+1] = temp_val &0xFF;
		}
		break;
	case TYPE_LHBM_OFF:
...
static int panel_set_gray_by_temperature (struct drm_panel *panel, int level)
{
	...
	csc_on[CSC_CMD_INDEX].para_list[0] = csc_index[1][ARRAY_SIZE(csc_index[1]) - 1];
	for ( int i = 0; i < ARRAY_SIZE(csc_index[0]); i++) {
		if (level < csc_index[0][i]) {
			csc_on[CSC_CMD_INDEX].para_list[0] = csc_index[1][i];
			break;
		}
	}
	if (ctx->gray_level == csc_on[CSC_CMD_INDEX].para_list[0])
		goto err;
	ctx->gray_level = csc_on[CSC_CMD_INDEX].para_list[0];
	pr_info("%s: level = %x\n", __func__, ctx->gray_level);
	mi_disp_panel_ddic_send_cmd(csc_on, ARRAY_SIZE(csc_on), format);
...
static int panel_set_dynamic_elvss_level (struct drm_panel *panel, int level)
{
	...
	if (panel_build_id < PANEL_BUILD_ID_P20) {
		pr_info("%s: panel build id is earlier than p2.0\n", __func__);
		return 0;
	}
	...
	case ELVSS_LEVEL_3:
		...
		break;
	case ELVSS_LEVEL_4:
		mi_disp_panel_ddic_send_cmd(elvss_level_4, ARRAY_SIZE(elvss_level_4), format);
		break;
	default:
...
static int lcm_probe(struct mipi_dsi_device *dsi)
{
	...
	pr_info("%s p12a-36-02-0a-dsc-vdo -\n", __func__);
	return ret;
}
...
static const struct of_device_id lcm_of_match[] = {
	{ .compatible = "p12a_36_02_0a_dsc_vdo,lcm", },
	{ }
};
...
static struct mipi_dsi_driver lcm_driver = {
	...
	.remove = lcm_remove,
	.driver = {
		.name = "p12a_36_02_0a_dsc_vdo,lcm",
		.owner = THIS_MODULE,
...
MODULE_AUTHOR("wuxuewen <wuxuewen@xiaomi.com>");
MODULE_DESCRIPTION("p12a_36_02_0a_dsc_vdo oled panel driver");
