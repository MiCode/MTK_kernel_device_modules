...
bool lcd_reset_keep_high_1st = false;
void set_lcd_reset_gpio_keep_high_1st(bool en)
{
	lcd_reset_keep_high_1st = en;
}
EXPORT_SYMBOL(set_lcd_reset_gpio_keep_high_1st);
...
#define LCM_DRIVER_NAME "dsi_panel_p15a_42_02_0a_dsc_vdo" 
static const char *panel_info = "panel_name=dsi_panel_p15a_42_02_0a_dsc_vdo";
...
static int lcm_unprepare(struct drm_panel *panel)
{
	...
	if (prox_lcd_reset_keep_high1 && esd_panel_dead == false) {
		pr_info("[LCM]:prox_lcd_reset_keep_high1 = %d\n",prox_lcd_reset_keep_high1);
		return 0;
	}
	lcm_dcs_write_seq_static(ctx, 0x10);
	if (lcd_reset_keep_high_1st && esd_panel_dead == false) {
	...
	void *handle, unsigned int level)
{
	...
	pr_info("CSOT %s,brightness 0x%02x level %d\n", __func__, level, brightness);
