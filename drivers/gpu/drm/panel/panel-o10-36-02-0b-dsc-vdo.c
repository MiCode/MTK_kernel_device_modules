...
#endif
#include "include/panel_o10_36_02_0b_alpha_data.h"
#include "include/panel-o10-36-02-0b-dsc-vdo.h"
...
static int lcm_disable(struct drm_panel *panel)
{
	...
	if (!ctx->enabled)
		return 0;
	if (ctx->lhbm_en) {
		push_table(ctx, lhbm_off_delay, sizeof(lhbm_off_delay) / sizeof(struct LCM_setting_table));
		pr_info("%s panel lhbm off\n", __func__);
	}
...
static void lcm_panel_init(struct lcm *ctx)
{
	...
	usleep_range(2 * 1000, (2* 1000)+20);
	gpiod_set_value(ctx->reset_gpio, 1);
	usleep_range(41 * 1000, (41 * 1000)+20);
			...
			struct drm_connector *connector, unsigned int mode)
{
	...
	if (dst_fps == 30) {
		...
		if (dsi && (dsi->mi_cfg.feature_val[DISP_COUNT_INFO_POWERSTATUS] == 1 ||
				dsi->mi_cfg.brightness_clone)) {
			ext_params_60hz.dyn_fps.dfps_cmd_table[2].para_list[1] = 0x80;
		} else if (dsi && (dsi->mi_cfg.feature_val[DISP_COUNT_INFO_POWERSTATUS] == 2 ||
				dsi->mi_cfg.feature_val[DISP_COUNT_INFO_POWERSTATUS] == 0)) {
	...
	if (ctx->dynamic_fps!=60 && ctx->peak_hdr_status) {
		ret = mi_disp_panel_ddic_send_cmd(apb_mode_off, ARRAY_SIZE(apb_mode_off), format);
		ctx->peak_hdr_status = 0;
	}else if(ctx->dynamic_fps==60 && !ctx->peak_hdr_status 
		&& dsi->mi_cfg.last_bl_level == PEAK_MAX_BL_LEVEL) {
		ret = mi_disp_panel_ddic_send_cmd(apb_mode_on, ARRAY_SIZE(apb_mode_on), format);
...
{
	char user_cmd_unlock1[] = {0x9C,0xA5,0xA5};
	char user_cmd_unlock2[] = {0xFD,0x5A,0x5A};
	char user_cmd_lock1[] = {0x9C,0x5A,0x5A};
	char user_cmd_lock2[] = {0xFD,0xA5,0xA5};
	char mode_switch[] = {0x48,0x33};
	char backlight_51[] = {0x51,0x0F,0xFF};
	//char gir_off_set1[] = {0x9F,0x08};
	//char gir_off_set2[] = {0xB2,0x10};
	char gir_on_set1[] = {0x9F,0x08};
	char gir_on_set2[] = {0xB2,0x12};
	char apb_mode_on[] = {0xCC,0x03,0xFF,0x18};
	char apb_mode_off[] = {0xCC,0x02,0xFF,0x18};
	...
	if (bl_level > PEAK_HDR_BL_LEVEL && bl_level <= PEAK_MAX_BL_LEVEL && !ctx->peak_hdr_status
		&& ctx->dynamic_fps ==60)
	{
		cb(dsi, handle, mode_switch, ARRAY_SIZE(mode_switch));
		cb(dsi, handle, backlight_51, ARRAY_SIZE(backlight_51));
		cb(dsi, handle, user_cmd_unlock1, ARRAY_SIZE(user_cmd_unlock1));
		cb(dsi, handle, user_cmd_unlock2, ARRAY_SIZE(user_cmd_unlock2));
		cb(dsi, handle, gir_on_set1, ARRAY_SIZE(gir_on_set1));
		cb(dsi, handle, gir_on_set2, ARRAY_SIZE(gir_on_set2));
		cb(dsi, handle, apb_mode_on, ARRAY_SIZE(apb_mode_on));
		cb(dsi, handle, user_cmd_lock1, ARRAY_SIZE(user_cmd_lock1));
		cb(dsi, handle, user_cmd_lock2, ARRAY_SIZE(user_cmd_lock2));
		ctx->gir_status = 1;
	...
	if (((bl_level != 0 && bl_level <= PEAK_HDR_BL_LEVEL) || ctx->dynamic_fps !=60)
		...
		cb(dsi, handle, gir_on_set1, ARRAY_SIZE(gir_on_set1));
		//cb(dsi, handle, gir_on_set2, ARRAY_SIZE(gir_on_set2));
		cb(dsi, handle, apb_mode_off, ARRAY_SIZE(apb_mode_off));
		cb(dsi, handle, user_cmd_lock1, ARRAY_SIZE(user_cmd_lock1));
		cb(dsi, handle, user_cmd_lock2, ARRAY_SIZE(user_cmd_lock2));
		ctx->gir_status = 1;
		mi_cfg->feature_val[DISP_FEATURE_FLAT_MODE]= 1;
...
static int panel_set_doze_brightness(struct drm_panel *panel, int doze_brightness)
{
	...
	if (DOZE_BRIGHTNESS_LBM  == doze_brightness || DOZE_BRIGHTNESS_HBM  == doze_brightness) {
		...
		{
			ext_params_30hz.dyn_fps.dfps_cmd_table[1].para_list[1] = 0x01;
			if (ctx->dynamic_fps == 30)
				ret = mi_disp_panel_ddic_send_cmd(lcm_aod_low_mode, ARRAY_SIZE(lcm_aod_low_mode), format);
			else
				ret = mi_disp_panel_ddic_send_cmd(lcm_aod_low_mode_normal, ARRAY_SIZE(lcm_aod_low_mode_normal), format);
		} else if (DOZE_BRIGHTNESS_HBM == doze_brightness) {
			ext_params_30hz.dyn_fps.dfps_cmd_table[1].para_list[1] = 0x03;
			if (ctx->dynamic_fps == 30)
				ret = mi_disp_panel_ddic_send_cmd(lcm_aod_high_mode, ARRAY_SIZE(lcm_aod_high_mode), format);
			else
				ret = mi_disp_panel_ddic_send_cmd(lcm_aod_high_mode_normal, ARRAY_SIZE(lcm_aod_high_mode_normal), format);
		}
		atomic_set(&doze_enable, 1);
	}
...
static int mi_disp_panel_update_lhbm_code(struct mtk_dsi * dsi, enum lhbm_cmd_type type, int bl_level)
{
	u8 alpha_buf[2] = {0};
	...
	if (!(type == TYPE_LHBM_OFF || type == TYPE_HLPM_OFF)) {
		...
		pr_info("lhbm update code, lhbm_cmd_type:%d, bl_level = %d, alpha = %d\n", type, bl_level, alpha_set[bl_level]);
	}

	switch (type) {
	case TYPE_WHITE_1200:
		alpha_buf[0] = 0xA0 + (alpha_set[bl_level] >> 8);
		alpha_buf[1] = alpha_set[bl_level] & 0xff;
		pr_info("[%d] alpha_buf[0] = %02hhx, alpha_buf[1] = %02hhx\n",
				type, alpha_buf[0],  alpha_buf[1]);
		lhbm_normal_white_1200nit[0].para_list[1] = alpha_buf[0];
		lhbm_normal_white_1200nit[0].para_list[2] = alpha_buf[1];
		break;
	case TYPE_WHITE_200:
		alpha_buf[0] = 0xA0 + (alpha_set[bl_level] >> 8);
		alpha_buf[1] = alpha_set[bl_level] & 0xff;
		pr_info("[%d] alpha_buf[0] = %02hhx, alpha_buf[1] = %02hhx\n",
				type, alpha_buf[0],  alpha_buf[1]);
		lhbm_normal_white_200nit[0].para_list[1] = alpha_buf[0];
		lhbm_normal_white_200nit[0].para_list[2] = alpha_buf[1];
		break;
	case TYPE_GREEN_500:
		alpha_buf[0] = 0xA0 + (alpha_set[bl_level] >> 8);
		alpha_buf[1] = alpha_set[bl_level] & 0xff;
		pr_info("[%d] alpha_buf[0] = %02hhx, alpha_buf[1] = %02hhx\n",
				type, alpha_buf[0],  alpha_buf[1]);
		lhbm_normal_green_500nit[0].para_list[1] = alpha_buf[0];
		lhbm_normal_green_500nit[0].para_list[2] = alpha_buf[1];
		break;
	case TYPE_LHBM_OFF:
...
static int panel_set_lhbm_fod(struct mtk_dsi *dsi, enum local_hbm_state lhbm_state)
{
	...
	case LOCAL_HBM_OFF_TO_NORMAL:
		...
		mi_disp_panel_update_lhbm_code(dsi, TYPE_LHBM_OFF, bl_level);
	...
	case LOCAL_HBM_OFF_TO_NORMAL_BACKLIGHT_RESTORE:
		...
		if (atomic_read(&doze_enable)) {
			if (mi_cfg->last_bl_level) {
				mi_disp_panel_update_lhbm_code(dsi, TYPE_LHBM_OFF, mi_cfg->last_bl_level);
				mi_disp_panel_ddic_send_cmd(lhbm_off, ARRAY_SIZE(lhbm_off), format);
			}else{
				mi_disp_panel_update_lhbm_code(dsi, TYPE_LHBM_OFF, 0);
				mi_disp_panel_ddic_send_cmd(lhbm_off, ARRAY_SIZE(lhbm_off), format);
			}
		}else{
			mi_disp_panel_update_lhbm_code(dsi, TYPE_LHBM_OFF, bl_level);
	...
	case LOCAL_HBM_OFF_TO_NORMAL_BACKLIGHT:
		...
		if (atomic_read(&doze_enable)) {
			if (mi_cfg->last_bl_level) {
				...
				mi_disp_panel_update_lhbm_code(dsi, TYPE_LHBM_OFF, bl_level_doze);
				mi_disp_panel_ddic_send_cmd(lhbm_off, ARRAY_SIZE(lhbm_off), format);
			}
		}else{
			mi_disp_panel_update_lhbm_code(dsi, TYPE_LHBM_OFF, bl_level);
	...
	case LOCAL_HBM_NORMAL_WHITE_110NIT:
		...
		if (atomic_read(&doze_enable)) {
			...
			mi_disp_panel_update_lhbm_code(dsi, TYPE_WHITE_200, doze_lbm_dbv_level);
		} else {
			mi_disp_panel_update_lhbm_code(dsi, TYPE_WHITE_200, bl_level);
		}
		format = FORMAT_LP_MODE | FORMAT_BLOCK;
	...
	case LOCAL_HBM_NORMAL_GREEN_500NIT:
		...
		mi_disp_panel_update_lhbm_code(dsi, TYPE_GREEN_500, bl_level);
	...
	case LOCAL_HBM_NORMAL_WHITE_1000NIT:
		...
		if (atomic_read(&doze_enable)) {
			...
			if (ctx->doze_brightness_state == DOZE_BRIGHTNESS_HBM)
				...
				mi_cfg->last_no_zero_bl_level = bl_level_doze;
			mutex_lock(&dsi->dsi_lock);
			mi_disp_panel_update_lhbm_code(dsi, TYPE_WHITE_1200, doze_lbm_dbv_level);
		} else {
			mi_disp_panel_update_lhbm_code(dsi, TYPE_WHITE_1200, bl_level);
		}
	...
	case LOCAL_HBM_HLPM_WHITE_110NIT:
		...
		mi_disp_panel_update_lhbm_code(dsi, TYPE_WHITE_200, bl_level_doze);
		format = FORMAT_LP_MODE | FORMAT_BLOCK;
		mi_disp_panel_ddic_send_cmd(lhbm_normal_white_200nit, ARRAY_SIZE(lhbm_normal_white_200nit), format);
	...
	case LOCAL_HBM_HLPM_WHITE_1000NIT:
		...
		mi_disp_panel_update_lhbm_code(dsi, TYPE_WHITE_1200, bl_level_doze);
...
#endif
	pr_info("%s o10-36-02-0b-dsc-vdo -\n", __func__);
	return ret;
}
...
MODULE_DESCRIPTION("o10_36_02_0b_dsc_vdo oled panel driver");
