...
bool lcd_reset_keep_high_3rd = false;
void set_lcd_reset_gpio_keep_high_3rd(bool en)
{
	lcd_reset_keep_high_3rd = en;
}
EXPORT_SYMBOL(set_lcd_reset_gpio_keep_high_3rd);
...
bool prox_lcd_reset_keep_high3 = false;
void set_prox_lcd_reset_gpio_keep_high3(bool en)
{
	prox_lcd_reset_keep_high3 = en;
}
EXPORT_SYMBOL(set_prox_lcd_reset_gpio_keep_high3);
...
#define LCM_DRIVER_NAME "dsi_panel_p15a_43_02_0c_dsc_vdo"
static const char *panel_info = "panel_name=dsi_panel_p15a_43_02_0c_dsc_vdo";
...
static int lcm_unprepare(struct drm_panel *panel)
{
	...
	lcm_dcs_write_seq_static(ctx, 0x68, 0x05, 0x02);
	lcm_dcs_write_seq_static(ctx, 0x28);
	msleep(20);

	if (prox_lcd_reset_keep_high3 && esd_panel_dead == false) {
		pr_info("[LCM]:prox_lcd_reset_keep_high3 = %d\n",prox_lcd_reset_keep_high3);
		return 0;
	}
	lcm_dcs_write_seq_static(ctx, 0x10);
	if (lcd_reset_keep_high_3rd && esd_panel_dead == false) {
	...
	if(lcm_shutdown_symbol){
		...
		if (IS_ERR(ctx->reset_gpio)) {
			...
			pr_info("%s: lcm_shutdown pull down reset\n", __func__);
...
static int lcm_prepare(struct drm_panel *panel)
{
	...
	if (ret < 0)
		pr_info("nt36528--kernel--bias----cmd=%0x--i2c write error----\n", cmd);
	else
		pr_info("nt36528--kernel--bias----cmd=%0x--i2c write success----\n", cmd);
	/* set AVEE -6.0V*/
	cmd = 0x01;
	data = 0x14;
	ret = lcd_bias_write_bytes(cmd, data);
...
#define NORMAL_MAX_REG	2639	//noraml段最高寄存器亮度值
#define HBM_MAX_REG	3330	//hbm段最高寄存器亮度值

static const int bl_base=500;
static int last_level = -1;
static bool bl_1st = false;
static int lcm_setbacklight_cmdq(void *dsi, dcs_write_gce cb,
	void *handle, unsigned int level)
{
	char bl_tb[] = {0x51, 0x07, 0xff};
  	char bl_dimming[] = {0x53, 0x2c};
	char bl_minimum_limit[] = {0x5E, 0x00, 0x00};
	char bl_cabc_optimization[] = {0x5E, 0x00, 0x0D};
	...
	pr_info("Truly %s,brightness 0x%02x level %d\n", __func__, level, brightness);
	...
	if(level == 0){
		msleep(25);
		cb(dsi, handle, bl_minimum_limit, ARRAY_SIZE(bl_minimum_limit));
	}
  	if (bl_1st) {
		cb(dsi, handle, bl_dimming, ARRAY_SIZE(bl_dimming));
		msleep(25);
		cb(dsi, handle, bl_cabc_optimization, ARRAY_SIZE(bl_cabc_optimization));
        }
  	last_level = level;
	return 0;
}
		...
		unsigned int dst_mode, enum MTK_PANEL_MODE_SWITCH_STAGE stage)
{
	...
	if (isFpsChange) {
		if (dst_fps == MODE0_FPS)
            ...
            mode_switch_to_90(panel, stage);
            pr_info("dsi_panel 1st,force to 90Hz=%d\n",dst_fps);
        }
	}
	return ret;
}
