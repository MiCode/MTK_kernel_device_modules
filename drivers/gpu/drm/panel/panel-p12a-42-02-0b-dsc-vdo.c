...
#endif
#include "include/panel_p12a_42_02_0b_alpha_data.h"
#include "include/panel-p12a-42-02-0b-dsc-vdo.h"
...
static void lcm_panel_vci_regulator_deinit(void)
{
	if (disp_vci && !IS_ERR(disp_vci)) {
		regulator_put(disp_vci);
		disp_vci = NULL;
	}
}
...
static void lcm_panel_vddi_regulator_deinit(void)
{
	if (disp_vddi && !IS_ERR(disp_vddi)) {
		regulator_put(disp_vddi);
		disp_vddi = NULL;
	}
}
...
static void push_table(struct lcm *ctx, struct LCM_setting_table *table, unsigned int count)
{
	...
	for (i = 0; i < count; i++) {
		...
		switch (cmd) {
		case REGFLAG_DELAY:
			usleep_range(table[i].count*1000, table[i].count*1000 + 200);
			break;
...
static void lcm_panel_init(struct lcm *ctx)
{
	...
	if (ctx->dynamic_fps == 120  || ctx->dynamic_fps == 30) {
		if (init_setting_vdo[FPS_INIT_INDEX].cmd == REG_DISPLAY_MODE_CTRL)
			init_setting_vdo[FPS_INIT_INDEX].para_list[0] = MODE_120HZ_30HZ;
		else
			pr_info("%s: please check FPS_INIT_INDEX\n", __func__);
		push_table(ctx, svm_120hz, sizeof(svm_120hz) / sizeof(struct LCM_setting_table));
	} else if (ctx->dynamic_fps == 90) {
		if (init_setting_vdo[FPS_INIT_INDEX].cmd == REG_DISPLAY_MODE_CTRL)
			init_setting_vdo[FPS_INIT_INDEX].para_list[0] = MODE_90HZ;
		else
			pr_info("%s: please check FPS_INIT_INDEX\n", __func__);
		push_table(ctx, svm_90hz, sizeof(svm_90hz) / sizeof(struct LCM_setting_table));
	} else if (ctx->dynamic_fps == 60) {
		if (init_setting_vdo[FPS_INIT_INDEX].cmd == REG_DISPLAY_MODE_CTRL)
			init_setting_vdo[FPS_INIT_INDEX].para_list[0] = MODE_60HZ;
		else
			pr_info("%s: please check FPS_INIT_INDEX\n", __func__);
		push_table(ctx, svm_60hz, sizeof(svm_60hz) / sizeof(struct LCM_setting_table));
	}

	if (dsi != NULL && dsi->mi_cfg.is_tddi_flag) {
	...
	if (panel_build_id >= PANEL_BUILD_ID_P20 && ctx->gray_level <= CSC_LEVEL_HEIGHT
		    &&  ctx->gray_level >= CSC_LEVEL_LOW) {
		csc_on[CSC_CMD_INDEX].para_list[1] = ctx->gray_level;
...
static int lcm_panel_poweroff(struct drm_panel *panel)
{
	...
	if (dsi->mi_cfg.is_tddi_flag) {
		if (!dsi->mi_cfg.tddi_gesture_flag || dsi->mi_cfg.panel_dead_flag) {
			...
			gpiod_set_value(ctx->reset_gpio, 0);
			devm_gpiod_put(ctx->dev, ctx->reset_gpio);
			usleep_range(1 * 1000, (1 * 1000)+200);
...
static int lcm_panel_poweron(struct drm_panel *panel)
{
	...
	if (dsi->mi_cfg.is_tddi_flag) {
		if (!dsi->mi_cfg.tddi_gesture_flag || dsi->mi_cfg.panel_dead_flag) {
			...
			usleep_range(9 * 1000, (9 * 1000)+200);
		...
		//VCI 3.0V
		lcm_panel_vci_enable(ctx->dev);
		usleep_range(12 * 1000, (12 * 1000)+200);
	}
	return 0;
}
...
static struct mtk_panel_params ext_params_30hz = {
	...
	.dyn_fps = {
		...
		.dfps_cmd_table[1] = {0, 3, {0x51,0x05,0x55} },
		.dfps_cmd_table[2] = {0, 1, {0x39} },
#else
		.dfps_cmd_table[0] = {0, 7, {0x51,0x03,0x28,0x00,0x00,0x05,0x55} },
...
static struct mtk_panel_params ext_params_60hz = {
	...
	.dyn_fps = {
		...
		.dfps_cmd_table[3] = {0, 2, {0x6F,0xE4}},
		.dfps_cmd_table[4] = {0, 7, {0xE5,0xFF,0xFF,0x8C,0x38,0x33,0x19}},
		.dfps_cmd_table[5] = {0, 2, {0x6F,0x02}},
		.dfps_cmd_table[6] = {0, 6, {0xE5,0x04,0x3C,0x34,0x10,0x04}},
		.dfps_cmd_table[7] = {0, 2, {0x6F,0x22}},
		.dfps_cmd_table[8] = {0, 2, {0xE5,0x18}},
		.dfps_cmd_table[9] = {0, 2, {0x6F,0x50}},
		.dfps_cmd_table[10] = {0, 4, {0xE7,0x00,0x00,0x08}},
		.dfps_cmd_table[11] = {0, 2, {0x6F,0x5C}},
		.dfps_cmd_table[12] = {0, 2, {0xE7,0x00}},
	},
	.physical_width_um = PHYSICAL_WIDTH,
	.physical_height_um = PHYSICAL_HEIGHT,
...
static struct mtk_panel_params ext_params_90hz = {
	...
	.dyn_fps = {
		...
		.dfps_cmd_table[2] = {0, 6, {0xF0,0x55,0xAA,0x52,0x08,0x01}},
		.dfps_cmd_table[3] = {0, 2, {0x6F,0xE4}},
		.dfps_cmd_table[4] = {0, 7, {0xE5,0xFF,0xFF,0x99,0x4C,0x3F,0x19}},
		.dfps_cmd_table[5] = {0, 2, {0x6F,0x02}},
		.dfps_cmd_table[6] = {0, 6, {0xE5,0x04,0x3C,0x20,0x08,0x00}},
		.dfps_cmd_table[7] = {0, 2, {0x6F,0x22}},
		.dfps_cmd_table[8] = {0, 2, {0xE5,0x18}},
		.dfps_cmd_table[9] = {0, 2, {0x6F,0x50}},
		.dfps_cmd_table[10] = {0, 4, {0xE7,0x00,0x00,0x00}},
		.dfps_cmd_table[11] = {0, 2, {0x6F,0x5C}},
		.dfps_cmd_table[12] = {0, 2, {0xE7,0xA0}},
	},
	.physical_width_um = PHYSICAL_WIDTH,
	.physical_height_um = PHYSICAL_HEIGHT,
	.change_fps_by_vfp_send_cmd = 1,
...
static struct mtk_panel_params ext_params_120hz = {
	...
	.dyn_fps = {
		...
		.dfps_cmd_table[2] = {0, 6, {0xF0,0x55,0xAA,0x52,0x08,0x01}},
		.dfps_cmd_table[3] = {0, 2, {0x6F,0xE4}},
		.dfps_cmd_table[4] = {0, 7, {0xE5,0xFF,0xFF,0xA5,0x4C,0x35,0x19}},
		.dfps_cmd_table[5] = {0, 2, {0x6F,0x02}},
		.dfps_cmd_table[6] = {0, 6, {0xE5,0x04,0x3C,0x20,0x04,0x00}},
		.dfps_cmd_table[7] = {0, 2, {0x6F,0x22}},
		.dfps_cmd_table[8] = {0, 2, {0xE5,0x14}},
		.dfps_cmd_table[9] = {0, 2, {0x6F,0x50}},
		.dfps_cmd_table[10] = {0, 4, {0xE7,0x00,0x00,0x00}},
		.dfps_cmd_table[11] = {0, 2, {0x6F,0x5C}},
		.dfps_cmd_table[12] = {0, 2, {0xE7,0x00}},
	...
	void *handle, unsigned int level)
{
	...
	struct lcm *ctx = NULL;
	if (!this_panel){
		pr_warn("%s: this_panel not initialized yet\n", __func__);
		return -ENODEV;
	}

	ctx = panel_to_lcm(this_panel);
...
static int panel_get_wp_info(struct drm_panel *panel, char *buf, size_t size)
{
	...
	/* try to get wp info from cmdline */
	if (sizeof(oled_wp_cmdline) >= sizeof(rx_buf) && sscanf(oled_wp_cmdline, "%02hhx%02hhx%02hhx%02hhx%02hhx%02hhx\n",
			&rx_buf[0], &rx_buf[1], &rx_buf[2], &rx_buf[3],
			&rx_buf[4], &rx_buf[5]) == 6) {
	...
	case TYPE_WHITE_1200:
		...
		for(i = 0; i < temp_val; i++) {
			if(bl_level < lhbm_ratio[i][RATIO_DBV]) {
				gamma_ratio = lhbm_ratio[i][RATIO_1200NIT];
				pr_info("TYPE_WHITE_1200 gamma_ratio:%d\n", gamma_ratio);
				break;
			}
		}

		for(i = 0; i < 3; i++) {
			temp_val = ((lhbm_cmdbuf_1200nit[2*i]<<8) + lhbm_cmdbuf_1200nit[2*i+1]) * gamma_ratio /1000;
			lhbm_normal_white_1200nit[5].para_list[2*i] = (temp_val >> 8) & 0xFF;
			lhbm_normal_white_1200nit[5].para_list[2*i+1] = temp_val &0xFF;
		}
		break;
	case TYPE_WHITE_200:
		temp_val = sizeof(lhbm_ratio) / sizeof(lhbm_ratio[0]);
		for(i = 0; i < temp_val; i++) {
			if(bl_level < lhbm_ratio[i][RATIO_DBV]) {
				gamma_ratio = lhbm_ratio[i][RATIO_250NIT];
				pr_info("TYPE_WHITE_200 gamma_ratio:%d\n", gamma_ratio);
				break;
			}
		}

		for(i = 0; i < 3; i++) {
			temp_val = ((lhbm_cmdbuf_200nit[2*i]<<8) + lhbm_cmdbuf_200nit[2*i+1]) * gamma_ratio /1000;
			lhbm_normal_white_200nit[5].para_list[2*i] = (temp_val >> 8) & 0xFF;
			lhbm_normal_white_200nit[5].para_list[2*i+1] = temp_val &0xFF;
		}
		break;
	case TYPE_LHBM_OFF:
...
static int panel_set_gray_by_temperature (struct drm_panel *panel, int level)
{
	...
	ctx = panel_to_lcm(panel);
	if (!ctx->enabled) {
		ctx->gray_level = temperature;
		ret = -1;
		pr_err("%s: panel isn't enabled\n", __func__);
		goto err;
	}

	if (temperature < CSC_LEVEL_LOW) temperature = CSC_LEVEL_LOW;
	if (temperature > CSC_LEVEL_HEIGHT) temperature = CSC_LEVEL_HEIGHT;
	if (ctx->gray_level == temperature)
		goto err;

	ctx->gray_level = temperature;
	csc_on[CSC_CMD_INDEX].para_list[1] = temperature;
...
static int lcm_probe(struct mipi_dsi_device *dsi)
{
	...
	if (!ret)
		...
		lcm_panel_vci_regulator_deinit();
	}
	...
	if (ret < 0) {
		...
		drm_panel_remove(&ctx->panel);
		lcm_panel_vci_regulator_deinit();
		lcm_panel_vddi_regulator_deinit();
		return ret;
	}
#endif
	this_panel = &ctx->panel;

	pr_info("%s p12a-42-02-0b-dsc-vdo -\n", __func__);
	return ret;
}
...
static void lcm_remove(struct mipi_dsi_device *dsi)
{
	...
	lcm_panel_vddi_regulator_deinit();
	if (ctx->backlight) {
		ctx->backlight->props.brightness = 0;
		ctx->backlight->props.power = FB_BLANK_POWERDOWN;
		backlight_update_status(ctx->backlight);
...
static const struct of_device_id lcm_of_match[] = {
	{ .compatible = "p12a_42_02_0b_dsc_vdo,lcm", },
	{ }
};
...
static struct mipi_dsi_driver lcm_driver = {
	...
	.remove = lcm_remove,
	.driver = {
		.name = "p12a_42_02_0b_dsc_vdo,lcm",
		.owner = THIS_MODULE,
...
MODULE_DESCRIPTION("p12a_42_02_0b_dsc_vdo oled panel driver");
