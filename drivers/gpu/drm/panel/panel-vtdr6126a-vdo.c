...
static struct LCM_setting_table_v2 lcm_initialization_setting_v2_144hz[] = {
	...
	{2,{0x53,0x20}},
	//Normal DBV
	{3,{0x51,0x00,0x00}}, //default 0x00,0x00 by 20241202
	//demura control, 0x00:off, 0x09:on
	{2,{0x59,0x09}}, //default on,0x09 by 20241202
	...
	//RUF ON && RCN OFF
	{3,{0xF0,0xAA,0x18}},
	{2,{0xB0,0x80}}, //need enable first demura control[0x59:0x09]
	{2,{0xB2,0x00}},
	//Optimize the R-angle effect by 20241202 end

	//veas pps
	{95,{0x70,0x12,0x00,0x00,0x89,0x30,0x80,0x0A,0xA0,0x04,0xC8,
	...
	{3,{0xF0,0xAA,0x1C}},
	{19,{0xC0,0x02,0x46,0x02,0x18,0x02,0xB8,0x02,0x46,0x02,0x18,
	...
	{3,{0xF0,0xAA,0x10}},
	{2,{0x65,0x01}},
	{2,{0xD0,0xE8}},  //0xE8: Ramless AOD, 0xE6: DDIC AOD by 20250218
	...
	{12,{0xB0,0x01,0x00,0x0F,0xFF,0x1C,0x33,0x13,0x2C,0x2D,0x2E,0x2F}},  //10%
	{25,{0xB1,0x04,0x1A,0x03,0xEA,0x04,0x2A,0x04,0x15,0x03,0xF7,0x04,
	0x10,0x04,0x45,0x04,0x35,0x04,0x3A,0x04,0x00,0x04,0x00,0x04,0x00}},
	{25,{0xB2,0x04,0x1A,0x03,0xEA,0x04,0x2A,0x04,0x15,0x03,0xF7,0x04,
	0x10,0x04,0x45,0x04,0x35,0x04,0x3A,0x04,0x00,0x04,0x00,0x04,0x00}},
	{25,{0xB3,0x04,0x1A,0x03,0xEA,0x04,0x2A,0x04,0x15,0x03,0xF7,0x04,
	0x10,0x04,0x45,0x04,0x35,0x04,0x3A,0x04,0x00,0x04,0x00,0x04,0x00}},
	{25,{0xB4,0x04,0x1A,0x03,0xEA,0x04,0x2A,0x04,0x15,0x03,0xF7,0x04,
	0x10,0x04,0x45,0x04,0x35,0x04,0x3A,0x04,0x00,0x04,0x00,0x04,0x00}},
...
static struct LCM_setting_table_v2 lcm_initialization_setting_v2_120hz[] = {
	//frame select, 0x00:144Hz, 0x01:120Hz, 0x02:90Hz, 0x03:60Hz
	{2,{0x6C,0x01}},
...
static struct LCM_setting_table_v2 lcm_initialization_setting_v2_90hz[] = {
	//frame select, 0x00:144Hz, 0x01:120Hz, 0x02:90Hz, 0x03:60Hz
	{2,{0x6C,0x02}},
...
static struct LCM_setting_table_v2 lcm_initialization_setting_v2_60hz[] = {
	//frame select, 0x00:144Hz, 0x01:120Hz, 0x02:90Hz, 0x03:60Hz
	{2,{0x6C,0x03}},
	...
	void *handle, unsigned int level, struct mtk_dsi_cmd_option *cmd_opt)
{
	...
	/* need adjust dim speed(0x08 frame) for hbm diming */
	if ((ghbm_state == 2) && (dim_speed_change == 0)) {
		struct mipi_dsi_msg frame_speed_v2_tmp[ARRAY_SIZE(dim_frame_speed_v2)] = { 0 };

		pr_info("%s ghbm_state=%d, dim_speed_change=%d set 0x08.\n", __func__, ghbm_state, dim_speed_change);
		dim_frame_speed_v2[1].para_list[1] = 0x08;
		dim_frame_speed_v2[2].para_list[1] = 0x08;
		dim_frame_speed_v2[3].para_list[1] = 0x08;
		dim_frame_speed_v2[4].para_list[1] = 0x08;

		for (i = 0; i < ARRAY_SIZE(dim_frame_speed_v2); i++) {
			frame_speed_v2_tmp[i].tx_len= dim_frame_speed_v2[i].count;
			frame_speed_v2_tmp[i].tx_buf = dim_frame_speed_v2[i].para_list;
		}
	...
	if ((ghbm_state == 0) && (dim_speed_change == 1)) {
		...
		pr_info("%s ghbm_state=%d, dim_speed_change=%d set 0x18.\n", __func__, ghbm_state, dim_speed_change);
		dim_frame_speed_v2[1].para_list[1] = 0x18;
		dim_frame_speed_v2[2].para_list[1] = 0x18;
		dim_frame_speed_v2[3].para_list[1] = 0x18;
		dim_frame_speed_v2[4].para_list[1] = 0x18;
	...
	if ((g_dim_state == 0) && (g_need_dim_enable == 1)) {
		struct mipi_dsi_msg bl_dim_cmd = { 0 };

		bl_dim[1] = 0x28;
		bl_dim_cmd.tx_buf = bl_dim;
		bl_dim_cmd.tx_len = 2;
	...
	cb(dsi, handle, cmd_opt, &bl_tb0_msg);
	pr_info("[LCM] %s level = %d, mapped_level = %d, dim_state = %d\n", __func__, level, mapped_level, g_dim_state);
	//modify close dim for Sensor on Display 20241212 end
...
int lcm_setbacklight_cmdq_test_v2(void *dsi_drv, mtk_dsi_ddic_cmd cb,
			...
			struct mtk_dsi_cmd_option *cmd_opt)
{
	int ret = 0;
	struct lcm *ctx = panel_to_lcm(panel);
	static struct mipi_dsi_msg init_cmd_144hz[ARRAY_SIZE(lcm_initialization_setting_v2_144hz)] = { 0 };
	static struct mipi_dsi_msg init_cmd_120hz[ARRAY_SIZE(lcm_initialization_setting_v2_120hz)] = { 0 };
	static struct mipi_dsi_msg init_cmd_90hz[ARRAY_SIZE(lcm_initialization_setting_v2_90hz)] = { 0 };
	static struct mipi_dsi_msg init_cmd_60hz[ARRAY_SIZE(lcm_initialization_setting_v2_60hz)] = { 0 };
	static int flag;
	int i;
	...
	if (!flag) {
		flag = 1;
		for (i = 0; i < ARRAY_SIZE(lcm_initialization_setting_v2_144hz); i++) {
			init_cmd_144hz[i].tx_len= lcm_initialization_setting_v2_144hz[i].count;
			init_cmd_144hz[i].tx_buf = lcm_initialization_setting_v2_144hz[i].para_list;
		}

		for (i = 0; i < ARRAY_SIZE(lcm_initialization_setting_v2_120hz); i++) {
			init_cmd_120hz[i].tx_len= lcm_initialization_setting_v2_120hz[i].count;
			init_cmd_120hz[i].tx_buf = lcm_initialization_setting_v2_120hz[i].para_list;
		}

		for (i = 0; i < ARRAY_SIZE(lcm_initialization_setting_v2_90hz); i++) {
			init_cmd_90hz[i].tx_len= lcm_initialization_setting_v2_90hz[i].count;
			init_cmd_90hz[i].tx_buf = lcm_initialization_setting_v2_90hz[i].para_list;
		}

		for (i = 0; i < ARRAY_SIZE(lcm_initialization_setting_v2_60hz); i++) {
			init_cmd_60hz[i].tx_len= lcm_initialization_setting_v2_60hz[i].count;
			init_cmd_60hz[i].tx_buf = lcm_initialization_setting_v2_60hz[i].para_list;
		}
	}

	struct mtk_dsi_cmd_msg cmd_msg_144hz = {
		.is_package = 0,
		.cmd_num = ARRAY_SIZE(lcm_initialization_setting_v2_144hz),
		.transfer_mode = PACKET_LP_MODE,
		.cmd_msg = init_cmd_144hz,
	};

	struct mtk_dsi_cmd_msg cmd_msg_120hz = {
		.is_package = 0,
		.cmd_num = ARRAY_SIZE(lcm_initialization_setting_v2_120hz),
		.transfer_mode = PACKET_LP_MODE,
		.cmd_msg = init_cmd_120hz,
	};

	struct mtk_dsi_cmd_msg cmd_msg_90hz = {
		.is_package = 0,
		.cmd_num = ARRAY_SIZE(lcm_initialization_setting_v2_90hz),
		.transfer_mode = PACKET_LP_MODE,
		.cmd_msg = init_cmd_90hz,
	};

	struct mtk_dsi_cmd_msg cmd_msg_60hz = {
		.is_package = 0,
		.cmd_num = ARRAY_SIZE(lcm_initialization_setting_v2_60hz),
		.transfer_mode = PACKET_LP_MODE,
		.cmd_msg = init_cmd_60hz,
	};

	switch (g_lcm_fresh_mode) {
	case MODE_0_FPS:
		ret = cb(dsi_drv, handle, cmd_opt, &cmd_msg_144hz); //144Hz
		break;
	case MODE_1_FPS:
		ret = cb(dsi_drv, handle, cmd_opt, &cmd_msg_120hz); //120Hz
		break;
	case MODE_2_FPS:
		ret = cb(dsi_drv, handle, cmd_opt, &cmd_msg_90hz); //90Hz
		break;
	case MODE_3_FPS:
		ret = cb(dsi_drv, handle, cmd_opt, &cmd_msg_60hz); //60Hz
		break;
	}
	if (ret < 0) {
		pr_err("%s error, fps=%d\n", __func__, g_lcm_fresh_mode);
		goto end;
	};
			...
			struct mtk_dsi_cmd_option *cmd_opt)
{
	...
	if (!flag) {
		flag = 1;
		for (i = 0; i < ARRAY_SIZE(lcm_suspend_setting_v2); i++) {
			deinit_code[i].tx_len= lcm_suspend_setting_v2[i].count;
			deinit_code[i].tx_buf = lcm_suspend_setting_v2[i].para_list;
		}
	}
	...
	struct mtk_dsi_cmd_msg deinit_cmd = {
		...
		.is_package = 0,
		.rd_to_slot = 0,
		.cmd_num = ARRAY_SIZE(lcm_suspend_setting_v2),
		.transfer_mode = PACKET_LP_MODE,
		.cmd_msg = deinit_code,
	};
...
static struct mtk_panel_funcs ext_funcs = {
	...
	.mtk_dsi_cmd_test_lp_v2 = lcm_setbacklight_cmdq_test_v2,
};
#endif
