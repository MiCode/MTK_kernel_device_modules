...
bool nvt_gesture_lc_90hz_status = false;
void nvt_get_gesture_lc_90hz_status(bool gst_status)
{
	nvt_gesture_lc_90hz_status = gst_status;
	pr_info("[nt36523D] %s gst_status = %d\n", 
		__func__, gst_status);
}
EXPORT_SYMBOL_GPL(nvt_get_gesture_lc_90hz_status);
bool nvt_90hz_lc_proximity_status = false;
void nvt_90hz_lc_proximity_mode(bool proximity_mode)
{
	nvt_90hz_lc_proximity_status = proximity_mode;
	pr_info("[cts] %s proxmity_status = %d\n", __func__, nvt_90hz_lc_proximity_status);
}
EXPORT_SYMBOL_GPL(nvt_90hz_lc_proximity_mode);
...
static int lcm_unprepare(struct drm_panel *panel)
{
	...
	if (nvt_gesture_lc_90hz_status == true && nvt_90hz_lc_proximity_status == false) {
...
*/
	...
	if ((nvt_90hz_lc_proximity_status == false) && (nvt_gesture_lc_90hz_status == false)) {
		lcm_dcs_write_seq_static(ctx, MIPI_DCS_ENTER_SLEEP_MODE);
		lcm_mdelay(100);
		...
		pr_info("[nt36523d] %s nvt_90hz_lc_proximity_status = %d\n",
			__func__, nvt_90hz_lc_proximity_status);
		pr_info("[nt36523d] %s nvt_gesture_lc_90hz_status = %d\n",
			__func__, nvt_gesture_lc_90hz_status);
	}
...
static int lcm_probe(struct mipi_dsi_device *dsi)
{
	...
	if (lcm_name) {
		...
		if (strcmp("dsi_panel_n85_47_02_0b_90_vdo_lcm_drv",videolfb_tag->lcmname) == 0 ){
			pr_info(" find lcm: %s \n",videolfb_tag->lcmname);
		}
		else {
...
static struct mipi_dsi_driver lcm_driver = {
	...
	.remove = lcm_remove,
	.driver = {
		.name = "dsi_panel_n85_47_02_0b_90_vdo",
		.owner = THIS_MODULE,
