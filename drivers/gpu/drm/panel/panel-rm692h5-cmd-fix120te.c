...
#include "panel-rm692h5-cmd-fix120te.h"
		...
		struct mtk_dsi_cmd_option *cmd_opt)
{
	...
	static int flag;
	struct drm_display_mode *m = get_mode_by_id(connector, dst_mode);
	static struct mipi_dsi_msg fps_60hz[ARRAY_SIZE(cmd_set_fps_120hz)] = { 0 };
	...
	if (!flag) {
		...
		for (i = 0; i < ARRAY_SIZE(cmd_set_fps_120hz); i++) {
			fps_60hz[i].tx_len= cmd_set_fps_120hz[i].count;
			fps_60hz[i].tx_buf = cmd_set_fps_120hz[i].para_list;
		}
...
static const struct of_device_id lcm_of_match[] = {
	{ .compatible = "mt6991,rm692h5,cmd,fix120te", },
	{ }
};
...
static struct mipi_dsi_driver lcm_driver = {
	...
	.remove = lcm_remove,
	.driver = {
		.name = "panel-rm692h5-cmd-fix120te",
		.owner = THIS_MODULE,
