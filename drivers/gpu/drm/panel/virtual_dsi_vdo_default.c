...
static struct drm_display_mode default_mode = {
	.clock = 139043,  //h_total * v_total * fps
	.hdisplay = HAC,
	.hsync_start = HAC + HFP,
	...
	void *handle, unsigned int level)
{
    ...
    pr_err("[%s]: larks backlight level is %d \r\n",__func__, level);
	level = level*4;
...
static int lcm_get_modes(struct drm_panel *panel, struct drm_connector *connector)
{
	...
	connector->display_info.width_mm = 133;
...
static int lcm_probe(struct mipi_dsi_device *dsi)
{
	...
	if (lcm_name) {
		...
		if (strcmp("virtual_dsi_vdo_default_drv",videolfb_tag->lcmname) == 0 ){
			pr_info(" find lcm: %s \n",videolfb_tag->lcmname);
		}
		else {
