...
#include "mi_dsi_panel.h"
#include "include/panel-o12-36-02-0b-dsc-cmd.h"
...
#define PANEL_BUILD_ID_P10	0x32
#define PANEL_BUILD_ID_P11	0x43
...
static void lcm_panel_init(struct lcm *ctx)
{
	...
	get_build_id();
	pr_info("%s: init setting, build_id:0x%02x\n", __func__, panel_build_id);
	if (panel_build_id >= PANEL_BUILD_ID_P11) {
		push_table(ctx, init_setting_p11, sizeof(init_setting_p11) / sizeof(struct LCM_setting_table));
	} else {
	...
	if (panel_build_id >= PANEL_BUILD_ID_P20) {
		if (ctx->dynamic_fps == 120) {
			push_table(ctx, mode_120hz_setting_p20,
				sizeof(mode_120hz_setting_p20) / sizeof(struct LCM_setting_table));
		} else if (ctx->dynamic_fps == 60) {
			push_table(ctx, mode_60hz_setting_p20,
				sizeof(mode_60hz_setting_p20) / sizeof(struct LCM_setting_table));
		} else if (ctx->dynamic_fps == 90) {
			push_table(ctx, mode_90hz_setting_p20,
				sizeof(mode_90hz_setting_p20) / sizeof(struct LCM_setting_table));
		} else if (ctx->dynamic_fps == 144) {
			push_table(ctx, mode_144hz_setting_p20,
				sizeof(mode_144hz_setting_p20) / sizeof(struct LCM_setting_table));
		}
	} else if (panel_build_id >= PANEL_BUILD_ID_P11) {
		if (ctx->dynamic_fps == 120) {
...
static struct mtk_panel_params ext_params = {
	...
	.dsc_params = {
		...
		.ext_pps_cfg = {
			.enable = 1,
			.rc_buf_thresh = o12_36_dphy_rc_buf_thresh,
			.range_min_qp = o12_36_dphy_range_min_qp,
			.range_max_qp = o12_36_dphy_range_max_qp,
			.range_bpg_ofs = o12_36_dphy_range_bpg_ofs,
			},
		},
	...
	void *handle, unsigned int level)
{
	...
	char backlight_off_tb[] = {0xA9,0x02,0x01,0xBA,0x09,0x09,0x56,0x02,0x01,0xBA,0x0F,0x0F,0xE6,0x02,0x01,0xBA,0x27,0x27,0x56,0x02,0x01,0xBA,0x2D,0x2D,0xE6,0x02,0x01,0xBB,0x41,0x41,0x56,0x02,0x01,0xBB,0x47,0x47,0xFA,0x02,0x01,0xBB,0x5F,0x5F,0x56,0x02,0x01,0xBB,0x65,0x65,0xFA,0x01,0x00,0x51,0x00,0x01,0x00,0x00};
...
static const struct of_device_id lcm_of_match[] = {
	{ .compatible = "o12_36_02_0b_dsc_cmd,lcm", },
	{ }
};
...
MODULE_DESCRIPTION("o12_36_02_0b_dsc_cmd oled panel driver");
