...
void arm_smmu_get_ste_used(const __le64 *ent, __le64 *used_bits)
{
	...
	/* S1 translates or Mediatek dynamic cfg bypass */
	if (cfg & BIT(0) || (ent[0] & cpu_to_le64(STRTAB_STE_0_S1CTXPTR_MASK))) {
		...
		used_bits[1] |=
			cpu_to_le64(STRTAB_STE_1_S1DSS | STRTAB_STE_1_S1CIR |
				    ...
				    STRTAB_STE_1_EATS | STRTAB_STE_1_TCU_PF);
			     ...
			     const struct arm_smmu_cd *target)
{
	...
	dev_info(master->smmu->dev,
		 "[%s] ssid:%u, cd0:0x%lx, cd1:0x%lx, cd2:0x%lx, cd3:0x%lx\n",
		 __func__, ssid, (unsigned long)target->data[0], (unsigned long)target->data[1],
		 (unsigned long)target->data[2], (unsigned long)target->data[3]);
...
void arm_smmu_ste_writer_sync_entry(struct arm_smmu_entry_writer *writer)
{
			...
			const struct arm_smmu_ste *target)
{
	...
	dev_info(smmu->dev,
		 "[%s] sid:%u, ste0:0x%lx, ste1:0x%lx, ste2:0x%lx, ste3:0x%lx, ste4:0x%lx\n",
		 __func__, sid, (unsigned long)target->data[0], (unsigned long)target->data[1],
		 (unsigned long)target->data[2], (unsigned long)target->data[3],
		 (unsigned long)target->data[4]);
}
					 ...
					 struct arm_smmu_ste *target)
{
	...
	for (i = 0; i < master->num_streams; ++i) {
		...
		if (smmu->impl && smmu->impl->smmu_setup_features)
			smmu->impl->smmu_setup_features(master, sid, target->data);
...
static int arm_smmu_attach_dev(struct iommu_domain *domain, struct device *dev)
{
	...
	if (!smmu_domain->smmu) {
		...
		dev_info(smmu->dev,
			 "[%s] dom_fin:%d, dev:%s, type:%d, feats:0x%x, bypass:%d, S1:%d, S2:%d\n",
			 __func__, ret, dev_name(master->dev),
			 domain->type, smmu->features,
...
static int arm_smmu_iotlb_sync_map(struct iommu_domain *domain, unsigned long iova,
				    size_t size)
{
	struct arm_smmu_domain *smmu_domain = to_smmu_domain(domain);
	struct arm_smmu_device *smmu = smmu_domain->smmu;
