				   ...
				   ioasid_t ssid)
{
	...
	if (ret) {
		dev_dbg(master->smmu->dev, "[%s] power_status:%d\n", __func__,
			ret);
		return ret;
	}
...
static int arm_smmu_init_structures(struct arm_smmu_device *smmu)
{
	...
	if (!(smmu->features & ARM_SMMU_FEAT_2_LVL_STRTAB))
		arm_smmu_init_initial_stes(smmu, smmu->strtab_cfg.linear.table,
...
int arm_smmu_trigger_irq(struct arm_smmu_device *smmu)
{
	...
	return arm_smmu_cmdq_issue_cmdlist(smmu, arm_smmu_get_cmdq(smmu, NULL),
					   cmd, 1, false);
}
