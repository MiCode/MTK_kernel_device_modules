...
int cs_dsp_load_then_verify_coeff(struct cs_dsp *dsp, const struct firmware *firmware,
			     const char *file, bool *success)
{
	LIST_HEAD(buf_list);
	struct regmap *regmap = dsp->regmap;
	...
	struct verify_info *verify_infos = NULL;
	int verify_count = 0;
	...
	while (pos < firmware->size &&
		...
		if (reg) {
			...
			// dynamically increment array to save the written address and data
			verify_infos = krealloc(verify_infos, (verify_count + 1) * sizeof(struct verify_info), GFP_KERNEL);
			if (!verify_infos) {
				cs_dsp_err(dsp, "Failed to allocate verify info\n");
				ret = -ENOMEM;
				goto out_fw;
			}

			verify_infos[verify_count].reg = reg;
			verify_infos[verify_count].buf = buf->buf;
			verify_infos[verify_count].len = le32_to_cpu(blk->len);
			verify_count++;
		}
	...
	// make sure the coeffecient completely writing to DSP
	ret = regmap_async_complete(regmap);
	if (ret != 0) {
		cs_dsp_err(dsp, "Failed to complete async write: %d\n", ret);
		goto free_verify_infos;
	}
	if (pos > firmware->size)
		cs_dsp_warn(dsp, "%s.%d: %zu bytes at end of file\n",
	...
	// begin verify process
	for (int i = 0; i < verify_count; i++) {
		cs_dsp_dbg(dsp, "%s: Verifying block %d at reg 0x%x, len %zu\n",
			   file, i, verify_infos[i].reg, verify_infos[i].len);

		read_size = verify_infos[i].len;
		read_buf = kzalloc(read_size, GFP_KERNEL);
		if (!read_buf) {
			cs_dsp_err(dsp, "Failed to allocate read buffer\n");
			ret = -ENOMEM;
			goto free_verify_infos;
		}

		// read back each block data
		ret = regmap_raw_read(regmap, verify_infos[i].reg, read_buf, read_size);
		if (ret != 0) {
			cs_dsp_err(dsp, "%s: Failed to read back from 0x%x: %d\n",
				   file, verify_infos[i].reg, ret);
			kfree(read_buf);
			read_buf = NULL;
			// next block
			continue;
		...
		// compare read back data with original data
		if (memcmp(verify_infos[i].buf, read_buf, read_size) != 0) {
			cs_dsp_err(dsp, "%s: Data verification failed at reg 0x%x\n",
				   file, verify_infos[i].reg);
			load_success = false;
		} else {
			cs_dsp_dbg(dsp, "%s: Data verification passed at reg 0x%x\n",
				   file, verify_infos[i].reg);
		}
...
EXPORT_SYMBOL_GPL(cs_dsp_load_then_verify_coeff);
