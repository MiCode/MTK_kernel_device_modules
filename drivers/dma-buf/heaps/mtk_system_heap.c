...
// extern struct mtk_dmabuf_page_pool *system_reserve_pools[NUM_ORDERS];

#define DMABUF_POOL_FILL_MARK (100 * SZ_1M)
#define DMABUF_POOL_LOW_MARK_PERCENT 40UL
#define DMABUF_POOL_LOW_MARK ((DMABUF_POOL_FILL_MARK * DMABUF_POOL_LOW_MARK_PERCENT) / 100) // 40M

#define DMABUF_POOL_REFILL_DEFER_WINDOW_MS 10
#define DMABUF_POOL_KTHREAD_NICE_VAL 10
#endif
...
// 这个wmark是否需要调整一下？
/*
static int mtk_dmabuf_pool_count_lowmark(struct mtk_dmabuf_page_pool *pool) {
        return DMABUF_POOL_LOW_MARK / (PAGE_SIZE << pool->order);
}

static bool mtk_dmabuf_pool_count_below_lowmark(struct mtk_dmabuf_page_pool *pool) {
        int count;
        unsigned long flags;
        ...
        return count < mtk_dmabuf_pool_count_lowmark(pool);
}
*/
static int get_mtk_dmabuf_pool_fillmark(struct mtk_dmabuf_page_pool *pool)
{
        return DMABUF_POOL_FILL_MARK / (PAGE_SIZE << pool->order);
}

static bool mtk_dmabuf_pool_fillmark_reached(struct mtk_dmabuf_page_pool *pool) {
        int count;
	...
	spin_unlock(&pool->lock);
        ...
        return count >= get_mtk_dmabuf_pool_fillmark(pool);
}
#endif
...
#if IS_ENABLED(CONFIG_MTK_DMABUF_HEAPS_RESERVE_POOL)
                        if (mtk_dmabuf_pool_fillmark_reached(page_pools[j]) &&
                                        need_free_to_reserve_pool(j))
                                mtk_dmabuf_page_pool_free(system_reserve_pools[j], page);
                        else
					    ...
					    struct mtk_dmabuf_page_pool **page_pools)
{
	...
	for (i = 0; i < NUM_ORDERS; i++) {
		...
		if (likely(page_pools)) {
			...
			if (!page)
				continue;
			//use pool_refill_low_water instead of dynamic_pool_count_below_lowmark
			if (page_pools && page_pools[i]->refill_kthread &&
				!page_pools[i]->refilling && page_pools[i]->need_refill &&
				page_pools[i]->need_refill(page_pools[i]))	//pool_refill_low_water
		...
		if (fatal_signal_pending(current)) {
			...
			goto free_buffer;
                ...
                if (reserve_extend_info.use_reserve_pool)
                        page = mtk_extend_sys_heap_alloc_largest_available(page_pools,
			   ...
			   bool uncached, bool pool)
{
	...
	if (!strcmp(name, "system")) {
		pr_info("%s: create reserve pool for system_heap", __func__);
#if IS_ENABLED(CONFIG_MTK_DMABUF_HEAPS_RESERVE_POOL)
		mtk_sys_heap_reserve_pool_init(heap_priv->page_pools);
#endif
	}
