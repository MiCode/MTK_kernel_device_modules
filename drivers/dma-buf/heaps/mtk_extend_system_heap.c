...
bool reserve_pool_enable;
EXPORT_SYMBOL_GPL(reserve_pool_enable);
struct mtk_dmabuf_page_pool *system_reserve_pools[NUM_ORDERS];
EXPORT_SYMBOL_GPL(system_reserve_pools);
...
static struct mtk_dmabuf_page_pool **config_show_pool[CONFIG_SHOW_MAX];
...
static bool reserve_pool_empty(void) {
	...
	for (i = 0; i < NUM_ORDERS; i++) {
		if (system_reserve_pools[i] == NULL) {
...
static int get_dynamic_pool_fillmark(struct mtk_dmabuf_page_pool *pool)
{
	return atomic_read(&reserve_extend_info
		.reserve_order[order_to_index(pool->order)]);
}
static bool dynamic_pool_fillmark_reached(struct mtk_dmabuf_page_pool *pool)
{
	int count;
...
static int get_dynamic_pool_lowmark(struct mtk_dmabuf_page_pool *pool)
{
	return (atomic_read(&reserve_extend_info
			.reserve_order[order_to_index(pool->order)]) *
			DYNAMIC_POOL_LOW_MARK_PERCENT) / 100;
}
static bool dynamic_pool_count_below_lowmark(struct mtk_dmabuf_page_pool *pool)
{
	int count;
...
static bool dynamic_pool_refill_ok(struct mtk_dmabuf_page_pool *pool)
{
	int i, mark;
...
static void mtk_dmabuf_page_pool_refill(struct mtk_dmabuf_page_pool *pool)
{
...
static int reserve_pool_refill_kthread(void *data)
{
	struct mtk_dmabuf_page_pool **pool_list = data;
	int i;
	for (;;) {
		for (i = 0; i < NUM_ORDERS; i++) {
			if (dynamic_pool_count_below_lowmark(pool_list[i])) {
				mtk_dmabuf_page_pool_refill(pool_list[i]);
...
bool need_free_to_reserve_pool(int order_index)
{
	if (reserve_extend_info.use_reserve_pool &&
			!dynamic_pool_fillmark_reached(system_reserve_pools[order_index])) {
		if (reserve_extend_info.debug & BACK_TO_RESERVE_POOL)
			pr_info("%s order[%d %d]", __func__, order_index,
					system_reserve_pools[order_index]->order);
		return true;
	} else
		return false;
}
EXPORT_SYMBOL_GPL(need_free_to_reserve_pool);
static void reserve_page_pool_free(struct mtk_dmabuf_page_pool **reserve_pool)
{
	int i;
		...
		struct kobj_attribute *attr, const char *buf, size_t len)
{
	...
	if (value) {
		...
		reserve_page_pool_free(system_reserve_pools);
	}
	return len;
}
...
static bool update_pool_pid(int new_pid)
{
	...
	for (i = 0; i < POOL_PROCESS_NUMBER; i++) {
		...
		if (pid < 0 || !find_task_by_vpid(pid)) {
			rcu_read_unlock();
			pr_info("pid update %d -> %d", pool_pid[i], new_pid);
			pool_pid[i] = new_pid;
			break;
		}
		rcu_read_unlock();
	}
	if (i == POOL_PROCESS_NUMBER) {
		pr_info("no find index for %d", new_pid);
		return false;
	}
...
static void clear_pid_from_pool_pid(void *unused1, void *unused2)
{
	int i;
	if (!reserve_extend_info.use_reserve_pool ||
			!(current->group_leader->flags & PF_EXITING)) {
		return;
	}
	for (i = 0; i < POOL_PROCESS_NUMBER; i++) {
		int pid;
		pid = pool_pid[i];
		if (pid == current->group_leader->pid) {
			pr_info("pid %d from list", pool_pid[i]);
			pool_pid[i] = -1;
			break;
		}
	}
...
static void reserve_extend_info_init(struct mtk_dmabuf_page_pool **global_pools)
{
	reserve_extend_info.debug = false;
	reserve_config_init(g_reserve_config);
	config_show_pool[SOURCE_POOL_FREE_ORDER] = global_pools;
	config_show_pool[RESERVE_POOL_FREE_ORDER] = system_reserve_pools;
	...
	register_trace_android_vh_mmput(clear_pid_from_pool_pid, NULL);
}
...
void mtk_sys_heap_reserve_pool_init(struct mtk_dmabuf_page_pool **global_pools)
{
	...
	set_mem_type_index();
	for (i = 0; i < NUM_ORDERS; i++) {
		system_reserve_pools[i] = mtk_dmabuf_page_pool_create(order_flags[i], orders[i]);

		if (IS_ERR_OR_NULL(system_reserve_pools[i])) {
			int j;
			pr_err("%s: page pool creation failed for the order %u pool!\n",
				__func__, orders[i]);
			for (j = 0; j < i; j++)
				mtk_dmabuf_page_pool_destroy(system_reserve_pools[j]);
		} else {
			pr_info("%s: order<%d> reserve page pool create success!\n", __func__
				, orders[i]);
			system_reserve_pools[i]->reserve_pool = true;
			system_reserve_pools[i]->order_index = orders[i];
			system_reserve_pools[i]->pool_list = system_reserve_pools;
		}
	}
	...
	if (IS_ENABLED(CONFIG_MTK_DMABUF_HEAPS_RESERVE_POOL)) {
		refill_kthread = kthread_run(reserve_pool_refill_kthread,
				system_reserve_pools, "reserve-refill");
		if (IS_ERR(refill_kthread)) {
			pr_err("%s: failed to create reserve-refill: %ld\n",
			       __func__, PTR_ERR(refill_kthread));
		} else {
			ret = sched_setattr(refill_kthread, &attr);
			if (ret) {
				pr_warn("%s: failed to set task priority for "
					"ret = %d\n",
					__func__, ret);
				kthread_stop(refill_kthread);
			} else {
				int i;
				for (i = 0; i < NUM_ORDERS; i++)
					system_reserve_pools[i]->refill_kthread = refill_kthread;
			}
		}
	}
}
EXPORT_SYMBOL_GPL(mtk_sys_heap_reserve_pool_init);

static bool is_server_app(struct mtk_dmabuf_page_pool **reserve_pools)
{
...
static inline void get_pool_count(struct mtk_dmabuf_page_pool *spool,
		struct mtk_dmabuf_page_pool *reserve_pool, int *spool_high_count,
...
static inline struct page *get_page_from_pool(struct mtk_dmabuf_page_pool *pool)
{
...
static inline struct page *get_page_from_buddy(bool served_app, struct mtk_dmabuf_page_pool *spool,
						struct mtk_dmabuf_page_pool *reserve_pool)
{
	s64 delta;
...
static inline void wakeup_refill_thread_by_wmark(bool served_app, struct mtk_dmabuf_page_pool *spool,
		struct mtk_dmabuf_page_pool *reserve_pool)
{
	if (spool->refill_kthread != NULL && spool->order && dynamic_pool_count_below_lowmark(spool))
	{
		if (reserve_extend_info.debug & RESERVE_POOL)
			pr_info("system_pool_lowmark: current_proc-order<%d, %d> wakeup"
				"system_pool-2\n", current->pid, spool->order);
		wake_up_process(spool->refill_kthread);
	}
	if (served_app && reserve_pool != NULL && reserve_pool->refill_kthread != NULL)
	{
		if (IS_ENABLED(CONFIG_MTK_DMABUF_HEAPS_RESERVE_POOL) &&
				reserve_pool->order &&
				dynamic_pool_count_below_lowmark(reserve_pool)) {
			if (reserve_extend_info.debug & RESERVE_POOL)
				pr_info("reserve_pool_lowmark: current_proc-order<%d, %d> wakeup"
					"reserve_pool-1\n", current->pid, reserve_pool->order);
			wake_up_process(reserve_pool->refill_kthread);
		}
	}
}
struct page *mtk_extend_sys_heap_alloc_largest_available(struct mtk_dmabuf_page_pool **global_pools,
						unsigned long size, unsigned int max_order)
{
	struct page *page = NULL;
	struct mtk_dmabuf_page_pool **g_pools = global_pools;
	struct mtk_dmabuf_page_pool **reserve_pools = system_reserve_pools;
