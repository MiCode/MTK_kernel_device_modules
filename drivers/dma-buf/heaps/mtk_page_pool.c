...
unsigned int orders[] = {8, 4, 0};
gfp_t order_flags[] = {HIGH_ORDER_GFP, HIGH_ORDER_GFP, LOW_ORDER_GFP};

static LIST_HEAD(pool_list);
static DEFINE_MUTEX(pool_list_lock);

u64 totalram;
EXPORT_SYMBOL_GPL(totalram);
...
EXPORT_SYMBOL_GPL(mtk_dmabuf_page_pool_remove);
...
inline bool can_do_shrink(struct mtk_dmabuf_page_pool *pool, bool high)
{
        if (!pool->reserve_pool && HIGH_MEM_DEVICE && reserve_pool_enable) {
                if (pool->order == 0) {
                        if (mtk_dmabuf_page_pool_total(pool, high) > ORDER_ZERO_PAGES)
                                return true;
                        else
                                return false;
                } else if (pool->order == 4) {
                        if (mtk_dmabuf_page_pool_total(pool, high) > ORDER_FOUR_PAGES)
                                return true;
                        else
                                return false;
                } else {
