...
static struct intr_flag cp_intr_flag[] = {
    ...
    { .reg = 0x17, .len = 8, .bit = {
                ...
                {.mask = BIT(4), .name = "ibus ocp flag", .notify = SC853X_NOTIFY_IBUSOCP},
                {.mask = BIT(3), .name = "ibat ocp flag", .notify = SC853X_NOTIFY_IBATOCP},
                {.mask = BIT(2), .name = "vbus ovp flag", .notify = SC853X_NOTIFY_VBUSOVP},
                {.mask = BIT(1), .name = "vout ovp flag", .notify = SC853X_NOTIFY_VOUTOVP},
                {.mask = BIT(0), .name = "vbat ovp flag", .notify =  SC853X_NOTIFY_VBATOVP},
                },
    },
    { .reg = 0x18, .len = 6, .bit = {
                {.mask = BIT(5), .name = "pmid2out uvp flag", .notify = SC853X_NOTIFY_OTHER},
                {.mask = BIT(4), .name = "pmid2out ovp flag", .notify =  SC853X_NOTIFY_VBUSOVP},
...
static int ops_sc8531_enable_qb(struct charger_device *chg_dev, bool enable)
{
  	pr_err("%s: qb enable is chg enable = %d\n", __func__, enable);
  	return 0;
}

static int sc8531_disable_ibus_ucp(struct sc853x_chip *sc, bool en)
{
    int ret = 0;

    ret = sc853x_field_write(sc, IBUS_UCP_DIS, en);
    if (ret < 0)
        return ret;
...
static int sc8531_disable_vac_ovp(struct sc853x_chip *sc, bool en)
{
    int ret = 0;

    ret = sc853x_field_write(sc, VAC_OVP_DIS, en);
    if (ret < 0)
        return ret;
    pr_err("%s %s:%d\n", sc->log_tag, __func__, en);
...
static int sc8531_disable_vbus2out_uvp(struct sc853x_chip *sc, bool en)
{
    int ret = 0;

    ret = sc853x_field_write(sc, PMID2OUT_UVP_DIS, en);
    if (ret < 0)
        return ret;
    pr_err("%s %s:%d\n", sc->log_tag, __func__, en);
...
static int sc_sc8531_rev_chg_config(struct charger_device *chg_dev, bool enable)
{
    struct sc853x_chip *sc = charger_get_data(chg_dev);
    int ret = 0;

    pr_err("%s: enable:%d\n", __func__, enable);
    ret |= sc8531_disable_ibus_ucp(sc, !!enable);
    ret |= sc8531_disable_vac_ovp(sc, !!enable);
    ret |= sc8531_disable_vbus2out_uvp(sc, !!enable);

    if (ret != 0) {
        pr_err("%s failed %s ibus/vbus/uvp protection\n", __func__, enable ? "disable" : "enable");
    } else {
        pr_err("%s success %s ibus/vbus/uvp protection\n", __func__, enable ? "disable" : "enable");
    }
...
static int sc8561_set_ss_timeout(struct sc853x_chip *sc, int timeout)
{
	int ret = 0;
	u8 val;
	switch (timeout) {
    ...
    ret = sc853x_field_write(sc, SS_TIMEOUT, val);
    if (ret)    
...
static int ops_cp_kick_wdt(struct charger_device *chg_dev)
{
	struct sc853x_chip *sc = charger_get_data(chg_dev);
	int ret = 0;
	u32 vbus_val = 0;

	/* Feed watchdog by reading VBUS ADC */
	ret = sc853x_get_adc_data(sc, ADC_VBUS, &vbus_val);
	if (ret)
		pr_err("%s failed to kick wdt by reading vbus\n", sc->log_tag);
	else
		pr_err("%s kick wdt success, vbus=%d\n", sc->log_tag, vbus_val);

	return ret;
}

static int ops_cp_enable_wdt(struct charger_device *chg_dev, bool enable)
{
	struct sc853x_chip *sc = charger_get_data(chg_dev);
	int ret = 0;

	if (enable) {
		/* Set watchdog timeout to 5s (value 2) */
		ret = sc853x_field_write(sc, WD_TIMEOUT, 2);
		if (ret) {
			pr_err("%s failed to set wdt timeout\n", sc->log_tag);
			return ret;
		}
		/* Enable watchdog (WD_TIMEOUT_DIS = 0) */
		ret = sc853x_field_write(sc, WD_TIMEOUT_DIS, 0);
		if (ret) {
			pr_err("%s failed to enable wdt\n", sc->log_tag);
			return ret;
		}
		pr_err("%s wdt enabled, timeout=5s\n", sc->log_tag);
	} else {
		/* Disable watchdog (WD_TIMEOUT_DIS = 1) */
		ret = sc853x_field_write(sc, WD_TIMEOUT_DIS, 1);
		if (ret) {
			pr_err("%s failed to disable wdt\n", sc->log_tag);
			return ret;
		}
...
static const struct charger_ops sc853x_chg_ops = {
	...
	.cp_set_qb = ops_sc8531_enable_qb,
	...
	.cp_disable_pmid2out_uvp = NULL,
	.cp_set_vout2out_uvp_high = NULL,
 	.cp_rev_chg_config = sc_sc8531_rev_chg_config,
	.cp_enable_wdt = ops_cp_enable_wdt,
	.kick_wdt = ops_cp_kick_wdt,
};
            ...
            if (cp_intr_flag[k].reg == i){
                for (j=0; j <  cp_intr_flag[k].len; j++) {
                    if (flag & cp_intr_flag[k].bit[j].mask) {
                        sc->fault_type |= BIT(cp_intr_flag[k].bit[j].notify);
