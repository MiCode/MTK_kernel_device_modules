...
static const struct mt6375_chg_range mt6375_chg_ranges[F_MAX] = {
	...
	[F_IEOC] = MT6375_CHG_RANGE(100, 500, 50, 1, false),
...
static unsigned long long calculate_r_ntc(unsigned long long v_in,
				unsigned int pullup_r, unsigned int pullup_v)
{
	unsigned long long r_ntc;
	if (v_in >= pullup_v)
		return 0;
	r_ntc = (unsigned long long)(v_in * pullup_r / (pullup_v - v_in));
	return r_ntc;
}
static int typec_ntc_r_to_temp(unsigned long long res)
{
	...
	delta_temp = (((res - ts_ntc_table[upper].res) * 1000) / (ts_ntc_table[lower].res - ts_ntc_table[upper].res)) ;
 
	pr_err("%s delta_temp = %d, lower_res = %d, upper_res = %d, curr_res = %lld",
		__func__, delta_temp, ts_ntc_table[lower].res, ts_ntc_table[upper].res, res);

	if(ts_ntc_table[lower].temp < 0)
		temp = ts_ntc_table[lower].temp - delta_temp;
	else
		temp = ts_ntc_table[lower].temp + delta_temp;
...
static int mt6375_get_typec_ntc1_temp(struct charger_device *chgdev, int *temp)
{
	...
	int ret = 0;
	unsigned long long v_in = 0;
	unsigned long long r_ntc = 0;
	unsigned int val = 0;

	ret = iio_read_channel_raw(&ddata->iio_adcs[AUXADC_VIN2], &val);
	...
	*temp = typec_ntc_r_to_temp(r_ntc) / 100;
	...
	if (*temp >= TYPEC_NTC_MAX_TEMP) {
		pr_err("%s typec ntc temp:%d, FAC not support\n", __func__, *temp);
		*temp = 250;
	}
#endif

	pr_err("%s val=%d, vin=%lld, r_ntc=%lld, typec_ntc1_temp=%d\n", __func__, (int)val, v_in, r_ntc, *temp);
	return 0;
}
static int mt6375_get_typec_ntc2_temp(struct charger_device *chgdev, int *temp)
{
	...
	ret = iio_read_channel_raw(&ddata->iio_adcs[AUXADC_VIN3], &val);
	v_in = typec_ntc_adc2volt(get_adc_data(val, 14));
	r_ntc = calculate_r_ntc(v_in, TS_RP, TS_VREF);
	*temp = typec_ntc_r_to_temp(r_ntc) / 100;
	...
	pr_err("%s val=%d, vin=%lld, r_ntc=%lld, typec_ntc2_temp=%d\n", __func__, (int)val, v_in, r_ntc, *temp);
	return 0;
}
...
static void mt6375_set_dpdm_voltage(struct mt6375_chg_data *ddata, int vdp, int vdm)
{
	...
	dev_info(ddata->dev, "%s: vdp:%dmv vdm:%dmv!\n", __func__, vdp, vdm);
...
static void mt6375_get_dpdm_voltage(struct mt6375_chg_data *ddata, int *dp, int *dm)
{
	...
	dev_info(ddata->dev, "%s: dp:%dmv dm:%dmv!\n", __func__, U_TO_M(*dp), U_TO_M(*dm));
}
					  ...
					  int attach)
{
	...
	if (!ddata->mtk_charger)
		ddata->mtk_charger = get_charger_by_name("mtk_charger");
	charger_dev_usb_get_property(ddata->mtk_charger, USB_PROP_TYPEC_MODE, &typec_mode);
...
static void mt6375_detect_hvchg_work(struct work_struct *work)
{
	...
	if (vbus < HVDCP_VBUS_LIMIT) {
		...
		if (info) {
			if (!ddata->mtk_charger)
				ddata->mtk_charger = get_charger_by_name("mtk_charger");
			charger_dev_usb_get_property(ddata->mtk_charger, USB_PROP_QUICK_CHARGE_TYPE, &intval);
				   ...
				   union power_supply_propval *val)
{
	...
	struct power_supply *mt_chg_psy = NULL;
	...
	mt_chg_psy = power_supply_get_by_name("mtk-master-charger");
	if (IS_ERR_OR_NULL(mt_chg_psy))
		return PTR_ERR(mt_chg_psy);

	info = (struct mtk_charger *)power_supply_get_drvdata(mt_chg_psy);
	if (IS_ERR_OR_NULL(info)) {
		dev_err(ddata->dev, "[%s]info is_err_or_null\n", __func__);
		return PTR_ERR(info);
	}
	...
	if (IS_ERR_OR_NULL(info->tcpc)) {
		...
		if (IS_ERR_OR_NULL(info->tcpc)) {
			dev_err(ddata->dev, "[%s]info->tcpc is_err_or_null\n", __func__);
			return PTR_ERR(info->tcpc);
		}
	}
	...
	case POWER_SUPPLY_PROP_ONLINE:
		if (info->pd_type != MTK_PD_CONNECT_PE_READY_SNK_APDO &&
				info->tcpc->adapt_pid == 0xf663 &&
				info->tcpc->adapt_vid == 0x2b01) {
			dev_err(ddata->dev, "[%s]is ZIMI ADAPTER\n", __func__);
			msleep(1000);
		}
		val->intval = atomic_read(&ddata->attach[ddata->active_idx]);
		online_status = val->intval;
		if (!val->intval) {
			ret = charger_dev_usb_get_property(ddata->mtk_charger, USB_PROP_CP_SM_RUN_STATE, &val->intval);
			if (ret < 0) {
				   ...
				   const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_INPUT_VOLTAGE_LIMIT:
		...
		vbus = vbus / 1000;
		mutex_lock(&ddata->pe_lock);
		if (vbus > HVDCP_VBUS_LIMIT && (ddata->psy_desc.type == POWER_SUPPLY_TYPE_USB_ACA))
...
static int mt6375_set_mivr(struct charger_device *chgdev, u32 uV)
{
	...
	if (val.intval < 5000) {
		ret = iio_read_channel_processed(&ddata->iio_adcs[ADC_CHAN_VBAT], &vbat);
		if (ret < 0) {
			dev_err(ddata->dev, "failed to read vbat\n");
			return ret;
		}
		vbat = vbat / 1000;
		if (vbat + 150 >= 4600)
			val.intval = 4700;
		mt_dbg(ddata->dev, "mivr=%duV, real mivr=%dmV, vbat=%dmV\n", uV, val.intval, vbat);
	} else
		mt_dbg(ddata->dev, "mivr=%duV\n", uV);
...
static void mt6375_abnormal_charging_judge(struct mt6375_chg_data *ddata, u32 *data)
{
	...
	if((data[1] & 0x80) == 0)
	{
		...
		ddata->chgdev->noti.vbusbad_stat = true;
	} else
		ddata->chgdev->noti.vbusbad_stat = false;
...
static int mt6375_dump_registers(struct charger_device *chgdev)
{
	...
	mt6375_chg_field_get(ddata, F_ST_PWR_RDY, &val);
...
static irqreturn_t mt6375_fl_pwr_rdy_handler(int irq, void *data)
{
	...
	if (!ddata->mtk_charger)
		ddata->mtk_charger = get_charger_by_name("mtk_charger");
	charger_dev_usb_get_property(ddata->mtk_charger, USB_PROP_CHARGE_FULL, &is_charge_full);
	if(is_charge_full)
...
static void shipmode_syscore_shutdown(void)
{
	...
	ddata = (struct mt6375_chg_data *)power_supply_get_drvdata(chg_psy);
	...
	if (!info->shipmode_flag) {
		dev_info(ddata->dev, "shipmode_flag == false\n");
		return;
	}

	ret = mt6375_set_shipping_mode(ddata);
	if (ret < 0)
		dev_err(ddata->dev, "set ship mode fail\n");
	else
		dev_info(ddata->dev, "set ship mode success\n");
