...
static void monitor_sw_cv(struct mtk_charger *info)
{
	...
	if (info->step_chg_index[0] == 1 && info->step_chg_index[1] == 0 && info->cycle_count >= 0) {
		info->sw_cv = 0;
...
static void monitor_usb_otg_burn(struct work_struct *work)
{
	...
	if (bcl_level == 1 && info->chg1_dev) {
		charger_dev_set_boost_current_limit(info->chg1_dev, 500000);
		mca_log_err("limit otg current bcl_level=%d\n", bcl_level);
	}
...
static int check_exit_ffc_mode(struct mtk_charger *info)
{
	...
	if (is_between(info->ffc_warm_tbat, info->ffc_little_high_tbat, info->temp_now)) {
		...
		normal_vterm = info->fv_normal_high;
	} else {
...
static int handle_ffc_charge(struct mtk_charger *info)
{
	...
	} else if (info->pmic_ffc_forbid && !info->cp_sm_run_state) {
		info->ffc_enable = false;
...
void reset_step_jeita_charge(struct mtk_charger *info)
{
	...
	if (info->step_chg_index[0] == 0 && info->cycle_count >= 0 &&
		...
		info->step_chg_index[0] = 1;
		info->step_chg_index[1] = 0;
		chr_err("max charge current limit time=%lld,%lld", info->fs_start_time, delta_time);
	}
	if (info->step_normal_chg && !info->ffc_enable) {
		info->step_chg_fcc = info->step_normal_chg_cfg[info->step_chg_index[0]].value;
	} else {
