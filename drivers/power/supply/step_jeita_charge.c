...
static void monitor_sw_cv(struct mtk_charger *info)
{
	...
	if (info->step_chg_index[0] == 1 && info->step_chg_index[1] == 0 && info->cycle_count > 30) {
		info->sw_cv = 0;
...
static void handle_step_charge(struct mtk_charger *info)
{
	...
	if (current_state != info->ffc_temp_state) {
		...
		if (info->step_chg_index[0] == 0 && info->cycle_count > 30 &&
...
static void battery_check_recharge(struct mtk_charger *info)
{
	...
	if (info->charge_full) {
		...
		bms_get_property(BMS_PROP_CAPACITY_RAW, &raw_soc);
#endif
		if ((((recharge_count >= 15) || (raw_soc < recharge_soc) || !info->real_full) && (info->jeita_chg_index[0] <= 4)) || recharge_time_flag) {
...
static void battery_check_voltage_diff(struct mtk_charger *info)
{
	...
	if((rsoc_diff >= 8 || vbat_diff > 400) && ((batt_ctl_gpio == 1) || (batt_ctl_gpio_slave == 1))){
...
static void reset_dfx_cyclial_report(struct mtk_charger *info)
{
	...
	if (((info->dfx_fake_report_type & BIT_MASK(CHG_DFX_VOLTAGE_DIFFERENCE)) || !(info->dfx_cyclial_report_type & BIT_MASK(CHG_DFX_VOLTAGE_DIFFERENCE)))) {
