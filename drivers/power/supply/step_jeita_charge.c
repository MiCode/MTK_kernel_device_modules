...
static void get_index(struct step_jeita_cfg0 *cfg, int fallback_hyst, int forward_hyst, int value, int *index, bool ignore_hyst)
{
	...
	chr_err("%s: value = %d, index[0] = %d, index[1] = %d, fallback_hyst = %d, forward_hyst = %d\n", __func__, value, index[0], index[1], fallback_hyst, forward_hyst);
...
static void monitor_sw_cv(struct mtk_charger *info)
{
	...
	if (info->step_chg_index[0] == 1 && info->step_chg_index[1] == 0 && info->cycle_count >= start_cycle_count) {
		info->sw_cv = 0;
	...
	if (info->sw_cv || info->suspend_recovery) {
		...
		vote(info->fcc_votable, STEP_CHARGE_VOTER, true, info->step_chg_fcc);
		chr_err("not in cv, vote step_chg_fcc:%d\n", info->step_chg_fcc);
	}
}
...
static void monitor_usb_otg_burn(struct work_struct *work)
{
	...
	if (IS_ERR_OR_NULL(info->bms_psy)) {
		chr_err("bms_psy invalid, use default soc/temp\n");
		info->soc = 15;
		info->temp_now = 150;
	} else {
	...
	if ((info->temp_now < 0 && info->soc <= 10) ||
		(info->temp_now < -100 && info->soc <= 25) ||
		(info->temp_now < -200 && info->soc <= 35)) {
			need_disable = true;
		}

	mca_log_err("bat temp =%d info soc = %d, disable_otg_incold =%d. need_disable==%d\n",
					info->temp_now, info->soc, info->disable_otg_incold, need_disable);
	if (!info->disable_otg_incold && info->support_disable_otg_incold && need_disable) {
	...
	} else if (info->disable_otg_incold && info->support_disable_otg_incold && !need_disable) {
...
static void short_monitor_usb_otg_burn(struct work_struct *work)
{
	...
	if(((type_temp >= TYPEC_NTC_TEMP_COMBINED_THRESHOLD && fitting_board_temp <= TYPEC_NTC_TEMP_FITTING_THRESHOLD) ||
		...
		info->short_typec_otg_burn_status = true;
		...
		info->short_typec_otg_burn_status = false;
...
static void otg_wdt_kick_work_func(struct work_struct *work)
{
	struct mtk_charger *info = container_of(work, struct mtk_charger, otg_wdt_kick_work.work);
	int ret = 0;
	u32 cp_vbus = 0;
	int wdt_kick_delay_time = 1000; /* 1s interval for watchdog kick */

	if (!info->support_otg_wdt_kick) {
		mca_log_err("otg wdt kick not supported\n");
		return;
	}

	/* Only kick watchdog when OTG is enabled */
	if (!info->otg_enable) {
		mca_log_err("otg not enabled, skip wdt kick\n");
		return;
	}

	/* Get cp_master device if not already */
	if (!info->cp_master) {
		info->cp_master = get_charger_by_name("cp_master");
		if (!info->cp_master) {
			mca_log_err("failed to get cp_master\n");
			goto out;
		}
	}

	/* Enable watchdog if not already enabled */
	if (!info->otg_wdt_enabled) {
		ret = charger_dev_cp_enable_wdt(info->cp_master, true);
		if (ret) {
			mca_log_err("failed to enable otg wdt, ret=%d\n", ret);
			goto out;
		}
		info->otg_wdt_enabled = true;
		mca_log_err("otg wdt enabled\n");
	}

	/* Kick watchdog by reading VBUS from cp_master */
	ret = charger_dev_get_vbus(info->cp_master, &cp_vbus);
	if (ret) {
		mca_log_err("failed to kick otg wdt, ret=%d\n", ret);
	} else {
		mca_log_err("otg wdt kick success, cp_vbus=%d\n", cp_vbus);
	}

out:
	/* Schedule next kick */
	if (info->otg_enable && info->support_otg_wdt_kick) {
		schedule_delayed_work(&info->otg_wdt_kick_work, msecs_to_jiffies(wdt_kick_delay_time));
	}
}
...
static void handle_jeita_charge(struct mtk_charger *info)
{
	...
	if(info->jeita_chg_index[0] == info->step_jeita_tuple_count - 2 && info->jeita_chg_index[1] == info->step_jeita_tuple_count - 1)
	{
	...
	if (is_between(info->jeita_fcc_cfg[0].low_threshold, info->jeita_fcc_cfg[info->step_jeita_tuple_count - 1].high_threshold, info->temp_now) && !info->input_suspend && !info->typec_burn && !info->short_typec_burn && info->bms_i2c_error_count < 10)
...
static void monitor_reduce_cv(struct mtk_charger *info)
{
	...
	if (vote_enable != last_vote_enable || curr_fv != last_vote_val) {
...
{
	static bool last_ffc_state = false;
	bool ffc_changed = (last_ffc_state != info->ffc_enable);
	last_ffc_state = info->ffc_enable;
	...
	if (info->support_pchg_policy) {
		...
		if (info->step_normal_chg && !info->ffc_enable) {
			get_index(info->step_normal_chg_cfg, info->step_fallback_hyst, info->step_forward_hyst, info->vbat_now, info->step_chg_index, ffc_changed);
		} else {
			get_index(info->step_chg_cfg, info->step_fallback_hyst, info->step_forward_hyst, info->vbat_now, info->step_chg_index, ffc_changed);
		}
...
static void monitor_thermal_limit(struct mtk_charger *info)
{
	...
	if(info->wlschg_support && info->wls_online) {
		chr_err("[CHARGE_LOOP] disable usb thermal vote\n");
...
static int check_exit_ffc_mode(struct mtk_charger *info)
{
	...
	if (info->charge_full)
		return 0;
	if (info->jeita_chg_index[0] == info->jeita_temp_normal){
	...
	if (is_between(info->ffc_medium_tbat, info->ffc_warm_tbat, info->temp_now)) {
		ffc_iterm = info->iterm_ffc_little_warm;
	} else if (is_between(info->ffc_warm_tbat, info->ffc_little_high_tbat, info->temp_now)) {
		ffc_iterm = info->iterm_ffc_warm;
	} else if (is_between(info->ffc_little_high_tbat, info->ffc_high_tbat, info->temp_now)) {
		ffc_iterm = info->iterm_ffc_hot;
	} else {
		ffc_iterm = info->iterm_ffc;
	}
	...
	} else if(info->switch_pmic_normal && info->soc > 95) {
...
static int handle_ffc_charge(struct mtk_charger *info)
{
	...
	if(info->switch_pmic_normal){
		info->ffc_enable = false;
			...
			((info->pd_type == MTK_PD_CONNECT_PE_READY_SNK_APDO && info->pd_adapter->verifed) || (info->wls_qc_enable && info->wls_adapter_type >= WLS_ADAPTER_XIAOMI_QC3 && !info->ffc_disable_force)) &&
			(info->thermal_level <= ENABLE_FCC_ITERM_LEVEL) && info->jeita_chg_index[0] == info->jeita_temp_normal) || info->suspend_recovery)
		info->ffc_enable = true;
	else
		info->ffc_enable = false;

	if (info->jeita_chg_index[0] <= 1) {
		iterm = info->iterm;
	} else {
	...
	if (is_between(info->ffc_medium_tbat, info->ffc_warm_tbat, info->temp_now_s)) {
		iterm_ffc_s = min(info->slave_iterm_ffc_little_warm, ITERM_FCC_WARM);
	} else if (is_between(info->ffc_warm_tbat, info->ffc_little_high_tbat, info->temp_now_s)) {
		iterm_ffc_s = min(info->slave_iterm_ffc_warm, ITERM_FCC_WARM);
	} else if (is_between(info->ffc_little_high_tbat, info->ffc_high_tbat, info->temp_now_s)) {
		iterm_ffc_s = min(info->slave_iterm_ffc_hot, ITERM_FCC_WARM);
	} else {
		iterm_ffc_s = min(info->slave_iterm_ffc, ITERM_FCC_WARM);
	}
#endif
	...
	if (info->ffc_enable) {
...
#else
		vote(info->iterm_votable, FFC_VOTER, true, iterm_ffc);
#endif
		val = true;
		...
		if (info->jeita_chg_index[0] == info->jeita_temp_normal) {
			vote(info->fv_votable, FFC_VOTER, true, fv_normal  - info->diff_fv_val + info->pmic_comp_v);
		} else {
...
static void monitor_wls_thermal_limit(struct mtk_charger *info)
{
	...
	if ((info->wls_thermal_level < 0) || (info->wls_thermal_level > 15)) {
		chr_err("wls_thermal: level %d invalid, quit\n", info->wls_thermal_level);
		return;
	...
	if ((info->wls_last_thermal_level == info->wls_thermal_level) && (info->wls_adapter_type == adapter_type)) {
		chr_info("wls_thermal: level %d and adapter %d not change, quit\n", info->wls_thermal_level, info->wls_adapter_type);
		return;
	}
	...
	case WLS_ADAPTER_XIAOMI_PD_100W:
		...
		break;
	default:
		chr_err("wls_thermal: no support thermal adapter: %d", adapter_type);
		break;
	}
...
static void battery_check_recharge(struct mtk_charger *info)
{
	...
	if (info->charge_full) {
		...
		if (info->support_eea_chg == 1) {
			if (info->soc < 95)
				recharge_by_soc = true;
		} else {
			if (raw_soc < recharge_soc)
				recharge_by_soc = true;
		}

		if ((((recharge_count >= 15) || recharge_by_soc || !info->real_full) && (info->jeita_chg_index[0] <= info->jeita_temp_normal)) || recharge_time_flag) {
			...
			mca_log_err("start recharge, recharge_count = %d, recharge_by_soc = %d, recharge_time_flag = %d\n", recharge_count, recharge_by_soc, recharge_time_flag);
...
{
	static int full_count = 0, iterm = 0, threshold_mv = 0;
	...
	chr_err("%s current stat : [%d %d %d]\n", __func__, info->vbat_now_master, info->current_now_m, info->bbc_charge_done);
...
#ifdef CONFIG_SUPPORT_DUAL_BATTERY
		if ((info->vbat_now_master >= fv_effective - threshold_mv) &&
				((-info->current_now_m <= iterm) || ((-info->current_now_m <= iterm_effective + 100) && info->bbc_charge_done))) {
#else
		if ((info->vbat_now >= fv_effective - threshold_mv) && info->current_now <= 0 &&
...
#ifdef CONFIG_SUPPORT_DUAL_BATTERY
		if (info->eoc_count >= 6 && !info->dual_vbat_diff_flag) {
...
static void slave_battery_check_full(struct mtk_charger *info)
{
	...
	if (!info->charge_full_s) {
		...
		if (info->eoc_count_s >= 6 && !info->dual_vbat_diff_flag) {
...
static void battery_check_terminate(struct mtk_charger *info)
{
	...
	if (!info->charge_full) {
#ifdef CONFIG_SUPPORT_DUAL_BATTERY
		if (info->eoc_count >= 6 && info->eoc_count_s >= 6) {
#else
		if (info->eoc_count >= 6) {
#endif
			info->eoc_count = 0;
...
static void check_dfx_param_data(struct mtk_charger *info)
{
	...
	if (ret) {
		...
		bms_get_property(BMS_PROP_FG_IN_SLEEP, &fg_in_sleep);
		if (fg_in_sleep) {
			chr_err("fg in sleep, set vbat 3700\n");
			info->vbat_now = 3700;
		} else {
	...
	chr_err("soc=%d, vbat=%d, vbat2=%d, ibat=%d, tbat=%d, cycle=%d, auth=%d, cp_vbat=%d\n", info->soc, info->vbat_now, info->vbat_now_slave, info->current_now, info->temp_now, info->cycle_count, info->gauge_authentic, info->cp_vbat);
}
...
static void update_dfx_cyclial_report(struct mtk_charger *info)
{
	...
	if ((info->dfx_fake_report_type & BIT_MASK(CHG_DFX_SOC_NOT_FULL)) || !(info->dfx_cyclial_report_type & BIT_MASK(CHG_DFX_SOC_NOT_FULL))) {
		if (info->real_full && info->jeita_chg_index[0] <= info->jeita_temp_normal) {
			if (info->soc != 100) {
	...
	if ((info->dfx_fake_report_type & BIT_MASK(CHG_DFX_HIGH_TEMP_DISCHARGING)) || !(info->dfx_cyclial_report_type & BIT_MASK(CHG_DFX_HIGH_TEMP_DISCHARGING))) {
		if (info->jeita_chg_index[0] == info->jeita_temp_hot && !info->charge_full && !info->bbc_charge_enable && info->vbat_now < get_effective_result(info->fv_votable))
	...
	if ((info->dfx_fake_report_type & BIT_MASK(CHG_DFX_SMART_ENDURANCE_SOC_ERR)) || !(info->dfx_cyclial_report_type & BIT_MASK(CHG_DFX_SMART_ENDURANCE_SOC_ERR))) {
		if ((info->dfx_fake_report_type & BIT_MASK(CHG_DFX_SMART_ENDURANCE_SOC_ERR))
				|| (info->smart_chg[SMART_CHG_ENDURANCE_PRO].en_ret && info->entry_soc < SMART_CHG_SOCLMT_TRIG_DEFAULT && info->soc > SMART_CHG_SOCLMT_TRIG_DEFAULT && !info->smart_chg[SMART_CHG_ENDURANCE_PRO].ignore_upload)) {
	...
	if ((info->dfx_fake_report_type & BIT_MASK(CHG_DFX_BATTERY_OVERVOLTAGE)) || !(info->dfx_cyclial_report_type & BIT_MASK(CHG_DFX_BATTERY_OVERVOLTAGE))) {
		...
		if (!fv_in_hysteresis) {
			if (info->vbat_now > fv_vote && info->vbat_now <= fv_vote + 5 && !info->charge_full && info->jeita_chg_index[0] != info->jeita_temp_hot) {
				batt_ovp_cnt++;
			...
			if ((info->dfx_fake_report_type & BIT_MASK(CHG_DFX_BATTERY_OVERVOLTAGE))
					|| (((info->vbat_now > (fv_vote + 5)) || batt_ovp_cnt >= 5) && !info->charge_full && info->jeita_chg_index[0] != info->jeita_temp_hot)) {
				batt_ovp_cnt = 0;
...
static void smart_chg_handle_sensing_chg(struct mtk_charger *info)
{
	...
	int smart_sic_mode = (info->smart_snschg_v2_support || info->smart_snschg_v3_support)? -1 : BALANCED_MODE;
	...
	case PER_YOUTUBE_SCENE:
		active_stat = true;
		if (info->smart_snschg_v3_support) {
			if(posture_st >= DESKTOP_STAT && posture_st <= HOLDER_STAT)
				smart_sic_mode = SUPERCHG_MODE;
			else if (posture_st >= TWOHAND_H_STAT && posture_st <= TWOHAND_V_STAT)
				smart_sic_mode = MIDDLE_MODE;
			else if(posture_st == ONEHAND_STAT || posture_st == ANS_CALL_STAT)
				smart_sic_mode = BALANCED_MODE;
		} else {
	...
	case CGAME_2_SCENE:
		if (info->smart_snschg_v3_support) {
			active_stat = true;
			if (posture_st == TWOHAND_H_STAT || posture_st == ANS_CALL_STAT)
				smart_sic_mode = BALANCED_MODE;
			else
				smart_sic_mode = SUPERCHG_MODE;
		}
...
_sensing_chg_quit:
	...
	if (info->smart_snschg_v2_support || info->smart_snschg_v3_support) {
		...
		if (sic_mode == BALANCED_MODE && info->sic_mode_setter == UPPER_SIC_MODE_SETTER) {
			chr_err("%s, sensing chg switching not open, quit.\n", __func__);
			return;
		}
		if (smart_sic_mode != info->smart_sic_mode) {
			...
			if (active_stat && smart_sic_mode != sic_mode) {
				if (report_retry_cnt++ < 3) {
					chr_err("%s, retry to report sic_mode %d, cnt %d.\n", __func__, smart_sic_mode, report_retry_cnt);
...
static void monitor_smart_chg(struct mtk_charger *info)
{
	...
	if(info->smart_snschg_support || info->smart_snschg_v2_support || info->smart_snschg_v3_support)
...
static void monitor_fv_descent(struct mtk_charger *info)
{
	...
	if(!info->cp_sm_run_state && ((info->set_smart_batt_diff_fv == 0 && info->set_smart_fv_diff_fv == 0) || info->diff_fv_val > max(info->set_smart_batt_diff_fv, info->set_smart_fv_diff_fv))) {
		...
		if(!isEffective){
			fv_vote = (get_effective_result(info->fv_votable)/10)*10;
...
static void buckchg_flip_jeita_update(struct mtk_charger *info)
  {
	...
	//step
	get_index(info->flip_step_chg_cfg, info->step_fallback_hyst, info->step_forward_hyst, vbat, info->flip_step_chg_index, false);
	if (info->flip_step_chg_index[0] == info->step_jeita_tuple_count - 1)
...
static int parse_battery_cycle_dts(struct mtk_charger *info, int cycle)
{
	...
	ret = snprintf(name, sizeof(name), "iterm_ffc_little_hot_%d_cycle%s", cycle, cell_supplier);
	if (ret >= sizeof(name))
		chr_err("%s: type_c name is truncated\n", __func__);
	...
	if (of_property_read_u32(np, name, &val) >= 0)
		info->iterm_ffc_little_hot = val;
	else {
		chr_err("failed to parse %s use default\n", name);
		info->iterm_ffc_little_hot = 800;
	}
...
void reset_step_jeita_charge(struct mtk_charger *info)
{
	...
	if (info->step_normal_chg && !info->ffc_enable) {
		get_index(info->step_normal_chg_cfg, info->step_fallback_hyst, info->step_forward_hyst, info->vbat_now, info->step_chg_index, true);
	} else {
...
int step_jeita_init(struct mtk_charger *info, struct device *dev)
{
	...
	info->smart_snschg_v3_support = of_property_read_bool(np, "smart_snschg_v3_support");
	...
	INIT_DELAYED_WORK(&info->otg_wdt_kick_work, otg_wdt_kick_work_func);
	if (info->smart_snschg_v2_support || info->smart_snschg_v3_support)
