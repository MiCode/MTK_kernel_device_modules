...
static void handle_step_charge(struct mtk_charger *info)
{
	...
	if (is_between(info->ffc_low_tbat, info->ffc_medium_tbat, info->temp_now)) {
	...
	if (current_state != info->ffc_temp_state) {
		...
		if (info->step_normal_chg && !info->ffc_enable) {
			get_index(info->step_normal_chg_cfg, info->step_fallback_hyst, info->step_forward_hyst, info->vbat_now, info->step_chg_index, false);
		} else {
			get_index(info->step_chg_cfg, info->step_fallback_hyst, info->step_forward_hyst, info->vbat_now, info->step_chg_index, false);
		}
		calc_delta_time(info->cp_start_time, &delta_time);
		if (info->step_chg_index[0] == 0 && info->cycle_count >= start_cycle_count &&
				info->ffc_temp_state != BAT_TEMP_LOW &&
				delta_time >= MAXCURR_MONITOR_TIME_S * 1000 &&
			...
			//vote(info->fcc_votable, STEP_CHARGE_VOTER, true, info->step_chg_fcc);
			chr_err("max charge current limit time=%lld,%lld", info->cp_start_time, delta_time);
		}
	}
...
static int handle_ffc_charge(struct mtk_charger *info)
{
	...
	} else if (info->jeita_chg_index[0] == 0) {
...
static void update_dfx_cyclial_report(struct mtk_charger *info)
{
	...
	if ((info->dfx_fake_report_type & BIT_MASK(CHG_DFX_HIGH_TEMP_DISCHARGING)) || !(info->dfx_cyclial_report_type & BIT_MASK(CHG_DFX_HIGH_TEMP_DISCHARGING))) {
		if ((info->dfx_fake_report_type & BIT_MASK(CHG_DFX_HIGH_TEMP_DISCHARGING)) ||
				(info->jeita_chg_index[0] == info->jeita_temp_hot && !info->charge_full && !info->bbc_charge_enable && info->vbat_now < get_effective_result(info->fv_votable))) {
	...
	if ((info->dfx_fake_report_type & BIT_MASK(CHG_DFX_BATTERY_OVERVOLTAGE)) || !(info->dfx_cyclial_report_type & BIT_MASK(CHG_DFX_BATTERY_OVERVOLTAGE))) {
		...
		if (info->vbat_now > fv_vote && info->vbat_now <= fv_vote + 5 && !info->charge_full) {
			batt_ovp_cnt++;
		...
		if ((info->dfx_fake_report_type & BIT_MASK(CHG_DFX_BATTERY_OVERVOLTAGE))
				|| (((info->vbat_now > (fv_vote + 5)) || batt_ovp_cnt >= 5) && !info->charge_full)) {
			batt_ovp_cnt = 0;
...
static int parse_battery_cycle_dts(struct mtk_charger *info, int cycle)
{
	...
	if (info->batt_cell_supplier == BMS_CELL_ATL) {
		cell_supplier = "_atl";
	} else {
		cell_supplier = "_cos";
	}
