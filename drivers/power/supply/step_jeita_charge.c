...
static void short_monitor_typec_burn(struct work_struct *work)
{
	...
	chr_err("%s typec_temp=%d-%d fit_temp=%d, otg_enable=%d, status:%d,%d,%d,%d\n",
                __func__, type_temp1, sub_temp1, fitting_board_temp, info->otg_enable, info->short_typec_burn,
	...
	if (((type_temp1 >= TYPEC_NTC_TEMP_COMBINED_THRESHOLD && fitting_board_temp != 0 && fitting_board_temp <= TYPEC_NTC_TEMP_FITTING_THRESHOLD) ||
		...
		!info->otg_enable && enable_anti_burn_confirm()) {
		chr_err("%s: trigger anti burn now, temp=%d fit_temp=%d\n", __func__, type_temp1, fitting_board_temp);
		update_connect_temp(info);
...
static void battery_check_recharge(struct mtk_charger *info)
{
	...
	if (info->charge_full) {
		...
		if ((((recharge_count >= 15) || recharge_by_soc || !info->real_full) && (info->jeita_chg_index[0] <= 4)) || recharge_time_flag) {
...
static void battery_check_full(struct mtk_charger *info)
{
	...
	chr_err("%s diff_fv_val = %d, iterm = %d, iterm_effective = %d, fv_effective = %d, full_count = %d, info->eoc_count=%d\n", __func__,
...
static void slave_battery_check_full(struct mtk_charger *info)
{
	...
	chr_err("%s diff_fv_val = %d, slave_iterm = %d, iterm_effective = %d, fv_effective = %d, full_count = %d, info->eoc_count=%d\n", __func__,
...
static void check_charge_data(struct mtk_charger *info)
{
	...
	if (info->input_suspend || info->typec_burn || info->short_typec_burn)
		chr_err("[CHARGE_LOOP] input_suspend = %d, typec_burn = %d, short_typec_burn = %d\n", info->input_suspend, info->typec_burn, info->short_typec_burn);
}
...
static void monitor_slave_connector(struct mtk_charger *info)
{
	...
	if (gpio_val == 1 || adc_val >= 1400 || fg_chip_ok == 0 || slave_fg_chip_ok == 0)
		conn_ok = false;
	chr_err("%s conn_ok:%d, adc:%d, gpio:%d, fg_chip_ok:%d, %d\n",__func__, conn_ok, 
...
static void monitor_isc_and_soa_charge(struct mtk_charger *info)//NFG1000A & NFG1000B支持该功能
{
	...
	chr_err("[%s]:slave_isc_alert_level=%d, slave_soa_alert_level=%d\n", __func__,
