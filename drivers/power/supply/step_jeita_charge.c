...
static void monitor_sw_cv(struct mtk_charger *info)
{
	...
	if (info->project_no == DALI_CN) {
		start_cycle_count = 30;
	}
	...
	if (info->project_no == DALI_CN || info->project_no == KLEE_CN || info->project_no == KLEE_GL) {
...
out:
	if(info->screen_status == DISPLAY_SCREEN_ON || (info->screen_status == DISPLAY_SCREEN_OFF && info->lpd_flag)) {
		if (info->project_no == RODIN_CN || info->project_no == RODIN_IN || info->project_no == RODIN_GL) {
...
static int check_exit_ffc_mode(struct mtk_charger *info)
{
	...
	if (info->project_no == GOYA_GL && info->charge_full)
		return 0;
	...
	if (info->project_no == TURNER_CN) {
		if (is_between(info->ffc_medium_tbat, info->ffc_warm_tbat, info->temp_now)) {
			ffc_iterm = info->iterm_ffc_little_warm + info->slave_iterm_ffc_little_warm;
			...
			ffc_iterm = info->iterm_ffc_hot + info->slave_iterm_ffc_hot;
		} else {
...
static int handle_ffc_charge(struct mtk_charger *info)
{
	...
	if (info->project_no == TURNER_CN || info->project_no == GOYA_GL)
	...
	if (info->project_no == KLIMT_GL && info->ffc_enable && info->wls_online) {
		if (info->vbat_now > fv_normal - 30) {
			mca_log_err("wls ffc chg close, vbat:%d fv_normal:%d\n", info->vbat_now, fv_normal);
			info->ffc_enable = false;
			info->ffc_disable_force = true;
		}
	}
...
static void battery_check_recharge(struct mtk_charger *info)
{
	...
	if (info->support_eea_chg == 1 && info->cycle_count <= 300 && info->project_no != TURNER_CN) {
		info->supplementary_electricity = true;
	}

	if (!info->supplementary_electricity && info->project_no != TURNER_CN) {
...
static void battery_check_full(struct mtk_charger *info)
{
	...
	if (!info->charge_full) {
		...
		if (info->project_no == KLIMT_GL && info->wls_online && iterm_effective > ITERM_FFC_HOT && threshold_mv == 40) {
			threshold_mv = 25;
		}
...
static void check_dfx_param_data(struct mtk_charger *info)
{
	...
	if(info->project_no == TURNER_CN && !IS_ERR_OR_NULL(info->bms_slave_psy)){
...
static void check_charge_data(struct mtk_charger *info)
{
	...
	} else if (info->project_no != RODIN_GL && info->project_no != KLIMT_GL) {
		if (info->ffc_enable)
		{
			if (info->cycle_count > 100 && info->cycle_count <= 300) {
				info->diff_fv_val =  20;
...
static void update_dfx_cyclial_report(struct mtk_charger *info)
{
	...
	if ((info->dfx_fake_report_type & BIT_MASK(CHG_DFX_HIGH_TEMP_DISCHARGING)) || !(info->dfx_cyclial_report_type & BIT_MASK(CHG_DFX_HIGH_TEMP_DISCHARGING))) {
		if ((info->dfx_fake_report_type & BIT_MASK(CHG_DFX_HIGH_TEMP_DISCHARGING)) || (!info->charge_full && info->jeita_chg_index[0] == info->jeita_temp_hot && !info->bbc_charge_enable)) {
...
static void reset_dfx_cyclial_report(struct mtk_charger *info)
{
	...
	if (((info->dfx_fake_report_type & BIT_MASK(CHG_DFX_VOLTAGE_DIFFERENCE)) || !(info->dfx_cyclial_report_type & BIT_MASK(CHG_DFX_VOLTAGE_DIFFERENCE))) && (info->project_no == TURNER_CN)) {
...
static int parse_battery_cycle_dts(struct mtk_charger *info, int cycle)
{
	...
	if(info->project_no == RODIN_GL)
	{
	...
	} else if (info->project_no == DALI_CN) {
		ret = bms_get_property(BMS_PROP_CELL_SUPPLIER, &val);
		if (ret)
			chr_err("failed to get gauge batt_cell_supplier\n");
		else
	...
	} else if (info->project_no == KLEE_GL) {
		ret = bms_get_property(BMS_PROP_CELL_SUPPLIER, &val);
		if (ret)
			chr_err("failed to get gauge batt_cell_supplier\n");
		else
		...
		if (info->batt_cell_supplier == BMS_CELL_LWN) {
			cell_supplier = "_lwn";
		} else {
			cell_supplier = "_cos_gl";
		}
	} else if (info->project_no == DASH_CN) {
		if (is_between(info->ffc_medium_tbat, info->ffc_high_tbat, info->temp_now)) {
			cell_temp = "_warm";
		}
	}
	...
	/* iterm_ffc parameter */
	if (info->project_no == DALI_CN || info->project_no == GOYA_GL ||
			info->project_no == KLEE_CN || info->project_no == DASH_CN ||
			info->project_no == KLEE_GL) {
		cycle = 1;
	}
	...
	if(info->project_no == TURNER_CN){
		...
		for (i = 0; i < STEP_JEITA_TUPLE_COUNT; i++)
		chr_info("FLIP_STEP_FCC %d %d %d\n", info->flip_step_chg_cfg[i].low_threshold, info->flip_step_chg_cfg[i].high_threshold, info->flip_step_chg_cfg[i].value);
	}
...
static int parse_step_charge_config(struct mtk_charger *info, bool force_update)
{
	...
	if (info->project_no == DALI_CN) {
		...
		} else if (cycle_count > 100 && cycle_count <= 150) {
			battery_cycle = BATTERY_CYCLE_100_TO_150;
		} else if (cycle_count > 150 && cycle_count <= 300) {
			battery_cycle = BATTERY_CYCLE_150_TO_300;
		...
		if (cycle_count <= 80) {
			battery_cycle = BATTERY_CYCLE_1_TO_80;
		} else if (cycle_count > 80 && cycle_count <= 100) {
			battery_cycle = BATTERY_CYCLE_80_TO_100;
		} else if (cycle_count > 100 && cycle_count <= 180) {
			battery_cycle = BATTERY_CYCLE_100_TO_180;
		} else if (cycle_count > 180 && cycle_count <= 300) {
			battery_cycle = BATTERY_CYCLE_180_TO_300;
		} else if (cycle_count > 300 && cycle_count <= 800) {
			battery_cycle = BATTERY_CYCLE_300_TO_800;
		} else {
			battery_cycle = BATTERY_CYCLE_800_TO_MORE;
...
int step_jeita_init(struct mtk_charger *info, struct device *dev)
{
	...
	if (info->project_no != RODIN_GL && info->project_no != KLEE_GL) {
