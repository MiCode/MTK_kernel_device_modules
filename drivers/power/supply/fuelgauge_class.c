...
int fuelgauge_dev_read_real_temp(struct fg_device *fg_dev, int *real_temp)
{
	if (fg_dev != NULL && fg_dev->ops != NULL &&
	    fg_dev->ops->read_real_temp)
		return fg_dev->ops->read_real_temp(fg_dev, real_temp);

	return -EOPNOTSUPP;
}
EXPORT_SYMBOL(fuelgauge_dev_read_real_temp);

int fuelgauge_dev_read_voltage_max(struct fg_device *fg_dev, int *voltage_max)
{
	if (fg_dev != NULL && fg_dev->ops != NULL &&
	    fg_dev->ops->read_voltage_max)
		return fg_dev->ops->read_voltage_max(fg_dev, voltage_max);

	return -EOPNOTSUPP;
}
EXPORT_SYMBOL(fuelgauge_dev_read_voltage_max);

int fuelgauge_dev_read_temp_max(struct fg_device *fg_dev, int *temp_max)
{
	if (fg_dev != NULL && fg_dev->ops != NULL &&
	    fg_dev->ops->read_temp_max)
		return fg_dev->ops->read_temp_max(fg_dev, temp_max);

	return -EOPNOTSUPP;
}
EXPORT_SYMBOL(fuelgauge_dev_read_temp_max);

int fuelgauge_dev_read_over_vol_duration(struct fg_device *fg_dev, int *over_vol_duration)
{
	if (fg_dev != NULL && fg_dev->ops != NULL &&
	    fg_dev->ops->read_over_vol_duration)
		return fg_dev->ops->read_over_vol_duration(fg_dev, over_vol_duration);

	return -EOPNOTSUPP;
}
EXPORT_SYMBOL(fuelgauge_dev_read_over_vol_duration);
...
int fuelgauge_dev_fg_get_ui_soh(struct fg_device *fg_dev, char *buf)
{
	if (fg_dev != NULL && fg_dev->ops != NULL &&
	    fg_dev->ops->fg_get_ui_soh)
		return fg_dev->ops->fg_get_ui_soh(fg_dev, buf);

	return -EOPNOTSUPP;
}
EXPORT_SYMBOL(fuelgauge_dev_fg_get_ui_soh);

int fuelgauge_dev_fg_set_ui_soh(struct fg_device *fg_dev, char *buf)
{
	if (fg_dev != NULL && fg_dev->ops != NULL &&
	    fg_dev->ops->fg_set_ui_soh)
		return fg_dev->ops->fg_set_ui_soh(fg_dev, buf);

	return -EOPNOTSUPP;
}
EXPORT_SYMBOL(fuelgauge_dev_fg_set_ui_soh);

int fuelgauge_dev_fg_get_soh_sn(struct fg_device *fg_dev, char *buf)
{
	if (fg_dev != NULL && fg_dev->ops != NULL &&
	    fg_dev->ops->fg_get_soh_sn)
		return fg_dev->ops->fg_get_soh_sn(fg_dev, buf);

	return -EOPNOTSUPP;
}
EXPORT_SYMBOL(fuelgauge_dev_fg_get_soh_sn);

int fuelgauge_dev_fg_get_batt_sn(struct fg_device *fg_dev, char *buf)
{
	if (fg_dev != NULL && fg_dev->ops != NULL &&
	    fg_dev->ops->fg_get_batt_sn)
		return fg_dev->ops->fg_get_batt_sn(fg_dev, buf);

	return -EOPNOTSUPP;
}
EXPORT_SYMBOL(fuelgauge_dev_fg_get_batt_sn);

int fuelgauge_dev_sc_fg_read_charging_status(struct fg_device *fg_dev)
{
	if (fg_dev != NULL && fg_dev->ops != NULL &&
	    fg_dev->ops->sc_fg_read_charging_status)
		return fg_dev->ops->sc_fg_read_charging_status(fg_dev);

	return -EOPNOTSUPP;
}
EXPORT_SYMBOL(fuelgauge_dev_sc_fg_read_charging_status);

int fuelgauge_dev_sc_fg_read_volt_range(struct fg_device *fg_dev)
{
	if (fg_dev != NULL && fg_dev->ops != NULL &&
	    fg_dev->ops->sc_fg_read_volt_range)
		return fg_dev->ops->sc_fg_read_volt_range(fg_dev);

	return -EOPNOTSUPP;
}
EXPORT_SYMBOL(fuelgauge_dev_sc_fg_read_volt_range);

int fuelgauge_dev_sc_fg_read_temp_range(struct fg_device *fg_dev)
{
	if (fg_dev != NULL && fg_dev->ops != NULL &&
	    fg_dev->ops->sc_fg_read_temp_range)
		return fg_dev->ops->sc_fg_read_temp_range(fg_dev);

	return -EOPNOTSUPP;
}
EXPORT_SYMBOL(fuelgauge_dev_sc_fg_read_temp_range);

int fuelgauge_dev_sc_fg_ext_cmd_read_operation_status(struct fg_device *fg_dev)
{
	if (fg_dev != NULL && fg_dev->ops != NULL &&
	    fg_dev->ops->sc_fg_ext_cmd_read_operation_status)
		return fg_dev->ops->sc_fg_ext_cmd_read_operation_status(fg_dev);

	return -EOPNOTSUPP;
}
EXPORT_SYMBOL(fuelgauge_dev_sc_fg_ext_cmd_read_operation_status);

int fuelgauge_dev_sc_fg_ext_cmd_read_dev_status(struct fg_device *fg_dev)
{
	if (fg_dev != NULL && fg_dev->ops != NULL &&
	    fg_dev->ops->sc_fg_ext_cmd_read_dev_status)
		return fg_dev->ops->sc_fg_ext_cmd_read_dev_status(fg_dev);

	return -EOPNOTSUPP;
}
EXPORT_SYMBOL(fuelgauge_dev_sc_fg_ext_cmd_read_dev_status);

int fuelgauge_dev_sc_fg_ext_cmd_read_host_cmd_status(struct fg_device *fg_dev)
{
	if (fg_dev != NULL && fg_dev->ops != NULL &&
	    fg_dev->ops->sc_fg_ext_cmd_read_host_cmd_status)
		return fg_dev->ops->sc_fg_ext_cmd_read_host_cmd_status(fg_dev);

	return -EOPNOTSUPP;
}
EXPORT_SYMBOL(fuelgauge_dev_sc_fg_ext_cmd_read_host_cmd_status);

int fuelgauge_dev_sc_fg_ext_cmd_read_term_volt(struct fg_device *fg_dev)
{
	if (fg_dev != NULL && fg_dev->ops != NULL &&
	    fg_dev->ops->sc_fg_ext_cmd_read_term_volt)
		return fg_dev->ops->sc_fg_ext_cmd_read_term_volt(fg_dev);

	return -EOPNOTSUPP;
}
EXPORT_SYMBOL(fuelgauge_dev_sc_fg_ext_cmd_read_term_volt);

int fuelgauge_dev_sc_fg_ext_cmd_read_xt_status(struct fg_device *fg_dev)
{
	if (fg_dev != NULL && fg_dev->ops != NULL &&
	    fg_dev->ops->sc_fg_ext_cmd_read_xt_status)
		return fg_dev->ops->sc_fg_ext_cmd_read_xt_status(fg_dev);

	return -EOPNOTSUPP;
}
EXPORT_SYMBOL(fuelgauge_dev_sc_fg_ext_cmd_read_xt_status);

int fuelgauge_dev_sc_fg_ext_cmd_read_xt_status2(struct fg_device *fg_dev)
{
	if (fg_dev != NULL && fg_dev->ops != NULL &&
	    fg_dev->ops->sc_fg_ext_cmd_read_xt_status2)
		return fg_dev->ops->sc_fg_ext_cmd_read_xt_status2(fg_dev);

	return -EOPNOTSUPP;
}
EXPORT_SYMBOL(fuelgauge_dev_sc_fg_ext_cmd_read_xt_status2);

int fuelgauge_dev_sc_fg_ext_cmd_read_xt_status3(struct fg_device *fg_dev)
{
	if (fg_dev != NULL && fg_dev->ops != NULL &&
	    fg_dev->ops->sc_fg_ext_cmd_read_xt_status3)
		return fg_dev->ops->sc_fg_ext_cmd_read_xt_status3(fg_dev);

	return -EOPNOTSUPP;
}
EXPORT_SYMBOL(fuelgauge_dev_sc_fg_ext_cmd_read_xt_status3);
