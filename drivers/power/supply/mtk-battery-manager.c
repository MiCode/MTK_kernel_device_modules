	...
	union power_supply_propval *val)
{
	...
	gm = bm->gm1;
	if (!gm->ti_bms_psy) {
		if(gm->project_no == TURNER_CN)
			gm->ti_bms_psy = power_supply_get_by_name("bms_strategy");
		else
	...
	if (IS_ERR_OR_NULL(chg_psy)) {
		...
		if (!online.intval && !wls_online) {
			...
			if (status.intval == POWER_SUPPLY_STATUS_NOT_CHARGING) {
				...
				if (wls_online) {
					pr_err("wls online, set status to charging\n");
					bs_data->bat_status = POWER_SUPPLY_STATUS_CHARGING;
					status.intval = POWER_SUPPLY_STATUS_CHARGING;
				}
			} else {
		...
		int *temp)
{
	...
	if (gm->project_no == DALI_CN || gm->project_no == KLIMT_GL || gm->project_no == GOYA_GL ||
			gm->project_no == KLEE_CN || gm->project_no == KLEE_GL || gm->project_no == DASH_CN) {
		bms_get_property(BMS_PROP_REAL_TEMP, &batt_temp);
		pr_err("batt_thermal real_temp:%d\n", batt_temp);
	} else {
		power_supply_get_property(gm->ti_bms_psy, POWER_SUPPLY_PROP_TEMP, &val);
		batt_temp = val.intval;
		pr_err("batt_thermal temp:%d\n", batt_temp);
	}
