	...
	bool disable)
{
	...
	if (disable == info->force_disable_pp[CHG1_SETTING] || info->input_suspend)
		goto out;
	...
	struct chg_limit_setting *setting)
{
	...
	if (info->typec_port1_plugin) {
		info->chr_type = get_charger_type(info);
	...
	if (info->real_type == XMUSB350_TYPE_FLOAT && (info->pd_type != MTK_PD_CONNECT_PE_READY_SNK && info->pd_type != MTK_PD_CONNECT_PE_READY_SNK_APDO && info->pd_type != MTK_PD_CONNECT_PE_READY_SNK_PD30)) {
		pdata->input_current_limit =  1000000;
		pdata->charging_current_limit = 1000000;
		is_basic = true;
		mca_log_err("[CHG_CURRENT_LIMIT](b) float type, %d %d\n", pdata->input_current_limit, pdata->charging_current_limit);
		goto done;
 ...
 retry:
		...
		} else if (info->project_no == PROJECT_CN && info->pmic_parachg_mode) {
			chr_info("pmic_parachg_mode, skip!\n");
		} else if(get_vbus(info) < 6000) {
			if (info->pd_adapter->adapter_svid == USB_PD_MI_SVID) {
				pdata->input_current_limit = (info->cp_quit_icl_flag)? 1000000 : 3000000;
			} else {
				pdata->input_current_limit = (info->cp_quit_icl_flag)? 1000000 : 1500000;
			}
		} else {
		...
		if (info->project_no == PROJECT_CN) {
			pdata->charging_current_limit = min(pdata->charging_current_limit, 1300000);
			pdata->input_current_limit = min(pdata->input_current_limit, 1600000);
		}
#endif

		adapter_icl = get_client_vote(info->icl_votable, ADAPTER_ICL_VOTER);
		if (adapter_icl > 0)
			pdata->input_current_limit = min(pdata->input_current_limit, adapter_icl * 1000);
		info->real_type = XMUSB350_TYPE_PD;
		mca_log_err("[CHG_CURRENT_LIMIT](c) pd adapter_imax:%d,%d, thermal_ichg:%d, sic_ichg:%d\n",
			info->adapter_imax, adapter_icl, info->thermal_current, info->sic_current);
		is_basic = true;
		goto done;
	...
	if (info->chr_type == POWER_SUPPLY_TYPE_USB &&
		...
		if (cap.nr == 1) {
			...
			chr_err("[REV_CHG] [CHG_CURRENT_LIMIT](k) C to C set input current %dma charging\n", adapter_imax);
			...
			if (info->support_quick_revchg) {
				if (info->project_no == PROJECT_CN) //9V1.4A
					pdata->input_current_limit =  min(1400000, adapter_imax * 1000);
				else
  				...
  				if (cap.nr >= 2) {
  					...
  					chr_err("[REV_CHG] cap.nr=%d, skip index 1 access\n", cap.nr);
  				}
				pdata->charging_current_limit = 3500000;
  				chr_err("[REV_CHG] RevCHG C to C\n");
			} else {
	...
	if (info->en_cts_mode) {
		if (info->ta_status[PD] == TA_ATTACH) {
			...
			if ((pdata->usb_input_current_limit <=
				pdata->input_current_limit) &&
				(pdata->usb_input_current_limit != -1) &&
				(info->chr_type != POWER_SUPPLY_TYPE_USB_CDP)) {
					pdata->input_current_limit =
	...
	if (info->project_no == PROJECT_CN && !info->en_cts_mode && pdata->pd_input_current_limit != -1) {
		if (pdata->pd_input_current_limit <=
			pdata->input_current_limit) {
			pdata->input_current_limit =
					pdata->pd_input_current_limit;
		...
		chr_err("[CHG_CURRENT_LIMIT](s) pdtest: %d, pd: %d, ret: %d\n",
			info->en_cts_mode, pdata->pd_input_current_limit, pdata->input_current_limit);
	}
	...
	if (info->project_no == PROJECT_CN) {
		if (info->bms_i2c_error_count >= 10 && info->slave_bms_i2c_error_count >= 10) {
		...
		} else if(info->bms_i2c_error_count >= 10 || info->slave_bms_i2c_error_count >= 10){
			pdata->charging_current_limit = min(1000000, pdata->charging_current_limit);
			pdata->input_current_limit =  min(1000000, pdata->input_current_limit);
		}
	} else {
	...
	if (info->pmic_parachg_mode) {
		if (info->project_no == PROJECT_CN) {
			pdata->charging_current_limit = min(4000000, pdata->charging_current_limit);
			pdata->input_current_limit = min(2500000, pdata->input_current_limit);
		} else {
			pdata->charging_current_limit = min(2000000, pdata->charging_current_limit);
			pdata->input_current_limit =  min(5500000, pdata->input_current_limit);
		}
	}
	...
	if (!info->pmic_parachg_mode && info->project_no == PROJECT_CN && get_vbus(info) < 6000) {
		pdata->charging_current_limit = min(3000000, pdata->charging_current_limit);
		pdata->input_current_limit = min(3000000, pdata->input_current_limit);
		mca_log_err("[CHG_CURRENT_LIMIT] ichg = %duA icl %duA\n", pdata->charging_current_limit, pdata->input_current_limit);
	}
