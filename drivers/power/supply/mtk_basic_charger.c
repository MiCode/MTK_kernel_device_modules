	...
	bool disable)
{
	...
	if (disable == info->force_disable_pp[CHG1_SETTING] || !!info->input_suspend != disable)
		goto out;
...
retry:
		...
		} else if (info->project_no == TURNER_CN) {
	...
	if (info->chr_type == POWER_SUPPLY_TYPE_USB &&
		...
		if(smart_pwrboost_get_flag())
		{
			chr_err("[CHG_CURRENT_LIMIT](g) smart_chg_outdoor triggger, DCP set input current 2000mA charging\n");
		...
		if(cap.nr == 1)
		{
			...
			if (info->project_no == TURNER_CN) {
				pdata->input_current_limit =  min(2000000, adapter_imax * 1000);
				if (get_vbus(info) < 6000 && cap.max_mv[1] == 9000) {
					adapter_dev_set_cap(info->pd_adapter, MTK_PD_APDO, cap.max_mv[1], cap.ma[1]);
					chr_err("[REV_18W_CHG] request 9000mv\n");
				}
				pdata->charging_current_limit = 3500000;
				chr_err("[REV_18W_CHG] 18W C to C\n");
			} else if (info->support_quick_revchg) {
				...
				pdata->charging_current_limit = 5500000;
	...
	if (info->pmic_parachg_mode) {
		...
		} else if (info->project_no == GOYA_GL) {
			pdata->charging_current_limit = min(3000000, pdata->charging_current_limit);
			pdata->input_current_limit = min(1600000, pdata->input_current_limit);
		} else if (info->project_no == TURNER_CN) {
		...
		} else if (info->project_no == KLEE_CN) {
			pdata->charging_current_limit = min(2000000, pdata->charging_current_limit);
			pdata->input_current_limit =  min(1400000, pdata->input_current_limit);
		} else {
			pdata->charging_current_limit = min(2000000, pdata->charging_current_limit);
