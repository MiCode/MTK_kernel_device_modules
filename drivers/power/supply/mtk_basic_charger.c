...
retry:
	...
	if (info->chr_type == POWER_SUPPLY_TYPE_USB &&
		...
		if(cap.nr == 1) {
			...
			pdata->input_current_limit =  min(1600000, adapter_imax * 1000);
			pdata->charging_current_limit = min(info->thermal_current * 1000, 3150 * 1000);
			chr_err("Misidentification C to C , pd use vote current charging=%d\n", info->thermal_current);
