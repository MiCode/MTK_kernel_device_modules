    ...
    if (!chip->fc_flag) {
        ...
        case ADAPTER_XIAOMI_PD_100W:
            if (chip->is_sailboat_tx) {
                ...
                sc96281_log_info("50/60/100W adapters set fcc 9000mA\n");
                sc96281_set_pmic_fcc(chip, 9000);
                if (chip->is_car_tx)
                    chip->is_vout_range_set_done = true;
            }
            chip->pre_vol = EPP_PLUS_VOUT;
...
static void sc96281_pre_authen_process(struct sc96281_chg *chip)
{
    int ret = 0;
    uint8_t adapter_type = ADAPTER_NONE;
    int len = 0;
    ...
    ret = read_cust(chip, cust_rx.mi_ctx.adapter_type, &adapter_type);
    if (ret) {
        sc96281_log_info("pre authen get adapter type failed\n");
        return;
    }
    ...
    if (adapter_type) {
        ...
        sc96281_log_info("pre authen use default adapter type\n");
...
static void sc96281_chg_handler(struct sc96281_chg *chip, u16 int_flag)
{
    ...
    if (int_flag & RX_INT_POWER_ON) {
        ...
        sc96281_pre_authen_process(chip);
        if (chip->epp) {
    ...
    if (gpio_is_valid(chip->power_good_gpio)) {
        ...
        if (ret) {
            ...
            if (pg_low_cnt++ < 1) {
                ...
                schedule_delayed_work(&chip->wireless_pg_det_work, msecs_to_jiffies(2000));
                return;
            }
            mca_log_err("power_good real low, wireless detached\n");
            pg_low_cnt = 0;
