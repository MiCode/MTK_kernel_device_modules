...
static int sc96281_set_reverse_pmic_boost(struct sc96281_chg *chip, int purpose, int enable)
{
    ...
    if (enable) {
        ...
        if (otg_enable) {
            regulator_set_voltage(vbus, 5000000, INT_MAX);
        } else {
...
static void sc96281_pg_det_work(struct work_struct *work)
{
    ...
    if (gpio_is_valid(chip->power_good_gpio)) {
        ...
        if (ret) {
            sc96281_log_info("power_good high, wireless attached\n");
            if (!chip->wls_wakelock->active)
            ...
            sc96281_log_info("power_good low, wireless detached\n");
...
static int sc96281_parse_dt(struct sc96281_chg *chip)
{
    ...
    if (!node) {
        sc96281_log_err("%s:No DT data Failing Probe\n", __func__);
        return -EINVAL;
    }
    ...
    if (chip->q_value_supprot) {
        ...
        sc96281_log_info("[%s] tx_q1[0x%02x 0x%02x], tx_q2[0x%02x 0x%02x]\n", __func__,
