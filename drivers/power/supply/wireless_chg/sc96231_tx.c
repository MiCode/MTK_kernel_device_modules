...
__maybe_unused
static int sc96231_set_charge_type(struct sc96231 *sc, uint8_t charge_type)
{
    int ret = 0;

    ret = writeCust(sc, cust_tx.CHARGE_TYPE, &charge_type);
    if (ret < 0) {
        mca_log_err("sc96231 set charge type fail\n");
        return ret;
    }
    mca_log_err("sc96231 set charge type %d success\n", charge_type);
    return ret;
}
...
__maybe_unused
static int sc96231_download_fw(void *data, bool using_bin)
{
	int ret = 0;
    struct sc96231 *sc = (struct sc96231 *)data;
	...
	if (using_bin) {
		ret = sc96231_get_image_fwver(sc->fw_bin, sc->fw_bin_length, &image_ver);
	} else {
		ret = sc96231_get_image_fwver(sc->fw_data_ptr, sc->fw_data_size, &image_ver);
	}
	...
	if(using_bin){
		ret = sc96231_mtp_program(sc, sc->fw_bin, sc->fw_bin_length);
	} else {
...
static int sc96231_wls_enable_reverse_chg(struct wireless_charger_device *chg_dev, bool enable)
{
    int rc = 0;
    ...
    if (!enable || chip->proc_data.reverse_chg_sts != REVERSE_STATE_TRANSFER) {
        rc = mca_wireless_rev_enable_reverse_charge(chip, enable);
        if (rc < 0){
            return rc;
        }
        if (!enable)
    ...
    if (!chip->support_tx_only) {
        rc = mca_wireless_rev_set_user_reverse_chg(chip, enable);
        if (rc < 0)
            return rc;
    }
...
static int sc96231_wls_get_reverse_chg(struct wireless_charger_device *chg_dev, bool *enable)
{
    struct sc96231 *chip = dev_get_drvdata(&chg_dev->dev);
    *enable = chip->proc_data.reverse_chg_en;
    return 0;
}

static int sc96231_wls_get_reverse_chg_state(struct wireless_charger_device *chg_dev, int *state)
{
    struct sc96231 *chip = dev_get_drvdata(&chg_dev->dev);
    *state = chip->proc_data.reverse_chg_sts;
    return 0;
}
...
static int sc96231_get_q_value(struct sc96231 *sc, int *value)
{
    int ret = 0;

    ret = sc96231_tx_set_cmd(sc, AP_CMD_MI_CHECK_Q);
    if (ret) {
        mca_log_err("check Q fail\n");
    } else {
        mca_log_info("check Q success\n");
    }

    msleep(1000);

    ret = readCust(sc, cust_tx.Reserved01AA, value);
    if (ret < 0) {
        mca_log_err("tx get Q fail\n");
    } else {
        mca_log_info("tx get Q :%d\n", *value);
    }

    return ret;
}

static int sc96231_wls_get_tx_q_value(struct wireless_charger_device *chg_dev, int *tx_q_value)
{
    struct sc96231 *info = dev_get_drvdata(&chg_dev->dev);
    int ret = 0;
    ...
    if (!info->support_tx_only) {
        mca_log_info("don't support check q\n");
        return 0;
    }

    if (info->proc_data.fw_updating || info->proc_data.reverse_chg_en) {
        mca_log_info("fw updating or revchg enable, don't check q\n");
        return 0;
    }

    info->is_check_q = true;
    ret = mca_wireless_rev_charge_config(info, true);
    if (ret < 0)
        goto check_exit;

    (void)sc96231_get_q_value(info, tx_q_value);

 check_exit:
    ret = mca_wireless_rev_charge_config(info, false);
    info->is_check_q = false;
    mca_wireless_rev_enable_reverse_charge(info, true);
    return ret;
}

static int sc96231_wls_is_i2c_ok(struct wireless_charger_device *chg_dev, bool *i2c_ok)
{
    struct sc96231 *chip = dev_get_drvdata(&chg_dev->dev);
    int ret = 0;

    if (chip->proc_data.fw_updating || chip->proc_data.reverse_chg_en) {
        mca_log_err("fw_updating or reverse_chg_en, not check\n");
        *i2c_ok = false;
        return 0;
    }
...
static const struct wireless_charger_properties sc96231_chg_props = {
 ...
 static const struct wireless_charger_ops sc96231_wls_ops = {
    ...
    .wls_enable_reverse_chg = sc96231_wls_enable_reverse_chg,
    .wls_get_reverse_chg = sc96231_wls_get_reverse_chg,
    .wls_get_reverse_chg_state = sc96231_wls_get_reverse_chg_state,
    .wls_is_i2c_ok = sc96231_wls_is_i2c_ok,
    .wls_get_tx_q_value = sc96231_wls_get_tx_q_value,
};
...
static void mca_wireless_reverse_chg_handler(struct sc96231 *info)
{
	...
	case RTX_INT_TX_DET_RX:
		...
		sc96231_get_tx_ss(&info->proc_data.ss_voltage, info);
		if (info->proc_data.ss_voltage < 100) {
			mca_log_info("tx get ss_reg value: %d, pen place err: ss\n", info->proc_data.ss_voltage);
			mca_wireless_rev_enable_reverse_charge(info, false);
			info->proc_data.ss_voltage = 0;
			cancel_delayed_work_sync(&info->pen_data_handle_work);
	...
	case RTX_INT_TX_BLE_CONNECT:
		...
		event_data.event_len = len;
		mca_event_report_uevent(&event_data);
		sc96231_get_fw_version_check(&check_result, info);
		mca_log_info("rx firmware version: boot = 0x%x, rx = 0x%x, tx = 0x%x, check_result = %d\n",
			info->wls_fw_data->fw_boot_id, info->wls_fw_data->fw_rx_id, info->wls_fw_data->fw_tx_id, check_result);
		break;
	default:
		break;
	}
...
static void xm_wireless_rev_charge_config_work(struct work_struct *work)
{
    ...
    ret = sc96231_set_charge_type(info, 0x05);
}
...
int mca_wireless_rev_enable_reverse_charge(struct sc96231 *info, bool enable)
{
    ...
    if (enable) {
        if (info->support_tx_only) {
            ...
            if (info->is_check_q) {
                mca_log_info("is check q, don't enable reverse charge\n");
                return -1;
            }
        }
        ...
        if (!info->proc_data.wireless_reverse_closing) {
            ...
            if (!info->proc_data.revchg_config_close)
...
static int sc96231_parse_dt(struct sc96231 *chip)
{
    ...
    chip->hall3_gpio = of_get_named_gpio(node, "hall3-int", 0);
    if (!gpio_is_valid(chip->hall3_gpio)) {
        mca_log_err("fail_hall3_gpio %d\n", chip->hall3_gpio);
        return -EINVAL;
    }

    chip->hall4_gpio = of_get_named_gpio(node, "hall4-int", 0);
    if (!gpio_is_valid(chip->hall4_gpio)) {
        mca_log_err("fail_hall4_gpio %d\n", chip->hall4_gpio);
        return -EINVAL;
    }

    chip->hall3_s_gpio = of_get_named_gpio(node, "hall3-s-int", 0);
    if (!gpio_is_valid(chip->hall3_s_gpio)) {
        mca_log_err("fail_hall3_s_gpio %d\n", chip->hall3_s_gpio);
        return -EINVAL;
    }

    chip->hall4_s_gpio = of_get_named_gpio(node, "hall4-s-int", 0);
    ...
    ret = gpio_request(chip->reverse_txon_gpio, "reverse-txon-gpio");
    if (ret)
        mca_log_err("request reverse_txon_gpio [%d] fail, ret=%d\n", chip->reverse_txon_gpio, ret);
    else {
    ...
    ret = gpio_request(chip->reverse_boost_gpio, "reverse-boost-gpio");
    if (ret)
        mca_log_err("request reverse_boost_gpio [%d] fail, ret=%d\n", chip->reverse_boost_gpio, ret);
    else {
...
static void sc96231_ppe_hall_interrupt_work(struct work_struct *work)
{
    ...
    if (!hall3_s_gpio_val || !hall4_s_gpio_val) {
...
static void sc96231_hall_interrupt_work(struct work_struct *work)
{
    ...
    if (!hall3_gpio_val || !hall4_gpio_val) {
        ...
        mca_log_info("pen detached!\n");
        mca_wireless_rev_process_hall_change(chip, 0);
    }

exit:
    mutex_unlock(&chip->hall_int_lock);
    return;
}
...
static int sc96231_firmware_update_func(struct sc96231 *chip, u8 cmd)
{
    int ret = 0;

    switch (cmd) {
    case FW_UPDATE_CHECK:
        chip->using_bin = false;
...
static void xm_wireless_rev_tx_firmware_update_work(struct work_struct *work)
{
    int ret = 0;
    u8 check_result = 0;
    struct sc96231 *chip = container_of(work, struct sc96231, tx_firmware_update.work);
    int len;
    char event[MCA_EVENT_NOTIFY_SIZE] = { 0 };
    struct mca_event_notify_data event_data = { 0 };

    if (chip->proc_data.reverse_chg_en) {
        mca_wireless_rev_enable_reverse_charge(chip, false);
        chip->proc_data.reverse_chg_sts = REVERSE_STATE_ENDTRANS;
        len = snprintf(event, MCA_EVENT_NOTIFY_SIZE,
            "POWER_SUPPLY_REVERSE_CHG_STATE=%d", REVERSE_STATE_ENDTRANS);
        event_data.event = event;
    ...
    if (chip->support_tx_only && chip->is_check_q) {
        mca_log_info("checking q, don't begin fw update\n");
        return;
    }
    ...
    ret = sc96231_download_fw(chip, chip->using_bin);
    if (ret < 0) {
        mca_log_info("fw download failed!\n");
        goto exit;
    }

    ret = mca_wireless_rev_charge_config(chip, false);
    msleep(1000);
    ret = mca_wireless_rev_charge_config(chip, true);
...
exit:
    ...
    if (chip->support_tx_only)
        mca_wireless_rev_enable_reverse_charge(chip, true);
}
...
static void xm_wireless_rev_pen_data_handle_work(struct work_struct *work)
{
	...
	len = snprintf(event, MCA_EVENT_NOTIFY_SIZE,
		"POWER_SUPPLY_PEN_HALL3=%d", pen_hall3);
	event_data.event = event;
	event_data.event_len = len;

	len2 = snprintf(event2, MCA_EVENT_NOTIFY_SIZE,
		"POWER_SUPPLY_PEN_HALL4=%d", pen_hall4);
	event_data2.event = event2;
	event_data2.event_len = len2;

	mca_event_report_multi_uevent(info, &event_data, &event_data2);
	...
	if (pen_full_flag) {
		...
		mca_wireless_rev_enable_reverse_charge(info, false);
	} else if ((pen_soc == 100) && (info->full_soc_count < PEN_SOC_FULL_COUNT)) {
		info->full_soc_count++;
		mca_log_info("pens soc is 100 count: %d\n", info->full_soc_count);
	} else {
		info->full_soc_count = 0;
	}

	if (info->full_soc_count == PEN_SOC_FULL_COUNT) {
...
static void xm_wireless_rev_pen_place_err_check_work(struct work_struct *work)
{
    ...
    sc96231_get_tx_ss(&info->proc_data.ss_voltage, info);
    if (!info->proc_data.ss_voltage && (info->hall3_online || info->hall4_online)) {
        mca_log_info("pen place err check timeout, pen place err: hall\n");
...
static void sc96231_init_fw_check_work(struct work_struct *work)
{
    struct sc96231 *chip = container_of(work, struct sc96231, init_fw_check_work.work);
    int ret = 0;
    u8 check_result = 0;
    int uisoc = 0, vbat = 0;
    ...
    if (!chip->batt_psy)
        ...
        uisoc = val.intval;
        power_supply_get_property(chip->batt_psy, POWER_SUPPLY_PROP_VOLTAGE_NOW, &val);
        vbat = val.intval;
    }

    if (chip->fw_update || chip->proc_data.fw_updating || (vbat < 3800000 || uisoc <= 5)) {
        mca_log_err("wls fw updating, no need to check fw, vbat:%d, uisoc:%d\n", vbat, uisoc);
        return;
    }
    ...
    if (check_result != (RX_CHECK_SUCCESS | TX_CHECK_SUCCESS | BOOT_CHECK_SUCCESS)) {
        ...
        ret = sc96231_download_fw(chip, false);
        if (ret < 0) {
            mca_log_info("fw download failed!\n");
            goto exit;
        }
    ...
    size_t count)
{
    ...
    if ((cmd > FW_UPDATE_NONE) && (cmd < FW_UPDATE_MAX)) {
        ret = sc96231_firmware_update_func(chip, cmd);
        if (ret < 0) {
...
static int sc96231_charger_probe(struct i2c_client *client)
{
    ...
    INIT_DELAYED_WORK(&chip->init_fw_check_work, sc96231_init_fw_check_work);

    sc96231_irq_init(chip);

    chip->chg_dev = wireless_charger_device_register("sc96231_tx",
            chip->dev, chip, &sc96231_wls_ops, &sc96231_chg_props);
    ...
    schedule_delayed_work(&chip->init_fw_check_work, msecs_to_jiffies(10 * 1000));
#endif
...
static void sc96231_charger_remove(struct i2c_client *client)
{
    ...
    mutex_destroy(&chip->hall_int_lock);
