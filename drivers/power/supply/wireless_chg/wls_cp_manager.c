...
static bool wls_check_psy(struct wls_cp_pm *wls_cm)
{
	...
	if (!wls_cm->usb_psy) {
		mca_log_err("failed to get usb_psy\n");
		return false;
	} else {
	...
	if (!wls_cm->wls_psy) {
		mca_log_err("failed to get wls_psy\n");
		return false;
	}
	...
	if (!wls_cm->bms_psy) {
		mca_log_err("failed to get bms_psy\n");
		return false;
	}
...
static bool wls_check_votable(struct wls_cp_pm *wls_cm)
{
	...
	if (!wls_cm->fv_votable) {
		mca_log_err("failed to get fv_votable\n");
		return false;
	}
...
static int wls_mode_switch_state_machine(struct wls_cp_pm *wls_cm)
{
	...
	if (wls_cm->adp_mode == FORWARD_2_1_CHARGER_MODE) {
		cp_mode = SC8561_FORWARD_2_1_CHARGER_MODE;
		goto exit;
	} else if (wls_cm->adp_mode == FORWARD_4_1_CHARGER_MODE) {
		cp_mode = SC8561_FORWARD_4_1_CHARGER_MODE;
		goto exit;
	}
...
static void wls_multi_mode_switch(struct wls_cp_pm *wls_cm)
{
	...
	wls_cm->entry_ibus = min(wls_cm->target_fcc / res, wls_cm->dts_config.max_ibus);
...
static bool wls_taper_charge(struct wls_cp_pm *wls_cm)
{
	...
	if (wls_cm->taper_count != cnt && cnt != 0 && wls_cm->support_pmic_parachg) {
		res = (wls_cm->cp_work_mode == SC8561_FORWARD_4_1_CHARGER_MODE) ? 2 : 3;
		para_icl = wls_cm->dts_config.pmic_parachg_icl * res;
		vote(wls_cm->icl_votable, WLS_PARACHG_VOTER, true, para_icl);
	}

exit:
	mca_log_err("taper:%d [soc:%d cv_vbat:%d cv_vote:%d cv_ibat:%d cnt:%d-%d full:%d icl:%d]\n",
		taper_chg, wls_cm->soc, cv_vbat, cv_vote, cv_ibat, cnt, wls_cm->taper_count, wls_cm->charge_full, para_icl);
...
static int wls_check_condition(struct wls_cp_pm *wls_cm)
{
	...
	if (wls_cm->state == WLS_PM_STATE_TUNE && wls_taper_charge(wls_cm)) {
		...
		return WLS_SM_HOLD;
	} else if (!is_between(MIN_JEITA_CHG_INDEX, MAX_JEITA_CHG_INDEX, wls_cm->jeita_chg_index)) {
...
static int wls_cm_tune_rx_vout(struct wls_cp_pm *wls_cm)
{
	...
	ibus_limit = min(wls_cm->target_fcc / res , wls_cm->dts_config.max_ibus);
...
static void wls_sm_stepper_pmic_icl(struct wls_cp_pm * wls_cm,
	int start_icl, int end_icl, int step_ma, int ms)
{
	int temp_icl = start_icl;
	vote(wls_cm->icl_votable, WLS_PARACHG_VOTER, true, temp_icl);
	if (start_icl < end_icl) {
		while (temp_icl < end_icl) {
			vote(wls_cm->icl_votable, WLS_PARACHG_VOTER, true, temp_icl);
			mca_log_err("limit icl:%dmA\n", temp_icl);
			temp_icl += step_ma;
			msleep(ms);
		}
	} else {
	...
	vote(wls_cm->icl_votable, WLS_PARACHG_VOTER, true, end_icl);
	return;
}
...
static bool wls_handle_sm(struct wls_cp_pm *wls_cm)
{
	...
	case WLS_PM_STATE_ENTRY:
		...
		if (wls_cm->sm_status == WLS_SM_EXIT) {
			...
			if (wls_cm->cp_work_mode == SC8561_FORWARD_2_1_CHARGER_MODE) {
				...
				charger_dev_cp_device_wls_chg_init(wls_cm->master_dev, CP_FORWARD_2_TO_1);
			} else {
				charger_dev_cp_set_mode(wls_cm->master_dev, SC8561_FORWARD_4_1_CHARGER_MODE);
				charger_dev_cp_device_wls_chg_init(wls_cm->master_dev, CP_FORWARD_4_TO_1);
			}
	...
	case WLS_PM_STATE_INIT_VBUS:
		wls_cm->sm_status = wls_check_condition(wls_cm);
		if (wls_cm->sm_status == WLS_SM_EXIT) {
			mca_log_err("WLS_PM_STATE_INIT_VBUS taper charge done\n");
			wls_move_sm(wls_cm, WLS_PM_STATE_EXIT);
			break;
		} else if (wls_cm->sm_status == WLS_SM_HOLD) {
			mca_log_err("WLS_PM_STATE_INIT_VBUS sm hold\n");
			wls_move_sm(wls_cm, WLS_PM_STATE_EXIT);
			break;
		}
		if(wls_cm->cp_work_mode == SC8561_FORWARD_2_1_CHARGER_MODE) {
			res = 2;
			pmic_icl = wls_cm->dts_config.pmic_parachg_icl * 2;
		} else {
			res = 4;
			pmic_icl = wls_cm->dts_config.pmic_parachg_icl;
		}
	...
	case WLS_PM_STATE_ENABLE_CP:
		wls_cm->sm_status = wls_check_condition(wls_cm);
		if (wls_cm->sm_status == WLS_SM_EXIT) {
			mca_log_err("WLS_PM_STATE_ENABLE_CP taper charge done\n");
			wls_move_sm(wls_cm, WLS_PM_STATE_EXIT);
			break;
		} else if (wls_cm->sm_status == WLS_SM_HOLD) {
			mca_log_err("WLS_PM_STATE_ENABLE_CP sm hold\n");
			wls_move_sm(wls_cm, WLS_PM_STATE_EXIT);
			break;
		}
		if(wls_cm->cp_work_mode == SC8561_FORWARD_2_1_CHARGER_MODE) {
		...
		if (wls_cm->master_cp_enable) {
			...
			cur_icl = get_effective_result(wls_cm->icl_votable);
			if (wls_cm->support_pmic_parachg) {
				mca_log_err("pmic decline charging current with %dmA\n", pmic_icl);
				wls_sm_stepper_pmic_icl(wls_cm, cur_icl, pmic_icl, 100, 20);
			}
	...
	case WLS_PM_STATE_EXIT:
		...
		charger_dev_enable_cp_ucp(wls_cm->master_dev, true);
		msleep(1000);
		charger_dev_reset_eoc_state(wls_cm->charger_dev);
		...
		if (wls_cm->charge_full) {
			...
			charger_dev_enable(wls_cm->charger_dev, true);
		charger_dev_cp_dump_register(wls_cm->master_dev);
		if (wls_cm->sm_status == WLS_SM_EXIT) {
		...
		break;
	default:
		mca_log_err("not supportted wls_sm_state\n");
		break;
	}
	return false;
}
	...
	if (!wls_handle_sm(wls_cm) && wls_cm->wls_cm_active) {
		if (wls_cm->no_delay) {
			...
			case WLS_PM_STATE_TUNE:
				...
				break;
			default:
				mca_log_err("not supportted pdm_sm_state\n");
				break;
			}
		}
...
static void wls_pm_disconnect(struct wls_cp_pm *wls_cm)
{
	...
	wls_cm->max_curr_chg_flag = false;
	wls_cm->fastchg_time_start = -1;
	wls_cm->adp_mode = FORWARD_4_1_CHARGER_MODE;
	if (!wls_cm->support_pmic_parachg)
		charger_dev_enable_pmic_ovp(wls_cm->master_dev, true);
	wls_move_sm(wls_cm, WLS_PM_STATE_ENTRY);
}
...
static void wls_psy_change(struct work_struct *work)
{
	...
	int ret = 0, val = 0;
	bool is_vout_setting_ready = false, pg_low_debunce = false;

	mca_log_info("wls_psy_change\n");

	ret = wls_get_property(WLS_PROP_QC_ENABLE, &val);
	if (ret < 0) {
		mca_log_err("failed to get qc enable\n");
		goto out;
	} else
	...
	if (ret < 0) {
		mca_log_err("failed to get tx adapter\n");
		goto out;
	} else
		wls_cm->wls_adapter_type = val;

	wireless_charger_dev_is_vout_range_set_done(wls_cm->wls_dev, &is_vout_setting_ready);
	wireless_charger_dev_wls_get_power_good_low_debounce(wls_cm->wls_dev, &pg_low_debunce);

	mca_log_err("adapter:%d, wls_qc_enable:%d, vout_range_ready:%d, pg_low_debunce:%d\n",
		wls_cm->wls_adapter_type, wls_cm->wls_qc_enable, is_vout_setting_ready, pg_low_debunce);
	...
	if ((!wls_cm->wls_cm_active) && wls_cm->wls_qc_enable && is_vout_setting_ready && (wls_cm->wls_adapter_type >= WLS_ADAPTER_XIAOMI_QC3)) {
		...
		mca_log_info("set main_sm_work\n");
		wls_move_sm(wls_cm, WLS_PM_STATE_ENTRY);
		schedule_delayed_work(&wls_cm->main_sm_work, msecs_to_jiffies(1000));
	} else if (wls_cm->wls_cm_active && (wls_cm->wls_adapter_type == WLS_ADAPTER_UNKNOWN || pg_low_debunce)) {
		wls_cm->wls_cm_active = false;
		mca_log_info("cancel wls state machine\n");
...
static ssize_t wls_store_log_level(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	mca_log_info("store log_level = %d\n", log_level);
	return count;
}
...
static ssize_t wls_show_switch2_1_enter(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	mca_log_info("show switch2_1_enter= %d\n", wls_cm->dts_config.switch2_1_enter);
...
static ssize_t wls_store_switch2_1_enter(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	mca_log_info("store switch2_1_enter = %d\n", wls_cm->dts_config.switch2_1_enter);
...
static ssize_t wls_show_switch2_1_exit(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	mca_log_info("show switch2_1_exit = %d\n", wls_cm->dts_config.switch2_1_exit);
...
static ssize_t wls_store_switch2_1_exit(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	mca_log_info("store switch2_1_exit = %d\n", wls_cm->dts_config.switch2_1_exit);
...
static int wls_cp_probe(struct platform_device *pdev)
{
	...
	if (!wls_check_cp_dev(wls_cm)) {
		mca_log_err("failed to check charger device\n");
		return -ENODEV;
	}
	if (!wls_check_psy(wls_cm)) {
		mca_log_err("failed to check psy\n");
		return -ENODEV;
	}
	if (!wls_check_votable(wls_cm)) {
		mca_log_err("failed to check votable\n");
		return -ENODEV;
	}
	...
	if (ret) {
		mca_log_err("failed to register sysfs\n");
		return ret;
	}

	mca_log_info("wls_cp probe success\n");
	return ret;
}
static void wls_cp_remove(struct platform_device *pdev)
{
