...
#ifdef READ_BYTE_USE_TIMESTAMPS
	memcpy(&sd->prevtv[0], &sd->tv[0], sizeof(sd->prevtv));
	sd->prevtvi = sd->tvi;
...
ssize_t stick_read_frame(struct stick_device * sd, uint8_t * buf)
{
	...
	END_UNINTERRUPTIBLE_SECTION
		...
		DEV_WARN(sd->dev,
			 ...
			 __func__, (int)ret, sd->bit_read, rcv_byte,
			 sd->last_cpu);
		dump_timestamps(sd);
		if (ret != ST1WIRE_OK) {
#ifdef READ_BYTE_USE_TIMESTAMPS
	...
	END_UNINTERRUPTIBLE_SECTION if (ret != ST1WIRE_OK) {
		DEV_WARN(sd->dev,
			 ...
			 __func__, (int)ret, sd->bit_read, frame_length,
			 sd->last_cpu);
		dump_timestamps(sd);
#ifdef READ_BYTE_USE_TIMESTAMPS
#ifdef TRY_CALIBRATION
		...
		for (i = 0; i < frame_length; i++) {
			...
			END_UNINTERRUPTIBLE_SECTION if (ret != ST1WIRE_OK) {
				DEV_WARN(sd->dev,
					 ...
					 __func__, i, frame_length, (int)ret,
					 sd->bit_read, buf[i],
					 sd->last_cpu);
				dump_timestamps(sd);
#ifdef READ_BYTE_USE_TIMESTAMPS
...
uint8_t page0_data[32];
static int st_get_battery_id(struct auth_device *auth_dev, u8 *bat_id)
{
	if (page0_data[0] == SWD_SN1 && page0_data[1] == SWD_SN2
		&& page0_data[5] == BATT_5500MAH)
		*bat_id = BATTERY_VENDOR_FIRST;
	else if (page0_data[0] == NVT_SN1 && page0_data[1] == NVT_SN2
		&& page0_data[5] == BATT_5500MAH)
		*bat_id = BATTERY_VENDOR_SECOND;
	else if (page0_data[0] == SWD_SN1 && page0_data[1] == SWD_SN2
		&& page0_data[5] == BATT_5110MAH)
		*bat_id = BATTERY_VENDOR_THIRD;
	else if (page0_data[0] == NVT_SN1 && page0_data[1] == NVT_SN2
		&& page0_data[5] == BATT_5110MAH)
		*bat_id = BATTERY_VENDOR_FOURTH;
	else
		*bat_id = BATTERY_VENDOR_UNKNOW;
	
	pr_err("page0_data:0x%x 0x%x 0x%x 0x%x 0x%x 0x%x batt_id:%d\n",
...
static int st_get_batt_sn(struct auth_device *auth_dev, u8 *soh_sn)
{
	int i = 0;

	memcpy(soh_sn, page0_data, 32);
	pr_err("%s batt_sn:\n", __func__);
	pr_err("%s %s\n", __func__, soh_sn);
	for (i = 0; i < 32; i++) {
...
extern stick_ReturnCode_t stickapitest_set_ui_soh(u8 *ui_soh_data, int len, int raw_soh);
extern stick_ReturnCode_t stickapitest_get_ui_soh(u8 *ui_soh_data, int len);
static int st_get_ui_soh(struct auth_device *auth_dev, u8 *ui_soh_data, int len)
{
	int ret = 0;
	ret = stickapitest_get_ui_soh(ui_soh_data, len);
	if (ret < 0) {
		pr_err("%s fail\n", __func__);
		return ret;
	}
...
static int st_set_ui_soh(struct auth_device *auth_dev, u8 *ui_soh_data, int len, int raw_soh)
{
	int ret = 0;
	ret = stickapitest_set_ui_soh(ui_soh_data, len, raw_soh);
	if (ret < 0) {
		pr_err("%s fail\n", __func__);
		return ret;
	}
...
static struct auth_ops st_auth_ops = {
	...
	.get_batt_sn = st_get_batt_sn,
	.get_ui_soh = st_get_ui_soh,
	.set_ui_soh = st_set_ui_soh,
};
...
static int stick_platform_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR_OR_NULL(sd->auth_dev)) {
		DEV_ERR(sd->dev, "auth device register fail\n");
		return -ENODEV;
	}
