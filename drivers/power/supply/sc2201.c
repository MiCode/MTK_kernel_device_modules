...
static int sc2201_charger_set_usbsw(struct sc2201_chip *sc, enum sc2201_usbsw usbsw)
{
	...
	int typec_port1_after_status = 0;
	usb_get_property(USB_PROP_TYPEC_PORT0_PLUGIN, &typec_port0_status);
	usb_get_property(USB_PROP_TYPEC_PORT1_AFTER_PLUGIN, &typec_port1_after_status);
	if (typec_port0_status == 3 || typec_port1_after_status == 1)
		return 0;
...
int sc2201_inter_force_dpdm(struct sc2201_chip *sc) {
	int ret;
...
EXPORT_SYMBOL(sc2201_inter_force_dpdm);
...
int sc2201_get_charger_type(struct sc2201_chip *sc) {
	...
	mca_log_err("sc bc12 type: 0x%02x\n", reg_val);
	return ret;
}
...
{
	static const int max_wait_cnt = 50;
	int i = 0, ret = 0;
	bool en = true;
	...
	if (i == max_wait_cnt) {
		mca_log_err("%s, CDP timeout\n", __func__);
		en = false;
	} else
		mca_log_err("%s, CDP free\n", __func__);
...
int sc2201_rerun_bc12(struct sc2201_chip *sc)
{
	int ret = 0, reg_val = 0;

	sc2201_info("%s entry", __func__);

	ret = sc2201_field_read(sc, EN_BC1P2, &reg_val);
	if (ret) {
		sc2201_err("%s read en BC1P2 flag err %d\n", __func__, ret);
		return ret;
	} else
		sc2201_info("%s read en BC1P2 flag %d\n", __func__, reg_val);
...
EXPORT_SYMBOL(sc2201_rerun_bc12);
...
static irqreturn_t sc2201_irq_handler(int irq, void *data)
{
	...
	if (delayed_work_pending(&sc->bc12_timeout_dwork)) {
		...
		if (reg_val == 1) {
			...
			//cancel_delayed_work_sync(&sc->bc12_timeout_dwork);
			sc2201_info( "%s bc12 handler timeout \n", __func__);
			mutex_lock(&sc->bc_detect_lock);
			sc->bc12_detect = false;
			sc->bc12_detect_done = true;
			mutex_unlock(&sc->bc_detect_lock);
...
static int sc2201_irq_register(struct sc2201_chip *sc)
{
	...
	ret = gpio_request_one(sc->irq_gpio, GPIOF_IN, "sc2201_irq");
	if (ret) {
...
//		mca_log_err("PD connected, NO BC12\n");
//		return;
//	}
...
int xm_get_chg_type(struct sc2201_chip *sc){
	...
	case BC12_TYPE_NO_INPUT:
		...
		break;
	case BC12_TYPE_SDP:
		sc->psy_desc_type = POWER_SUPPLY_TYPE_USB;
		break;
	case BC12_TYPE_CDP:
...
static int xmusb350_init_psy(struct sc2201_chip *chip)
{
	...
	chip->xmusb350_psy_desc.usb_types = BIT(POWER_SUPPLY_USB_TYPE_UNKNOWN) |
	...
	chip->xmusb350_psy_desc.get_property = xmusb350_get_property, 	chip->xmusb350_psy_desc.set_property = xmusb350_set_property,
