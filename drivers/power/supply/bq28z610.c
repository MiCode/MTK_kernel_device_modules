...
static int fg_read_word(struct bq_fg_chip *bq, u8 reg, u16 *val)
{
	...
	if (ret) {
		mca_log_err("I2C failed to read 0x%02x\n", reg);
		return ret;
	}
...
static int fg_write_block(struct bq_fg_chip *bq, u8 reg, u8 *data, u8 len)
{
	...
	for (i = 0; i < len; i++) {
		...
		if (ret) {
			mca_log_err("I2C failed to write 0x%02x\n", reg + i);
			return ret;
		}
	}
...
static int bq_battery_soc_smooth_tracking_new(struct bq_fg_chip *bq, int raw_soc, int batt_soc, int batt_ma)
{
	...
	if (bq->rsoc == 0 && ((bq->vbat < bq->normal_shutdown_vbat && bq->tbat > 0) || (bq->vbat < bq->normal_shutdown_vbat && bq->tbat < 0))) {
		bq->vbatt_empty = true;
		unit_time = 2000;
		bq->monitor_delay = FG_MONITOR_DELAY_2S;
...
static int fg_calc_shutdown_avgcurrent(struct bq_fg_chip *bq)
{
	static int cur_ibat[10] = {0};
	static int current_index = 0;
	static int last_chg_status = POWER_SUPPLY_STATUS_CHARGING;
	static ktime_t last_chg_change_time = 0;
	int avg_cur = 0;
	int sum = 0;
	union power_supply_propval pval = {0, };
	ktime_t now = ktime_get_boottime();
	bool in_cooldown = true;
	...
	if (last_chg_status != pval.intval) {
		last_chg_change_time = now;
		last_chg_status = pval.intval;
		fg_err("%s: charge status changed to %d\n", __func__, pval.intval);
	}

	in_cooldown = (last_chg_status == POWER_SUPPLY_STATUS_NOT_CHARGING) &&
					(ktime_ms_delta(now, last_chg_change_time) < bq->calc_shutdown_vol_after_charging_time_s * 1000);

	if (bq->temp_state >= BAT_COOL || bq->ui_soc >= bq->calc_shutdown_vol_soc || pval.intval == POWER_SUPPLY_STATUS_CHARGING || in_cooldown) {
		memset(cur_ibat, 0, sizeof(cur_ibat));
		current_index = 0;
		fg_err("%s: no need calc_avgcurr, temp_state=%d, ui_soc=%d, chg_status=%d, in_cooldown=%d\n", __func__, bq->temp_state, bq->ui_soc, pval.intval, in_cooldown);
		return bq->calc_shutdown_vol_current + 1;
	}

	if (current_index < 10) {
		cur_ibat[current_index++] = bq->ibat;
		return bq->calc_shutdown_vol_current + 1;
	} else {
		memmove(cur_ibat, cur_ibat + 1, sizeof(int) * 9);
		cur_ibat[9] = bq->ibat;
		for (int i = 0; i < current_index; i++) {
			sum += cur_ibat[i];
			fg_err("%s: cur%d=%d\n", __func__, i, cur_ibat[i]);
		}
		avg_cur = sum / 10;
		fg_err("%s: avg_cur=%d\n", __func__, avg_cur);
	}
...
static int fg_count_shutdown_vol_dash_cn(struct bq_fg_chip *bq, int count)
{
	int count_vol = 0;
	int avg_cur = 0;

	avg_cur = fg_calc_shutdown_avgcurrent(bq);
	switch(bq->temp_state) {
		case BAT_COLD:
			if (avg_cur > bq->calc_shutdown_vol_current) {
				if (count < 90) {
					count_vol = 2800;
				} else if (count >= 90 && count < 169) {
					count_vol = 2800;
				} else if (count >= 169 && count < 452) {
					count_vol = 2800;
				} else if (count >= 452 && count < 1051) {
					count_vol = 2850;
				} else {
					count_vol = 2900;
				}
...
static int fg_cycle_shutdown_vol_dash_cn(struct bq_fg_chip *bq)
{
	int cycle_vol = 0;
...
static void battery_shutdown_vol_update(struct bq_fg_chip *bq)
{
	...
	u8 data[2] = {0xff, 0xff};
	int count;
	int count_3v0 = 0, count_3v2 = 0, count_3v4 = 0;
	...
	if (bq->fake_dod_count) {
		...
		count = count_3v0*20/10 + count_3v2*18/10 + count_3v4*15/10;
	}
	bq->dod_count = count;

	run_shutdown_temp_state_machine(bq);
	count_vol = fg_count_shutdown_vol_dash_cn(bq, count);
	cycle_vol = fg_cycle_shutdown_vol_dash_cn(bq);
	shutdown_vol = max(count_vol, cycle_vol);
	mca_log_err("count=%d,%d,%d,%d vol=%d,%d,%d\n",
			count, count_3v0, count_3v2, count_3v4,
			shutdown_vol, count_vol, cycle_vol);


	term_vol = shutdown_vol + 100;
	if (term_vol != bq->term_vol) {
		bq->term_vol = term_vol;
		data[0] = term_vol & 0xFF;
		data[1] = (term_vol >> 8) & 0xFF;
		ret = fg_mac_write_block(bq, FG_MAC_CMD_MIX_TERM_VOLT, data, 2);
		if (ret) {
			mca_log_err("dynamic_shutdown_vol failed to write FG_MAC_CMD_MIX_TERM_VOLT\n");
		}

		ret = fg_mac_read_block(bq, FG_MAC_CMD_MIX_TERM_VOLT, t_buf, 2);
		mca_log_err("dynamic_shutdown_vol term_vol=%d t_buf[0]=0x%02x t_buf[1]=0x%02x FG_MAC_CMD_MIX_TERM_VOLT=%d\n",
			term_vol, t_buf[0], t_buf[1], t_buf[1] << 8 | t_buf[0]);
	}
