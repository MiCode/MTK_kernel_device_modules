...
static int parse_dod_shutdown_voltage_dts(struct bq_fg_chip *bq, int temp_range);
static int parse_cycle_shutdown_voltage_dts(struct bq_fg_chip *bq, int temp_range);
static int parse_cold_dod_shutdown_voltage_dts(struct bq_fg_chip *bq, int temp_range);
...
static int fg_read_byte(struct bq_fg_chip *bq, u8 reg, u8 *val)
{
    ...
    ret = regmap_read(bq->regmap, reg, &data);
    if (ret) {
...
static int fg_write_byte(struct bq_fg_chip *bq, u8 reg, u8 val)
{
    ...
    mutex_lock(&bq->i2c_rw_lock);
    ret = regmap_write(bq->regmap, reg, (unsigned int)val);
    if (ret) {
        mutex_unlock(&bq->i2c_rw_lock);
...
static int fg_read_block(struct bq_fg_chip *bq, u8 reg, u8 *buf, u8 len)
{
    ...
    ret = regmap_raw_read(bq->regmap, reg, buf, len);
    if (ret) {
            mca_log_err("%s I2C failed to read 0x%02x\n", bq->log_tag, reg);
            return ret;
    }
...
static int fg_write_block(struct bq_fg_chip *bq, u8 reg, u8 *data, u8 len)
{
    ...
    ret = regmap_raw_write(bq->regmap, reg, data, len);
    if (ret) {
            mca_log_err("%s I2C failed to write 0x%02x\n", bq->log_tag, reg);
            return ret;
    }
...
static int fg_mac_read_block(struct bq_fg_chip *bq, u16 cmd, u8 *buf, u8 len)
{
    ...
    if (cmd == FG_MAC_CMD_IF_SIG)
    {
        msleep(1100); // 1100ms
        mca_log_err("%s cmd=0x%04x, delay 1100ms\n", bq->log_tag, cmd);
    }
...
static int fg_mac_write_block(struct bq_fg_chip *bq, u16 cmd, u8 *data, u8 len)
{
    ...
    u8 t_buf[40];
    int i;
    bool secure_delay_100_ms = false;
    bool secure_delay_300_ms = false;

    if (len > 32)
        return -1;

    if (cmd > FG_MAC_CMD_UPDATE_VERSION || cmd == FG_MAC_CMD_UI_SOH || cmd == FG_MAC_CMD_RECORD_VOLTAGE_LEVEL)
        secure_delay_100_ms = true;
    else if (cmd == FG_MAC_CMD_CTRL_STATUS)
        secure_delay_300_ms = true;
    ...
    if (secure_delay_100_ms)
        msleep(100);
    else if (secure_delay_300_ms)
        msleep(300);
    mutex_unlock(&bq->i2c_rw_lock);
...
static int fg_sha256_auth(struct bq_fg_chip *bq, u8 *challenge, int length)
{
    ...
    ret = fg_mac_write_block(bq, FG_MAC_CMD_CTRL_STATUS, challenge, length);
    if (ret) {
        mca_log_err("%s failed to write CTRL_STATUS ret=%d\n", bq->log_tag, ret);
        return ret;
    }

   ret = fg_mac_read_block(bq, FG_MAC_CMD_CTRL_STATUS, bq->digest, length);
    if (ret) {
        mca_log_err("%s failed to read CTRL_STATUS, ret=%d\n", bq->log_tag, ret);
    }
    return 0;
}
...
static int fg_read_ntc_cac(struct bq_fg_chip *bq)
{
    int ret;
    u16 ave_curr;

    if (bq->regs[BQ_FG_REG_NTC_CAC] == INVALID_REG_ADDR) {
        fg_err("BQ_FG_REG_NTC_CAC command not supported!\n");
        return 0;
    }

    ret = fg_read_word(bq, bq->regs[BQ_FG_REG_NTC_CAC], &ave_curr);

    if (ret < 0)
        fg_err("could not read BQ_FG_REG_NTC_CAC, ret=%d\n", ret);
...
static int fg_read_real_temp(struct bq_fg_chip *bq)
{
    ...
    ret = fg_read_word(bq, bq->regs[BQ_FG_REG_REAL_TEMP], &tbat);
    if (ret < 0) {
...
static int fg_read_vsiloss(struct bq_fg_chip *bq)
{
    u8 data[32] = {0};
    int ret = 0;
    int vsiloss = 0;
    ...
    if (bq->fake_vsiloss) {
        return bq->fake_vsiloss;
    }

    ret = fg_mac_read_block(bq, FG_MAC_CMD_V_SILOSS, data, 32);
    if (ret < 0) {
        mca_log_err("could not read vsiloss = %d\n", ret);
        return ret;
    }
    mca_log_err("t_buf[0]=0x%02x t_buf[1]=0x%02x vsiloss=%d\n",
        data[0], data[1], data[1] << 8 | data[0]);

    vsiloss = (data[1] << 8) | data[0];
    return vsiloss;
}

static int fg_read_iaver(struct bq_fg_chip *bq)
{
    u8 data[32] = {0};
    int ret = 0;
    int iaver = 0;
    ...
    if (bq->fake_iaver) {
        return bq->fake_iaver;
    }

    ret = fg_mac_read_block(bq, FG_MAC_CMD_I_AVER, data, 32);
    if (ret < 0) {
        mca_log_err("could not read i_aver = %d\n", ret);
        return ret;
    }
    mca_log_err("t_buf[2]=0x%02x t_buf[3]=0x%02x iaver=%d\n",
        data[2], data[3], data[3] << 8 | data[2]);

    iaver = (short)((data[3] << 8) | data[2]);
    return iaver;
}
...
static void fg_read_qmax(struct bq_fg_chip *bq)
{
    ...
    if (bq->device_name == BQ_FG_BQ27Z561 || bq->device_name == BQ_FG_NFG1000A || bq->device_name == BQ_FG_NFG1000B || bq->device_name == BQ_FG_MPC7012B) {
...
static int get_param_index(struct shutdown_voltage_param *param, int value, int max_length)
{
    int i = 0, index = 0;

    if (value < param[0].low) {
        index = 0;
        return index;
    }

    if (value > param[max_length - 1].high) {
        index = max_length - 1;
        return index;
    }

    for (i = 0; i < max_length; i++) {
        if (is_between(param[i].low, param[i].high, value)) {
            index = i;
            break;
        }
    }

    mca_log_err("value = %d, index = %d\n", value, index);
    return index;
}

static void get_temp_index(struct temp_range *param, int fallback_hyst, int forward_hyst, int value, int *index, bool ignore_hyst)
{
	int new_index = 0, i = 0;

	//mca_log_err("value = %d, index[0] = %d, index[1] = %d\n", value, index[0], index[1]);
	...
	if (value > param[SHUTDOWN_TUPLE_COUNT - 1].high)
		new_index = SHUTDOWN_TUPLE_COUNT - 1;

	for (i = 0; i < SHUTDOWN_TUPLE_COUNT; i++) {
		if (is_between(param[i].low, param[i].high, value)) {
			new_index = i;
			break;
		}
	...
	if (ignore_hyst) {
		...
		if (new_index > index[0]) {
			if (value < (param[new_index].low + forward_hyst))
				new_index = index[0];
		} else if (new_index < index[0]) {
			if (value > (param[new_index].high - fallback_hyst))
				new_index = index[0];
		}
		index[1] = index[0];
		index[0] = new_index;
	...
	mca_log_err("value = %d, index[0] = %d, index[1] = %d, new_index = %d\n", value, index[0], index[1], new_index);
	return;
}

#define IAVER_ENABLE_TIME 64800000 //18H
static void battery_shutdown_vol_v2(struct bq_fg_chip *bq)
{
    int V_siloss = 0, V_Rt = 0, delta_v = 0, I_aver = 0;
    int dod_index = 0, new_dod_index = 0;
    int i = 0;
    ...
    if (!bq->dynamic_shutdown_voltage_support_v2) {
        return;
    }

    if (cyclecount != bq->cycle_count) {
        cyclecount = bq->cycle_count;
        /* get [10-60C] dod para */
        parse_dod_shutdown_voltage_dts(bq, 3);
        dod_index = get_param_index(bq->dod_param, bq->dod_count, bq->dod_param_length);
        V_Rt = bq->dod_param[dod_index].termVolt;
        V_siloss = fg_read_vsiloss(bq);
        bq->vsiloss = V_siloss;
        delta_v = (V_Rt + V_siloss) / 2 - V_Rt;
        mca_log_err("V_Rt:%d, V_siloss:%d, delta_v:%d", V_Rt, V_siloss, delta_v);
        if (delta_v >= 0 && delta_v <= 100) {
            V_Rt += delta_v;
            delta_v = abs(bq->dod_param[dod_index].termVolt - V_Rt);
            for (i = dod_index + 1;i < bq->dod_param_length; i++) {
                if (abs(bq->dod_param[i].termVolt - V_Rt) < delta_v) {
                    new_dod_index = i;
                    bq->siloss_gear = new_dod_index - dod_index;
                    break;
                }
            }
            mca_log_err("siloss_gear:%d,index:%d,%d", bq->siloss_gear, dod_index, new_dod_index);
        }
        parse_dod_shutdown_voltage_dts(bq, bq->temp_dod_index[0]);
    }
    ...
    if (I_aver >= -300 && I_aver <= 0 && !bq->iaver_time_flag
        ...
        bq->iaver_time_flag = true;
        bq->iaver_time = ktime_get_boottime();
    } else if (I_aver < -300 || I_aver > 0) {
        bq->iaver_time_flag = false;
    }
    delta_time = ktime_ms_delta(ktime_get_boottime(), bq->iaver_time);
    if (bq->iaver_time_flag && delta_time >= IAVER_ENABLE_TIME) {
        if (bq->rsoc <= 50 && bq->rsoc >= 10) {
            bq->iaver_gear = 1;
        }
    } else {
        bq->iaver_gear = 0;
    }
    mca_log_err("I_aver: %d, iaver_time_flag: %d, iaver_gear: %d, delta_time:%d", I_aver, bq->iaver_time_flag, bq->iaver_gear, delta_time);

    if(bq->rsoc > 50) {
        bq->shutdown_vol_delay = 600000;
    } else {
        bq->shutdown_vol_delay = 30000;
    }
}

#define LOW_CUR_COUNT_MAX   10
static bool battery_cold_shutdown_vol(struct bq_fg_chip *bq)
{
    int i_aver = 0;
    int delta_time = 0;
    union power_supply_propval pval = {0, };
    int charging_status = POWER_SUPPLY_STATUS_DISCHARGING;
    static bool dischg_iaver_time_flag = false;
    static ktime_t dischg_iaver_time = 0;
    static int low_current_count = 0;
    int rc = 0;
    ...
    charging_status = pval.intval;
    if (charging_status == POWER_SUPPLY_STATUS_DISCHARGING || charging_status == POWER_SUPPLY_STATUS_NOT_CHARGING) {
        if (!dischg_iaver_time_flag) {
            dischg_iaver_time_flag = true;
            dischg_iaver_time = ktime_get_boottime();
        }
    }
    else {
        dischg_iaver_time_flag = false;
        return false;
    }

    delta_time = (ktime_ms_delta(ktime_get_boottime(), dischg_iaver_time)) / MSEC_PER_SEC;
    if ((dischg_iaver_time_flag && delta_time < bq->cold_shutdown_vol_after_charging_time_s)
        || (bq->rsoc >= bq->cold_shutdown_vol_soc)){
        mca_log_err("no need raise shutdown_vol, rsoc: %d, cold_shutdown_vol_soc: %d, dischg_flag: %d, delta_time:%d",
            bq->rsoc, bq->cold_shutdown_vol_soc, dischg_iaver_time_flag, delta_time);
        low_current_count = 0;
        return false;
    }

    i_aver = -fg_read_iaver(bq);
    if (i_aver <= bq->cold_shutdown_vol_current && i_aver >= 0 && low_current_count < LOW_CUR_COUNT_MAX)
        low_current_count++;
    else if (i_aver > bq->cold_shutdown_vol_current)
        low_current_count = 0;

    mca_log_err("i_aver: %d, cold_shutdown_vol_current: %d, low_current_count: %d",
        i_aver, bq->cold_shutdown_vol_current, low_current_count);

    return (low_current_count >= LOW_CUR_COUNT_MAX);
}
...
static void battery_shutdown_vol_update(struct bq_fg_chip *bq)
{
    ...
    int temp_dod_index = bq->temp_dod_index[0];
    int temp_cycle_index = bq->temp_cycle_index[0];
    int temp_cold_dod_index = bq->temp_cold_dod_index[0];
    u16 addr = bq->device_name == BQ_FG_MPC7012B ? FG_MAC_CMD_MIX_TERM_VOLT_G2 : FG_MAC_CMD_MIX_TERM_VOLT;

    if (bq->dynamic_shutdown_voltage_support || bq->dynamic_shutdown_voltage_support_v2) {
        ...
        /* susport vsiloss and iaver compensation*/
        battery_shutdown_vol_v2(bq);

        /* support dynamic shutdown voltage */
        get_temp_index(bq->dod_temp_range, bq->temp_hyst, bq->temp_hyst, bq->tbat, bq->temp_dod_index, false);
        get_temp_index(bq->cycle_temp_range, bq->temp_hyst, bq->temp_hyst, bq->tbat, bq->temp_cycle_index, false);
        if (temp_dod_index != bq->temp_dod_index[0]) {
            parse_dod_shutdown_voltage_dts(bq, bq->temp_dod_index[0]);
        }
        if (temp_cycle_index != bq->temp_cycle_index[0]) {
            parse_cycle_shutdown_voltage_dts(bq, bq->temp_cycle_index[0]);
        }

        bq->dod_index = get_param_index(bq->dod_param, count, bq->dod_param_length);
        bq->dod_index = bq->dod_index + bq->siloss_gear + bq->iaver_gear;
        if (bq->dod_index > bq->dod_param_length - 1) {
            bq->dod_index = bq->dod_param_length - 1;
        }
        bq->cycle_index = get_param_index(bq->cycle_param, bq->cycle_count, bq->cycle_param_length);

        shutdown_vol = max(bq->dod_param[bq->dod_index].shutdownVolt, bq->cycle_param[bq->cycle_index].shutdownVolt);
        delay_vol = max(bq->dod_param[bq->dod_index].shutdownDelayVolt, bq->cycle_param[bq->cycle_index].shutdownDelayVolt);
        term_vol = max(bq->dod_param[bq->dod_index].termVolt, bq->cycle_param[bq->cycle_index].termVolt);

        mca_log_err("count=%d,%d,%d,%d tbat=%d, cyc=%d, vol=%d,%d,%d\n",
                count, count_3v0, count_3v2, count_3v4,
                bq->tbat, bq->cycle_count,
                shutdown_vol, delay_vol, term_vol);

        mca_log_err("dod_index=%d,%d,%d\n", bq->temp_dod_index[0], bq->temp_dod_index[1], temp_dod_index);

        /* support boost voltage when below zero and small current*/
        if (bq->cold_dynamic_shutdown_voltage_support && bq->tbat <= BATT_COLD_THRESHOLD && battery_cold_shutdown_vol(bq)) {
            get_temp_index(bq->dod_cold_temp_range, bq->temp_hyst, bq->temp_hyst, bq->tbat, bq->temp_cold_dod_index, false);
            if (temp_cold_dod_index != bq->temp_cold_dod_index[0]) {
                parse_cold_dod_shutdown_voltage_dts(bq, bq->temp_cold_dod_index[0]);
            }

            bq->dod_index = get_param_index(bq->cold_dod_param, count, bq->cold_dod_param_length);
            shutdown_vol = max(bq->cold_dod_param[bq->dod_index].shutdownVolt, shutdown_vol);
            delay_vol = max(bq->cold_dod_param[bq->dod_index].shutdownDelayVolt, delay_vol);
            term_vol = max(bq->cold_dod_param[bq->dod_index].termVolt, term_vol);

            mca_log_err("count=%d,%d,%d,%d tbat=%d, cyc=%d, vol=%d,%d,%d, cold_dod_index=%d,%d,%d\n",
                    count, count_3v0, count_3v2, count_3v4,
                    bq->tbat, bq->cycle_count,
                    shutdown_vol, delay_vol, term_vol,
                    bq->temp_cold_dod_index[0], bq->temp_cold_dod_index[1], temp_cold_dod_index);
        }
        ...
        if (shutdown_vol != bq->shutdown_voltage) {
            ...
            ret = fg_mac_write_block(bq, addr, data, 2);
            if (ret) {
                fg_info("%s:dynamic_shutdown_vol failed to write FG_MAC_CMD_MIX_TERM_VOLT\n", __func__);
            }

            ret = fg_mac_read_block(bq, addr, t_buf, 2);
            mca_log_err("dynamic_shutdown_vol t_buf[0]=0x%02x t_buf[1]=0x%02x FG_MAC_CMD_MIX_TERM_VOLT=%d\n",
                    t_buf[0], t_buf[1], t_buf[1] << 8 | t_buf[0]);
        }

        bq->shutdown_voltage = shutdown_vol;
        bq->normal_shutdown_vbat = bq->shutdown_voltage;
        bq->critical_shutdown_vbat = delay_vol;
        bq->cool_critical_shutdown_vbat = delay_vol;
        bq->old_critical_shutdown_vbat = delay_vol;
    } else {
        shutdown_vol = 3200;
        bq->shutdown_voltage = shutdown_vol;
        bq->normal_shutdown_vbat = bq->shutdown_voltage;
    ...
    mca_log_err("shutdown voltage=%d,%d,%d,%d,%d \n", bq->shutdown_voltage, bq->normal_shutdown_vbat,
        bq->critical_shutdown_vbat, bq->cool_critical_shutdown_vbat, bq->old_critical_shutdown_vbat);
}

static void battery_shutdown_vol_update_workfunc(struct work_struct *work)
{
    struct bq_fg_chip *bq = container_of(work, struct bq_fg_chip, battery_shutdown_vol_update_work.work);

    battery_shutdown_vol_update(bq);

    schedule_delayed_work(&bq->battery_shutdown_vol_update_work, msecs_to_jiffies(bq->shutdown_vol_delay));
}
...
static void fg_update_status(struct bq_fg_chip *bq)
{
    ...
    fg_read_volt(bq);
    fg_read_status(bq);
    if (bq->dynamic_shutdown_voltage_support) {
        battery_shutdown_vol_update(bq);
    }
...
out:
        mca_log_err("[FG_STATUS] [UISOC RSOC RAWSOC TEMP_SOC SOH UISOH] = [%d %d %d %d %d %d], [VBAT CELL0 CELL1 IBAT TBAT FC FAST_MODE] = [%d %d %d %d %d %d %d], [RM FCC] = [%d %d]\n",
            bq->ui_soc, bq->rsoc, bq->raw_soc, temp_soc, bq->soh, bq->ui_soh, bq->vbat, bq->cell_voltage[0], bq->cell_voltage[1], bq->ibat, bq->tbat, bq->batt_fc, bq->fast_chg, bq->rm, bq->fcc);
...
static int vsiloss_set(struct bq_fg_chip *gm,
    struct mtk_bms_sysfs_field_info *attr,
    int val)
{
    if (gm)
...
static int vsiloss_get(struct bq_fg_chip *gm,
    struct mtk_bms_sysfs_field_info *attr,
    int *val)
{
    if (gm) {
...
static int iaver_get(struct bq_fg_chip *gm,
    struct mtk_bms_sysfs_field_info *attr,
    int *val)
{
    if (gm) {
...
static int iaver_set(struct bq_fg_chip *gm,
    struct mtk_bms_sysfs_field_info *attr,
    int val)
{
    if (gm)
...
static int ntc_cac_get(struct bq_fg_chip *gm,
        struct mtk_bms_sysfs_field_info *attr,
        int *val)
{
    if (gm)
        *val = fg_read_ntc_cac(gm);
    else
        *val = 0;
    fg_err("%s %d\n", __func__, *val);
    return 0;
...
int bms_get_batt_sn(struct power_supply *psy, char *buf)
{
    ...
    gm = (struct bq_fg_chip *)power_supply_get_drvdata(psy);
    if (gm == NULL) {
        fg_err("failed to get gm\n");
        return -1;
    }

    ret = fg_mac_read_block(gm, FG_MAC_CMD_BATT_SN, t_buf, 32);
    if (ret < 0) {
...
static int fg_show_calc_rvalue(struct bq_fg_chip *bq)
{
    ...
    if (bq->device_name != BQ_FG_MPC7012B) {
        K1 = 4500;
        K2 = 4500;
        K3 = 7000;
        K4 = 4500;
    ...
    if (bq->device_name == BQ_FG_MPC7012B)
        ret = fg_mac_read_block(bq, FG_MAC_CMD_CALC_RVALUE_G2, data_buf, 32);
    else
        ret = fg_mac_read_block(bq, FG_MAC_CMD_CALC_RVALUE, data_buf, 32);
...
static int support_batt_manager_4p0_get(struct bq_fg_chip *gm,
    struct mtk_bms_sysfs_field_info *attr,
    int *val)
{
    if (gm->device_name != BQ_FG_MPC7012B) {
        *val = 0;
    } else {
        *val = 1;
    }
...
static int ui_soh_get(struct bq_fg_chip *gm)
{
    ...
    ret = fg_mac_read_block(gm, FG_MAC_CMD_UI_SOH, ui_soh_data, 32);
    if (ret < 0)
    {
    ...
    int *val)
{
    ...
    if (gm) {
        if (gm->device_name == BQ_FG_MPC7012B)
            ret = fg_mac_read_block(gm, FG_MAC_CMD_MIX_TERM_VOLT_G2, t_buf, 2);
        else
    ...
    int val)
{
    ...
    if (gm) {
        ...
        data[1] = (term_vol >> 8) & 0xFF;
        if (gm->device_name == BQ_FG_MPC7012B)
            ret = fg_mac_write_block(gm, FG_MAC_CMD_MIX_TERM_VOLT_G2, data, 2);
        else
    ...
    int *val)
{
    ...
    if (gm) {
        ret = fg_mac_read_block(gm, FG_MAC_CMD_DF_SIGN, t_buf, 2);
        if (ret) {
            fg_err("failed to read FG_MAC_CMD_DF_SIGN\n");
            return 0;
        }
    ...
    int *val)
{
    ...
    if (gm) {
        ret = fg_mac_read_block(gm, FG_MAC_CMD_CHEM_DF_SIGN, t_buf, 2);
        if (ret) {
            fg_err("failed to read FG_MAC_CMD_CHEM_DF_SIGN\n");
            return 0;
        }
        fg_info("%s:t_buf[0]=0x%02x t_buf[1]=0x%02x val=%02x\n", __func__,
            t_buf[0], t_buf[1], t_buf[0] << 8 | t_buf[1]);
...
static int app_checksum_get(struct bq_fg_chip *gm,
    struct mtk_bms_sysfs_field_info *attr,
    int *val)
{
    u8 t_buf[4] = {0};
    ...
    if (gm) {
        ret = fg_mac_read_block(gm, FG_MAC_CMD_IF_SIG, t_buf, 2);
        if (ret) {
            fg_err("failed to read FG_MAC_CMD_IF_SIG\n");
            return 0;
        }
        mca_log_err("read buf is 0x%02x 0x%02x  app_val = 0x%02x\n", t_buf[0], t_buf[1], t_buf[0] << 8 | t_buf[1]);
        *val = t_buf[0] << 8 | t_buf[1];
     } else {
        *val = 0;
     }
        ...
        struct device_attribute *attr, char *buf)
{
    ...
    if (usb_attr->prop >= BMS_PROP_DF_CHECK && usb_attr->prop <= BMS_PROP_APP_CHECKSUM) {
    ...
    int *val)
{
    ...
    if (gm->device_name == BQ_FG_MPC7012B)
        ret = fg_mac_read_block(gm, FG_MAC_CMD_OVER_VOL_DURATION_G2, t_buf, 12);
    else
        ret = fg_mac_read_block(gm, FG_MAC_CMD_OVER_VOL_DURATION, t_buf, 12);
    if (ret)
        fg_err("failed to get FG_MAC_CMD_OVER_VOL_DURATION\n");
    *val = (t_buf[11] << 24) | (t_buf[10] << 16) | (t_buf[9] << 8) | (t_buf[8]);
    ...
    int *val)
{
    ...
    if(gm->device_name != BQ_FG_NFG1000A && gm->device_name != BQ_FG_NFG1000B && gm->device_name != BQ_FG_MPC7012B)
    {
...
static struct mtk_bms_sysfs_field_info bms_sysfs_field_tbl[] = {
    ...
    BMS_SYSFS_FIELD_RO(support_batt_manager_4p0, BMS_PROP_SUPPORT_BATT_MANAGER_4P0),
    ...
    BMS_SYSFS_FIELD_RW(vsiloss, BMS_PROP_VSILOSS),
    BMS_SYSFS_FIELD_RW(iaver, BMS_PROP_IAVER),
...
static int fg_get_property(struct power_supply *psy, enum power_supply_property psp, union power_supply_propval *val)
{
    ...
    case POWER_SUPPLY_PROP_CAPACITY:
        ...
        if (bq->enable_shutdown_delay) {
            ...
            if (last_shutdown_delay != bq->shutdown_delay || (val->intval == 0 && last_ui_soc != 0)) {
                last_shutdown_delay = bq->shutdown_delay;
                last_ui_soc = val->intval;
                if (bq->fg_psy)
...
int bms_get_manufacturing_date(struct power_supply *psy, char *buf)
{
    struct bq_fg_chip *gm;
    u8 t_buf[64] = { 0 };
    int ret = 0;
...
EXPORT_SYMBOL(bms_get_manufacturing_date);

int bms_get_first_usage_date(struct power_supply *psy, char *buf)
{
    struct bq_fg_chip *gm;
    u8 t_buf[64] = { 0 };
    int ret;
    ...
    ret = fg_mac_read_block(gm, FG_MAC_CMD_UI_SOH, t_buf, 32);
    if (ret < 0) {// read error, show 9999999
        memcpy(buf,"99999999",8);
        fg_err("failed to get first_usage_date\n");
        return ret;
...
EXPORT_SYMBOL(bms_get_first_usage_date);

int bms_set_first_usage_date(struct power_supply *psy, char *buf, size_t size)
{
    struct bq_fg_chip *gm;
    int ret, len;
    u8 data[32] = { 0 };
    ...
    tmp_buf = kzalloc(size + 1, GFP_KERNEL);
    if (!tmp_buf) {
        fg_err("Failed to allocate memory for tmp_buf\n");
        return size;
    }
    remove_whitespace(buf, tmp_buf);
    len = strlen(tmp_buf);
    ...
    if (!is_all_digits(tmp_buf) || !size) {
        fg_err("input is not all digits! errorstr = %s data_len = [%ld]\n", tmp_buf, size);
        kfree(tmp_buf);
        return size;
    }

    ret = fg_mac_read_block(gm, FG_MAC_CMD_UI_SOH, data, 32);
    if (ret < 0) {
        fg_err("failed to get first_usage\n");
        kfree(tmp_buf);
        return size;
    ...
    while (retry++ < RETRY_UPDATE_COUNT) {
        ret = fg_mac_write_block(gm, FG_MAC_CMD_UI_SOH, data, 32);
        if (ret < 0) {
            msleep(100);
            continue;
        }
        ...
        ret = fg_mac_read_block(gm, FG_MAC_CMD_UI_SOH, data_temp, 32);
        if (ret < 0) {
            fg_err("failed to read first_usage_date\n");
            continue;
        }
...
EXPORT_SYMBOL(bms_set_first_usage_date);
...
static ssize_t fg_show_ui_soh(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    ret = fg_mac_read_block(bq, FG_MAC_CMD_UI_SOH, ui_soh_data, 32);
    if (ret < 0)
    {
        fg_err("failed to read ui_soh_data \n");
        return ret;
...
static int parse_dod_temp_range_dts(struct bq_fg_chip *bq)
{
    int total_length = 0, i = 0, ret = 0;
    struct device_node *np = bq->dev->of_node;

    total_length = of_property_count_elems_of_size(np, "dod_temp_range", sizeof(u32));
    if (total_length < 0) {
        mca_log_err("failed to read total_length of config\n");
        return -EINVAL;
    }

    ret = of_property_read_u32_array(np, "dod_temp_range", (u32 *)bq->dod_temp_range, total_length);
    if (ret) {
        mca_log_err("failed to parse dod_temp_range\n");
        return ret;
    }

    for (i = 0; i < SHUTDOWN_TUPLE_COUNT; i++) {
        mca_log_err("dod_temp_range %d %d \n", bq->dod_temp_range[i].low, bq->dod_temp_range[i].high);
    }
    return 0;
}

static int parse_cycle_temp_range_dts(struct bq_fg_chip *bq)
{
    int total_length = 0, i = 0, ret = 0;
    struct device_node *np = bq->dev->of_node;

    total_length = of_property_count_elems_of_size(np, "cycle_temp_range", sizeof(u32));
    if (total_length < 0) {
        mca_log_err("failed to read total_length of config\n");
        return -EINVAL;
    }

    ret = of_property_read_u32_array(np, "cycle_temp_range", (u32 *)bq->cycle_temp_range, total_length);
    if (ret) {
        mca_log_err("failed to parse cycle_temp_range\n");
        return ret;
    }

    for (i = 0; i < SHUTDOWN_TUPLE_COUNT; i++) {
        mca_log_err("cycle_temp_range %d %d \n",
            bq->cycle_temp_range[i].low, bq->cycle_temp_range[i].high);
    }
    return 0;
}
...
static int parse_dod_shutdown_voltage_dts(struct bq_fg_chip *bq, int temp_range)
{
    ...
    /* dod parameter */
    memset(bq->dod_param, 0, sizeof(bq->dod_param));
    ret = snprintf(name, sizeof(name), "dod_shutdown_voltage_%d", temp_range);
    if (ret >= sizeof(name))
        mca_log_err("dod_shutdown_voltage name is truncated\n");
    ...
    ret = of_property_read_u32_array(np, name, (u32 *)bq->dod_param, total_length);
    if (ret) {
        mca_log_err("failed to parse %s\n", name);
        return ret;
    }

    bq->dod_param_length = total_length / (sizeof(struct shutdown_voltage_param) / sizeof(u32));
    mca_log_err("dod_param_length %d,%d\n", bq->dod_param_length, total_length);

    for (i = 0; i < bq->dod_param_length; i++) {
        mca_log_err("dod_shutdown_voltage %d %d %d %d %d\n",
            bq->dod_param[i].low, bq->dod_param[i].high, bq->dod_param[i].shutdownVolt,
            bq->dod_param[i].shutdownDelayVolt, bq->dod_param[i].termVolt);
    }
    return 0;
}
...
static int parse_cycle_shutdown_voltage_dts(struct bq_fg_chip *bq, int temp_range)
{
    ...
    /* cycle parameter */
    memset(bq->cycle_param, 0, sizeof(bq->cycle_param));
    ret = snprintf(name, sizeof(name), "cycle_shutdown_voltage_%d", temp_range);
    if (ret >= sizeof(name))
        mca_log_err("dod_shutdown_voltage name is truncated\n");
    ...
    ret = of_property_read_u32_array(np, name, (u32 *)bq->cycle_param, total_length);
    if (ret) {
        mca_log_err("failed to parse %s\n", name);
        return ret;
    }

    bq->cycle_param_length = total_length / (sizeof(struct shutdown_voltage_param) / sizeof(u32));
    mca_log_err("cycle_param_length %d,%d\n", bq->cycle_param_length, total_length);

    for (i = 0; i < bq->cycle_param_length; i++) {
        mca_log_err("cycle_shutdown_voltage %d %d %d %d %d\n",
            bq->cycle_param[i].low, bq->cycle_param[i].high, bq->cycle_param[i].shutdownVolt,
            bq->cycle_param[i].shutdownDelayVolt, bq->cycle_param[i].termVolt);
    }

    return 0;
}

static int parse_dod_cold_temp_range_dts(struct bq_fg_chip *bq)
{
    int total_length = 0, i = 0, ret = 0;
    struct device_node *np = bq->dev->of_node;

    total_length = of_property_count_elems_of_size(np, "dod_cold_temp_range", sizeof(u32));
    if (total_length < 0) {
        mca_log_err("failed to read total_length of config\n");
        return -EINVAL;
    }

    ret = of_property_read_u32_array(np, "dod_cold_temp_range", (u32 *)bq->dod_cold_temp_range, total_length);
    if (ret) {
        mca_log_err("failed to parse dod_cold_temp_range\n");
        return ret;
    }

    for (i = 0; i < SHUTDOWN_TUPLE_COUNT; i++) {
        mca_log_err("dod_cold_temp_range %d %d \n", bq->dod_cold_temp_range[i].low, bq->dod_cold_temp_range[i].high);
    }
    return 0;
}
...
static int parse_cold_dod_shutdown_voltage_dts(struct bq_fg_chip *bq, int temp_range)
{
    ...
    /* dod parameter */
    memset(bq->cold_dod_param, 0, sizeof(bq->cold_dod_param));
    ret = snprintf(name, sizeof(name), "cold_dod_shutdown_voltage_%d", temp_range);
    if (ret >= sizeof(name))
        mca_log_err("cold_dod_shutdown_voltage name is truncated\n");
    ...
    ret = of_property_read_u32_array(np, name, (u32 *)bq->cold_dod_param, total_length);
    if (ret) {
        mca_log_err("failed to parse %s\n", name);
        return ret;
    }

    bq->cold_dod_param_length = total_length / (sizeof(struct shutdown_voltage_param) / sizeof(u32));
    mca_log_err("cold_dod_param_length %d,%d\n", bq->cold_dod_param_length, total_length);

    for (i = 0; i < bq->cold_dod_param_length; i++) {
        mca_log_err("dod_shutdown_voltage %d %d %d %d %d\n",
            bq->cold_dod_param[i].low, bq->cold_dod_param[i].high, bq->cold_dod_param[i].shutdownVolt,
            bq->cold_dod_param[i].shutdownDelayVolt, bq->cold_dod_param[i].termVolt);
    }
    return 0;
}

#define COLD_SHUTDOWN_VOL_CURRENT               300
#define COLD_SHUTDOWN_VOL_SOC                   6
#define COLD_SHUTDOWN_VOL_AFTER_CHGING_TIME_S   180
static int fg_parse_dt(struct bq_fg_chip *bq)
{
    ...
    if (gpio_is_valid(bq->slave_connect_gpio)) {
        ret = devm_gpio_request(bq->dev, bq->slave_connect_gpio, "slave_connect_gpio");
        if (ret)
            fg_err("Failed to request slave_connect_gpio: %d\n", ret);
    } else {
    ...
    ret = of_property_read_u32(node, "temp_hyst", &bq->temp_hyst);
    if (ret) {
        fg_err("%s failed to parse temp_hyst\n", bq->log_tag);
        bq->temp_hyst = 10;
    }
    ...
    bq->dynamic_shutdown_voltage_support = of_property_read_bool(node, "dynamic_shutdown_voltage_support");
    bq->dynamic_shutdown_voltage_support_v2 = of_property_read_bool(node, "dynamic_shutdown_voltage_support_v2");

    bq->cold_dynamic_shutdown_voltage_support = of_property_read_bool(node, "cold_dynamic_shutdown_voltage_support");
    if (bq->cold_dynamic_shutdown_voltage_support) {
        ret = of_property_read_u32(node, "cold_shutdown_vol_current", &bq->cold_shutdown_vol_current);
        if (ret) {
            fg_err("%s failed to parse cold_shutdown_vol_current\n", bq->log_tag);
            bq->cold_shutdown_vol_current = COLD_SHUTDOWN_VOL_CURRENT;
        }

        ret = of_property_read_u32(node, "cold_shutdown_vol_soc", &bq->cold_shutdown_vol_soc);
        if (ret) {
            fg_err("%s failed to parse cold_shutdown_vol_soc\n", bq->log_tag);
            bq->cold_shutdown_vol_soc = COLD_SHUTDOWN_VOL_SOC;
        }

        ret = of_property_read_u32(node, "cold_shutdown_vol_after_charging_time_s", &bq->cold_shutdown_vol_after_charging_time_s);
        if (ret) {
            fg_err("%s failed to parse cold_shutdown_vol_after_charging_time_s\n", bq->log_tag);
            bq->cold_shutdown_vol_after_charging_time_s = COLD_SHUTDOWN_VOL_AFTER_CHGING_TIME_S;
        }
        parse_dod_cold_temp_range_dts(bq);
        parse_cold_dod_shutdown_voltage_dts(bq, 0);
    }

    parse_dod_temp_range_dts(bq);
    parse_cycle_temp_range_dts(bq);
    parse_dod_shutdown_voltage_dts(bq, 0);
    parse_cycle_shutdown_voltage_dts(bq, 0);
...
static int fg_check_device(struct bq_fg_chip *bq)
{
    ...
    if (!strncmp(data, "MI", 2)) {
        ...
        } else if (!strncmp(&data[2], "7", 1)) {
            bq->device_name = BQ_FG_MPC7012B;
            bq->battery_vendor = BATTERY_PACK_VENDOR_MPC7012B;
            strcpy(bq->model_name, "mpc7012b");
            strcpy(bq->log_tag, "[XMCHG_MPC7012B]");
...
static int fg_probe(struct i2c_client *client)
{
    ...
    if (bq->dynamic_shutdown_voltage_support_v2) {
        INIT_DELAYED_WORK(&bq->battery_shutdown_vol_update_work, battery_shutdown_vol_update_workfunc);
        schedule_delayed_work(&bq->battery_shutdown_vol_update_work, msecs_to_jiffies(6000));
    }
