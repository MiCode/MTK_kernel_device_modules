...
static int fg_read_real_temp(struct bq_fg_chip *bq)
{
	...
	if (product_name == KLEE_CN || product_name == KLEE_GL || product_name == DASH_CN) {
...
static int bq_battery_soc_smooth_tracking_new(struct bq_fg_chip *bq, int raw_soc, int batt_soc, int batt_ma)
{
	...
	if (product_name == KLEE_CN && bq->rsoc == 0 && bq->tbat < 0) {
		bq->monitor_delay = FG_MONITOR_DELAY_2S;
	}
...
static int fg_count_shutdown_vol_klee_cn(struct bq_fg_chip *bq, int count)
{
	int count_vol = 0;
...
static int fg_cycle_shutdown_vol_klee_cn(struct bq_fg_chip *bq)
{
	int cycle_vol = 0;
	int cycle = bq->cycle_count;
...
static int fg_count_term_vol_klee_cn(struct bq_fg_chip *bq, int count)
{
	int count_vol = 0;
...
static int fg_cycle_term_vol_klee_cn(struct bq_fg_chip *bq)
{
	int cycle_vol = 0;
	int cycle = bq->cycle_count;
...
static int fg_count_shutdown_vol_klee_gl(struct bq_fg_chip *bq, int count)
{
	int count_vol = 0;
...
static int fg_cycle_shutdown_vol_klee_gl(struct bq_fg_chip *bq)
{
	int cycle_vol = 0;
	int cycle = bq->cycle_count;
...
static void battery_shutdown_vol_update(struct bq_fg_chip *bq)
{
	...
	int count_term_vol = 0;
	int cycle_term_vol = 0;

	if (product_name == RODIN_CN || product_name == RODIN_IN || product_name == DALI_CN ||
			product_name == KLEE_CN || product_name == KLEE_GL || product_name == DASH_CN) {
		if (bq->fake_dod_count) {
			...
			if (product_name == KLEE_CN || product_name == KLEE_GL) {
		...
		if (product_name == DALI_CN  || product_name == DASH_CN) {
			count_vol = fg_count_shutdown_vol_dali(bq, count);
			cycle_vol = fg_cycle_shutdown_vol_dali(bq);
		} else if (product_name == KLEE_CN) {
			count_vol = fg_count_shutdown_vol_klee_cn(bq, count);
			cycle_vol = fg_cycle_shutdown_vol_klee_cn(bq);
		} else if (product_name == KLEE_GL) {
			count_vol = fg_count_shutdown_vol_klee_gl(bq, count);
			cycle_vol = fg_cycle_shutdown_vol_klee_gl(bq);
		} else {
		...
		/* set shutdown_vol */
		if (product_name == KLEE_CN) {
			count_term_vol = fg_count_term_vol_klee_cn(bq, count);
			cycle_term_vol = fg_cycle_term_vol_klee_cn(bq);
			term_vol = max(count_term_vol, cycle_term_vol);
		} else  {
			if (bq->tbat < 0) {
				term_vol = shutdown_vol + 100;
			} else {
				if (shutdown_vol == 2800 && (product_name == RODIN_CN || product_name == RODIN_IN || product_name == DALI_CN)) {
					term_vol = shutdown_vol + 100;
				} else {
			...
			if (shutdown_vol <= 2500) {
				term_vol = shutdown_vol + 150;
			}
		}
		if (term_vol != bq->term_vol) {
			...
			fg_info("%s:dynamic_shutdown_vol term_vol=%d t_buf[0]=0x%02x t_buf[1]=0x%02x FG_MAC_CMD_MIX_TERM_VOLT=%d\n", __func__,
	...
	if (product_name == KLEE_CN && bq->temp_state <= BAT_LITTLE_COLD) {
		bq->critical_shutdown_vbat = bq->shutdown_voltage + 150;
		bq->cool_critical_shutdown_vbat = bq->shutdown_voltage + 150;
		bq->old_critical_shutdown_vbat = bq->shutdown_voltage + 150;
	} else if (shutdown_vol >= 3300) {
...
static int fg_parse_dt(struct bq_fg_chip *bq)
{
	...
	if (product_name == KLEE_GL) {
		ret = of_property_read_u32(node, "initial_termv_gl", &bq->initial_termv);
		size = 0;
		of_get_property(node, "soh_comp_array_gl", &size);
		if (size) {
			...
			if (!bq->soh_comp_array || IS_ERR(bq->soh_comp_array)) {
				...
				if (bq->soh_comp_len % 2) {
					fg_err("%s invalid soh_comp_array_gl\n", bq->log_tag);
					return -EINVAL;
				}
		...
		ret = of_property_read_u32(node, "initial_termv", &bq->initial_termv);
		size = 0;
