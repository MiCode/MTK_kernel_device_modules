...
static int fg_read_real_temp(struct bq_fg_chip *bq)
{
	...
	if (ret < 0) {
		fg_err("%s failed to read real TBAT\n", bq->log_tag);
		tbat = 2980;
		if(bq->i2c_error_count < 10)
			bq->i2c_error_count++;
	} else {
...
retry:
	...
	if (bq->vbat <= 1000) {
		if(bq->fg_error_count < 10)
			bq->fg_error_count++;
		fg_err("%s VBAT=%d abnormal count=%d\n", bq->log_tag, bq->vbat, bq->fg_error_count);
	} else {
		bq->fg_error_count = 0;
	}
...
static unsigned int get_delta_soh(struct bq_fg_chip *bq)
{
	unsigned int delta_soh = 0;
	int delta_termv = 0;
	int i = 0;

	if (!bq->term_vol || !bq->initial_termv || !bq->soh_comp_array) {
		fg_err("%s not support soh compsation\n", bq->log_tag);
		return 0;
	}

	delta_termv = bq->term_vol - bq->initial_termv;

	if (bq->soh_comp_len % 2) {
		fg_err("%s invalid soh_comp_array\n", bq->log_tag);
		return 0;
	}

	for (i = 0; i < bq->soh_comp_len; i += 2) {
		if (delta_termv <= bq->soh_comp_array[i]) {
			delta_soh = bq->soh_comp_array[i + 1];
			break;
		}
	}
	if (i >= bq->soh_comp_len)
		delta_soh = bq->soh_comp_array[bq->soh_comp_len-1];
	if (delta_soh > 20)
		delta_soh = 20;
	fg_err("%s %s:%d,%d\n", bq->log_tag, __func__, delta_termv, delta_soh);
...
retry:
	...
	delta_soh = get_delta_soh(bq);
	soh += delta_soh;
	if (soh > 100)
		soh = 100;
	fg_err("%s soh=%d,%d\n", bq->log_tag, soh, delta_soh);
	return soh;
}
...
static void battery_shutdown_vol_update(struct bq_fg_chip *bq)
{
	...
	if (shutdown_vol >= 3300) {
		...
		bq->cool_critical_shutdown_vbat = bq->shutdown_voltage + 40;
		bq->old_critical_shutdown_vbat = bq->shutdown_voltage + 40;
	} else {
	...
	mca_log_err("shutdown voltage=%d,%d,%d,%d,%d state=%d,%d-%d\n", bq->shutdown_voltage, bq->normal_shutdown_vbat,
...
static void battery_shutdown_vol_update_v1(struct bq_fg_chip *bq)
{
	int ret = 0;
	int shutdown_vol = 3200;
	int term_vol = 3200;
	...
	u16 addr = bq->device_name == BQ_FG_MPC7012 ? FG_MAC_CMD_MIX_TERM_VOLT_G2 : FG_MAC_CMD_MIX_TERM_VOLT;
	...
	if (bq->dynamic_shutdown_voltage_support_v2) {
		...
		get_temp_index(bq->dod_temp_range, 10, 10, bq->tbat, bq->temp_dod_index, false);
		get_temp_index(bq->cycle_temp_range, 10, 10, bq->tbat, bq->temp_cycle_index, false);
...
static void battery_shutdown_vol_update_workfunc(struct work_struct *work)
{
	...
	battery_shutdown_vol_update_v1(bq);

	schedule_delayed_work(&bq->battery_shutdown_vol_update_work, msecs_to_jiffies(bq->shutdown_vol_delay));
}

static void batt_health_update(struct bq_fg_chip *bq)
{
	if (!bq) {
		mca_log_err("bq is NULL\n");
		return;
	}

	if (bq->tbat <= -100)
		bq->batt_health = POWER_SUPPLY_HEALTH_COLD;
	else if (bq->tbat <= 150)
		bq->batt_health = POWER_SUPPLY_HEALTH_COOL;
	else if (bq->tbat <= 480)
		bq->batt_health = POWER_SUPPLY_HEALTH_GOOD;
	else if (bq->tbat <= 520)
		bq->batt_health = POWER_SUPPLY_HEALTH_WARM;
	else if (bq->tbat < 600)
		bq->batt_health = POWER_SUPPLY_HEALTH_HOT;
	else
		bq->batt_health = POWER_SUPPLY_HEALTH_OVERHEAT;

	mca_log_err("batt_health_update: tbat=%d, batt_health=%d\n",
		bq->tbat, bq->batt_health);
}
...
static int fg_get_fast_mode_times(struct bq_fg_chip *bq, int *fast_mode_times)
{
	u8 data[RANDOM_CHALLENGE_LEN_BQ27Z561] = {0};
	int ret = 0;

	ret = fg_mac_read_block(bq, FG_MAC_CMD_FAST_MODE_STS, data, RANDOM_CHALLENGE_LEN_BQ27Z561);
	if (ret < 0) {
		mca_log_err("could not read fast_mode_times = %d\n", ret);
		return ret;
	}

	*fast_mode_times = (data[1] << 8) | data[0];

	return 0;
}

static int fg_set_fast_mode_times(struct bq_fg_chip *bq, int fast_mode_times)
{
	u8 data[2] = {0};
	int ret = 0;

	data[0] = (u8)(fast_mode_times);
	data[1] = (u8)(fast_mode_times >> 8);
	ret = fg_mac_write_block(bq, FG_MAC_CMD_FAST_MODE_STS, data, 2);
	if (ret < 0) {
		mca_log_err("could not set fast_mode_times = %d\n", ret);
		return ret;
	}
...
static void fg_check_fast_mode_times(struct bq_fg_chip *bq)
{
	int fast_mode_times = 0;
	...
	if (IS_ERR_OR_NULL(chg_psy)) {
		mca_log_err("cannot get primary_chg\n");
		return;
	}

	ret = power_supply_get_property(chg_psy, POWER_SUPPLY_PROP_ONLINE, &online);
	if (ret < 0) {
		mca_log_err("get online failed: %d\n", ret);
		power_supply_put(chg_psy);
		return;
	}

	fg_get_fast_mode_times(bq, &fast_mode_times);
	if (online.intval != 0 && fast_mode_times == 0) {
		fg_set_fast_mode_times(bq, 7200);
		mca_log_err("enable fast mode, times=7200 (online=%d)\n", online.intval);
	} else if (online.intval == 0 && fast_mode_times != 0) {
		fg_set_fast_mode_times(bq, 0);
		mca_log_err("disable fast mode, times=0 (online=%d)\n", online.intval);
	}
...
out:
		...
		/*for power tool*/
		mca_log_err("UISOC:%d CURRENT:%d RSOC:%d VOLTAGE:%d TEMP:%d HEALTH:%d RM:%d F:%d RAWSOC:%d, DODCOUNT:%d\n",
			bq->ui_soc, bq->ibat * 1000, bq->rsoc, bq->vbat, bq->tbat,
			bq->batt_health, bq->rm * 1000, bq->fcc * 1000, bq->raw_soc, bq->dod_count);
...
static void fg_monitor_workfunc(struct work_struct *work)
{
	...
	if (bq->support_fg_fast_sample)
		fg_check_fast_mode_times(bq);
	...
	int val)
{
	if (gm) {
		...
		if (gm->is_eea_model && gm->rsoc < 100) {
...
static int check_fast_mode_times_set(struct bq_fg_chip *gm,
	struct mtk_bms_sysfs_field_info *attr,
	int val)
{
	if (!gm)
	...
	if (val && gm->support_fg_fast_sample) {
		fg_check_fast_mode_times(gm);
		mca_log_err("trigger fg_check_fast_mode_times\n");
	} else {
		mca_log_err("skip, val=%d, support_fg_fast_sample=%d\n",
			val, gm->support_fg_fast_sample);
	}

	return 0;
}

static int check_fast_mode_times_get(struct bq_fg_chip *gm,
		struct mtk_bms_sysfs_field_info *attr,
		int *val)
{
	int ret;
	...
	ret = fg_get_fast_mode_times(gm, val);
	if (ret < 0) {
		mca_log_err("fg_get_fast_mode_times failed, ret=%d\n", ret);
		*val = 0;
		return ret;
	}

	mca_log_err("fast_mode_times=%d\n", *val);
	return 0;
}

static int fg_in_sleep_get(struct bq_fg_chip *gm,
		struct mtk_bms_sysfs_field_info *attr,
		int *val)
{
	if (!gm) {
	...
	*val = atomic_read(&gm->fg_in_sleep);

	fg_err("%s: fg_in_sleep=%d\n", __func__, *val);
	return 0;
}
...
static int fg_show_calc_rvalue(struct bq_fg_chip *bq)
{
	...
	if (bq->device_name != BQ_FG_MPC7012) {
		K1 = 4500;
		K2 = 4500;
		K3 = 7000;
		K4 = 4500;
...
static int support_batt_manager_4p0_get(struct bq_fg_chip *gm,
	...
	int *val)
{
	if (gm->device_name != BQ_FG_MPC7012) {
		*val = 0;
	} else {
		*val = 1;
	}
	...
	int *val)
{
	...
	if(gm->device_name != BQ_FG_NFG1000A && gm->device_name != BQ_FG_NFG1000B && gm->device_name!= BQ_FG_MPC7012)
	{
...
static struct mtk_bms_sysfs_field_info bms_sysfs_field_tbl[] = {
	...
	BMS_SYSFS_FIELD_RW(check_fast_mode_times, BMS_PROP_CHECK_FAST_MODE_TIMES),
	BMS_SYSFS_FIELD_RO(fg_in_sleep, BMS_PROP_FG_IN_SLEEP),
};
...
static int fg_get_property(struct power_supply *psy, enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CAPACITY:
		...
		if (bq->i2c_error_count >= 10 || bq->fg_error_count >= 10) {
			val->intval = 15;
			break;
		}
		...
		if (bq->enable_shutdown_delay) {
			...
			if (last_shutdown_delay != bq->shutdown_delay || (val->intval == 0 && last_ui_soc != 0)) {
				...
				mca_log_err("power_supply_changed\n");
			}
		}
		break;
	...
	case POWER_SUPPLY_PROP_CHARGE_FULL:
		if (bq->device_name == BQ_FG_BQ27Z561 || bq->device_name == BQ_FG_NFG1000A || bq->device_name == BQ_FG_NFG1000B || bq->device_name == BQ_FG_MPC7012)
			val->intval = bq->fcc;
		else if (bq->device_name == BQ_FG_BQ28Z610)
			val->intval = bq->fcc * 2;
		else
...
static int fg_parse_dt(struct bq_fg_chip *bq)
{
	...
	bq->support_fg_fast_sample = of_property_read_bool(node, "support_fg_fast_sample");
	...
	ret = of_property_read_u32(node, "calc_shutdown_vol_current", &bq->calc_shutdown_vol_current);
	if (ret) {
		fg_err("%s failed to parse calc_shutdown_vol_current\n", bq->log_tag);
		bq->calc_shutdown_vol_current = 300;
		ret = 0;
	}

	ret = of_property_read_u32(node, "calc_shutdown_vol_soc", &bq->calc_shutdown_vol_soc);
	if (ret) {
		fg_err("%s failed to parse calc_shutdown_vol_soc\n", bq->log_tag);
		bq->calc_shutdown_vol_soc = 6;
		ret = 0;
	}

	ret = of_property_read_u32(node, "calc_shutdown_vol_after_charging_time_s", &bq->calc_shutdown_vol_after_charging_time_s);
	if (ret) {
		fg_err("%s failed to parse calc_shutdown_vol_after_charging_time_s\n", bq->log_tag);
		bq->calc_shutdown_vol_after_charging_time_s = 180;
		ret = 0;
	}
	...
	of_property_read_u32(node, "initial_termv", &bq->initial_termv);
	size = 0;
	of_get_property(node, "soh_comp_array", &size);
	if (size) {
		bq->soh_comp_array = devm_kzalloc(bq->dev, size, GFP_KERNEL);
		if (!bq->soh_comp_array || IS_ERR(bq->soh_comp_array)) {
			fg_err("%s error alloc memory for soh_comp_array\n", bq->log_tag);
			return -ENOMEM;
		} else {
			memset(bq->soh_comp_array, 0, size);
			bq->soh_comp_len =
				(size / sizeof(*bq->soh_comp_array));
			if (bq->soh_comp_len % 2) {
				fg_err("%s invalid soh_comp_array\n", bq->log_tag);
				return -EINVAL;
			}
			...
			fg_err("%s size=%d,%d\n", bq->log_tag, size, bq->soh_comp_len);
			for (i=0; i<bq->soh_comp_len; i+=2)
				fg_err("%s: %d %d\n", bq->log_tag, bq->soh_comp_array[i], bq->soh_comp_array[i+1]);
		}
	}
	fg_err("%s initial_termv=%d\n", bq->log_tag, bq->initial_termv);
...
static int fg_check_device(struct bq_fg_chip *bq)
{
	...
	if (!strncmp(data, "MI", 2)) {
		...
		if (!strncmp(&data[2], "4", 1)) {
			...
			bq->device_name = BQ_FG_MPC7012;
			bq->battery_vendor = BATTERY_PACK_VENDOR_MPC7012;
			strcpy(bq->model_name, "mpc7012");
			strcpy(bq->log_tag, "[XMCHG_MPC7012]");
...
static void fg_shutdown(struct i2c_client *client)
{
	struct bq_fg_chip *bq = i2c_get_clientdata(client);
	if (!bq) {
		fg_err("bq_fg: %s called with NULL bq\n", __func__);
		return;
	}
