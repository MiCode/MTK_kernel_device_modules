...
{
	return charger_partition;
}
EXPORT_SYMBOL(get_charger_partition);
static void* rw_buf;
...
static bool get_charger_partition_info(void)
{
	...
	unsigned long i; // for traversal of all partitions
	...
	xa_for_each(&sdkp->disk->part_tbl, i, part) {
		// check partition name
		if (part->bd_meta_info && strcmp((char *)part->bd_meta_info->volname, PARTITION_NAME) == 0) {
			part_number = i;
			pr_err("[ChgPartition] charger partition part number is %d\n", part_number);
			break;
		} else {
			pr_debug("[ChgPartition] %lu is %s partition\n", i, part->bd_meta_info->volname);
		}
	}

	part = xa_load(&sdkp->disk->part_tbl, part_number);
	if (!part) {
		pr_err("[ChgPartition] %s device is null\n", __func__);
		return false;
	}
	...
	pr_err("[charger] %s partion: %d %s start %llu(block) size %llu(block)\n",
			__func__, part_number, charger_partition->part_info_part_name, part_info.part_start, part_info.part_size);
	return true;
}
...
int get_charger_partition_info_1(void)
{
	...
	struct ChargerPartition *cp = charger_partition;
	...
	if (cp) {
		mutex_lock(&cp->lock);
		memcpy(&cp->info_1, info_1, sizeof(*info_1));
...
int get_charger_partition_info_2(void)
{
	...
	pr_err("[charger] %s ret: %d, info_2->eea_mode: %u\n", __func__, ret, info_2->eea_mode);
	if (info_2->eea_mode == 1) {
		charger_partition->is_eea_mode = 1;
		usb_set_property(USB_PROP_SUPPORT_EEA_CHARGE, info_2->eea_mode);
	}
...
int set_charger_partition_info_2(int val)
{
	int ret = 0;
	charger_partition_info_2 info_2 = {.eea_mode = 0, .test = 0x34567890, .reserved = 0};

	if (val == charger_partition->is_eea_mode) {
		pr_err("[charger] %s same value val=%d\n", __func__, val);
		return 0;
	}
	...
	pr_err("[charger] %s ret: %d, info_2.eea_mode: %u, info_2.test: 0x%0x\n", __func__, ret, info_2.eea_mode, info_2.test);
...
int get_charger_partition_info_3(int *val)
{
	int ret = 0;
	charger_partition_info_3 *info_3 = NULL;

	ret = charger_partition_alloc(CHARGER_PARTITION_HOST_KERNEL, CHARGER_PARTITION_INFO_3, sizeof(charger_partition_info_3));
	if (ret < 0) {
		pr_err("[charger] %s failed to alloc\n", __func__);
		return -1;
	}

	info_3 = (charger_partition_info_3 *)charger_partition_read(CHARGER_PARTITION_HOST_KERNEL, CHARGER_PARTITION_INFO_3, sizeof(charger_partition_info_3));
	if (!info_3) {
		pr_err("[charger] %s failed to read\n", __func__);
		ret = charger_partition_dealloc(CHARGER_PARTITION_HOST_KERNEL, CHARGER_PARTITION_INFO_3, sizeof(charger_partition_info_3));
		if (ret < 0) {
			pr_err("[charger] %s failed to dealloc\n", __func__);
			return -1;
		}
	...
	pr_err("[charger] %s ret: %d, info_3->tx_q_value: %u\n", __func__, ret, info_3->tx_q_value);
	*val = info_3->tx_q_value;
...
EXPORT_SYMBOL(get_charger_partition_info_3);

int set_charger_partition_info_3(int val)
{
	int ret = 0;
	charger_partition_info_3 info_3 = {.tx_q_value = 0, .reserved = 0};
	...
	info_3.tx_q_value = val;
	ret = charger_partition_write(CHARGER_PARTITION_HOST_KERNEL, CHARGER_PARTITION_INFO_3, (void *)&info_3, sizeof(charger_partition_info_3));
	if (ret < 0) {
		chr_err("[charger] %s failed to write\n", __func__);
		ret = charger_partition_dealloc(CHARGER_PARTITION_HOST_KERNEL, CHARGER_PARTITION_INFO_3, sizeof(charger_partition_info_3));
		if (ret < 0) {
...
EXPORT_SYMBOL(set_charger_partition_info_3);
#endif
...
int charger_partition_init(struct mtk_charger *info)
{
	charger_partition = (struct ChargerPartition *)kzalloc(sizeof(struct ChargerPartition), GFP_KERNEL);
	charger_partition->project_no = info->project_no;

	if (!charger_partition)
		return -ENOMEM;
	mutex_init(&charger_partition->lock);
...
{
	if (charger_partition) {
		cancel_delayed_work_sync(&charger_partition->charger_partition_work);
		kfree(charger_partition);
		charger_partition = NULL;
	}
}
