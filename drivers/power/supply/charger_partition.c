...
static void charger_partition_work(struct work_struct *work)
{
	...
	if (!IS_ERR_OR_NULL(pcba_msg->rsc)) {
		if (strstr(pcba_msg->rsc, "cn") || strstr(pcba_msg->rsc, "in"))
			charger_partition->part_info_part_number = 87;
		else
			charger_partition->part_info_part_number = 88;

		pr_err("[charger] %s pcba part_info_part_number = %d\n", __func__, charger_partition->part_info_part_number);
	} else {
		charger_partition->part_info_part_number = 87;
		pr_err("[charger] %s pcba_msg->rsc = NULL part_number = %d\n", __func__, charger_partition->part_info_part_number);
	}
...
EXPORT_SYMBOL(charger_partition_init);
...
EXPORT_SYMBOL(charger_partition_exit);
