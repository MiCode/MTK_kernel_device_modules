...
static int parse_config_string(struct ut_info *ut)
{
	...
	buf_size += 1;
	char *tmp_str = devm_kzalloc(ut->dev, buf_size, GFP_KERNEL);
	if (!tmp_str) {
		mca_log_err("memory alloc for output str failed!\n");
		return -ENOMEM;
	}
	mutex_lock(&ut->config_str_lock);
	char *str_p = tmp_str;
	
	for (i = 0; i < ARRAY_SIZE(ti); ++i) {
		if (ti[i].dts_value) {
			size_t remain = buf_size - (str_p - tmp_str);
			if (i == 0)
	...
	ut->config_str = tmp_str;
	mutex_unlock(&ut->config_str_lock);
	return 0;
}
...
static void ut_test_remove(struct platform_device *pdev)
{
	struct ut_info *ut = platform_get_drvdata(pdev);
	atomic_set(&ut->exiting, 1);
