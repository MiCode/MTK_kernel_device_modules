...
int get_effective_result_exclude_client(struct votable *votable, const char *exclude_client_str)
{
	int i;
	int effective_result = -EINVAL;

	for (i = 0; i < votable->num_clients; i++) {
		if (votable->client_strs[i] && strcmp(votable->client_strs[i], exclude_client_str)) {
			if (votable->votes[i].enabled) {
				if (effective_result == -EINVAL)
					effective_result = votable->votes[i].value;
				else
					effective_result = effective_result <= votable->votes[i].value ? effective_result : votable->votes[i].value;
			}
		}
	}
...
EXPORT_SYMBOL(get_effective_result_exclude_client);
...
int vote(struct votable *votable, const char *client_str, bool enabled, int val)
{
	...
	if (!votable->voted_on || (effective_result != votable->effective_result)) {
		...
		pr_info("%s: effective vote %s, val = %d, override_result = %d\n", votable->name,
