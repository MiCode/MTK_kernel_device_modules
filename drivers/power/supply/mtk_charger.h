...
#define DFX_CHG_CYCLIAL_RECHECK_CNT 10
#define DFX_CHG_CP_TDIE_HOT_CHECK_CNT 0
#define DFX_BATT_AUTH_CHECK_CNT 1
#define DFX_CHG_HIGH_TEMP_DISCHARGING_CNT 10
...
#define CHG_DISCONN_ERR_CNT_MAX	5
#define CHG_DISCONN_ERR_INTERVAL	2 /* 2s */
#define CHG_DISCONN_NORMAL_INTERVAL	310 /* 310s */

#define LOW_POWER_CHARGER_VTERM_DROP_MV 10
#define LOW_POWER_CHARGER_ITERM_MA 100
...
enum xm_chg_dfx_type {
	...
	CHG_DFX_CP_TDIE_HOT,
	CHG_DFX_CHG_BATT_MISS, //dual batt miss
	...
	CHG_DFX_CP_IBUS_OCP,
	CHG_DFX_CP_IBAT_OCP,
	CHG_DFX_BATTERY_OVERVOLTAGE,
	CHG_DFX_ERP_RECHG_SOC_ERR,
	CHG_DFX_ERP_NOT_STOP_CHG_ERR,
...
static const char *const xm_chg_dfx_report_text[] = {
	"DEFAULT_TEXT",
	"lpdDischarge", "lpdDischargeReset", "fgI2cErr", "cpErr", "VbatSocNotMatch", "BattAuthFail", "chgBattCycle",
	"PdAuthFail", "CpEnFail", "noneStandartChg", "corrosionDischarge", "CpVbatOVP", "AntiBurnTirg",
	"SocNotFull", "SmartEnduraTrig", "SmartNaviTrig", "battLinkerAbsent", "CpTdieHot", "chgBattMiss",
	"SmartEnduraSocErr", "SmartNaviSocErr", "TbatCold", "wlsRxI2CErr", "TbatHot", "NotChgInLowTemp", "NotChgInHighTemp",
	"AntiFail","DualVbatDiff","DualBattAuthFail", "CpVbusOVP", "CpIbusOCP", "CpIbatOCP",
	"BatteryOverVoltage", "RechgSocErr", "NotStopChgErr",
};

static int xm_chg_dfx_id[CHG_DFX_MAX_INDEX] = {
	0,
	909002003, 909002003, 909005001, 909005002, 909006001, 909007001,
	909003001, 909001004, 909001005, 909002001, 909002002, 909002006,
	909002012, 909003002, 909003004, 909003006, 909005003, 909005004,
	909005009, 909006010, 909006011, 909009002, 909013004, 909009001,
	909005007, 909005008, 909009003, 909014002, 909007002, 909002004,
	909002005, 909002007, 909010002, 909014020, 909014021,
};
...
#define THIRD_PPS_PARA_MAX_GROUP 16
enum quick_charge_third_pps_para_ele {
	QUICK_CHG_PD_USB_VENDOR_ID = 0,
	QUICK_CHG_PD_PRODUCT_VDO,
	QUICK_CHG_PD_MAX_POWER,
	QUICK_CHG_ENABLE_CP,
	QUICK_CHG_MAX_VOLT,
	QUICK_CHG_MAX_CURRENT,
	QUICK_CHG_PPS_PARA_MAX,
};

struct charge_thrid_pps_para {
	int usb_vendor_id;
	int product_vdo;
	int max_power;
	int enable_cp;
...
struct mtk_charger {
	...
	struct work_struct smart_bypass_hold_timer_handler_work;
	...
	struct step_jeita_cfg0 flip_step_normal_chg_cfg[STEP_JEITA_TUPLE_COUNT];
	struct charge_thrid_pps_para third_pps_para[THIRD_PPS_PARA_MAX_GROUP];
	...
	int step_normal_chg_index[2];
	int step_ffc_chg_index[2];
	int jeita_chg_index[2];
	int flip_jeita_chg_index[2];
	int flip_step_chg_index[2];
	int flip_step_normal_chg_index[2];
	int flip_step_ffc_chg_index[2];
	int step_chg_fcc;
	int jeita_chg_fcc;
	int current_now;
	int vbat_now;
	...
	ktime_t fs_start_time;
	int fs_start_current;
	int fs_time_limit;
	int ffc_temp_state;
	int max_step_low;
	int max_step_normal;
	int dec_gap;
	int dec_step;
	int vbat_over_fv_times;
	bool en_pmic_dec_vote;
	bool otg_enable;
	bool pd_verify_done;
	bool pd_verifed;
	bool warm_term;
	...
	bool support_low_ibat_switch_buck;
	int normal_cutoff_margin_mv;

	struct step_jeita_cfg0 reduce_cv_backup[STEP_JEITA_TUPLE_COUNT];
	bool reduce_cv_applied;
	int  reduce_cv_valid_cnt;
	int  reduce_cv_cfg_type;
	...
	struct regmap *mt6661_regmap;
	bool mt6368_moscon1_control;
	bool mt6373_moscon1_control;
	bool mt6373_moscon2_control;
	bool mt6661_moscon1_control;
	bool mt6661_moscon2_control;
	bool night_charging;
	bool night_charge_enable;
	bool mishow_charge_enable;
	bool sic_support;
	...
	int typec_portx_dpdm_attach;
	struct tcpc_device *tcpc1;
	struct wakeup_source *rqc_wakelock;
#endif
	// start of rqc vars
	...
	// CC toggle switch
	struct notifier_block tcpc_cc_det_nb;
	struct delayed_work hrtime_otg_work;
	struct alarm otg_switch_work_timer;
	...
	unsigned long dfx_single_report_type;
	unsigned long dfx_cyclial_report_type;
	unsigned long dfx_fake_report_type;
	int dfx_cyclial_check_count;
	int dfx_soc_not_full_cnt;
	int dfx_batt_auth_check_cnt;
	int dfx_hightemp_dischg_cnt;
	...
	bool bypass_auth_m;
	int btb_missing_status;
#endif
	bool support_lp_chgr_term_adjust;
	int lp_chgr_iterm;
	int lp_chgr_vterm_drop;
	bool direct_cap_change_support;
	bool vdm_adapter_close_vbus_support;

	bool chg_slidedata_report_support;
	struct alarm chg_slidedata_report_timer;
	struct delayed_work chg_slidedata_report_det_work;

	int chg_disconnect_cnt;
	ktime_t chg_start_time;
	ktime_t chg_start_time_last;
};
...
/* PD info structure for reverse cable */
struct rev_cable_info {
	...
	bool is_a_port;      /* true if is A port */
	bool partner_is_xm;  /* true if vid = 0x2717[XM] */
...
enum wls_property {
	...
	WLS_PROP_MAGNETIC_CASE_FLAG_N,
	WLS_PROP_MAGNETIC_CASE_FLAG_S,
#ifdef CONFIG_WLS_SUPPORT_TX_Q_CHECK
	WLS_PROP_WIRELESS_TX_Q_VALUE,
	WLS_PROP_CHARGER_PARTITION_TXQVALUE,
#endif
};
...
extern int mtk_set_mt6661_moscon1(struct mtk_charger *info, bool en, int drv_sel);
extern int mtk_set_mt6661_moscon2(struct mtk_charger *info, bool en, int drv_sel);
...
#ifdef CONFIG_SUPPORT_SOUTHCHIP_PDPHY
/* functions for check after plugin port bc12 type */
extern int check_after_plugin_bc12_type(struct mtk_charger *info);
#endif
