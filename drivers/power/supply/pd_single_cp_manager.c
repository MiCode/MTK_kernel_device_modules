...
static bool pdm_taper_charge(struct usbpd_pm *pdpm)
{
	...
	if (pdpm->charge_full){
		pdm_err("pdm_taper_charge charge full:%d\n", pdpm->charge_full);
		return true;
	}
#ifdef CONFIG_SUPPORT_DUAL_BATTERY
	if(pdpm->charge_full_m || pdpm->charge_full_s){
		pdm_err("pdm_taper_charge charge full batt:[%d %d]\n", pdpm->charge_full_m, pdpm->charge_full_s);
		return true;
	}
#endif
...
static int pdm_check_condition(struct usbpd_pm *pdpm)
{
	...
	if (pdpm->state == PD_PM_STATE_TUNE && pdm_taper_charge(pdpm)) {
		...
		return PDM_SM_EXIT;
	} else if(pdpm->bms_i2c_error_count >= 10 || pdpm->slave_bms_i2c_error_count >= 10 || !pdpm->gauge_authentic || !pdpm->slave_gauge_authentic) {
		pdm_err("i2c_error_count=[%d %d], gauge_authentic=[%d %d]\n", pdpm->bms_i2c_error_count, pdpm->slave_bms_i2c_error_count,
		...
		pdm_err("PDM_SM_HOLD  only input_suspend=%d can't triger typec burn\n", pdpm->input_suspend);
		return PDM_SM_HOLD;
	} else if (pdpm->typec_port_num == 0 && pdpm->typec_burn == true) {
		pdm_err("PDM_SM_HOLD port0 triger typec_burn=%d\n", pdpm->typec_burn);
		return PDM_SM_HOLD;
	} else if (pdpm->typec_port_num == 1 && pdpm->short_typec_burn == true) {
		pdm_err("PDM_SM_HOLD O88 port1 triger short_typec_burn=%d\n", pdpm->short_typec_burn);
		return PDM_SM_HOLD;
