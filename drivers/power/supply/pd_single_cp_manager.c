...
static int pdm_tune_pdo(struct usbpd_pm *pdpm)
{
	...
	if (pdpm->cp_work_mode == SC8561_FORWARD_1_1_CHARGER_MODE)
		ibus_limit = min(ibus_limit, 5000);
	...
	if (pdpm->ibat_step >= QUICK_STEP_LEVEL_3 && final_step >= LARGE_STEP) {
		final_step = pdpm->ibat_step;
	}
	...
	if (pdpm->pmic_parachg_mode == 1) {
		if (pdpm->whole_ibus >= 4500) {
				final_step = -1;
				pdm_err("drop voltage now, ibus = %d\n", pdpm->whole_ibus);
		}else if (final_step > 0 && pdpm->whole_ibus >= 4450) {
				final_step = 0;
				pdm_err("NO more voltage needs, ibus = %d\n", pdpm->whole_ibus);
		}
		if (pdpm->apdo_max_ibus > 5000)
				pdpm->apdo_max_ibus = 5000;
		pdm_err("pmic_parachg_mode limit final step = %d\n", final_step);
	}
...
*/
	...
	case PD_PM_STATE_EXIT:
		...
		if (pdpm->pd_type == MTK_PD_CONNECT_PE_READY_SNK_APDO) {
			...
			if (!device_inited) {
				mca_log_err("cp_cur_vbus = %d not < %d after 10 retries, still cp_device_init\n", cp_cur_vbus, exit_vbus_threshold);
				charger_dev_cp_device_init(pdpm->master_dev, CP_FORWARD_1_TO_1);
			}
		}
		pdpm->enable_cp_count = 0;
