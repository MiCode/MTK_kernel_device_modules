...
static bool pdm_check_cp_dev(struct usbpd_pm *pdpm)
{
	...
	if (!pdpm->mtk_charger)
		pdpm->mtk_charger = get_charger_by_name("mtk_charger");

	if (!pdpm->mtk_charger) {
		pdm_err("failed to get mtk_charger\n");
		return false;
	}
	return true;
}
...
static bool pdm_evaluate_src_caps(struct usbpd_pm *pdpm)
{
	...
	//charger_dev_usb_get_property(pdpm->mtk_charger, USB_PROP_ADAPTER_ID, &temp);
	pdpm->adapter_id = temp;
  	charger_dev_usb_get_property(pdpm->mtk_charger, USB_PROP_CHARGE_FULL, &temp);
...
//			ret = adapter_dev_set_cap_xm(pinfo->pd_adapter, MTK_PD_APDO_START, DEFAULT_PDO_VBUS_1S, DEFAULT_PDO_IBUS_1S);
	...
	if (pinfo->lpd_charging_limit || fg_status & FG_EER_I2C_FAIL || fg_status & FG_ERR_AUTH_FAIL || fg_status & FG_ERR_CHG_WATT || pinfo->typec_burn_status) {
		adapter_dev_set_cap_xm(pinfo->pd_adapter, MTK_PD_APDO, pdpm->cap.max_mv[0], pdpm->cap.ma[0]);
		pdm_err("ERROR: request pdo %dmv %dma\n", pdpm->cap.max_mv[0], pdpm->cap.ma[0]);
	}
...
static bool pdm_taper_charge(struct usbpd_pm *pdpm)
{
	...
	if (pdpm->taper_count > MAX_TAPER_COUNT) {
		if (pdpm->jeita_chg_index == MAX_JEITA_CHG_INDEX)
			vote(pinfo->fcc_votable, CP_CHG_DONE, true, cv_ibat);
		return true;
	} else
		return false;
}
static void pdm_update_status(struct usbpd_pm *pdpm)
{
	...
	power_supply_get_property(pdpm->batt_psy, POWER_SUPPLY_PROP_CURRENT_NOW, &val);
	pdpm->ibat = val.intval / 1000;
	power_supply_get_property(pdpm->batt_psy, POWER_SUPPLY_PROP_VOLTAGE_NOW, &val);
	pdpm->vbat = val.intval / 1000;
	power_supply_get_property(pdpm->batt_psy, POWER_SUPPLY_PROP_CAPACITY, &val);
	pdpm->soc = val.intval;
	power_supply_get_property(pdpm->batt_psy, POWER_SUPPLY_PROP_CYCLE_COUNT, &val);
	pdpm->cycle_count = val.intval;
	charger_dev_usb_get_property(pdpm->mtk_charger, USB_PROP_JEITA_CHG_INDEX, &temp);
	pdpm->jeita_chg_index = temp;
	charger_dev_usb_get_property(pdpm->mtk_charger, USB_PROP_FFC_ENABLE, &temp);
	pdpm->ffc_enable = temp;
	charger_dev_usb_get_property(pdpm->mtk_charger, USB_PROP_CHARGE_FULL, &temp);
	pdpm->charge_full = temp;
	charger_dev_usb_get_property(pdpm->mtk_charger, USB_PROP_SW_CV, &temp);
	pdpm->sw_cv = temp;
	charger_dev_usb_get_property(pdpm->mtk_charger, USB_PROP_INPUT_SUSPEND, &temp);
	pdpm->input_suspend = temp;
	charger_dev_usb_get_property(pdpm->mtk_charger, USB_PROP_TYPEC_BURN, &temp);
	pdpm->typec_burn = 0;
	charger_dev_usb_get_property(pdpm->mtk_charger, USB_PROP_PD_AUTHENTICATION, &temp);
	pdpm->pd_verifed = temp;
	charger_dev_night_charging_get_flag(pdpm->mtk_charger, &pdpm->night_charging);
	...
	charger_dev_smart_soclmt_get_flag(pdpm->mtk_charger, &pdpm->smart_soclmt_trig);
	...
	master_cp_ibat = pdpm->master_cp_ibus * 2;
	if (master_cp_ibat > 12700) {
		mtk_charger_fw_notifier_call_chain(CHG_FW_EVT_CP_IBAT_OCP, NULL);
	}
	...
	pdm_info("master_cp_bypass=%d, bypass_support=%d, thermal_limit_fcc=%d, night_charging=%d, smart_soclmt=%d\n",
		pdpm->master_cp_bypass, pdpm->bypass_support, pdpm->thermal_limit_fcc,  pdpm->night_charging, pdpm->smart_soclmt_trig);
}
...
static void pdm_multi_mode_switch(struct usbpd_pm *pdpm)
{
	...
	pdpm->vbus_low_gap = pdpm->vbat * res * 5/100;
...
static int pdm_tune_pdo(struct usbpd_pm *pdpm)
{
	...
	{
		//fv = pdpm->ffc_enable ? (pdpm->dts_config.fv_ffc - 4) : ((pdpm->jeita_chg_index == 2 || pdpm->jeita_chg_index == 3)? pdpm->dts_config.fv - 10:pdpm->dts_config.fv);
...
*/
	...
	ibus_limit = min(min((pdpm->target_fcc / res - 100), pdpm->apdo_max_ibus), pdpm->dts_config.max_ibus);
	/*remove limit ibus */
	...
	else if ((fv - pdpm->vbat <= 5) && (fv - pdpm->vbat >= 0))
		pdpm->vbat_step = ZERO_STEP;
	else if (fv - pdpm->vbat < -5)
		pdpm->vbat_step = -MEDIUM_STEP;
...
static int pdm_check_condition(struct usbpd_pm *pdpm)
{
	...
	struct fuel_gauge_dev *gauge = fuel_gauge_find_dev_by_name("fuel_gauge");
	int fg_status = fuel_gauge_check_fg_status(gauge);
	char *cp_name = charger_dev_get_cp_dev_name(pdpm->master_dev);
	...
	if (pdpm->jeita_chg_index != MAX_JEITA_CHG_INDEX)
		vote(pinfo->fcc_votable, CP_CHG_DONE, false, 0);

	pdm_err("min_thermal_limit_fcc =%d\n", min_thermal_limit_fcc);
	if (pinfo->warm_term || (fg_status & FG_ERR_AUTH_FAIL) || pinfo->lpd_charging_limit) {
		pdm_err("PDM_SM_HOLD warm_term =%d, fg_status = %d, limit=%d\n", pdpm->state, fg_status, pinfo->lpd_charging_limit);
		return PDM_SM_HOLD;
        } else if (pdpm->state == PD_PM_STATE_TUNE && pdm_taper_charge(pdpm)) {
		pdm_err("pdm_taper_charge state=%d\n", pdpm->state);
		if (pdpm->charge_full || pdpm->jeita_chg_index < MAX_JEITA_CHG_INDEX)
			return PDM_SM_EXIT;
		else
			return PDM_SM_HOLD;
		...
		return PDM_SM_EXIT;
	} else if (fg_status & FG_EER_I2C_FAIL || fg_status & FG_ERR_CHG_WATT) {
		pdm_err("PDM_SM_EXIT fg status error=%d\n", fg_status);
		return PDM_SM_EXIT;
	} else if (((strstr(cp_name, "sc8541") && pdpm->vbat <= 3100) || (strstr(cp_name, "bq25960") && pdpm->vbat <= 3300)) && pdpm->state == PD_PM_STATE_ENTRY) {
		pdm_err("cp device not ready to charge\n");
		return PDM_SM_HOLD;
	}
...
static bool pdm_handle_sm(struct usbpd_pm *pdpm)
{
	...
	case PD_PM_STATE_ENTRY:
		...
		if (pdpm->sm_status == PDM_SM_EXIT) {
			...
			charger_dev_usb_set_property(pdpm->mtk_charger, USB_PROP_CP_SM_RUN_STATE, true);
	...
	case PD_PM_STATE_INIT_VBUS:
		...
		pdpm->request_voltage = min(pdpm->request_voltage, min(pdpm->dts_config.max_vbus, pdpm->apdo_max_vbus));
	...
	case PD_PM_STATE_ENABLE_CP:
		...
		if (pdpm->master_cp_enable) {
			...
			mtk_charger_fw_notifier_call_chain(CHG_FW_EVT_CP_EN, &pdpm->master_cp_enable);
	...
	case PD_PM_STATE_TUNE:
		...
		if (pdpm->sm_status == PDM_SM_EXIT) {
			...
			if (ret == MTK_ADAPTER_ERROR || ret == MTK_ADAPTER_ADJUST) {
				...
				pdm_err("MTK_ADAPTER_ERROR or MTK_ADAPTER_ADJUST\n");
				pdm_move_sm(pdpm, PD_PM_STATE_EXIT);
			}
		}
		break;
	case PD_PM_STATE_EXIT:
		...
		if (!pdpm->input_suspend) {
			...
			msleep(300);
			pdm_info("input_suspend is false enable buck_en\n");
		}
		charger_dev_enable(pdpm->master_dev, false);
		charger_dev_usb_set_property(pdpm->mtk_charger, USB_PROP_CP_SM_RUN_STATE, false);
		msleep(100);
		if (pdpm->pd_type == MTK_PD_CONNECT_PE_READY_SNK_APDO) {
			...
			ret = adapter_dev_set_cap_xm(pinfo->pd_adapter, MTK_PD_APDO, req_vbus, req_ibus);
			if (ret == MTK_ADAPTER_ERROR || ret == MTK_ADAPTER_ADJUST) {
				adapter_dev_set_cap_xm(pinfo->pd_adapter, MTK_PD_APDO, req_vbus, req_ibus-1000);
				pdm_err("exit request fail, retry vbus=%d ibus=%d\n", req_vbus, req_ibus-1000);
			}
		}
		msleep(500);
		charger_dev_get_vbus(pdpm->charger_dev, &pdpm->mt6375_vbus);
		pdm_err("pdm exit vbus=%d\n", pdpm->mt6375_vbus);
		if (pdpm->mt6375_vbus < 10000000 && gpio_is_valid(pdpm->vbus_control_gpio) && !pdpm->smart_soclmt_trig)
			gpio_set_value(pdpm->vbus_control_gpio, 0);

		if (pdpm->mt6375_vbus < 6500000)
			charger_dev_set_mivr(pdpm->charger_dev, 4600000);

		if (!pdpm->input_suspend && !pdpm->smart_soclmt_trig) {
			charger_dev_enable_powerpath(pdpm->charger_dev, true);
...
static void pdm_main_sm(struct work_struct *work)
{
	...
	if (!pdm_handle_sm(pdpm) && pdpm->pdm_active) {
		...
		pdpm->main_sm_work_running = true;
	} else {
		pdpm->main_sm_work_running = false;
		pdm_err("stop main_sm_work\n");
	}
}
...
static void usbpd_pm_disconnect(struct usbpd_pm *pdpm)
{
	...
	charger_dev_usb_set_property(pdpm->mtk_charger, USB_PROP_APDO_MAX, 0);
	charger_dev_usb_set_property(pdpm->mtk_charger, USB_PROP_CP_SM_RUN_STATE, false);
...
static void pdm_psy_change(struct work_struct *work)
{
	...
	ret = charger_dev_usb_get_property(pdpm->mtk_charger, USB_PROP_PD_TYPE, &val);
	if (ret) {
	...
	ret = charger_dev_usb_get_property(pdpm->mtk_charger, USB_PROP_PD_VERIFY_DONE, &val);
	if (ret) {
	...
	ret = charger_dev_usb_get_property(pdpm->mtk_charger, USB_PROP_CP_CHARGE_RECOVERY, &val);
	if (ret) {
	...
	pdm_info("[pd_type pd_verify_process pdm_active suspend_recovery] = [%d %d %d %d], ibat:%d\n", pdpm->pd_type, pdpm->pd_verify_done, pdpm->pdm_active, pdpm->suspend_recovery, pinfo->current_now);
	if ((pinfo->current_now > 0 && pinfo->soc < 90) && !pdpm->main_sm_work_running && (pdpm->pd_type == MTK_PD_CONNECT_PE_READY_SNK_APDO) && pdpm->pd_verify_done && (fg_status & FG_BATT_AUTH_DONE)) {
		pdpm->pdm_active = false;
	}

	if ((!pdpm->pdm_active || pdpm->suspend_recovery) && (pdpm->pd_type == MTK_PD_CONNECT_PE_READY_SNK || pdpm->pd_type == MTK_PD_CONNECT_PE_READY_SNK_PD30 || pdpm->pd_type == MTK_PD_CONNECT_PE_READY_SNK_APDO) && pdpm->pd_verify_done && (fg_status & FG_BATT_AUTH_DONE)) {
		if (pdm_evaluate_src_caps(pdpm)) {
			...
			if(pdpm->suspend_recovery)
				charger_dev_usb_set_property(pdpm->mtk_charger, USB_PROP_CP_CHARGE_RECOVERY, !pdpm->suspend_recovery);
			schedule_delayed_work(&pdpm->main_sm_work, msecs_to_jiffies(200));
			pdpm->main_sm_work_running = true;
		}
