...
static bool pdm_taper_charge(struct usbpd_pm *pdpm)
{
	...
	cv_vbat = cv_vbat < (cv_vote - 30) ? cv_vbat : (cv_vote - 30);
	if (pdpm->charge_full)
		return true;
...
static void pdm_multi_mode_switch(struct usbpd_pm *pdpm)
{
	...
	pdpm->vbus_low_gap = pdpm->vbat * res * 10/100;
	pdpm->vbus_high_gap = 	pdpm->vbat * res * 20/100;
...
static int pdm_check_condition(struct usbpd_pm *pdpm)
{
	...
	ret = usb_get_property(USB_PROP_IS_ATO_SOC_CONTROL, &val);
	if (!ret) {
		pdm_err("PDM_SM_HOLD is_ato_soc_control:%d\n", val);
		if (val)
			return PDM_SM_HOLD;
	}
#endif
...
#ifdef CONFIG_FACTORY_BUILD
		if(pdpm->is_ato_soc_control) {
			msleep(1000);
			pdm_err("is_ato_soc_control pdm exit, disable charging\n");
			charger_dev_enable(pdpm->charger_dev, false);
		}
#endif
