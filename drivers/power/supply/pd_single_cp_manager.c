...
static bool pdm_evaluate_src_caps(struct usbpd_pm *pdpm)
{
	...
	if(!pdpm->batt_psy)
	{
		mca_log_err("bat_psy is NULL，reset batt_psy\n");
		pdpm->batt_psy = power_supply_get_by_name("battery");
	}
	bm = (struct mtk_battery_manager *)power_supply_get_drvdata(pdpm->batt_psy);
	battery_drvdata = bm->gm1;
	mca_log_err("hifi_connected=%d\n", battery_drvdata->hifi_connected);
	...
	for (i = 0; i < pdpm->cap.nr; i++) {
		...
		if (pdpm->cap.ma[i] > 2800)
			pps_current = 2800;
		else
			pps_current = pdpm->cap.ma[i] - 200;
		mca_log_err("pps adapter AICR can't above 2.8A , current = %d\n", pps_current);
		vote(pinfo->icl_votable, ADAPTER_ICL_VOTER, true, pps_current);
		legal_pdo = true;
	}

	mca_log_err("pd_type=%d, pd_4_1_mode=%d, pd_1_1_mode=%d\n",
		...
		for (i = 0; i <= pdpm->cap.nr - 1; i++) {
			...
			if (pdpm->cap.max_mv[i] == PD2_VBUS && !pdpm->charge_full) {
				if (pdpm->cap.ma[i] < 2000) {
					pd20_current = pdpm->cap.ma[i] - 300;
					mca_log_err("below 18W PD2, AICR decrease 300ma\n");
				} else
					pd20_current = 1800;
				vote(pinfo->icl_votable, ADAPTER_ICL_VOTER, true, pd20_current);
				ret = adapter_dev_set_cap(pinfo->pd_adapter, MTK_PD_APDO, pdpm->cap.max_mv[i], pdpm->cap.ma[i]);
				mca_log_err("[REV_18W_CHG] MAX_fixed_PDO = [%d %d %d], charge_full:%d\n", pdpm->cap.max_mv[i], pdpm->cap.ma[i], pdpm->cap.maxwatt[i] / 1000000, pdpm->charge_full);
			}
		}
	}
...
void pdm_taper_reduce_current(struct usbpd_pm *pdpm, int cv_ibat)
{
	...
	max_jeita_index = MAX_JEITA_CHG_INDEX_KLEE;

	min_jeta = MIN_JEITA_CHG_INDEX;
	...
	if (pdpm->vbat >= curr_cv_fv - 10) {
		...
		fcc_thres = max(max(curr_iterm, cv_ibat), min_fcc) + 350;
...
static void pdm_update_status(struct usbpd_pm *pdpm)
{
	...
	if (!pdpm->pd_verifed) {
		...
		} else if (pdpm->dts_config.max_pdo_vbus == 11000 && pdpm->target_fcc > 12000) {
...
static void pdm_multi_mode_switch(struct usbpd_pm *pdpm)
{
	...
	pdpm->entry_vbus = min(min(((pdpm->vbat * res) + pdpm->vbus_low_gap + 100), pdpm->dts_config.max_vbus), pdpm->apdo_max_vbus - 500);
...
static int pdm_tune_pmic_charging(struct usbpd_pm *pdpm)
{
	...
	if (pinfo == NULL) {
		mca_log_err("%s: pinfo is NULL!\n", __func__);
		return 0;
	}

	if ((pdpm->pmic_parachg_mode == 0) && pdpm->target_fcc > 9000 && (-1 * pdpm->ibat) >= 9000) {
		/* enable pmic charging */
		pdpm->pmic_parachg_mode = 1;
		usb_set_property(USB_PROP_PMIC_PARACHG_MODE, pdpm->pmic_parachg_mode);
		charger_dev_set_input_current(pdpm->charger_dev, 1600000);
		charger_dev_set_charging_current(pdpm->charger_dev, 3000000);
		charger_dev_enable_powerpath(pdpm->charger_dev, true);
		charger_dev_enable(pdpm->charger_dev, true);
		mca_log_err("%s: enable pmic charging.\n", __func__);
	}

	if ((pdpm->pmic_parachg_mode == 1) && pdpm->whole_ibus < 4500) {
		...
		mca_log_err("%s: disable pmic charging.\n", __func__);
	}
...
*/
	...
	if (pdpm->adapter_id == 0x00) {
		ibus_limit = min(ibus_limit, pdpm->apdo_max_ibus - 200);
	}

	vbus_limit = min(pdpm->dts_config.max_vbus, pdpm->apdo_max_vbus - 500);
	...
	if (ibus_limit - pdpm->total_ibus > LARGE_IBUS_DIFF)
		...
		pdpm->ibus_step = SMALL_STEP;
	else if (ibus_limit - pdpm->total_ibus < -(SMALL_IBUS_DIFF + 110))
		pdpm->ibus_step = -MEDIUM_STEP;
...
static int pdm_check_condition(struct usbpd_pm *pdpm)
{
	...
	if (pdpm->state == PD_PM_STATE_TUNE && pdm_taper_charge(pdpm)) {
		...
		return PDM_SM_EXIT;
	} else if(pdpm->bms_i2c_error_count >= 10 || pdpm->slave_bms_i2c_error_count >= 10 || (!pdpm->bypass_auth_m && !pdpm->gauge_authentic) || !pdpm->slave_gauge_authentic) {
		...
		return PDM_SM_HOLD;
	} else if (!is_between(min_jeta, max_jeita_index, pdpm->jeita_chg_index)) {
		...
		return PDM_SM_HOLD;
	} //else if (pdpm->state == PD_PM_STATE_ENTRY && pdpm->vbat > high_vbat && fv > JEITA_WARM_FV_THR) {
		//mca_log_err("PDM_SM_EXIT state=%d, vbat=%d, fv=%d, thres=%d\n", pdpm->state, pdpm->vbat, fv, high_vbat);
		//return PDM_SM_EXIT;
	//}
...
*/
	...
	case PD_PM_STATE_ENABLE_CP:
		...
		pdpm->sm_status = pdm_check_condition(pdpm);
		if (pdpm->sm_status == PDM_SM_EXIT) {
			mca_log_info("[XMC_PDM] PDM_STATE_ENABLE_CP taper charge done\n");
			pdm_move_sm(pdpm, PD_PM_STATE_EXIT);
			break;
		} else if (pdpm->sm_status == PDM_SM_HOLD) {
			pdm_move_sm(pdpm, PD_PM_STATE_EXIT);
	...
	case PD_PM_STATE_EXIT:
		...
		if (pdpm->pd_type == MTK_PD_CONNECT_PE_READY_SNK_APDO) {
			if (pdpm->enable_cp_count >= MAX_ENABLE_CP_COUNT) {
				...
				charger_dev_set_input_current(pdpm->charger_dev, 1800000);
			...
			mca_log_err("exit request vbus=%d ibus=%d\n", req_vbus, req_ibus);
...
static int pd_policy_parse_dt(struct usbpd_pm *pdpm)
{
	...
	rc = of_property_read_u32(node, "cv_ibat_2_1", &pdpm->dts_config.cv_ibat_2_1);
	if (rc){
		chr_err("failed to parse cv_ibat_2_1\n");
		pdpm->dts_config.cv_ibat_2_1 = 1000;
	}
