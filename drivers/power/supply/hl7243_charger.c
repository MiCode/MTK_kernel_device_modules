...
enum hl7243_notify {
    ...
    HL7243_NOTIFY_IBUSOCP,
    HL7243_NOTIFY_VBUSOVP,
    HL7243_NOTIFY_VBATOVP,
...
static const struct reg_range hl7243_reg_range[] = {
    [HL7243_VUSB_OVP]      = HL7243_CHG_RANGE(11000, 24000, 1000, 11000, false),
    [HL7243_VWPC_OVP]      = HL7243_CHG_RANGE(11000, 23000, 1000, 11000, false),
...
int hl7243_enable_charge(struct hl7243_chip *hl, bool en)
{
    ...
    if(en)
    {
        if((mode==REV_1_4_CON_M)||(mode==REV_1_1_CON_M)||(mode==HL7243_DEV_MODE_MAX))
        {
            dev_err(hl->dev, "%s dont support REV_1_4 and REV_1_1\r\n", __func__);
            //return -1;
        }
        if((mode>=REV_1_4_CON_M)&&(mode<=HL7243_DEV_MODE_MAX))
        {
            ...
            if(mode!=REV_1_2_CON_M) // REV_1_4 and REV_1_1 check vbus state
            {
                ...
                {
                    dev_err(hl->dev, "error %s with vbus 5v set to rev_1_4 or rev_1_1 ", __func__);
                    dnoten = 1;
                }
            }
            ...
            if(mode==REV_1_2_CON_M)
            {
                ret += hl7243_field_write(hl, REG_60, 0x41);
                //ret += hl7243_field_write(hl, REG_1F, 0xF1);
            }else
            {
...
static int hl7243_set_revchg(struct hl7243_chip *hl, bool enable)
{
    int cp_mode;
    bool ovpgate_enable = false, charging_enable = false;

    if (enable) {
        hl7243_set_dev_mode(hl, REV_1_2_CON_M);
        hl7243_enable_usb_gate(hl, true);
        hl7243_init_protection(hl, 1);
        hl7243_rev_chg_config(hl, true);

        hl7243_get_usb_gate_enable(hl, &ovpgate_enable);
        hl7243_get_dev_mode(hl, &cp_mode);
        pr_info("cp mode: %d, ovpgate_enable status: %d", cp_mode, ovpgate_enable);

        if ((cp_mode != REV_1_2_CON_M) || (!ovpgate_enable))
            return -1;
    } else {
        hl7243_enable_charge(hl, false);
        hl7243_set_dev_mode(hl, FWD_4_1_CHG_M);
        hl7243_rev_chg_config(hl, false);

        hl7243_get_dev_mode(hl, &cp_mode);
        hl7243_check_charge_enabled(hl, &charging_enable);
...
static int mtk_hl7243_cp_set_revchg(struct charger_device *chg_dev, bool enable)
{
    struct hl7243_chip *hl = charger_get_data(chg_dev);
    int ret = 0;
    int i = 0;
    ...
    for (i = 0; i < CP_SET_REVCHG_RETRY; i++) {
        ret = hl7243_set_revchg(hl, enable);
        if (ret == 0)
            break;
