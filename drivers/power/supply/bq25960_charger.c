...
static int bq25960_get_adc_tdie(struct bq25960 *bq)
{
	...
	u16 tdie_adc;
	int ret;
	ret = regmap_read(bq->regmap, BQ25960_TDIE_ADC_MSB, &tdie_adc_msb);
	if (ret)
		return ret;
	ret = regmap_read(bq->regmap, BQ25960_TDIE_ADC_LSB, &tdie_adc_lsb);
	if (ret)
		return ret;
...
static void bq25960_notify_fault_status(struct bq25960 *bq, int notify)
{
	...
	case BQ25960_NOTIFY_VACOVP:
		charger_dev_notify(bq->chg_dev, CHARGER_DEV_NOTIFY_VAC_OVP);
		break;

	case BQ25960_NOTIFY_TDIEFLT:
		charger_dev_notify(bq->chg_dev, CHARGER_DEV_NOTIFY_TDIE_FLT);
		break;
...
// reverse output 9V
static int ti_bq25960_set_otg_config(struct charger_device *charger_pump, bool en)
{
	int ret = 0;
	//int val = 0;
	struct bq25960 *bq = charger_get_data(charger_pump);
	pr_err("%s [BQ25960] is %d\n", __func__, en);

	/* TO BE check
	ret = bq25960_field_read(bq, ADC_EN, &val);
	if (val)
		ret |= bq25960_field_write(bq, ADC_EN, false);
	*/

	mutex_lock(&bq->i2c_rw_lock);
	bq25960h_enable_otg(en);
	mutex_unlock(&bq->i2c_rw_lock);
...
static int ti_bq25960_init_protection(struct bq25960 *bq, int forward_work_mode)
{
	...
	if (forward_work_mode == CP_FORWARD_1_TO_1) {
		...
		ret = bq25960_set_busovp_th(bq, 14000);
...
static char* ti_bq25960_show_name(struct charger_device *chg_dev)
{
	return "bq25960";
}
...
static const struct charger_ops bq25960_chargerpump_ops = {
	...
	.cp_set_otg_config = ti_bq25960_set_otg_config,
	...
	.get_cp_dev_name = ti_bq25960_show_name,
};
