...
static int hl7603_parse_dt(struct boost_bypass_dev *bq)
{
    ...
    bq->enable_toff_fast = of_property_read_bool(np, "enable_toff_fast");
    return 0;
}
...
static int hl7603_set_voltage_threshold(struct boost_bypass_dev *bq, u32 vout_threshold)
{
    ...
    pr_err("vout_reg = 0x%02x\n", val);

    return ret;
}

static int config_toff_fast(struct boost_bypass_dev *bq)
{
    uint8_t Q3_toff_rate, new_rate, read_rate;
    int retry_count = 0;
    const int MAX_RETRY = 3;

retry:
    Q3_toff_rate =i2c_smbus_read_byte_data(bq->client, REG_22);
    new_rate = Q3_toff_rate & 0xBF;

    i2c_smbus_write_byte_data(bq->client, REG_A7, A7_UNLOCK_VAL);
    i2c_smbus_write_byte_data(bq->client, REG_22, new_rate);
    i2c_smbus_write_byte_data(bq->client, REG_A7, A7_LOCK_VAL);

    read_rate = i2c_smbus_read_byte_data(bq->client, REG_22);
    if (read_rate != new_rate) {
        retry_count++;
        if (retry_count < MAX_RETRY) {
            goto retry;
        } else {
            pr_err("set toff failed!\n");
            return 0;
        }
    }

    pr_err("set toff succeed\n");
	return 0;
}
