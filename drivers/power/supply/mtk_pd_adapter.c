					     ...
					     int report_idx)
{
	...
	unsigned long evt = TA_DETACH;

#ifdef CONFIG_SUPPORT_SOUTHCHIP_PDPHY
	int port0_after_plugin = 0, port1_after_plugin = 0;
	usb_get_property(USB_PROP_TYPEC_PORT0_AFTER_PLUGIN, &port0_after_plugin);
	usb_get_property(USB_PROP_TYPEC_PORT1_AFTER_PLUGIN, &port1_after_plugin);
	dev_info(info->dev, "%s port0:%d, port1:%d\n", __func__, port0_after_plugin, port1_after_plugin);
	for (i = 0; i < info->nr_port; i++) {
		if ((info->pd_type[i] != MTK_PD_CONNECT_NONE) && port0_after_plugin == 0 && port1_after_plugin == 0) {
			active_idx = i;
			break;
		}
	}
...
#endif
	dev_info(info->dev, "%s index = [%d %d %d] pd_type:%d\n", __func__,report_idx,
					pre_active_idx, active_idx, info->pd_type[pre_active_idx]);
	if (i >= info->nr_port)
		active_idx = pre_active_idx;
	evt = pd_type_to_adapter_event(info->pd_type[active_idx]);
				...
				unsigned long event, void *data)
{
	...
	case TCP_NOTIFY_PD_STATE:
		...
		case  PD_CONNECT_NONE:
			...
			if (port0_temp == 1 || port1_temp == 1) {
				dev_info(info->dev, "%s no need to reset\n", __func__);
				break;
			}
#endif
		...
		case PD_CONNECT_PE_READY_SNK_PD30:
			...
			usb_get_property(USB_PROP_TYPEC_PORT1_AFTER_PLUGIN, &port1_after_plugin);
			dev_info(info->dev, "%s port0:%d, port1:%d\n", __func__, port0_after_plugin, port1_after_plugin);
			if (port0_after_plugin == 1 || port1_after_plugin == 1) {
				dev_info(info->dev, "%s port0 and port1 after plugin no need to set uvdm state\n", __func__);
				break;
			} else
	...
	case TCP_NOTIFY_TYPEC_STATE:
		...
		if (old_state == TYPEC_UNATTACHED &&
			...
			usb_get_property(USB_PROP_TYPEC_PORT0_PLUGIN, &port0_temp);
			if (port0_temp == 1 || port1_temp == 1) {
				dev_info(info->dev, "%s no need reset to MTK_PD_CONNECT_NONE\n", __func__);
				break;
			}
#endif
			mutex_lock(&info->idx_lock);
	...
	case TCP_NOTIFY_SINK_VBUS:
		...
		if (info->project_no == GOYA_GL || info->project_no == KLIMT_GL ||
                    info->project_no == KLEE_CN || info->project_no == TURNER_CN) {
...
static int ssdev_typec_filter_apdo_power_for_report(struct mtk_pd_adapter_info *info, int apdo_max)
{
	if (info->project_no == DALI_CN || info->project_no == DASH_CN || info->project_no == KLEE_CN) {
		if (apdo_max >= 96)
	...
	} else if (info->project_no == RODIN_CN || info->project_no == RODIN_IN || info->project_no == RODIN_GL ||
			info->project_no == KLIMT_GL){
		if (apdo_max >= 96)
			return SSDEV_APDO_MAX_90W;
		else if (apdo_max >= 68 && apdo_max < 96)
			return SSDEV_APDO_MAX_90W;
				 ...
				 int active_idx, struct adapter_power_cap *cap)
{
	...
	if (pd_cap.nr != 0) {
		...
		apdo_max = ssdev_typec_filter_apdo_power_for_report(info, apdo_max);
