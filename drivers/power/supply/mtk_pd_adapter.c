...
static int pd_set_pd_verify_process(struct adapter_device *dev, int verifying)
{
	...
	if (!info->mtk_charger)
		info->mtk_charger = get_charger_by_name("mtk_charger");
	pr_info("[%s] pd verify in process:%d\n", __func__, verifying);
	ret = charger_dev_usb_set_property(info->mtk_charger, USB_PROP_PD_VERIFYING, verifying);
	ret = charger_dev_usb_set_property(info->mtk_charger, USB_PROP_PD_VERIFY_DONE, !verifying);
...
static int pd_get_svid(struct adapter_device *dev)
{
	...
	pr_info("[%s] get adapter message idx=%d ret=%d \n", __func__, active_idx, ret);
	if (ret == TCPM_SUCCESS) {
		...
		if (ret == TCPM_SUCCESS) {
			...
			pr_info("adapter_svid = %04x\n", info->adapter->adapter_svid);
			if (info->adapter->adapter_svid == USB_PD_ZIMI_SVID)
				info->adapter->adapter_svid = USB_PD_MI_SVID;
			pr_info("adapter_svid = %04x\n", info->adapter->adapter_svid);
			pr_info("adapter_id = %08x\n", info->adapter->adapter_id);
...
{
	int i;
	struct tcp_dpm_custom_vdm_data vdm_data = event->tcp_dpm_data.cvdm_data;
...
static int pd_get_pdos(struct adapter_device *dev)
{
	...
	pr_info("[%s] tcpm_inquire_pd_source_cap is %d.\n", __func__, ret);
	if (ret)
		return MTK_ADAPTER_ERROR;
	for (i = 0; i < 7; i++) {
		info->adapter->received_pdos[i] = cap.pdos[i];
		pr_info("[%s]: pdo[%d] { received_pdos is %08x, cap.pdos is %08x}\n",
				 ...
				 int active_idx, struct adapter_power_cap *cap)
{
	...
	if (pd_cap.nr != 0) {
		...
		for (i = 0; i < pd_cap.nr; i++) {
			...
			pr_info("[%s]VBUS = [%d,%d], IBUS = %d, WATT = %d, TYPE = %d\n", __func__,
		...
		charger_dev_usb_set_property(info->mtk_charger, USB_PROP_APDO_MAX, apdo_max);
		charger_dev_usb_set_property(info->mtk_charger, USB_PROP_ADAPTER_IMAX, adapter_imax);
	}
		      ...
		      struct adapter_power_cap *cap)
{
	...
	if (type == MTK_PD_APDO)
		...
		while (timeout < 15) {
			...
			pr_info("[%s] ret=%d\n", __func__, ret);
			if (ret == TCPM_SUCCESS) {
				pr_info("[%s] ready to get pps info\n", __func__);
				ret = charger_dev_usb_set_property(info->mtk_charger, USB_PROP_PD_AUTHENTICATION, 1);
				if (ret < 0)
					pr_err("[%s] failed to set authentication\n", __func__);
				ret = pd_get_cap_pdo(info, active_idx, cap);
				break;
			} else {
				pr_err("[%s] retry times = %d, for PPS ready\n", __func__, timeout);
				timeout++;
				msleep(80);
			}
		}
		...
		ret = charger_dev_usb_set_property(info->mtk_charger, USB_PROP_PD_AUTHENTICATION, 0);
		if (ret < 0)
			pr_err("[%s] failed to set authentication\n", __func__);
	} else
