	...
	u32 cyclecount)
{
    ...
    if (bq->ui_soh < soh_threashold) {
        ...
        mca_log_err("%s: use 800 scheme in advance, ui_soh:%d, soh_threashold:%d, cycle_count:%d",
...
static int bq_battery_soc_smooth_tracking_new(struct bq_fg_chip *bq, int raw_soc, int batt_soc, int batt_ma)
{
	...
	if ((charging_status == POWER_SUPPLY_STATUS_DISCHARGING || charging_status == POWER_SUPPLY_STATUS_NOT_CHARGING ) &&
		bq->rsoc == 0 && last_system_soc >= 1 && ((bq->vbat <= bq->normal_shutdown_vbat + 50 && bq->tbat > 0) || ((bq->vbat <= bq->normal_shutdown_vbat + 50) && bq->tbat <= 0))) {
			...
			mca_log_err("[%s] enter quicky smooth unit_time=%d monitor_delay=%d\n", bq->log_tag, unit_time, bq->monitor_delay);
	}
	if (bq->en_smooth_full) {
		unit_time = 30000;
		mca_log_err("[%s] battery charging full smooth unit_time=%d monitor_delay=%d\n", bq->log_tag, unit_time, bq->monitor_delay);
	}
	...
	if(soc_delta > 1 || (bq->vbat < 3400 && system_soc > 0) || (unit_time != 10000 && soc_delta == 1)){
		...
		mca_log_err("[%s] fg jump smooth soc_changed=%d\n", bq->log_tag, soc_changed);
	}
	...
	if (((charging_status == POWER_SUPPLY_STATUS_DISCHARGING) && (system_soc > last_system_soc)) || ((charging_status == POWER_SUPPLY_STATUS_CHARGING) && (system_soc < last_system_soc) && (ibat_pos_count < 3) && ((time.tv_sec > 10))))
		system_soc = last_system_soc;
	mca_log_err("[%s] smooth_new:sys_soc:%d last_sys_soc:%d soc_delta:%d charging_status:%d unit_time:%d batt_ma_avg=%d\n" ,
		bq->log_tag, system_soc, last_system_soc, soc_delta, charging_status, unit_time, batt_ma_avg);
	...
	if (bq->rsoc == 0 && ((bq->vbat < (bq->normal_shutdown_vbat -100) && bq->tbat > 0) || (bq->vbat < (bq->normal_shutdown_vbat -100) && bq->tbat < 0))) {
		...
		unit_time = 2000;
		bq->monitor_delay = FG_MONITOR_DELAY_2S;
		mca_log_err("[%s] uisoc::set 0 when volt = %dmV\n", bq->log_tag, bq->vbat);
		...
		system_soc = 1;
		mca_log_err("[%s] uisoc::hold 1 when volt = %dmV. \n", bq->log_tag, bq->vbat);
...
static void battery_shutdown_vol_update(struct bq_fg_chip *bq)
{
	...
	if (product_name == RODIN_CN || product_name == RODIN_IN) {
		if (bq->fake_dod_count) {
			...
			mca_log_err("%s: fake_dod_count=%d \n", __func__, bq->fake_dod_count);
		} else {
			/* read count */
			ret = fg_mac_read_block(bq, FG_MAC_CMD_MIXDATA1, t_buf, 32);
			if (ret) {
				mca_log_info("failed to get FG_MAC_CMD_MIXDATA1\n");
			} else {
				mca_log_err("%s: FG_MAC_CMD_MIXDATA1: %s\n", __func__, t_buf);
			}
		...
		mca_log_err("%s: count=%d,%d,%d,%d vol=%d,%d,%d\n", __func__,
		...
		if (shutdown_vol != bq->shutdown_voltage) {
			...
			if (ret) {
				mca_log_info("%s:dynamic_shutdown_vol failed to write FG_MAC_CMD_MIX_TERM_VOLT\n", __func__);
			}
...
static void fg_update_status(struct bq_fg_chip *bq)
{
	...
	mca_log_err("[%s] fg_update rsoc=%d, raw_soc=%d, vbat=%d, cycle_count=%d\n", bq->log_tag, bq->rsoc, bq->raw_soc, bq->vbat, bq->cycle_count);
...
out:
		mca_log_err("[%s] [FG_STATUS] [UISOC RSOC RAWSOC TEMP_SOC SOH UISOH] = [%d %d %d %d %d %d], [VBAT CELL0 CELL1 IBAT TBAT FC FAST_MODE] = [%d %d %d %d %d %d %d]\n",
			bq->log_tag, bq->ui_soc, bq->rsoc, bq->raw_soc, temp_soc, bq->soh, bq->ui_soh, bq->vbat, bq->cell_voltage[0], bq->cell_voltage[1], bq->ibat, bq->tbat, bq->batt_fc, bq->fast_chg);

		delta_temp = abs(last_temp - bq->tbat);
		if (bq->batt_psy && (last_soc != bq->ui_soc || delta_temp > 5 || bq->ui_soc == 0 || bq->rsoc == 0)) {
			mca_log_err("[%s] last_soc = %d, last_temp = %d, delta_temp = %d\n", bq->log_tag, last_soc, last_temp, delta_temp);
...
static int ui_soh_get(struct bq_fg_chip *gm)
{
	...
	mca_log_err("%s %d", __func__, ui_soh_data[0]);
...
static int ops_fg_read_cyclecount(struct fg_device *fg_dev, int *cyclecount)
{
	...
	*cyclecount = fg_read_cyclecount(bq);
	mutex_unlock(&bq->data_lock);
	mca_log_err("%s cycletount = %d \n", bq->log_tag, *cyclecount);
...
static int ops_fg_set_in_sleep(struct fg_device *fg_dev, int sleep)
{
	struct bq_fg_chip *bq = fuelgauge_get_data(fg_dev);
	if(bq == NULL)
		return -1;
...
static const struct fg_ops fuelgauge_dev_ops = {
	...
	.set_fg_in_sleep = ops_fg_set_in_sleep,
};
