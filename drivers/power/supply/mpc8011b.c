...
static int fg_sha256_auth(struct bq_fg_chip *bq, u8 *challenge, int length)
{
	...
	*/
	...
	if (ret < 0) {
		mutex_unlock(&bq->i2c_rw_lock);
		fg_err("write BQ_FG_REG_ALT_MAC fail, ret = %d\n", ret);
		return ret;
	}
	/*
	...
	*/
	...
	if (ret < 0) {
		mutex_unlock(&bq->i2c_rw_lock);
		fg_err("write BQ_FG_REG_MAC_DATA fail, ret = %d\n", ret);
		return ret;
	}
	...
	if (ret < 0) {
		mutex_unlock(&bq->i2c_rw_lock);
		fg_err("write BQ_FG_REG_MAC_CHKSUM fail, ret = %d\n", ret);
		return ret;
	}
	...
	if (ret < 0) {
		mutex_unlock(&bq->i2c_rw_lock);
		fg_err("write BQ_FG_REG_MAC_DATA_LEN fail, ret = %d\n", ret);
		return ret;
	}
	...
	mutex_unlock(&bq->i2c_rw_lock);
	if (ret < 0) {
		fg_err("read BQ_FG_REG_MAC_DATA fail, ret = %d\n", ret);
		return ret;
	}
...
static int fg_read_charging_status(struct bq_fg_chip *bq)
{
	u16 status = 0;
	bool retry = false;
	int ret = 0;
...
retry:
	ret = fg_read_word(bq, bq->regs[SC_FG_CHARGING_STATUS], &status);
	if (ret < 0) {
		if (!retry) {
			retry = true;
			msleep(10);
			...
			mca_log_err("%s failed to read charging status\n", bq->log_tag);
		}
	}
	fg_err("%s charging status:%x\n", bq->log_tag, status);
	return 0;
}

static int fg_read_volt_range(struct bq_fg_chip *bq)
{
	u16 status = 0;
	bool retry = false;
	int ret = 0;
...
retry:
	ret = fg_read_word(bq, bq->regs[SC_FG_VOLT_RANGE], &status);
	if (ret < 0) {
		if (!retry) {
			retry = true;
			msleep(10);
			...
			mca_log_err("%s failed to read volt range\n", bq->log_tag);
		}
	}
	fg_err("%s volt_range:%x\n", bq->log_tag, status);
	return 0;
}

static int fg_read_temp_range(struct bq_fg_chip *bq)
{
	u16 status = 0;
	bool retry = false;
	int ret = 0;
...
retry:
	ret = fg_read_word(bq, bq->regs[SC_FG_TEMP_RANGE], &status);
	if (ret < 0) {
		if (!retry) {
			retry = true;
			msleep(10);
	...
	fg_err("%s temp_range:%x\n", bq->log_tag, status);
	return 0;
}

static int fg_ext_cmd_read_operation_status(struct bq_fg_chip *bq)
{
	int ret = 0;
	u8 t_buf[40];
	...
	if (ret < 0) {
		mutex_unlock(&bq->i2c_rw_lock);
		mca_log_err(" fail to write operation status\n");
		return ret;
	}

	msleep(3);

	ret = fg_read_block(bq, bq->regs[BQ_FG_REG_MAC_DATA], t_buf, 2);
	if (ret < 0) {
		mutex_unlock(&bq->i2c_rw_lock);
		mca_log_err(" fail to read operation status\n");
		return ret;
	}
	mutex_unlock(&bq->i2c_rw_lock);
	fg_err("%s fg operation_status:[%x %x]\n", bq->log_tag, t_buf[0], t_buf[1]);
	return ret;
}

static int fg_ext_cmd_read_dev_status(struct bq_fg_chip *bq)
{
	int ret = 0;
	u8 t_buf[40];
	...
	if (ret < 0) {
		mutex_unlock(&bq->i2c_rw_lock);
		mca_log_err(" fail to write dev status\n");
		return ret;
	}
	...
	if (ret < 0) {
		mutex_unlock(&bq->i2c_rw_lock);
		mca_log_err(" fail to read dev status\n");
		return ret;
	}
	mutex_unlock(&bq->i2c_rw_lock);
	fg_err("%s fg dev_status:[%x %x]\n", bq->log_tag, t_buf[0], t_buf[1]);
	return ret;
}

static int fg_ext_cmd_read_host_cmd_status(struct bq_fg_chip *bq)
{
	int ret = 0;
	u8 t_buf[40];
	...
	if (ret < 0) {
		mutex_unlock(&bq->i2c_rw_lock);
		mca_log_err(" fail to write host cmd status\n");
		return ret;
	}
	...
	if (ret < 0) {
		mutex_unlock(&bq->i2c_rw_lock);
		mca_log_err(" fail to read host cmd status\n");
		return ret;
	}
	mutex_unlock(&bq->i2c_rw_lock);
	fg_err("%s fg host_cmd_status:[%x %x]\n", bq->log_tag, t_buf[0], t_buf[1]);
	return ret;
}

static int fg_ext_cmd_read_term_volt(struct bq_fg_chip *bq)
{
	int ret = 0;
	u8 t_buf[40];
	...
	if (ret < 0) {
		mutex_unlock(&bq->i2c_rw_lock);
		mca_log_err(" fail to write term volt\n");
		return ret;
	}
	...
	if (ret < 0) {
		mutex_unlock(&bq->i2c_rw_lock);
		mca_log_err(" fail to read term volt\n");
		return ret;
	}
	mutex_unlock(&bq->i2c_rw_lock);
	fg_err("%s fg term_volt:[%x %x]\n", bq->log_tag, t_buf[0], t_buf[1]);
	return ret;
}

static int fg_ext_cmd_read_xt_status(struct bq_fg_chip *bq)
{
	int ret = 0;
	u8 t_buf[40];
	...
	if (ret < 0) {
		mutex_unlock(&bq->i2c_rw_lock);
		mca_log_err(" fail to write xt status\n");
		return ret;
	}

	msleep(3);

	ret = fg_read_block(bq, bq->regs[BQ_FG_REG_MAC_DATA], t_buf, 32);
	if (ret < 0) {
		mutex_unlock(&bq->i2c_rw_lock);
		mca_log_err(" fail to read xt status\n");
		return ret;
	}
	mutex_unlock(&bq->i2c_rw_lock);
	fg_err("%s fg xt_status:[%x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x]\n", 
				bq->log_tag, t_buf[0], t_buf[1], t_buf[2], t_buf[3], t_buf[4], t_buf[5], t_buf[6], t_buf[7], t_buf[8], t_buf[9], 
				t_buf[10], t_buf[11], t_buf[12], t_buf[13], t_buf[14], t_buf[15], t_buf[16], t_buf[17], t_buf[18], t_buf[19],
				t_buf[20], t_buf[21], t_buf[22], t_buf[23], t_buf[24], t_buf[25]);
	return ret;
}

static int fg_ext_cmd_read_xt_status2(struct bq_fg_chip *bq)
{
	int ret = 0;
	u8 t_buf[40];
	...
	if (ret < 0) {
		mutex_unlock(&bq->i2c_rw_lock);
		mca_log_err(" fail to write xt status2\n");
		return ret;
	}
	...
	if (ret < 0) {
		mutex_unlock(&bq->i2c_rw_lock);
		mca_log_err(" fail to read xt status2\n");
		return ret;
	}
	mutex_unlock(&bq->i2c_rw_lock);
	fg_err("%s fg xt_status2:[%x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x]\n", 
				bq->log_tag, t_buf[0], t_buf[1], t_buf[2], t_buf[3], t_buf[4], t_buf[5], t_buf[6], t_buf[7], t_buf[8], t_buf[9], 
				t_buf[10], t_buf[11], t_buf[12], t_buf[13], t_buf[14], t_buf[15], t_buf[16], t_buf[17], t_buf[18], t_buf[19],
				t_buf[20], t_buf[21], t_buf[22], t_buf[23], t_buf[24], t_buf[25], t_buf[26], t_buf[27], t_buf[28], t_buf[29]);
	return ret;
}

static int fg_ext_cmd_read_xt_status3(struct bq_fg_chip *bq)
{
	int ret = 0;
	u8 t_buf[40];
	...
	if (ret < 0) {
		mutex_unlock(&bq->i2c_rw_lock);
		mca_log_err(" fail to write xt status3\n");
		return ret;
	}
	...
	if (ret < 0) {
		mutex_unlock(&bq->i2c_rw_lock);
		mca_log_err(" fail to read xt status3\n");
		return ret;
	}
	mutex_unlock(&bq->i2c_rw_lock);
	fg_err("%s fg xt_status3:[%x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x]\n", 
				bq->log_tag, t_buf[0], t_buf[1], t_buf[2], t_buf[3], t_buf[4], t_buf[5], t_buf[6], t_buf[7], t_buf[8], t_buf[9], 
				t_buf[10], t_buf[11], t_buf[12], t_buf[13], t_buf[14], t_buf[15], t_buf[16], t_buf[17], t_buf[18], t_buf[19],
				t_buf[20], t_buf[21], t_buf[22], t_buf[23], t_buf[24], t_buf[25], t_buf[26], t_buf[27]);
	return ret;
}

static int fg_read_real_temp(struct bq_fg_chip *bq, int *temp)
{
	u16 tbat = 0;
	int ret = 0;
...
static void fg_read_qmax(struct bq_fg_chip *bq)
{
	...
	if (bq->device_name == BQ_FG_BQ27Z561 || bq->device_name == BQ_FG_NFG1000A || bq->device_name == BQ_FG_NFG1000B || bq->device_name == SC_FG_58561) {
...
static int fg_read_voltage_max(struct bq_fg_chip *bq, int *val) {
...
static int fg_read_temp_max(struct bq_fg_chip *bq, int *val) {
	u8 data_limetime1[32];
	int ret = 0;
...
static int fg_read_over_vol_duration(struct bq_fg_chip *bq, int *val) {
	u8 t_buf[32] = {0};
	int ret = 0;
...
int bms_get_batt_sn(struct bq_fg_chip *gm, char *buf)
{
	u8 t_buf[32] = {0};
	int ret = 0, i = 0;
	...
	int *val)
{
	...
	ret = fg_read_real_temp(gm, val);
	if (ret < 0) {
		*val = 250;
		mca_log_err("%s read real temp failed, ret=%d\n",
			    gm->log_tag, ret);
		return ret;
	}
	fg_err("%s %d\n", gm->log_tag, *val);
...
static int battery_num_get(struct bq_fg_chip *gm,
	struct mtk_bms_sysfs_field_info *attr,
	int *val)
{
	if (gm) {
	...
	int val)
{
	if (gm)
	{
		 if (gm->dev_role == FUELGAUGE_MASTER && gpio_is_valid(gm->batt1_control_gpio))
		{
			...
			fg_err("success to reset batt1_control_gpio %d\n", !!val);
		}
		if (gm->dev_role == FUELGAUGE_SLAVE && gpio_is_valid(gm->batt2_control_gpio))
		{
			...
			fg_err("success to reset batt2_control_gpio %d\n", !!val);
		}
	}
	mca_log_err("%s %d\n", gm->log_tag, val);
	return 0;
	...
	if (gm){
		 if (gm->dev_role == FUELGAUGE_MASTER && gpio_is_valid(gm->batt1_control_gpio))
		{
			*val = gpio_get_value(gm->batt1_control_gpio); 
			fg_err("get batt1_control_gpio val:%d\n", *val);
		}
		if (gm->dev_role == FUELGAUGE_SLAVE && gpio_is_valid(gm->batt2_control_gpio))
		{
			*val = gpio_get_value(gm->batt2_control_gpio); 
			fg_err("get batt2_control_gpio val:%d\n", *val);
		}
	}else
		*val = 0;
	fg_err("%s %d\n", gm->log_tag, *val);
...
static struct mtk_bms_sysfs_field_info bms_sysfs_field_tbl[] = {
	...
	BMS_SYSFS_FIELD_RO(battery_num, BMS_PROP_BATTERY_NUM),
...
static int fg_get_property(struct power_supply *psy, enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_TEMP:
		...
		fg_read_temperature(bq, &tbat_temp);
		bq->tbat = tbat_temp;
		if (!battery_get_psy(bq)) {
	...
	case POWER_SUPPLY_PROP_CHARGE_FULL:
		fg_read_fcc(bq);
		val->intval = bq->fcc * 1000;
		break;
	case POWER_SUPPLY_PROP_CHARGE_FULL_DESIGN:
		bq->dc = fg_read_dc(bq);
		val->intval = bq->dc * 1000;
		break;
...
static int fg_parse_dt(struct bq_fg_chip *bq)
{
	...
	if (of_property_read_u32(node, "fg-type", &bq->fg_type)) {
		mca_log_err("failed to parse fg-type, set default value %d\n", BQ_FG_TYPE_SINGLE);
		bq->fg_type = BQ_FG_TYPE_SINGLE;
	}
...
static int fg_check_device(struct bq_fg_chip *bq)
{
	...
	if (!strncmp(data, "MI", 2)) {
		...
		} else if (!strncmp(&data[2], "J", 1)) {
			bq->device_name = SC_FG_58561;
			bq->battery_vendor = BATTERY_PACK_VENDOR_SC58561;
			strcpy(bq->model_name, "sc58561");
			strcpy(bq->log_tag, "[XMCHG_SC58561]");
		} else {
...
static int ops_fg_read_real_temp(struct fg_device *fg_dev, int *real_temp)
{
	struct bq_fg_chip *bq = fuelgauge_get_data(fg_dev);
	int ret = 0;
	ret = fg_read_real_temp(bq, real_temp);

	return ret;
}

static int ops_fg_read_voltage_max(struct fg_device *fg_dev, int *voltage_max)
{
	struct bq_fg_chip *bq = fuelgauge_get_data(fg_dev);
	if (!bq || !voltage_max) {
		return -EINVAL;
	}
	return fg_read_voltage_max(bq, voltage_max);
}

static int ops_fg_read_temp_max(struct fg_device *fg_dev, int *temp_max)
{
	struct bq_fg_chip *bq = fuelgauge_get_data(fg_dev);
	if (!bq || !temp_max) {
		return -EINVAL;
	}
	return fg_read_temp_max(bq, temp_max);
}

static int ops_fg_read_over_vol_duration(struct fg_device *fg_dev, int *over_vol_duration)
{
	struct bq_fg_chip *bq = fuelgauge_get_data(fg_dev);
	if (!bq || !over_vol_duration) {
		return -EINVAL;
	}
	return fg_read_over_vol_duration(bq, over_vol_duration);
}
...
static int ops_fg_get_batt_sn(struct fg_device *fg_dev, char *buf)
{
	struct bq_fg_chip *bq = fuelgauge_get_data(fg_dev);
	bms_get_batt_sn(bq, buf);
	return 0;
}

static int ops_fg_read_charging_status(struct fg_device *fg_dev)
{
	struct bq_fg_chip *bq = fuelgauge_get_data(fg_dev);
	if(bq->device_name != SC_FG_58561)
		return 0;
	fg_read_charging_status(bq);
	return 0;
}

static int ops_fg_read_volt_range(struct fg_device *fg_dev)
{
	struct bq_fg_chip *bq = fuelgauge_get_data(fg_dev);
	if(bq->device_name != SC_FG_58561)
		return 0;
	fg_read_volt_range(bq);
	return 0;
}

static int ops_fg_read_temp_range(struct fg_device *fg_dev)
{
	struct bq_fg_chip *bq = fuelgauge_get_data(fg_dev);
	if(bq->device_name != SC_FG_58561)
		return 0;
	fg_read_temp_range(bq);
	return 0;
}

static int ops_fg_ext_cmd_read_operation_status(struct fg_device *fg_dev)
{
	struct bq_fg_chip *bq = fuelgauge_get_data(fg_dev);
	if(bq->device_name != SC_FG_58561)
		return 0;
	fg_ext_cmd_read_operation_status(bq);
	return 0;
}

static int ops_fg_ext_cmd_read_dev_status(struct fg_device *fg_dev)
{
	struct bq_fg_chip *bq = fuelgauge_get_data(fg_dev);
	if(bq->device_name != SC_FG_58561)
		return 0;
	fg_ext_cmd_read_dev_status(bq);
	return 0;
}

static int ops_fg_ext_cmd_read_host_cmd_status(struct fg_device *fg_dev)
{
	struct bq_fg_chip *bq = fuelgauge_get_data(fg_dev);
	if(bq->device_name != SC_FG_58561)
		return 0;
	fg_ext_cmd_read_host_cmd_status(bq);
	return 0;
}

static int ops_fg_ext_cmd_read_term_volt(struct fg_device *fg_dev)
{
	struct bq_fg_chip *bq = fuelgauge_get_data(fg_dev);
	if(bq->device_name != SC_FG_58561)
		return 0;
	fg_ext_cmd_read_term_volt(bq);
	return 0;
}

static int ops_fg_ext_cmd_read_xt_status(struct fg_device *fg_dev)
{
	struct bq_fg_chip *bq = fuelgauge_get_data(fg_dev);
	if(bq->device_name != SC_FG_58561)
		return 0;
	fg_ext_cmd_read_xt_status(bq);
	return 0;
}

static int ops_fg_ext_cmd_read_xt_status2(struct fg_device *fg_dev)
{
	struct bq_fg_chip *bq = fuelgauge_get_data(fg_dev);
	if(bq->device_name != SC_FG_58561)
		return 0;
	fg_ext_cmd_read_xt_status2(bq);
	return 0;
}

static int ops_fg_ext_cmd_read_xt_status3(struct fg_device *fg_dev)
{
	struct bq_fg_chip *bq = fuelgauge_get_data(fg_dev);
	if(bq->device_name != SC_FG_58561)
		return 0;
	fg_ext_cmd_read_xt_status3(bq);
	return 0;
}
...
static const struct fg_ops fuelgauge_dev_ops = {
	...
	.read_real_temp = ops_fg_read_real_temp,
	.read_voltage_max = ops_fg_read_voltage_max,
	.read_temp_max = ops_fg_read_temp_max,
	.read_over_vol_duration = ops_fg_read_over_vol_duration,
	...
	.fg_get_batt_sn = ops_fg_get_batt_sn,
	.sc_fg_read_charging_status = ops_fg_read_charging_status,
	.sc_fg_read_volt_range = ops_fg_read_volt_range,
	.sc_fg_read_temp_range = ops_fg_read_temp_range,
	.sc_fg_ext_cmd_read_operation_status = ops_fg_ext_cmd_read_operation_status,
	.sc_fg_ext_cmd_read_dev_status = ops_fg_ext_cmd_read_dev_status,
	.sc_fg_ext_cmd_read_host_cmd_status = ops_fg_ext_cmd_read_host_cmd_status,
	.sc_fg_ext_cmd_read_term_volt = ops_fg_ext_cmd_read_term_volt,
	.sc_fg_ext_cmd_read_xt_status = ops_fg_ext_cmd_read_xt_status,
	.sc_fg_ext_cmd_read_xt_status2 = ops_fg_ext_cmd_read_xt_status2,
	.sc_fg_ext_cmd_read_xt_status3 = ops_fg_ext_cmd_read_xt_status3,
};
