...
static int sc858x_set_revchg(struct sc858x_chip *sc, bool enable)
{
	int cp_mode;
	bool ovpgate_enable = false, charging_enable = false;

	if (enable) {
		sc858x_set_operation_mode(sc, REVERSE_1_2_CONVERTER_MODE);
		sc858x_enable_ovpgate(sc, true);
        sc858x_init_protection(sc, 1);

		sc858x_get_ovpgate_enable(sc, &ovpgate_enable);
		sc858x_get_operation_mode(sc, &cp_mode);
		pr_info("cp mode: %d, ovpgate_enable status: %d", cp_mode, ovpgate_enable);

		if ((cp_mode != REVERSE_1_2_CONVERTER_MODE) || (!ovpgate_enable))
			return -1;
	} else {
		...
		sc858x_get_operation_mode(sc, &cp_mode);
		sc858x_check_charge_enabled(sc, &charging_enable);
		pr_info("cp mode: %d, charging_enable: %d", cp_mode, charging_enable);
...
static int ops_cp_set_revchg(struct charger_device *chg_dev, bool enable)
{
	struct sc858x_chip *sc = charger_get_data(chg_dev);
	int ret = 0;
	int i = 0;
	...
	for (i = 0; i < CP_SET_REVCHG_RETRY; i++) {
		ret = sc858x_set_revchg(sc, enable);
		if (ret == 0)
			break;
