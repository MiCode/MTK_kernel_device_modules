...
__maybe_unused static int sc8541_set_acdrv1_enable(struct sc8541_chip *sc, bool en)
{
	int ret = 0;
...
__maybe_unused static int sc8541_set_acdrv2_enable(struct sc8541_chip *sc, bool en)
{
	int ret = 0;
...
__maybe_unused static int sc8541_disable_ibus_ucp(struct sc8541_chip *sc, bool en)
{
	int ret = 0;

	ret = sc8541_field_write(sc, IBUS_UCP_DIS, en);
	if(ret < 0)
		return ret;
	pr_err("%s %s:%d\n", sc->log_tag, __func__, en);
...
__maybe_unused static int sc8541_disable_vbus_in_range(struct sc8541_chip *sc, bool en)
{
	int ret = 0;

	ret = sc8541_field_write(sc, VBUS_IN_RANGE_DIS, en);
	if(ret < 0)
		return ret;
	pr_err("%s %s:%d\n", sc->log_tag, __func__, en);
...
__maybe_unused static int sc8541_disable_ss_timeout(struct sc8541_chip *sc, bool en)
{
	int ret = 0;

	ret = sc8541_field_write(sc, SS_TIMEOUT_DIS, en);
	if(ret < 0)
		return ret;
	pr_err("%s %s:%d\n", sc->log_tag, __func__, en);
...
__maybe_unused static int sc8541_disable_PMID2OUT_UVP(struct sc8541_chip *sc, bool en)
{
	int ret = 0;

	ret = sc8541_field_write(sc, PMID2OUT_UVP_DIS, en);
	if(ret < 0)
		return ret;
	pr_err("%s %s:%d\n", sc->log_tag, __func__, en);
...
static int sc8541_enable_acdrv_manual(struct sc8541_chip *sc, bool enable)
{
	...
	if (!enable && (typec_port0_status == 3 || typec_port1_status == 3)) {
		pr_err("otg is on, don't reset to auto mode");
		return 0;
	}
#endif
	pr_err("%s manual=%d\n", sc->log_tag, enable);
	return sc8541_field_write(sc, ACDRV_MANUAL_EN, !!enable);
}
...
__maybe_unused static int sc8541_init_device(struct sc8541_chip *sc)
{
	...
	sc8541_enable_acdrv_manual(sc, false);
	sc8541_set_acdrv_enable(sc, true);
...
static int sc_sc8541_enable_acdrv_manual(struct charger_device *chg_dev, bool enable)
{
	...
	ret = sc8541_enable_acdrv_manual(sc, enable);
	if (ret)
...
static int sc_sc8541_set_usb_gate_en(struct charger_device *chg_dev, bool enable)
{
	struct sc8541_chip *sc = charger_get_data(chg_dev);
	int ret = 0;
...
#ifdef CONFIG_SUPPORT_SOUTHCHIP_PDPHY
	ret = sc8541_set_acdrv1_enable(sc, !!enable);
#else
...
static int ops_cp_get_tdie(struct charger_device *chg_dev, u32 *val)
{
	...
	ret = sc8541_get_adc_data(sc, ADC_TDIE, val);
	if (ret)
...
static int ops_cp_get_project_vendor(struct charger_device *chg_dev, int *val)
{
	struct sc8541_chip *sc = charger_get_data(chg_dev);
	int ret = 0;

	if (sc)
		*val = sc->project_vendor;
	else
		*val = CP_CHIP_VENDOR_NONE;

	pr_err("%s project_vendor=%d \n", sc->log_tag, *val);

	return ret;
}

static int sc_sc8541_set_wpcgate_en(struct charger_device *chg_dev, bool enable)
{
	struct sc8541_chip *sc = charger_get_data(chg_dev);
	int ret = 0;

	pr_err("%s: usb_gate_en %d\n", __func__, enable);
	ret = sc8541_set_acdrv2_enable(sc, !!enable);

	return ret;
}

static int sc_sc8541_enable_qb(struct charger_device *chg_dev, bool enable)
{
...
static int sc_sc8541_rev_chg_config(struct charger_device *chg_dev, bool enable)
{
	struct sc8541_chip *sc = charger_get_data(chg_dev);
	int ret = 0;

	pr_err("%s: enable:%d\n", __func__, enable);
	ret |= sc8541_disable_ibus_ucp(sc, !!enable);
	ret |= sc8541_disable_vbus_in_range(sc, !!enable);
	ret |= sc8541_disable_ss_timeout(sc, !!enable);
	ret |= sc8541_disable_PMID2OUT_UVP(sc, !!enable);

	if(ret != 0){
		pr_err("%s failed %s ibus/vbus/ss/uvp protection\n", __func__, enable ? "disable" : "enable");
	}else{
		pr_err("%s success %s ibus/vbus/ss/uvp protection\n", __func__, enable ? "disable" : "enable");
	}
...
static const struct charger_ops sc_sc8541_chargerpump_ops = {
	...
	.set_usb_gate_en = sc_sc8541_set_usb_gate_en,
	.set_wpc_gate_en = sc_sc8541_set_wpcgate_en,
	.cp_chip_ok = ops_cp_get_chip_ok,
	.cp_get_tdie = ops_cp_get_tdie,
	.cp_get_project_vendor = ops_cp_get_project_vendor,
	.cp_enable_wpcgate = sc_sc8541_set_wpcgate_en,
	.cp_set_qb = sc_sc8541_enable_qb,
	.cp_rev_chg_config = sc_sc8541_rev_chg_config,
...
static int sc8541_parse_dt(struct sc8541_chip *sc, struct device *dev)
{
	...
	int i;
	int ret;
	struct pinctrl *cp_pinctrl;
	struct pinctrl_state *cp_cfg;
	struct {
		char *name;
		int *conv_data;
	} props[] = {
	...
	cp_pinctrl = devm_pinctrl_get(sc->dev);
	if (! IS_ERR_OR_NULL(cp_pinctrl)) {
		cp_cfg = pinctrl_lookup_state(cp_pinctrl, "charger_quick_cp");
		if (! IS_ERR_OR_NULL(cp_cfg)) {
			pinctrl_select_state(cp_pinctrl, cp_cfg);
			dev_err(sc->dev, "success to config cp_cfg\n");
		} else {
			dev_err(sc->dev, "failed to parse cp_cfg\n");
		}
	} else {
		dev_err(sc->dev, "sc8541 failed to get pinctrl\n");
	}
	...
	of_property_read_u32(np, "project_vendor", &sc->project_vendor);
	if (ret < 0) {
			dev_err(sc->dev, "can not read\n");
			sc->project_vendor = CP_CHIP_VENDOR_NONE;
	}
