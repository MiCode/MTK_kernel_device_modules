...
static int sc_sc8541_set_otg_config(struct charger_device *charger_pump, bool en)
{
	struct sc8541_chip *sc = charger_get_data(charger_pump);
	int ret = 0;
	...
	//ret = sc8541_enable_adc(sc, en);
	if (en) {
		pr_err("[REVCHG]  is sc_sc8541_set_otg_confige\n");
		sc8541_dump_reg(sc);
		...
		msleep(200);
		pr_err("[REVCHG] in the end : sc8541_dump_reg\n");
		sc8541_dump_reg(sc);
	} else {
...
static char* sc_sc8541_show_name(struct charger_device *chg_dev)
{
	return "sc8541";
}
...
static const struct charger_ops sc_sc8541_chargerpump_ops = {
	...
	.cp_set_otg_config = sc_sc8541_set_otg_config,
	.get_cp_dev_name = sc_sc8541_show_name,
};
/*
...
static void sc8541_notify_fault_status(struct sc8541_chip *sc, int notify)
{
	...
	case SC8541_NOTIFY_IBATOCP:
		charger_dev_notify(sc->chg_dev, CHARGER_DEV_NOTIFY_IBATOCP);
		break;

	case SC8541_NOTIFY_IBUSOCP:
		charger_dev_notify(sc->chg_dev, CHARGER_DEV_NOTIFY_IBUSOCP);
		break;

	case SC8541_NOTIFY_VBATOVP:
		charger_dev_notify(sc->chg_dev, CHARGER_DEV_NOTIFY_BAT_OVP);
		break;

	case SC8541_NOTIFY_VBUSOVP:
		charger_dev_notify(sc->chg_dev, CHARGER_DEV_NOTIFY_VBUS_OVP);
		break;

	case SC8541_NOTIFY_VACOVP:
		charger_dev_notify(sc->chg_dev, CHARGER_DEV_NOTIFY_VAC_OVP);
		break;

	case SC8541_NOTIFY_TDIEFLT:
		charger_dev_notify(sc->chg_dev, CHARGER_DEV_NOTIFY_TDIE_FLT);
		break;
	...
	for (i=0;i < ARRAY_SIZE(cp_intr_flag);i++) {
		...
		for (j=0; j <  cp_intr_flag[i].len; j++) {
			if (flag & cp_intr_flag[i].bit[j].mask) {
				sc8541_notify_fault_status(sc, cp_intr_flag[i].bit[j].notify);
