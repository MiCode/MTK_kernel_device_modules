...
#define ln_err(fmt, ...)                \
do {                                    \
	if (info->dev_role == LN8000_MASTER)   \
...
#define LN8000_IS_PRIMARY(info) (info->dev_role == LN8000_MASTER)
...
static int ln8000_check_work_mode(struct ln8000_info *info, int work_mode)
{
	info->mode = work_mode;
	...
	dev_err(info->dev, "%s: work_mode=%d \n", info->log_tag, info->mode);
...
static const struct of_device_id ln8000_dt_match[] = {
	...
	{ .compatible = "lionsemi,ln8000-standalone",
		.data = &ln8000_mode_data[LN8000_STANDALONE],
	},
	{ },
};
...
// 	{ "ln8000", LN8000_MASTER },
// 	{ "ln8000-secondary", LN8000_SLAVE },
// 	{ }
// };
...
static int ln8000_get_dev_role(struct i2c_client *client)
{
	...
	sprintf(buf, "%d", *(const int *)of_id->data);
	memcpy(&data, buf, 4);
...
static int ln8000_psy_register(struct ln8000_info *info)
{
	...
	info->psy_desc.name = ln8000_psy_name[info->mode];
	...
	int *val)
{
	...
	if (gm) {
		...
		*val = 0;
	// ln_err("%s dev=%d, cp_ibus=%d\n", gm->log_tag, gm->dev_role, *val);
	// dev_err(info->dev, "%s: work_mode=%d \n", info->log_tag, info->mode);
	return 0;
}
...
static int ln8000_probe(struct i2c_client *client)
{
	...
	if (IS_ERR_VALUE((unsigned long)ret)) {
		...
		if (IS_ERR_VALUE((unsigned long)ret)) {
			...
			if (ret != LN8000_DEVICE_ID) {
				dev_err(&client->dev, "fail to detect ln8000 on i2c_bus(addr=0x%x), exit\n", client->addr);
				return -ENODEV;
			}
		}
	}
	...
	match = of_match_node(ln8000_dt_match, node);
	if (!match)
	{
		ln_err("device tree match not found!")
		return -ENODEV;
	}

	info->mode = *(int *)match->data;
	ret = ln8000_check_work_mode(info, info->mode);
	if (ret) {
