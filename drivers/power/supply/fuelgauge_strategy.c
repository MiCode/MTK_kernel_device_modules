...
static int bq_battery_soc_smooth_tracking_new(struct strategy_fg *fg, int raw_soc, int batt_soc, int batt_ma)
{
	...
	if ((charging_status == POWER_SUPPLY_STATUS_DISCHARGING || charging_status == POWER_SUPPLY_STATUS_NOT_CHARGING ) &&
		fg->rsoc == 0 && last_system_soc >= 0 && ((fg->vbat <= fg->normal_shutdown_vbat + 200 && fg->tbat > 0) || ((fg->vbat <= fg->normal_shutdown_vbat + 50) && fg->tbat <= 0))) {
			fuelgauge_dev_read_avg_current(fg->fg_master_dev, &batt_ma_avg);
			if(batt_ma_avg >= 2000){
				unit_time = 5000;
	...
	if (fg->rsoc == 0 && ((fg->vbat < (fg->normal_shutdown_vbat -300) && fg->tbat > 0) || (fg->vbat < (fg->normal_shutdown_vbat - 350) && fg->tbat < 0))) {
		...
		mca_log_err("uisoc::set 0 when volt = %dmV\n", fg->vbat);
										...
										((fg->tbat >= BATT_COOL_THRESHOLD) && (fg->vbat >= fg->normal_shutdown_vbat + 50)) || ((time.tv_sec <= 10)))) {
		system_soc = 1;
...
static int fuelgauge_strategy_read_temp(struct strategy_fg *fg) {
	int ret = 0;
	int temp_master = 0, temp_slave = 0;
	if(fg == NULL)
		return 0;
	ret = fuelgauge_dev_read_temperature(fg->fg_master_dev, &temp_master);
	ret |= fuelgauge_dev_read_temperature(fg->fg_slave_dev, &temp_slave);
	if (ret) {
		fg->tbat = 250;
		return -1;
	}
	if (temp_master <= fg->cool_threshold || temp_slave <= fg->cool_threshold)
		fg->tbat = min(temp_master, temp_slave);
	else if (min(temp_master, temp_slave) > EXTREME_HIGH_DEGREE)
		fg->tbat = min(temp_master, temp_slave);
	else
		fg->tbat = max(temp_master, temp_slave);
	return 0;
}
...
static int fuelgauge_strategy_get_chip_ok(struct strategy_fg *fg) {
	int chip_ok_master = 0, chip_ok_slave = 0;
	if(fg == NULL)
		return -1;
	fuelgauge_dev_get_fg_chip_ok(fg->fg_master_dev, &chip_ok_master);
	fuelgauge_dev_get_fg_chip_ok(fg->fg_slave_dev, &chip_ok_slave);
	return 0;
}
...
static int fuelgauge_strategy_read_real_temp(struct strategy_fg *fg) {
	int ret;
	int real_temp = 0, real_temp_master = 0, real_temp_slave = 0;
	if(fg == NULL)
		return 0;
	ret = fuelgauge_dev_read_real_temp(fg->fg_master_dev, &real_temp_master);
	ret |= fuelgauge_dev_read_real_temp(fg->fg_slave_dev, &real_temp_slave);
	if (ret) {
		return 250;
	}
	if (real_temp_master <= fg->cool_threshold || real_temp_slave <= fg->cool_threshold)
		real_temp = min(real_temp_master, real_temp_slave);
	else if (min(real_temp_master, real_temp_slave) > EXTREME_HIGH_DEGREE)
		real_temp = min(real_temp_master, real_temp_slave);
	else
		real_temp = max(real_temp_master, real_temp_slave);
	return real_temp;
}

static int fuelgauge_strategy_read_voltage_max(struct strategy_fg *fg) {
	int voltage_max_master = 3700, voltage_max_slave = 3700;
	if(fg == NULL)
		return 0;
	fuelgauge_dev_read_voltage_max(fg->fg_master_dev, &voltage_max_master);
	fuelgauge_dev_read_voltage_max(fg->fg_slave_dev, &voltage_max_slave);

	return max(voltage_max_master, voltage_max_slave);
}

static int fuelgauge_strategy_read_temp_max(struct strategy_fg *fg) {
	int temp_max_master = 250, temp_max_slave = 250;
	if(fg == NULL)
		return 0;
	fuelgauge_dev_read_temp_max(fg->fg_master_dev, &temp_max_master);
	fuelgauge_dev_read_temp_max(fg->fg_slave_dev, &temp_max_slave);

	return max(temp_max_master, temp_max_slave);
}

static int fuelgauge_strategy_read_over_vol_duration(struct strategy_fg *fg) {

	int over_vol_duration_master = 0, over_vol_duration_slave = 0;
	if(fg == NULL)
		return 0;
	fuelgauge_dev_read_over_vol_duration(fg->fg_master_dev, &over_vol_duration_master);
	fuelgauge_dev_read_over_vol_duration(fg->fg_slave_dev, &over_vol_duration_slave);

	return max(over_vol_duration_master, over_vol_duration_slave);
}
...
static void fg_update_status(struct strategy_fg *fg)
{
	...
	fuelgauge_strategy_read_temp(fg);
	fuelgauge_strategy_read_bat_voltage(fg);
	fuelgauge_strategy_get_chip_ok(fg);
	...
	fuelgauge_dev_sc_fg_read_charging_status(fg->fg_master_dev);
	fuelgauge_dev_sc_fg_read_charging_status(fg->fg_slave_dev);
	fuelgauge_dev_sc_fg_read_volt_range(fg->fg_master_dev);
	fuelgauge_dev_sc_fg_read_volt_range(fg->fg_slave_dev);
	fuelgauge_dev_sc_fg_read_temp_range(fg->fg_master_dev);
	fuelgauge_dev_sc_fg_read_temp_range(fg->fg_slave_dev);
	fuelgauge_dev_sc_fg_ext_cmd_read_operation_status(fg->fg_master_dev);
	fuelgauge_dev_sc_fg_ext_cmd_read_operation_status(fg->fg_slave_dev);
	fuelgauge_dev_sc_fg_ext_cmd_read_dev_status(fg->fg_master_dev);
	fuelgauge_dev_sc_fg_ext_cmd_read_dev_status(fg->fg_slave_dev);
	fuelgauge_dev_sc_fg_ext_cmd_read_host_cmd_status(fg->fg_master_dev);
	fuelgauge_dev_sc_fg_ext_cmd_read_host_cmd_status(fg->fg_slave_dev);
	fuelgauge_dev_sc_fg_ext_cmd_read_term_volt(fg->fg_master_dev);
	fuelgauge_dev_sc_fg_ext_cmd_read_term_volt(fg->fg_slave_dev);
	fuelgauge_dev_sc_fg_ext_cmd_read_xt_status(fg->fg_master_dev);
	fuelgauge_dev_sc_fg_ext_cmd_read_xt_status(fg->fg_slave_dev);
	fuelgauge_dev_sc_fg_ext_cmd_read_xt_status2(fg->fg_master_dev);
	fuelgauge_dev_sc_fg_ext_cmd_read_xt_status2(fg->fg_slave_dev);
	fuelgauge_dev_sc_fg_ext_cmd_read_xt_status3(fg->fg_master_dev);
	fuelgauge_dev_sc_fg_ext_cmd_read_xt_status3(fg->fg_slave_dev);
	...
	if (!battery_get_psy(fg)) {
		...
		if((gm->night_charging || gm->smart_chg[SMART_CHG_ENDURANCE_PRO].en_ret) && fg->ui_soc > 80 && fg->ui_soc > last_soc){
			fg->ui_soc = last_soc;
			mca_log_err("%s last_soc = %d, soclmt[Night Endurance] = [%d %d]\n", fg->log_tag, last_soc, gm->night_charging, gm->smart_chg[SMART_CHG_ENDURANCE_PRO].en_ret);
		}
...
static int strategy_fg_parse_dt(struct strategy_fg *fg)
{
	...
	ret = of_property_read_u32(node, "cool_threshold", &fg->cool_threshold);
	if (ret)
		mca_log_err("%s failed to parse cool_threshold\n", fg->log_tag);
...
static int fg_strategy_get_property(struct power_supply *psy, enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CAPACITY:
		...
		if (fuelgauge_strategy_get_i2c_error_count(bq) >= 10) {
			val->intval = 15;
			break;
		}
		...
		if (bq->enable_shutdown_delay) {
			if (val->intval <= 1) {
				...
				if (pval.intval != POWER_SUPPLY_STATUS_CHARGING) {
					if(bq->shutdown_delay == true) {
						val->intval = 1;
						if(bq->vbat <= bq->critical_shutdown_vbat - 200){
							mca_log_err("%s vbat too lower, shutdown\n", bq->log_tag);
							val->intval = 0;
						}
					...
					bq->shutdown_delay = false;
					if ((bq->vbat >= (bq->critical_shutdown_vbat - 100)) &&
							bq->shutdown_flag == false) {
						val->intval = 1;
					}
				}
			...
			mca_log_info("%s shutdown= %d, soc =%d\n", bq->log_tag, bq->shutdown_delay, val->intval);
		}
		break;
	case POWER_SUPPLY_PROP_TEMP:
		...
		fuelgauge_strategy_read_temp(bq);
		if (!battery_get_psy(bq)) {
	...
	case POWER_SUPPLY_PROP_CHARGE_FULL_DESIGN:
		fuelgauge_strategy_get_design_capacity(bq);
		val->intval = bq->dc;
		val->intval *= 1000;
		break;
...
{
	if (gm) {
		fuelgauge_strategy_get_raw_soc(gm);
		*val = gm->raw_soc;
		mca_log_err("%s %d\n", gm->log_tag, *val);
	} else {
		*val = 0;
		mca_log_err("gm is NULL, val=%d\n", *val);
	}
	return 0;
}
...
static int real_temp_get(struct strategy_fg *gm,
	struct mtk_bms_strategy_sysfs_field_info *attr,
	int *val)
{
	if (gm)
		*val = fuelgauge_strategy_read_real_temp(gm);
	else
		*val = 250;
	mca_log_err("%s %d\n", gm->log_tag, *val);
	return 0;
...
static int voltage_max_get(struct strategy_fg *gm,
	struct mtk_bms_strategy_sysfs_field_info *attr,
	int *val)
{
	if (gm)
		*val = fuelgauge_strategy_read_voltage_max(gm);
	else
		*val = 3700;
	return 0;
}

static int temp_max_get(struct strategy_fg *gm,
	struct mtk_bms_strategy_sysfs_field_info *attr,
	int *val)
{
	if (gm)
		*val = fuelgauge_strategy_read_temp_max(gm);
	else
		*val = 250;
	return 0;
}

static int over_vol_duration_get(struct strategy_fg *gm,
	struct mtk_bms_strategy_sysfs_field_info *attr,
	int *val)
{
	if (gm)
		*val = fuelgauge_strategy_read_over_vol_duration(gm);
	else
		*val = 0;
	return 0;
}
...
static struct mtk_bms_strategy_sysfs_field_info bms_sysfs_field_tbl[] = {
	...
	BMS_STRATEGY_SYSFS_FIELD_RO(real_temp, BMS_STRATEGY_PROP_REAL_TEMP),
	BMS_STRATEGY_SYSFS_FIELD_RO(voltage_max, BMS_STRATEGY_PROP_VOLTAGE_MAX),
	BMS_STRATEGY_SYSFS_FIELD_RO(temp_max, BMS_STRATEGY_PROP_TEMP_MAX),
	BMS_STRATEGY_SYSFS_FIELD_RO(over_vol_duration, BMS_STRATEGY_PROP_OVER_VOL_DURATION),
};
...
static void strategy_fg_remove(struct platform_device *pdev)
{
	return;
}
