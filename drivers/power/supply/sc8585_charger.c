...
static int sc858x_set_usbovp_th(struct sc858x_chip *sc, int threshold)
{
    ...
    if (ret) {
	    pr_err("%s failed set VUSB_OVP, ret=%d \n", sc->log_tag, ret);
    }
...
static int sc858x_parse_dt(struct sc858x_chip *sc, struct device *dev)
{
    ...
    if (!gpio_is_valid(sc->irq_gpio)) {
        pr_err("%s fail to parse sc_irq_gpio\n", __func__);
        return ret;
    }
...
static int sc858x_register_interrupt(struct sc858x_chip *sc)
{
    ...
    if (gpio_is_valid(sc->irq_gpio)) {
        ret = devm_gpio_request_one(sc->dev, sc->irq_gpio, GPIOF_IN,"sc858x_irq");
        if (ret) {
...
static int sc858x_wlschg_init_protection(struct sc858x_chip *sc, int forward_work_mode)
{
	int ret = 0;

	pr_err("%s wls_chg forward_work_mode=%d\n", sc->log_tag, forward_work_mode);
	if (forward_work_mode == CP_FORWARD_4_TO_1) {
		ret = sc858x_set_busovp_th(sc, 22000);
		ret = sc858x_set_busocp_th(sc, 4625);
		ret = sc858x_set_vwpcovp_th(sc, 22000);
...
static int ops_cp_device_wls_chg_init(struct charger_device *chg_dev, int value)
{
	struct sc858x_chip *sc = charger_get_data(chg_dev);
	int ret = 0;

        //ret = sc858x_init_device(sc);
	ret = sc858x_wlschg_init_protection(sc, value);
	if (ret)
		pr_err("%s failed to init cp charge\n", sc->log_tag);
...
static int sc858x_set_revchg(struct sc858x_chip *sc, bool enable)
{
	...
	if (enable) {
		sc8561_set_ss_timeout(sc, 0x00);
		sc858x_set_pmid2outuvp_th(sc, 139);
		sc858x_set_vout2out_uvp_high(sc, true);
		sc858x_set_operation_mode(sc, REVERSE_1_2_CONVERTER_MODE);
		sc->op_mode = REVERSE_1_2_CONVERTER_MODE;
		sc858x_enable_ovpgate(sc, true);
		sc858x_enable_qb(sc, true);
		sc858x_init_protection(sc, CP_FORWARD_2_TO_1);
		...
		sc858x_init_device(sc);
		// sc8561_set_ss_timeout(sc, 5120);
		// sc858x_set_pmid2outuvp_th(sc, 30);
		// sc858x_set_vout2out_uvp_high(sc, false);
		// sc858x_dump_register(sc);
		sc858x_enable_adc(sc, false);
		sc858x_get_operation_mode(sc, &cp_mode);
...
static const struct charger_ops sc858x_chg_ops = {
	...
	.cp_device_wls_chg_init = ops_cp_device_wls_chg_init,
};
