				 ...
				 struct tcp_notify *noti)
{
	...
	if (usb_comm)
		...
		if (!IS_ERR_OR_NULL(chg_psy)) {
			...
			if (ret < 0)
				dev_err(mci->dev, "Failed to set type(%d)\n", ret);
		}
	}
...
{
	struct charger_device *mtk_charger = NULL;
	struct mci_notifier_block *pd_nb =
	...
	if (!mtk_charger)
			mtk_charger = get_charger_by_name("mtk_charger");
	switch (event) {
	...
	case TCP_NOTIFY_TYPEC_STATE:
		...
		if (old_state == TYPEC_UNATTACHED &&
			...
			charger_dev_usb_get_property(mtk_charger, USB_PROP_PMIC_VBUS, &vbus);
			if (vbus > 3600) {
		...
		charger_dev_usb_set_property(mtk_charger, USB_PROP_TYPEC_CC_ORIENTATION, noti->typec_state.polarity);
		charger_dev_usb_set_property(mtk_charger, USB_PROP_TYPEC_MODE, typec_mode);
		break;
