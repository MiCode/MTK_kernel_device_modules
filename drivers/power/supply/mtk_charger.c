				...
				struct device *dev)
{
	...
	if (of_property_read_u32(np, "ac_charger_current", &val) >= 0) {
		...
		chr_err("get default value1 AC_CHARGER_CURRENT:%d\n", info->data.ac_charger_current);
	} else if (of_property_read_u32(np, "ac-charger-current", &val) >= 0) {
		info->data.ac_charger_current = val;
		chr_err("get default alue2 AC_CHARGER_CURRENT:%d\n", info->data.ac_charger_current);
	} else {
	...
	info->support_disable_otg_incold = of_property_read_bool(np, "support_disable_otg_incold");
	info->support_otg_wdt_kick = of_property_read_bool(np, "support_otg_wdt_kick");
	...
	if (of_property_read_u32(np, "ffc_low_tbat_gl", &val) >= 0)
		info->ffc_low_tbat = val;
	else {
		chr_err("failed to parse ffc_low_tbat_gl use default\n");
		info->ffc_low_tbat = 160;
	}

	if (of_property_read_u32(np, "ffc_medium_tbat_gl", &val) >= 0)
		info->ffc_medium_tbat = val;
	else {
		chr_err("failed to parse ffc_medium_tbat_gl use default\n");
		info->ffc_medium_tbat = 360;
	}

	if (of_property_read_u32(np, "ffc_warm_tbat_gl", &val) >= 0)
		info->ffc_warm_tbat = val;
	else {
		chr_err("failed to parse ffc_warm_tbat_gl use default\n");
		info->ffc_warm_tbat = 360;
	}

	if (of_property_read_u32(np, "ffc_little_high_tbat_gl", &val) >= 0)
		info->ffc_little_high_tbat = val;
	else {
		chr_err("failed to parse ffc_little_high_tbat_gl use default\n");
		info->ffc_little_high_tbat = 400;
	}

	if (of_property_read_u32(np, "ffc_high_tbat_gl", &val) >= 0)
		info->ffc_high_tbat = val;
	else {
		chr_err("failed to parse ffc_high_tbat_gl use default\n");
		info->ffc_high_tbat = 460;
	}

	if (of_property_read_u32(np, "fv_gl", &val) >= 0)
		info->fv = val;
	else {
		chr_err("failed to parse fv_gl use default\n");
		info->fv = 4450;
	}

	if (of_property_read_u32(np, "fv_normal_gl", &val) >= 0)
		info->fv_normal = val;
	else {
		chr_err("failed to parse fv_normal_gl use fv\n");
		info->fv_normal = info->fv;
	}

	if (of_property_read_u32(np, "fv_normal_warm_gl", &val) >= 0)
		info->fv_normal_warm = val;
	else {
		chr_err("failed to parse fv_normal_warm_gl use fv\n");
		info->fv_normal_warm = info->fv;
	}

	if (of_property_read_u32(np, "fv_normal_high_gl", &val) >= 0)
		info->fv_normal_high = val;
	else {
		chr_err("failed to parse fv_normal_high_gl use fv\n");
		info->fv_normal_high = info->fv;
	}

	if (of_property_read_u32(np, "fv_ffc_gl", &val) >= 0)
		info->fv_ffc = val;
	else {
		chr_err("failed to parse fv_ffc_gl use default\n");
		info->fv_ffc = 4450;
	}
	if (of_property_read_u32(np, "iterm_gl", &val) >= 0)
		info->iterm = val;
	else {
		chr_err("failed to parse iterm_gl use default\n");
		info->iterm = 200;
	}
	if (of_property_read_u32(np, "iterm_warm_gl", &val) >= 0)
		info->iterm_warm = val;
	else {
		chr_err("failed to parse iterm_warm_gl use default\n");
		info->iterm_warm = info->iterm;
	}
	...
	info->rust_support = of_property_read_bool(np, "rust_support");
	...
	chr_info("parse iterm_2nd = %d, iterm_ffc_2nd = %d,iterm_warm_2nd=%d,  iterm_ffc_warm_2nd=%d, en_floatgnd=%d, rust_support=%d\n",
		info->iterm_2nd, info->iterm_ffc_2nd, info->iterm_warm_2nd, info->iterm_ffc_warm_2nd, info->en_floatgnd, info->rust_support);
...
unsigned int dfx_get_fake_report_type(void)
{
	...
	chr_err("%s dfx_fake_report_type=0x%x.\n", __func__, pinfo->dfx_fake_report_type);
...
{
	bool smart_pwrboost_trig = false;

	if(pinfo == NULL || pinfo->smart_chg == NULL)
		return 0;

	smart_pwrboost_trig = pinfo->smart_chg[SMART_CHG_TRAVELWAIT].en_ret || pinfo->smart_chg[SMART_CHG_OUTDOOR_CHARGE].en_ret;
	chr_err("%s smart_pwrboost_trig=%d[%d %d]\n", __func__,
		smart_pwrboost_trig, pinfo->smart_chg[SMART_CHG_TRAVELWAIT].en_ret, pinfo->smart_chg[SMART_CHG_OUTDOOR_CHARGE].en_ret);

	return smart_pwrboost_trig;
}
...
void lpd_charging_set_to_charger(int en)
{
	if(pinfo == NULL)
		return;
	pinfo->lpd_charging_limit = !!en;
	chr_err("%s lpd_charging_limit=%d\n", __func__, pinfo->lpd_charging_limit);
}
EXPORT_SYMBOL(lpd_charging_set_to_charger);
...
static void charger_set_cp_ovp(struct mtk_charger *info)
{
	...
	switch (info->real_type) {
		...
		case XMUSB350_TYPE_PD:
			if ((info->pd_type == MTK_PD_CONNECT_PE_READY_SNK || info->pd_type == MTK_PD_CONNECT_PE_READY_SNK_PD30) && (info->ta_status[PD] == TA_ATTACH))
...
static int mtk_charger_plug_out(struct mtk_charger *info)
{
	...
	info->cp_start_time = -1;
	info->last_ffc_enable = false;
	info->ffc_disable_force = false;
	...
	info->disable_otg_incold = false;
	bms_set_property(BMS_PROP_CHECK_FAST_MODE_TIMES, 1);
	info->last_rqc_status = 0;
	info->reduce_cv_applied = false;
	info->reduce_cv_valid_cnt = 0;
	info->reduce_cv_cfg_type = 0;
	info->wait_ibus_check = false;
	info->reverse_quick_charge = 0;
	...
	if ((info->smart_snschg_support || info->smart_snschg_v2_support || info->smart_snschg_v3_support) && info->usb_psy) {
		...
		if (!info->smart_snschg_v2_support && !info->smart_snschg_v3_support && !info->smart_snschg_nonsic_support) {
	...
	if (info->smart_snschg_v2_support || info->smart_snschg_v3_support)
				...
				int chr_type)
{
	...
	chr_err("%s wls_online=%d\n", __func__, info->wls_online);
	if (info->wls_dev && info->wls_online) {
		...
		chr_err("wls fw can update = %d check_resurt=%d\n", info->wls_fw_state, check_result);
...
#ifdef CONFIG_SUPPORT_SOUTHCHIP_PDPHY
	if(pinfo != NULL && pinfo->tcpc1 != NULL) {
		tcpci_set_cc(pinfo->tcpc1, TYPEC_CC_DRP);
		chr_err("set_cc_drp_work_func p1 enter\n");
	}
#endif
				...
				unsigned long event, void *data)
{
	...
	case TCP_NOTIFY_TYPEC_STATE:
		...
		if (old_state == TYPEC_UNATTACHED &&
			...
			cancel_delayed_work(&pinfo->rust_detection_work);
			pinfo->first_lpd_det = true;
			for(i=0;i<5;i++)
				pinfo->lpd_res[i] = 255;
		...
		if (!info->tcpc1) {
			...
			if(info->tcpc1)
			{
				...
				register_tcp_dev_notifier(info->tcpc1,
						 &info->tcpc_rust_det_nb, TCP_NOTIFY_TYPE_ALL);
				chr_err("register tcpc_rust_det_nb p1 ok\n");
			}
			else
...
static bool rev_cable_check_partner_vid(struct mtk_charger *gm, struct rev_cable_info *info)
{
	...
	if (adapter_vid == XM_VID) {
		...
		mca_log_err("partner VID match Xiaomi (0x%04x)\n", adapter_vid);
	} else {
		info->partner_is_xm = false;
		mca_log_err("partner VID = 0x%04x (not Xiaomi)\n", adapter_vid);
	}
...
static bool rev_cable_decision(struct mtk_charger *gm, struct rev_cable_info *info)
{
	...
	if (info->partner_is_xm)
		mca_log_err("partner is Xiaomi phone\n");
	else
	...
	if (info->partner_is_xm) {
		if (info->is_a_port) {
			if (info->is_sink) {
				mca_log_err("Xiaomi: A-port + SINK -> need PR_SWAP\n");
				need_swap = true;
			} else {
				mca_log_err("Xiaomi: A-port + SRC -> direct reverse charging\n");
				return true;
			}
		} else {
			if (info->is_sink) {
				mca_log_err("Xiaomi: B-port + SINK -> no action\n");
				return false;
			} else {
				mca_log_err("Xiaomi: B-port + SRC -> no action\n");
				return false;
			}
		}
	} else {
	...
	if (need_swap && gm->swap_count == 0) {
		mca_log_err("trigger PR_SWAP via try.SRC policy\n");
		gm->revcable_swap_trigger = true;
		...
		return false;
	} else if (gm->swap_count != 0) {
		mca_log_err("skip trigger SWAP because swap_count = %d\n", gm->swap_count);
	}
...
bool is_rev_cable_boostable(struct mtk_charger *gm)
{
	...
	if (!rev_cable_get_power_role(gm, &info)) {
		mca_log_err("get power role failed\n");
		return false;
	}

	if (info.is_sink) {
		mca_log_err("flow: SINK branch\n");
		...
		(void)rev_cable_check_partner_vid(gm, &info);

		if (!rev_cable_check_identity(gm, &info)) {
			mca_log_err("sink: not reverse cable, exit\n");
			return false;
		}

		return rev_cable_decision(gm, &info);
	}

	mca_log_err("flow: SOURCE branch\n");

	if (gm->revcable_swap_trigger)
	{
		gm->revcable_swap_trigger = false;
		gm->revcable_swap_triggered = true;
		gm->swap_count = 1;
		mca_log_err("source: sink swap_triggered, reverse cable -> pass directly\n");
		return true;
	}
	if (!rev_cable_check_identity(gm, &info)) {
...
static u32 vary_pdo_by_vendor(struct mtk_charger *gm)
{
	u32 new_pdo = 0;
	int uisoc = 0;
	int is_iphone17_series = 0;
	int pd_svid = gm->pd_state_svid & 0x0000FFFF;;
	usb_get_property(USB_PROP_TYPEC_IS_IPHONE17_SERIES, &is_iphone17_series);
	uisoc = get_uisoc(gm);

	if(gm->screen_status || !gm->revchg_bcl) {
		new_pdo = 0x0002d064; /* 9v1a */
		mca_log_err("screen is on, xiaomi and 3rd-party RQC BC 9v/1a pdos");
	} else {
		if (pd_svid == USB_PD_MI_SVID) {
			new_pdo = 0x0002d0fa; /* 9v2.5a */
			mca_log_err("screen is off, xiaomi RQC BC 9v/2.5a pdos");
		} else {
			if (uisoc > 70) {
				new_pdo = 0x0002d0fa; /* 9v2.5a */
				mca_log_err("screen is off, 3rd-party RQC BC 9v/2.5a pdos");
			} else if (uisoc >= 50) {
				new_pdo = 0x0002d0c8; /* 9v2a */
				mca_log_err("screen is off, 3rd-party RQC BC 9v/2a pdos");
			}
		}
	}
	...
	struct mtk_charger *info, int reverse_quick_charge)
{
...
#ifdef CONFIG_SUPPORT_SOUTHCHIP_PDPHY
	mca_log_err("P0: %d, P1: %d, P1 is later: %d\n",
		info->typec_port0_plugin, info->typec_port1_plugin,
		info->typec_port1_after_plugin);
	if ((info->typec_port1_plugin == 3 && info->typec_port0_plugin != 3) ||
		(info->typec_port1_plugin == 3 && info->typec_port1_plugin == 3 && info->typec_port_num == 1))
		tcpc = info->tcpc1;
#endif
	if (!tcpc) {
	...
	if (info && reverse_quick_charge) {
		new_pdo= vary_pdo_by_vendor(info);
		mca_log_err("reverse chg BC pdos");
		xm_set_reverse_chg_pdo(info, tcpc, new_pdo);
	}
}

static void xm_handle_18w_reverse_chg_pdo(
	struct mtk_charger *info, int reverse_quick_charge)
{
	struct tcpc_device *tcpc = info->tcpc;
	u32 new_pdo = 0;
	...
	if (info && reverse_quick_charge) {
		new_pdo = 0x0002d0c8;
		xm_set_reverse_chg_pdo(info, tcpc, new_pdo);
		mca_log_err("18W reverse chg BC 9v/2a pdos");
	}
}
...
static void rqc_status_check(struct mtk_charger *gm)
{
	...
	union power_supply_propval pval = {0,};
	u32 flags = 0;
	bool support_dr_power = false;
	static int last_otg_source_mode = -1;
	static u32 last_flags = 0;
	bool recover_ok = false;
	bool is_pd = false;
	bool soc_block = false;
	...
	flags = tcpm_inquire_dpm_flags(gm->tcpc);
	otg_source_mode = (gm->pd_state_svid >> 16) & 0xFFFF;
	if (otg_source_mode != last_otg_source_mode) {
		last_flags = tcpm_inquire_dpm_flags(gm->tcpc);
		mca_log_err("rqc::pd_state changed (%d -> %d), refresh dpm_flags=0x%x\n",
			last_otg_source_mode, otg_source_mode, last_flags);
		last_otg_source_mode = otg_source_mode;
	}
	support_dr_power = last_flags & DPM_FLAGS_PARTNER_DR_POWER;
	...
	mca_log_err("uisoc=%d, battery_temp=%d, fitting_board_temp=%d, otg_source_mode=%d, pd_svid=%d, typec_otg_burn=%d, support_dr_power=%d\n",
		uisoc, battery_temp, fitting_board_temp, otg_source_mode, pd_svid, gm->typec_otg_burn, support_dr_power);

	if ((fitting_board_temp >= 40000) || (battery_temp < 0) || gm->typec_otg_burn)
		gm->rqc_blocked = 1;
	else if ((fitting_board_temp < 35000) && (battery_temp > 50) && !gm->typec_otg_burn)
		gm->rqc_blocked = 0;

	mca_log_err("rqc_blocked=%d\n", gm->rqc_blocked);

	if (((otg_source_mode == PD_CONNECT_PE_READY_SRC_PD30) ||
		(otg_source_mode == PD_CONNECT_PE_READY_SRC && support_dr_power)) &&
		uisoc > 30 && battery_temp >= 0 && fitting_board_temp < 40000 && !gm->typec_otg_burn && !gm->rqc_blocked) {
		gm->rqc_condition_status = 1;
	if (pd_svid != USB_PD_MI_SVID && uisoc < 60)
	...
	is_pd = ((otg_source_mode == PD_CONNECT_PE_READY_SRC_PD30) ||
				(otg_source_mode == PD_CONNECT_PE_READY_SRC && support_dr_power));
	soc_block = (uisoc <= 30) || (pd_svid != USB_PD_MI_SVID && uisoc < 60);
	if (gm->last_rqc_blocked == 1 && gm->rqc_blocked == 0) {
		if (is_pd) {
			recover_ok = !soc_block;
			if (soc_block) {
				gm->stop_monitor_rqc = true;
				mca_log_err("stop monitor_reverse_chg_func: recover path but soc too low\n");
			}
		}
	} else if (gm->rqc_blocked == 1 || (gm->rev_cable_boostable && !gm->rev_cable_boosted)) {
		if (is_pd && soc_block) {
			gm->stop_monitor_rqc = true;
			mca_log_err("stop monitor_reverse_chg_func: block path soc too low\n");
		}
	}
	...
	if (gm->rqc_condition_status && !gm->last_rqc_status) {
		if (gm->rev_cable_boostable && !gm->rev_cable_boosted) {
			...
			mca_log_err("rqc status match, call xm_handle_reverse_chg_pdo\n");
			xm_handle_reverse_chg_pdo(gm, gm->rev_cable_boostable);
		}
		if (recover_ok) {
			gm->reverse_quick_charge = 1;
			mca_log_err("blocked cleared & conditions ok -> auto recover reverse pdo\n");
			xm_handle_reverse_chg_pdo(gm, gm->reverse_quick_charge);
		}
		if ((!gm->support_revcable || gm->rev_cable_checked || !gm->rev_cable_boostable)
			&& !recover_ok) {
			mca_log_err("rqc rising edge -> report uevent true to popup\n");
			xm_reverse_chg_uevent(true);
		}
	}

	gm->last_rqc_status = gm->rqc_condition_status;
	gm->last_rqc_blocked = gm->rqc_blocked;
}
	...
	int val)
{
	...
	mca_log_err("P0: %d, P1: %d, P1 is later: %d\n",
		...
		gm->typec_port1_after_plugin);
	if ((gm->typec_port1_plugin == 3 && gm->typec_port0_plugin != 3) ||
		(gm->typec_port1_plugin == 3 && gm->typec_port1_plugin == 3 && gm->typec_port_num == 1))
		tcpc = gm->tcpc1;
#endif
	if (!tcpc)
		return 0;
	mca_log_err("set: %d, rev_cable:[%d %d], rqc_blocked:%d\n", val, gm->rev_cable_boostable, gm->rev_cable_boosted, gm->rqc_blocked);
	if (val) {
		if ((gm->rev_cable_boostable && !gm->rev_cable_boosted ) || gm->rqc_blocked) {
			mca_log_err("SCHEDULE rqc_work");
			schedule_delayed_work(&gm->monitor_rqc_work, 0);
		} else {
			mca_log_err("check status once");
			rqc_status_check(gm);
			gm->stop_monitor_rqc = false;
		}
	} else {
	...
	int val)
{
	...
	if (gm->rev_cable_check && !gm->rev_cable_checked) {
		...
		gm->revcable_swap_trigger = false;
		gm->revcable_swap_triggered = false;
		gm->swap_count = 0;
		gm->rqc_blocked = 0;
		gm->last_rqc_blocked = 0;
...
{
	if (gm) {
		*val = ((gm->rev_cable_checked & 0x1) << 31) |
			((gm->rev_cable_boostable & 0x7FFF) << 16) |
			(gm->rev_cable_boosted & 0xFFFF);
	} else {
		*val = 0;
	}

	mca_log_err(
		"rev_cable_checked=%d, rev_cable_boostable=%d, rev_cable_boosted=%d, val=0x%x\n",
		gm ? gm->rev_cable_checked : -1,
	...
	int val)
{
	...
	if (gm->reverse_quick_charge) {
		if (gm->support_quick_revchg) {
			...
			xm_handle_18w_reverse_chg_pdo(gm, gm->reverse_quick_charge);
		}
		schedule_delayed_work(&gm->monitor_rqc_work, 0);
		mca_log_err("start monitor rqc work\n");
	} else {
		xm_set_reverse_chg_pdo(gm, gm->tcpc, 0);
		cancel_delayed_work_sync(&gm->monitor_rqc_work);
	...
	int val)
{
	if (gm) {
		...
		if (gm->pd_state_svid && gm->support_quick_revchg && !gm->rev_cable_boostable) {
			rqc_status_check(gm);
			gm->stop_monitor_rqc = false;
		}
	}
...
static int pd_pid_bcd_set(struct mtk_charger *gm,
	struct mtk_usb_sysfs_field_info *attr,
	int val)
{
	if (!gm) {
		mca_log_err("pd_pid_bcd_set: gm is NULL\n");
		return 0;
	}

	gm->pd_pid_bcd = val;
	mca_log_err("pd_pid_bcd: %d\n", gm->pd_pid_bcd);

	return 0;
}


static int is_iphone17_series_get(struct mtk_charger *gm,
	struct mtk_usb_sysfs_field_info *attr,
	int *val)
{
	int pd_svid = 0;
	...
	if (gm) {
		pd_svid = gm->pd_state_svid & 0xFFFF;
		pd_pid = (gm->pd_pid_bcd >> 16) & 0xFFFF;
		if (pd_svid == APLLE_SVID &&
			(pd_pid == IPHONE_17_PID || pd_pid == IPHONE_17_PRO_PID || pd_pid == IPHONE_17_PM_PID)) {
			*val = 1;
		} else {
			*val = 0;
		}
...
static int pd_pid_bcd_get(struct mtk_charger *gm,
	struct mtk_usb_sysfs_field_info *attr,
	int *val)
{
	if (gm)
...
static int swap_count_set(struct mtk_charger *gm,
	struct mtk_usb_sysfs_field_info *attr,
	int val)
{
	if (!gm) {
		mca_log_err("swap_count_set: gm is NULL\n");
		return 0;
	}

	gm->swap_count = val;
	mca_log_err("swap_count: %d\n", gm->swap_count);

	return 0;
}

static int swap_count_get(struct mtk_charger *gm,
	struct mtk_usb_sysfs_field_info *attr,
	int *val)
{
	if (gm)
...
static int revcbl_swap_stat_get(struct mtk_charger *gm,
	struct mtk_usb_sysfs_field_info *attr,
	int *val)
{
	if (!gm) {
		// No charger info, treat as "not triggered"
		*val = 0; // 0: Not triggered
	} else if (gm->revcable_swap_trigger) {
		// Swap is currently in progress
		*val = 1; // 1: Trigger in progress
	} else if (gm->revcable_swap_triggered) {
		// Swap has completed
		*val = 2; // 2: Trigger completed
	} else {
		// Swap has not been triggered yet
		*val = 0; // 0: Not triggered
	}

	// Log the current reverse cable swap status
	mca_log_err("revcbl_swap_stat=%d\n", *val);
	return 0;
}

static int cc_toggle_18w_set(struct mtk_charger *gm,
	struct mtk_usb_sysfs_field_info *attr,
	int val)
{
	if (gm) {
		gm->cc_toggle_18w = val;
	}

	if (gm->cc_toggle_18w) {
		tcpm_typec_change_role_postpone(gm->tcpc,
...
static int cc_toggle_18w_get(struct mtk_charger *gm,
	struct mtk_usb_sysfs_field_info *attr,
	int *val)
{
	if (gm)
		*val = !!gm->cc_toggle_18w;
	else
		*val = 0;
	mca_log_err(": %d\n", *val);
	return 0;
	...
	int *val)
{
	...
	if (gm && !gm->wls_online) {
		if (!gm->short_fake_typec_temp) {
			zone = thermal_zone_get_zone_by_name("ts_ntc");
			if (zone) {
				thermal_zone_get_temp(zone, val);
				*val = *val / 100;
			} else {
				chr_err("Error: thermal zone 'ts_ntc' not found.\n");
				*val = 0;
			}
		} else {
	...
	int *val)
{
	...
	if (gm && !gm->wls_online) {
		if (!gm->short_fake_typec_temp) {
			zone = thermal_zone_get_zone_by_name("sub_ntc");
			if (zone) {
				thermal_zone_get_temp(zone, val);
				*val = *val / 100;
			} else {
				chr_err("Error: thermal zone 'sub_ntc' not found.\n");
				*val = 0;
			}
		} else {
			*val = gm->short_fake_typec_temp;
	...
	int val)
{
	...
	if (gm->otg_enable) {
		if (gm->wlschg_support) {
			...
			if (gm->cp_master) {
				charger_dev_cp_set_mode(gm->cp_master, SC8561_REVERSE_1_1_CONVERTER_MODE);
		...
		if (pinfo->typec_port1_plugin == 3 && !p1_antiburn_enable) {
			...
			schedule_delayed_work(&gm->short_usb_otg_monitor_work, 500);
		...
		schedule_delayed_work(&gm->usb_otg_monitor_work, 500);
#endif
		/* Start OTG watchdog kick work if supported */
		if (gm->support_otg_wdt_kick) {
			schedule_delayed_work(&gm->otg_wdt_kick_work, msecs_to_jiffies(100));
			chr_err("%s: start otg wdt kick work\n", __func__);
		}
		...
		if (gm->wlschg_support) {
			...
			if (gm->cp_master) {
				charger_dev_cp_set_mode(gm->cp_master, SC8561_FORWARD_4_1_CHARGER_MODE);
				charger_dev_enable_cp_usb_gate(gm->cp_master, false);
				charger_dev_enable_acdrv_manual(gm->cp_master, false);
			}
		}
		if (gm->cp_master) {
			charger_dev_cp_set_mode(gm->cp_master, SC8561_FORWARD_2_1_CHARGER_MODE);
			charger_dev_enable_acdrv_manual(gm->cp_master, false);
			charger_dev_enable_cp_usb_gate(gm->cp_master, false);
		}
		gm->wait_ibus_check = false;
		gm->typec_otg_burn = false;
		gm->short_typec_otg_burn = false;
		if (gm->disable_otg_incold && gm->support_disable_otg_incold) {
...
#ifdef CONFIG_SUPPORT_SOUTHCHIP_PDPHY
		gm->short_typec_otg_burn_status = false;
		...
		cancel_delayed_work_sync(&gm->usb_otg_monitor_work);
#endif
		/* Cancel OTG watchdog kick work and disable wdt if supported */
		if (gm->support_otg_wdt_kick) {
			cancel_delayed_work_sync(&gm->otg_wdt_kick_work);
			/* Disable watchdog */
			if (gm->cp_master) {
				charger_dev_cp_enable_wdt(gm->cp_master, false);
				chr_err("%s: disable otg wdt\n", __func__);
			}
			gm->otg_wdt_enabled = false;
			chr_err("%s: cancel otg wdt kick work\n", __func__);
		}
	}
	...
	int val)
{
	...
	if (gm->scene != val) {
		switch (val) {
		case PHONE_SCENE:
		case VIDEOCHAT_SCENE:
		case VIDEO_SCENE:
		case PER_VIDEO_SCENE:
		case DANMU_SCENE:
		case PER_DANMU_SCENE:
		case YOUTUBE_SCENE:
		case PER_YOUTUBE_SCENE:
		case CLASS0_SCENE:
		case PER_CLASS0_SCENE:
			if (gm->smart_snschg_v2_support || gm->smart_snschg_v3_support)
				need_report = true;
			break;
		case TGAME_SCENE:
		case MGAME_SCENE:
...
static struct mtk_usb_sysfs_field_info usb_sysfs_field_tbl[] = {
	...
	USB_SYSFS_FIELD_RW(cc_toggle_18w, USB_PROP_TYPEC_CC_TOGGLE_18W),
	...
	USB_SYSFS_FIELD_RW(pd_pid_bcd, USB_PROP_TYPEC_PD_PID_BCD),
	USB_SYSFS_FIELD_RO(is_iphone17_series, USB_PROP_TYPEC_IS_IPHONE17_SERIES),
	USB_SYSFS_FIELD_RW(swap_count, USB_PROP_TYPEC_SWAP_COUNT),
	USB_SYSFS_FIELD_RO(revcbl_swap_stat, USB_PROP_TYPEC_REVCBL_SWAP_STAT),
};
	...
	int val)
{
	if (gm->wls_dev)
		...
		chr_err("%s no wls_dev, don't set rx_sleep_mode\n", __func__);
...
static bool rqc_check_ibus_stable(struct mtk_charger *gm)
{
	int ret = 0;
	u32 master_ibus = 0;
	int stable_count = 0;
	int i;
	u32 ibus_threshold = 1500;
	int is_iphone17_series = 0;

	if (!gm || !gm->cp_master) {
		mca_log_err("invalid cp_master pointer\n");
		return false;
	}

	usb_get_property(USB_PROP_TYPEC_IS_IPHONE17_SERIES, &is_iphone17_series);
	if (is_iphone17_series) {
		ibus_threshold = 500;
	}

	ret = charger_dev_cp_enable_adc(gm->cp_master, true);
	if (ret) {
		mca_log_err("enable adc fail, ret=%d\n", ret);
		return false;
	}

	for (i = 0; i < 5; i++) {
		ret = charger_dev_get_ibus(gm->cp_master, &master_ibus);
		if (ret) {
			mca_log_err("get_ibus fail, ret=%d\n", ret);
			break;
		}

		mca_log_err("sample[%d]: ibus=%d, stable_count=%d, threshold=%d\n", i, master_ibus, stable_count, ibus_threshold);

		if (master_ibus < ibus_threshold)
			stable_count++;
		else
			stable_count = 0;
	...
	ret = charger_dev_cp_enable_adc(gm->cp_master, false);
	if (ret)
		mca_log_err("disable adc fail, ret=%d\n", ret);

	if (stable_count >= 5) {
		mca_log_err("ibus stable < 1.5A\n");
		return true;
	}

	mca_log_err("ibus not stable yet\n");
	return false;
}
...
static void monitor_reverse_chg_func(struct work_struct *work)
{
	...
	mca_log_err("quick chg sign = %d, rqc status = %d, revchg_bcl = %d, wait_ibus_check = %d\n",
		rev_quick_chg, gm->rqc_condition_status, gm->revchg_bcl, gm->wait_ibus_check);
	...
	if (gm->support_quick_revchg && gm->screen_status &&
		(rev_quick_chg || (gm->rev_cable_boostable && gm->rev_cable_boosted)) && gm->rqc_condition_status) {
		ibat_count = 0;

		if (gm->revchg_bcl) {
			if (!gm->wait_ibus_check) {
				new_pdo = vary_pdo_by_vendor(gm);
				BC_pdos = true;
				gm->wait_ibus_check = true;
				mca_log_err("rqc::apply BC mode, set 9V/1A PDO, wait to check ibus next cycle\n");
			}
			else {
				if (rqc_check_ibus_stable(gm)) {
					report_val = 2;
					bcl_uevent_report = true;
					gm->wait_ibus_check = false;
					mca_log_err("rqc::ibus stable < 1.5A, report 2 (BCL on)\n");
				} else {
					mca_log_err("rqc::ibus not stable yet, wait next cycle\n");
				}
			}
		}
...
#ifdef CONFIG_SUPPORT_DUAL_BATTERY
static int reverse_pen_soc_get(struct mtk_charger *gm,
	struct mtk_wls_sysfs_field_info *attr,
	int *val)
{
	int pen_soc = 0;
	if (!gm->wls_dev){
		gm->wls_dev = get_wireless_charger_by_name(wls_chip_vendor_name[gm->wlschg_vendor]);
		chr_err("%s failed to get %s\n", __func__, wls_chip_vendor_name[gm->wlschg_vendor]);
	}
	if (gm->wls_dev){
		wireless_charger_dev_wls_get_pen_soc(gm->wls_dev, &pen_soc);
		*val = pen_soc;
	} else
		*val = 0;
	chr_err("%s val:%d\n", __func__, *val);
...
#ifdef CONFIG_SUPPORT_DUAL_BATTERY
	WLS_SYSFS_FIELD_RO(reverse_pen_soc, WLS_PROP_REVERSE_PEN_SOC),
#endif
};
...
#if defined(CONFIG_RUST_DETECTION)
			if(pinfo->rust_support && !pinfo->typec_attach)
			{
				cancel_delayed_work(&pinfo->rust_detection_work);
				pinfo->first_lpd_det = true;
			}
...
static int mtk_charger_probe(struct platform_device *pdev)
{
	...
	info->bypass_auth_m = false;
#endif
	info->step_jeita_tuple_count = STEP_JEITA_TUPLE_COUNT;
	info->jeita_temp_normal = 5;
	info->jeita_temp_hot = 6;
