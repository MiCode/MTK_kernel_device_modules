...
static void check_dynamic_mivr(struct mtk_charger *info)
{
	...
	if (!is_fast_charge) {
		...
		if (vbat < info->data.min_charger_voltage_2 / 1000 - 200) {
			...
			chr_err("%s vbat=%d, set mivr to %d\n", __func__, vbat, info->data.min_charger_voltage_1);
...
static void charger_set_cp_ovp(struct mtk_charger *info)
{
	...
	switch (info->real_type) {
		...
		case XMUSB350_TYPE_PD:
			if (info->pd_type != MTK_PD_CONNECT_PE_READY_SNK_APDO && (info->ta_status[PD] == TA_ATTACH)) {
...
static u32 vary_pdo_by_vendor(struct mtk_charger *gm)
{
	...
	if(gm->screen_status || !gm->revchg_bcl) {
		if (is_iphone17_series) {
			new_pdo = 0x0004b032; /* 15v0.5a */
			mca_log_err("screen is on, apple 17 RQC BC 15v/0.5a pdos");
		} else {
		...
		if (is_iphone17_series) {
			new_pdo = 0x0004b0b4; /* 15v1.8a */
			mca_log_err("screen is off, apple 17 RQC BC 15v/1.8a pdos");
		} else if (pd_svid == USB_PD_MI_SVID) {
			new_pdo = 0x0002d12c; /* 9v3a */
			mca_log_err("screen is off, xiaomi RQC BC 9v/3a pdos");
		} else {
			if (uisoc > 85) {
				new_pdo = 0x0002d12c; /* 9v3a */
				mca_log_err("screen is off, 3rd-party RQC BC 9v/3a pdos");
			} else if (uisoc >= 60) {
