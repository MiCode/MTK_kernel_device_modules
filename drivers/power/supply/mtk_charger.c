				...
				struct device *dev)
{
	...
	if (info->wlschg_support || info->project_no == TURNER_CN) {
...
{
	if (pinfo)
		pinfo->dfx_fake_report_type = fake_type;
...
static bool mtk_chg_check_vbus(struct mtk_charger *info)
{
	...
	if (info->wls_pmic_parachg_mode == 1) {
		...
		if (info->project_no == DALI_CN || info->project_no == KLIMT_GL ||
				info->project_no == KLEE_CN || info->project_no == KLEE_GL ||
				info->project_no == DASH_CN) {
			max_vol = 22000000;
		} else if (info->project_no == GOYA_GL || info->project_no == TURNER_CN) {
			max_vol = 11000000;
		} else {
			max_vol = 22000000;
		}
...
{
	if (info->project_no != DALI_CN && info->project_no != KLIMT_GL && info->project_no != DASH_CN &&
		info->project_no != KLEE_CN && info->project_no != KLEE_GL)
	{
		return;
	}
...
static int mtk_charger_plug_out(struct mtk_charger *info)
{
	...
	if(info->project_no == TURNER_CN && !info->short_typec_burn) {
				...
				unsigned long event, void *data)
{
	...
	case TCP_NOTIFY_SINK_VBUS:
		...
		if (pinfo->project_no == RODIN_GL || pinfo->project_no == KLIMT_GL || pinfo->project_no == GOYA_GL ||
			pinfo->project_no == KLEE_GL) {
...
#ifdef CONFIG_SUPPORT_SOUTHCHIP_PDPHY
			if((pinfo->project_no == TURNER_CN) && pinfo->short_typec_burn && pinfo->typec_port1_plugin) {
	...
	struct mtk_charger *info, int reverse_quick_charge)
{
	...
	if (info && reverse_quick_charge) {
		...
		if (info->screen_status || !info->revchg_bcl) {
			...
			mca_log_err("reverse chg BC 9v/3a pdos");
		}
...
static void xm_reverse_chg_uevent(int vote)
{
	...
	struct mca_event_notify_data event_data = { 0 };
	int len;
	// make sure none-zero value report once only, eg: 1, 2, 3
	if (vote == last_vote)
		return;
...
#define APLLE_SVID 0x05ac
#define IPHONE_15_PID 0x7513
#define IPHONE_16_PID 0x7517
#define IPHONE_17_PID 0x751b
#define IPHONE_17_PRO_PID 0x7519
#define IPHONE_17_PM_PID 0x7519
static void rqc_status_check(struct mtk_charger *gm)
{
	...
	mca_log_err("uisoc=%d, battery_temp=%d, fitting_board_temp=%d, otg_source_mode=%d, pd_svid=%x, pd_pid=%x, anti_burn=%d\n",
		uisoc, battery_temp, fitting_board_temp, otg_source_mode, pd_svid, pd_pid, gm->typec_otg_burn);

	if (otg_source_mode == PD_CONNECT_PE_READY_SRC_PD30 &&
		uisoc > 30 && battery_temp > 0 && fitting_board_temp < 40000 && !gm->typec_otg_burn) {
		gm->rqc_condition_status = 1;
		if ((pd_svid != USB_PD_MI_SVID && pd_svid != APLLE_SVID && uisoc < 50) ||
			(pd_svid == APLLE_SVID && uisoc < 70 && pd_pid != IPHONE_17_PID && pd_pid != IPHONE_17_PRO_PID && pd_pid != IPHONE_17_PM_PID) ||
			(pd_svid == APLLE_SVID && uisoc < 90 && (pd_pid == IPHONE_17_PID || pd_pid == IPHONE_17_PRO_PID || pd_pid == IPHONE_17_PM_PID))) {
	...
	int val)
{
	...
	if (gm->reverse_quick_charge && gm->rqc_condition_status) {
...
static int vary_pdo_by_vendor(struct mtk_charger *mcharger)
{
	...
	pd_pid = (mcharger->pd_pid_bcd >> 16) & 0xFFFF;
	...
	if (pd_svid != APLLE_SVID) {
		if (pd_svid == USB_PD_MI_SVID) {
			if (uisoc > 85) {
				new_pdo = 0x0002d12c; /* 9v3a */
				mca_log_err("screen is off, RQC BC 9v/3a pdos");
			} else if (uisoc >= 30) {
	...
	} else if (pd_svid == APLLE_SVID &&  (pd_pid == IPHONE_17_PID || pd_pid == IPHONE_17_PRO_PID || pd_pid == IPHONE_17_PM_PID)) {
		if (uisoc >= 90) {
			new_pdo = 0x0002d0fa; /* 9v2.5a */
			mca_log_err("screen is off, apple not 17 RQC BC 9v/2.5a pdos");
		}
	} else if (pd_svid == APLLE_SVID &&  pd_pid != IPHONE_17_PID && pd_pid != IPHONE_17_PRO_PID && pd_pid != IPHONE_17_PM_PID) {
		if (uisoc > 85) {
			new_pdo = 0x0002d12c; /* 9v3a */
...
static void mtk_charger_external_power_wls_changed(struct power_supply *psy)
{
	...
	info = (struct mtk_charger *)power_supply_get_drvdata(psy);
	if(info && info->project_no == TURNER_CN){
		return;
	}
	if (!info->bms_psy) {
		info->bms_psy = power_supply_get_by_name("bms");
	...
	if (evdata && evdata->data){
		...
		if ((val == MI_DISP_DPMS_EVENT) && (blank == MI_DISP_DPMS_POWERDOWN
			...
			if(pinfo->rust_support && !pinfo->typec_attach)
			{
				cancel_delayed_work(&pinfo->rust_detection_work);
				pinfo->first_lpd_det = true;
				if (pinfo->project_no == RODIN_CN || pinfo->project_no == RODIN_IN || pinfo->project_no == RODIN_GL) {
					schedule_delayed_work(&pinfo->rust_detection_work, 0);
					chr_err("%s schedule_monitor_delayed_work\n", __func__);
				}
			}
...
static int mtk_charger_probe(struct platform_device *pdev)
{
	...
	if (info->project_no == KLEE_GL) {
		...
		info->step_jeita_tuple_count = STEP_JEITA_TUPLE_COUNT - 1;
		info->jeita_temp_normal = 4;
		info->jeita_temp_hot = 5;
	}
