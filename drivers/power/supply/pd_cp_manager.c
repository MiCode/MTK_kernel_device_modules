...
#else
#include "bq28z610.h"
#endif /* CONFIG_SUPPORT_DUAL_BATTERY */

#ifndef MCA_LOG_TAG
#define MCA_LOG_TAG "pd_cp_manager"
#endif
...
static void pdm_update_status(struct usbpd_pm *pdpm)
{
	...
	mca_log_err("mt6375 vbus= %d, master_vbus=%d, master_ibus=%d, master_vbatt=%d, slave_ibus=%d, total_ibus=%d, master_enable=%d, slave_enable=%d, cycle_count=%d\n",
	...
	mca_log_err("soc=%d, tbat = %d, vbatt=%d, ibatt=%d, jeita_index=%d, vbatt_step=%d, ibatt_step=%d, vbus_step=%d, ibus_step=%d, final_step=%d, target_fcc=%d, sw_cv=%d, charge_full=%d, input_suspend=%d, typec_burn=%d, i2c_error_count=%d, bms_slave_connect_error=%d\n",
	...
	mca_log_err("master_cp_bypass=%d, slave_cp_bypass=%d, bypass_support=%d, thermal_limit_fcc=%d, smart_soclmt=%d, smart_pwrboost=%d, eea_chg_support_flag=%d\n",
