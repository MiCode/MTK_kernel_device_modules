				     ...
				     enum mt6379_attach_trigger trig, int attach)
{
	...
	if (typec_port0_status == 0 && typec_port1_status == 0 &&
		active_idx > 0) {
		mca_log_err("no attach, reset active_idx to 0\n");
		active_idx = 0;
	}
#endif
...
static int mt6379_get_charger_status(struct mt6379_charger_data *cdata, int *psy_status)
{
	...
	*psy_status = POWER_SUPPLY_STATUS_DISCHARGING;
	...
	if (vbus > 3600 && stat == 1)
		stat = 4;
#ifdef CONFIG_SUPPORT_SOUTHCHIP_PDPHY
	ret = iio_read_channel_processed(&cdata->iio_adcs[ADC_CHAN_WLSVIN], &wls_vbus);
	if (ret < 0) {
	...
	if (typec_port1_num == 1 && ((stat == 0 && vbus < 3600) || (stat ==1 && (wls_vbus/1000) > 3600)))
		stat = 4;
#endif
	dev_err(cdata->dev, "%s, get mt6379 charger status=%d vbus = %d, wls_vbus = %d\n", __func__, stat, vbus, wls_vbus/1000);
...
static int mt6379_set_shipping_mode(struct mt6379_charger_data *cdata)
{
	...
	if (ret) {
		mca_log_err("Failed to disable ship reset\n");
		return ret;
	}

	ret = mt6379_charger_field_set(cdata, F_BATFET_DISDLY, 2);
	if (ret) {
		mca_log_err("Failed to disable ship mode delay\n");
		return ret;
	}
	...
	if (ret) {
		mca_log_err("Failed to disable chg buck en\n");
		return ret;
	}
...
static int mt6379_set_shipmode(struct notifier_block *reboot_notifier, unsigned long mode, void *nouse)
{
	...
	if (mode == SYS_POWER_OFF || mode == SYS_RESTART) {
		...
		if(gm->shipmode_flag) {
			...
			charger_dev_cp_reg_reset(cdata->cp_master);

			mca_log_err("successfully set shipmode\n");
			ret = mt6379_set_shipping_mode(cdata);
			if(ret < 0){
				mca_log_err("failed to set shipmode\n");
				return ret;
			}
		}
	}
				   ...
				   const char *buf, size_t count)
{
	...
	if (ret) {
		mca_log_err("Failed to parse number\n");
		return ret;
	}
