...
static void mt6379_detect_hvchg_work(struct work_struct *work)
{
	...
	if (cdata->project_no == DALI_CN || cdata->project_no == KLIMT_GL  || cdata->project_no == DASH_CN) {
		if (!cdata->cp_master) {
			cp_count++;
			cdata->cp_master = get_charger_by_name("cp_master");
			if (cp_count < 2)
...
static int mt6379_charger_probe(struct platform_device *pdev)
{
	...
	cdata->dev = dev;
    cdata->project_no = KLEE_CN;

	dev_info(dev, "%s: project_no = %d\n", __func__, cdata->project_no);
