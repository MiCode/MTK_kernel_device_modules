...
static int sc760x_set_power_limit_en(struct sc7601_device *sc760x, bool enable)
{
	...
	ret = ls_update_bits(sc760x->client, SC7601_REG_02, SC760X_POW_LIM_DIS_MASK,
			enable ? SC760X_POW_LIM_DIS_ENABLE : SC760X_POW_LIM_DIS_DISABLE);
	return ret;
}
...
static int sc7601_init_device(struct sc7601_device *sc760x)
{
	...
	ret = sc760x_set_power_limit_en(sc760x, false);
