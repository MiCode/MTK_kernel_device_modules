...
static void mca_log_get_cur_time(struct mca_cur_time *tm)
{
	...
	now.tv_sec += g_mca_log_info.time_offset;
	...
	struct mca_log_buf_info *info)
{
	...
	if (len >= MCA_LOG_SINGLE_LOG_MAX_LEN - 1) {
		len = MCA_LOG_SINGLE_LOG_MAX_LEN - 1;
		buff[MCA_LOG_SINGLE_LOG_MAX_LEN - 2] = '\n';
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	case MCA_LOG_ATTR_TIME_OFFSET:
		__mca_log_err("update time offset: %s", buf);
		log_info->time_offset = value;
		break;
	default:
		break;
	}
