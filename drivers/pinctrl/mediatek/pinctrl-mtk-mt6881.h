/* SPDX-License-Identifier: GPL-2.0 */
/*
 * Copyright (C) 2025 MediaTek Inc.
 * Author: Light Hsieh <light.hsieh@mediatek.com>
 *
 */

#ifndef __PINCTRL_MTK_MT6881_H
#define __PINCTRL_MTK_MT6881_H

#include "pinctrl-paris.h"

static const struct mtk_pin_desc mtk_pins_mt6881[] = {
	MTK_PIN_NODRV(
		0, "GPIO0",
		MTK_EINT_FUNCTION(0, 0),
		MTK_FUNCTION(0, "GPIO0"),
		MTK_FUNCTION(2, "DISP_ERROR0")
	),
	MTK_PIN_NODRV(
		1, "GPIO1",
		MTK_EINT_FUNCTION(0, 1),
		MTK_FUNCTION(0, "GPIO1"),
		MTK_FUNCTION(6, "TSFDC_EN"),
		MTK_FUNCTION(7, "UFS_MPHY_SCL")
	),
	MTK_PIN_NODRV(
		2, "GPIO2",
		MTK_EINT_FUNCTION(0, 2),
		MTK_FUNCTION(0, "GPIO2"),
		MTK_FUNCTION(1, "USB_DRVVBUS"),
		MTK_FUNCTION(2, "DISP_ERROR1"),
		MTK_FUNCTION(6, "TSFDC_TSSEL0"),
		MTK_FUNCTION(7, "UFS_MPHY_SDA")
	),
	MTK_PIN_NODRV(
		3, "GPIO3",
		MTK_EINT_FUNCTION(0, 3),
		MTK_FUNCTION(0, "GPIO3"),
		MTK_FUNCTION(1, "IDDIG"),
		MTK_FUNCTION(6, "TSFDC_TSSEL1")
	),
	MTK_PIN_NODRV(
		4, "GPIO4",
		MTK_EINT_FUNCTION(0, 4),
		MTK_FUNCTION(0, "GPIO4"),
		MTK_FUNCTION(1, "VBUSVALID"),
		MTK_FUNCTION(4, "CONN_TCXOENA_REQ"),
		MTK_FUNCTION(6, "TSFDC_TSSEL2")
	),
	MTK_PIN_NODRV(
		5, "GPIO5",
		MTK_EINT_FUNCTION(0, 5),
		MTK_FUNCTION(0, "GPIO5"),
		MTK_FUNCTION(1, "KPROW0"),
		MTK_FUNCTION(6, "GPS_PPS")
	),
	MTK_PIN_NODRV(
		6, "GPIO6",
		MTK_EINT_FUNCTION(0, 6),
		MTK_FUNCTION(0, "GPIO6"),
		MTK_FUNCTION(1, "DISP_PWM"),
		MTK_FUNCTION(4, "UCTS2"),
		MTK_FUNCTION(6, "TSFDC_VCO_RST")
	),
	MTK_PIN_NODRV(
		7, "GPIO7",
		MTK_EINT_FUNCTION(0, 7),
		MTK_FUNCTION(0, "GPIO7"),
		MTK_FUNCTION(1, "DISP_PWM1"),
		MTK_FUNCTION(2, "I2SIN_DMA0_DI3"),
		MTK_FUNCTION(4, "URTS2"),
		MTK_FUNCTION(6, "TSFDC_RCK_SELB")
	),
	MTK_PIN_NODRV(
		8, "GPIO8",
		MTK_EINT_FUNCTION(0, 8),
		MTK_FUNCTION(0, "GPIO8"),
		MTK_FUNCTION(1, "DSI_TE1")
	),
	MTK_PIN_NODRV(
		9, "GPIO9",
		MTK_EINT_FUNCTION(0, 9),
		MTK_FUNCTION(0, "GPIO9"),
		MTK_FUNCTION(1, "LCM_RST1"),
		MTK_FUNCTION(6, "SPMI_P_TRIG_FLAG"),
		MTK_FUNCTION(7, "DBG_MON_B0")
	),
	MTK_PIN_NODRV(
		10, "GPIO10",
		MTK_EINT_FUNCTION(0, 10),
		MTK_FUNCTION(0, "GPIO10"),
		MTK_FUNCTION(1, "BPI_BUS10"),
		MTK_FUNCTION(2, "CONN_BPI_BUS10"),
		MTK_FUNCTION(4, "CONN_TCXOENA_REQ"),
		MTK_FUNCTION(7, "DBG_MON_B1")
	),
	MTK_PIN_NODRV(
		11, "GPIO11",
		MTK_EINT_FUNCTION(0, 11),
		MTK_FUNCTION(0, "GPIO11"),
		MTK_FUNCTION(1, "BPI_BUS11"),
		MTK_FUNCTION(2, "CONN_BPI_BUS11_OLAT0"),
		MTK_FUNCTION(5, "HFRP_URTS1"),
		MTK_FUNCTION(7, "DBG_MON_B2")
	),
	MTK_PIN_NODRV(
		12, "GPIO12",
		MTK_EINT_FUNCTION(0, 12),
		MTK_FUNCTION(0, "GPIO12"),
		MTK_FUNCTION(1, "BPI_BUS12"),
		MTK_FUNCTION(2, "CONN_BPI_BUS12_OLAT1"),
		MTK_FUNCTION(3, "ANT_SEL18"),
		MTK_FUNCTION(4, "CLKM0"),
		MTK_FUNCTION(5, "HFRP_UCTS1"),
		MTK_FUNCTION(7, "DBG_MON_B3")
	),
	MTK_PIN_NODRV(
		13, "GPIO13",
		MTK_EINT_FUNCTION(0, 13),
		MTK_FUNCTION(0, "GPIO13"),
		MTK_FUNCTION(1, "BPI_BUS13"),
		MTK_FUNCTION(2, "CONN_BPI_BUS13_OLAT2"),
		MTK_FUNCTION(3, "ANT_SEL19"),
		MTK_FUNCTION(4, "CLKM1"),
		MTK_FUNCTION(7, "DBG_MON_B4")
	),
	MTK_PIN_NODRV(
		14, "GPIO14",
		MTK_EINT_FUNCTION(0, 14),
		MTK_FUNCTION(0, "GPIO14"),
		MTK_FUNCTION(1, "BPI_BUS14"),
		MTK_FUNCTION(2, "CONN_BPI_BUS14_OLAT3"),
		MTK_FUNCTION(3, "ANT_SEL20"),
		MTK_FUNCTION(4, "CLKM2"),
		MTK_FUNCTION(7, "DBG_MON_B5")
	),
	MTK_PIN_NODRV(
		15, "GPIO15",
		MTK_EINT_FUNCTION(0, 15),
		MTK_FUNCTION(0, "GPIO15"),
		MTK_FUNCTION(1, "BPI_BUS15"),
		MTK_FUNCTION(2, "CONN_BPI_BUS15_OLAT4"),
		MTK_FUNCTION(3, "ANT_SEL21"),
		MTK_FUNCTION(4, "CLKM3"),
		MTK_FUNCTION(7, "DBG_MON_B6")
	),
	MTK_PIN_NODRV(
		16, "GPIO16",
		MTK_EINT_FUNCTION(0, 16),
		MTK_FUNCTION(0, "GPIO16"),
		MTK_FUNCTION(1, "KPCOL0_VLP")
	),
	MTK_PIN_NODRV(
		17, "GPIO17",
		MTK_EINT_FUNCTION(0, 17),
		MTK_FUNCTION(0, "GPIO17"),
		MTK_FUNCTION(1, "AP_GOOD"),
		MTK_FUNCTION(4, "GPS_PPS"),
		MTK_FUNCTION(6, "AGPS_SYNC")
	),
	MTK_PIN_NODRV(
		18, "GPIO18",
		MTK_EINT_FUNCTION(0, 18),
		MTK_FUNCTION(0, "GPIO18"),
		MTK_FUNCTION(1, "UTXD1"),
		MTK_FUNCTION(3, "TP_UTXD1_VCORE"),
		MTK_FUNCTION(5, "HFRP_UTXD1"),
		MTK_FUNCTION(6, "MD_UTXD0"),
		MTK_FUNCTION(7, "CONN_WIFI_TXD")
	),
	MTK_PIN_NODRV(
		19, "GPIO19",
		MTK_EINT_FUNCTION(0, 19),
		MTK_FUNCTION(0, "GPIO19"),
		MTK_FUNCTION(1, "URXD1"),
		MTK_FUNCTION(3, "TP_URXD1_VCORE"),
		MTK_FUNCTION(5, "HFRP_URXD1"),
		MTK_FUNCTION(6, "MD_URXD0")
	),
	MTK_PIN_NODRV(
		20, "GPIO20",
		MTK_EINT_FUNCTION(0, 20),
		MTK_FUNCTION(0, "GPIO20"),
		MTK_FUNCTION(1, "SPI7_CLK"),
		MTK_FUNCTION(2, "SPIS0_CLK"),
		MTK_FUNCTION(5, "MD_URTS0"),
		MTK_FUNCTION(6, "UTXD4"),
		MTK_FUNCTION(7, "DBG_MON_A0")
	),
	MTK_PIN_NODRV(
		21, "GPIO21",
		MTK_EINT_FUNCTION(0, 21),
		MTK_FUNCTION(0, "GPIO21"),
		MTK_FUNCTION(1, "SPI7_CSB"),
		MTK_FUNCTION(2, "SPIS0_CSB"),
		MTK_FUNCTION(5, "MD_UCTS0"),
		MTK_FUNCTION(6, "URXD4"),
		MTK_FUNCTION(7, "DBG_MON_A1")
	),
	MTK_PIN_NODRV(
		22, "GPIO22",
		MTK_EINT_FUNCTION(0, 22),
		MTK_FUNCTION(0, "GPIO22"),
		MTK_FUNCTION(1, "SPI7_MO"),
		MTK_FUNCTION(2, "SPIS0_MOSI"),
		MTK_FUNCTION(5, "MD_URTS1"),
		MTK_FUNCTION(6, "UTXD5"),
		MTK_FUNCTION(7, "DBG_MON_A2")
	),
	MTK_PIN_NODRV(
		23, "GPIO23",
		MTK_EINT_FUNCTION(0, 23),
		MTK_FUNCTION(0, "GPIO23"),
		MTK_FUNCTION(1, "SPI7_MI"),
		MTK_FUNCTION(2, "SPIS0_MISO"),
		MTK_FUNCTION(5, "MD_UCTS1"),
		MTK_FUNCTION(6, "URXD5"),
		MTK_FUNCTION(7, "DBG_MON_A3")
	),
	MTK_PIN_NODRV(
		24, "GPIO24",
		MTK_EINT_FUNCTION(0, 24),
		MTK_FUNCTION(0, "GPIO24"),
		MTK_FUNCTION(1, "SPI6_CLK"),
		MTK_FUNCTION(2, "SPIS1_CLK"),
		MTK_FUNCTION(4, "TP_GPIO0_AO"),
		MTK_FUNCTION(7, "DBG_MON_A4")
	),
	MTK_PIN_NODRV(
		25, "GPIO25",
		MTK_EINT_FUNCTION(0, 25),
		MTK_FUNCTION(0, "GPIO25"),
		MTK_FUNCTION(1, "SPI6_CSB"),
		MTK_FUNCTION(2, "SPIS1_CSB"),
		MTK_FUNCTION(4, "TP_GPIO1_AO"),
		MTK_FUNCTION(7, "DBG_MON_A5")
	),
	MTK_PIN_NODRV(
		26, "GPIO26",
		MTK_EINT_FUNCTION(0, 26),
		MTK_FUNCTION(0, "GPIO26"),
		MTK_FUNCTION(1, "SPI6_MO"),
		MTK_FUNCTION(2, "SPIS1_MOSI"),
		MTK_FUNCTION(3, "DMIC_CLK"),
		MTK_FUNCTION(4, "SCP_DMIC_CLK"),
		MTK_FUNCTION(7, "DBG_MON_A6")
	),
	MTK_PIN_NODRV(
		27, "GPIO27",
		MTK_EINT_FUNCTION(0, 27),
		MTK_FUNCTION(0, "GPIO27"),
		MTK_FUNCTION(1, "SPI6_MI"),
		MTK_FUNCTION(2, "SPIS1_MISO"),
		MTK_FUNCTION(3, "DMIC_DAT"),
		MTK_FUNCTION(4, "SCP_DMIC_DAT"),
		MTK_FUNCTION(7, "DBG_MON_A7")
	),
	MTK_PIN_NODRV(
		28, "GPIO28",
		MTK_EINT_FUNCTION(0, 28),
		MTK_FUNCTION(0, "GPIO28"),
		MTK_FUNCTION(1, "SPI5_CLK"),
		MTK_FUNCTION(2, "FMI2SIN_BCK"),
		MTK_FUNCTION(4, "CLKM0"),
		MTK_FUNCTION(5, "DAP_SONIC_SWCK"),
		MTK_FUNCTION(6, "MD_UTXD1"),
		MTK_FUNCTION(7, "DBG_MON_A8")
	),
	MTK_PIN_NODRV(
		29, "GPIO29",
		MTK_EINT_FUNCTION(0, 29),
		MTK_FUNCTION(0, "GPIO29"),
		MTK_FUNCTION(1, "SPI5_CSB"),
		MTK_FUNCTION(2, "FMI2SIN_LRCK"),
		MTK_FUNCTION(4, "CLKM1"),
		MTK_FUNCTION(5, "DAP_SONIC_SWD"),
		MTK_FUNCTION(6, "MD_URXD1"),
		MTK_FUNCTION(7, "DBG_MON_A9")
	),
	MTK_PIN_NODRV(
		30, "GPIO30",
		MTK_EINT_FUNCTION(0, 30),
		MTK_FUNCTION(0, "GPIO30"),
		MTK_FUNCTION(1, "SPI5_MO"),
		MTK_FUNCTION(2, "FMI2SIN_MCK"),
		MTK_FUNCTION(4, "CLKM2"),
		MTK_FUNCTION(5, "DAP_MD32_SWCK"),
		MTK_FUNCTION(7, "DBG_MON_A10")
	),
	MTK_PIN_NODRV(
		31, "GPIO31",
		MTK_EINT_FUNCTION(0, 31),
		MTK_FUNCTION(0, "GPIO31"),
		MTK_FUNCTION(1, "SPI5_MI"),
		MTK_FUNCTION(2, "FMI2SIN_DI"),
		MTK_FUNCTION(4, "CLKM3"),
		MTK_FUNCTION(5, "DAP_MD32_SWD"),
		MTK_FUNCTION(7, "DBG_MON_A11")
	),
	MTK_PIN_NODRV(
		32, "GPIO32",
		MTK_EINT_FUNCTION(0, 32),
		MTK_FUNCTION(0, "GPIO32"),
		MTK_FUNCTION(1, "SPI4_CLK"),
		MTK_FUNCTION(2, "I2SIN0_BCK"),
		MTK_FUNCTION(4, "CLKM0"),
		MTK_FUNCTION(5, "CMFLASH0"),
		MTK_FUNCTION(6, "UCTS3"),
		MTK_FUNCTION(7, "DBG_MON_A12")
	),
	MTK_PIN_NODRV(
		33, "GPIO33",
		MTK_EINT_FUNCTION(0, 33),
		MTK_FUNCTION(0, "GPIO33"),
		MTK_FUNCTION(1, "SPI4_CSB"),
		MTK_FUNCTION(2, "I2SIN0_LRCK"),
		MTK_FUNCTION(4, "CLKM1"),
		MTK_FUNCTION(5, "CMFLASH1"),
		MTK_FUNCTION(6, "URTS3"),
		MTK_FUNCTION(7, "DBG_MON_A13")
	),
	MTK_PIN_NODRV(
		34, "GPIO34",
		MTK_EINT_FUNCTION(0, 34),
		MTK_FUNCTION(0, "GPIO34"),
		MTK_FUNCTION(1, "SPI4_MO"),
		MTK_FUNCTION(2, "I2SIN0_MCK"),
		MTK_FUNCTION(4, "CLKM2"),
		MTK_FUNCTION(5, "CMFLASH2"),
		MTK_FUNCTION(6, "UTXD3"),
		MTK_FUNCTION(7, "DBG_MON_A14")
	),
	MTK_PIN_NODRV(
		35, "GPIO35",
		MTK_EINT_FUNCTION(0, 35),
		MTK_FUNCTION(0, "GPIO35"),
		MTK_FUNCTION(1, "SPI4_MI"),
		MTK_FUNCTION(2, "I2SIN0_DI"),
		MTK_FUNCTION(4, "CLKM3"),
		MTK_FUNCTION(5, "CMFLASH3"),
		MTK_FUNCTION(6, "URXD3"),
		MTK_FUNCTION(7, "DBG_MON_A15")
	),
	MTK_PIN_NODRV(
		36, "GPIO36",
		MTK_EINT_FUNCTION(0, 36),
		MTK_FUNCTION(0, "GPIO36"),
		MTK_FUNCTION(1, "SPI3_CLK"),
		MTK_FUNCTION(2, "SCP_SPI3_CK"),
		MTK_FUNCTION(3, "MD_INT0"),
		MTK_FUNCTION(4, "TP_GPIO5_AO"),
		MTK_FUNCTION(5, "UTXD4"),
		MTK_FUNCTION(6, "MD_UTXD0"),
		MTK_FUNCTION(7, "DBG_MON_A16")
	),
	MTK_PIN_NODRV(
		37, "GPIO37",
		MTK_EINT_FUNCTION(0, 37),
		MTK_FUNCTION(0, "GPIO37"),
		MTK_FUNCTION(1, "SPI3_CSB"),
		MTK_FUNCTION(2, "SCP_SPI3_CS"),
		MTK_FUNCTION(3, "MD_INT3"),
		MTK_FUNCTION(4, "TP_GPIO6_AO"),
		MTK_FUNCTION(5, "URXD4"),
		MTK_FUNCTION(6, "MD_URXD0"),
		MTK_FUNCTION(7, "DBG_MON_A17")
	),
	MTK_PIN_NODRV(
		38, "GPIO38",
		MTK_EINT_FUNCTION(0, 38),
		MTK_FUNCTION(0, "GPIO38"),
		MTK_FUNCTION(1, "SPI3_MO"),
		MTK_FUNCTION(2, "SCP_SPI3_MO"),
		MTK_FUNCTION(3, "MD_INT4"),
		MTK_FUNCTION(4, "TP_GPIO7_AO"),
		MTK_FUNCTION(5, "UTXD5"),
		MTK_FUNCTION(7, "DBG_MON_A18")
	),
	MTK_PIN_NODRV(
		39, "GPIO39",
		MTK_EINT_FUNCTION(0, 39),
		MTK_FUNCTION(0, "GPIO39"),
		MTK_FUNCTION(1, "SPI3_MI"),
		MTK_FUNCTION(2, "SCP_SPI3_MI"),
		MTK_FUNCTION(4, "TP_GPIO8_AO"),
		MTK_FUNCTION(5, "URXD5"),
		MTK_FUNCTION(7, "DBG_MON_A19")
	),
	MTK_PIN_NODRV(
		40, "GPIO40",
		MTK_EINT_FUNCTION(0, 40),
		MTK_FUNCTION(0, "GPIO40"),
		MTK_FUNCTION(1, "SPI2_CLK"),
		MTK_FUNCTION(2, "SCP_SPI2_CK"),
		MTK_FUNCTION(4, "TP_GPIO9_AO"),
		MTK_FUNCTION(5, "UCTS3"),
		MTK_FUNCTION(6, "TP_UTXD1_VLP"),
		MTK_FUNCTION(7, "DBG_MON_A20")
	),
	MTK_PIN_NODRV(
		41, "GPIO41",
		MTK_EINT_FUNCTION(0, 41),
		MTK_FUNCTION(0, "GPIO41"),
		MTK_FUNCTION(1, "SPI2_CSB"),
		MTK_FUNCTION(2, "SCP_SPI2_CS"),
		MTK_FUNCTION(4, "TP_GPIO10_AO"),
		MTK_FUNCTION(5, "URTS3"),
		MTK_FUNCTION(6, "TP_URXD1_VLP"),
		MTK_FUNCTION(7, "DBG_MON_A21")
	),
	MTK_PIN_NODRV(
		42, "GPIO42",
		MTK_EINT_FUNCTION(0, 42),
		MTK_FUNCTION(0, "GPIO42"),
		MTK_FUNCTION(1, "SPI2_MO"),
		MTK_FUNCTION(2, "SCP_SPI2_MO"),
		MTK_FUNCTION(4, "TP_GPIO11_AO"),
		MTK_FUNCTION(5, "UTXD3"),
		MTK_FUNCTION(6, "TP_URTS1_VLP"),
		MTK_FUNCTION(7, "DBG_MON_A22")
	),
	MTK_PIN_NODRV(
		43, "GPIO43",
		MTK_EINT_FUNCTION(0, 43),
		MTK_FUNCTION(0, "GPIO43"),
		MTK_FUNCTION(1, "SPI2_MI"),
		MTK_FUNCTION(2, "SCP_SPI2_MI"),
		MTK_FUNCTION(4, "TP_GPIO12_AO"),
		MTK_FUNCTION(5, "URXD3"),
		MTK_FUNCTION(6, "TP_UCTS1_VLP"),
		MTK_FUNCTION(7, "DBG_MON_A23")
	),
	MTK_PIN_NODRV(
		44, "GPIO44",
		MTK_EINT_FUNCTION(0, 44),
		MTK_FUNCTION(0, "GPIO44"),
		MTK_FUNCTION(1, "SPI1_CLK"),
		MTK_FUNCTION(2, "SCP_SPI1_CK"),
		MTK_FUNCTION(3, "DMIC1_CLK"),
		MTK_FUNCTION(4, "TP_GPIO13_AO"),
		MTK_FUNCTION(5, "TP_UTXD2_VLP"),
		MTK_FUNCTION(6, "UTXD2"),
		MTK_FUNCTION(7, "DBG_MON_A24")
	),
	MTK_PIN_NODRV(
		45, "GPIO45",
		MTK_EINT_FUNCTION(0, 45),
		MTK_FUNCTION(0, "GPIO45"),
		MTK_FUNCTION(1, "SPI1_CSB"),
		MTK_FUNCTION(2, "SCP_SPI1_CS"),
		MTK_FUNCTION(3, "DMIC1_DAT"),
		MTK_FUNCTION(4, "TP_GPIO14_AO"),
		MTK_FUNCTION(5, "TP_URXD2_VLP"),
		MTK_FUNCTION(6, "URXD2"),
		MTK_FUNCTION(7, "DBG_MON_A25")
	),
	MTK_PIN_NODRV(
		46, "GPIO46",
		MTK_EINT_FUNCTION(0, 46),
		MTK_FUNCTION(0, "GPIO46"),
		MTK_FUNCTION(1, "SPI1_MO"),
		MTK_FUNCTION(2, "SCP_SPI1_MO"),
		MTK_FUNCTION(3, "DMIC_CLK"),
		MTK_FUNCTION(4, "SCP_DMIC_CLK"),
		MTK_FUNCTION(5, "TP_URTS2_VLP"),
		MTK_FUNCTION(6, "UCTS2"),
		MTK_FUNCTION(7, "DBG_MON_A26")
	),
	MTK_PIN_NODRV(
		47, "GPIO47",
		MTK_EINT_FUNCTION(0, 47),
		MTK_FUNCTION(0, "GPIO47"),
		MTK_FUNCTION(1, "SPI1_MI"),
		MTK_FUNCTION(2, "SCP_SPI1_MI"),
		MTK_FUNCTION(3, "DMIC_DAT"),
		MTK_FUNCTION(4, "SCP_DMIC_DAT"),
		MTK_FUNCTION(5, "TP_UCTS2_VLP"),
		MTK_FUNCTION(6, "URTS2"),
		MTK_FUNCTION(7, "DBG_MON_A27")
	),
	MTK_PIN_NODRV(
		48, "GPIO48",
		MTK_EINT_FUNCTION(0, 48),
		MTK_FUNCTION(0, "GPIO48"),
		MTK_FUNCTION(1, "SPI0_CLK"),
		MTK_FUNCTION(2, "SCP_SPI0_CK"),
		MTK_FUNCTION(3, "MD_INT0"),
		MTK_FUNCTION(4, "SPM_JTAG_TCK_VLP"),
		MTK_FUNCTION(5, "CONN_BGF_MCU_TCK")
	),
	MTK_PIN_NODRV(
		49, "GPIO49",
		MTK_EINT_FUNCTION(0, 49),
		MTK_FUNCTION(0, "GPIO49"),
		MTK_FUNCTION(1, "SPI0_CSB"),
		MTK_FUNCTION(2, "SCP_SPI0_CS"),
		MTK_FUNCTION(3, "MD_INT3"),
		MTK_FUNCTION(4, "SPM_JTAG_TMS_VLP"),
		MTK_FUNCTION(5, "CONN_BGF_MCU_TMS")
	),
	MTK_PIN_NODRV(
		50, "GPIO50",
		MTK_EINT_FUNCTION(0, 50),
		MTK_FUNCTION(0, "GPIO50"),
		MTK_FUNCTION(1, "SPI0_MO"),
		MTK_FUNCTION(2, "SCP_SPI0_MO"),
		MTK_FUNCTION(3, "MD_INT4"),
		MTK_FUNCTION(4, "SPM_JTAG_TDO_VLP"),
		MTK_FUNCTION(5, "CONN_BGF_MCU_TDO")
	),
	MTK_PIN_NODRV(
		51, "GPIO51",
		MTK_EINT_FUNCTION(0, 51),
		MTK_FUNCTION(0, "GPIO51"),
		MTK_FUNCTION(1, "SPI0_MI"),
		MTK_FUNCTION(2, "SCP_SPI0_MI"),
		MTK_FUNCTION(4, "SPM_JTAG_TDI_VLP"),
		MTK_FUNCTION(5, "CONN_BGF_MCU_TDI")
	),
	MTK_PIN_NODRV(
		52, "GPIO52",
		MTK_EINT_FUNCTION(0, 52),
		MTK_FUNCTION(0, "GPIO52"),
		MTK_FUNCTION(1, "SRCLKENAI0"),
		MTK_FUNCTION(3, "DISP_PWM1"),
		MTK_FUNCTION(4, "SPM_JTAG_TRSTN_VLP"),
		MTK_FUNCTION(5, "CONN_BGF_MCU_TRST_B"),
		MTK_FUNCTION(6, "SPMI_M_TRIG_FLAG")
	),
	MTK_PIN_NODRV(
		53, "GPIO53",
		MTK_EINT_FUNCTION(0, 53),
		MTK_FUNCTION(0, "GPIO53"),
		MTK_FUNCTION(1, "MSDC1_CLK"),
		MTK_FUNCTION(2, "MD1_SIM2_SCLK"),
		MTK_FUNCTION(3, "CONN_DSP_JCK"),
		MTK_FUNCTION(4, "UDI_TCK"),
		MTK_FUNCTION(5, "IPU_JTAG_TCK"),
		MTK_FUNCTION(6, "SSPM_JTAG_TCK_VCORE")
	),
	MTK_PIN_NODRV(
		54, "GPIO54",
		MTK_EINT_FUNCTION(0, 54),
		MTK_FUNCTION(0, "GPIO54"),
		MTK_FUNCTION(1, "MSDC1_CMD"),
		MTK_FUNCTION(2, "CONN_BGF_MCU_AICE_TMSC"),
		MTK_FUNCTION(3, "CONN_DSP_JMS"),
		MTK_FUNCTION(4, "UDI_TMS"),
		MTK_FUNCTION(5, "IPU_JTAG_TMS"),
		MTK_FUNCTION(6, "SSPM_JTAG_TMS_VCORE")
	),
	MTK_PIN_NODRV(
		55, "GPIO55",
		MTK_EINT_FUNCTION(0, 55),
		MTK_FUNCTION(0, "GPIO55"),
		MTK_FUNCTION(1, "MSDC1_DAT0"),
		MTK_FUNCTION(2, "MD1_SIM2_SRST"),
		MTK_FUNCTION(3, "CONN_DSP_JDI"),
		MTK_FUNCTION(4, "UDI_TDI_0"),
		MTK_FUNCTION(5, "IPU_JTAG_TDI"),
		MTK_FUNCTION(6, "SSPM_JTAG_TDI_VCORE")
	),
	MTK_PIN_NODRV(
		56, "GPIO56",
		MTK_EINT_FUNCTION(0, 56),
		MTK_FUNCTION(0, "GPIO56"),
		MTK_FUNCTION(1, "MSDC1_DAT1"),
		MTK_FUNCTION(2, "MD1_SIM2_SIO"),
		MTK_FUNCTION(3, "CONN_DSP_JDO"),
		MTK_FUNCTION(4, "UDI_TDO_0"),
		MTK_FUNCTION(5, "IPU_JTAG_TDO"),
		MTK_FUNCTION(6, "SSPM_JTAG_TDO_VCORE"),
		MTK_FUNCTION(7, "CONN_WF_MCU_AICE_TCKC")
	),
	MTK_PIN_NODRV(
		57, "GPIO57",
		MTK_EINT_FUNCTION(0, 57),
		MTK_FUNCTION(0, "GPIO57"),
		MTK_FUNCTION(1, "MSDC1_DAT2"),
		MTK_FUNCTION(2, "CONN_BGF_MCU_AICE_TCKC"),
		MTK_FUNCTION(3, "CONN_DSP_JINTP"),
		MTK_FUNCTION(4, "UDI_NTRST"),
		MTK_FUNCTION(5, "IPU_JTAG_TRST"),
		MTK_FUNCTION(6, "SSPM_JTAG_TRSTN_VCORE")
	),
	MTK_PIN_NODRV(
		58, "GPIO58",
		MTK_EINT_FUNCTION(0, 58),
		MTK_FUNCTION(0, "GPIO58"),
		MTK_FUNCTION(1, "MSDC1_DAT3"),
		MTK_FUNCTION(4, "CONN_TCXOENA_REQ"),
		MTK_FUNCTION(6, "AGPS_SYNC"),
		MTK_FUNCTION(7, "CONN_WF_MCU_AICE_TMSC")
	),
	MTK_PIN_NODRV(
		59, "GPIO59",
		MTK_EINT_FUNCTION(0, 59),
		MTK_FUNCTION(0, "GPIO59"),
		MTK_FUNCTION(1, "DSI_TE"),
		MTK_FUNCTION(7, "DBG_MON_A28")
	),
	MTK_PIN_NODRV(
		60, "GPIO60",
		MTK_EINT_FUNCTION(0, 60),
		MTK_FUNCTION(0, "GPIO60"),
		MTK_FUNCTION(1, "LCM_RST"),
		MTK_FUNCTION(7, "DBG_MON_A29")
	),
	MTK_PIN_NODRV(
		61, "GPIO61",
		MTK_EINT_FUNCTION(0, 61),
		MTK_FUNCTION(0, "GPIO61"),
		MTK_FUNCTION(1, "DISP_PWM"),
		MTK_FUNCTION(3, "PWM_0")
	),
	MTK_PIN_NODRV(
		62, "GPIO62",
		MTK_EINT_FUNCTION(0, 62),
		MTK_FUNCTION(0, "GPIO62"),
		MTK_FUNCTION(1, "BPI_BUS22"),
		MTK_FUNCTION(3, "UCTS2"),
		MTK_FUNCTION(4, "UDI_TDI_1")
	),
	MTK_PIN_NODRV(
		63, "GPIO63",
		MTK_EINT_FUNCTION(0, 63),
		MTK_FUNCTION(0, "GPIO63"),
		MTK_FUNCTION(1, "BPI_BUS23"),
		MTK_FUNCTION(3, "URTS2"),
		MTK_FUNCTION(4, "UDI_TDO_1")
	),
	MTK_PIN_NODRV(
		64, "GPIO64",
		MTK_EINT_FUNCTION(0, 64),
		MTK_FUNCTION(0, "GPIO64"),
		MTK_FUNCTION(1, "DMIC1_CLK"),
		MTK_FUNCTION(4, "UDI_TDI_2")
	),
	MTK_PIN_NODRV(
		65, "GPIO65",
		MTK_EINT_FUNCTION(0, 65),
		MTK_FUNCTION(0, "GPIO65"),
		MTK_FUNCTION(1, "DMIC1_DAT"),
		MTK_FUNCTION(4, "UDI_TDO_2")
	),
	MTK_PIN_NODRV(
		66, "GPIO66",
		MTK_EINT_FUNCTION(0, 66),
		MTK_FUNCTION(0, "GPIO66"),
		MTK_FUNCTION(1, "CMMCLK0"),
		MTK_FUNCTION(4, "UDI_TDI_3"),
		MTK_FUNCTION(7, "DBG_MON_B7")
	),
	MTK_PIN_NODRV(
		67, "GPIO67",
		MTK_EINT_FUNCTION(0, 67),
		MTK_FUNCTION(0, "GPIO67"),
		MTK_FUNCTION(1, "CMMCLK1"),
		MTK_FUNCTION(4, "UDI_TDO_3"),
		MTK_FUNCTION(7, "DBG_MON_B8")
	),
	MTK_PIN_NODRV(
		68, "GPIO68",
		MTK_EINT_FUNCTION(0, 68),
		MTK_FUNCTION(0, "GPIO68"),
		MTK_FUNCTION(1, "CMMCLK2"),
		MTK_FUNCTION(4, "UDI_TDI_4"),
		MTK_FUNCTION(7, "DBG_MON_B9")
	),
	MTK_PIN_NODRV(
		69, "GPIO69",
		MTK_EINT_FUNCTION(0, 69),
		MTK_FUNCTION(0, "GPIO69"),
		MTK_FUNCTION(1, "CMMCLK3"),
		MTK_FUNCTION(4, "UDI_TDO_4"),
		MTK_FUNCTION(7, "DBG_MON_B10")
	),
	MTK_PIN_NODRV(
		70, "GPIO70",
		MTK_EINT_FUNCTION(0, 70),
		MTK_FUNCTION(0, "GPIO70"),
		MTK_FUNCTION(1, "I2SIN1_BCK"),
		MTK_FUNCTION(2, "I2SIN_DMA0_BCK"),
		MTK_FUNCTION(4, "MFG_EB_JTAG_TCK"),
		MTK_FUNCTION(6, "SCP_JTAG0_TCK_VCORE"),
		MTK_FUNCTION(7, "CONN_WF_MCU_TCK")
	),
	MTK_PIN_NODRV(
		71, "GPIO71",
		MTK_EINT_FUNCTION(0, 71),
		MTK_FUNCTION(0, "GPIO71"),
		MTK_FUNCTION(1, "I2SIN1_LRCK"),
		MTK_FUNCTION(2, "I2SIN_DMA0_LRCK"),
		MTK_FUNCTION(4, "MFG_EB_JTAG_TMS"),
		MTK_FUNCTION(6, "SCP_JTAG0_TMS_VCORE"),
		MTK_FUNCTION(7, "CONN_WF_MCU_TMS")
	),
	MTK_PIN_NODRV(
		72, "GPIO72",
		MTK_EINT_FUNCTION(0, 72),
		MTK_FUNCTION(0, "GPIO72"),
		MTK_FUNCTION(1, "I2SIN1_DI"),
		MTK_FUNCTION(2, "I2SIN_DMA0_DI"),
		MTK_FUNCTION(4, "MFG_EB_JTAG_TDI"),
		MTK_FUNCTION(6, "SCP_JTAG0_TDI_VCORE"),
		MTK_FUNCTION(7, "CONN_WF_MCU_TDI")
	),
	MTK_PIN_NODRV(
		73, "GPIO73",
		MTK_EINT_FUNCTION(0, 73),
		MTK_FUNCTION(0, "GPIO73"),
		MTK_FUNCTION(1, "I2SOUT1_DO"),
		MTK_FUNCTION(2, "I2SIN_DMA0_DI1"),
		MTK_FUNCTION(4, "MFG_EB_JTAG_TDO"),
		MTK_FUNCTION(6, "SCP_JTAG0_TDO_VCORE"),
		MTK_FUNCTION(7, "CONN_WF_MCU_TDO")
	),
	MTK_PIN_NODRV(
		74, "GPIO74",
		MTK_EINT_FUNCTION(0, 74),
		MTK_FUNCTION(0, "GPIO74"),
		MTK_FUNCTION(1, "I2SIN1_MCK"),
		MTK_FUNCTION(2, "I2SIN_DMA0_DI2"),
		MTK_FUNCTION(4, "MFG_EB_JTAG_TRSTN"),
		MTK_FUNCTION(6, "SCP_JTAG0_TRSTN_VCORE"),
		MTK_FUNCTION(7, "CONN_WF_MCU_TRST_B")
	),
	MTK_PIN_NODRV(
		75, "GPIO75",
		MTK_EINT_FUNCTION(0, 75),
		MTK_FUNCTION(0, "GPIO75"),
		MTK_FUNCTION(1, "I2SIN2_BCK"),
		MTK_FUNCTION(5, "SCP_JTAG0_TCK_VLP"),
		MTK_FUNCTION(6, "SSPM_JTAG_TCK_VLP"),
		MTK_FUNCTION(7, "IO_JTAG_TCK")
	),
	MTK_PIN_NODRV(
		76, "GPIO76",
		MTK_EINT_FUNCTION(0, 76),
		MTK_FUNCTION(0, "GPIO76"),
		MTK_FUNCTION(1, "I2SIN2_LRCK"),
		MTK_FUNCTION(5, "SCP_JTAG0_TMS_VLP"),
		MTK_FUNCTION(6, "SSPM_JTAG_TMS_VLP"),
		MTK_FUNCTION(7, "IO_JTAG_TMS")
	),
	MTK_PIN_NODRV(
		77, "GPIO77",
		MTK_EINT_FUNCTION(0, 77),
		MTK_FUNCTION(0, "GPIO77"),
		MTK_FUNCTION(1, "I2SIN2_DI"),
		MTK_FUNCTION(5, "SCP_JTAG0_TDI_VLP"),
		MTK_FUNCTION(6, "SSPM_JTAG_TDI_VLP"),
		MTK_FUNCTION(7, "IO_JTAG_TDI")
	),
	MTK_PIN_NODRV(
		78, "GPIO78",
		MTK_EINT_FUNCTION(0, 78),
		MTK_FUNCTION(0, "GPIO78"),
		MTK_FUNCTION(1, "I2SOUT2_DO"),
		MTK_FUNCTION(5, "SCP_JTAG0_TDO_VLP"),
		MTK_FUNCTION(6, "SSPM_JTAG_TDO_VLP"),
		MTK_FUNCTION(7, "IO_JTAG_TDO")
	),
	MTK_PIN_NODRV(
		79, "GPIO79",
		MTK_EINT_FUNCTION(0, 79),
		MTK_FUNCTION(0, "GPIO79"),
		MTK_FUNCTION(1, "I2SIN2_MCK"),
		MTK_FUNCTION(5, "SCP_JTAG0_TRSTN_VLP"),
		MTK_FUNCTION(6, "SSPM_JTAG_TRSTN_VLP"),
		MTK_FUNCTION(7, "IO_JTAG_TRSTN")
	),
	MTK_PIN_NODRV(
		80, "GPIO80",
		MTK_EINT_FUNCTION(0, 80),
		MTK_FUNCTION(0, "GPIO80"),
		MTK_FUNCTION(1, "SCP_SCL0"),
		MTK_FUNCTION(2, "UCTS3"),
		MTK_FUNCTION(3, "CMMCLK4"),
		MTK_FUNCTION(4, "TP_GPIO2_AO"),
		MTK_FUNCTION(5, "PWM_VLP"),
		MTK_FUNCTION(6, "TP_UTXD2_VLP")
	),
	MTK_PIN_NODRV(
		81, "GPIO81",
		MTK_EINT_FUNCTION(0, 81),
		MTK_FUNCTION(0, "GPIO81"),
		MTK_FUNCTION(1, "SCP_SDA0"),
		MTK_FUNCTION(2, "URTS3"),
		MTK_FUNCTION(3, "CMMCLK5"),
		MTK_FUNCTION(4, "TP_GPIO3_AO"),
		MTK_FUNCTION(5, "SRCLKENAI1"),
		MTK_FUNCTION(6, "TP_URXD2_VLP")
	),
	MTK_PIN_NODRV(
		82, "GPIO82",
		MTK_EINT_FUNCTION(0, 82),
		MTK_FUNCTION(0, "GPIO82"),
		MTK_FUNCTION(1, "SCP_SCL1"),
		MTK_FUNCTION(2, "UTXD3"),
		MTK_FUNCTION(3, "CMMCLK4"),
		MTK_FUNCTION(4, "TP_GPIO4_AO"),
		MTK_FUNCTION(5, "MD_INT3"),
		MTK_FUNCTION(6, "TP_URTS2_VLP")
	),
	MTK_PIN_NODRV(
		83, "GPIO83",
		MTK_EINT_FUNCTION(0, 83),
		MTK_FUNCTION(0, "GPIO83"),
		MTK_FUNCTION(1, "SCP_SDA1"),
		MTK_FUNCTION(2, "URXD3"),
		MTK_FUNCTION(3, "CMMCLK5"),
		MTK_FUNCTION(4, "TP_GPIO5_AO"),
		MTK_FUNCTION(5, "MD_INT4"),
		MTK_FUNCTION(6, "TP_UCTS2_VLP")
	),
	MTK_PIN_NODRV(
		84, "GPIO84",
		MTK_EINT_FUNCTION(0, 84),
		MTK_FUNCTION(0, "GPIO84"),
		MTK_FUNCTION(1, "SCP_SCL2"),
		MTK_FUNCTION(2, "MD_MCIF_UTXD0"),
		MTK_FUNCTION(3, "MD_UTXD0"),
		MTK_FUNCTION(4, "TP_GPIO6_AO"),
		MTK_FUNCTION(5, "SSPM_UTXD_AO_VLP"),
		MTK_FUNCTION(6, "TP_UTXD1_VLP"),
		MTK_FUNCTION(7, "CONN_BGF_UART0_TXD")
	),
	MTK_PIN_NODRV(
		85, "GPIO85",
		MTK_EINT_FUNCTION(0, 85),
		MTK_FUNCTION(0, "GPIO85"),
		MTK_FUNCTION(1, "SCP_SDA2"),
		MTK_FUNCTION(2, "MD_MCIF_URXD0"),
		MTK_FUNCTION(3, "MD_URXD0"),
		MTK_FUNCTION(4, "TP_GPIO7_AO"),
		MTK_FUNCTION(5, "SSPM_URXD_AO_VLP"),
		MTK_FUNCTION(6, "TP_URXD1_VLP"),
		MTK_FUNCTION(7, "CONN_BGF_UART0_RXD")
	),
	MTK_PIN_NODRV(
		86, "GPIO86",
		MTK_EINT_FUNCTION(0, 86),
		MTK_FUNCTION(0, "GPIO86"),
		MTK_FUNCTION(1, "SCP_SCL3"),
		MTK_FUNCTION(3, "I2SIN6_BCK"),
		MTK_FUNCTION(4, "TP_GPIO8_AO"),
		MTK_FUNCTION(5, "EXTIF0_ACT"),
		MTK_FUNCTION(6, "TP_URTS1_VLP")
	),
	MTK_PIN_NODRV(
		87, "GPIO87",
		MTK_EINT_FUNCTION(0, 87),
		MTK_FUNCTION(0, "GPIO87"),
		MTK_FUNCTION(1, "SCP_SDA3"),
		MTK_FUNCTION(3, "I2SOUT6_DO"),
		MTK_FUNCTION(4, "TP_GPIO9_AO"),
		MTK_FUNCTION(5, "EXTIF0_PRI"),
		MTK_FUNCTION(6, "TP_UCTS1_VLP")
	),
	MTK_PIN_NODRV(
		88, "GPIO88",
		MTK_EINT_FUNCTION(0, 88),
		MTK_FUNCTION(0, "GPIO88"),
		MTK_FUNCTION(1, "SCP_SCL4"),
		MTK_FUNCTION(3, "I2SOUT6_DO1"),
		MTK_FUNCTION(4, "TP_GPIO10_AO"),
		MTK_FUNCTION(5, "EXTIF0_GNT_B")
	),
	MTK_PIN_NODRV(
		89, "GPIO89",
		MTK_EINT_FUNCTION(0, 89),
		MTK_FUNCTION(0, "GPIO89"),
		MTK_FUNCTION(1, "SCP_SDA4"),
		MTK_FUNCTION(3, "I2SOUT6_DO2"),
		MTK_FUNCTION(4, "TP_GPIO11_AO"),
		MTK_FUNCTION(5, "MD_INT0")
	),
	MTK_PIN_NODRV(
		90, "GPIO90",
		MTK_EINT_FUNCTION(0, 90),
		MTK_FUNCTION(0, "GPIO90"),
		MTK_FUNCTION(1, "SCP_SCL5"),
		MTK_FUNCTION(3, "I2SIN6_LRCK"),
		MTK_FUNCTION(4, "TP_GPIO12_AO"),
		MTK_FUNCTION(6, "MD_GPS_L1_BLANK")
	),
	MTK_PIN_NODRV(
		91, "GPIO91",
		MTK_EINT_FUNCTION(0, 91),
		MTK_FUNCTION(0, "GPIO91"),
		MTK_FUNCTION(1, "SCP_SDA5"),
		MTK_FUNCTION(3, "I2SOUT6_DO3"),
		MTK_FUNCTION(4, "TP_GPIO13_AO"),
		MTK_FUNCTION(6, "MD_GPS_L5_BLANK")
	),
	MTK_PIN_NODRV(
		92, "GPIO92",
		MTK_EINT_FUNCTION(0, 92),
		MTK_FUNCTION(0, "GPIO92"),
		MTK_FUNCTION(1, "GPS_L1_ELNA_EN"),
		MTK_FUNCTION(2, "BT_RST"),
		MTK_FUNCTION(7, "DBG_MON_B11")
	),
	MTK_PIN_NODRV(
		93, "GPIO93",
		MTK_EINT_FUNCTION(0, 93),
		MTK_FUNCTION(0, "GPIO93"),
		MTK_FUNCTION(1, "GPS_L5_ELNA_EN"),
		MTK_FUNCTION(2, "WF_RST"),
		MTK_FUNCTION(7, "DBG_MON_B12")
	),
	MTK_PIN_NODRV(
		94, "GPIO94",
		MTK_EINT_FUNCTION(0, 94),
		MTK_FUNCTION(0, "GPIO94"),
		MTK_FUNCTION(1, "ANT_SEL0"),
		MTK_FUNCTION(2, "PCIE_PERSTN"),
		MTK_FUNCTION(4, "TP_GPIO14_AO")
	),
	MTK_PIN_NODRV(
		95, "GPIO95",
		MTK_EINT_FUNCTION(0, 95),
		MTK_FUNCTION(0, "GPIO95"),
		MTK_FUNCTION(1, "ANT_SEL1"),
		MTK_FUNCTION(2, "PCIE_WAKEN"),
		MTK_FUNCTION(4, "TP_GPIO15_AO"),
		MTK_FUNCTION(5, "PWM_3")
	),
	MTK_PIN_NODRV(
		96, "GPIO96",
		MTK_EINT_FUNCTION(0, 96),
		MTK_FUNCTION(0, "GPIO96"),
		MTK_FUNCTION(1, "ANT_SEL2"),
		MTK_FUNCTION(2, "PCIE_CLKREQN"),
		MTK_FUNCTION(4, "TP_GPIO0_AO"),
		MTK_FUNCTION(6, "EXT_FRAME_SYNC")
	),
	MTK_PIN_NODRV(
		97, "GPIO97",
		MTK_EINT_FUNCTION(0, 97),
		MTK_FUNCTION(0, "GPIO97"),
		MTK_FUNCTION(1, "ANT_SEL3"),
		MTK_FUNCTION(2, "TP_UTXD2_VLP"),
		MTK_FUNCTION(3, "I2SIN6_DI"),
		MTK_FUNCTION(4, "TP_GPIO1_AO"),
		MTK_FUNCTION(5, "PWM_2"),
		MTK_FUNCTION(6, "MD_UTXD1")
	),
	MTK_PIN_NODRV(
		98, "GPIO98",
		MTK_EINT_FUNCTION(0, 98),
		MTK_FUNCTION(0, "GPIO98"),
		MTK_FUNCTION(1, "ANT_SEL4"),
		MTK_FUNCTION(2, "TP_URXD2_VLP"),
		MTK_FUNCTION(3, "I2SIN6_DI1"),
		MTK_FUNCTION(4, "TP_GPIO2_AO"),
		MTK_FUNCTION(6, "MD_URXD1")
	),
	MTK_PIN_NODRV(
		99, "GPIO99",
		MTK_EINT_FUNCTION(0, 99),
		MTK_FUNCTION(0, "GPIO99"),
		MTK_FUNCTION(1, "ANT_SEL5"),
		MTK_FUNCTION(2, "PTA_TXD"),
		MTK_FUNCTION(3, "I2SIN6_DI2"),
		MTK_FUNCTION(4, "CONN_TCXOENA_REQ"),
		MTK_FUNCTION(6, "AGPS_SYNC")
	),
	MTK_PIN_NODRV(
		100, "GPIO100",
		MTK_EINT_FUNCTION(0, 100),
		MTK_FUNCTION(0, "GPIO100"),
		MTK_FUNCTION(1, "ANT_SEL6"),
		MTK_FUNCTION(2, "PTA_RXD"),
		MTK_FUNCTION(3, "I2SIN6_DI3"),
		MTK_FUNCTION(5, "PWM_1")
	),
	MTK_PIN_NODRV(
		101, "GPIO101",
		MTK_EINT_FUNCTION(0, 101),
		MTK_FUNCTION(0, "GPIO101"),
		MTK_FUNCTION(1, "ANT_SEL7"),
		MTK_FUNCTION(2, "CONN_HRST_B_2"),
		MTK_FUNCTION(3, "I2SIN6_MCK")
	),
	MTK_PIN_NODRV(
		102, "GPIO102",
		MTK_EINT_FUNCTION(0, 102),
		MTK_FUNCTION(0, "GPIO102"),
		MTK_FUNCTION(1, "CONN_HRST_B"),
		MTK_FUNCTION(2, "CONN_RST"),
		MTK_FUNCTION(4, "TP_GPIO3_AO")
	),
	MTK_PIN_NODRV(
		103, "GPIO103",
		MTK_EINT_FUNCTION(0, 103),
		MTK_FUNCTION(0, "GPIO103"),
		MTK_FUNCTION(1, "CONN_TOP_CLK"),
		MTK_FUNCTION(3, "UTXD3"),
		MTK_FUNCTION(4, "CONN_UARTHUB_UART_TX")
	),
	MTK_PIN_NODRV(
		104, "GPIO104",
		MTK_EINT_FUNCTION(0, 104),
		MTK_FUNCTION(0, "GPIO104"),
		MTK_FUNCTION(1, "CONN_TOP_DATA"),
		MTK_FUNCTION(3, "URXD3"),
		MTK_FUNCTION(4, "CONN_UARTHUB_UART_RX")
	),
	MTK_PIN_NODRV(
		105, "GPIO105",
		MTK_EINT_FUNCTION(0, 105),
		MTK_FUNCTION(0, "GPIO105"),
		MTK_FUNCTION(1, "CONN_BT_CLK"),
		MTK_FUNCTION(2, "CONN_FAULTB"),
		MTK_FUNCTION(4, "TP_GPIO4_AO")
	),
	MTK_PIN_NODRV(
		106, "GPIO106",
		MTK_EINT_FUNCTION(0, 106),
		MTK_FUNCTION(0, "GPIO106"),
		MTK_FUNCTION(1, "CONN_BT_DATA"),
		MTK_FUNCTION(2, "CONN_PMIC_EN")
	),
	MTK_PIN_NODRV(
		107, "GPIO107",
		MTK_EINT_FUNCTION(0, 107),
		MTK_FUNCTION(0, "GPIO107"),
		MTK_FUNCTION(1, "CONN_WB_PTA"),
		MTK_FUNCTION(2, "I2SIN2_BCK"),
		MTK_FUNCTION(6, "TP_UTXD1_VCORE")
	),
	MTK_PIN_NODRV(
		108, "GPIO108",
		MTK_EINT_FUNCTION(0, 108),
		MTK_FUNCTION(0, "GPIO108"),
		MTK_FUNCTION(1, "CONN_WF_CTRL0"),
		MTK_FUNCTION(2, "I2SIN2_LRCK"),
		MTK_FUNCTION(6, "TP_URXD1_VCORE")
	),
	MTK_PIN_NODRV(
		109, "GPIO109",
		MTK_EINT_FUNCTION(0, 109),
		MTK_FUNCTION(0, "GPIO109"),
		MTK_FUNCTION(1, "CONN_WF_CTRL1"),
		MTK_FUNCTION(2, "I2SIN2_DI"),
		MTK_FUNCTION(6, "TP_URTS1_VCORE")
	),
	MTK_PIN_NODRV(
		110, "GPIO110",
		MTK_EINT_FUNCTION(0, 110),
		MTK_FUNCTION(0, "GPIO110"),
		MTK_FUNCTION(1, "CONN_WF_CTRL2"),
		MTK_FUNCTION(2, "I2SOUT2_DO"),
		MTK_FUNCTION(6, "TP_UCTS1_VCORE")
	),
	MTK_PIN_NODRV(
		111, "GPIO111",
		MTK_EINT_FUNCTION(0, 111),
		MTK_FUNCTION(0, "GPIO111"),
		MTK_FUNCTION(1, "CONN_WF_CTRL3"),
		MTK_FUNCTION(2, "CONN_TOP_CLK_2")
	),
	MTK_PIN_NODRV(
		112, "GPIO112",
		MTK_EINT_FUNCTION(0, 112),
		MTK_FUNCTION(0, "GPIO112"),
		MTK_FUNCTION(1, "CONN_WF_CTRL4"),
		MTK_FUNCTION(2, "CONN_TOP_DATA_2")
	),
	MTK_PIN_NODRV(
		113, "GPIO113",
		MTK_EINT_FUNCTION(0, 113),
		MTK_FUNCTION(0, "GPIO113"),
		MTK_FUNCTION(1, "PWM_0"),
		MTK_FUNCTION(2, "MD1_SIM2_SCLK")
	),
	MTK_PIN_NODRV(
		114, "GPIO114",
		MTK_EINT_FUNCTION(0, 114),
		MTK_FUNCTION(0, "GPIO114"),
		MTK_FUNCTION(1, "PWM_1"),
		MTK_FUNCTION(2, "MD1_SIM2_SRST"),
		MTK_FUNCTION(5, "TP_UTXD2_VCORE"),
		MTK_FUNCTION(6, "UTXD2"),
		MTK_FUNCTION(7, "DBG_MON_B13")
	),
	MTK_PIN_NODRV(
		115, "GPIO115",
		MTK_EINT_FUNCTION(0, 115),
		MTK_FUNCTION(0, "GPIO115"),
		MTK_FUNCTION(1, "PWM_2"),
		MTK_FUNCTION(2, "MD1_SIM2_SIO"),
		MTK_FUNCTION(5, "TP_URXD2_VCORE"),
		MTK_FUNCTION(6, "URXD2"),
		MTK_FUNCTION(7, "DBG_MON_B14")
	),
	MTK_PIN_NODRV(
		116, "GPIO116",
		MTK_EINT_FUNCTION(0, 116),
		MTK_FUNCTION(0, "GPIO116"),
		MTK_FUNCTION(1, "PWM_3"),
		MTK_FUNCTION(2, "MD1_SIM1_SCLK")
	),
	MTK_PIN_NODRV(
		117, "GPIO117",
		MTK_EINT_FUNCTION(0, 117),
		MTK_FUNCTION(0, "GPIO117"),
		MTK_FUNCTION(1, "DSI_TE1"),
		MTK_FUNCTION(2, "MD1_SIM1_SRST"),
		MTK_FUNCTION(4, "UDI_TDI_5"),
		MTK_FUNCTION(6, "MD_GPS_L1_BLANK"),
		MTK_FUNCTION(7, "DBG_MON_A30")
	),
	MTK_PIN_NODRV(
		118, "GPIO118",
		MTK_EINT_FUNCTION(0, 118),
		MTK_FUNCTION(0, "GPIO118"),
		MTK_FUNCTION(1, "LCM_RST1"),
		MTK_FUNCTION(2, "MD1_SIM1_SIO"),
		MTK_FUNCTION(4, "UDI_TDO_5"),
		MTK_FUNCTION(6, "MD_GPS_L5_BLANK"),
		MTK_FUNCTION(7, "DBG_MON_A31")
	),
	MTK_PIN_NODRV(
		119, "GPIO119",
		MTK_EINT_FUNCTION(0, 119),
		MTK_FUNCTION(0, "GPIO119"),
		MTK_FUNCTION(1, "MD_INT1_C2K_UIM0_HOT_PLUG"),
		MTK_FUNCTION(5, "SRCLKENA1")
	),
	MTK_PIN_NODRV(
		120, "GPIO120",
		MTK_EINT_FUNCTION(0, 120),
		MTK_FUNCTION(0, "GPIO120"),
		MTK_FUNCTION(1, "MD_INT2_C2K_UIM1_HOT_PLUG"),
		MTK_FUNCTION(4, "TP_GPIO15_AO"),
		MTK_FUNCTION(5, "SRCLKENA2")
	),
	MTK_PIN_NODRV(
		121, "GPIO121",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO121"),
		MTK_FUNCTION(1, "SCL0")
	),
	MTK_PIN_NODRV(
		122, "GPIO122",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO122"),
		MTK_FUNCTION(1, "SDA0")
	),
	MTK_PIN_NODRV(
		123, "GPIO123",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO123"),
		MTK_FUNCTION(1, "SCL1")
	),
	MTK_PIN_NODRV(
		124, "GPIO124",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO124"),
		MTK_FUNCTION(1, "SDA1")
	),
	MTK_PIN_NODRV(
		125, "GPIO125",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO125"),
		MTK_FUNCTION(1, "SCL2"),
		MTK_FUNCTION(2, "SCP_SCL2"),
		MTK_FUNCTION(4, "UDI_TDI_6")
	),
	MTK_PIN_NODRV(
		126, "GPIO126",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO126"),
		MTK_FUNCTION(1, "SDA2"),
		MTK_FUNCTION(2, "SCP_SDA2"),
		MTK_FUNCTION(4, "UDI_TDO_6")
	),
	MTK_PIN_NODRV(
		127, "GPIO127",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO127"),
		MTK_FUNCTION(1, "SCL3"),
		MTK_FUNCTION(6, "MBISTREADEN_TRIGGER")
	),
	MTK_PIN_NODRV(
		128, "GPIO128",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO128"),
		MTK_FUNCTION(1, "SDA3"),
		MTK_FUNCTION(6, "MBISTWRITEEN_TRIGGER")
	),
	MTK_PIN_NODRV(
		129, "GPIO129",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO129"),
		MTK_FUNCTION(1, "SCL4"),
		MTK_FUNCTION(2, "SCP_SCL4"),
		MTK_FUNCTION(4, "UDI_TDI_7")
	),
	MTK_PIN_NODRV(
		130, "GPIO130",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO130"),
		MTK_FUNCTION(1, "SDA4"),
		MTK_FUNCTION(2, "SCP_SDA4"),
		MTK_FUNCTION(4, "UDI_TDO_7")
	),
	MTK_PIN_NODRV(
		131, "GPIO131",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO131"),
		MTK_FUNCTION(1, "SCL5"),
		MTK_FUNCTION(4, "TP_GPIO0_AO")
	),
	MTK_PIN_NODRV(
		132, "GPIO132",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO132"),
		MTK_FUNCTION(1, "SDA5"),
		MTK_FUNCTION(4, "TP_GPIO1_AO")
	),
	MTK_PIN_NODRV(
		133, "GPIO133",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO133"),
		MTK_FUNCTION(1, "SCL6")
	),
	MTK_PIN_NODRV(
		134, "GPIO134",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO134"),
		MTK_FUNCTION(1, "SDA6")
	),
	MTK_PIN_NODRV(
		135, "GPIO135",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO135"),
		MTK_FUNCTION(1, "SCL7")
	),
	MTK_PIN_NODRV(
		136, "GPIO136",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO136"),
		MTK_FUNCTION(1, "SDA7")
	),
	MTK_PIN_NODRV(
		137, "GPIO137",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO137"),
		MTK_FUNCTION(1, "SCL8")
	),
	MTK_PIN_NODRV(
		138, "GPIO138",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO138"),
		MTK_FUNCTION(1, "SDA8")
	),
	MTK_PIN_NODRV(
		139, "GPIO139",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO139"),
		MTK_FUNCTION(1, "SCL9"),
		MTK_FUNCTION(2, "UTXD2")
	),
	MTK_PIN_NODRV(
		140, "GPIO140",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO140"),
		MTK_FUNCTION(1, "SDA9"),
		MTK_FUNCTION(2, "URXD2")
	),
	MTK_PIN_NODRV(
		141, "GPIO141",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO141"),
		MTK_FUNCTION(1, "SCL10"),
		MTK_FUNCTION(2, "UCTS2"),
		MTK_FUNCTION(3, "GPS_L1_ELNA_EN")
	),
	MTK_PIN_NODRV(
		142, "GPIO142",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO142"),
		MTK_FUNCTION(1, "SDA10"),
		MTK_FUNCTION(2, "URTS2"),
		MTK_FUNCTION(3, "GPS_L5_ELNA_EN")
	),
	MTK_PIN_NODRV(
		143, "GPIO143",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO143"),
		MTK_FUNCTION(1, "SCL11"),
		MTK_FUNCTION(2, "UTXD4"),
		MTK_FUNCTION(3, "PWM_2"),
		MTK_FUNCTION(4, "DISP_ERROR0"),
		MTK_FUNCTION(6, "TP_URTS2_VCORE")
	),
	MTK_PIN_NODRV(
		144, "GPIO144",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO144"),
		MTK_FUNCTION(1, "SDA11"),
		MTK_FUNCTION(2, "URXD4"),
		MTK_FUNCTION(3, "PWM_3"),
		MTK_FUNCTION(4, "DISP_ERROR1"),
		MTK_FUNCTION(6, "TP_UCTS2_VCORE")
	),
	MTK_PIN_NODRV(
		145, "GPIO145",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO145"),
		MTK_FUNCTION(1, "SCL12"),
		MTK_FUNCTION(2, "UTXD5"),
		MTK_FUNCTION(3, "MD_UTXD1"),
		MTK_FUNCTION(5, "SSPM_UTXD_AO_VCORE"),
		MTK_FUNCTION(6, "TP_UTXD2_VCORE"),
		MTK_FUNCTION(7, "CONN_BT_TXD")
	),
	MTK_PIN_NODRV(
		146, "GPIO146",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO146"),
		MTK_FUNCTION(1, "SDA12"),
		MTK_FUNCTION(2, "URXD5"),
		MTK_FUNCTION(3, "MD_URXD1"),
		MTK_FUNCTION(5, "SSPM_URXD_AO_VCORE"),
		MTK_FUNCTION(6, "TP_URXD2_VCORE")
	),
	MTK_PIN_NODRV(
		147, "GPIO147",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO147"),
		MTK_FUNCTION(1, "BPI_BUS0"),
		MTK_FUNCTION(3, "ANT_SEL8"),
		MTK_FUNCTION(4, "CLKM0"),
		MTK_FUNCTION(7, "DBG_MON_B15")
	),
	MTK_PIN_NODRV(
		148, "GPIO148",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO148"),
		MTK_FUNCTION(1, "BPI_BUS1"),
		MTK_FUNCTION(3, "ANT_SEL9"),
		MTK_FUNCTION(4, "CLKM1"),
		MTK_FUNCTION(7, "DBG_MON_B16")
	),
	MTK_PIN_NODRV(
		149, "GPIO149",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO149"),
		MTK_FUNCTION(1, "BPI_BUS2"),
		MTK_FUNCTION(3, "ANT_SEL10"),
		MTK_FUNCTION(4, "CLKM2"),
		MTK_FUNCTION(7, "DBG_MON_B17")
	),
	MTK_PIN_NODRV(
		150, "GPIO150",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO150"),
		MTK_FUNCTION(1, "BPI_BUS3"),
		MTK_FUNCTION(3, "ANT_SEL11"),
		MTK_FUNCTION(4, "CLKM3"),
		MTK_FUNCTION(6, "MD_GPS_BLANK_EXT"),
		MTK_FUNCTION(7, "DBG_MON_B18")
	),
	MTK_PIN_NODRV(
		151, "GPIO151",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO151"),
		MTK_FUNCTION(1, "BPI_BUS4"),
		MTK_FUNCTION(3, "ANT_SEL12"),
		MTK_FUNCTION(6, "MD32_0_RXD"),
		MTK_FUNCTION(7, "DBG_MON_B19")
	),
	MTK_PIN_NODRV(
		152, "GPIO152",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO152"),
		MTK_FUNCTION(1, "BPI_BUS5"),
		MTK_FUNCTION(3, "ANT_SEL13"),
		MTK_FUNCTION(6, "MD32_0_TXD"),
		MTK_FUNCTION(7, "DBG_MON_B20")
	),
	MTK_PIN_NODRV(
		153, "GPIO153",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO153"),
		MTK_FUNCTION(1, "BPI_BUS6"),
		MTK_FUNCTION(2, "CONN_BPI_BUS6"),
		MTK_FUNCTION(3, "ANT_SEL14"),
		MTK_FUNCTION(6, "MD32_0_GPIO0"),
		MTK_FUNCTION(7, "DBG_MON_B21")
	),
	MTK_PIN_NODRV(
		154, "GPIO154",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO154"),
		MTK_FUNCTION(1, "BPI_BUS7"),
		MTK_FUNCTION(2, "CONN_BPI_BUS7"),
		MTK_FUNCTION(3, "ANT_SEL15"),
		MTK_FUNCTION(6, "MD32_1_RXD"),
		MTK_FUNCTION(7, "DBG_MON_B22")
	),
	MTK_PIN_NODRV(
		155, "GPIO155",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO155"),
		MTK_FUNCTION(1, "BPI_BUS8"),
		MTK_FUNCTION(2, "CONN_BPI_BUS8"),
		MTK_FUNCTION(3, "ANT_SEL16"),
		MTK_FUNCTION(6, "MD32_1_TXD"),
		MTK_FUNCTION(7, "DBG_MON_B23")
	),
	MTK_PIN_NODRV(
		156, "GPIO156",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO156"),
		MTK_FUNCTION(1, "BPI_BUS9"),
		MTK_FUNCTION(2, "CONN_BPI_BUS9"),
		MTK_FUNCTION(3, "ANT_SEL17"),
		MTK_FUNCTION(6, "MD32_1_GPIO0"),
		MTK_FUNCTION(7, "DBG_MON_B24")
	),
	MTK_PIN_NODRV(
		157, "GPIO157",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO157"),
		MTK_FUNCTION(1, "MD_UCNT_A_TGL")
	),
	MTK_PIN_NODRV(
		158, "GPIO158",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO158"),
		MTK_FUNCTION(1, "DIGRF_IRQ")
	),
	MTK_PIN_NODRV(
		159, "GPIO159",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO159"),
		MTK_FUNCTION(1, "MIPI_M_SCLK")
	),
	MTK_PIN_NODRV(
		160, "GPIO160",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO160"),
		MTK_FUNCTION(1, "MIPI_M_SDATA")
	),
	MTK_PIN_NODRV(
		161, "GPIO161",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO161"),
		MTK_FUNCTION(1, "MIPI0_D_SCLK"),
		MTK_FUNCTION(2, "CONN_MIPI0_SCLK"),
		MTK_FUNCTION(3, "BPI_BUS16"),
		MTK_FUNCTION(4, "CONN_BPI_BUS16_OLAT5"),
		MTK_FUNCTION(6, "MD_GPS_L1_BLANK")
	),
	MTK_PIN_NODRV(
		162, "GPIO162",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO162"),
		MTK_FUNCTION(1, "MIPI0_D_SDATA"),
		MTK_FUNCTION(2, "CONN_MIPI0_SDATA"),
		MTK_FUNCTION(3, "BPI_BUS17"),
		MTK_FUNCTION(4, "CONN_BPI_BUS17_ANT0"),
		MTK_FUNCTION(6, "MD_GPS_L5_BLANK")
	),
	MTK_PIN_NODRV(
		163, "GPIO163",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO163"),
		MTK_FUNCTION(1, "MIPI1_D_SCLK"),
		MTK_FUNCTION(2, "CONN_MIPI1_SCLK"),
		MTK_FUNCTION(3, "BPI_BUS18"),
		MTK_FUNCTION(4, "CONN_BPI_BUS18_ANT1"),
		MTK_FUNCTION(5, "MIPI3_D_SCLK")
	),
	MTK_PIN_NODRV(
		164, "GPIO164",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO164"),
		MTK_FUNCTION(1, "MIPI1_D_SDATA"),
		MTK_FUNCTION(2, "CONN_MIPI1_SDATA"),
		MTK_FUNCTION(3, "BPI_BUS19"),
		MTK_FUNCTION(4, "CONN_BPI_BUS19_ANT2"),
		MTK_FUNCTION(5, "MIPI3_D_SDATA"),
		MTK_FUNCTION(6, "MD_GPS_BLANK_EXT")
	),
	MTK_PIN_NODRV(
		165, "GPIO165",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO165"),
		MTK_FUNCTION(1, "MIPI2_D_SCLK"),
		MTK_FUNCTION(3, "BPI_BUS20"),
		MTK_FUNCTION(4, "CONN_BPI_BUS20_ANT3"),
		MTK_FUNCTION(5, "MIPI4_D_SCLK")
	),
	MTK_PIN_NODRV(
		166, "GPIO166",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO166"),
		MTK_FUNCTION(1, "MIPI2_D_SDATA"),
		MTK_FUNCTION(3, "BPI_BUS21"),
		MTK_FUNCTION(4, "CONN_BPI_BUS21_ANT4"),
		MTK_FUNCTION(5, "MIPI4_D_SDATA")
	),
	MTK_PIN_NODRV(
		167, "GPIO167",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO167"),
		MTK_FUNCTION(1, "UTXD0")
	),
	MTK_PIN_NODRV(
		168, "GPIO168",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO168"),
		MTK_FUNCTION(1, "URXD0")
	),
	MTK_PIN_NODRV(
		169, "GPIO169",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO169"),
		MTK_FUNCTION(1, "JTRSTN_SEL1_VCORE"),
		MTK_FUNCTION(6, "SPM_JTAG_TRSTN_VCORE")
	),
	MTK_PIN_NODRV(
		170, "GPIO170",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO170"),
		MTK_FUNCTION(1, "JTDO_SEL1_VCORE"),
		MTK_FUNCTION(6, "SPM_JTAG_TDO_VCORE")
	),
	MTK_PIN_NODRV(
		171, "GPIO171",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO171"),
		MTK_FUNCTION(1, "JTCK_SEL1_VCORE"),
		MTK_FUNCTION(6, "SPM_JTAG_TCK_VCORE")
	),
	MTK_PIN_NODRV(
		172, "GPIO172",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO172"),
		MTK_FUNCTION(1, "JTMS_SEL1_VCORE"),
		MTK_FUNCTION(6, "SPM_JTAG_TMS_VCORE")
	),
	MTK_PIN_NODRV(
		173, "GPIO173",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO173"),
		MTK_FUNCTION(1, "JTDI_SEL1_VCORE"),
		MTK_FUNCTION(6, "SPM_JTAG_TDI_VCORE")
	),
	MTK_PIN_NODRV(
		174, "GPIO174",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO174"),
		MTK_FUNCTION(1, "MD1_SIM1_SCLK"),
		MTK_FUNCTION(6, "TSFDC_26M")
	),
	MTK_PIN_NODRV(
		175, "GPIO175",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO175"),
		MTK_FUNCTION(1, "MD1_SIM1_SRST"),
		MTK_FUNCTION(2, "HFRP_JTAG0_TCK"),
		MTK_FUNCTION(3, "MCUPM_JTAG_TCK"),
		MTK_FUNCTION(4, "APU_JTAG_TCK"),
		MTK_FUNCTION(5, "CONN_DSP_L5_JCK"),
		MTK_FUNCTION(6, "TSFDC_SDO")
	),
	MTK_PIN_NODRV(
		176, "GPIO176",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO176"),
		MTK_FUNCTION(1, "MD1_SIM1_SIO"),
		MTK_FUNCTION(2, "HFRP_JTAG0_TRSTN"),
		MTK_FUNCTION(3, "MCUPM_JTAG_TRSTN"),
		MTK_FUNCTION(4, "APU_JTAG_TRST"),
		MTK_FUNCTION(5, "CONN_DSP_L5_JINTP"),
		MTK_FUNCTION(6, "TSFDC_FOUT")
	),
	MTK_PIN_NODRV(
		177, "GPIO177",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO177"),
		MTK_FUNCTION(1, "MD1_SIM2_SCLK"),
		MTK_FUNCTION(2, "HFRP_JTAG0_TDI"),
		MTK_FUNCTION(3, "MCUPM_JTAG_TDI"),
		MTK_FUNCTION(4, "APU_JTAG_TDI"),
		MTK_FUNCTION(5, "CONN_DSP_L5_JDI"),
		MTK_FUNCTION(6, "TSFDC_SCK")
	),
	MTK_PIN_NODRV(
		178, "GPIO178",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO178"),
		MTK_FUNCTION(1, "MD1_SIM2_SRST"),
		MTK_FUNCTION(2, "HFRP_JTAG0_TMS"),
		MTK_FUNCTION(3, "MCUPM_JTAG_TMS"),
		MTK_FUNCTION(4, "APU_JTAG_TMS"),
		MTK_FUNCTION(5, "CONN_DSP_L5_JMS"),
		MTK_FUNCTION(6, "TSFDC_SDI")
	),
	MTK_PIN_NODRV(
		179, "GPIO179",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO179"),
		MTK_FUNCTION(1, "MD1_SIM2_SIO"),
		MTK_FUNCTION(2, "HFRP_JTAG0_TDO"),
		MTK_FUNCTION(3, "MCUPM_JTAG_TDO"),
		MTK_FUNCTION(4, "APU_JTAG_TDO"),
		MTK_FUNCTION(5, "CONN_DSP_L5_JDO"),
		MTK_FUNCTION(6, "TSFDC_SCF")
	),
	MTK_PIN_NODRV(
		180, "GPIO180",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO180"),
		MTK_FUNCTION(1, "SPMI_M_SCL")
	),
	MTK_PIN_NODRV(
		181, "GPIO181",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO181"),
		MTK_FUNCTION(1, "SPMI_M_SDA")
	),
	MTK_PIN_NODRV(
		182, "GPIO182",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO182"),
		MTK_FUNCTION(1, "SPMI_P_SCL")
	),
	MTK_PIN_NODRV(
		183, "GPIO183",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO183"),
		MTK_FUNCTION(1, "SPMI_P_SDA")
	),
	MTK_PIN_NODRV(
		184, "GPIO184",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO184"),
		MTK_FUNCTION(1, "SRCLKENA0")
	),
	MTK_PIN_NODRV(
		185, "GPIO185",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO185"),
		MTK_FUNCTION(1, "SCP_VREQ_VAO")
	),
	MTK_PIN_NODRV(
		186, "GPIO186",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO186"),
		MTK_FUNCTION(1, "WATCHDOG")
	),
	MTK_PIN_NODRV(
		187, "GPIO187",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO187"),
		MTK_FUNCTION(1, "RTC32K_CK")
	),
	MTK_PIN_NODRV(
		188, "GPIO188",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO188"),
		MTK_FUNCTION(1, "AUD_CLK_MOSI"),
		MTK_FUNCTION(3, "AUD_CLK_MOSI")
	),
	MTK_PIN_NODRV(
		189, "GPIO189",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO189"),
		MTK_FUNCTION(1, "AUD_SYNC_MOSI"),
		MTK_FUNCTION(4, "I2SIN_DMA0_BCK"),
		MTK_FUNCTION(7, "DBG_MON_B25")
	),
	MTK_PIN_NODRV(
		190, "GPIO190",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO190"),
		MTK_FUNCTION(1, "AUD_DAT_MOSI0"),
		MTK_FUNCTION(3, "AUD_DAT_MOSI0"),
		MTK_FUNCTION(7, "DBG_MON_B26")
	),
	MTK_PIN_NODRV(
		191, "GPIO191",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO191"),
		MTK_FUNCTION(1, "AUD_DAT_MOSI1"),
		MTK_FUNCTION(3, "AUD_DAT_MOSI1"),
		MTK_FUNCTION(7, "DBG_MON_B27")
	),
	MTK_PIN_NODRV(
		192, "GPIO192",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO192"),
		MTK_FUNCTION(1, "AUD_DAT_MOSI2"),
		MTK_FUNCTION(4, "I2SIN_DMA0_LRCK"),
		MTK_FUNCTION(7, "DBG_MON_B28")
	),
	MTK_PIN_NODRV(
		193, "GPIO193",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO193"),
		MTK_FUNCTION(1, "AUD_NLE_MOSI0"),
		MTK_FUNCTION(2, "AUD_SYNC_MISO"),
		MTK_FUNCTION(4, "I2SIN_DMA0_DI")
	),
	MTK_PIN_NODRV(
		194, "GPIO194",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO194"),
		MTK_FUNCTION(1, "AUD_NLE_MOSI1"),
		MTK_FUNCTION(2, "AUD_CLK_MISO"),
		MTK_FUNCTION(4, "I2SIN_DMA0_DI1")
	),
	MTK_PIN_NODRV(
		195, "GPIO195",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO195"),
		MTK_FUNCTION(1, "AUD_DAT_MISO0"),
		MTK_FUNCTION(2, "VOW_DAT_MISO"),
		MTK_FUNCTION(3, "AUD_DAT_MISO0"),
		MTK_FUNCTION(7, "DBG_MON_B29")
	),
	MTK_PIN_NODRV(
		196, "GPIO196",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO196"),
		MTK_FUNCTION(1, "AUD_DAT_MISO1"),
		MTK_FUNCTION(2, "VOW_CLK_MISO"),
		MTK_FUNCTION(3, "AUD_CLK_MISO"),
		MTK_FUNCTION(4, "I2SIN_DMA0_DI2"),
		MTK_FUNCTION(7, "DBG_MON_B30")
	),
	MTK_PIN_NODRV(
		197, "GPIO197",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO197"),
		MTK_FUNCTION(1, "AUD_DAT_MISO2"),
		MTK_FUNCTION(3, "AUD_DAT_MISO1"),
		MTK_FUNCTION(4, "I2SIN_DMA0_DI3"),
		MTK_FUNCTION(7, "DBG_MON_B31")
	),
	MTK_PIN_NODRV(
		198, "GPIO198",
		MTK_EINT_FUNCTION(NO_EINT_SUPPORT, NO_EINT_SUPPORT),
		MTK_FUNCTION(0, "GPIO198"),
		MTK_FUNCTION(1, "MCU_PMIC_POC_I")
	),
};

#endif /* __PINCTRL_MTK_MT6881_H */
