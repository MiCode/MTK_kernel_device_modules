		   ...
		   size_t size)
{
	...
	if (IS_ERR(shash)) {
		rc = PTR_ERR(shash);
		pr_notice("shash(sha1) allocation failed\n");
		return rc;
	}
...
err:
	pr_notice("could not register %s driver\n",
...
int tee_device_register(struct tee_device *teedev)
{
	...
	if (teedev->flags & TEE_DEVICE_FLAG_REGISTERED) {
		dev_notice(&teedev->dev, "attempt to register twice\n");
		return -EINVAL;
	}
...
static int __init tee_init(void)
{
	...
	if (IS_ERR(tee_class)) {
		pr_notice("couldn't create class\n");
		return PTR_ERR(tee_class);
	}

	rc = alloc_chrdev_region(&tee_devt, 0, TEE_NUM_DEVICES, "tee");
	if (rc) {
		pr_notice("failed to allocate char dev region\n");
		goto out_unreg_class;
	}

	rc = bus_register(&tee_bus_type);
	if (rc) {
		pr_notice("failed to register tee bus\n");
