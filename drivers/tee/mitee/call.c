...
static int auth_ca_name(void)
{
	...
	/* match one white list */
	if(index < ARRAY_SIZE(white_list))
		return 0;

	pr_err("fail to auth ca path: %s\n", ca_path);
	return -1;
}
		       ...
		       struct tee_param *param)
{
	...
	if (DEBUG_CALL)
		pr_info("%s: UUID: 0x%x%x%x%x-0x%x%x-0x%x%x-0x%x%x%x%x%x%x%x%x\n",
			__func__, arg->uuid[0], arg->uuid[1], arg->uuid[2],
	...
	if (msg_arg->ret == TEEC_SUCCESS) {
		...
		pr_warn("%s failed, msg_arg->ret: %#x\n", __func__,
			msg_arg->ret);
		kfree(sess);
	}
