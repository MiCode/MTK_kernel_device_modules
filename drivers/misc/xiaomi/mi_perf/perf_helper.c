			...
			size_t count, loff_t *data)
{
	...
	if (pKcompactd)
	{
		if (set_cpus_allowed_ptr(pKcompactd, &mask))
			printk(KERN_ERR "bind kcompactd fail\n");
	}
...
static int __init perf_helper_init(void)
{
	...
	if (!kswapd_affinity_entry)
		printk(KERN_ERR "create kswapd_affinity node failed\n");
