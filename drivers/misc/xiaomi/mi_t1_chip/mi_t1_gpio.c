...
static int MI_T1_GPIO_COUNT = 3;//default for N12
...
struct mi_t1_gpio_data {
	...
	struct pinctrl *pinctrl; //setting all gpio default value
	struct pinctrl_state *gpio_default_cfg;
...
#define MI_T1_IOCTL_GPIO_ANT4_VC1_SET		_IOWR(MI_T1_IOCTL_MAGIC, 0, int)
#define MI_T1_IOCTL_GPIO_ANT4_VC2_SET		_IOWR(MI_T1_IOCTL_MAGIC, 1, int)
#define MI_T1_IOCTL_GPIO_ANT4_VC3_SET		_IOWR(MI_T1_IOCTL_MAGIC, 2, int)

#define MI_T1_IOCTL_GPIO_ANT4_VC1_GET		_IOWR(MI_T1_IOCTL_MAGIC, 3, int)
#define MI_T1_IOCTL_GPIO_ANT4_VC2_GET		_IOWR(MI_T1_IOCTL_MAGIC, 4, int)
#define MI_T1_IOCTL_GPIO_ANT4_VC3_GET		_IOWR(MI_T1_IOCTL_MAGIC, 5, int)
...
static ssize_t gpio_mi_t1_set_general(struct device *device, int gpio_type, const char *buf, size_t count)
{
	...
	{
		rc = -EINVAL;
		dev_err(device, "mi_t1 %s: invalid input %c! only 0 or 1 is valid.\n", __func__, *buf);
	}

unlock_ret:
	mutex_unlock(&mi_t1_data->lock);
	dev_info(device, "%s, gpio_type [%d] set, rc [%d], buf = %c\n", __func__, gpio_type, rc, *buf);
	return count;
}
...
static ssize_t gpio_mi_t1_get_ant4_vc1(struct device *device,
		struct device_attribute *attr, char *buf)
{
	return gpio_mi_t1_get_general(device, MI_T1_GPIO_ANT4_VC1, buf);
}

static ssize_t gpio_mi_t1_set_ant4_vc1(struct device *device,
                              struct device_attribute *attr,
                              const char *buf, size_t count)
{
	return gpio_mi_t1_set_general(device, MI_T1_GPIO_ANT4_VC1, buf, count);
}

static DEVICE_ATTR(ant4_vc1, MI_T1_MODE_MSK, gpio_mi_t1_get_ant4_vc1, gpio_mi_t1_set_ant4_vc1);


static ssize_t gpio_mi_t1_get_ant4_vc2(struct device *device,
		struct device_attribute *attr, char *buf)
{
	return gpio_mi_t1_get_general(device, MI_T1_GPIO_ANT4_VC2, buf);
}

static ssize_t gpio_mi_t1_set_ant4_vc2(struct device *device,
                              struct device_attribute *attr,
                              const char *buf, size_t count)
{
	return gpio_mi_t1_set_general(device, MI_T1_GPIO_ANT4_VC2, buf, count);
}

static DEVICE_ATTR(ant4_vc2, MI_T1_MODE_MSK, gpio_mi_t1_get_ant4_vc2, gpio_mi_t1_set_ant4_vc2);


static ssize_t gpio_mi_t1_get_ant4_vc3(struct device *device,
		struct device_attribute *attr, char *buf)
{
	return gpio_mi_t1_get_general(device, MI_T1_GPIO_ANT4_VC3, buf);
}

static ssize_t gpio_mi_t1_set_ant4_vc3(struct device *device,
                              struct device_attribute *attr,
                              const char *buf, size_t count)
{
	return gpio_mi_t1_set_general(device, MI_T1_GPIO_ANT4_VC3, buf, count);
}

static DEVICE_ATTR(ant4_vc3, MI_T1_MODE_MSK, gpio_mi_t1_get_ant4_vc3, gpio_mi_t1_set_ant4_vc3);


static struct attribute *attributes[] = {
	&dev_attr_ant4_vc1.attr,
	&dev_attr_ant4_vc2.attr,
	&dev_attr_ant4_vc3.attr,
	NULL
};
				...
				unsigned long __user ioctl_param)
{
	...
	case MI_T1_IOCTL_GPIO_ANT4_VC1_SET:
		pr_info("mi_t1 MI_T1_IOCTL_GPIO_ANT4_VC1_SET !\n");
		get_user(gpio_value, p);
		if(!(mi_t1_data->data))
			break;
		gpio_num = mi_t1_data->data[MI_T1_GPIO_ANT4_VC1].gpio_num;
	...
	case MI_T1_IOCTL_GPIO_ANT4_VC2_SET:
		pr_info("mi_t1 MI_T1_IOCTL_GPIO_ANT4_VC2_SET !\n");
		get_user(gpio_value, p);
		if(!(mi_t1_data->data))
			break;
		gpio_num = mi_t1_data->data[MI_T1_GPIO_ANT4_VC2].gpio_num;
		if (gpio_is_valid(gpio_num)) {
			ret = gpio_direction_output(gpio_num, gpio_value);
			if (ret) {
				pr_info("mi_t1 %s: fail to set gpio %d !\n", __func__, gpio_num);
	...
	case MI_T1_IOCTL_GPIO_ANT4_VC3_SET:
		pr_info("mi_t1 MI_T1_IOCTL_GPIO_ANT4_VC3_SET !\n");
		get_user(gpio_value, p);
		if(!(mi_t1_data->data))
			break;
		gpio_num = mi_t1_data->data[MI_T1_GPIO_ANT4_VC3].gpio_num;
		if (gpio_is_valid(gpio_num)) {
			ret = gpio_direction_output(gpio_num, gpio_value);
			if (ret) {
				pr_info("mi_t1 %s: fail to set gpio %d !\n", __func__, gpio_num);
	...
	case MI_T1_IOCTL_GPIO_ANT4_VC1_GET:
		pr_info("mi_t1 MI_T1_IOCTL_GPIO_ANT4_VC1_GET !\n");
		if(!(mi_t1_data->data))
			break;
		gpio_num = mi_t1_data->data[MI_T1_GPIO_ANT4_VC1].gpio_num;
		if (gpio_is_valid(gpio_num)) {
	...
	case MI_T1_IOCTL_GPIO_ANT4_VC2_GET:
		pr_info("mi_t1 MI_T1_IOCTL_GPIO_ANT4_VC2_GET !\n");
		if(!(mi_t1_data->data))
			break;
		gpio_num = mi_t1_data->data[MI_T1_GPIO_ANT4_VC2].gpio_num;
		if (gpio_is_valid(gpio_num)) {
	...
	case MI_T1_IOCTL_GPIO_ANT4_VC3_GET:
		pr_info("mi_t1 MI_T1_IOCTL_GPIO_ANT4_VC3_GET !\n");
		if(!(mi_t1_data->data))
			break;
		gpio_num = mi_t1_data->data[MI_T1_GPIO_ANT4_VC3].gpio_num;
		if (gpio_is_valid(gpio_num)) {
...
static void gpio_mi_t1_set_all_default_value(struct device *dev, 
	struct mi_t1_gpio_data * mi_t1_data){
	
	int ret = 0;
	
	mi_t1_data->pinctrl = devm_pinctrl_get(dev);
	if (IS_ERR(mi_t1_data->pinctrl)) {
		ret = PTR_ERR(mi_t1_data->pinctrl);
		pr_err("failed to get gpio_mi_t1 pinctrl, ret=%d\n", ret);
		return ;
	}
	
	mi_t1_data->gpio_default_cfg = pinctrl_lookup_state(mi_t1_data->pinctrl,"t1_gpio_vc_all");
	if (IS_ERR(mi_t1_data->gpio_default_cfg)) {
		ret = PTR_ERR(mi_t1_data->gpio_default_cfg);
		pr_err("failed to get gpio_mi_t1 default state, ret=%d\n",ret);
		return ;
	}
	
	pinctrl_select_state(mi_t1_data->pinctrl, mi_t1_data->gpio_default_cfg);
	pr_info("%s success. \n", __func__);
...
static int gpio_mi_t1_probe(struct platform_device *pdev)
{
	...
	gpio_mi_t1_set_all_default_value(dev, mi_t1_data);
...
static void gpio_mi_t1_remove(struct platform_device *pdev)
{
