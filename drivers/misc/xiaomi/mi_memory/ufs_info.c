...
{
	struct ufs_dev_info *dev_info = &g_ufs_info->hba->dev_info;
	if (dev_info->wmanufacturerid == UFS_VENDOR_MICRON)
		look_up_scsi_device(SCSI_LUN_MICRON);
	else
		look_up_scsi_device(SCSI_LUN);
...
int ufs_get_string_desc(struct ufs_hba *hba, u8 **buf, enum device_desc_param pname, bool ascii_std)
{
	u8 index;
	int ret = 0;
...
struct ufs_info_t *init_ufs_info(void)
{
	...
	ufs_get_string_desc(g_ufs_info->hba, &g_ufs_info->ufs_name, DEVICE_DESC_PARAM_PRDCT_NAME, SD_ASCII_STD);
	ufs_get_string_desc(g_ufs_info->hba, &g_ufs_info->ufs_fwver, DEVICE_DESC_PARAM_PRDCT_REV, SD_ASCII_STD);
...
static int ufshcd_read_desc(struct ufs_hba *hba, enum desc_idn desc_id, int desc_index, void *buf, u8 size)
{
	int ret = 0;
...
static ssize_t dump_string_desc_serial_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	ret = ufs_get_string_desc(g_ufs_info->hba, &serial_number, DEVICE_DESC_PARAM_SN, SD_RAW);
	...
	if (serial_number) {
		...
		for (i = 2; i <  serial_number[QUERY_DESC_LENGTH_OFFSET]; i += 2) {
			count += snprintf((buf + count), PAGE_SIZE, "%02x%02x", serial_number[i], serial_number[i+1]);
		}
...
static ssize_t dump_device_desc_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	err = ufs_read_desc_param(g_ufs_info->hba, QUERY_DESC_IDN_DEVICE, 0, DEVICE_DESC_PARAM_LEN, &buff_len, BYTE);

	if (err) {
		dev_err(g_ufs_info->hba->dev, "%s ufs_read_desc_param failed, err = %d\n", __func__, err);
	...
	if (err) {
		dev_err(g_ufs_info->hba->dev, "%s ufs_read_desc failed, err = %d\n", __func__, err);
		count += snprintf((buf + count), PAGE_SIZE, "ufshcd_read_desc fail, err = %d\n", err);
		return count;
	}
...
static ssize_t err_state_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (g_ufs_info)
		ret = snprintf(buf, PAGE_SIZE, "%llu\n", p_ufs_xiaomi->err_state.err_occurred);
...
static ssize_t err_reason_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (g_ufs_info) {
		ret = snprintf(buf, PAGE_SIZE, "%s%s%s%s%s%s%s%s%s%s",
									p_ufs_xiaomi->err_state.err_reason[0],
									p_ufs_xiaomi->err_state.err_reason[1],
									p_ufs_xiaomi->err_state.err_reason[2],
									p_ufs_xiaomi->err_state.err_reason[3],
									p_ufs_xiaomi->err_state.err_reason[4],
									p_ufs_xiaomi->err_state.err_reason[5],
									p_ufs_xiaomi->err_state.err_reason[6],
									p_ufs_xiaomi->err_state.err_reason[7],
									p_ufs_xiaomi->err_state.err_reason[8],
									p_ufs_xiaomi->err_state.err_reason[9]);
	}
	return ret;
}
