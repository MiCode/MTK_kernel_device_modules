...
static int mtkts_usb_debug_log;
...
#define MTKTS_USB_TEMP_CRIT 60000	/* 60.000 degree Celsius */

#define mtkts_usb_dprintk(fmt, args...)   \
do {                                    \
	if (mtkts_usb_debug_log) {                \
		pr_notice("[Thermal/TZ/USB]" fmt, ##args); \
	}                                   \
} while (0)


#define mtkts_usb_printk(fmt, args...) \
pr_debug("[Thermal/TZ/USB]" fmt, ##args)
...
/* USB_TEMPERATURE USB_Temperature_Table[] = {0}; */
...
static __s32 mtkts_usb_thermistor_conver_temp(__s32 Res)
{
	int i = 0;
	int asize = 0;
	__s32 RES1 = 0, RES2 = 0;
	__s32 TAP_Value = -200, TMP1 = 0, TMP2 = 0;
#ifdef APPLY_PRECISE_USB_TEMP
	TAP_Value = TAP_Value * 1000;
#endif
	asize = (ntc_tbl_size / sizeof(struct USB_TEMPERATURE));

	/* mtkts_usb_dprintk("mtkts_usb_thermistor_conver_temp() :
	 * asize = %d, Res = %d\n",asize,Res);
	 */
	if (Res >= USB_Temperature_Table[0].TemperatureR) {
		...
		/* mtkts_usb_dprintk("%d : RES1 = %d,TMP1 = %d\n",
		 * __LINE__,RES1,TMP1);
		 */

		for (i = 0; i < asize; i++) {
			if (Res >= USB_Temperature_Table[i].TemperatureR) {
				RES2 = USB_Temperature_Table[i].TemperatureR;
				TMP2 = USB_Temperature_Table[i].USB_Temp;
				/* mtkts_usb_dprintk("%d :i=%d, RES2 = %d,
			...
			/* mtkts_usb_dprintk("%d :i=%d, RES1 = %d,TMP1 = %d\n",
...
static int get_hw_usb_temp(void)
{
	...
	if (ret < 0) {
		mtkts_usb_printk("Busy/Timeout, IIO ch read failed %d\n", ret);
		return ret;
	}
	...
	mtkts_usb_dprintk("APtery output mV = %d\n", ret);
	output = mtk_ts_usb_volt_to_temp(ret);
	mtkts_usb_dprintk("USB output temperature = %d\n", output);
	return output;
}

static DEFINE_MUTEX(USB_lock);
/*int ts_AP_at_boot_time = 0;*/
int mtkts_usb_get_hw_temp(void)
{
	int t_ret = 0;
	int t_ret2 = 0;
	...
	t_ret = get_hw_usb_temp();
#ifndef APPLY_PRECISE_USB_TEMP
	t_ret = t_ret * 1000;
#endif
	...
	if (t_ret2 < 0)
		pr_notice("[Thermal/TZ/USB]wakeup_ta_algo out of memory\n");

	usb_cur_temp = t_ret;

	if (t_ret > 40000)	/* abnormal high temp */
		mtkts_usb_printk("T_AP=%d\n", t_ret);

	mtkts_usb_dprintk("[%s] T_AP, %d\n", __func__, t_ret);
	return t_ret;
}

static int mtkts_usb_get_temp(struct thermal_zone_device *thermal, int *t)
{
	*t = mtkts_usb_get_hw_temp();

	if ((int) *t > 40000)
	    /* mtkts_usb_dprintk("T=%d\n", (int) *t); */
	    pr_notice("[Thermal/TZ/USB] %s USB NTC Temp=%d\n", __func__, (int) *t);
...
struct thermal_zone_device *thermal, struct thermal_cooling_device *cdev)
{
	...
	mtkts_usb_dprintk("[%s] binding OK, %d\n", __func__, table_val);
	return 0;
}

static int mtkts_usb_unbind(struct thermal_zone_device *thermal,
			    struct thermal_cooling_device *cdev)
{
	...
	mtkts_usb_dprintk("[%s] unbinding OK\n", __func__);
	return 0;
}


static int mtkts_usb_change_mode(
...
static int mtkts_usb_get_crit_temp(
struct thermal_zone_device *thermal, int *temperature)
{
	*temperature = MTKTS_USB_TEMP_CRIT;
	return 0;
}

/* bind callback functions to thermalzone */
static struct thermal_zone_device_ops mtkts_USB_dev_ops = {
	.bind = mtkts_usb_bind,
	.unbind = mtkts_usb_unbind,
	.get_temp = mtkts_usb_get_temp,
	.change_mode = mtkts_usb_change_mode,
	.get_crit_temp = mtkts_usb_get_crit_temp,
};



static int mtkts_usb_read(struct seq_file *m, void *v)
{
...
static int mtkts_usb_register_thermal(void);
static void mtkts_usb_unregister_thermal(void);

static ssize_t mtkts_usb_write(
struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
	...
	if (sscanf
		...
		down(&sem_mutex);
		mtkts_usb_dprintk(
			"[%s] mtkts_usb_unregister_thermal\n", __func__);

		mtkts_usb_unregister_thermal();

		if (num_trip < 0 || num_trip > 10) {
			#if IS_ENABLED(CONFIG_MTK_AEE_FEATURE)
			aee_kernel_warning_api(__FILE__, __LINE__,
					DB_OPT_DEFAULT, "mtkts_usb_write",
					"Bad argument");
			#endif
			mtkts_usb_dprintk("[%s] bad argument\n", __func__);
			kfree(ptr_mtktsusb_data);
			up(&sem_mutex);
			return -EINVAL;
		}
		...
		mtkts_usb_dprintk("trip_9_temp=%d,time_ms=%d\n",
			trip_temp[9], interval * 1000);

		mtkts_usb_dprintk(
			"[%s] mtkts_usb_register_thermal\n", __func__);
...
struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
	...
	ptr_mtktsusb_parm_data = kmalloc(
				sizeof(*ptr_mtktsusb_parm_data), GFP_KERNEL);
	...
	len = (count < (sizeof(ptr_mtktsusb_parm_data->desc) - 1)) ?
			count : (sizeof(ptr_mtktsusb_parm_data->desc) - 1);
	...
	if (sscanf
	    (ptr_mtktsusb_parm_data->desc, "%9s %d %9s %d %15s %d %9s %d %d",
		ptr_mtktsusb_parm_data->pull_R, &ptr_mtktsusb_parm_data->valR,
		ptr_mtktsusb_parm_data->pull_V, &ptr_mtktsusb_parm_data->valV,
		...
		if (!strcmp(ptr_mtktsusb_parm_data->pull_R, "PUP_R")) {
			...
			mtkts_usb_dprintk("g_RAP_pull_up_R=%d\n",
							g_RAP_pull_up_R);
		} else {
			mtkts_usb_printk(
		...
		if (!strcmp(ptr_mtktsusb_parm_data->pull_V, "PUP_VOLT")) {
			g_RAP_pull_up_voltage = ptr_mtktsusb_parm_data->valV;
			mtkts_usb_dprintk("g_Rat_pull_up_voltage=%d\n",
							g_RAP_pull_up_voltage);
		} else {
			mtkts_usb_printk(
		...
		if (!strcmp(ptr_mtktsusb_parm_data->overcrilow,
			...
			g_TAP_over_critical_low =
					ptr_mtktsusb_parm_data->over_cri_low;
			mtkts_usb_dprintk("g_TAP_over_critical_low=%d\n",
						g_TAP_over_critical_low);
		} else {
			mtkts_usb_printk(
		...
		if (!strcmp(ptr_mtktsusb_parm_data->NTC_TABLE, "NTC_TABLE")) {
			g_RAP_ntc_table = ptr_mtktsusb_parm_data->ntc_table;
			mtkts_usb_dprintk("g_RAP_ntc_table=%d\n",
							g_RAP_ntc_table);
		} else {
			mtkts_usb_printk(
		...
		mtkts_usb_dprintk("adc_channel=%d\n",
					ptr_mtktsusb_parm_data->adc_channel);
		mtkts_usb_dprintk("g_RAP_ADC_channel=%d\n",
						g_RAP_ADC_channel);
	...
	mtkts_usb_printk("[%s] bad argument\n", __func__);
	kfree(ptr_mtktsusb_parm_data);
	return -EINVAL;
}
...
static int mtkts_usb_register_thermal(void)
{
	...
	/* trips : trip 0~1 */
	thz_dev = mtk_thermal_zone_device_register("mtktsusb",
						trips,
						num_trip,
						NULL,
						&mtkts_USB_dev_ops, 0, 0, 0,
						interval * 1000);
...
static int mtkts_usb_open(struct inode *inode, struct file *file)
{
	return single_open(file, mtkts_usb_read, NULL);
}

static const struct proc_ops mtkts_AP_fops = {
	.proc_open = mtkts_usb_open,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_write = mtkts_usb_write,
...
static const struct proc_ops mtkts_AP_param_fops = {
	.proc_open = mtkts_usb_param_open,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_write = mtkts_usb_param_write,
	.proc_release = single_release,
};

#if IS_ENABLED(CONFIG_DEVICE_MODULES_MEDIATEK_MT6577_AUXADC)
static int mtkts_usb_probe(struct platform_device *pdev)
{
	int err = 0;
	int ret = 0;
	...
	pr_notice("[Thermal/TZ/USB] %s line=%d\n", __func__, __LINE__);

	if (!pdev->dev.of_node) {
		mtkts_usb_printk("[%s] Only DT based supported\n",
			__func__);
		return -ENODEV;
	}
	...
	if (ret) {
		mtkts_usb_printk("[%s] fail to get auxadc iio ch0: %d\n",
			__func__, ret);
		return ret;
	}
	...
	if (!mtkts_AP_dir) {
		...
		entry = proc_create("tzusb", 0664, mtkts_AP_dir,
							&mtkts_AP_fops);
		if (entry)
			proc_set_user(entry, uid, gid);

	entry = proc_create("tzusb_param", 0664, mtkts_AP_dir,
				&mtkts_AP_param_fops);
		if (entry)
			proc_set_user(entry, uid, gid);
...
static struct platform_driver mtk_thermal_usb_driver = {
	.remove = NULL,
	.shutdown = NULL,
	.probe = mtkts_usb_probe,
	.suspend = NULL,
	.resume = NULL,
	.driver = {
...
int mtkts_usb_init(void)
{
...
#if IS_ENABLED(CONFIG_DEVICE_MODULES_MEDIATEK_MT6577_AUXADC)
	err = platform_driver_register(&mtk_thermal_usb_driver);
	if (err) {
		mtkts_usb_printk("thermal driver callback register failed.\n");
		return err;
	}
#else
	/* setup default table */
	...
	mtkts_AP_dir = mtk_thermal_get_proc_drv_therm_dir_entry();
	if (!mtkts_AP_dir) {
		mtkts_usb_dprintk("[%s]: mkdir /proc/driver/thermal failed\n",
								__func__);
	} else {
		entry = proc_create("tzusb", 0664, mtkts_AP_dir,
							&mtkts_AP_fops);
...
//module_init(mtkts_usb_init);
//module_exit(mtkts_usb_exit);
MODULE_LICENSE("GPL");
