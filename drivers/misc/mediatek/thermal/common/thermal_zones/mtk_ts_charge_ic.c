...
static int mtkts_chargeic_debug_log;
...
int chargeic_cur_temp = 1;


#define MTKTS_CHARGEIC_TEMP_CRIT 60000	/* 60.000 degree Celsius */

#define mtkts_chargeic_dprintk(fmt, args...)   \
do {                                    \
	if (mtkts_chargeic_debug_log) {                \
		pr_notice("[Thermal/TZ/CHARGEIC]" fmt, ##args); \
	}                                   \
} while (0)


#define mtkts_chargeic_printk(fmt, args...) \
pr_debug("[Thermal/TZ/CHARGEIC]" fmt, ##args)
...
static int g_TAP_over_critical_low = CHARGEIC_TAP_OVER_CRITICAL_LOW;
static int g_RAP_pull_up_voltage = CHARGEIC_RAP_PULL_UP_VOLTAGE;
static int g_RAP_ntc_table = CHARGEIC_RAP_NTC_TABLE;
static int g_RAP_ADC_channel = CHARGEIC_RAP_ADC_CHANNEL;
static int g_AP_TemperatureR;
/* CHARGEIC_TEMPERATURE CHARGEIC_Temperature_Table[] = {0}; */

static struct CHARGEIC_TEMPERATURE *CHARGEIC_Temperature_Table;
static int ntc_tbl_size;

/* AP_NTC_BL197 */
static struct CHARGEIC_TEMPERATURE CHARGEIC_Temperature_Table1[] = {
	{-40, 74354},		/* FIX_ME */
	{-35, 74354},		/* FIX_ME */
	{-30, 74354},		/* FIX_ME */
	{-25, 74354},		/* FIX_ME */
...
/* AP_NTC_TSM_1 */
static struct CHARGEIC_TEMPERATURE CHARGEIC_Temperature_Table2[] = {
	{-40, 70603},		/* FIX_ME */
	{-35, 70603},		/* FIX_ME */
	{-30, 70603},		/* FIX_ME */
	{-25, 70603},		/* FIX_ME */
...
/* AP_NTC_10_SEN_1 */
static struct CHARGEIC_TEMPERATURE CHARGEIC_Temperature_Table3[] = {
	{-40, 74354},		/* FIX_ME */
	{-35, 74354},		/* FIX_ME */
	{-30, 74354},		/* FIX_ME */
	{-25, 74354},		/* FIX_ME */
...
static struct CHARGEIC_TEMPERATURE CHARGEIC_Temperature_Table4[] = {
	{-40, 188500},
	{-35, 144290},
	{-30, 111330},
	{-25, 86560},
...
/* AP_NTC_47 */
static struct CHARGEIC_TEMPERATURE CHARGEIC_Temperature_Table5[] = {
	{-40, 483954},		/* FIX_ME */
	{-35, 483954},		/* FIX_ME */
	{-30, 483954},		/* FIX_ME */
	{-25, 483954},		/* FIX_ME */
...
/* NTCG104EF104F(100K) */
static struct CHARGEIC_TEMPERATURE CHARGEIC_Temperature_Table6[] = {
	{-40, 4251000},
	{-35, 3005000},
	{-30, 2149000},
	{-25, 1554000},
...
static struct CHARGEIC_TEMPERATURE CHARGEIC_Temperature_Table7[] = {
	{-40, 4397119},
	{-35, 3088599},
	{-30, 2197225},
	{-25, 1581881},
...
static __s32 mtkts_chargeic_thermistor_conver_temp(__s32 Res)
{
	...
	/* mtkts_chargeic_dprintk("mtkts_chargeic_thermistor_conver_temp() :
	 * asize = %d, Res = %d\n",asize,Res);
	 */
	if (Res >= CHARGEIC_Temperature_Table[0].TemperatureR) {
		TAP_Value = -40;	/* min */
#ifdef APPLY_PRECISE_CHARGEIC_TEMP
		TAP_Value = TAP_Value * 1000;
#endif
	} else if (Res <= CHARGEIC_Temperature_Table[asize - 1].TemperatureR) {
		TAP_Value = 125;	/* max */
#ifdef APPLY_PRECISE_CHARGEIC_TEMP
		TAP_Value = TAP_Value * 1000;
#endif
	} else {
		RES1 = CHARGEIC_Temperature_Table[0].TemperatureR;
		TMP1 = CHARGEIC_Temperature_Table[0].CHARGEIC_Temp;
		/* mtkts_chargeic_dprintk("%d : RES1 = %d,TMP1 = %d\n",
		 * __LINE__,RES1,TMP1);
		 */

		for (i = 0; i < asize; i++) {
			if (Res >= CHARGEIC_Temperature_Table[i].TemperatureR) {
				RES2 = CHARGEIC_Temperature_Table[i].TemperatureR;
				TMP2 = CHARGEIC_Temperature_Table[i].CHARGEIC_Temp;
				/* mtkts_chargeic_dprintk("%d :i=%d, RES2 = %d,
				 * TMP2 = %d\n",__LINE__,i,RES2,TMP2);
				 */
				break;
			}
			RES1 = CHARGEIC_Temperature_Table[i].TemperatureR;
			TMP1 = CHARGEIC_Temperature_Table[i].CHARGEIC_Temp;
			/* mtkts_chargeic_dprintk("%d :i=%d, RES1 = %d,TMP1 = %d\n",
...
static int mtkts_chargeic_get_temp(struct thermal_zone_device *thermal, int *t)
{
	*t = mtkts_chargeic_get_hw_temp();
	...
	if ((int) *t > 40000)
	    /* mtkts_chargeic_dprintk("T=%d\n", (int) *t); */
	    pr_notice("[Thermal/TZ/CHARGEIC] %s charge_ic NTC Temp=%d\n", __func__, (int) *t);
...
static int mtkts_chargeic_bind(
struct thermal_zone_device *thermal, struct thermal_cooling_device *cdev)
{
	...
	mtkts_chargeic_dprintk("[%s] binding OK, %d\n", __func__, table_val);
	return 0;
}

static int mtkts_chargeic_unbind(struct thermal_zone_device *thermal,
			    struct thermal_cooling_device *cdev)
{
	...
	mtkts_chargeic_dprintk("[%s] unbinding OK\n", __func__);
	return 0;
}


static int mtkts_chargeic_change_mode(
...
static int mtkts_chargeic_get_crit_temp(
struct thermal_zone_device *thermal, int *temperature)
{
	*temperature = MTKTS_CHARGEIC_TEMP_CRIT;
	return 0;
}

/* bind callback functions to thermalzone */
static struct thermal_zone_device_ops mtkts_CHARGEIC_dev_ops = {
	.bind = mtkts_chargeic_bind,
	.unbind = mtkts_chargeic_unbind,
	.get_temp = mtkts_chargeic_get_temp,
	.change_mode = mtkts_chargeic_change_mode,
	.get_crit_temp = mtkts_chargeic_get_crit_temp,
};



static int mtkts_chargeic_read(struct seq_file *m, void *v)
{
...
static ssize_t mtkts_chargeic_write(
struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
	int len = 0, i;

	struct mtktschargeic_data {
		int trip[10];
		int t_type[10];
	...
	struct mtktschargeic_data *ptr_mtktschargeic_data = kmalloc(
					sizeof(*ptr_mtktschargeic_data), GFP_KERNEL);

	if (ptr_mtktschargeic_data == NULL)
		return -ENOMEM;


	len = (count < (sizeof(ptr_mtktschargeic_data->desc) - 1)) ?
				count : (sizeof(ptr_mtktschargeic_data->desc) - 1);

	if (copy_from_user(ptr_mtktschargeic_data->desc, buffer, len)) {
		kfree(ptr_mtktschargeic_data);
		return 0;
	}

	ptr_mtktschargeic_data->desc[len] = '\0';

	if (sscanf
	    (ptr_mtktschargeic_data->desc,
	     "%d %d %d %19s %d %d %19s %d %d %19s %d %d %19s %d %d %19s %d %d %19s %d %d %19s %d %d %19s %d %d %19s %d %d %19s %d",
		&num_trip,
		&ptr_mtktschargeic_data->trip[0], &ptr_mtktschargeic_data->t_type[0],
		ptr_mtktschargeic_data->bind0,
		&ptr_mtktschargeic_data->trip[1], &ptr_mtktschargeic_data->t_type[1],
		ptr_mtktschargeic_data->bind1,
		&ptr_mtktschargeic_data->trip[2], &ptr_mtktschargeic_data->t_type[2],
		ptr_mtktschargeic_data->bind2,
		&ptr_mtktschargeic_data->trip[3], &ptr_mtktschargeic_data->t_type[3],
		ptr_mtktschargeic_data->bind3,
		&ptr_mtktschargeic_data->trip[4], &ptr_mtktschargeic_data->t_type[4],
		ptr_mtktschargeic_data->bind4,
		&ptr_mtktschargeic_data->trip[5], &ptr_mtktschargeic_data->t_type[5],
		ptr_mtktschargeic_data->bind5,
		&ptr_mtktschargeic_data->trip[6], &ptr_mtktschargeic_data->t_type[6],
		ptr_mtktschargeic_data->bind6,
		&ptr_mtktschargeic_data->trip[7], &ptr_mtktschargeic_data->t_type[7],
		ptr_mtktschargeic_data->bind7,
		&ptr_mtktschargeic_data->trip[8], &ptr_mtktschargeic_data->t_type[8],
		ptr_mtktschargeic_data->bind8,
		&ptr_mtktschargeic_data->trip[9], &ptr_mtktschargeic_data->t_type[9],
		ptr_mtktschargeic_data->bind9,
		&ptr_mtktschargeic_data->time_msec) == 32) {

		down(&sem_mutex);
		mtkts_chargeic_dprintk(
			"[%s] mtkts_chargeic_unregister_thermal\n", __func__);
		...
		if (num_trip < 0 || num_trip > 10) {
			...
			aee_kernel_warning_api(__FILE__, __LINE__,
					DB_OPT_DEFAULT, "mtkts_chargeic_write",
					"Bad argument");
			#endif
			mtkts_chargeic_dprintk("[%s] bad argument\n", __func__);
			kfree(ptr_mtktschargeic_data);
			up(&sem_mutex);
			return -EINVAL;
		}
		...
		for (i = 0; i < num_trip; i++)
			g_THERMAL_TRIP[i] = ptr_mtktschargeic_data->t_type[i];
		...
		for (i = 0; i < 20; i++) {
			g_bind0[i] = ptr_mtktschargeic_data->bind0[i];
			g_bind1[i] = ptr_mtktschargeic_data->bind1[i];
			g_bind2[i] = ptr_mtktschargeic_data->bind2[i];
			g_bind3[i] = ptr_mtktschargeic_data->bind3[i];
			g_bind4[i] = ptr_mtktschargeic_data->bind4[i];
			g_bind5[i] = ptr_mtktschargeic_data->bind5[i];
			g_bind6[i] = ptr_mtktschargeic_data->bind6[i];
			g_bind7[i] = ptr_mtktschargeic_data->bind7[i];
			g_bind8[i] = ptr_mtktschargeic_data->bind8[i];
			g_bind9[i] = ptr_mtktschargeic_data->bind9[i];
		}
		...
		for (i = 0; i < num_trip; i++)
			trip_temp[i] = ptr_mtktschargeic_data->trip[i];

		interval = ptr_mtktschargeic_data->time_msec / 1000;
		...
		mtkts_chargeic_dprintk("trip_9_temp=%d,time_ms=%d\n",
			trip_temp[9], interval * 1000);

		mtkts_chargeic_dprintk(
			"[%s] mtkts_chargeic_register_thermal\n", __func__);
...
void mtkts_chargeic_prepare_table(int table_num)
{
	...
	switch (table_num) {
	case 1:		/* AP_NTC_BL197 */
		CHARGEIC_Temperature_Table = CHARGEIC_Temperature_Table1;
		ntc_tbl_size = sizeof(CHARGEIC_Temperature_Table1);
		break;
	case 2:		/* AP_NTC_TSM_1 */
		CHARGEIC_Temperature_Table = CHARGEIC_Temperature_Table2;
		ntc_tbl_size = sizeof(CHARGEIC_Temperature_Table2);
		break;
	case 3:		/* AP_NTC_10_SEN_1 */
		CHARGEIC_Temperature_Table = CHARGEIC_Temperature_Table3;
		ntc_tbl_size = sizeof(CHARGEIC_Temperature_Table3);
		break;
	case 4:		/* AP_NTC_10 */
		CHARGEIC_Temperature_Table = CHARGEIC_Temperature_Table4;
		ntc_tbl_size = sizeof(CHARGEIC_Temperature_Table4);
		break;
	case 5:		/* AP_NTC_47 */
		CHARGEIC_Temperature_Table = CHARGEIC_Temperature_Table5;
		ntc_tbl_size = sizeof(CHARGEIC_Temperature_Table5);
		break;
	case 6:		/* NTCG104EF104F */
		CHARGEIC_Temperature_Table = CHARGEIC_Temperature_Table6;
		ntc_tbl_size = sizeof(CHARGEIC_Temperature_Table6);
		break;
	case 7:		/* NCP15WF104F03RC */
		CHARGEIC_Temperature_Table = CHARGEIC_Temperature_Table7;
		ntc_tbl_size = sizeof(CHARGEIC_Temperature_Table7);
		break;
	default:		/* AP_NTC_10 */
		CHARGEIC_Temperature_Table = CHARGEIC_Temperature_Table4;
		ntc_tbl_size = sizeof(CHARGEIC_Temperature_Table4);
...
static int mtkts_chargeic_param_read(struct seq_file *m, void *v)
{
	seq_printf(m, "%d\n", g_RAP_pull_up_R);
...
static ssize_t mtkts_chargeic_param_write(
struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
	int len = 0;
	struct mtktschargeic_param_data {
		char desc[512];
	...
	struct mtktschargeic_param_data *ptr_mtktschargeic_parm_data;

	ptr_mtktschargeic_parm_data = kmalloc(
				sizeof(*ptr_mtktschargeic_parm_data), GFP_KERNEL);

	if (ptr_mtktschargeic_parm_data == NULL)
		return -ENOMEM;
	 ...
	 */
	ptr_mtktschargeic_parm_data->adc_channel = 11;

	len = (count < (sizeof(ptr_mtktschargeic_parm_data->desc) - 1)) ?
			count : (sizeof(ptr_mtktschargeic_parm_data->desc) - 1);

	if (copy_from_user(ptr_mtktschargeic_parm_data->desc, buffer, len)) {
		kfree(ptr_mtktschargeic_parm_data);
		return 0;
	}

	ptr_mtktschargeic_parm_data->desc[len] = '\0';

	mtkts_chargeic_dprintk("[%s]\n", __func__);

	if (sscanf
	    (ptr_mtktschargeic_parm_data->desc, "%9s %d %9s %d %15s %d %9s %d %d",
		ptr_mtktschargeic_parm_data->pull_R, &ptr_mtktschargeic_parm_data->valR,
		ptr_mtktschargeic_parm_data->pull_V, &ptr_mtktschargeic_parm_data->valV,
		ptr_mtktschargeic_parm_data->overcrilow,
		&ptr_mtktschargeic_parm_data->over_cri_low,
		ptr_mtktschargeic_parm_data->NTC_TABLE,
		&ptr_mtktschargeic_parm_data->ntc_table,
		&ptr_mtktschargeic_parm_data->adc_channel) >= 8) {

		if (!strcmp(ptr_mtktschargeic_parm_data->pull_R, "PUP_R")) {
			g_RAP_pull_up_R = ptr_mtktschargeic_parm_data->valR;
			mtkts_chargeic_dprintk("g_RAP_pull_up_R=%d\n",
							g_RAP_pull_up_R);
		} else {
			mtkts_chargeic_printk(
		...
		if (!strcmp(ptr_mtktschargeic_parm_data->pull_V, "PUP_VOLT")) {
			g_RAP_pull_up_voltage = ptr_mtktschargeic_parm_data->valV;
			mtkts_chargeic_dprintk("g_Rat_pull_up_voltage=%d\n",
							g_RAP_pull_up_voltage);
		} else {
			mtkts_chargeic_printk(
		...
		if (!strcmp(ptr_mtktschargeic_parm_data->overcrilow,
			"OVER_CRITICAL_L")) {
			g_TAP_over_critical_low =
					ptr_mtktschargeic_parm_data->over_cri_low;
			mtkts_chargeic_dprintk("g_TAP_over_critical_low=%d\n",
						g_TAP_over_critical_low);
		} else {
			mtkts_chargeic_printk(
		...
		if (!strcmp(ptr_mtktschargeic_parm_data->NTC_TABLE, "NTC_TABLE")) {
			g_RAP_ntc_table = ptr_mtktschargeic_parm_data->ntc_table;
			mtkts_chargeic_dprintk("g_RAP_ntc_table=%d\n",
							g_RAP_ntc_table);
		} else {
			mtkts_chargeic_printk(
		 ...
		 * choose "adc_channel=11" to check if there is any param input
		 */
		if ((ptr_mtktschargeic_parm_data->adc_channel >= 2)
		&& (ptr_mtktschargeic_parm_data->adc_channel <= 11))
			 ...
			 */
			if (ptr_mtktschargeic_parm_data->adc_channel != 11)
				g_RAP_ADC_channel =
					ptr_mtktschargeic_parm_data->adc_channel;
			else
				g_RAP_ADC_channel = AUX_IN0_NTC;
		}
		mtkts_chargeic_dprintk("adc_channel=%d\n",
					ptr_mtktschargeic_parm_data->adc_channel);
		mtkts_chargeic_dprintk("g_RAP_ADC_channel=%d\n",
						g_RAP_ADC_channel);
	...
	mtkts_chargeic_printk("[%s] bad argument\n", __func__);
	kfree(ptr_mtktschargeic_parm_data);
	return -EINVAL;
}
...
static int mtkts_chargeic_register_thermal(void)
{
	...
	/* trips : trip 0~1 */
	thz_dev = mtk_thermal_zone_device_register("mtktschargeic",
						trips,
						num_trip,
						NULL,
...
static int mtkts_chargeic_open(struct inode *inode, struct file *file)
{
	return single_open(file, mtkts_chargeic_read, NULL);
}

static const struct proc_ops mtkts_AP_fops = {
	.proc_open = mtkts_chargeic_open,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_write = mtkts_chargeic_write,
	.proc_release = single_release,
};


static int mtkts_chargeic_param_open(struct inode *inode, struct file *file)
{
	return single_open(file, mtkts_chargeic_param_read, NULL);
}

static const struct proc_ops mtkts_AP_param_fops = {
	.proc_open = mtkts_chargeic_param_open,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_write = mtkts_chargeic_param_write,
	.proc_release = single_release,
};
...
static int mtkts_chargeic_probe(struct platform_device *pdev)
{
	...
	pr_notice("[Thermal/TZ/CHARGEIC] %s line=%d\n", __func__, __LINE__);
	...
	if (ret) {
		mtkts_chargeic_printk("[%s] fail to get auxadc iio ch0: %d\n",
			__func__, ret);
		return ret;
	}
	...
	if (!mtkts_AP_dir) {
		...
		entry = proc_create("tzchargeic", 0664, mtkts_AP_dir,
							&mtkts_AP_fops);
		if (entry)
			proc_set_user(entry, uid, gid);

	entry = proc_create("tzchargeic_param", 0664, mtkts_AP_dir,
				&mtkts_AP_param_fops);
		if (entry)
			proc_set_user(entry, uid, gid);
...
int mtkts_chargeic_init(void)
{
	...
	mtkts_AP_dir = mtk_thermal_get_proc_drv_therm_dir_entry();
	if (!mtkts_AP_dir) {
		mtkts_chargeic_dprintk("[%s]: mkdir /proc/driver/thermal failed\n",
								__func__);
	} else {
		entry = proc_create("tzchargeic", 0664, mtkts_AP_dir,
							&mtkts_AP_fops);
