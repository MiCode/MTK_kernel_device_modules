...
static ssize_t mcupm_outlier_ic_show(struct device *kobj,
				 struct device_attribute *attr, char *buf)
{
	struct mcupm_ipi_data_s ipi_data;
	int ret = 0;
	...
	return snprintf(buf, PAGE_SIZE, "not support Qurey outlier ic.\n");
}
DEVICE_ATTR_RO(mcupm_outlier_ic);
...
int mcupm_plt_module_init(void)
{
	...
	ret = mcupm_sysfs_create_file(&dev_attr_mcupm_outlier_ic);
	if (unlikely(ret != 0))
		goto error;
