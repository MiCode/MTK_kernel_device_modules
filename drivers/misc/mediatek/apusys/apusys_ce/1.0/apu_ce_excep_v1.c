...
enum CE_V1_JOB_ID {
	CE_V1_JOB_ID_TPPA_PLUS_BW_ACC = 0,
	CE_V1_JOB_ID_NORM2LP,
	CE_V1_JOB_ID_LP2NORM,
	CE_V1_JOB_ID_RESERVED_3,
	CE_V1_JOB_ID_RESERVED_4,
	CE_V1_JOB_ID_RESERVED_5,
	CE_V1_JOB_ID_RESERVED_6,
	CE_V1_JOB_ID_RESERVED_7,
	CE_V1_JOB_ID_RESERVED_8,
	CE_V1_JOB_ID_RESERVED_9,
	CE_V1_JOB_ID_RESERVED_10,
	CE_V1_JOB_ID_RESERVED_11,
	CE_V1_JOB_ID_RESERVED_12,
	CE_V1_JOB_ID_RESERVED_13,
	CE_V1_JOB_ID_ACX_MDLA_MTCMOS_OFF,
	CE_V1_JOB_ID_RCX_MDLA_MTCMOS_OFF,
	CE_V1_JOB_ID_RCX_WAKEUP,
	CE_V1_JOB_ID_RCX_SLEEP,
	CE_V1_JOB_ID_TPPA_PLUS_PSC,
	CE_V1_JOB_ID_MNOC_BW_JOB_KICKER,
	CE_V1_JOB_ID_BRISKET_OUTER_LOOP,
	CE_V1_JOB_ID_RESERVED_21,
	CE_V1_JOB_ID_BW_PREDICTION,
	CE_V1_JOB_ID_QOS_EVENT_DRIVEN,
	CE_V1_JOB_ID_RESERVED_24,
	CE_V1_JOB_ID_RESERVED_25,
	CE_V1_JOB_ID_SMMU_RESTORE,
	CE_V1_JOB_ID_RCX_NOC_BW_ACC,
	CE_V1_JOB_ID_ACX0_NOC_BW_ACC,
	CE_V1_JOB_ID_ACX1_NOC_BW_ACC,
	CE_V1_JOB_ID_NCX_NOC_BW_ACC,
	CE_V1_JOB_ID_DVFS,
	CE_V1_JOB_ID_MAX
};

const char *CE_V1_JOB_NAME[CE_V1_JOB_ID_MAX] = {
...
const enum CE_V1_JOB_ID CE_V1_HW_TIMER[] = {
	CE_V1_JOB_ID_TPPA_PLUS_PSC,
	CE_V1_JOB_ID_MNOC_BW_JOB_KICKER,
	CE_V1_JOB_ID_BRISKET_OUTER_LOOP,
	CE_V1_JOB_ID_MAX
...
const enum CE_V1_JOB_ID CE_V1_HW_TIMER_EXP_BYPASS[] = {
	CE_V1_JOB_ID_BRISKET_OUTER_LOOP,
};
...
static struct apu_ce_v1_ops *g_apu_ce_ops;
...
{
	if (job_id < CE_V1_JOB_ID_MAX)
		return CE_V1_JOB_NAME[job_id];
	else
...
static int get_exception_job_id(struct device *dev, int32_t *job_id, bool *by_pass)
{
	...
	op = GET_SMC_OP_V1(SMC_OP_APU_V1_ACE_ABN_IRQ_FLAG_CE,
	...
	op = GET_SMC_OP_V1(SMC_OP_APU_V1_ACE_CE0_TASK_ING,
	...
	op = GET_SMC_OP_V1(SMC_OP_APU_V1_ACE_CE3_TASK_ING,
	...
	if (ce_flag) {
		if (ce_flag & APU_V1_CE_0_IRQ_MASK)
			exception_ce_id = 0;
		else if (ce_flag & APU_V1_CE_1_IRQ_MASK)
			exception_ce_id = 1;
		else if (ce_flag & APU_V1_CE_2_IRQ_MASK)
			exception_ce_id = 2;
		else if (ce_flag & APU_V1_CE_3_IRQ_MASK)
			exception_ce_id = 3;
	} else if (ace_flag) {
		if (ace_flag & APU_V1_CE_MISS_TYPE2_REQ_FLAG_0_MSK) {
			exception_timer_id = 0;
			w1c_val = APU_V1_CE_MISS_TYPE2_REQ_FLAG_0_MSK;
		} else if (ace_flag & APU_V1_CE_MISS_TYPE2_REQ_FLAG_1_MSK) {
			exception_timer_id = 1;
			w1c_val = APU_V1_CE_MISS_TYPE2_REQ_FLAG_1_MSK;
		} else if (ace_flag & APU_V1_CE_MISS_TYPE2_REQ_FLAG_2_MSK) {
			exception_timer_id = 2;
			w1c_val = APU_V1_CE_MISS_TYPE2_REQ_FLAG_2_MSK;
		} else if (ace_flag & APU_V1_CE_MISS_TYPE2_REQ_FLAG_3_MSK) {
			exception_timer_id = 3;
			w1c_val = APU_V1_CE_MISS_TYPE2_REQ_FLAG_3_MSK;
		}

		else if (ace_flag & APU_V1_CE_NON_ALIGNED_APB_FLAG_MSK) {
			if (ace_flag & APU_V1_CE_NON_ALIGNED_APB_OUT_FLAG_MSK)
				apb_status = apb_out_status;
			else if (ace_flag & APU_V1_CE_NON_ALIGNED_APB_IN_FLAG_MSK)
				apb_status = apb_in_status;

			if (apb_status & APU_V1_CE_APB_ERR_STATUS_CE0_MSK)
				exception_ce_id = 0;
			else if (apb_status & APU_V1_CE_APB_ERR_STATUS_CE1_MSK)
				exception_ce_id = 1;
			else if (apb_status & APU_V1_CE_APB_ERR_STATUS_CE2_MSK)
				exception_ce_id = 2;
			else if (apb_status & APU_V1_CE_APB_ERR_STATUS_CE3_MSK)
				exception_ce_id = 3;
		}
	}
	...
	if (exception_ce_id >= 0) {
		...
		*job_id = (ce_task[exception_ce_id] >>
			APU_V1_CE_TASK_JOB_SFT) & APU_V1_CE_TASK_JOB_MSK;
	...
	if (exception_timer_id >= 0) {
		...
		*job_id = CE_V1_HW_TIMER[exception_timer_id];

		dev_info(dev, "HW_Timer_%d mapping to job id %d (%s)\n",
			exception_timer_id, *job_id, get_ce_job_name_by_id(*job_id));

		for (i = 0; i < sizeof(CE_V1_HW_TIMER_EXP_BYPASS) / sizeof(enum CE_V1_JOB_ID); i++)
			if (*job_id == CE_V1_HW_TIMER_EXP_BYPASS[i]) {
				...
				*by_pass = true;
				...
				apusys_rv_smc_call(dev, MTK_APUSYS_KERNEL_OP_APUSYS_CE_REG_WRITE,
					SMC_OP_APU_V1_ACE_ABN_IRQ_FLAG_ACE_SW, w1c_val, NULL, NULL, NULL);
			}
	}
...
static void log_ce_register(struct device *dev)
{
	...
	op = GET_SMC_OP_V1(SMC_OP_APU_V1_CE0_RUN_INSTR,
	...
	op = GET_SMC_OP_V1(SMC_OP_APU_V1_CE3_RUN_INSTR,
	...
	op = GET_SMC_OP_V1(SMC_OP_APU_V1_CE2_RUN_PC,
	...
	op = GET_SMC_OP_V1(SMC_OP_APU_V1_ACE_CMD_Q_STATUS_3,
...
static irqreturn_t apu_ce_isr(int irq, void *private_data)
{
	...
	if (g_apu_ce_ops->check_apu_exp_irq(apu)) {
		...
		apusys_rv_smc_call(dev, MTK_APUSYS_KERNEL_OP_APUSYS_CE_REG_WRITE,
			SMC_OP_APU_V1_CE0_STEP, 0x1, NULL, NULL, NULL);
		apusys_rv_smc_call(dev, MTK_APUSYS_KERNEL_OP_APUSYS_CE_REG_WRITE,
			SMC_OP_APU_V1_CE1_STEP, 0x1, NULL, NULL, NULL);
		apusys_rv_smc_call(dev, MTK_APUSYS_KERNEL_OP_APUSYS_CE_REG_WRITE,
			SMC_OP_APU_V1_CE2_STEP, 0x1, NULL, NULL, NULL);
		apusys_rv_smc_call(dev, MTK_APUSYS_KERNEL_OP_APUSYS_CE_REG_WRITE,
			SMC_OP_APU_V1_CE3_STEP, 0x1, NULL, NULL, NULL);
...
static int ce_fw_sram_show(struct seq_file *s, void *v)
{
	...
	while (ce_reg_addr[offset++] == APU_V1_CE_REG_DUMP_MAGIC_NUM) {
...
const struct apu_ce_v1_ops v1_apu_ce_ops = {
	.check_apu_exp_irq = gernel_check_apu_exp_irq,
};

const struct apu_ce_v1_ops mt6897_apu_ce_ops = {
	.check_apu_exp_irq = mt6897_check_apu_exp_irq,
};

static const struct of_device_id apu_ce_of_match[] = {
	{ .compatible = "mediatek,mt6878-apusys_rv", .data = &v1_apu_ce_ops},
	{ .compatible = "mediatek,mt6897-apusys_rv", .data = &mt6897_apu_ce_ops},
	{ .compatible = "mediatek,mt6899-apusys_rv", .data = &v1_apu_ce_ops},
	{ .compatible = "mediatek,mt6989-apusys_rv", .data = &v1_apu_ce_ops},
	{ .compatible = "mediatek,mt6991-apusys_rv", .data = &v1_apu_ce_ops},
	{},
};
...
int apu_ce_excep_init_v1(struct platform_device *pdev, struct mtk_apu *apu)
{
	...
	for (i = 0; i < ARRAY_SIZE(apu_ce_of_match); i++)
		if (of_device_is_compatible(
				pdev->dev.of_node, apu_ce_of_match[i].compatible)) {
			g_apu_ce_ops = (struct apu_ce_v1_ops *)apu_ce_of_match[i].data;
			break;
		}
