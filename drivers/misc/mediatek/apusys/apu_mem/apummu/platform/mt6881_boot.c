...
enum eAPUMMU_6881_PAGESIZE {
	eAPUMMU_6881_PAGE_LEN_128KB,
	eAPUMMU_6881_PAGE_LEN_256KB,
	eAPUMMU_6881_PAGE_LEN_512KB,
	eAPUMMU_6881_PAGE_LEN_1MB,
	eAPUMMU_6881_PAGE_LEN_128MB,
	eAPUMMU_6881_PAGE_LEN_256MB,
	eAPUMMU_6881_PAGE_LEN_512MB,
	eAPUMMU_6881_PAGE_LEN_4GB,
};
...
static void dump_table(u32 vsid)
{
	u32 seg, j, i, val[4];

	for (j = 0; j < 10; j++) {
		// init
		seg = seg_offset(vsid, j);

		for (i = 0; i < 4; i++)
			val[i]  = ioread32(cmut_base + seg + (0x0000 + (i*4)));

		dbg_printf("0x%08x: %08x %08x %08x %08x\n", CMU_TOP_BASE + seg, val[0], val[1], val[2], val[3]);
	}
}
 ...
 * eAPUMMU_PAGE_LEN_4MB,
 * eAPUMMU_PAGE_LEN_8MB,
 * eAPUMMU_PAGE_LEN_16MB,
 * eAPUMMU_PAGE_LEN_128MB,
 * eAPUMMU_PAGE_LEN_256MB,
 * eAPUMMU_PAGE_LEN_512MB,
 * eAPUMMU_PAGE_LEN_1GB,
 * eAPUMMU_PAGE_LEN_2GB,
 * eAPUMMU_PAGE_LEN_4GB,
 * eAPUMMU_PAGE_LEN_8GB,
 * eAPUMMU_PAGE_LEN_16GB,
 * eAPUMMU_PAGE_LEN_64GB,
};
...
static u32 ammu_page_len_remap(u32 page_len)
{
	u32 ret_page_len;
	...
	if (page_len <= eAPUMMU_PAGE_LEN_1MB)
		...
		ret_page_len = eAPUMMU_6881_PAGE_LEN_256MB;
	else if (page_len <= eAPUMMU_PAGE_LEN_512MB)
		ret_page_len = eAPUMMU_6881_PAGE_LEN_512MB;
	else
		ret_page_len = eAPUMMU_6881_PAGE_LEN_4GB;
...
{
	u32 seg, val, remap_page_len;

	remap_page_len = ammu_page_len_remap(page_len);
	 ...
	 */
	val  = (remap_page_len & 0x7) <<  0;
	val |= (0x0)                  <<  3;
	val |= (in_adr >> 12)         << 10;
	dbg_printf("0x%x = 0x%x\n", CMU_TOP_BASE + seg + 0x0000, val);
...
static int add_rv_map(u32 map_adr)
{
	...
	ret = add_seg(VSID_RSV_UPRV, 2, 0, 0, eAPUMMU_PAGE_LEN_4GB, domain, ns, SID_NO_SEC);
	if (ret)
		goto exit;
...
static int mt6881_rv_boot(u32 uP_seg_output, u8 uP_hw_thread,
	u32 logger_seg_output, enum eAPUMMUPAGESIZE logger_page_size,
	u32 XPU_seg_output, enum eAPUMMUPAGESIZE XPU_page_size)
{
	int ret;

	dbg_printf("uP_seg_output = 0x%x, uP_hw_thread = %u\n",
		uP_seg_output, uP_hw_thread);
	...
	dbg_printf("VSID_RSV_UPRV = %u\n", VSID_RSV_UPRV);
	dump_table(VSID_RSV_UPRV);
	...
	dbg_printf("VSID_RSV_LOGGER = %u\n", VSID_RSV_LOGGER);
	dump_table(VSID_RSV_LOGGER);
...
const struct mtk_apu_ammudata mt6881_ammudata = {
	.ops = {
		.rv_boot = mt6881_rv_boot,
	},
};
