...
static void apu_setup_apummu(struct mtk_apu *apu, int boundary, int ns, int domain)
{
	...
	if (apu->platdata->flags & F_SECURE_BOOT) {
		...
		dev_info(dev, "%s: call rv_boot 0x%llx 0x%llx 0x%llx\n", __func__,
			(unsigned long long) apu->code_da,
			(unsigned long long) hw_logger_addr,
			(unsigned long long) tcm_addr);
...
static int mt6881_rproc_start(struct mtk_apu *apu)
{
	int ns = 1; /* Non Secure */
	int domain = 0;
...
static int mt6881_rproc_stop(struct mtk_apu *apu)
{
	struct device *dev = apu->dev;
...
static void mt6881_apu_pwr_wake_lock(struct mtk_apu *apu, uint32_t id)
{
#if IS_ENABLED(CONFIG_PM_SLEEP)
	unsigned long flags;
...
static void mt6881_apu_pwr_wake_unlock(struct mtk_apu *apu, uint32_t id)
{
#if IS_ENABLED(CONFIG_PM_SLEEP)
	unsigned long flags;
...
static int mt6881_cold_boot_power_on(struct mtk_apu *apu)
{
	struct device *dev = apu->dev;
	int ret;
...
static int mt6881_polling_rpc_status(struct mtk_apu *apu, u32 pwr_stat, u32 timeout)
{
	int ret = 0;
	void *addr = 0;
	uint32_t val = 0;
 ...
 */
static int mt6881_power_on_off_locked(struct mtk_apu *apu, u32 id, u32 on, u32 off)
{
	int ret = 0;
	struct device *dev = apu->dev;
	...
	if (on == 1 && off == 0) {
		...
		if (apu->ipi_pwr_ref_cnt[id] == U32_MAX) {
			...
			apu->local_pwr_ref_cnt++;
			mt6881_apu_pwr_wake_lock(apu, id);
			...
			if (apu->local_pwr_ref_cnt == 1) {
				...
				if (pwr_ready == 1 && id != APU_IPI_SCP_NP_RECOVER && rpc_state == 1) {
					...
					mt6881_apu_pwr_wake_unlock(apu, id);
					apu->ipi_pwr_ref_cnt[id]--;
					apu->local_pwr_ref_cnt--;
					return -EBUSY;
				}
...
static int mt6881_power_on_off(struct mtk_apu *apu, u32 id, u32 on, u32 off)
{
	int ret = 0;
	struct device *dev = apu->dev;
	/* struct timespec64 ts, te; */
	...
	for (i = 0; i < retry_cnt; i++) {
		...
		profile_start(&ts);
		ret = mt6881_power_on_off_locked(apu, id, on, off);
...
static int mt6881_ipi_send_pre(struct mtk_apu *apu, uint32_t id, bool is_host_initiated)
{
...
static void mt6881_debug_info_dump(struct mtk_apu *apu, struct seq_file *s)
{
	int i;
...
static int mt6881_irq_affin_init(struct mtk_apu *apu)
{
	int i;
...
static int mt6881_irq_affin_set(struct mtk_apu *apu)
{
...
static int mt6881_irq_affin_unset(struct mtk_apu *apu)
{
	irq_set_affinity_hint(apu->mbox0_irq_number, &normal_cpus);
...
static int mt6881_irq_affin_clear(struct mtk_apu *apu)
{
...
static int mt6881_apu_memmap_init(struct mtk_apu *apu)
{
...
static void mt6881_apu_memmap_remove(struct mtk_apu *apu)
{
}

static void mt6881_rv_cg_gating(struct mtk_apu *apu)
{
	iowrite32(0x0, apu->md32_sysctrl + MD32_CLK_EN);
}

static void mt6881_rv_cg_ungating(struct mtk_apu *apu)
{
	iowrite32(0x1, apu->md32_sysctrl + MD32_CLK_EN);
}

static void mt6881_rv_cachedump(struct mtk_apu *apu)
{
	int offset;
	unsigned long flags;
...
static void apu_polling_on_work_func(struct work_struct *p_work)
{
	...
	ret = mt6881_polling_rpc_status(apu, 1, 1);
	if (ret) {
		apu_regdump();
		apu->bypass_pwr_off_chk = true;
...
static int mt6881_apu_power_init(struct mtk_apu *apu)
{
...
static int mt6881_rproc_init(struct mtk_apu *apu)
{
	int ret;
#if IS_ENABLED(CONFIG_PM_SLEEP)
	apu->wake_lock_ref_cnt = 0;
	spin_lock_init(&apu->wakelock_spinlock);
	ws = wakeup_source_register(NULL, "mt6881_apusys_rv");
	if (!ws) {
	...
	mt6881_apu_pwr_wake_lock(apu, APU_IPI_INIT);
#endif
	...
	ret = mt6881_cold_boot_power_on(apu);
	if (ret)
		dev_info(apu->dev, "%s: call mt6881_cold_boot_power_on fail(%d)\n",
			__func__, ret);

	return ret;
}

static int mt6881_rproc_exit(struct mtk_apu *apu)
{
#if IS_ENABLED(CONFIG_PM_SLEEP)
...
const struct mtk_apu_platdata mt6881_platdata = {
	...
	.ops		= {
		...
		.start	= mt6881_rproc_start,
		.stop	= mt6881_rproc_stop,
		.apu_memmap_init = mt6881_apu_memmap_init,
		.apu_memmap_remove = mt6881_apu_memmap_remove,
		.power_on_off = mt6881_power_on_off,
		.polling_rpc_status = mt6881_polling_rpc_status,
		.wake_lock = mt6881_apu_pwr_wake_lock,
		.wake_unlock = mt6881_apu_pwr_wake_unlock,
		.debug_info_dump = mt6881_debug_info_dump,
		.cg_gating = mt6881_rv_cg_gating,
		.cg_ungating = mt6881_rv_cg_ungating,
		.rv_cachedump = mt6881_rv_cachedump,
		.power_init = mt6881_apu_power_init,
		.ipi_send_pre = mt6881_ipi_send_pre,
		.irq_affin_init = mt6881_irq_affin_init,
