...
static struct mdw_pb_mgr *g_pb = NULL;

/* transfer mdw policy to pbm's mode */
static inline enum mdw_pwrplcy_type mdw_pbmode2policy(enum pbm_mode type)
{
...
static int mdw_pb_put_cnt(enum mdw_pwrplcy_type type, uint32_t cnt)
{
	int ret = 0, val = 0;
	enum pbm_mode pb_type = mdw_rv_policy2pbmode(type);
...
{
	return mdw_pb_put_cnt(type, 1);
}

static void mdw_pb_put_type(enum pbm_mode pb_type)
{
	int cnt = 0;
	...
	mdw_pb_put_cnt(mdw_pbmode2policy(pb_type), cnt);
}

static void mdw_pb_norm_put_func(struct work_struct *wk)
{
	mdw_pb_put_type(PBM_MODE_NORMAL);
}

static void mdw_pb_perf_put_func(struct work_struct *wk)
{
	mdw_pb_put_type(PBM_MODE_PERFORMANCE);
}

/* order should match enum pbm_mode */
static void(*mdw_pb_put_func[PBM_MODE_MAX])(struct work_struct *wk) = {
	mdw_pb_norm_put_func,
	mdw_pb_perf_put_func,
};

int mdw_pb_init(struct mdw_device *mdev)
{
	uint32_t i = 0;

	/* pb mgr */
	g_pb = devm_kzalloc(mdw_dev->misc_dev->this_device, sizeof(*g_pb), GFP_KERNEL);
	if (IS_ERR_OR_NULL(g_pb))
		return -ENOMEM;
	...
	for (i = 0; i < PBM_MODE_MAX; i++) {
		INIT_DELAYED_WORK(&g_pb->put_wk[i], mdw_pb_put_func[i]);
...
void mdw_pb_deinit(struct mdw_device *mdev)
{
	devm_kfree(mdw_dev->misc_dev->this_device, g_pb);
}