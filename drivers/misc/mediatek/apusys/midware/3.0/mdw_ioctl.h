...
enum mdw_mem_flag {
	...
	MDW_MEM_FLAG_ATTR_SLC_DC,
	MDW_MEM_FLAG_ATTR_SHAREABLE, // coherence
};
...
#define F_MDW_MEM_ATTR_SLC_DC (1ULL << MDW_MEM_FLAG_ATTR_SLC_DC)
#define F_MDW_MEM_ATTR_SHAREABLE (1ULL << MDW_MEM_FLAG_ATTR_SHAREABLE)
...
struct mdw_cmd_in {
	...
	union {
		struct {
			...
			uint64_t prev_mem_transactions;
			/* exec order */
			uint64_t execute_orders;
			uint64_t predecessors;
			uint32_t predecessors_num;
			uint64_t pack_friends;
			uint32_t pack_friends_num;
			uint64_t end_vertices;
