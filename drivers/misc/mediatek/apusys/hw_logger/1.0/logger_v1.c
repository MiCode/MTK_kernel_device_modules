...
static int hw_logger_buf_alloc(struct device *dev)
{
	...
	if (ret || (aov_log_buf_sz == 0)) {
		HWLOGR_INFO("not support aov_log_buf_sz(%d)(ret = %d)\n",
			aov_log_buf_sz, ret);
		ret = 0;
	} else if (aov_log_buf_sz != 0) {
		HWLOGR_INFO("aov_log_buf_sz = %d\n", aov_log_buf_sz);
...
static int logger_v1_dump_tcm_log(void)
{
...
static unsigned long long get_t_size(void)
{
...
static int logger_v1_config_init(struct mtk_apu *apu)
{
	unsigned long flags;
...
static int logger_v1_ipi_init(struct mtk_apu *apu)
{
	int ret = 0;
...
static void logger_v1_ipi_remove(struct mtk_apu *apu)
{
...
static int hw_logger_copy_buf(void)
{
	int ret = 0;
...
static int logger_v1_power_on(void)
{
...
static int logger_v1_deep_idle_enter_pre(void)
{
...
static int logger_v1_deep_idle_enter_post(void)
{
...
static int logger_v1_deep_idle_leave(void)
{
							...
							size_t count, loff_t *f_pos)
{
	...
	if (ret) {
		...
		ret2 = apu_power_on_off(g_apu->pdev, APU_IPI_LOG_LEVEL, 0, 1);
...
static int logger_v1_probe(struct platform_device *pdev)
{
...
static int logger_v1_remove(struct platform_device *pdev)
{
	struct device *dev = &pdev->dev;
...
static void logger_v1_shutdown(struct platform_device *pdev)
{
	HWLOGR_INFO("in\n");
...
const struct hw_logger_v1_ops v1_ops = {
	.power_on = logger_v1_power_on,
	.deep_idle_enter_pre = logger_v1_deep_idle_enter_pre,
	.deep_idle_enter_post = logger_v1_deep_idle_enter_post,
	.deep_idle_leave = logger_v1_deep_idle_leave,
	.dump_tcm_log = logger_v1_dump_tcm_log,
};

const struct mtk_apu_logger_platdata logger_v1_platdata = {
	.ops = {
		.v1_ops = &v1_ops,
		.probe = logger_v1_probe,
		.remove = logger_v1_remove,
		.shutdown = logger_v1_shutdown,
		.config_init = logger_v1_config_init,
		.ipi_init = logger_v1_ipi_init,
		.ipi_remove = logger_v1_ipi_remove,
	},
};
