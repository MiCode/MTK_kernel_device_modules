 ...
 */
#ifndef __MVPU30_SEC_H__
#define __MVPU30_SEC_H__
...
void mvpu30_set_sec_log_lvl(int log_lvl);

// image
bool mvpu30_get_mvpu_algo_available(void);
uint32_t mvpu30_get_ptn_total_size(void);
uint32_t mvpu30_get_ptn_size(uint32_t hash);
bool mvpu30_get_ptn_hash(uint32_t hash);
uint32_t mvpu30_get_kerbin_total_size(void);
uint32_t mvpu30_get_ker_img_offset(void);

void mvpu30_get_ker_info(uint32_t hash, uint32_t *ker_bin_offset, uint32_t *ker_bin_num);
void mvpu30_set_ker_iova(uint32_t ker_bin_offset, uint32_t ker_bin_num, uint32_t *ker_bin_each_iova);

// buf map
void mvpu30_map_base_buf_id(uint32_t buf_num,
...
// mem pool
uint32_t mvpu30_get_saved_session_id(void *session);
uint32_t mvpu30_get_avail_session_id(void);

void mvpu30_clear_session(void *session);

void mvpu30_update_session_id(uint32_t session_id, void *session);

uint32_t mvpu30_get_saved_hash_id(uint32_t session_id, uint32_t batch_name_hash);
uint32_t mvpu30_get_avail_hash_id(uint32_t session_id);

void mvpu30_clear_hash(uint32_t session_id, uint32_t hash_id);
void mvpu30_free_all_hash(uint32_t session_id);

int mvpu30_update_hash_pool(void *session,
							bool algo_in_img,
							uint32_t session_id,
							uint32_t hash_id,
...
#ifdef FULL_RP_INFO
int mvpu30_save_hash_info(uint32_t session_id,
						uint32_t hash_id,
						uint32_t buf_num,
						uint32_t rp_num,
						uint32_t *sec_chk_addr,
...
bool mvpu30_get_hash_info(void *session,
						uint32_t batch_name_hash,
						uint32_t *session_id,
						uint32_t *hash_id,
						uint32_t buf_num);

int mvpu30_replace_img_knl(void *session,
					uint32_t buf_num,
					uint32_t *sec_chk_addr,
					uint32_t *sec_buf_attr,
					uint32_t rp_num,
...
bool mvpu30_set_rp_skip_buf(uint32_t session_id,
						uint32_t hash_id,
						uint32_t buf_num,
						uint32_t *sec_chk_addr,
						uint32_t *mem_is_kernel,
...
// replacement
int mvpu30_update_new_base_addr(bool algo_in_img,
						bool algo_in_pool,
						uint32_t session_id,
						uint32_t hash_id,
						uint32_t *sec_chk_addr,
...
int mvpu30_replace_mem(uint32_t session_id,
					uint32_t hash_id,
					uint32_t *mem_is_kernel,
					bool algo_in_pool,
					uint32_t *rp_skip_buf,
...
int mvpu30_replace_kerarg(void *session,
					uint32_t session_id,
					uint32_t hash_id,
					uint32_t kerarg_num,
					uint32_t *sec_chk_addr,
...
void mvpu30_get_pool_kreg_iova(uint32_t *kreg_iova_pool,
						uint32_t session_id,
						uint32_t hash_id,
						uint32_t buf_cmd_kreg);

int mvpu30_add_img_mpu(void *mvpu_cmd);

int mvpu30_update_mpu(void *mvpu_cmd,
					uint32_t session_id,
					uint32_t hash_id,
					uint32_t *sec_chk_addr,
					uint32_t *sec_buf_size,
...
bool mvpu30_mem_use_iova(uint32_t addr);

int mvpu30_check_iova(void *session,
					void *cmd,
					uint32_t desc_type,
					uint32_t chk_num,
					uint32_t chk_base,
...
int mvpu30_check_batch_flow(void *session,
						void *cmd,
						uint32_t sec_level,
						uint32_t *kreg_kva,
						uint32_t knl_num);

int mvpu30_load_img(struct device *dev);

int mvpu30_sec_init(struct device *dev);

int mvpu30_sec_sysfs_init(struct kobject *root_dir);

#endif

#endif /* __MVPU30_SEC_H__ */
