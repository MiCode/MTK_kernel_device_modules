...
void mvpu30_clear_session(void *session)
{
	...
	for (session_id = 0; session_id < MAX_SAVE_SESSION; session_id++) {
		...
		if (saved_session[session_id] == (uint64_t)session) {
			...
			mvpu30_free_all_hash(session_id);
						...
						uint32_t buf_num)
{
	...
	*session_id = mvpu30_get_saved_session_id(session);

	if (*session_id == 0xFFFFFFFF)
		return false;

	*hash_id = mvpu30_get_saved_hash_id(*session_id, batch_name_hash);
...
void mvpu30_CopyArgToPrimem(char *dst_ptr, char *src_ptr, int src_size)
{
...
void mvpu30_CheckPrimemArg(char *dst_ptr, int src_size)
{
	int i, j;
	...
	for (cnt = 0; cnt < kerarg_num; cnt++) {
		...
		for (i = 0; i < primem_num; i++) {
			if ((kerarg_buf_id[cnt] == primem_src_buf_id[i]) &&
				...
				if (mvpu_loglvl_sec >= APUSYS_MVPU_LOG_DBG) {
					pr_info("[MVPU][Sec] mvpu30_CopyArgToPrimem buf[%d]->buf[%d], ker_ofst 0x%08x, dst_ofst (0x%08x + 0x%08x), size 0x%x\n",
							primem_src_buf_id[i],
							primem_dst_buf_id[i],
							kerarg_offset[cnt],
							primem_dst_offset[i],
				...
				mvpu30_CopyArgToPrimem(primem_ptr, kerarg_ptr, kerarg_size[cnt]);

				if (mvpu_loglvl_sec >= APUSYS_MVPU_LOG_ALL)
					mvpu30_CheckPrimemArg(primem_ptr, kerarg_size[cnt]);
...
void mvpu30_region_sort(uint32_t region_num, uint32_t *region)
{
	int i = 0, j = 0;
	uint32_t tmp = 0;
...
int mvpu30_region_info_set(uint32_t buf_num,
							uint32_t *sec_chk_addr,
							uint32_t *sec_buf_size,
							uint32_t *sec_buf_attr,
							...
							bool protect_phase)
{
	...
	//Sorting
	mvpu30_region_sort(buf_io_total, buf_io_addr);
...
int mvpu30_region_merge(uint32_t region_num,
...
int mvpu30_region_mpu_set(uint32_t session_id,
						uint32_t hash_id,
						uint32_t pmu_buff,
						uint32_t buff_size,
						uint32_t *mpu_seg,
						...
						bool protect_phase)
{
	...
	//Sorting
	mvpu30_region_sort(MVPU_MPU_SEGMENT_NUMS, mpu_seg);
...
int mvpu30_add_img_mpu(void *mvpu_cmd)
{
	...
	mvpu_req = (struct mvpu_request_v30 *)mvpu_cmd;
	...
	if (mvpu_req->mpu_num > MVPU_MPU_SEGMENT_NUMS - 3) {
		...
		mvpu30_region_sort(MVPU_MPU_SEGMENT_NUMS, mvpu_req->mpu_seg);
					...
					bool protect_phase)
{
	...
	buf_io_total = mvpu30_region_info_set(buf_num,
	...
	buf_io_total_merged =
			mvpu30_region_merge(buf_io_total, buf_io_addr, buf_io_size, buf_io_addr_merged);
...
#ifdef MPU_READY
	total_mpu_cnt = mvpu30_region_mpu_set(session_id, hash_id,
					...
					uint32_t chk_size)
{
	...
	if (mvpu30_mem_use_iova(chk_base) == false)
		return 0;
	...
	case DESC_TYPE_EDMA:
		...
		for (i = 0; i < 2; i++) {
			...
			if (mvpu30_mem_use_iova(desc_addr) == false)
				continue;
						...
						uint32_t knl_num)
{
	...
	for (i = 0; i < knl_num; i = i + sample) {
		...
		for (int kreg_content_idx = 0; kreg_content_idx < 72; kreg_content_idx += 4) {
			pr_info("[MVPU][Sec] kreg[%d] dump [0x%03x] 0x%08x_%08x_%08x_%08x\n", i, kreg_content_idx*4,
				buf_ptr[kreg_content_idx], buf_ptr[kreg_content_idx+1], buf_ptr[kreg_content_idx+2], buf_ptr[kreg_content_idx+3]);
		}
		...
		if ((buf_ptr[0] & 0x2) != 0) {
			...
			ret = mvpu30_check_iova(session, cmd, DESC_TYPE_NONE, 0, chk_base, chk_size);
			if (ret != 0) {
		...
		for (j = 0; j < 6; j = j + 3) {
			...
			ret = mvpu30_check_iova(session, cmd, DESC_TYPE_GLSU, chk_num, chk_base, chk_size);
			if (ret != 0) {
				pr_info("[MVPU][Sec] [ERROR] KREG[%03d] GLSU desc num(0x%08x) base(0x%08x), size(0x%08x) FAIL\n",
					i, chk_num, chk_base, chk_size);
				goto END;
			}
			...
			ret = mvpu30_check_iova(session, cmd, DESC_TYPE_EDMA, chk_num, chk_base, chk_size);
			if (ret != 0) {
				pr_info("[MVPU][Sec] [ERROR] KREG[%03d] EDMA desc num(0x%08x) base(0x%08x), size(0x%08x) FAIL\n",
					i, chk_num, chk_base, chk_size);
				goto END;
			}
		}
...
int mvpu30_load_img(struct device *dev)
{
	...
	ker_img_offset = mvpu30_get_ker_img_offset();
	//ker_img_iova = mvpu_algo_iova + ker_img_offset;

	ptn_img_size = mvpu30_get_ptn_total_size();
	knl_img_size = mvpu30_get_kerbin_total_size();
			 ...
			 char *buf)
{
	...
	if (ptn_total_size == 0)
		ptn_total_size = mvpu30_get_ptn_total_size();
	else
		pr_info("[MVPU] already get ptn_total_size: 0x%llx\n", ptn_total_size);

	if (kerbin_total_size == 0)
		kerbin_total_size = mvpu30_get_kerbin_total_size();
	else
