...
{
	int ret = 0;
	struct mvpu_request_v30 *mvpu_req;
	...
	if (mvpu_algo_available == true)
		algo_in_img = mvpu30_get_ptn_hash(batch_name_hash);

	if (algo_in_img == true && mvpu30_get_mvpu_algo_available() == false) {
	...
	mutex_lock(&mvpu30_pool_lock);

	algo_in_pool = mvpu30_get_hash_info(session,
						batch_name_hash,
						&session_id,
						&hash_id,
						buf_num);
	...
	if (mvpu30_mem_use_iova(mvpu_req->pmu_buff)) {
	...
	if ((batch_name_hash & MVPU_BATCH_MASK) !=
		...
		for (i = 0; i < buf_num; i++) {
			...
			if (mvpu30_mem_use_iova(sec_chk_addr[i]) == false) {
				if (sec_chk_addr[i] == 0) {
					buf_cmd_cnt++;
		...
		ret = mvpu30_update_mpu(mvpu_req, session_id, hash_id,
	...
	if (algo_in_pool == false) {
		if ((buf_num == 0 || rp_num == 0) ||
			...
			knl_num = (mvpu_req->header.reg_treg_bdh_w2.dwValue & 0xFFFF0000) >> 16;
			ret = mvpu30_check_batch_flow(session, cmd_hnd->cmd, sec_level, kreg_kva, knl_num);

			if (ret != 0) {
				pr_info("[MVPU][Sec] integrity checked FAIL\n");
				pr_info("[MVPU][Sec] bundle header 0x%08x_%08x_%08x_%08x\n", mvpu_req->header.reg_treg_bdh_w0.dwValue, mvpu_req->header.reg_treg_bdh_w1.dwValue, mvpu_req->header.reg_treg_bdh_w2.dwValue, mvpu_req->header.reg_treg_bdh_w3.dwValue);
				pr_info("[MVPU][Sec] bundle header 0x%08x_%08x_%08x_%08x\n", mvpu_req->header.reg_treg_bdh_w4.dwValue, mvpu_req->header.reg_treg_bdh_w5.dwValue, mvpu_req->header.reg_treg_bdh_w6.dwValue, mvpu_req->header.reg_treg_bdh_w7.dwValue);
				goto END_WITH_MUTEX;
			}
	...
	if (algo_in_img) {
		mvpu30_get_ker_info(batch_name_hash, &ker_bin_offset, &ker_bin_num);
		...
		mvpu30_set_ker_iova(ker_bin_offset, ker_bin_num, ker_bin_each_iova);
	}
	...
	if ((sec_chk_addr != NULL) && (sec_buf_attr != NULL)) {
		...
		mvpu30_map_base_buf_id(buf_num,
						sec_chk_addr,
						sec_buf_attr,
						rp_num,
						target_buf_old_map,
	...
	if ((sec_level != SEC_LVL_PROTECT) && (algo_in_img == true)) {
		ret = mvpu30_replace_img_knl(session,
					buf_num,
					sec_chk_addr,
					sec_buf_attr,
					rp_num,
		...
		if (mvpu_req->mpu_num != 0)
			ret = mvpu30_add_img_mpu(mvpu_req);
	...
	if (session_id != 0xFFFFFFFF) {
		if (hash_id == 0xFFFFFFFF) {
			hash_id = mvpu30_get_avail_hash_id(session_id);
			...
			mvpu30_clear_hash(session_id, hash_id);

			ret = mvpu30_update_hash_pool(session,
						algo_in_img,
						session_id,
						hash_id,
						batch_name_hash,
		...
		session_id = mvpu30_get_avail_session_id();
		...
		mvpu30_update_session_id(session_id, session);
...
REPLACE_MEM:
	...
	if (algo_in_pool == true) {
		algo_all_same_buf = mvpu30_set_rp_skip_buf(session_id,
						hash_id,
						buf_num,
						sec_chk_addr,
						sec_buf_attr,
	...
	ret = mvpu30_update_new_base_addr(algo_in_img,
							algo_in_pool,
							session_id,
							hash_id,
							sec_chk_addr,
...
#ifdef FULL_RP_INFO
	ret = mvpu30_save_hash_info(session_id,
						hash_id,
						buf_num,
						rp_num,
						sec_chk_addr,
	...
	ret = mvpu30_replace_mem(session_id, hash_id,
					sec_buf_attr,
					algo_in_pool,
					rp_skip_buf,
					rp_num,
...
TRIGGER_SETTING:
#ifdef MVPU_SEC_KREG_IN_POOL
	mvpu30_get_pool_kreg_iova(&kreg_iova_pool,
						session_id,
						hash_id,
						buf_cmd_kreg);

	// set KREG iova to uP
	mvpu_req->header.reg_treg_bdh_w0.dwValue = kreg_iova_pool;
#endif

	if (kerarg_num != 0 && algo_in_pool == true) {
		ret = mvpu30_replace_kerarg(session,
					session_id, hash_id,
					kerarg_num,
					sec_chk_addr,
					kerarg_buf_id,
