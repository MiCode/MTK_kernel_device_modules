...
int mvpu30_ipi_send(uint32_t type, uint32_t dir, uint64_t *val)
{
...
static int mvpu30_rpmsg_tx_cb(struct rpmsg_device *rpdev, void *data,
...
static int mvpu30_rpmsg_rx_cb(struct rpmsg_device *rpdev, void *data,
		int len, void *priv, u32 src)
{
	struct mvpu_ipi_data *d = (struct mvpu_ipi_data *)data;
...
static struct rpmsg_driver mvpu30_rpmsg_tx_drv = {
	.drv = {
		.name = "mvpu-tx-rpmsg",
		.owner = THIS_MODULE,
	...
	.callback = mvpu30_rpmsg_tx_cb,
	.remove = mvpu_rpmsg_remove,
};

static struct rpmsg_driver mvpu30_rpmsg_rx_drv = {
	.drv = {
		.name = "mvpu-rx-rpmsg",
		.owner = THIS_MODULE,
	...
	.callback = mvpu30_rpmsg_rx_cb,
	.remove = mvpu_rpmsg_remove,
};

int mvpu30_ipi_init(void)
{
	int ret;
	...
	ret = register_rpmsg_driver(&mvpu30_rpmsg_rx_drv);
	if (ret)
		pr_info("failed to register mvpu rx rpmsg\n");

	ret = register_rpmsg_driver(&mvpu30_rpmsg_tx_drv);
	if (ret)
...
void mvpu30_ipi_deinit(void)
{
	unregister_rpmsg_driver(&mvpu30_rpmsg_tx_drv);
	unregister_rpmsg_driver(&mvpu30_rpmsg_rx_drv);
