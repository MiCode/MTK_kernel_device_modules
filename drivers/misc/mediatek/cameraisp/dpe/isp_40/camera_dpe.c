...
static int compat_get_DPE_dve_enque_req_data(
	unsigned long arg,
	struct DPE_DVERequest *data)
{

	long ret = -1;
	struct compat_DPE_DVERequest data32 = {0};

	ret = (long)copy_from_user(&data32, compat_ptr(arg),
		(unsigned long)sizeof(struct compat_DPE_DVERequest));
...
{	
	...
	if (copy_to_user(compat_ptr(arg), &data32,
			sizeof(struct compat_DPE_DVERequest)) != 0) {
		LOG_NOTICE("copy_to_user failed");
		ret = -EFAULT;
	}
	return ret;
...
static int compat_get_DPE_wmfe_enque_req_data(
	unsigned long arg,
	struct DPE_WMFERequest *data)
{

	long ret = -1;
	struct compat_DPE_WMFERequest data32 = {0};

	ret = (long)copy_from_user(&data32, compat_ptr(arg),
		(unsigned long)sizeof(struct compat_DPE_WMFERequest));
	...
	data->m_pWmfeConfig = compat_ptr(data32.m_pWmfeConfig);
	data->m_ReqNum = data32.m_ReqNum;
...
{	
	...
	if (copy_to_user(compat_ptr(arg), &data32,
			sizeof(struct compat_DPE_WMFERequest)) != 0) {
		LOG_NOTICE("copy_to_user failed");
		ret = -EFAULT;
	}
	return ret;
	...
	struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case COMPAT_DPE_DVE_ENQUE_REQ:
	{
		...
		err = compat_get_DPE_dve_enque_req_data(arg, &data);
		if (err) {
		...
		ret =
		    filp->f_op->unlocked_ioctl(filp, DPE_DVE_ENQUE_REQ,
					       (unsigned long)&data);
		err = compat_put_DPE_dve_enque_req_data(arg, &data);
		if (err) {
			LOG_INF("COMPAT_DPE_DVE_ENQUE_REQ error!!!\n");
			return err;
		}
	...
	case COMPAT_DPE_DVE_DEQUE_REQ:
	{
		...
		err = compat_get_DPE_dve_deque_req_data(arg, &data);
		if (err) {
		...
		ret =
		    filp->f_op->unlocked_ioctl(filp, DPE_DVE_DEQUE_REQ,
					       (unsigned long)&data);
		err = compat_put_DPE_dve_deque_req_data(arg, &data);
		if (err) {
			LOG_INF("COMPAT_DPE_DVE_DEQUE_REQ error!!!\n");
			return err;
		}
	...
	case COMPAT_DPE_WMFE_ENQUE_REQ:
	{
		...
		err = compat_get_DPE_wmfe_enque_req_data(arg, &data);
		if (err) {
		...
		ret =
		    filp->f_op->unlocked_ioctl(filp, DPE_WMFE_ENQUE_REQ,
					       (unsigned long)&data);
		err = compat_put_DPE_wmfe_enque_req_data(arg, &data);
		if (err) {
			LOG_INF("COMPAT_DPE_WMFE_ENQUE_REQ error!!!\n");
			return err;
		}
	...
	case COMPAT_DPE_WMFE_DEQUE_REQ:
	{
		...
		err = compat_get_DPE_wmfe_deque_req_data(arg, &data);
		if (err) {
		...
		ret =
		    filp->f_op->unlocked_ioctl(filp, DPE_WMFE_DEQUE_REQ,
					       (unsigned long)&data);
		err = compat_put_DPE_wmfe_deque_req_data(arg, &data);
		if (err) {
			LOG_INF("COMPAT_DPE_WMFE_DEQUE_REQ error!!!\n");
			return err;
		}
