...
static enum sgm37864_led_mode mLedMode = LED_NONE;
...
static int sgm37864_enable_ctrl_single(struct sgm37864_flash *flash,
...
static int sgm37864_enable_ctrl_fusion(struct sgm37864_flash *flash, bool on)
{
	int rval;
			      ...
			      enum sgm37864_led_id led_no, bool on)
{
	...
	if (mLedMode == LED_FUSION_CHANNEL && led_no == SGM37864_LED1) {
	...
	if (mLedMode == LED_DUAL_CHANNEL) {
		rval = sgm37864_enable_ctrl_single(flash, led_no, on);
	} else if (mLedMode == LED_FUSION_CHANNEL){
		rval = sgm37864_enable_ctrl_fusion(flash, on);
	} else {
...
static int sgm37864_torch_brt_ctrl_single(struct sgm37864_flash *flash,
...
static int sgm37864_torch_brt_ctrl_fusion(struct sgm37864_flash *flash, unsigned int brt)
{
	int rval;
	u8 br_bits;
	int torch_cur_avg = 0;
	...
	torch_cur_avg = brt / 2;
	if (torch_cur_avg < SGM37864_TORCH_BRT_MIN_1)
		return sgm37864_enable_ctrl(flash, SGM37864_LED0, false);

	if (flash->need_cooler == 0) {
		flash->ori_current[SGM37864_LED0] = brt;
	} else {
		if (brt > flash->target_current[SGM37864_LED0]) {
			brt = flash->target_current[SGM37864_LED0];
			pr_info("thermal limit current:%d\n", brt);
		}
	}
				 ...
				 enum sgm37864_led_id led_no, unsigned int brt)
{
	...
	if (mLedMode == LED_DUAL_CHANNEL) {
		rval = sgm37864_torch_brt_ctrl_single(flash, led_no, brt);
	} else if (mLedMode == LED_FUSION_CHANNEL) {
		rval = sgm37864_torch_brt_ctrl_fusion(flash, brt);
	}
...
static int sgm37864_flash_brt_ctrl_single(struct sgm37864_flash *flash,
				 enum sgm37864_led_id led_no, unsigned int brt)
{
	int rval;
	u8 br_bits;
...
static int sgm37864_flash_brt_ctrl_fusion(struct sgm37864_flash *flash,
				 unsigned int brt)
{
	int rval;
	u8 br_bits;
	...
	if (flash->need_cooler == 1 && brt > flash->target_current[SGM37864_LED0]) {
		brt = flash->target_current[SGM37864_LED0];
		pr_info("thermal limit current:%d\n", brt);
	}
				 ...
				 enum sgm37864_led_id led_no, unsigned int brt)
{
	...
	if (mLedMode == LED_DUAL_CHANNEL) {
		rval = sgm37864_flash_brt_ctrl_single(flash, led_no, brt);
	} else if (mLedMode == LED_FUSION_CHANNEL) {
		rval = sgm37864_flash_brt_ctrl_fusion(flash, brt);
	}
...
{
	struct sgm37864_flash *flash = NULL;
	int rval = -EINVAL;
	...
	flash = to_sgm37864_flash(ctrl, led_no);
				...
				enum sgm37864_led_id led_no)
{
	...
	v4l2_ctrl_new_std(hdl, ops, V4L2_CID_FLASH_TORCH_INTENSITY,
			  SGM37864_TORCH_BRT_MIN_1, max_torch_brt,
			  SGM37864_TORCH_BRT_STEP_1, max_torch_brt);
...
static int sgm37864_init(struct sgm37864_flash *flash)
{
	...
	flashlight_kicker_pbm_by_device_id(
				&flash->flash_dev_id[SGM37864_LED0],547);
	mdelay(1);
#endif
...
static int sgm37864_ioctl(unsigned int cmd, unsigned long arg)
{
	...
	case XIAOMI_IOC_SET_ONOFF_SINGLE_LED:
		pr_info("XIAOMI_IOC_SET_ONOFF_SINGLE_LED(%d): %d\n",
				channel, (int)fl_arg->arg);
		if ((int)fl_arg->arg) {
			sgm37864_enable_ctrl_single(sgm37864_flash_data, channel, true);
		} else {
			sgm37864_enable_ctrl_single(sgm37864_flash_data, channel, false);
		}
		break;
	...
	case XIAOMI_IOC_SET_FLASH_SINGLE_CUR:
		pr_info("XIAOMI_IOC_SET_FLASH_SINGLE_CUR(%d): %d\n",
				channel, (int)fl_arg->arg);
		sgm37864_flash_brt_ctrl_single(sgm37864_flash_data, channel, fl_arg->arg);
		break;
	case XIAOMI_IOC_SET_TORCH_SINGLE_CUR:
		pr_info("XIAOMI_IOC_SET_TORCH_SINGLE_CUR(%d): %d\n",
				channel, (int)fl_arg->arg);
		sgm37864_torch_brt_ctrl_single(sgm37864_flash_data, channel, fl_arg->arg);
		break;
	...
	case FLASH_IOC_SET_SCENARIO:
		...
		if (scenario & FLASHLIGHT_SCENARIO_CAMERA_MASK) {
			flashlight_kicker_pbm_by_device_id(
				&sgm37864_flash_data->flash_dev_id[SGM37864_LED0],4303);
		} else {
			flashlight_kicker_pbm_by_device_id(
				&sgm37864_flash_data->flash_dev_id[SGM37864_LED0],547);
		}
#endif
		break;
	default:
...
static int sgm37864_probe(struct i2c_client *client)
{
	...
	rval = sgm37864_subdev_init(flash, SGM37864_LED0, "led0");
	if (rval < 0)
		return rval;
	
	if (mLedMode == LED_DUAL_CHANNEL) {
		rval = sgm37864_subdev_init(flash, SGM37864_LED1, "led1");
		if (rval < 0)
			return rval;
	}
...
static void sgm37864_remove(struct i2c_client *client)
{
	...
	v4l2_device_unregister_subdev(&flash->subdev_led[SGM37864_LED0]);
	v4l2_ctrl_handler_free(&flash->ctrls_led[SGM37864_LED0]);
	media_entity_cleanup(&flash->subdev_led[SGM37864_LED0].entity);
	if (mLedMode == LED_DUAL_CHANNEL) {
		v4l2_device_unregister_subdev(&flash->subdev_led[SGM37864_LED1]);
		v4l2_ctrl_handler_free(&flash->ctrls_led[SGM37864_LED1]);
		media_entity_cleanup(&flash->subdev_led[SGM37864_LED1].entity);
	}
