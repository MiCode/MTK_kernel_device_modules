...
static int sku_cmdline_match(char *match_str, char *result, int size)
{
        ...
        if (!cmdline_node) {
            TMS_ERR("%s:line%d: NULL pointer!!!\n", __func__, __LINE__);
            return -EINVAL;
        }
        ret = of_property_read_string(cmdline_node, "bootargs", &cmdline);
        if (ret) {
            TMS_ERR("%s failed to read bootargs\n", __func__);
            return -EINVAL;
        }
        match = strstr(cmdline, match_str);
        if (!match) {
            TMS_ERR("Mmatch: %s fail in cmdline\n", match_str);
            return -EINVAL;
        }
        match_end = strstr((match + match_str_len), " ");
        if (!match_end) {
            TMS_ERR("Match end of : %s fail in cmdline\n", match_str);
            return -EINVAL;
        }
        len = match_end - (match + match_str_len);
        if (len < 0 || len > size) {
            TMS_ERR("Match cmdline :%s fail, len = %d\n", match_str, len);
            return -EINVAL;
        }
...
{
        char *match_str = "rsc.name=";
        char expected_str[] = "lapis_cn";
        char result[40] = {0};
        if (sku_cmdline_match(match_str, result, sizeof(result)) == 0) {
           if (strcmp(result, expected_str) == 0) {
               TMS_INFO("Chip supports download_gpio (sku=%s)\n", result);
               return true;
           }
        }
        TMS_INFO("Chip does not support download_gpio (sku=%s)\n", result);
        return false;
}
                               ...
                               const struct i2c_device_id *id, chip_t chip_type)
{
    ...
    case NXP_SN220P:
        TMS_INFO("NXP SN220P driver start probe\n");
        nfc_i2c_dev_probe(client, id);
        TMS_INFO("NXP SN220P ese driver start probe\n");
        p61_dev_init();
        break;
...
static int guidev_chip_match(struct guide_dev *gdata)
{
    ...
    for (count = 0; count < ARRAY_SIZE(info_list); count++) {
        ...
        } else if (info_list[count].type == NXP_SN220P) {
...
static void guidev_dev_remove(struct i2c_client *client)
{
    ...
    case NXP_SN220P:
        TMS_INFO("NXP SN220P nfc start removed\n");
...
static int guide_suspend(struct device *device)
{
    ...
    case NXP_SN220P:
        TMS_INFO("NXP SN220P nfc start suspend\n");
...
static int guide_resume(struct device *device)
{
    ...
    case NXP_SN220P:
        TMS_INFO("NXP SN220P nfc start resume\n");
