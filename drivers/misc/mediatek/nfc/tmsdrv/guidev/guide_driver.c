...
const char *THN31S_chip_name= "THN31SWC2";
const char *THN31F_chip_name = "THN31FGB1A";
const char *SN220X_chip_name = "SN220X";
const uint8_t THN31F_major_version = 0xD2;
const uint8_t THN31S_major_version= 0xD1;
const uint8_t SN220X_major_version = 0x01;
...
//Begin +Extb HONGMI-216534 ,dulinyu.wt,add, 20240528, set dl_support according boardid
/*****************************************************************************
* boardid check functions
*****************************************************************************/
...
int get_board_id_from_dtb(char *board_mem) {
    ...
    char *ptr=NULL;
    int res_len=-1;
    char board_id_word[]="androidboot.board_id=";
    proj_info_node  = of_find_node_by_name(NULL,"chosen");
    if (NULL == proj_info_node) {
        TMS_ERR("%s:device chosen node not exist.",__func__);
        return res_len;
    }
    res_len = of_property_read_string(proj_info_node,"hwinfo",&read_board_id);
    if (res_len != 0) {
        TMS_ERR("%s:device bootargs read fail:res_len=%d.",__func__,res_len);
        return res_len;
    }
    ptr = strstr(read_board_id,board_id_word);
    if (ptr != NULL) {
        strncpy(board_mem, ptr+strlen(board_id_word), 9); //copy the result to pointer
        board_mem[9] = '\0';
        return 0;
    }
    TMS_ERR("%s:strstr get pointer board_id failed.",__func__);
    return -1;
}
...
bool support_download_gpio(void)
{
    ...
    if (ret == 0) {
        if(!strcmp(boardid, "S99116IA1") || !strcmp(boardid, "S99116EA1") || !strcmp(boardid, "S99116AA1"))
        {
...
//End +Extb HONGMI-216534 ,dulinyu.wt,add, 20240528, set dl_support according boardid
...
static int tms_chip_identification(struct match_info info, uint8_t *buf)
{
    ...
    if (buf[cmp_byte] == THN31F_major_version) {
        hardwareinfo_set_prop(HARDWARE_NFC, THN31F_chip_name);
        TMS_INFO("major version is %02x, chip_name is %s\n", buf[cmp_byte],  THN31F_chip_name);
    }else if (buf[cmp_byte] == THN31S_major_version) {
        hardwareinfo_set_prop(HARDWARE_NFC, THN31S_chip_name);
        TMS_INFO("major version is %02x, chip_name is %s\n", buf[cmp_byte],  THN31S_chip_name);
    }else if (buf[cmp_byte] == SN220X_major_version) {
        hardwareinfo_set_prop(HARDWARE_NFC, SN220X_chip_name);
        TMS_INFO("major version is %02x, chip_name is %s\n", buf[cmp_byte],  SN220X_chip_name);
    }
    TMS_INFO("Normal end-\n");
    return ret;
}
...
static int guidev_chip_match(struct guide_dev *gdata)
{
    ...
    for (count = 0; count < ARRAY_SIZE(info_list); count++) {
        ...
        if (info_list[count].type == TMS_THN31) {
            if(info_list[count].pattern == TMS_FW) {
                TMS_INFO("no need to check chip info\n");
                ret = SUCCESS;
            } else {
