...
{
	aw_dbg("%s enter! set bst_vol = 0x%d\n", __func__, bst_vol);
			       ...
			       struct device_attribute *attr, char *buf)
{
	...
	mutex_unlock(&aw86927->lock);
/* P16 code for HQFEAT-93923 by p-dongfeiju1 at 2025/03/20 start */
	len += snprintf(buf + len, PAGE_SIZE - len, "%d\n", aw86927->f0);
/* P16 code for HQFEAT-93923 by p-dongfeiju1 at 2025/03/20 end */
	return len;
}
					...
					struct ff_effect *old)
{
	...
	aw_info("%s: effect->type=0x%x,FF_CONSTANT=0x%x,FF_PERIODIC=0x%x\n",
				   ...
				   int val)
{
	...
	aw_info("%s: effect_id=%d , activate_mode = %d val = %d\n",
