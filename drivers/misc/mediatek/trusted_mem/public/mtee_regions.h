/* SPDX-License-Identifier: GPL-2.0 */
/*
 * Copyright (c) 2019 MediaTek Inc.
 */

#ifndef MTEE_REGIONS_H
#define MTEE_REGIONS_H

enum MTEE_MCHUNKS_ID {
	/* DO NOT modify existed IDs */
	MTEE_MCHUNKS_PROT = 0,
	MTEE_MCHUNKS_HAPP = 1,
	MTEE_MCHUNKS_HAPP_EXTRA = 2,
	MTEE_MCHUNKS_SDSP = 3,
	MTEE_MCHUNKS_SDSP_SHARED_VPU_TEE = 4,
	MTEE_MCHUNKS_SDSP_SHARED_MTEE_TEE = 5,
	MTEE_MCHUNKS_SDSP_SHARED_VPU_MTEE_TEE = 6,
	MTEE_MCHUNKS_CELLINFO = 7,
	MTEE_MCHUNKS_SVP = 8,
	MTEE_MCHUNKS_WFD = 9,
	MTEE_MCHUNKS_SAPU_DATA_SHM = 10,
	MTEE_MCHUNKS_SAPU_ENGINE_SHM = 11,
	MTEE_MCHUNKS_TUI = 12,
	MTEE_MCHUNKS_AP_MD_SHM = 14,
	MTEE_MCHUNKS_AP_SCP_SHM = 15,
	MTEE_MCHUNKS_TEE = 16,

	MTEE_MCHUNKS_MAX_ID = 17,
	MTEE_MCUHNKS_INVALID = 0xFFFFFFFF,
};

enum MTEE_MCHUNKS_HANDLE_TYPE {
	/* DO NOT modify existed types */
	MTEE_MCHUNKS_HANDLE_DEFAULT = 0,
	MTEE_MCHUNKS_HANDLE_GZ32 = 1,
	MTEE_MCHUNKS_HANDLE_FFA = 2,

	MTEE_MCHUNKS_HANDLE_MAX_TYPE = 3,
	MTEE_MCHUNKS_HANDLE_INVALID_TYPE = 0xFFFFFFFF,
};

#endif /* end of MTEE_REGIONS_H */
