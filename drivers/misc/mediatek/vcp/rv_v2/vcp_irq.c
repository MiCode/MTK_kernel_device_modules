...
void wait_vcp_ready_to_reboot(void)
{
	...
	if (retry == 0) {
		C0_status = readl(R_CORE0_STATUS);
		if (vcpreg.core_nums == 2)
			C1_status = readl(R_CORE1_STATUS);

		pr_notice("[VCP] wakeup timeout C0 H0:0x%lx H1:0x%lx C1 H0:0x%lx H1:0x%lx Status: 0x%x 0x%x\n",
			C0_H0, C0_H1, C1_H0, C1_H1, C0_status, C1_status);
	}
...
static void vcp_A_wdt_handler(struct tasklet_struct *t)
{
	...
	pr_notice("[VCP] %s trigger wdt (%x,%x)\n", reg1?"MMUP":"VCP", reg1, reg0);
	...
	if (vcp_set_reset_status() == RESET_STATUS_STOP) {
		...
		if (reg1)
			vcp_send_reset_wq(MMUP_ID, RESET_TYPE_WDT);
		else
			vcp_send_reset_wq(VCP_ID, RESET_TYPE_WDT);
	} else
