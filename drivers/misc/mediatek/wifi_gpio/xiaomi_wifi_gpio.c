...
/*****************************************************************************
* country code check functions
*****************************************************************************/
//return failed: < 0  success:0    
static int get_hwc_from_dtb(char *board_mem) {
    ...
    pr_err("%s:get chosen node and hwinfo success, hwinfo=%s.\n",__func__,read_hwc);
    ptr = strstr(read_hwc,hwc_word);
    if (ptr != NULL) {
        ptr_e = strstr(ptr," "); //get the end position of hwc
        if (ptr_e != NULL) {
             pr_err("%s:strstr get kongge pointer from ptr.\n",__func__);
             res_len = ptr_e - (ptr+strlen(hwc_word));
             if (res_len < 0) {
                 pr_err("%s, get result failed,res_len=%d.\n", __func__,res_len);
                 return res_len;
             }
             strncpy(board_mem, ptr+strlen(hwc_word), res_len); //copy the result to pointer
             board_mem[res_len] = '\0';
             //pr_info("%s, get result %s .\n", __func__,board_mem);
             pr_err("%s, get result %s .\n", __func__,board_mem);
             return 0;
        }
        pr_err("%s:strstr get pointer ptr_e failed.",__func__);
    }
...
static bool check_country_cn(void){
	int ret;
	char hwc[16]={0};
	//memset(hwc, 0 , sizeof(hwc));
	ret = get_hwc_from_dtb(hwc);
	if (ret == 0) {
		pr_err("hwc get success: %s.",hwc);
		if (!strcmp(hwc,"CN"))
		{
			pr_err("hwc get success CN RETURN: %s.",hwc); 
			return true;
		}
	}
	return false;
...
static int xiaomi_wifi_gpio_probe(struct platform_device *pdev)
{
	...
	dev_info(dev, "success to get debounce-time = %d.\n",ant_data->debounce_time);
	gpio_num = of_get_named_gpio(np, "status-gpio", 0);
	if (gpio_num < 0) {
		dev_err(dev, "Failed to get ant gpio %d\n", gpio_num);
		return gpio_num;
	}
	ret = devm_gpio_request(dev, gpio_num, "status-gpio");
	if (ret) {
...
static int __init wifi_gpio_driver_init(void)
{
	...
	pr_err("%s driver init finished.", __func__);
	err = genl_register_family(&sar_fam);
	if (err) {
		pr_err("%s driver init failed for genl_register_family failed,err=%d.\n", __func__,err);
		return err;
	}
