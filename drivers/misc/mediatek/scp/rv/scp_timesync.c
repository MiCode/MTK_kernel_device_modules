...
#define scp_ts_write(id, val) \
	writel(val, id)

#define TIMESYNC_TAG	"[SCP_TS]"
...
static u8 scp_base_ver;
static u32 timesync_max_ver;
static u32 timesync_header_ver_ofs;
static u32 timesync_header_ver_mask;

/* timesync definition */
bool scp_timesync_flag;

static int scp_timesync_notify_callback(struct notifier_block *this,
					unsigned long event, void *ptr);

static struct notifier_block scp_timesync_notify = {
	.notifier_call = scp_timesync_notify_callback,
};
...
static void scp_ts_update(int suspended, u64 tick, u64 ts)
{
	u32 header, val;

	scp_base_ver = (scp_base_ver + 1)%(timesync_max_ver+1);

	/* make header: freeze and version */
	header = suspended ? TIMESYNC_HEADER_FREEZE : 0;

	header |= ((scp_base_ver << timesync_header_ver_ofs) &
		timesync_header_ver_mask);

	/* write scp mbox, need to awawke scp avoid peri off */
	if (scp_awake_lock((void *)SCP_A_ID) == -1) {
		pr_notice("%s %s: awake scp fail\n",TIMESYNC_TAG ,__func__);
		return;
	}
	...
	val |= header;
	scp_ts_write(SCP_TS_MBOX_TICK_H, val);

	/* fix update sequence to promise atomicity */
	mb();

	val = tick & 0xFFFFFFFF;
	scp_ts_write(SCP_TS_MBOX_TICK_L, val);
	...
	/* update ts, l -> h */
	val = ts & 0xFFFFFFFF;
	scp_ts_write(SCP_TS_MBOX_TS_L, val);
	...
	val |= header;
	scp_ts_write(SCP_TS_MBOX_TS_H, val);

	/* fix update sequence to promise atomicity */
	mb();

	if (scp_awake_unlock((void *)SCP_A_ID) == -1)
		pr_notice("%s %s: awake unlock fail\n",TIMESYNC_TAG ,__func__);
...
{
	timesync_workqueue = create_workqueue("scp_ts_wq");
	...
	scp_A_register_notify(&scp_timesync_notify);
	// timesync_sync_base(TIMESYNC_FLAG_SYNC);
