...
#if IS_ENABLED(CONFIG_MQOS_MTK)
#include "../../../../drivers/mihw/include/mqos_sched.h"
#endif
...
{
#if IS_ENABLED(CONFIG_MQOS_MTK)
	if (is_qi_feat_enabled(QI_RWSEM_INHERIT) && is_qos_task(current))
		return;
#endif
...
{
#if IS_ENABLED(CONFIG_MQOS_MTK)
	if (is_qi_feat_enabled(QI_BINDER) && is_qos_task(t->from ? t->from->task : NULL))
		return;
#endif
						...
						struct plist_head *hb_chain, bool *already_on_hb)
{
	...
	if (*already_on_hb)
		return;
#if IS_ENABLED(CONFIG_MQOS_MTK)
	// qos task, return and go QI_FUTEX_INHERIT
	if (is_qi_feat_enabled(QI_FUTEX_INHERIT) && is_qos_task(current))
		return;
#endif
