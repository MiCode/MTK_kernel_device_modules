...
#define KMSG_COMPONENT "mtk_zram"
...
static void kcompressd_try_to_sleep(struct zram *zram, int hid)
{
	DEFINE_WAIT(wait);
	...
	prepare_to_wait(&zram->kcompressd_wait[hid], &wait, TASK_INTERRUPTIBLE);
...
static int kcompressd(void *p)
{
	...
	struct kcompressd_para *para = (struct kcompressd_para *)p;
	struct zram *zram = para->zram;
	...
	for ( ; ; ) {
		...
		kcompressd_try_to_sleep(zram, hid);
		ret = try_to_freeze();
...
static int update_nr_kcompressd(int id, void *ptr, void *data)
{
	...
	int nr_threads = nr_kcompressd_current;
	struct zram *zram = (struct zram *)ptr;

	last_idx = nr_threads - 1;
	if (nr_kcompressd < nr_threads) {
		...
		increase = nr_kcompressd - nr_threads;
		...
		const struct kernel_param *kp)
{
	...
	if (ret < 0 || temp > MAX_KCOMPRESSD_NUM)
		return ret;

	*((unsigned int *)kp->arg) = temp;

	if (nr_kcompressd_current == nr_kcompressd)
		return 0;

	idr_for_each(&mtk_zram_index_idr, &update_nr_kcompressd, NULL);

	pr_info("kswapd_thread count changed, old:%d new:%d\n",
		nr_kcompressd_current, nr_kcompressd);
	nr_kcompressd_current = nr_kcompressd;
...
static int zram_add(void)
{
	...
	ret = idr_alloc(&mtk_zram_index_idr, zram, 0, 0, GFP_KERNEL);
	if (ret < 0)
		goto out_free_dev;
	device_id = ret;
...
out_cleanup_disk:
	put_disk(zram->disk);
out_free_idr:
	idr_remove(&mtk_zram_index_idr, device_id);
out_free_dev:
	kfree(zram);
	return ret;
}
			...
			size_t count)
{
	...
	zram = idr_find(&mtk_zram_index_idr, dev_id);
	if (zram) {
		ret = zram_remove(zram);
		if (!ret)
			idr_remove(&mtk_zram_index_idr, dev_id);
	} else {
		ret = -ENODEV;
	}
...
static struct attribute *mtk_zram_control_class_attrs[] = {
	&class_attr_hot_add.attr,
	&class_attr_hot_remove.attr,
	NULL,
};
ATTRIBUTE_GROUPS(mtk_zram_control_class);

static struct class mtk_zram_control_class = {
	.name		= "mtk_zram-control",
	.class_groups	= mtk_zram_control_class_groups,
};
...
{
	class_unregister(&mtk_zram_control_class);
	idr_for_each(&mtk_zram_index_idr, &zram_remove_cb, NULL);
	zram_debugfs_destroy();
	idr_destroy(&mtk_zram_index_idr);
...
static int __init zram_init(void)
{
	...
	ret = cpuhp_setup_state_multi(CPUHP_ZCOMP_PREPARE, "block/mtk_zram:prepare",
				      zcomp_cpu_up_prepare, zcomp_cpu_dead);
	if (ret < 0)
		return ret;

	ret = class_register(&mtk_zram_control_class);
	if (ret) {
		pr_err("%s: Unable to register zram-control class\n", __func__);
		cpuhp_remove_multi_state(CPUHP_ZCOMP_PREPARE);
		return ret;
	}
