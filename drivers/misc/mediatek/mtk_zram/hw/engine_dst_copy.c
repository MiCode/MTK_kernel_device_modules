...
size_t zram_get_obj_size_export(struct zram *zram, u32 index);
static char srcbuf[4096];
int simulate_dc_decompress(struct hwfifo *fifo, uint32_t entry)
{
	...
	memcpy(srcbuf, cmd_buf_addr_to_va((uint32_t *)&cmdp->word_2_value), comp_len);
...
{
	if (comp_len > SZ_4K)
		pr_info("%s: comp_len(%u)\n", __func__, comp_len);

	return ENGINE_BUF_ENABLE;
}

static void compressed_to_dst(struct compress_cmd *cmdp, unsigned int bufsel,
		void *compressed, unsigned int comp_len)
{
	uint32_t *dst_addr = (uint32_t *)&cmdp->word_7_value;
	void *dst, *src = compressed;

	if (comp_len > SZ_4K)
		pr_info("%s: comp_len(%u) bufsel(%x)\n", __func__, comp_len, bufsel);

	dst = cmd_buf_addr_to_va(dst_addr);
	memcpy(dst, src, comp_len);
}

int simulate_dc_compress(struct hwfifo *fifo, uint32_t entry)
{
...
static void __dcomp_process_completed_cmd(struct hwfifo *fifo, uint32_t entry, bool async)
{
	...
	if (err && async) {
		...
		// CHECK DEC ERR
		if (simulate_dc_decompress(fifo, entry)) {
