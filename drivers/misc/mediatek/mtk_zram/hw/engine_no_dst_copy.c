...
#define ENGINE_NUM_OF_BUF_SIZES_FOR_NDC	(ENGINE_NUM_OF_BUF_SIZES - 1)

/************************************************
 ***************** Test only ********************
 ************************************************/

static int sw_decompress(struct zram *zram, struct page *page, void *src, unsigned int comp_len)
{
	int ret = 0;
	...
	ret = zcomp_decompress(zstrm, src, comp_len, dst);
	kunmap_local(dst);
...
static char srcbuf[4096];
int simulate_ndc_decompress(struct hwfifo *fifo, uint32_t entry)
{
	...
	unsigned int comp_len = zram_get_obj_size_export(zram, index);
	struct hwcomp_buf_t src;
	int ret;

	memcpy(&src, &(ZRAM_TE_NDC(zram, index))->compressed, sizeof(struct hwcomp_buf_t));
	ret = hwcomp_buf_read(srcbuf, &src, comp_len);
	if (ret) {
		pr_info("%s: failed to convert buffer: %d.\n", __func__, ret);
		goto exit;
	}

	ret = sw_decompress(zram, pp_info->page, srcbuf, comp_len);

exit:
	if (ret)
		cmdp->status = DCOMP_CMD_ERROR;
	else
		cmdp->status = DCOMP_CMD_DECOMPRESSED;
...
static bool repeat_pattern_check(struct compress_cmd *cmdp, struct page *page)
{
	void *src;
	...
	long_val = src;
	last_pos = 4096 / sizeof(*long_val) - 1;
	if (long_val[0] != long_val[last_pos])
		goto exit;

	for (pos = 1; pos < last_pos; pos++) {
		if (long_val[pos] != long_val[0])
			goto exit;
	}

	byte_val = src;
	last_pos = sizeof(unsigned long) / sizeof(*byte_val) - 1;
	for (pos = 1; pos <= last_pos; pos++) {
		if (byte_val[pos] != byte_val[0])
			goto exit;
	}

	repeat = true;
	cmdp->repeat_pattern = byte_val[0];
...
static int sw_compress(struct zram *zram, struct page *page, unsigned int *comp_len, void **compressed)
{
	int ret = 0;
	struct zcomp_strm *zstrm;
	void *src;
	...
	*compressed = zstrm->buffer;

	return ret;
}

static void sw_compress_end(struct zram *zram)
{
	zcomp_stream_put(zram->comps[ZRAM_PRIMARY_COMP]);
}

static uint32_t compose_compress_bufsel(unsigned int comp_len)
{
	unsigned int rounded_comp_len;
	unsigned int round_size = ENGINE_MIN_BUF_SIZES;
	unsigned int tmp_bufsel, bufsel = 0x0;
	unsigned int bit_sets = 0;
	int bit;
#ifdef ZRAM_ENGINE_DEBUG
	int nbits;
	char tmp_bins[8], bins[8];
#endif

	do {
		rounded_comp_len = round_up(comp_len, round_size);
		tmp_bufsel = rounded_comp_len >> 6;
		for (bit = 0; bit < ENGINE_NUM_OF_BUF_SIZES_FOR_NDC; bit++) {
			if (tmp_bufsel & (0x1 << bit))
				bit_sets++;
		}

		/* Found valid bufsel */
		if (bit_sets <= ENGINE_MAX_BUF_SELECTED)
			break;
	...
	for (bit = 0; bit < ENGINE_NUM_OF_BUF_SIZES_FOR_NDC; bit++) {
		if (tmp_bufsel & (0x1 << bit))
			bufsel |= 0x1 << (ENGINE_NUM_OF_BUF_SIZES_FOR_NDC - 1 - bit);
	}

#ifdef ZRAM_ENGINE_DEBUG
	nbits = printbinary(tmp_bins, tmp_bufsel, 7);
	nbits = printbinary(bins, bufsel, 7);
	pr_info("%s: comp_len(%u)(%s)(%s)\n", __func__, comp_len, tmp_bins, bins);
#endif

	return bufsel;
}

extern const unsigned int bufsz[];
static void compressed_to_dst(struct compress_cmd *cmdp, unsigned int bufsel,
		void *compressed, unsigned int comp_len)
{
	uint32_t *dst_addr = (uint32_t *)&cmdp->word_4_value;
	void *dst, *src = compressed;
	unsigned int remaining = comp_len;
	unsigned int dst_size = bufsz[0], copy_size = dst_size;
	int bit;

	for (bit = 0; bit < ENGINE_NUM_OF_BUF_SIZES_FOR_NDC; bit++) {
		if (bufsel & (0x1 << bit)) {
			dst = cmd_buf_addr_to_va(dst_addr);
	...
	if ((src - compressed) != comp_len)
		pr_info("%s: src(%lx) compressed(%lx) comp_len(%d) bufsel(%x)\n",
			__func__, (unsigned long)src, (unsigned long)compressed, comp_len, bufsel);
}

int simulate_ndc_compress(struct hwfifo *fifo, uint32_t entry)
{
	...
	repeat = repeat_pattern_check(cmdp, pp_info->page);
	if (repeat) {
		cmdp->status = COMP_CMD_REPEATED;
		goto exit;
	}

	ret = sw_compress(pp_info->zram, pp_info->page, &comp_len, &compressed);

	if (ret) {
		cmdp->status = COMP_CMD_ERROR;
	} else {
		cmdp->compr_size = comp_len;

		if (comp_len > ENGINE_MAX_COMPRESSIBLE_SIZE) {
			cmdp->status = COMP_CMD_INCOMPRESSIBLE;
		} else {
			cmdp->status = COMP_CMD_COMPRESSED;
			cmdp->bufsel = compose_compress_bufsel(comp_len);
			/* Copy to dst buffers in cmdp according to bufsel */
			compressed_to_dst(cmdp, cmdp->bufsel, compressed, comp_len);
		}
	}

	sw_compress_end(pp_info->zram);
