...
int mbraink_v6991_set_pmu_enable(bool enable)
{
	int ret = 0;
...
int mbraink_v6991_get_pmu_info(struct mbraink_pmu_info *pmuInfo)
{
	int cpu;
	int i = 0;
...
static struct mbraink_pmu_ops mbraink_v6991_pmu_ops = {
	.set_pmu_enable = mbraink_v6991_set_pmu_enable,
	.get_pmu_info = mbraink_v6991_get_pmu_info,
};

int mbraink_v6991_pmu_init(void)
{
	int ret = 0;

	ret = register_mbraink_pmu_ops(&mbraink_v6991_pmu_ops);
	return ret;
}

int mbraink_v6991_pmu_deinit(void)
{
	int ret = 0;
