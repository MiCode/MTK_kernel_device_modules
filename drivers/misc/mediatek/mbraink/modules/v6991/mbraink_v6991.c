...
static ssize_t mbraink_platform_info_show(struct device *dev,
								...
								char *buf)
{
	return snprintf(buf, PAGE_SIZE, "show mbraink v6991 information...\n");
}
...
static int mbraink_v6991_probe(struct platform_device *pdev)
{
	...
	ret = mbraink_v6991_power_init(mbraink_v6991_device);
	if (ret)
	...
	ret = mbraink_v6991_pmu_init();
	if (ret)
		pr_notice("[MBK_v6991] mbraink v6991 pmu init failed.\n");

	ret = mbraink_v6991_touch_init();
	if (ret)
		pr_notice("[MBK_v6991] mbraink v6991 touch init failed.\n");

	return ret;
}

static void mbraink_v6991_remove(struct platform_device *pdev)
{
	struct device *mbraink_v6991_device = &pdev->dev;
	...
	mbraink_v6991_pmu_deinit();
	mbraink_v6991_touch_deinit();
}
