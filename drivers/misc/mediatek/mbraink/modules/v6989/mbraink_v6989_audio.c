...
static int mbraink_v6989_audio_getIdleRatioInfo(
...
static struct mbraink_audio_ops mbraink_v6989_audio_ops = {
	.setUdmFeatureEn = NULL,
	.getIdleRatioInfo = mbraink_v6989_audio_getIdleRatioInfo,
};

int mbraink_v6989_audio_init(void)
{
	int ret = 0;

	ret = register_mbraink_audio_ops(&mbraink_v6989_audio_ops);
	return ret;
}

int mbraink_v6989_audio_deinit(void)
{
	int ret = 0;
