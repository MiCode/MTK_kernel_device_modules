...
static void mbraink_v6899_gpu_setOpMode(int OpMode)
{
	gOpMode = OpMode;
}

static int mbraink_v6899_gpu_getOpMode(void)
{
	return gOpMode;
}

static ssize_t mbraink_v6899_gpu_getTimeoutCouterReport(char *pBuf)
{
	ssize_t size = 0;
	...
	int perf_idx, int sbe_ctrl, unsigned long long ts)
{
	...
	opMode = (sbe_ctrl > 0) ? mbraink_op_mode_sbe : mbraink_v6899_gpu_getOpMode();
...
static int mbraink_v6899_gpu_setFeatureEnable(bool bEnable)
{
	if (bEnable == true) {
#if IS_ENABLED(CONFIG_MTK_FPSGO_V3) || IS_ENABLED(CONFIG_MTK_FPSGO)
...
void mbraink_v6899_gpu_setQ2QTimeoutInNS(unsigned long long q2qTimeoutInNS)
{
	gq2qTimeoutInNs = q2qTimeoutInNS;
}

unsigned long long mbraink_v6899_gpu_getQ2QTimeoutInNS(void)
{
	return gq2qTimeoutInNs;
}

void mbraink_v6899_gpu_fpsgoSetGameMode(int isGameMode)
{
	gisGameMode= isGameMode;
}

static int mbraink_v6899_gpu_getOppInfo(struct mbraink_gpu_opp_info *gOppInfo)
{
	int ret = 0;
	int i = 0;
...
static int mbraink_v6899_gpu_getStateInfo(struct mbraink_gpu_state_info *gStateInfo)
{
	int ret = 0;
...
static int mbraink_v6899_gpu_getLoadingInfo(struct mbraink_gpu_loading_info *gLoadingInfo)
{
	int ret = 0;
...
void mbraink_v6899_gpu_setPerfIdxTimeoutInNS(unsigned long long perfIdxTimeoutInNS)
{
	gperfIdxTimeoutInNs = perfIdxTimeoutInNS;
}

void mbraink_v6899_gpu_setPerfIdxLimit(int perfIdxLimit)
{
	if (perfIdxLimit <= 100)
		gperfIdxLimit = perfIdxLimit;
}

void mbraink_v6899_gpu_dumpPerfIdxList(void)
{
	struct mbraink_gpu_perfidx_info *iter;
	int n = 0;
...
static struct mbraink_gpu_ops mbraink_v6899_gpu_ops = {
	.setFeatureEnable = mbraink_v6899_gpu_setFeatureEnable,
	.getTimeoutCounterReport = mbraink_v6899_gpu_getTimeoutCouterReport,
	.getOppInfo = mbraink_v6899_gpu_getOppInfo,
	.getStateInfo = mbraink_v6899_gpu_getStateInfo,
	.getLoadingInfo = mbraink_v6899_gpu_getLoadingInfo,
	.setOpMode = mbraink_v6899_gpu_setOpMode,
};

int mbraink_v6899_gpu_init(void)
{
	int ret = 0;

	ret = register_mbraink_gpu_ops(&mbraink_v6899_gpu_ops);
	return ret;
}

int mbraink_v6899_gpu_deinit(void)
{
	int ret = 0;

	ret = unregister_mbraink_gpu_ops();
	ret = mbraink_v6899_gpu_setFeatureEnable(false);
	return ret;
}
