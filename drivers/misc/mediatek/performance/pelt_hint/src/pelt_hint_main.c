	...
	struct task_struct *p, bool task_sleep, int *ret)
{
	...
	chi_main_trace("[pelt_hint] pid:%d gai_task_flag:%d gai_cpu_boost_enable:%d",
	...
	if (gai_cpu_boost_enable == 2 && !chi_tsk->gai_task_flag) {
		if (p->tgid == detect_fork_tgid) {
			...
			chi_main_trace("[pelt_hint] pid:%d is new task, gai_task_flag:%d",
	...
	if (ewma & UTIL_AVG_UNCHANGED) {
		chi_main_trace("[pelt_hint] UTIL_AVG_UNCHANGED mask not reset pid:%d ewma:%u", p->pid, ewma);
		return;
	}

	dequeued = task_util(p);
	chi_main_trace("[pelt_hint] pid:%d util:%u util_est:%u runnable_avg:%lu cpu:%d(%d)",
	...
	ewma >>= final_ratio;
	ewma++;
	chi_main_trace("[pelt_hint] pid:%d final_ratio:%u util_est:%u", p->pid, final_ratio, ewma);
done:
