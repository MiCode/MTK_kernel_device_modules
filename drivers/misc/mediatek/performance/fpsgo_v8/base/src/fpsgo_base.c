...
static struct rb_root fps_control_by_tgid_tree;
static struct rb_root fps_control_by_pid_tree;
...
#endif
static struct rb_root no_boost_info_tree;
static struct rb_root jank_detection_info_tree;
static struct rb_root wait_fpsgo_enable_tree;
...
void fpsgo_other2comp_flush_acquire_table(void)
{
	fpsgo_render_tree_lock(__func__);
	fpsgo_delete_acquire_info(2, 0, 0);
	fpsgo_render_tree_unlock(__func__);
}
EXPORT_SYMBOL(fpsgo_other2comp_flush_acquire_table);
	...
	unsigned long long buffer_id, int force)
{
	...
	iter_thr->render_key.key2 = buffer_id;
	iter_thr->buffer_id = buffer_id;
	iter_thr->tgid = tgid;
	iter_thr->frame_type = BY_PASS_TYPE;
	iter_thr->target_render_flag = 0;
	iter_thr->producer_info_ready = 0;
	...
	unsigned long long *bufID_arr, int max_num)
{
	...
	for (rbn = rb_first(&render_pid_tree); rbn; rbn = rb_next(rbn)) {
		...
		if (iter->tgid != tgid)
			continue;
		if (index < max_num) {
			pid_arr[index] = iter->pid;
			bufID_arr[index] = iter->buffer_id;
			index++;
		}
	}
...
void fpsgo_delete_render_info(struct render_info *iter)
{
	...
	if (max_ret && iter->pid == max_pid && iter->buffer_id == max_buffer_id)
		check_max_blc = 1;
...
int is_to_delete_fpsgo_attr(struct fpsgo_attr_by_pid *fpsgo_attr)
{
	...
	if	(boost_attr.aa_enable_by_pid == BY_PID_DEFAULT_VAL &&
			boost_attr.rescue_enable_by_pid == BY_PID_DEFAULT_VAL &&
			boost_attr.rescue_second_enable_by_pid == BY_PID_DEFAULT_VAL &&
			boost_attr.rescue_second_time_by_pid == BY_PID_DEFAULT_VAL &&
			boost_attr.rescue_second_group_by_pid == BY_PID_DEFAULT_VAL &&
...
static void fpsgo_delete_oldest_fps_control_info(int mode)
{
	unsigned long long min_ts = ULLONG_MAX;
	struct fps_control_info *min_iter = NULL, *tmp_iter = NULL;
	struct rb_root *rbr = NULL;
	struct rb_node *rbn = NULL;

	// mode = 0 by process control, mode = 1 by render control
	rbr = mode ? &fps_control_by_pid_tree : &fps_control_by_tgid_tree;
	for (rbn = rb_first(rbr); rbn; rbn = rb_next(rbn)) {
		tmp_iter = rb_entry(rbn, struct fps_control_info, entry);
	...
	if (min_iter) {
		rb_erase(&min_iter->entry, rbr);
		kfree(min_iter);
		total_fps_control_num--;
	}
}
	...
	unsigned long long buffer_id, int force)
{
	struct rb_node **p;
	struct rb_node *parent = NULL;
	struct fps_control_info *tmp_iter = NULL;
	struct fbt_render_key tmp_key = {.key1 = pid, .key2 = buffer_id};

	// mode = 0 by process control, mode = 1 by render control
	p = mode ? &fps_control_by_pid_tree.rb_node :
				&fps_control_by_tgid_tree.rb_node;
	while (*p) {
		parent = *p;
		tmp_iter = rb_entry(parent, struct fps_control_info, entry);

		if (render_key_compare(tmp_key, tmp_iter->key) < 0)
			p = &(*p)->rb_left;
		else if (render_key_compare(tmp_key, tmp_iter->key) > 0)
			p = &(*p)->rb_right;
		else {
			tmp_iter->ts = fpsgo_get_time();
			return tmp_iter;
		}
	}
	...
	tmp_iter = kzalloc(sizeof(struct fps_control_info), GFP_KERNEL);
	if (!tmp_iter)
		return NULL;

	tmp_iter->pid = pid;
	tmp_iter->buffer_id = buffer_id;
	tmp_iter->ts = fpsgo_get_time();
	tmp_iter->key = tmp_key;

	rb_link_node(&tmp_iter->entry, parent, p);
	rb_insert_color(&tmp_iter->entry, mode ? &fps_control_by_pid_tree : &fps_control_by_tgid_tree);
	total_fps_control_num++;

	if (total_fps_control_num >= FPSGO_MAX_TREE_SIZE)
		fpsgo_delete_oldest_fps_control_info(mode);
...
{
	struct fps_control_info *data;

	// mode = 0 by process control, mode = 1 by render control
	data = fpsgo_search_and_add_fps_control_info(mode, pid, buffer_id, 0);
	if (data) {
		rb_erase(&data->entry, mode ? &fps_control_by_pid_tree : &fps_control_by_tgid_tree);
		kfree(data);
		total_fps_control_num--;
	}
}
...
{
	int index = 0;
	struct fps_control_info *iter = NULL;
	struct rb_root *rbr = NULL;
	struct rb_node *rbn = NULL;
	...
	rbr = mode ? &fps_control_by_pid_tree : &fps_control_by_tgid_tree;
	for (rbn = rb_first(rbr); rbn; rbn = rb_next(rbn)) {
		iter = rb_entry(rbn, struct fps_control_info, entry);
		arr[index].pid = iter->pid;
		arr[index].buffer_id = iter->buffer_id;
...
static void fpsgo_check_hwui_info_status(void)
{
	struct hwui_info *iter = NULL;
	struct rb_node *rbn = NULL;

	rbn = rb_first(&hwui_info_tree);
	while (rbn) {
		iter = rb_entry(rbn, struct hwui_info, entry);
		if (fpsgo_get_tgid(iter->pid))
			rbn = rb_next(rbn);
		else {
			rb_erase(rbn, &hwui_info_tree);
			kfree(iter);
			rbn = rb_first(&hwui_info_tree);
		}
	}
...
static void fpsgo_check_user_render_status(void)
{
	...
	while (rbn) {
		...
		if (local_tgid) {
			...
			fpsgo_thread_unlock(&iter->thr_mlock);
			fpsgo_render_tree_unlock(__func__);
			fpsgo_other2comp_user_close(local_tgid, local_rtid, local_bufID);
			fpsgo_render_tree_lock(__func__);
			rbn = rb_first(&render_pid_tree);
		}
	}
...
int fpsgo_check_render_info_status(void)
{
	...
	unsigned long long *delete_bufID = NULL;
	...
	delete_pid = kcalloc(FPSGO_MAX_RENDER_INFO_SIZE, sizeof(int), GFP_KERNEL);
	delete_bufID = kcalloc(FPSGO_MAX_RENDER_INFO_SIZE, sizeof(unsigned long long), GFP_KERNEL);
	...
	while (n) {
		...
		if (iter->t_enqueue_start < expire_ts &&
			...
			if (delete_num < FPSGO_MAX_RENDER_INFO_SIZE) {
				delete_pid[delete_num] = iter->pid;
				delete_bufID[delete_num] = iter->buffer_id;
				delete_num++;
			}
			fpsgo_thread_unlock(&iter->thr_mlock);
			fpsgo_delete_render_info(iter);
			n = rb_first(&render_pid_tree);
		} else {
...
void fpsgo_clear(void)
{
	...
	while (n) {
		...
		if (delete_pid && delete_bufID && delete_num < FPSGO_MAX_RENDER_INFO_SIZE) {
			delete_pid[delete_num] = iter->pid;
			delete_bufID[delete_num] = iter->buffer_id;
			delete_num++;
		}
...
int fpsgo_get_all_render_info(struct hlist_head *arr)
{
	...
	for (rbn = rb_first(&render_pid_tree); rbn; rbn = rb_next(rbn)) {
		iter = rb_entry(rbn, struct render_info, render_key_node);
		hlist_add_head(&iter->render_list_node, arr);
		index++;
	}
...
{
	int index = 0;
	struct acquire_info *iter;
	struct rb_node *rbn;

	for (rbn = rb_first(&acquire_info_tree); rbn; rbn = rb_next(rbn)) {
		iter = rb_entry(rbn, struct acquire_info, entry);
		hlist_add_head(&iter->list_node, arr);
		index++;
	}
...
{
	struct rb_node **p = &no_boost_info_tree.rb_node;
	struct rb_node *parent = NULL;
	struct no_boost_info *iter = NULL;

	while (*p) {
		parent = *p;
		iter = rb_entry(parent, struct no_boost_info, entry);

		if (mode < iter->mode)
			p = &(*p)->rb_left;
		else if (mode > iter->mode)
			p = &(*p)->rb_right;
		else {
			if (id < iter->specific_id)
				p = &(*p)->rb_left;
			else if (id > iter->specific_id)
				p = &(*p)->rb_right;
			else
				return iter;
		}
	...
	if (!create || total_no_boost_info_num >= FPSGO_MAX_NO_BOOST_INFO_SIZE)
		return NULL;

	iter = kzalloc(sizeof(struct no_boost_info), GFP_KERNEL);
	if (!iter)
		return NULL;
	...
	rb_link_node(&iter->entry, parent, p);
	rb_insert_color(&iter->entry, &no_boost_info_tree);
	total_no_boost_info_num++;
...
void fpsgo_delete_no_boost_info(int mode, int id)
{
	...
	iter = fpsgo_get_no_boost_info(mode, id, 0);
	if (iter) {
		rb_erase(&iter->entry, &no_boost_info_tree);
		kfree(iter);
		total_no_boost_info_num--;
	}
}
...
int fpsgo_get_all_no_boost_info(int max_count, struct no_boost_info *arr)
{
	...
	for (rbn = rb_first(&no_boost_info_tree); rbn; rbn = rb_next(rbn)) {
		iter = rb_entry(rbn, struct no_boost_info, entry);
		if (index >= max_count)
			break;
		memcpy(&arr[index], iter, sizeof(struct no_boost_info));
		index++;
	}
	...
	struct render_frame_info *frame_info_arr)
{
	...
	for (i = 0; i < index; i++) {
		...
		if (test_bit(GET_FPSGO_QUEUE_FPS, &mask) ||
			...
			memset(&info, 0, sizeof(struct render_fps_info));
			fpsgo_other2fstb_get_fps_info(f_iter->pid, f_iter->buffer_id, &info);
			f_iter->queue_fps = info.queue_fps;
			f_iter->target_fps = info.raw_target_fps;
			f_iter->target_fps_diff = info.target_fps_diff;
		}
		...
		char *buf)
{
	...
	pos += length;
	length = scnprintf(temp + pos, FPSGO_SYSFS_MAX_BUFF_SIZE - pos,
				" powerRL_enable, powerRL_perf_min, powerRL_current_cap\n");
	pos += length;
	length = scnprintf(temp + pos, FPSGO_SYSFS_MAX_BUFF_SIZE - pos,
	...
	for (n = rb_first(&render_pid_tree); n != NULL; n = rb_next(n)) {
		...
		length = scnprintf(temp + pos,
			...
			attr_item.powerRL_perf_min_by_pid,
			attr_item.powerRL_current_cap_by_pid);
		pos += length;
		...
		char *buf)
{
	...
	for (n = rb_first(&fpsgo_attr_by_pid_tree); n != NULL; n = rb_next(n)) {
		...
		length = scnprintf(temp + pos,
				FPSGO_SYSFS_MAX_BUFF_SIZE - pos,
				"\n NEW tgid: %4d, %4d,\n",
			iter->tgid, attr_item.aa_enable_by_pid);
		pos += length;
		...
		char *buf)
{
	...
	length = scnprintf(temp + pos, FPSGO_SYSFS_MAX_BUFF_SIZE - pos,
		"\n NEW tgid: TGID, aa_enable, rescue_enable,\n");
	pos += length;
	...
	for (n = rb_first(&fpsgo_attr_by_tid_tree); n != NULL; n = rb_next(n)) {
		...
		length = scnprintf(temp + pos,
				FPSGO_SYSFS_MAX_BUFF_SIZE - pos,
				"\n NEW tid: %4d, %4d, %4d,\n",
			iter->tid, attr_item.aa_enable_by_pid, attr_item.rescue_enable_by_pid);
		pos += length;
		...
		char *buf)
{
	...
	for (rbn = rb_first(&render_pid_tree); rbn; rbn = rb_next(rbn)) {
		r_iter = rb_entry(rbn, struct render_info, render_key_node);
		length = scnprintf(temp + pos, FPSGO_SYSFS_MAX_BUFF_SIZE - pos,
				"%dth\t[%d][0x%llx]\tapi:%d\tqueue_SF:%d\tready:%d\tframe_type:%d\ttarget_render_flag:%d\thwui:%d\tmaster_type:%lu\n",
				i+1, r_iter->pid, r_iter->buffer_id,
				r_iter->api, r_iter->queue_SF, r_iter->producer_info_ready,
				r_iter->frame_type, r_iter->target_render_flag, r_iter->hwui, r_iter->master_type);
		pos += length;
		i++;
	}
	...
	unsigned long long bufID, int fps, int op, int cmd)
{
	...
	case FBT_RENDER_INFO:
		if (op) {
			r_iter = fpsgo_search_and_add_render_info(pid, bufID, 0);
			fpsgo_delete_render_info(r_iter);
		} else {
	...
	case FPSGO_CONTROL_INFO:
		op ? fpsgo_delete_fps_control_info(0, tgid, 0) :
			fpsgo_search_and_add_fps_control_info(0, tgid, 0, 1);
		break;
...
int init_fpsgo_common(void)
{
	...
	fps_control_by_tgid_tree = RB_ROOT;
	fps_control_by_pid_tree = RB_ROOT;
