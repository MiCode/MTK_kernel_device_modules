...
void fbt_set_hard_limit_locked(int input, struct cpu_ctrl_data *pld)
{
	...
	if (cluster_num == 1 || (pld && pld[max_cap_cluster].max == -1)) {
		set_ceiling = 0;
		set_margin = 0;
	}
...
static void fbt_task_set_pmu(struct rb_root *pmu_info_tree, int separate_aa,
...
#if IS_ENABLED(CONFIG_ARM64)
	xgf_trace("%s %s %s %d 0x%llx ret=%d size:%d dep-list %s",
			__func__, func, tag, pid, buffer_id, ret, size, dep_str);
#else
	xgf_trace("%s %s %s %d 0x%x size:%d",
...
static void fbt_query_dep_list_loading(struct render_info *thr)
{
	...
	unsigned long long cur_runtime;
	...
	window = thr->t_enqueue_start - thr->dep_loading_ts;
	for (i = 0; i < thr->dep_valid_size; i++) {
		prev_runtime = thr->dep_arr[i].latest_runtime;
		rcu_read_lock();
		tsk = find_task_by_vpid(thr->dep_arr[i].pid);
		if (tsk) {
			get_task_struct(tsk);
			cur_runtime = (u64)fpsgo_task_sched_runtime(tsk);
			put_task_struct(tsk);
		} else
			cur_runtime = 0;
		rcu_read_unlock();
		...
		fpsgo_main_trace("[fbt][%d][0x%llx] dep:%d loading:%d runtime:%llu->%llu",
			thr->pid, thr->buffer_id,
			thr->dep_arr[i].pid, thr->dep_arr[i].loading, prev_runtime, cur_runtime);
	}
...
static int __cmp2(const void *a, const void *b)
{
	return (((struct fpsgo_loading *)b)->pid)
		- (((struct fpsgo_loading *)a)->pid);
}

static int fbt_filter_dep_list(struct fpsgo_loading *dep_arr, int *num,
	int pid, unsigned long long buffer_id)
{
	int i, j;
	int orig_num;
	int filter_num = 0;
	struct no_boost_info *filter_arr = NULL;

	if (!dep_arr || !num || *num <= 0)
		return -EINVAL;

	orig_num = *num;
	filter_arr = kcalloc(FPSGO_MAX_NO_BOOST_INFO_SIZE, sizeof(struct no_boost_info), GFP_KERNEL);
	if (!filter_arr)
		return -ENOMEM;

	filter_num = fpsgo_get_all_no_boost_info(FPSGO_MAX_NO_BOOST_INFO_SIZE, filter_arr);
	if (!filter_num)
		goto out;

	for (i = 0; i < filter_num; i++) {
		for (j = 0; j < orig_num; j++) {
			if ((filter_arr[i].mode == 0 &&
					fpsgo_get_tgid(dep_arr[j].pid) == filter_arr[i].specific_id) ||
				(filter_arr[i].mode == 1 &&
					dep_arr[j].pid == filter_arr[i].specific_id)) {
				dep_arr[j].pid = 0;
				break;
			}
		}
...
static int fbt_get_dep_list(struct render_info *thr)
{
	...
	if (thr->attr.gcc_enable_by_pid || thr->raw_runtime) {
		tmp_dep_arr = kcalloc(MAX_DEP_NUM, sizeof(struct task_info), GFP_KERNEL);
		if (tmp_dep_arr) {
			count = fpsgo_other2xgf_get_critical_tasks(pid, MAX_DEP_NUM,
				tmp_dep_arr, !test_bit(KTF_TYPE, &thr->master_type), thr->buffer_id);
			if (count > 0 && count <= MAX_DEP_NUM) {
				for (i = 0; i < count; i++) {
					dep_new[i].pid = tmp_dep_arr[i].pid;
					dep_new[i].action = tmp_dep_arr[i].action;
					dep_new[i].prio = tmp_dep_arr[i].vip_prio;
					dep_new[i].timeout = tmp_dep_arr[i].vip_timeout;
				}
			}
			kfree(tmp_dep_arr);
		} else {
			memcpy(dep_new, thr->dep_arr,
				thr->dep_valid_size * sizeof(struct fpsgo_loading));
			count = thr->dep_valid_size;
			ret = 3;
		}
	} else
		count = 0;
...
static int fbt_get_max_cap(int floor, int bhr_local)
{
	...
	max_cap = MIN(floor + bhr_local, max_cap);
	max_cap = MAX(floor, max_cap);
	...
	int *final_max_util, int *final_max_util_b, int *final_max_util_m)
{
	...
	// Get max_cap
	max_cap = fbt_get_max_cap(min_cap, bhr_local);

	// Get max_cap_b, max_cap_m
	if (separate_aa_final) {
		max_cap_b = fbt_get_max_cap(min_cap_b, bhr_local);
		max_cap_m = fbt_get_max_cap(min_cap_m, bhr_local);
	}
			...
			unsigned long long cur_ts, int jerk)
{
	...
	if (boost_affinity_final || separate_aa_final || boost_VIP_final)
	...
	for (i = 0; i < thr->dep_valid_size; i++) {
		...
		fbt_task_cap(fl->pid, min_cap, min_cap_b, min_cap_m, min_cap_other, max_cap,
				...
				fl->heavyidx, fl->action);

		if (powerRL_enable_final && bat_psy && cur_ts)
			fbt_task_set_pmu(&thr->pmu_info_tree, separate_aa_final,
...
void fbt_set_render_boost_attr(struct render_info *thr)
{
	...
	if (pid_attr.aa_enable_by_pid != BY_PID_DEFAULT_VAL)
		render_attr->aa_enable_by_pid =
			pid_attr.aa_enable_by_pid;
...
by_tid:
	...
	if (pid_attr.aa_enable_by_pid != BY_PID_DEFAULT_VAL)
		render_attr->aa_enable_by_pid = pid_attr.aa_enable_by_pid;
	if (pid_attr.rescue_enable_by_pid != BY_PID_DEFAULT_VAL)
		render_attr->rescue_enable_by_pid = pid_attr.rescue_enable_by_pid;
	if (pid_attr.gcc_enable_by_pid != BY_PID_DEFAULT_VAL)
...
static unsigned int fbt_enhance_floor(unsigned int blc_wt, int enh)
{
	int final_blc_wt;

	final_blc_wt = blc_wt + enh;
	final_blc_wt = clamp(final_blc_wt, 1, 100);

	return final_blc_wt;
}

unsigned int fbt_get_new_base_blc(struct cpu_ctrl_data *pld,
	int floor, int enhance)
{
	int cluster;
	unsigned int blc_wt = 0U;
	unsigned int max_blc_wt = 100U;
	...
	blc_wt = fbt_enhance_floor(floor, enhance);
	max_blc_wt = (fbt_get_max_cap(MAX(blc_wt, max_blc), rescue_bhr) << 10) / 100;
	for (cluster = 0 ; cluster < cluster_num; cluster++) {
		pld[cluster].min = -1;
		if (suppress_ceiling)
			pld[cluster].max = fbt_cluster_X2Y(cluster, max_blc_wt, CAP, FREQ, 1, __func__);
		else
			pld[cluster].max = -1;
	}
...
static void fbt_do_sjerk(struct work_struct *work)
{
	...
	unsigned long long cb_bufID = 0;
	struct render_frame_info *info = NULL;

	jerk = container_of(work, struct fbt_sjerk, work);

	if (!jerk || !jerk->pid || !jerk->buffer_id || !jerk->jerking)
		return;

	fpsgo_render_tree_lock(__func__);

	thr = fpsgo_search_and_add_render_info(jerk->pid, jerk->buffer_id, 0);
	if (!thr) {
		fpsgo_render_tree_unlock(__func__);
		return;
	}
	...
	cb_pid = thr->pid;
	cb_bufID = thr->buffer_id;
	info = kzalloc(sizeof(struct render_frame_info), GFP_KERNEL);
	if (info)
		info->jerk_boost_flag = FPSGO_JERK_SECOND;
...
EXIT:
	...
	if (cb_pid && cb_bufID && info) {
		cb_mask = 1 << GET_FPSGO_JERK_BOOST;
		fpsgo_notify_frame_info_callback(cb_pid, cb_mask, cb_bufID, info);
	}
	kfree(info);
}
...
static void fbt_do_jerk_locked(struct render_info *thr, struct fbt_jerk *jerk, int jerk_id)
{
	...
	if (rescue_second_enable_final && thr->pid == max_blc_pid
		...
		if (thr->boost_info.t_duration && thr->boost_info.t_duration
			...
			fbt_set_sjerk(thr->pid, thr->buffer_id, jerk_id, st2wnt);
		}
	}

	blc_wt = fbt_get_new_base_blc(pld, blc_wt, rescue_enhance_f);
	if (separate_aa_final) {
		blc_wt_b = fbt_get_new_base_blc(pld, blc_wt_b, rescue_enhance_f);
		blc_wt_m = fbt_get_new_base_blc(pld, blc_wt_m, rescue_enhance_f);
	}
...
static void fbt_do_jerk(struct work_struct *work)
{
	...
	if (fpsgo_check_fbt_jerk_work_addr_invalid(work)) {
		fpsgo_render_tree_unlock(__func__);
		return;
	}
...
EXIT:
	...
	if (jerk->last_check == FPSGO_JERK_NEED) {
		cb_pid = thr->pid;
		cb_bufID = thr->buffer_id;
		info = kzalloc(sizeof(struct render_frame_info), GFP_KERNEL);
		if (info)
			info->jerk_boost_flag = FPSGO_JERK_FIRST;
	}
	...
	unsigned long long buffer_id)
{
	...
	max_blc_wt = (fbt_get_max_cap(blc_wt, bhr) << 10) / 100;
	for (cluster = 0 ; cluster < cluster_num; cluster++) {
		pld[cluster].min = -1;
		if (suppress_ceiling)
			pld[cluster].max = fbt_cluster_X2Y(cluster, max_blc_wt, CAP, FREQ, 1, __func__);
...
int fpsgo_other2fbt_calculate_frame_loading(long loading,
...
EXPORT_SYMBOL(fpsgo_other2fbt_calculate_frame_loading);
...
unsigned int fpsgo_other2fbt_calculate_blc(long aa, unsigned long long target_time,
		...
		if (!ret && last_target_t_ns) {
			...
			if (last_target_fps_ori == target_fps_ori) {
				if (separate_aa_active) {
					fpsgo_other2fbt_calculate_blc(aa_b, last_target_t_100us, 1,
						t_q2q_100us, 0, &test_blc_wt_b);
					fpsgo_other2fbt_calculate_blc(aa_m, last_target_t_100us, 1,
					...
					fpsgo_other2fbt_calculate_blc(aa_n, last_target_t_100us, 1,
	...
	long loading, unsigned int target_fpks, int cooler_on)
{
	...
	avg_exp_time_ns = fpsgo_other2fstb_get_app_self_ctrl_time(thread_info->pid, thread_info->buffer_id);
	if (avg_exp_time_ns)
...
/*
 * General API to get FPSGO FBT cpu capacity area
 * @cluster: which cluster, if larger than actual number of cluster, use max cpu capacity algorithm
 * @pid: only use to print log
 * @buffer_id: only use to print log
 * @loading: return result of cpu capacity area
 * @start_ts_100us: begin timestamp of cpu capacity area
 * @end_ts_100us: end timestamp of cpu capacity area
 */
int fpsgo_other2fbt_get_cpu_capacity_area(int cluster, int pid,
	unsigned long long buffer_id, unsigned long long *loading,
	unsigned long long start_ts_ns, unsigned long long end_ts_ns)
{
	int ret;
	unsigned long long start_ts_100us = nsec_to_100usec_ull(start_ts_ns);
	unsigned long long end_ts_100us = nsec_to_100usec_ull(end_ts_ns);

	if (cluster_num <= 0 || !loading)
		return -EINVAL;

	*loading = 0;
	if (cluster >= cluster_num)
		ret = fbt_get_loading_from_buffer(pid, buffer_id,
			start_ts_100us, loading, end_ts_100us,
			prev_cb_ts, lastest_ts, lastest_obv, &last_obv, &last_cb_ts);
	else
		ret = fbt_get_cl_loading_from_buffer(pid, buffer_id,
			start_ts_100us, loading, end_ts_100us, cluster);

	return ret;
}
EXPORT_SYMBOL(fpsgo_other2fbt_get_cpu_capacity_area);
...
static void fbt_frame_start(struct render_info *thr, unsigned long long ts)
{
	...
	if (targetfps <= 0) {
		fpsgo_main_trace("pid:%d buffer_id:0x%llx not have target(%d)",
			thr->pid, thr->buffer_id, targetfps);
		fbt_reset_boost(thr);
		return;
	}
		...
		unsigned long long ts)
{
	...
	if (thr->attr.aa_enable_by_pid != 1) {
		fpsgo_systrace_c_fbt(thr->pid, thr->buffer_id,
			0, "not calculate AA");
		return;
	}
...
{
	int i;
	struct fbt_thread_blc *blc_link = NULL;
	struct fbt_boost_info *boost;
	long *cl_loading = NULL;
...
void fbt_jank_thread_uclamp(int boost, int pid, int min_cap_ratio, int max_cap_ratio)
{
	...
	if (boost) {
		...
		if (local_min_cap < 100) {
			local_max_cap = fbt_get_max_cap(local_min_cap, bhr);
...
static void fbt_jank_thread_restore(int pid)
{
	...
	mutex_lock(&fbt_mlock);
	iter = fpsgo_search_and_add_render_info(max_blc_pid, max_blc_buffer_id, 0);
	mutex_unlock(&fbt_mlock);
	...
	if (min_cap_final < 100) {
		...
		case FPSGO_JERK_INACTIVE:
			max_cap_final = fbt_get_max_cap(min_cap_final, bhr);
			break;
		case FPSGO_JERK_FIRST:
			max_cap_final = fbt_get_max_cap(min_cap_final, 100);
			break;
		...
		char *buf)
{
	...
	length = scnprintf(temp + posi,
		FPSGO_SYSFS_MAX_BUFF_SIZE - posi,
		"enable\tmax_blc\tmax_pid\tmax_bufID\tvsync\n");
	posi += length;
		...
		const char *buf, size_t count)
{
	...
	if (!strcmp(cmd, "aa_enable")) {
		if ((val == 0 || val == 1) && action == 's')
			boost_attr->aa_enable_by_pid = val;
		else if (val == BY_PID_DEFAULT_VAL && action == 'u')
			boost_attr->aa_enable_by_pid = BY_PID_DEFAULT_VAL;
	} else if (!strcmp(cmd, "rescue_enable")) {
		if ((val == 0 || val == 1) && action == 's')
