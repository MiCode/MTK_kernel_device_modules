...
static HLIST_HEAD(xgf_render_if_list);
static HLIST_HEAD(xgff_frames);
static HLIST_HEAD(xgf_spid_list);
static HLIST_HEAD(xgf_wspid_list);
static HLIST_HEAD(xgf_policy_cmd_list);
...
int (*xgf_est_runtime_fp)(
	...
	int *spid_list, int spid_list_num,
...
int xgf_get_record_trace_enable(void)
{
	return xgf_record_trace_enable;
}
...
/* let Task Turbo get cameraHAL pid from ThirdCamPowerhalControl */
...
/* let Task Turbo get cameraserver pid from ThirdCamPowerhalControl */
...
static void xgf_delete_policy_cmd(int mode, struct xgf_policy_cmd *iter)
{
	...
	hlist_for_each_entry_safe(tmp_iter, h, &xgf_policy_cmd_list, hlist) {
...
static struct xgf_policy_cmd *xgf_get_policy_cmd(int mode, int id,
	unsigned long long bufID, int force)
{
	struct xgf_policy_cmd *iter = NULL;
	struct hlist_node *h = NULL;

	hlist_for_each_entry_safe(iter, h, &xgf_policy_cmd_list, hlist) {
		if (iter->mode == mode) {
	...
	hlist_add_head(&iter->hlist, &xgf_policy_cmd_list);
	total_xgf_policy_cmd_num++;

	if (total_xgf_policy_cmd_num > XGF_MAX_POLICY_CMD_NUM)
		xgf_delete_policy_cmd(mode, NULL);

out:
	return iter;
}

static void xgf_set_policy_cmd(int cmd, int mode, int id, int value, int op,
	unsigned long long bufID)
{
	struct xgf_policy_cmd *iter;

	iter = xgf_get_policy_cmd(mode, id, bufID, op);
	if (iter) {
		if (cmd == 0)
...
static void xgf_update_policy_cmd(struct xgf_render_if *iter)
{
	...
	policy = xgf_get_policy_cmd(0, iter->tgid, 0, 0);
	if (!policy)
		goto by_render;
...
int is_xgf_calculate_dep_enable(int tgid, int pid, unsigned long long bufID)
{
	int enable = 0;
	struct xgf_policy_cmd *iter;

	mutex_lock(&xgf_policy_cmd_lock);
	iter = xgf_get_policy_cmd(1, pid, bufID, 0);
	if (iter && iter->calculate_dep_enable != BY_PID_DEFAULT_VAL)
		enable = iter->calculate_dep_enable;
	else {
		iter = xgf_get_policy_cmd(0, tgid, 0, 0);
		if (iter && iter->calculate_dep_enable != BY_PID_DEFAULT_VAL)
			enable = iter->calculate_dep_enable;
	}
	mutex_unlock(&xgf_policy_cmd_lock);
...
static struct xgf_spid *xgf_get_spid(int tgid, int rpid, int tid,
	unsigned long long bufID, struct hlist_head *head)
{
	struct xgf_spid *xgf_spid_iter = NULL;
	struct hlist_node *h = NULL;

	hlist_for_each_entry_safe(xgf_spid_iter, h, head, hlist) {
		if (xgf_spid_iter->tgid == tgid &&
			xgf_spid_iter->rpid == rpid &&
			xgf_spid_iter->bufID == bufID &&
			xgf_spid_iter->tid == tid)
			return xgf_spid_iter;
	}
...
static void xgf_set_spid(int tgid, int rpid, int tid, int action,
	int vip_prio, int vip_timeout, unsigned long long bufID, struct xgf_spid *iter)
{
	if (!iter)
		return;
	...
	iter->bufID = bufID;
	iter->action = action;
	iter->vip_prio = vip_prio;
	iter->vip_timeout = vip_timeout;
}
	...
	if (gtsk) {
		...
		for_each_thread(gtsk, sib) {
			...
			hlist_for_each_entry(xgf_spid_iter, &xgf_spid_list, hlist) {
				...
				/*
				 * TODO(CHI):
				 *	need to consider conflict that use general API (fpsgo_other2xgf_set_critical_tasks)
				 *	and use xgf.cfg together
				 */
				if (xgf_get_spid(tgid, rpid, sib->pid, bufID, &xgf_wspid_list))
					goto tsk_out;
				...
				xgf_set_spid(tgid, rpid, sib->pid, xgf_spid_iter->action,
					0, 0, bufID, new_xgf_spid);
			}
tsk_out:
			put_task_struct(sib);
		}
	...
	unsigned long master_type, unsigned long long ts, int create)
{
	...
	if (iter || !create || is_exceed_max_xgf_render_if_num())
		goto out;
	...
	iter->tgid = xgf_get_process_id(pid);
	iter->pid = pid;
	iter->spid = 0;
	iter->bufid = bufID;
	...
	struct xgf_render_if *render, int create)
{
	...
	rb_insert_color(&iter->rb_node, &render->dep_list);
	render->dep_list_size++;

	return iter;
}

static void xgf_del_dep_task(int tid, struct xgf_render_if *render)
{
	struct xgf_dep *iter;
	...
	struct xgf_render_if *render)
{
	...
	for (i = 0; i < dep_list_num; i++)
		xgf_add_dep_task(dep_list[i], render, 1);
}
...
static void xgf_add_spid2dep(struct xgf_render_if *render)
{
	struct xgf_spid *spid_iter;
	struct xgf_dep *dep_iter;
	struct hlist_node *h;

	hlist_for_each_entry_safe(spid_iter, h, &xgf_wspid_list, hlist) {
		if (spid_iter->rpid == render->pid && spid_iter->bufID == render->bufid) {
			if (spid_iter->action == XGF_DEL_DEP ||
				spid_iter->action == XGF_FORCE_BOOST)
				xgf_del_dep_task(spid_iter->tid, render);
			else {
				dep_iter = xgf_add_dep_task(spid_iter->tid, render, 1);
				if (dep_iter) {
					dep_iter->action = spid_iter->action;
					dep_iter->vip_prio = spid_iter->vip_prio;
					dep_iter->vip_timeout = spid_iter->vip_timeout;
				}
			}
		}
	}
...
static int xgf_get_specific_action_spid(int target_action, int max_num,
	int *arr, struct xgf_render_if *render_iter)
{
	int index = 0;
...
void fpsgo_comp2xgf_delete_render_info(int pid, unsigned long long bufID)
{
	struct xgf_render_if *iter;

	mutex_lock(&xgf_main_lock);
	iter = xgf_get_render_if(pid, bufID, 0, 0, 0);
	if (iter)
...
static int xgf_get_specific_task(struct xgf_render_if *render)
{
	int len, ret = 0;
	long long diff;
	unsigned long long now_ts = fpsgo_get_time();
...
/*
 * General API to calculate critical tasks & union cpu running time
 * @pid: one key of render_info data structure
 * @bufID: one key of render_info data structure
 * @raw_running_time: return actual union cpu running time of critical tasks
 * @ema_running_time: return ema union cpu running time of critical tasks
 * @enq_running_time: return actual union cpu running time of critical tasks in queueBuffer
 * @def_start_ts: begin timestamp of window to calculate, if input is 0, use previous @def_end_ts
 * @def_end_ts: end timestamp of window to calculate
 * @t_dequeue_start: begin timestamp of dequeueBuffer, if not need can set 0 as input
 * @t_dequeue_end: end timestamp of dequeueBuffer, if not need can set 0 as input
 * @t_enqueue_start: begin timestamp of queueBuffer, if not need can set 0 as input
 * @t_enqueue_end: end timestamp of queueBuffer, if not need can set 0 as input
 * @skip: only update timestamp
 */
void fpsgo_other2xgf_calculate_dep(int pid, unsigned long long bufID,
	...
	int skip)
{
	...
	int new_spid;
	int do_extra_sub = 0;
	int *local_dep_list = NULL, *raw_dep_list = NULL, *local_spid_list = NULL;
	int local_dep_list_num = 0, raw_dep_list_num = 0, local_spid_list_num = 0;
	...
	xgf_update_policy_cmd(iter);

	new_spid = xgf_get_specific_task(iter);
	if (new_spid > 0) {
	...
	local_spid_list = xgf_alloc_array(MAX_DEP_NUM, sizeof(int));
	if (local_spid_list)
		local_spid_list_num = xgf_get_specific_action_spid(XGF_ADD_DEP_FORCE_CPU_TIME,
			MAX_DEP_NUM, local_spid_list, iter);
	...
	ret = xgf_enter_est_runtime(iter->pid, iter->bufid,
				...
				local_spid_list, local_spid_list_num,
	...
	if (raw_running_time)
		*raw_running_time = local_raw_t_cpu;
	if (ema_running_time)
		*ema_running_time = local_ema_t_cpu;
	if (enq_running_time)
...
out:
	...
	xgf_free(local_spid_list);
	xgf_free(local_dep_list);

	mutex_unlock(&xgf_main_lock);
}
EXPORT_SYMBOL(fpsgo_other2xgf_calculate_dep);

/*
 * General API to notify FPSGO XGF specific tasks with specific action
 * @rpid: one key of render_info data structure
 * @bufID: one key of render_info data structure
 * @arr: specific task with action
 * @num: specific task num
 * @use: bool value for set or unset
 */
int fpsgo_other2xgf_set_critical_tasks(int rpid, unsigned long long bufID,
	struct task_info *arr, int num, int use)
{
	...
	ts = fpsgo_get_time();
	r_iter = xgf_get_render_if(rpid, bufID, local_master_type, ts, use);
	if (!r_iter) {
		ret = -ENOMEM;
		goto out;
	}
	...
	for (i = 0; i < num && arr; i++) {
		...
		if (!spid_iter) {
			 ...
			 *	and use xgf.cfg together
			 */
			use ? xgf_set_spid(r_iter->tgid, rpid, arr[i].pid,
				arr[i].action, arr[i].vip_prio, arr[i].vip_timeout, bufID, spid_iter) :
					xgf_delete_spid(spid_iter, &xgf_wspid_list);
		}
	}
...
EXPORT_SYMBOL(fpsgo_other2xgf_set_critical_tasks);

/*
 * General API to get FPSGO XGF critical tasks
	...
	struct task_info *arr, int filter_non_cfs, unsigned long long bufID)
{
	...
	for (rbn = rb_first(&render_iter->dep_list); rbn; rbn = rb_next(rbn)) {
		...
		if (index < max_num) {
			...
			arr[index].vip_prio = xd_iter->vip_prio;
			arr[index].vip_timeout = xd_iter->vip_timeout;
			index++;
		}
	}
...
EXPORT_SYMBOL(fpsgo_other2xgf_get_critical_tasks);

/*
 * General API to set FPSGO XGF algorithm
...
int fpsgo_ktf2xgf_add_delete_render_info(int mode, int pid, unsigned long long bufID)
{
	...
	r_iter = xgf_get_render_if(pid, bufID, local_master_type, ts, mode);
	if (!r_iter) {
		ret = -ENOMEM;
		goto out;
	}
	...
	if (mode) {
		if (!xgf_add_dep_task(pid, r_iter, 1)) {
			ret = -ENOMEM;
			goto out;
		}
	} else
...
#define XGF_SYSFS_WRITE_POLICY_CMD_BY_PROCESS(name, cmd, min, max); \
static ssize_t name##_store(struct kobject *kobj, \
		struct kobj_attribute *attr, \
		const char *buf, size_t count) \
{ \
	...
	if ((count > 0) && (count < FPSGO_SYSFS_MAX_BUFF_SIZE)) { \
		if (scnprintf(acBuffer, FPSGO_SYSFS_MAX_BUFF_SIZE, "%s", buf)) { \
			if (sscanf(acBuffer, "%d %d", &tgid, &arg) == 2) { \
				mutex_lock(&xgf_policy_cmd_lock); \
				if (arg >= (min) && arg <= (max)) \
					xgf_set_policy_cmd(cmd, 0, tgid, arg, 1, 0); \
				else \
					xgf_set_policy_cmd(cmd, 0, tgid, BY_PID_DEFAULT_VAL, 0, 0); \
				mutex_unlock(&xgf_policy_cmd_lock); \
			} \
		} \
	} \
...
#define XGF_SYSFS_WRITE_POLICY_CMD_BY_RENDER(name, cmd, min, max); \
static ssize_t name##_store(struct kobject *kobj, \
		struct kobj_attribute *attr, \
		const char *buf, size_t count) \
{ \
	...
	if ((count > 0) && (count < FPSGO_SYSFS_MAX_BUFF_SIZE)) { \
		if (scnprintf(acBuffer, FPSGO_SYSFS_MAX_BUFF_SIZE, "%s", buf)) { \
			if (sscanf(acBuffer, "%d %llu %d", &pid, &bufID, &arg) == 3) { \
				mutex_lock(&xgf_policy_cmd_lock); \
				if (arg >= (min) && arg <= (max)) \
					xgf_set_policy_cmd(cmd, 1, pid, arg, 1, bufID); \
				else \
					xgf_set_policy_cmd(cmd, 1, pid, BY_PID_DEFAULT_VAL, 0, bufID); \
				mutex_unlock(&xgf_policy_cmd_lock); \
			} \
		} \
	} \
...
XGF_SYSFS_READ(xgf_record_trace_enable, 1, xgf_record_trace_enable);
static ssize_t xgf_record_trace_enable_store(struct kobject *kobj,
	...
	if ((count > 0) && (count < FPSGO_SYSFS_MAX_BUFF_SIZE)) {
		if (scnprintf(acBuffer, FPSGO_SYSFS_MAX_BUFF_SIZE, "%s", buf)) {
			if (kstrtoint(acBuffer, 0, &arg) == 0) {
				enable = !!arg;
				mutex_lock(&xgf_global_var_lock);
				if (xgf_record_trace_enable != enable) {
					enable ? xgf_tracing_register() : xgf_tracing_unregister();
					xgf_record_trace_enable = enable;
					FPSGO_LOGE("[xgf] xgf_record_trace_enable:%d\n", xgf_record_trace_enable);
				}
				mutex_unlock(&xgf_global_var_lock);
			}
		}
...
static KOBJ_ATTR_RW(xgf_record_trace_enable);

XGF_SYSFS_WRITE_POLICY_CMD_BY_PROCESS(xgf_ema2_enable_by_process, 0, 0, 1);
static KOBJ_ATTR_WO(xgf_ema2_enable_by_process);
...
static KOBJ_ATTR_WO(xgf_calculate_dep_enable_by_process);
		...
		char *buf)
{
	...
	hlist_for_each_entry_safe(r_iter, r_tmp, &xgf_render_if_list, hlist) {
		length = scnprintf(temp + pos, FPSGO_SYSFS_MAX_BUFF_SIZE - pos,
			"pid:%d bufID:0x%llx type:%lu size:%d\n",
...
void fpsgo_ktf2xgf_fuzz_test_node(char *input_data, int op, int cmd)
{
	...
	case XGF_EMA2_ENABLE_BY_PID:
		if (op)
			fpsgo_ktf_test_write_node(kobj, attr, buf, xgf_ema2_enable_by_process_store);
		break;
	case XGF_FILTER_DEP_TASK_ENABLE_GLOBAL:
		break;
	case XGF_FILTER_DEP_TASK_ENABLE_BY_PID:
		if (op)
			fpsgo_ktf_test_write_node(kobj, attr, buf, xgf_filter_dep_task_enable_by_process_store);
		break;
...
int __init init_xgf(void)
{
	...
	if (!fpsgo_sysfs_create_dir(NULL, "xgf", &xgf_kobj)) {
		...
		fpsgo_sysfs_create_file(xgf_kobj, &kobj_attr_xgf_ema2_enable_by_process);
		fpsgo_sysfs_create_file(xgf_kobj, &kobj_attr_xgf_ema2_enable_by_render);
		...
		fpsgo_sysfs_create_file(xgf_kobj, &kobj_attr_xgf_filter_dep_task_enable_by_process);
		fpsgo_sysfs_create_file(xgf_kobj, &kobj_attr_xgf_filter_dep_task_enable_by_render);
...
int __exit exit_xgf(void)
{
	...
	fpsgo_sysfs_remove_file(xgf_kobj, &kobj_attr_xgf_ema2_enable_by_process);
	fpsgo_sysfs_remove_file(xgf_kobj, &kobj_attr_xgf_ema2_enable_by_render);
	...
	fpsgo_sysfs_remove_file(xgf_kobj, &kobj_attr_xgf_filter_dep_task_enable_by_process);
	fpsgo_sysfs_remove_file(xgf_kobj, &kobj_attr_xgf_filter_dep_task_enable_by_render);
