...
#if IS_ENABLED(CONFIG_MI_UX_DEPLIST)
    if(qudeq == 1){
		 mi_deplist_render_qudep(pid);
	}
#endif
...
{
	struct FPSGO_NOTIFIER_PUSH_TAG *vpPush;
// MIUI ADD: Performance_UXDepList
#if IS_ENABLED(CONFIG_MI_UX_DEPLIST)
	...
	if (tsk){
		...
		if(connectedAPI == 1 || connectedAPI == 2 ){
			mi_deplist_set_render_task(tsk,1);
		}else if(connectedAPI == 0){
			mi_deplist_set_render_task(tsk,0);
		}
		put_task_struct(tsk);
