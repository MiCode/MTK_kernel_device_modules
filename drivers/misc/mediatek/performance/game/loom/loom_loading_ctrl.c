...
int loom_update_userlimit_cpufreq_min(int cid, int value)
{
	if (!freq_min_request || cid < 0 || cid >= policy_num)
		return -1;
...
int loom_update_userlimit_cpufreq_max(int cid, int value)
{
	if (!freq_max_request || cid < 0 || cid >= policy_num)
		return -1;
...
int loom_get_user_expected_fps(void)
{
	return g_expected_fps;
}
...
void loom_delete_loading_ctrl_info(struct loom_loading_ctrl *lc_info)
{
	...
	loom_update_userlimit_cpufreq_max(lc_info->cluster,
		FREQ_QOS_MAX_DEFAULT_VALUE);
	loom_update_userlimit_cpufreq_min(lc_info->cluster,
...
int loom_loading_ctrl_operation(struct loom_loading_ctrl *lc_info, unsigned long long ts, int cluster, int cpu)
{
	...
	if (avail_window_count < g_loading_window_size) {
		...
		if (lc_cal_freq_fp) {
			...
			loom_update_userlimit_cpufreq_max(lc_info->cluster, now_freq_max);
			game_systrace_c(GAME_DEBUG_MANDATORY, lc_info->tid, 0, now_freq_max,
				"loading_ctrl_C%d_freq_max", lc_info->cluster);

			loom_update_userlimit_cpufreq_min(cluster, now_freq);
