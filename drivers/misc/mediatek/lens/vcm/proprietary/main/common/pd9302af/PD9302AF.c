...
static int s4PD9302AF_ReadReg(unsigned short * a_pu2Result)
{
    int  i4RetValue = 0;
    char pBuff[2];
    ...
    if (i4RetValue < 0)
    {
        pr_info("[PD9302AF] I2C read failed!! \n");
        return -1;
    }
...
static int s4PD9302AF_WriteReg(u16 a_u2Data)
{
    int  i4RetValue = 0;
    ...
    if (i4RetValue < 0)
    {
        pr_info("[PD9302AF] I2C send failed!! \n");
        return -1;
    }
...
static int initAF(void)
{
	...
	char puSendCmd5[2]={(char)(0x07),(char)(0x77)};
	...
	if (i4RetValue < 0)
	{
	   pr_info("[PD9302AF] I2C send cmd1 failed!! \n");
	}
	i4RetValue = i2c_master_send(g_pstAF_I2Cclient, puSendCmd2, 2);

	if (i4RetValue < 0)
	{
	   pr_info("[PD9302AF] I2C send cmd2 failed!! \n");
	}
    ...
    if (*g_pAF_Opened == 1)
    {
        ...
        ret = s4PD9302AF_ReadReg(&InitPos);


        spin_lock(g_pAF_SpinLock);
        if(ret == 0)
        {
            pr_info("[PD9302AF] Init Pos %6d \n", InitPos);
...
static inline int moveAF(unsigned long a_u4Position)
{
    ...
    pr_info("[PD9302AF] move [curr] %ld [target] %ld\n", g_u4CurrPosition, g_u4TargetPosition);

    spin_lock(g_pAF_SpinLock);
    spin_unlock(g_pAF_SpinLock);

    if(s4PD9302AF_WriteReg((unsigned short)g_u4TargetPosition) == 0)
    {
        spin_lock(g_pAF_SpinLock);
    ...
    {
        pr_info("[PD9302AF] set I2C failed when moving the motor \n");
    }
...
int PD9302AF_Release(struct inode *a_pstInode, struct file *a_pstFile)
{
	...
	if (*g_pAF_Opened == 2) {
		LOG_INF("Wait\n");
		pr_info("[PD9302AF] feee \n");

		s4PD9302AF_WriteReg(0x0138);
		mdelay(15);
		s4PD9302AF_WriteReg(0x019c);
		mdelay(15);
		s4PD9302AF_WriteReg(0x0200);
		mdelay(15);
	}
