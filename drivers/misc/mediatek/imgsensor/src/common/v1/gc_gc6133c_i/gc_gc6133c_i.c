...
static int GC6133C_GetSensorID(struct gc6133c *gc6133c)
{
    ...
    do {
		gc6133c_i2c_read(gc6133c, 0xf0, &reg_data);
        ...
        retry--;
		pr_err("%s gc6133c get sensorid retry %d time\n", __func__, retry);
    } while (retry > 0);
	return -1;
}
// define AVDD
static void gc6133c_avdd_control(struct gc6133c *gc6133c, bool flag)
{
	...
	if (IS_ERR(gc6133c->vcama)) {
		qvga_dev_err(gc6133c->dev, "%s AVDD get regulator failed\n", __func__);
		regulator_put(gc6133c->vcama);
		return;
	}

	if (flag) {
		regulator_set_voltage(gc6133c->vcama, 2800000, 2800000);
		ret = regulator_enable(gc6133c->vcama);
	} else {
		ret = regulator_disable(gc6133c->vcama);
	}
...
static void gc6133c_iovdd_control(struct gc6133c *gc6133c, bool flag)
{
    int ret;
    qvga_dev_info(gc6133c->dev, "%s enter\n", __func__);
    if (IS_ERR(gc6133c->vcamio)) {
        qvga_dev_err(gc6133c->dev, "%s  IOVDD get regulator failed\n", __func__);
        regulator_put(gc6133c->vcamio);
        return;
    }
    if (flag) {
        regulator_set_voltage(gc6133c->vcamio, 1800000, 1800000);
        ret = regulator_enable(gc6133c->vcamio);
    } else {
        ret = regulator_disable(gc6133c->vcamio);
    }
    if(ret)
	pr_err("%s regulator enable %d failed\n", __func__, flag);
    return;
			       ...
			       size_t len)
{
	...
	if (sscanf(buf, "%x %x", &databuf[0], &databuf[1]) == 2) {
		...
		gc6133c_i2c_read(g_gc6133c, databuf[0], &reg_data);
...
/* code at 2022/09/1 end */
	...
	gc6133c_i2c_read(g_gc6133c, 0x93, &reg_data);
...
/*	if (gpio_is_valid(gc6133c->reset_gpio)) {
	...
	//power on  camera
	gc6133c->vcamio = regulator_get(gc6133c->dev,"vcamio");
	gc6133c_iovdd_control(gc6133c, true);
	udelay(10);
	gc6133c->vcama = regulator_get(gc6133c->dev,"vcama");
	gc6133c_avdd_control(gc6133c, true);
	udelay(10);
	ret = pinctrl_select_state(gc6133c->gc6133c_pinctrl, gc6133c_mclk_on);
	udelay(10);
...
static void gc6133c_i2c_remove(struct i2c_client *client)
{
...
MODULE_AUTHOR("hanlina@longcheer.com>");
