...
struct pinctrl_state *sc080cs_mclk_on;
struct pinctrl_state *sc080cs_mclk_off;
/*****************************************************************
...
static struct sc080cs *sc080cs_malloc_init(struct i2c_client *client)
{
	...
	pr_err("%s enter , client_addr = 0x%02x\n", __func__,
...
void sc080cs_Init(struct sc080cs *sc080cs)
{
	...
	sc080cs_i2c_write(sc080cs, 0x518b,0x03);//again
	sc080cs_i2c_write(sc080cs, 0x518c,0x20);
	sc080cs_i2c_write(sc080cs, 0x518d,0x01);//expourse set
...
// define AVDD
static void sc080cs_avdd_control(struct sc080cs *sc080cs, bool flag)
{
    int ret;
	qvga_dev_info(sc080cs->dev, "%s enter\n", __func__);
	if (IS_ERR(sc080cs->vcama)) {
		qvga_dev_err(sc080cs->dev, "%s AVDD get regulator failed\n", __func__);
		regulator_put(sc080cs->vcama);
		return;
	}

	if (flag) {
		regulator_set_voltage(sc080cs->vcama, 2800000, 2800000);
		ret = regulator_enable(sc080cs->vcama);
	} else {
		ret = regulator_disable(sc080cs->vcama);
	}
        if(ret != 0)
	pr_err("%s regulator enable %d failed\n", __func__, flag);

    mdelay(10);
    ret = regulator_get_voltage(sc080cs->vcama);
    if (ret > 0)
        qvga_dev_err(sc080cs->dev, "%s  get AVDD vol:%d\n", __func__,ret);
    else
	qvga_dev_err(sc080cs->dev, "%s  get AVDD failed\n", __func__);

	return;
}
// define IOVDD
static void sc080cs_iovdd_control(struct sc080cs *sc080cs, bool flag)
{
    int ret;
    qvga_dev_info(sc080cs->dev, "%s enter\n", __func__);
    if (IS_ERR(sc080cs->vcamio)) {
        qvga_dev_err(sc080cs->dev, "%s  IOVDD get regulator failed\n", __func__);
        regulator_put(sc080cs->vcamio);
        return;
    }
    if (flag) {
        regulator_set_voltage(sc080cs->vcamio, 1800000, 1800000);
        ret = regulator_enable(sc080cs->vcamio);
    } else {
        ret = regulator_disable(sc080cs->vcamio);
    }
    if(ret != 0)
	pr_err("%s regulator enable %d failed\n", __func__, flag);

    mdelay(10);
    ret = regulator_get_voltage(sc080cs->vcamio);
    if (ret > 0)
        qvga_dev_err(sc080cs->dev, "%s  get IOVDD vol:%d\n", __func__,ret);
    else
	qvga_dev_err(sc080cs->dev, "%s  get IOVDD failed\n", __func__);

    return;
}
#if 0
void qvga_i2c4_mclk(bool flag)
{
	int ret;

	if(g_sc080cs == NULL || g_sc080cs->sc080cs_pinctrl == NULL || sc080cs_mclk_on == NULL || sc080cs_mclk_off == NULL)
	{
		return;
	}
	if(flag)
	{
		ret = pinctrl_select_state(g_sc080cs->sc080cs_pinctrl, sc080cs_mclk_on);
	}else{
		ret = pinctrl_select_state(g_sc080cs->sc080cs_pinctrl, sc080cs_mclk_off);
	}

	if (ret < 0) {
		qvga_dev_err(g_sc080cs->dev, "%s: sc080cs_pinctrl select failed for mclk %d\n", __func__,flag);
	}
}
#endif
...
{
	//sc080cs_iovdd_control(sc080cs, true);
	udelay(10);
	sc080cs_avdd_control(sc080cs, true);
	udelay(20);
...
static void sc080cs_hw_off(struct sc080cs *sc080cs)
{
	...
	sc080cs_avdd_control(sc080cs, false);
	//sc080cs_iovdd_control(sc080cs, false);
	sc080cs->hwen_flag = 0;
}
				...
				struct device_attribute *attr, char *buf)
{
	...
	//u16 light = 0;
	reg_data1 = sc080cs_i2c_read(g_sc080cs, 0x5160);
	//reg_data2 = sc080cs_i2c_read(g_sc080cs, 0x518b);
	//light = reg_data1;
        //qvga_dev_err(g_sc080cs->dev, "%s: sc080cs light=%d, %d, %d\n",   __func__, light, reg_data1, reg_data2);
        //qvga_dev_err(g_sc080cs->dev, "0x518b=0x%x,0x518c=0x%x,0x518d=0x%x,0x518e=0x%x\n",   sc080cs_i2c_read(g_sc080cs, 0x518b),sc080cs_i2c_read(g_sc080cs, 0x518c),sc080cs_i2c_read(g_sc080cs, 0x518d),sc080cs_i2c_read(g_sc080cs, 0x518e));
				...
				size_t len)
{
	...
	ret = sc080cs_GetSensorID(g_sc080cs);
	if (ret < 0) {
		qvga_dev_err(g_sc080cs->dev,"%s: start!\n", __func__);
	}
...
static int sc080cs_i2c_probe(struct i2c_client *client)
{
	...
	g_sc080cs->sc080cs_pinctrl = devm_pinctrl_get(&client->dev);
	if (IS_ERR_OR_NULL(g_sc080cs->sc080cs_pinctrl)) {
		qvga_dev_err(&client->dev, "%s: g_sc080cs->sc080cs_pinctrl not defined\n", __func__);
	} else {
		set_state = pinctrl_lookup_state(g_sc080cs->sc080cs_pinctrl, SC080CS_MCLK_ON);
		if (IS_ERR_OR_NULL(set_state)) {
			qvga_dev_err(&client->dev, "%s: g_sc080cs->sc080cs_pinctrl lookup failed for mclk on\n", __func__);
		} else {
			sc080cs_mclk_on = set_state;
		}
		set_state = pinctrl_lookup_state(g_sc080cs->sc080cs_pinctrl, SC080CS_MCLK_OFF);
		if (IS_ERR_OR_NULL(set_state)) {
			qvga_dev_err(&client->dev, "%s: g_sc080cs->sc080cs_pinctrl lookup failed for mclk off\n", __func__);
		} else {
	...
	//power on camera
	sc080cs->vcamio = regulator_get(sc080cs->dev,"vcamio");
	sc080cs_iovdd_control(sc080cs, true);
	udelay(10);
	sc080cs->vcama = regulator_get(sc080cs->dev,"vcama");
	sc080cs_avdd_control(sc080cs, true);
	mdelay(1);
	ret = pinctrl_select_state(g_sc080cs->sc080cs_pinctrl, sc080cs_mclk_on);
	if (ret < 0) {
		qvga_dev_err(&client->dev, "%s: g_sc080cs->sc080cs_pinctrl select failed for mclk on\n", __func__);
	}
	sc080cs_hw_on_reset(sc080cs);
	mdelay(5);
	sc080cs->hwen_flag = 1;
	...
	ret = pinctrl_select_state(g_sc080cs->sc080cs_pinctrl, sc080cs_mclk_off);
	if (ret < 0) {
		qvga_dev_err(&client->dev, "%s: g_sc080cs->sc080cs_pinctrl select failed for mclk off\n", __func__);
	}
#endif
	sc080cs_avdd_control(sc080cs, false);
	sc080cs_iovdd_control(sc080cs, false);
...
static void sc080cs_i2c_remove(struct i2c_client *client)
{
	struct sc080cs *sc080cs = i2c_get_clientdata(client);
	if (gpio_is_valid(sc080cs->reset_gpio))
		gpio_free(sc080cs->reset_gpio);
	devm_kfree(&client->dev, sc080cs);
	sc080cs = NULL;
}
...
module_exit(sc080cs_yuv_exit);
MODULE_AUTHOR("liushuangshuang1@longcheer.com>");
MODULE_DESCRIPTION("sc080cs yuv driver");
