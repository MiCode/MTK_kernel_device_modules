 ...
 * Filename:
 * ---------
 *     gc08a8_cxt_front_iii_mipi_raw_Sensor.c
 *
 * Project:
 * --------
 *     ALPS
...
#if 1
static kal_uint16 read_cmos_sensor_gc08a8_otp(kal_uint32 addr)
{
	kal_uint16 get_byte = 0;
...
static void gc08a8_fusion_id_read(void)
{
	int i;
	for (i=0; i<12; i++) {
		fusion_id_front[i] = read_cmos_sensor_gc08a8_otp(0x10+i);
...
static void gc08a8_sn_id_read(void)
{
	int i;
	for (i=0; i<25; i++) {
	        sn_front[i] = read_cmos_sensor_gc08a8_otp(0xEDC+i);
