...
static void xhci_link_rings(struct xhci_hcd *xhci, struct xhci_ring *src, struct xhci_ring *dst)
{
	...
	xhci_link_segments_(src->last_seg, dst->enq_seg->next, dst->type, chain_links);
	xhci_link_segments_(dst->enq_seg, src->first_seg, dst->type, chain_links);
...
static int xhci_alloc_segments_for_ring(struct xhci_hcd *xhci, struct xhci_ring *ring, gfp_t flags)
{
	...
	while (num < ring->num_segs) {
		...
		xhci_link_segments_(prev, next, ring->type, chain_links);
		prev = next;
		num++;
	}
	xhci_link_segments_(prev, ring->first_seg, ring->type, chain_links);
	ring->last_seg = prev;
 ...
 */
struct xhci_ring *xhci_ring_alloc_(struct xhci_hcd *xhci, unsigned int num_segs,
		...
		unsigned int max_packet, gfp_t mem_flags)
{
	...
	for (cur_stream = 1; cur_stream < num_streams; cur_stream++) {
		stream_info->stream_rings[cur_stream] =
			xhci_ring_alloc_(xhci, 2, TYPE_STREAM, max_packet, mem_flags);
		...
		struct usb_device *udev, gfp_t flags)
{
	...
	dev->eps[0].ring = xhci_ring_alloc_(xhci, 2, TYPE_CTRL, 0, flags);
	if (!dev->eps[0].ring)
		goto fail;
		...
		gfp_t mem_flags)
{
	...
	if (xhci_vendor_is_usb_offload_enabled(xhci, virt_dev, ep_index) &&
		...
		virt_dev->eps[ep_index].new_ring =
			xhci_ring_alloc_(xhci, 2, ring_type, max_packet, mem_flags);
	}
...
xhci_alloc_interrupter(struct xhci_hcd *xhci, unsigned int segs, gfp_t flags)
{
	...
	ir->event_ring = xhci_ring_alloc_(xhci, segs, TYPE_EVENT, 0, flags);
	if (!ir->event_ring) {
	...
	u32 imod_interval, unsigned int intr_num)
{
	...
	if (xhci_vendor_is_usb_offload_enabled(xhci, NULL, 0))
		ir = xhci_vendor_alloc_interrupter(xhci, segs, GFP_KERNEL);
	else
...
int xhci_mem_init(struct xhci_hcd *xhci, gfp_t flags)
{
	...
	xhci->cmd_ring = xhci_ring_alloc_(xhci, 1, TYPE_COMMAND, 0, flags);
	if (!xhci->cmd_ring)
		goto fail;
	xhci_dbg_trace_(xhci, trace_xhci_dbg_init_,
