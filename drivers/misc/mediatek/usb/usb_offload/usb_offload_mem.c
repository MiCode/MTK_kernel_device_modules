...
int mtk_offload_init_rsv_sram(int min_alloc_order)
{
	...
	sram_pwr_on = 0;
	if (sram_version == 0x3) {
		ret = mtk_offload_rsv_sram_pwr_ctrl(true);
		if (ret) {
			USB_OFFLOAD_ERR("Fail to enable SRAM power, ret:%d\n", ret);
			soc_free_sram(&rsv_sram);
			usb_offload_mem_buffer[mem_id].is_valid = false;
			goto INIT_RSV_SRAM_DONE;
		}
	} else
	...
	memset_io((void __iomem *)usb_offload_mem_buffer[mem_id].va_addr, 0, size);
	...
	if (uodev->adv_lowpwr && mem_id == USB_OFFLOAD_MEM_SRAM_ID) {
		...
		if (ret == 0) {
			if (!is_rsv) {
				...
				/* Wait for SRAM power to stabilize before accessing */
				usleep_range(1000, 1500);
				/* Clear dynamically allocated SRAM to avoid dirty data.
				 * Reserved pool is cleared in mtk_offload_init_rsv_sram(),
				 * but dynamic allocation needs explicit clearing here.
				 */
				memset_io((void __iomem *)buf->dma_area, 0, size);
			}
			goto ALLOC_SUCCESS;
		}
	}
