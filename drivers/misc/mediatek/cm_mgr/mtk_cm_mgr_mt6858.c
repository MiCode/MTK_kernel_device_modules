...
u32 cm_mgr_get_perfs_mt6858(int num)
{
...
static int cm_mgr_check_dram_type(void)
{
	...
	if ((ddr_type == TYPE_LPDDR4) || (ddr_type == TYPE_LPDDR4X) || (ddr_type == TYPE_LPDDR4P))
		cm_mgr_idx = CM_MGR_LP4;
	else if ((ddr_type == TYPE_LPDDR5) || (ddr_type == TYPE_LPDDR5X))
...
static void check_cm_mgr_status_mt6858(unsigned int cluster, unsigned int freq,
					unsigned int idx)
{
...
static int platform_cm_mgr_probe(struct platform_device *pdev)
{
	...
	// local_hk.cm_mgr_get_perfs = cm_mgr_get_perfs_mt6858;
	local_hk.check_cm_mgr_status = check_cm_mgr_status_mt6858;
...
static const struct of_device_id platform_cm_mgr_of_match[] = {
	{
		.compatible = "mediatek,mt6858-cm_mgr",
	},
	{},
};
