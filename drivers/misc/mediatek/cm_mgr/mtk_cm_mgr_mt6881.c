...
{
	unsigned int opp, offset=0, type, value, i=0, l;
	//Dynamic Table
	if (cm_mem_dynamic) {
		type = 1;
		l = cm_num_opp * nr_vc0;
...
u32 cm_mgr_get_perfs_mt6881(int num)
{
...
static void check_cm_mgr_status_mt6881(unsigned int cluster, unsigned int freq,
					unsigned int idx)
{
...
static int cm_mgr_check_dts_setting_mt6881(struct platform_device *pdev)
{
#if IS_ENABLED(CONFIG_MTK_DVFSRC)
	int i = 0;
#endif /* CONFIG_MTK_DVFSRC */
...
static int platform_cm_mgr_probe(struct platform_device *pdev)
{
	...
	ret = cm_mgr_check_dts_setting_mt6881(pdev);
	if (ret) {
	...
	// local_hk.cm_mgr_get_perfs = cm_mgr_get_perfs_mt6881;
	local_hk.check_cm_mgr_status = check_cm_mgr_status_mt6881;
...
static const struct of_device_id platform_cm_mgr_of_match[] = {
	{
		.compatible = "mediatek,mt6881-cm_mgr",
	},
	{},
};
...
MODULE_AUTHOR("Olivia-wh Lin<Olivia-wh.lin@mediatek.com>");
MODULE_LICENSE("GPL");
