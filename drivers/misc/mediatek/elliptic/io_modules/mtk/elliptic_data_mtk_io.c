...
#define ELLIPTIC_DRAM_PAYLOAD_MAX_OFFSET ((uint16_t) 64)
...
static void elliptic_data_io_ipi_handle_task(unsigned long data)
{
    ...
    while (kfifo_is_empty_spinlocked(&elliptic_ipi_handler_data.fifo, &elliptic_ipi_handler_data.tasklet_fifo_isr_spinlock) == 0) { // Read all ipi messages in fifo
        msg_size = kfifo_out_spinlocked(&elliptic_ipi_handler_data.fifo, &ipi_msg, sizeof(elliptic_ipi_scp_to_host_message_t),&elliptic_ipi_handler_data.tasklet_fifo_isr_spinlock);
        ...
        while( current_ipi_counter != target_ipi_message_count ) {
            ...
            else if((current_ipi_counter == target_ipi_message_count ) && (ipi_msg.header.data_size <= sizeof(ipi_msg.data) ) )
            {
            ...
            else if( header == NULL )
            {
                ...
                pr_err( "[ELUS] did not find payload with id %u, target_ipi_message_count=%u", (unsigned int)current_ipi_counter,   (unsigned int)target_ipi_message_count);
                continue;
            }
            ...
            case ELLIPTIC_ULTRASOUND_PARAM_ID_DIAGNOSTICS_DATA:
                copy_to_local_ap_cache("diag_data",
                            ...
                            header, payload);
                break;
	case ELLIPTIC_ULTRASOUND_PARAM_ID_GRYO_FREQ:
                copy_to_local_ap_cache("gyro_freq",
                            ELLIPTIC_OBJ_ID_GYRO_FREQ,
                            ELLIPTIC_GYRO_FREQ_SIZE,
                            header, payload);
                break;
            case ELLIPTIC_ULTRASOUND_PARAM_ID_ENGINE_DATA:
	    case ELLIPTIC_ULTRASOUND_PARAM_ID_ENGINE_BINPAYLOAD:
	...
	pr_info("[ELUS]tasklet finished\n");
}

////
/* Will be called from MTK SCP IPI driver when data arrives from DSP */
static int elliptic_data_io_ipi_handler(
	unsigned int id, void *prdata, void *data, unsigned int len)
{
    unsigned int ret;
    pr_info("[ELUS] %s() enter", __func__);
    ret = kfifo_in_spinlocked(&elliptic_ipi_handler_data.fifo, data, sizeof(elliptic_ipi_scp_to_host_message_t),&elliptic_ipi_handler_data.tasklet_fifo_isr_spinlock);
...
int elliptic_data_io_initialize(void)
{
	...
	current_ipi_counter = 0;
	spin_lock_init(&elliptic_ipi_handler_data.tasklet_fifo_isr_spinlock);
	...
	if (status != SCP_IPI_DONE) {
		pr_err("[ELUS] failed to register scp ipi");
		return -1;
	}else{
		pr_err("[ELUS] Register scp ipi, recv id=%d, send id=%d\n", IPI_IN_ELLIPTIC_ULTRA_0, IPI_OUT_ELLIPTIC_ULTRA_0);
	}

	return 0;
}

static DEFINE_MUTEX(ipi_send_mutex);
	...
	size_t data_size)
{
	...
	mutex_lock(&ipi_send_mutex);
	pr_debug("[ELUS] %s, get lock\n", __func__);
	/* clear send buffer */
	...
	do {
		ipi_result = mtk_ipi_send(&scp_ipidev,
					IPI_OUT_ELLIPTIC_ULTRA_0,
					0,
					&host_message,
					PIN_OUT_SIZE_ELLIPTIC_ULTRA_0,
					0);
		if (ipi_result == IPI_PIN_BUSY) {
			if (retry++ == ELLIPTIC_SCP_IPI_RETRY_TIMES) {
				pr_err("%s: [ELUS] retry fail\n", __func__);
				break;
			}
			if (retry % 100 == 0)
				usleep_range(1000, 2000);
		}
	} while (ipi_result == IPI_PIN_BUSY);

	if (ipi_result != IPI_ACTION_DONE) {
		pr_err("[ELUS] %s failed to send %d \n", __func__, ipi_result);
		mutex_unlock(&ipi_send_mutex);
		return 0;
	}
