									...
									int cp_mode)
{
	...
	if (revchg_is_on) {
		// enable revert cp boost
		if (cp_mode == SC8561_REVERSE_1_2_CONVERTER_MODE) {
			mca_log_err("[REV_CHG] start revert 1_2 cp");
			charger_dev_enable_acdrv_manual(cp_master, true);
			charger_dev_cp_set_adjustadble_timeout(cp_master, 0x00);
			charger_dev_cp_set_pmid2outuvp_th(cp_master, 139);
			charger_dev_cp_set_vout2out_uvp_high(cp_master, true);
			charger_dev_cp_set_mode(cp_master, SC8561_REVERSE_1_2_CONVERTER_MODE);
			charger_dev_cp_enable_wpcgate(cp_master, false);
			charger_dev_cp_enable_ovpgate(cp_master, true);
			charger_dev_cp_rev_chg_config(cp_master, true);
			charger_dev_cp_set_qb(cp_master, true);
			//msleep(1500);
			charger_dev_enable(cp_master, true);
			charger_dev_cp_dump_register(cp_master);
			extcon->vbus_vol_request = 9000;
		} else if (cp_mode == SC8561_REVERSE_1_4_CONVERTER_MODE) {
			mca_log_err("[REV_CHG] start revert 1_4 cp");
			charger_dev_enable_acdrv_manual(cp_master, true);
			charger_dev_cp_set_adjustadble_timeout(cp_master, 0x00);
			charger_dev_cp_set_pmid2outuvp_th(cp_master, 54);
			charger_dev_cp_set_vout2out_uvp_high(cp_master, true);
			charger_dev_cp_set_mode(cp_master, SC8561_REVERSE_1_4_CONVERTER_MODE);
			charger_dev_cp_enable_wpcgate(cp_master, false);
			charger_dev_cp_enable_ovpgate(cp_master, true);
			charger_dev_cp_set_qb(cp_master, true);
			//msleep(1500);
		...
		charger_dev_cp_set_qb(cp_master, false);
		charger_dev_cp_set_mode(cp_master, SC8561_FORWARD_2_1_CHARGER_MODE);
		charger_dev_cp_set_adjustadble_timeout(cp_master, 5120);
		charger_dev_cp_set_pmid2outuvp_th(cp_master, 30);
		charger_dev_cp_set_vout2out_uvp_high(cp_master, false);
		charger_dev_cp_dump_register(cp_master);
		charger_dev_cp_rev_chg_config(cp_master, false);
		usb_set_property(USB_PROP_TYPEC_REVERSE_CHG, 0);
	}
							...
							bool is_on)
{
	...
	int set_rev_cp_mode = 0;
	int revcbl_swap_stat = 0;
	bool rev_pwr_delay = true;
#ifdef CONFIG_SUPPORT_SOUTHCHIP_PDPHY
	int typec_port1 = 0;
	int typec_port0 = 0;
#endif
	if (extcon->support_quick_revchg) {
		usb_get_property(USB_PROP_TYPEC_IS_IPHONE17_SERIES, &is_iphone17_series);
		if (is_iphone17_series && !extcon->enable_iphone_quick_revchg) {
			set_rev_cp_mode = SC8561_REVERSE_1_4_CONVERTER_MODE;
		} else {
			set_rev_cp_mode = SC8561_REVERSE_1_2_CONVERTER_MODE;
		}
	}
	/* vbus is optional */
	...
	rev_cable_boostable = (rev_cable_status >> 16) & 0x7FFF;
	rev_cable_boosted   = rev_cable_status & 0xFFFF;
	mca_log_err("rev_cable: checked=%d, boostable=%d, boosted=%d (raw=0x%x)\n",
					rev_cable_checked, rev_cable_boostable, rev_cable_boosted, rev_cable_status);

	if ((extcon->vbus_on == is_on && ! (rev_quick_chg || rev_cable_boostable || rev_cable_boosted) && last_rqc_vbus == extcon->vbus_vol_request) || !vbus)
		return 0;
#ifdef CONFIG_SUPPORT_SOUTHCHIP_PDPHY
	...
	if (is_on) {
		...
		} else if (extcon->support_quick_revchg && extcon->otg_boost_src== BOOST_SRC_CHARGER) {
			charger_dev_enable_acdrv_manual(cp_master, true);
			charger_dev_is_enabled(cp_master, &is_CpEn);
			if (!is_CpEn && (rev_quick_chg || (rev_cable_boostable && rev_cable_boosted)) && extcon->vbus_vol_request > 5000) {
				charger_dev_cp_set_revchg(cp_master, true, set_rev_cp_mode);
			} else if (!(rev_quick_chg || (rev_cable_boostable && rev_cable_boosted))) {
				charger_dev_cp_set_mode(cp_master, SC8561_REVERSE_1_1_CONVERTER_MODE);
			}
			usb_get_property(USB_PROP_PMIC_VBUS, &vbus_vol);
			mca_log_err("get vbus_vol: %d\n", vbus_vol);
			if (vbus_vol < 800) {
				if (extcon->vbus_vol) {
					if (extcon->support_revcable) {
						usb_get_property(USB_PROP_TYPEC_REVCBL_SWAP_STAT, &revcbl_swap_stat);
						rev_pwr_delay = !revcbl_swap_stat;

						if (rev_pwr_delay) {
							mca_log_err("revcbl_swap_stat = %d, delay 150ms\n", revcbl_swap_stat);
							msleep(150);
						}
					}
				...
				mca_log_err("vbus regulator enable, vbus = %d, cur = %d\n",extcon->vbus_vol, extcon->vbus_cur);
			}
			if((rev_quick_chg || (rev_cable_boostable && rev_cable_boosted)) && extcon->vbus_vol_request > 5000){
				xm_enable_reverse_chg(cp_master, extcon, true, set_rev_cp_mode);
			}
			if (last_rqc_vbus > extcon->vbus_vol_request && !(rev_quick_chg || (rev_cable_boostable && rev_cable_boosted))) {
				xm_enable_reverse_chg(cp_master, extcon, false, set_rev_cp_mode);
			}
			usb_set_property(USB_PROP_TYPEC_RQC_CONDITION_CHECK, 1);
		} else {
			if (extcon->apple_delay_50ms) {
				mdelay(50); /* for apple compatible */
				mca_log_err("delay end!\n");
			} else {
				mca_log_err("no delay!\n");
			}
			charger_dev_cp_set_mode(cp_master, SC8561_REVERSE_1_1_CONVERTER_MODE);
			charger_dev_enable_acdrv_manual(cp_master, true);
			if (extcon->vbus_vol) {
		...
		if (gpio_is_valid(extcon->vdd_boost_en_gpio_a)) {
			...
			charger_dev_cp_set_revchg(cp_master, false, set_rev_cp_mode);
			xm_enable_reverse_chg(cp_master, extcon, false, set_rev_cp_mode);
			usb_set_property(USB_PROP_TYPEC_RQC_CONDITION_CHECK, 0);
...
static int mtk_usb_extcon_vbus_init(struct mtk_extcon_info *extcon)
{
	...
	extcon->support_quick_revchg = of_property_read_bool(dev->of_node, "support_quick_revchg");
	extcon->enable_iphone_quick_revchg = of_property_read_bool(dev->of_node, "enable_iphone_quick_revchg");
	extcon->support_revcable = of_property_read_bool(dev->of_node, "support_revcable");
	extcon->apple_delay_50ms = of_property_read_bool(dev->of_node, "apple_delay_50ms");
	if (!of_property_read_u32(dev->of_node, "otg_boost_src",
		&extcon->otg_boost_src))
		dev_info(dev, "otg_boost_src=%d", extcon->otg_boost_src);
