...
#ifdef CONFIG_SUPPORT_SOUTHCHIP_PDPHY
static void project_mtk_usb_extcon_psy_detector(struct work_struct *work)
{
...
#ifdef CONFIG_SUPPORT_SOUTHCHIP_PDPHY
	INIT_DELAYED_WORK(&extcon->wq_psy, project_mtk_usb_extcon_psy_detector);
#else
...
static int project_mtk_usb_extcon_set_vbus(struct mtk_extcon_info *extcon,
							bool is_on)
{
...
#ifdef CONFIG_SUPPORT_SOUTHCHIP_PDPHY
	project_mtk_usb_extcon_set_vbus(extcon, is_on);
#else
...
static int project_mtk_extcon_tcpc_notifier(struct notifier_block *nb,
		unsigned long event, void *data)
{
	...
	case TCP_NOTIFY_SOURCE_VBUS:
		...
		project_mtk_usb_extcon_set_vbus(extcon, vbus_on);
		break;
...
static int project_mtk_usb_extcon_tcpc_init(struct mtk_extcon_info *extcon)
{
	...
	for (i = 0; i < extcon->nr_port; i++) {
		...
		extcon->tcpc_nb[i].nb.notifier_call = project_mtk_extcon_tcpc_notifier;
...
static void mtk_usb_extcon_detect_cable(struct work_struct *work)
{
	...
	if (role == USB_ROLE_HOST)
		project_mtk_usb_extcon_set_vbus(extcon, true);
	else
		project_mtk_usb_extcon_set_vbus(extcon, false);
#else
	if (role == USB_ROLE_HOST)
...
static int mtk_usb_extcon_probe(struct platform_device *pdev)
{
	...
	/* tcpc */
#ifdef CONFIG_SUPPORT_SOUTHCHIP_PDPHY
	ret = project_mtk_usb_extcon_tcpc_init(extcon);
#else
