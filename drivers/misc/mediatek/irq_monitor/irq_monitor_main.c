...
static void irq_mon_tracepoint_init(void)
{
	for_each_kernel_tracepoint(irq_mon_tracepoint_lookup,
				   (void *)irq_mon_tracepoint_table);
	/* probe tracepoints for all tracers if default enabled */
	if (b_default_enabled) {
		irq_mon_tracer_tracing_set(&irq_handler_tracer, true);
		irq_mon_tracer_tracing_set(&softirq_tracer, true);
		irq_mon_tracer_tracing_set(&ipi_tracer, true);
		irq_mon_tracer_tracing_set(&irq_off_tracer, true);
		irq_mon_tracer_tracing_set(&preempt_off_tracer, true);
		irq_mon_tracer_tracing_set(&hrtimer_expire_tracer, true);
	}
}

static void irq_mon_tracepoint_exit(void)
{
	struct irq_mon_tracepoint *t;
