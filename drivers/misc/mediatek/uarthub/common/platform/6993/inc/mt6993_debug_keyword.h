...
BOU_UARTHUB_ERROR_LOG_INFO lst_uarthub_log_info[] = {
	...
	[log_uart1_det_xoff]                    = {"UART1_DET_XOFF",                    mod_uarthub, mod_uarthub},
	...
	[log_uart1_frame_error]                 = {"UART1_FRAME_ERROR",                 mod_uarthub, mod_uarthub},
	...
	[log_ap_tx_tmo_apdma_err]               = {"AP_TX_CMD_TMO_APDMA_ERR",           mod_uarthub, mod_ap_dma},
	...
	[log_uart1_tx_woffset_not_empty]        = {"UART1_TX_WOFFSET_NOT_EMPTY",        mod_uarthub, mod_uarthub},
	...
	[log_dev1_tx_timeout_err]               = {"DEV1_TX_TIMEOUT_ERR",               mod_uarthub, mod_uarthub},
	...
	[log_dev1_tx_pkt_type_err]              = {"DEV1_TX_PKT_TYPE_ERR",              mod_uarthub, mod_uarthub},
	...
	[log_dev1_tx_err]                       = {"DEV1_TX_ERR",                       mod_uarthub, mod_uarthub},
	[log_dev2_tx_err]                       = {"DEV2_TX_ERR",                       mod_uarthub, mod_adsp_host}
};

#define BT_OVER_UAER_DEBUG_LOG_ERROR_KEYWORD "BT_OVER_UART_LOG_ERROR"

#define BT_OVER_UAER_DUMP_LOG(_err, _log, _tag) \
	pr_notice("%s%s%s%s:%.2d-%.2d-%.4d-%d/%s/%s/%s/%s\n", \
		((_tag == NULL) ? "" : "["), \
		((_tag == NULL) ? "" : _tag), \
		((_tag == NULL) ? "" : "] " ), \
		BT_OVER_UAER_DEBUG_LOG_ERROR_KEYWORD, \
		lst_uarthub_log_info[_log].mod_id, \
		lst_uarthub_log_info[_log].next_mod_id, _log, \
		((_err == 0) ? 0 : 1), \
		((_err == 0) ? "warning" : "error"), \
		lst_uarthub_log_info[_log].name, \
		lst_mod_id_name[lst_uarthub_log_info[_log].mod_id].name, \
		lst_mod_id_name[lst_uarthub_log_info[_log].next_mod_id].name)
...
#define UARTHUB_DEBUG_PRINT_AP_TX_CMD_TMO_PKT_CNT_ERR_DEBUG_KEYWORD(_ft, _lt, _fr, _lr, _err) \
	do {\
		if (_ft == _lt)\
			BT_OVER_UAER_DUMP_LOG(_err, \
				log_ap_tx_tmo_tx_pkt_cnt_err, __func__);\
		else if (_fr == _lr)\
			BT_OVER_UAER_DUMP_LOG(_err, \
				log_ap_tx_tmo_rx_pkt_cnt_err, __func__);\
	} while (0)
...
_fat, _lat, _far, _lar, _fct, _lct, _fcr, _lcr, _err) \
	do {\
		if (apuart_base_map_mt6993[3] != NULL) {\
			if (_fat == _lat)\
				BT_OVER_UAER_DUMP_LOG(_err, \
					log_ap_tx_tmo_apuart_tx_byte_cnt_err, __func__);\
			else if (_fct == _lct)\
				BT_OVER_UAER_DUMP_LOG(_err, \
					log_ap_tx_tmo_uartcmm_tx_byte_cnt_err, __func__);\
			else if (_fcr == _lcr)\
				BT_OVER_UAER_DUMP_LOG(_err, \
					log_ap_tx_tmo_uartcmm_rx_byte_cnt_err, __func__);\
			else if (_far == _lar)\
				BT_OVER_UAER_DUMP_LOG(_err, \
					log_ap_tx_tmo_apuart_rx_byte_cnt_err, __func__);\
			else\
				BT_OVER_UAER_DUMP_LOG(_err, \
					log_ap_tx_tmo_apdma_err, __func__);\
		} else {\
