...
static int uarthub_ut_ip_timeout_init_fsm_ctrl_by_unit_mt6991(
	int dev_index, enum uarthub_trx_type trx, int enable_fsm);
static int uarthub_ut_ip_clear_rx_data_irq_by_unit_mt6991(int dev_index);
	...
	unsigned char *cbuf, int szBuf, int wp, const unsigned char *data, const int sz)
{
	...
	/* Bug :mt6991 : Always return 0 in top mode */
	return 0;
}
...
static int __uh_ut_check_dbgm_mt6991(
...
int uarthub_ut_ip_timeout_init_fsm_ctrl_by_unit_mt6991(
	int dev_index, enum uarthub_trx_type trx, int enable_fsm)
{
	...
	uarthub_clear_all_ut_irq_sta_mt6991();
	...
	for (dev_id = 0; dev_id < DEV_CMM; dev_id++)
		for (trx = 0; trx < TRX; trx++)
			for (fsm_en = 0; fsm_en < 2; fsm_en++) {
				...
				state = uarthub_ut_ip_timeout_init_fsm_ctrl_by_unit_mt6991(dev_id, trx, fsm_en);
...
int uarthub_ut_ip_clear_rx_data_irq_by_unit_mt6991(int dev_index)
{
	int state = 0;
	...
	uarthub_set_host_trx_request_mt6991(dev_index, TRX);
	usleep_range(3000, 3010);
#if !(SSPM_DRIVER_EN) || (UARTHUB_SUPPORT_FPGA)
	UARTHUB_REG_WRITE(IRQ_CLR_ADDR, 0xFFFFFFFF);
	usleep_range(1000, 1010);
#endif
	state = uarthub_is_host_uarthub_ready_state_mt6991(dev_index);
	if (state != 1) {
	...
	uarthub_clear_host_trx_request_mt6991(dev_index, TRX);
	usleep_range(3000, 3010);
#if !(SSPM_DRIVER_EN) || (UARTHUB_SUPPORT_FPGA)
	UARTHUB_REG_WRITE(IRQ_CLR_ADDR, 0xFFFFFFFF);
	usleep_range(1000, 1010);
...
int uarthub_ut_ip_clear_rx_data_irq_mt6991(void)
{
	...
	for (dev_id = 0; dev_id < DEV_CMM; dev_id++) {
		item_cnt++;
		state = uarthub_ut_ip_clear_rx_data_irq_by_unit_mt6991(dev_id);
...
int uarthub_verify_cmm_loopback_sta_mt6991(void)
{
	...
	usleep_range(1000, 1010);
#endif
	state = uarthub_is_host_uarthub_ready_state_mt6991(0);
	if (state != 1) {
		pr_notice("[%s] FAIL: dev0 uarthub is NOT ready(%d)", __func__, state);
		state = UARTHUB_UT_ERR_HUB_READY_STA;
		goto verify_err;
	...
	state = uarthub_uartip_send_data_internal_mt6991(
...
int uarthub_verify_cmm_trx_connsys_sta_mt6991(int rx_delay_ms)
{
	...
	uarthub_config_uartip_dma_en_ctrl_mt6991(3, RX, 0);
	...
	state = uarthub_uartip_write_tx_data_mt6991(0, BT_TEST_CMD, sizeof(BT_TEST_CMD));
	...
	recv_rx_len = 0;
	state = uarthub_uartip_read_rx_data_mt6991(3, evtBuf, TRX_BUF_LEN, &recv_rx_len);
	if (recv_rx_len > 0) {
		len = 0;
...
verify_err:
	...
	if (state != 0)
		__uh_ut_debug_dump(0, 1, 1, 1);
	/* Uninitialize */
	uarthub_config_uartip_dma_en_ctrl_mt6991(3, RX, 1);
	uarthub_reset_intfhub_mt6991();
	uarthub_set_host_loopback_ctrl_mt6991(0, 1, 0);
...
int uarthub_ut_ip_verify_debug_monitor_packet_info_mode_mt6991(void)
{
	...
	for (i = 0; i < ITEM_CNT_01; i++) {
		...
		/* bug: mt6991: rx ptr does not increase after seq of 0xFF and 0xF0 */
		if (i == 10)
			wp_rx -= 1;

		state = __uh_ut_check_dbgm_mt6991(DBGM_MODE_PKT_INFO, wp_tx, wp_rx, dbgm_tx_exp, dbgm_rx_exp);
		EXPECT_INT_EQ(state, 0, verify_err, "comparing debug mointor data is mismatched.");

		/* bug: mt6991:(recover) rx ptr does not increase after seq of 0xFF and 0xF0 */
		if (i == 10)
			wp_rx += 1;
...
int uarthub_ut_ip_verify_debug_monitor_check_data_mode_mt6991(void)
{
	...
	for (i = 0; i < ITEM_CNT_01; i++) {
		...
		state = __uh_ut_check_dbgm_mt6991(DBGM_MODE_CHECK_DATA, wp_tx, wp_rx, dbgm_tx_exp, dbgm_rx_exp);
		EXPECT_INT_EQ(state, 0, verify_err, "comparing debug mointor data is mismatched.");
...
int uarthub_ut_ip_verify_debug_monitor_crc_result_mode_mt6991(void)
{
	...
	for (i = 0; i < ITEM_CNT_01; i++){
		...
		state = __uh_ut_check_dbgm_mt6991(DBGM_MODE_CRC_RESULT, wp_tx, wp_rx, dbgm_tx_exp, dbgm_rx_exp);
		EXPECT_INT_EQ(state, 0, verify_err, "comparing debug mointor data is mismatched.");
