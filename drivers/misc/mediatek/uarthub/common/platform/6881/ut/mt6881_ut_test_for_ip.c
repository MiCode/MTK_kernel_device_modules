...
static int uarthub_ut_ip_clear_rx_data_irq_by_unit_mt6881(int dev_index);
...
#endif
	if (uarthub_is_host_uarthub_ready_state_mt6881(0) != 1)
		return -1;
...
static inline int __uh_ut_enable_loopback_ctrl(void)
{
	...
	uarthub_set_host_loopback_ctrl_mt6881(1, 1, 1);
	uarthub_set_host_loopback_ctrl_mt6881(2, 1, 1);
...
static inline int __uh_ut_disable_loopback_ctrl(void)
{
	...
	uarthub_set_host_loopback_ctrl_mt6881(1, 1, 0);
	uarthub_set_host_loopback_ctrl_mt6881(2, 1, 0);
	...
	unsigned char *cbuf, int szBuf, int wp, const unsigned char *data, const int sz)
{
	...
	/* Bug :mt6881 : Always return 0 in top mode */
	return 0;
}
...
static int __uh_ut_check_dbgm_mt6881(
	...
	int dev_index, enum uarthub_trx_type trx, int enable_fsm)
{
	 ...
	 *	1 - 6985 mode = timeout err + pkt err
	 *	0 - 6989 mode  = timeout err
	 */
	switch (dev_index) {
	case 0:
	...
	uarthub_clear_all_ut_irq_sta_mt6881();
...
#define __GET_VAL_FLD(f, v) (((v)&REG_FLD_MASK(f)) >> REG_FLD_SHIFT(f))
	...
	//Check pkt type err if 6985 mode (enable_fsm=1). No trigger otherwise.
...
int uarthub_ut_ip_help_info_mt6881(void)
{
	UTLOG("Help info.");
	return 0;
}
	...
	for (dev_id = 0; dev_id < DEV_CMM; dev_id++)
		for (trx = 0; trx < TRX; trx++)
			for (fsm_en = 0; fsm_en < 2; fsm_en++) {
				...
				state = uarthub_ut_ip_timeout_init_fsm_ctrl_by_unit_mt6881(dev_id, trx, fsm_en);
...
int uarthub_ut_ip_clear_rx_data_irq_by_unit_mt6881(int dev_index)
{
	int state = 0;
...
int uarthub_ut_ip_clear_rx_data_irq_mt6881(void)
{
	...
	for (dev_id = 0; dev_id < DEV_CMM; dev_id++) {
		item_cnt++;
		state = uarthub_ut_ip_clear_rx_data_irq_by_unit_mt6881(dev_id);
...
int uarthub_verify_cmm_loopback_sta_mt6881(void)
{
	...
	state = uarthub_uartip_send_data_internal_mt6881(
...
int uarthub_verify_cmm_trx_connsys_sta_mt6881(int rx_delay_ms)
{
	int state = 0;
...
int uarthub_ut_ip_verify_debug_monitor_packet_info_mode_mt6881(void)
{
	int state = 0;
	...
	for (i = 0; i < ITEM_CNT_01; i++) {
		...
		/* bug: mt6881: rx ptr does not increase after seq of 0xFF and 0xF0 */
		if (i == 10)
			wp_rx -= 1;

		state = __uh_ut_check_dbgm_mt6881(DBGM_MODE_PKT_INFO, wp_tx, wp_rx, dbgm_tx_exp, dbgm_rx_exp);
		EXPECT_INT_EQ(state, 0, verify_err, "comparing debug mointor data is mismatched.");

		/* bug: mt6881:(recover) rx ptr does not increase after seq of 0xFF and 0xF0 */
		if (i == 10)
			wp_rx += 1;
...
int uarthub_ut_ip_verify_debug_monitor_check_data_mode_mt6881(void)
{
	int state = 0;
	unsigned char dbgm_tx_exp[16] = {0};
	unsigned char dbgm_rx_exp[16] = {0};
	...
	for (i = 0; i < ITEM_CNT_01; i++) {
		...
		state = __uh_ut_check_dbgm_mt6881(DBGM_MODE_CHECK_DATA, wp_tx, wp_rx, dbgm_tx_exp, dbgm_rx_exp);
		EXPECT_INT_EQ(state, 0, verify_err, "comparing debug mointor data is mismatched.");
...
int uarthub_ut_ip_verify_debug_monitor_crc_result_mode_mt6881(void)
{
	int state = 0;
	unsigned char dbgm_tx_exp[16] = {0};
	unsigned char dbgm_rx_exp[16] = {0};
	...
	for (i = 0; i < ITEM_CNT_01; i++){
		...
		state = __uh_ut_check_dbgm_mt6881(DBGM_MODE_CRC_RESULT, wp_tx, wp_rx, dbgm_tx_exp, dbgm_rx_exp);
		EXPECT_INT_EQ(state, 0, verify_err, "comparing debug mointor data is mismatched.");
