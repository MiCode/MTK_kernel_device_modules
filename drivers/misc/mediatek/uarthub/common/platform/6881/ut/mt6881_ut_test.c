...
struct uarthub_ut_test_ops_struct mt6881_plat_ut_test_data = {
	.uarthub_plat_is_ut_testing = uarthub_is_ut_testing_mt6881,
	.uarthub_plat_is_host_uarthub_ready_state = uarthub_is_host_uarthub_ready_state_mt6881,
	.uarthub_plat_get_host_irq_sta = uarthub_get_host_irq_sta_mt6881,
	.uarthub_plat_get_intfhub_active_sta = uarthub_get_intfhub_active_sta_mt6881,
	.uarthub_plat_clear_host_irq = uarthub_clear_host_irq_mt6881,
	.uarthub_plat_mask_host_irq = uarthub_mask_host_irq_mt6881,
	.uarthub_plat_config_host_irq_ctrl = uarthub_config_host_irq_ctrl_mt6881,
	.uarthub_plat_get_host_rx_fifo_size = uarthub_get_host_rx_fifo_size_mt6881,
	.uarthub_plat_get_cmm_rx_fifo_size = uarthub_get_cmm_rx_fifo_size_mt6881,
	.uarthub_plat_config_uartip_dma_en_ctrl = uarthub_config_uartip_dma_en_ctrl_mt6881,
	.uarthub_plat_reset_fifo_trx = uarthub_reset_fifo_trx_mt6881,
	.uarthub_plat_uartip_write_data_to_tx_buf = uarthub_uartip_write_data_to_tx_buf_mt6881,
	.uarthub_plat_uartip_read_data_from_rx_buf = uarthub_uartip_read_data_from_rx_buf_mt6881,
	.uarthub_plat_is_uartip_tx_buf_empty_for_write =
		uarthub_is_uartip_tx_buf_empty_for_write_mt6881,
	.uarthub_plat_is_uartip_rx_buf_ready_for_read =
		uarthub_is_uartip_rx_buf_ready_for_read_mt6881,
	.uarthub_plat_is_uartip_throw_xoff = uarthub_is_uartip_throw_xoff_mt6881,
	.uarthub_plat_config_uartip_rx_fifo_trig_thr =
		uarthub_config_uartip_rx_fifo_trig_thr_mt6881,
};
...
int uarthub_get_host_rx_fifo_size_mt6881(int dev_index)
{
	...
	return UARTHUB_REG_READ_BIT(DEBUG_7(uartip_base_map_mt6881[dev_index]), 0x3F);
}

int uarthub_get_cmm_rx_fifo_size_mt6881(void)
{
	return UARTHUB_REG_READ_BIT(DEBUG_7(uartip_base_map_mt6881[uartip_id_cmm]), 0x3F);
}

int uarthub_config_uartip_dma_en_ctrl_mt6881(int dev_index, enum uarthub_trx_type trx, int enable)
{
	void __iomem *uarthub_dev_base = uartip_base_map_mt6881[uartip_id_ap];
	int i = 0;
...
int uarthub_reset_fifo_trx_mt6881(void)
{
	...
	for (i = 0; i <= UARTHUB_MAX_NUM_DEV_HOST; i++)
		UARTHUB_REG_WRITE(FCR_ADDR(uartip_base_map_mt6881[i]), 0xC0);
...
int uarthub_reset_intfhub_mt6881(void)
{
	...
	for (i = 0; i <= UARTHUB_MAX_NUM_DEV_HOST; i++)
		UARTHUB_REG_WRITE(FCR_ADDR(uartip_base_map_mt6881[i]), 0xC1);
...
int uarthub_uartip_write_data_to_tx_buf_mt6881(int dev_index, int tx_data)
{
	...
	UARTHUB_REG_WRITE(THR_ADDR(uartip_base_map_mt6881[dev_index]), tx_data);
	return 0;
}
...
int uarthub_uartip_read_data_from_rx_buf_mt6881(int dev_index)
{
	...
	return UARTHUB_REG_READ(RBR_ADDR(uartip_base_map_mt6881[dev_index]));
}
...
int uarthub_is_uartip_tx_buf_empty_for_write_mt6881(int dev_index)
{
	...
	return LSR_GET_TEMT(LSR_ADDR(uartip_base_map_mt6881[dev_index]));
}
...
int uarthub_is_uartip_rx_buf_ready_for_read_mt6881(int dev_index)
{
	...
	return LSR_GET_DR(LSR_ADDR(uartip_base_map_mt6881[dev_index]));
}
...
int uarthub_is_uartip_throw_xoff_mt6881(int dev_index)
{
	...
	return ((UARTHUB_DEBUG_GET_XCSTATE_WSEND_XOFF_VAL(
		uartip_base_map_mt6881[dev_index]) & 0x6) ? 1 : 0);
}
...
int uarthub_config_uartip_rx_fifo_trig_thr_mt6881(int dev_index, int size)
{
	...
	pr_info("[%s] FCR_RD_ADDR_1=[0x%x]\n",
		__func__, UARTHUB_REG_READ(FCR_RD_ADDR(uartip_base_map_mt6881[dev_index])));

	REG_FLD_RD_SET(FCR_FLD_RFTL1_RFTL0,
		FCR_RD_ADDR(uartip_base_map_mt6881[dev_index]),
		FCR_ADDR(uartip_base_map_mt6881[dev_index]), size);
...
int uarthub_uartip_write_tx_data_mt6881(int dev_index, unsigned char *p_tx_data, int tx_len)
{
	...
	for (i = 0; i < tx_len; i++) {
		retry = 200;
		while (retry-- > 0) {
			if (uarthub_is_uartip_tx_buf_empty_for_write_mt6881(dev_index) == 1) {
				uarthub_uartip_write_data_to_tx_buf_mt6881(dev_index, p_tx_data[i]);
				break;
			}
			usleep_range(5, 6);
		}
	...
	int dev_index, unsigned char *p_rx_data, int rx_len, int *p_recv_rx_len)
{
	...
	for (i = 0; i < rx_len; i++) {
		retry = 200;
		while (retry-- > 0) {
			if (uarthub_is_uartip_rx_buf_ready_for_read_mt6881(dev_index) == 1) {
				p_rx_data[i] =
					uarthub_uartip_read_data_from_rx_buf_mt6881(dev_index);
				*p_recv_rx_len = i+1;
				break;
			}
			usleep_range(5, 6);
		...
		if (retry <= 0) {
			state = 0;
			if (dev_index == 3)
				state = uarthub_get_cmm_rx_fifo_size_mt6881();
			else
				state = uarthub_get_host_rx_fifo_size_mt6881(dev_index);
			if (state > 0)
				retry = 200;
			else
				break;
			...
			int rx_dev_id, unsigned char *rcvBuf, int szBuf)
{
	...
	while (curTx < szTx) {
		for (retry = 200; retry > 0; retry--) {
			if (uarthub_is_uartip_tx_buf_empty_for_write_mt6881(tx_dev_id) == 1) {
				uarthub_uartip_write_data_to_tx_buf_mt6881(tx_dev_id, pTxData[curTx++]);
				break;
			}
			usleep_range(5, 6);
		}
	...
	while (szRcv < szBuf) {
		for (retry = 200; retry > 0; retry--) {
			if (uarthub_is_uartip_rx_buf_ready_for_read_mt6881(rx_dev_id) == 1) {
				rcvBuf[szRcv++] = uarthub_uartip_read_data_from_rx_buf_mt6881(rx_dev_id);
				break;
			}
			usleep_range(5, 6);
		}
		...
		if (retry == 0) {
			...
			if (rx_dev_id == DEV_CMM)
				szFIFO = uarthub_get_cmm_rx_fifo_size_mt6881();
			else
				szFIFO = uarthub_get_host_rx_fifo_size_mt6881(rx_dev_id);
	...
	int rx_dev_id, const unsigned char *pEvt, int szEvt)
{
	...
	uarthub_config_uartip_dma_en_ctrl_mt6881(tx_dev_id, TX, 0);
	if (rx_dev_id != DEV_OFF)
		uarthub_config_uartip_dma_en_ctrl_mt6881(rx_dev_id, RX, 0);
...
exit_:
	uarthub_config_uartip_dma_en_ctrl_mt6881(tx_dev_id, TX, 1);
	if (rx_dev_id != DEV_OFF)
		uarthub_config_uartip_dma_en_ctrl_mt6881(rx_dev_id, RX, 1);
	return ret;
}
	...
	int dev_index, unsigned char *p_tx_data, int tx_len, int dump_trxlog)
{
	...
	if (rx_dev_index >= 0)
		uarthub_config_uartip_dma_en_ctrl_mt6881(rx_dev_index, RX, 0);
	...
	state = uarthub_uartip_write_tx_data_mt6881(dev_index, p_tx_data, tx_len);
	...
	rx_len = 0;
	state = uarthub_uartip_read_rx_data_mt6881(rx_dev_index, evtBuf, TRX_BUF_LEN, &rx_len);
	...
	if (rx_dev_index == -1) {
		byte_cnt_rx_0 = UARTHUB_DEBUG_GET_OP_RX_REQ_VAL(uartip_base_map_mt6881[uartip_id_ap]);
		byte_cnt_tx_0 = UARTHUB_DEBUG_GET_IP_TX_DMA_VAL(uartip_base_map_mt6881[uartip_id_ap]);
		byte_cnt_rx_1 = UARTHUB_DEBUG_GET_OP_RX_REQ_VAL(uartip_base_map_mt6881[uartip_id_md]);
		byte_cnt_tx_1 = UARTHUB_DEBUG_GET_IP_TX_DMA_VAL(uartip_base_map_mt6881[uartip_id_md]);
		byte_cnt_rx_2 = UARTHUB_DEBUG_GET_OP_RX_REQ_VAL(uartip_base_map_mt6881[uartip_id_adsp]);
		byte_cnt_tx_2 = UARTHUB_DEBUG_GET_IP_TX_DMA_VAL(uartip_base_map_mt6881[uartip_id_adsp]);
...
verify_err:
	/* Uninitialize */
	if (rx_dev_index >= 0)
		uarthub_config_uartip_dma_en_ctrl_mt6881(rx_dev_index, RX, 1);
	//uarthub_reset_intfhub_mt6881();
...
int uarthub_is_apuart_tx_buf_empty_for_write_mt6881(int port_no)
{
	...
	return LSR_GET_TEMT(LSR_ADDR(apuart_base_map_mt6881[port_no]));
}
...
int uarthub_is_apuart_rx_buf_ready_for_read_mt6881(int port_no)
{
	...
	return LSR_GET_DR(LSR_ADDR(apuart_base_map_mt6881[port_no]));
}
...
int uarthub_apuart_write_data_to_tx_buf_mt6881(int port_no, int tx_data)
{
	...
	UARTHUB_REG_WRITE(THR_ADDR(apuart_base_map_mt6881[port_no]), tx_data);
	return 0;
}
...
int uarthub_apuart_read_data_from_rx_buf_mt6881(int port_no)
{
	...
	return UARTHUB_REG_READ(RBR_ADDR(apuart_base_map_mt6881[port_no]));
}
...
int uarthub_apuart_write_tx_data_mt6881(int port_no, unsigned char *p_tx_data, int tx_len)
{
	...
	for (i = 0; i < tx_len; i++) {
		retry = 200;
		while (retry-- > 0) {
			if (uarthub_is_apuart_tx_buf_empty_for_write_mt6881(port_no) == 1) {
				uarthub_apuart_write_data_to_tx_buf_mt6881(port_no, p_tx_data[i]);
				break;
			}
			usleep_range(5, 6);
		}
	...
	int port_no, unsigned char *p_rx_data, int rx_len, int *p_recv_rx_len)
{
	...
	for (i = 0; i < rx_len; i++) {
		retry = 200;
		while (retry-- > 0) {
			if (uarthub_is_apuart_rx_buf_ready_for_read_mt6881(port_no) == 1) {
				p_rx_data[i] =
					uarthub_apuart_read_data_from_rx_buf_mt6881(port_no);
				*p_recv_rx_len = i+1;
				break;
			}
			usleep_range(5, 6);
		...
		if (retry <= 0) {
			state = UARTHUB_DEBUG_GET_RX_WOFFSET_VAL(apuart_base_map_mt6881[port_no]);
			if (state > 0)
				retry = 200;
			else
				break;
...
int uarthub_init_default_apuart_config_mt6881(void)
{
	...
	if (!apuart_base_map_mt6881[1] || !apuart_base_map_mt6881[2] || !apuart_base_map_mt6881[3]) {
		pr_notice("[%s] apuart_base_map_mt6881[1/2/3] is not all init\n",
			__func__);
		return -1;
	}

	for (i = 1; i < 4; i++)
		uarthub_usb_rx_pin_ctrl_mt6881(apuart_base_map_mt6881[i], 1);

	for (i = 1; i < 4; i++) {
		uarthub_dev_base = apuart_base_map_mt6881[i];
	...
	for (i = 1; i < 4; i++)
		uarthub_usb_rx_pin_ctrl_mt6881(apuart_base_map_mt6881[i], 0);

	usleep_range(2000, 2010);

	/* 0x4c = 0x3,  rx/tx channel dma enable */
	for (i = 1; i < 4; i++)
		UARTHUB_REG_WRITE(DMA_EN_ADDR(apuart_base_map_mt6881[i]), 0x3);

	/* 0x08 = 0xC7, fifo control register */
	for (i = 1; i < 4; i++) {
		UARTHUB_REG_WRITE(RXTRI_AD_ADDR(apuart_base_map_mt6881[i]), 0x19);
		UARTHUB_REG_WRITE(FCR_ADDR(apuart_base_map_mt6881[i]), 0xC7);
	}
...
{
	uarthub_mask_host_irq_mt6881(0, BIT_0xFFFF_FFFF, 1);
	uarthub_clear_host_irq_mt6881(0, BIT_0xFFFF_FFFF);
	g_dev0_irq_sta = 0;
	g_dev0_inband_irq_sta = 0;
	uarthub_mask_host_irq_mt6881(0, BIT_0xFFFF_FFFF, 0);
