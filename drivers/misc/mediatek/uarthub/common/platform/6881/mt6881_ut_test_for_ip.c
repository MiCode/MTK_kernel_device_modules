...
static int uarthub_ut_ip_timeout_init_fsm_ctrl_by_unit_mt6881(
	int dev_index, enum uarthub_trx_type trx, int enable_fsm, int verify_index);
static int uarthub_ut_ip_clear_rx_data_irq_by_unit_mt6881(int dev_index, int verify_index);
static int uarthub_ut_ip_host_tx_packet_loopback_by_unit_mt6881(int dev_index, int verify_index);
#endif

#if UARTHUB_SUPPORT_UT_CASE
int uarthub_ut_ip_timeout_init_fsm_ctrl_by_unit_mt6881(
	int dev_index,
	...
	int verify_index)
{
	...
	uarthub_reset_intfhub_mt6881();
	uarthub_set_host_loopback_ctrl_mt6881(dev_index, 1, 1);
	uarthub_set_cmm_loopback_ctrl_mt6881(1, 1);
	uarthub_config_uartip_dma_en_ctrl_mt6881(((trx == TX) ? 3 : dev_index), RX, 0);
	...
	/* Verify */
	state = uarthub_uartip_write_tx_data_mt6881(
...
verify_err:
	if (state != 0) {
		...
		uarthub_dump_uartip_debug_info_mt6881(__func__, NULL, 0);
	}
	uarthub_set_host_loopback_ctrl_mt6881(dev_index, 1, 0);
	uarthub_set_cmm_loopback_ctrl_mt6881(1, 0);
	uarthub_reset_intfhub_mt6881();
	uarthub_config_uartip_dma_en_ctrl_mt6881(((trx == TX) ? 3 : dev_index), RX, 1);
...
#if UARTHUB_SUPPORT_UT_CASE
int uarthub_ut_ip_timeout_init_fsm_ctrl_mt6881(void)
{
	int state = 0;
	int verify_index = 0;

	/* Initialize */
	uarthub_set_host_trx_request_mt6881(0, TRX);
	usleep_range(3000, 3010);
#if !(UARTHUB_SUPPORT_SSPM_DRIVER) || (UARTHUB_SUPPORT_FPGA)
	UARTHUB_REG_WRITE(IRQ_CLR_ADDR, 0xFFFFFFFF);
	usleep_range(1000, 1010);
#endif
	state = uarthub_is_host_uarthub_ready_state_mt6881(0);
	if (state != 1) {
		pr_notice("[%s] FAIL: dev0 uarthub is NOT ready(%d)", __func__, state);
		state = UARTHUB_UT_ERR_HUB_READY_STA;
		goto verify_err;
	}
	uarthub_set_host_trx_request_mt6881(1, TRX);
	uarthub_set_host_trx_request_mt6881(2, TRX);

	/* Verify */
	state = uarthub_ut_ip_timeout_init_fsm_ctrl_by_unit_mt6881(0, TX, 0, ++verify_index);
	if (state != 0)
		goto verify_err;
	state = uarthub_ut_ip_timeout_init_fsm_ctrl_by_unit_mt6881(0, TX, 1, ++verify_index);
	if (state != 0)
		goto verify_err;
	state = uarthub_ut_ip_timeout_init_fsm_ctrl_by_unit_mt6881(0, RX, 0, ++verify_index);
	if (state != 0)
		goto verify_err;
	state = uarthub_ut_ip_timeout_init_fsm_ctrl_by_unit_mt6881(0, RX, 1, ++verify_index);
	if (state != 0)
		goto verify_err;

	state = uarthub_ut_ip_timeout_init_fsm_ctrl_by_unit_mt6881(1, TX, 0, ++verify_index);
	if (state != 0)
		goto verify_err;
	state = uarthub_ut_ip_timeout_init_fsm_ctrl_by_unit_mt6881(1, TX, 1, ++verify_index);
	if (state != 0)
		goto verify_err;
	state = uarthub_ut_ip_timeout_init_fsm_ctrl_by_unit_mt6881(1, RX, 0, ++verify_index);
	if (state != 0)
		goto verify_err;
	state = uarthub_ut_ip_timeout_init_fsm_ctrl_by_unit_mt6881(1, RX, 1, ++verify_index);
	if (state != 0)
		goto verify_err;

	state = uarthub_ut_ip_timeout_init_fsm_ctrl_by_unit_mt6881(2, TX, 0, ++verify_index);
	if (state != 0)
		goto verify_err;
	state = uarthub_ut_ip_timeout_init_fsm_ctrl_by_unit_mt6881(2, TX, 1, ++verify_index);
	if (state != 0)
		goto verify_err;
	state = uarthub_ut_ip_timeout_init_fsm_ctrl_by_unit_mt6881(2, RX, 0, ++verify_index);
	if (state != 0)
		goto verify_err;
	state = uarthub_ut_ip_timeout_init_fsm_ctrl_by_unit_mt6881(2, RX, 1, ++verify_index);
	if (state != 0)
		goto verify_err;
...
verify_err:
	/* Uninitialize */
	uarthub_clear_host_trx_request_mt6881(0, TRX);
	uarthub_clear_host_trx_request_mt6881(1, TRX);
	uarthub_clear_host_trx_request_mt6881(2, TRX);
	usleep_range(3000, 3010);
#if !(UARTHUB_SUPPORT_SSPM_DRIVER) || (UARTHUB_SUPPORT_FPGA)
	UARTHUB_REG_WRITE(IRQ_CLR_ADDR, 0xFFFFFFFF);
	usleep_range(1000, 1010);
...
#if UARTHUB_SUPPORT_UT_CASE
int uarthub_ut_ip_clear_rx_data_irq_by_unit_mt6881(int dev_index, int verify_index)
{
	int state = 0;
	...
	/* Initialize */
	uarthub_set_host_trx_request_mt6881(dev_index, TRX);
	usleep_range(3000, 3010);
#if !(UARTHUB_SUPPORT_SSPM_DRIVER) || (UARTHUB_SUPPORT_FPGA)
	UARTHUB_REG_WRITE(IRQ_CLR_ADDR, 0xFFFFFFFF);
	usleep_range(1000, 1010);
#endif
	state = uarthub_is_host_uarthub_ready_state_mt6881(dev_index);
	if (state != 1) {
	...
	uarthub_set_host_loopback_ctrl_mt6881(dev_index, 1, 1);
	uarthub_set_cmm_loopback_ctrl_mt6881(1, 1);
	uarthub_config_uartip_dma_en_ctrl_mt6881(dev_index, RX, 0);
	...
	state = uarthub_uartip_write_tx_data_mt6881(3, TEST_FF_CMD, sizeof(TEST_FF_CMD));
	...
	state = uarthub_uartip_write_tx_data_mt6881(3, TEST_F0_CMD, sizeof(TEST_F0_CMD));
	...
	recv_rx_len = 0;
	state = uarthub_uartip_read_rx_data_mt6881(dev_index, evtBuf, TRX_BUF_LEN, &recv_rx_len);
	...
	state = uarthub_uartip_write_tx_data_mt6881(3, TEST_SEND_CMD, sizeof(TEST_SEND_CMD));
...
#else
		pr_notice("[%s] RX FAIL(%d): uart_%d received size=[%d] is different with expect size=[%u] (dev[%d])",
			__func__, verify_index, dev_index,
			recv_rx_len, sizeof(TEST_RECV_CMD), dev_index);
#endif
		state = UARTHUB_UT_ERR_RX_FAIL;
...
verify_err:
	...
	uarthub_set_host_loopback_ctrl_mt6881(dev_index, 1, 0);
	uarthub_set_cmm_loopback_ctrl_mt6881(1, 0);
	uarthub_config_uartip_dma_en_ctrl_mt6881(dev_index, RX, 1);
	uarthub_reset_intfhub_mt6881();

	/* Uninitialize */
	uarthub_clear_host_trx_request_mt6881(dev_index, TRX);
	usleep_range(3000, 3010);
#if !(UARTHUB_SUPPORT_SSPM_DRIVER) || (UARTHUB_SUPPORT_FPGA)
	UARTHUB_REG_WRITE(IRQ_CLR_ADDR, 0xFFFFFFFF);
	usleep_range(1000, 1010);
...
#if UARTHUB_SUPPORT_UT_CASE
int uarthub_ut_ip_clear_rx_data_irq_mt6881(void)
{
	int state = 0;
	int verify_index = 0;
	...
	/* Verify */
	state = uarthub_ut_ip_clear_rx_data_irq_by_unit_mt6881(0, ++verify_index);
	if (state != 0)
		goto verify_err;
	state = uarthub_ut_ip_clear_rx_data_irq_by_unit_mt6881(1, ++verify_index);
	if (state != 0)
		goto verify_err;
	state = uarthub_ut_ip_clear_rx_data_irq_by_unit_mt6881(2, ++verify_index);
	if (state != 0)
		goto verify_err;
...
#if UARTHUB_SUPPORT_UT_CASE
int uarthub_ut_ip_host_tx_packet_loopback_by_unit_mt6881(int dev_index, int verify_index)
{
	int state = 0;
	unsigned char dmp_info_buf_recv[TRX_BUF_LEN];
	unsigned char dmp_info_buf[TRX_BUF_LEN];
	...
	uarthub_set_host_loopback_ctrl_mt6881(dev_index, 1, 1);
	uarthub_set_cmm_loopback_ctrl_mt6881(1, 1);
	uarthub_config_uartip_dma_en_ctrl_mt6881(3, RX, 0);
	...
	recv_rx_len = 0;
	state = uarthub_uartip_read_rx_data_mt6881(3, evtBuf, TRX_BUF_LEN, &recv_rx_len);
	...
	if (recv_rx_len != sizeof(TEST_RECV_CMD)) {
		...
		pr_notice("[%s] RX FAIL(%d): uart_cmm received size=[%d] is different with expect size=[%lu] (dev[%d])",
			...
			recv_rx_len, sizeof(TEST_RECV_CMD), dev_index);
#else
		pr_notice("[%s] RX FAIL(%d): uart_cmm received size=[%d] is different with expect size=[%u] (dev[%d])",
			__func__, verify_index,
			recv_rx_len, sizeof(TEST_RECV_CMD), dev_index);
#endif
		state = UARTHUB_UT_ERR_RX_FAIL;
...
verify_err:
	...
	uarthub_set_host_loopback_ctrl_mt6881(dev_index, 1, 0);
	uarthub_set_cmm_loopback_ctrl_mt6881(1, 0);
	uarthub_config_uartip_dma_en_ctrl_mt6881(3, RX, 1);
	uarthub_reset_intfhub_mt6881();
...
#if UARTHUB_SUPPORT_UT_CASE
int uarthub_ut_ip_host_tx_packet_loopback_mt6881(void)
{
	int state = 0;
	int verify_index = 0;
	...
	/* Verify */
	uarthub_ut_ip_host_tx_packet_loopback_by_unit_mt6881(0, ++verify_index);
	uarthub_ut_ip_host_tx_packet_loopback_by_unit_mt6881(1, ++verify_index);
	uarthub_ut_ip_host_tx_packet_loopback_by_unit_mt6881(2, ++verify_index);
...
int uarthub_verify_cmm_loopback_sta_mt6881(void)
{
	int state = 0;
	unsigned char dmp_info_buf_recv[TRX_BUF_LEN];
	unsigned char dmp_info_buf[TRX_BUF_LEN];
	...
	uarthub_reset_intfhub_mt6881();
	uarthub_set_host_loopback_ctrl_mt6881(0, 1, 1);
	uarthub_set_cmm_loopback_ctrl_mt6881(1, 1);
	uarthub_config_uartip_dma_en_ctrl_mt6881(3, RX, 0);
	...
	/* Verify */
	state = uarthub_uartip_write_tx_data_mt6881(0, BT_TEST_CMD, sizeof(BT_TEST_CMD));
...
#else
		pr_notice("[%s] RX FAIL: uart_cmm received size=[%d] is different with expect size=[%u]",
			__func__, recv_rx_len, sizeof(BT_TEST_EVT));
#endif
		state = UARTHUB_UT_ERR_RX_FAIL;
		goto verify_err;
...
verify_err:
	...
	/* Uninitialize */
	uarthub_set_host_loopback_ctrl_mt6881(0, 1, 0);
	uarthub_set_cmm_loopback_ctrl_mt6881(1, 0);
	uarthub_config_uartip_dma_en_ctrl_mt6881(3, RX, 1);
	uarthub_reset_intfhub_mt6881();
...
#if UARTHUB_SUPPORT_UT_API
int uarthub_verify_cmm_trx_combo_sta_mt6881(int rx_delay_ms)
{
	int state = 0;
	unsigned char dmp_info_buf_recv[TRX_BUF_LEN];
	unsigned char dmp_info_buf[TRX_BUF_LEN];
