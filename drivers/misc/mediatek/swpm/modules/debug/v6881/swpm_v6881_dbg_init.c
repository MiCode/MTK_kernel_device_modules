...
#include <swpm_v6881.h>
#include <swpm_v6881_ext.h>
#include <swpm_v6881_subsys.h>
...
static ssize_t swpm_psp_test_read(char *ToUser, size_t sz, void *priv)
{
	char *p = ToUser;
	unsigned int out1 = 0;
	unsigned int out2 = 0;
	...
	swpm_dbg_log("DDREN: main %u, start %u, size %u ret %d\n", out1, out2, out3, ret);
	...
	if (ret)
		pr_info("[SWPM] get_res_group_info fail (%d)\n", ret);
	swpm_dbg_log("APSRC: main %u, start %u, size %u ret %d\n", out1, out2, out3, ret);
	...
	if (ret)
		pr_info("[SWPM] get_res_group_info fail (%d)\n", ret);
	swpm_dbg_log("EMI: main %u, start %u, size %u ret %d\n", out1, out2, out3, ret);
	...
	if (ret)
		pr_info("[SWPM] get_res_group_info fail (%d)\n", ret);
	swpm_dbg_log("MAINPLL: main %u, start %u, size %u ret %d\n", out1, out2, out3, ret);
	...
	if (ret)
		pr_info("[SWPM] get_res_group_info fail (%d)\n", ret);
	swpm_dbg_log("INFRA: main %u, start %u, size %u ret %d\n", out1, out2, out3, ret);
	...
	if (ret)
		pr_info("[SWPM] get_res_group_info fail (%d)\n", ret);
	swpm_dbg_log("26M: main %u, start %u, size %u ret %d\n", out1, out2, out3, ret);
	...
	if (ret)
		pr_info("[SWPM] get_res_group_info fail (%d)\n", ret);
	swpm_dbg_log("PMIC: main %u, start %u, size %u ret %d\n", out1, out2, out3, ret);
	...
	if (ret)
		pr_info("[SWPM] get_res_group_info fail (%d)\n", ret);
	swpm_dbg_log("VCORE: main %u, start %u, size %u ret %d\n", out1, out2, out3, ret);
	...
	if (ret)
		pr_info("[SWPM] get_res_group_info fail (%d)\n", ret);
	swpm_dbg_log("PWR_OFF: main %u, start %u, size %u ret %d\n", out1, out2, out3, ret);
	...
	if (ret)
		pr_info("[SWPM] get_res_group_info fail (%d)\n", ret);
	swpm_dbg_log("PWR_ACT: main %u, start %u, size %u ret %d\n", out1, out2, out3, ret);
	...
	if (ret)
		pr_info("[SWPM] get_res_group_info fail (%d)\n", ret);
	swpm_dbg_log("SYS_STA: main %u, start %u, size %u ret %d\n", out1, out2, out3, ret);
	...
	swpm_dbg_log("GRP ID: VCORE_MD %u, VCORE_CONN %u, VCORE_SCP %u ret %d\n", out1, out2,
		out3, ret);
	...
	swpm_dbg_log("GRP ID: VCORE_ADSP %u, VCORE_PCIE0 %u, VCORE_PCIE1 %u ret %d\n", out1,
		out2, out3, ret);
	...
	swpm_dbg_log("GRP ID: VCORE_MMPROC %u, VCORE_UARTHUB %u, UNUSE %u ret=%d\n", out1,
		out2, out3, ret);
...
static const struct mtk_swpm_sysfs_op swpm_psp_test_fops = {
	.fs_read = swpm_psp_test_read,
};
...
static void swpm_v6881_dbg_fs_init(void)
{
...
static int __init swpm_v6881_dbg_early_initcall(void)
{
	return 0;
}
#ifndef MTK_SWPM_KERNEL_MODULE
subsys_initcall(swpm_v6881_dbg_early_initcall);
#endif

static int __init swpm_v6881_dbg_device_initcall(void)
{
	return 0;
}

static int __init swpm_v6881_dbg_late_initcall(void)
{
	/*
...
#ifndef MTK_SWPM_KERNEL_MODULE
late_initcall_sync(swpm_v6881_dbg_late_initcall);
#endif

int __init swpm_v6881_dbg_init(void)
{
	int ret = 0;
#ifdef MTK_SWPM_KERNEL_MODULE
	ret = swpm_v6881_dbg_early_initcall();
#endif
	if (ret)
		goto swpm_v6881_dbg_init_fail;

	ret = swpm_v6881_dbg_device_initcall();

	if (ret)
		goto swpm_v6881_dbg_init_fail;

#ifdef MTK_SWPM_KERNEL_MODULE
	ret = swpm_v6881_dbg_late_initcall();
#endif
	...
	return 0;
swpm_v6881_dbg_init_fail:
	return -EAGAIN;
}

void __exit swpm_v6881_dbg_exit(void)
{
	swpm_v6881_exit();
	swpm_v6881_ext_exit();
}

module_init(swpm_v6881_dbg_init);
module_exit(swpm_v6881_dbg_exit);

MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("v6881 software power model debug module");
