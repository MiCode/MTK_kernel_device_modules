...
static int sc2150_regmap_init(struct sc2150 *sc)
{
	...
	snprintf((char *)props->name, len+1, "%s", name);
	snprintf((char *)props->aliases, len+1, "%s", name);
	props->io_log_en = 0;
...
__maybe_unused
static int sc2150_test_mode(struct tcpc_device *tcpc, bool enable)
{
	int i, j;
	struct sc2150 *sc = tcpc_get_dev_data(tcpc);
	uint8_t test_mode_val[6] = {SC2150_TEST_KEY1, SC2150_TEST_KEY2, SC2150_TEST_KEY3,
		SC2150_TEST_KEY4, SC2150_TEST_KEY5, SC2150_TEST_KEY6};
	uint8_t exit_test_val = SC2150_TEST_KEY_RESET;

	dev_info(sc->dev, "sc2150 set test mode, enable = %s\n", enable ? "enable" : "disable");
	for (i = 0; i < 10; i++) {
		if (enable) {
			for(j = 0; j < 6; j++)
				sc2150_write_device(sc->client, SC2150_TEST_MODE_CTRL, 1, &test_mode_val[j]);
			break;
		} else if (!enable) {
			sc2150_write_device(sc->client, SC2150_TEST_MODE_CTRL, 1, &exit_test_val);
			break;
		}
	}
...
__maybe_unused
static int sc2150_set_continuous_time(struct tcpc_device *tcpc)
{
	int ret = 0;
	uint8_t val = 0;
	struct sc2150 *sc = tcpc_get_dev_data(tcpc);
	uint8_t countinous_time = 0x4E;

	ret = sc2150_write_device(sc->client, SC2150_COUNTINUS_TIME_CTRL, 1, &countinous_time);
	if (ret < 0)
		return ret;

	sc2150_read_device(sc->client, SC2150_COUNTINUS_TIME_CTRL, 1, &val);
...
{
	uint8_t mask = TCPC_V10_REG_POWER_STATUS_VBUS_PRES;
...
static int sc2150_init_alert(struct tcpc_device *tcpc)
{
	...
	//ret = gpio_request_one(sc->irq_gpio, GPIOF_IN, "sc2150_irq");
...
static int sc2150_tcpc_init(struct tcpc_device *tcpc, bool sw_reset)
{
    ...
    */
    ...
    // sc2150_test_mode(tcpc, true);
    // sc2150_set_continuous_time(tcpc);
    // sc2150_test_mode(tcpc, false);
...
static int sc2150_parse_dt(struct sc2150 *sc, struct device *dev)
{
    ...
    snprintf((char *)desc->name, len+1, "%s", name);
...
static int sc2150_i2c_probe(struct i2c_client *client)
{
    ...
    sc->mt6661_reg_ln60_5 = devm_regulator_get_optional(sc->dev, "typecidb");
    if (IS_ERR(sc->mt6661_reg_ln60_5)) {
        ret = PTR_ERR(sc->mt6661_reg_ln60_5);
        if ((ret != -ENODEV) && sc->dev->of_node) {
            pr_err("failed to get mt6661_reg_ln60_5 regulator\n");
        }
        pr_err("unable to get mt6661_reg_ln60_5 regulator\n");
    } else {
        regulator_set_voltage(sc->mt6661_reg_ln60_5, 1800000, 1800000);
        ret = regulator_enable(sc->mt6661_reg_ln60_5);
        if (ret) {
            pr_err("mt6661_reg_ln60_5 enable failed\n");
        }
        pr_err("success to set mt6661_reg_ln60_5 as 1800mv\n");
    }
...
static int sc2150_i2c_suspend(struct device *dev)
{
    ...
    if (device_may_wakeup(dev)) {
        ...
        enable_irq_wake(sc->cid_irq);
    }
    disable_irq(sc->irq);
    disable_irq(sc->cid_irq);
...
static int sc2150_i2c_resume(struct device *dev)
{
    ...
    dev_info(dev, "%s\n", __func__);
    enable_irq(sc->irq);
    enable_irq(sc->cid_irq);
    if (device_may_wakeup(dev)) {
        disable_irq_wake(sc->irq);
        disable_irq_wake(sc->cid_irq);
    }
    return 0;
}
