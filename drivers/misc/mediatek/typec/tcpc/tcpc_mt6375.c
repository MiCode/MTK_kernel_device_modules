...
static int mt6375_get_cc(struct tcpc_device *tcpc, int *cc1, int *cc2)
{
	...
	*cc2 = TCPC_V10_REG_CC_STATUS_CC2(status);
	...
	MT6375_INFO("pre cc:%d %d curr cc:%d %d\n",
...
static int mt6375_en_floatgnd(struct tcpc_device *tcpc, bool en)
{
	int ret;
	struct mt6375_tcpc_data *ddata = tcpc_get_dev_data(tcpc);

	ret = mt6375_enable_floating_ground(ddata, en);
	if (ret < 0)
		return ret;
...
static struct tcpc_ops mt6375_tcpc_ops = {
	...
	.enable_floating_ground = mt6375_en_floatgnd,
#if CONFIG_CABLE_TYPE_DETECTION
