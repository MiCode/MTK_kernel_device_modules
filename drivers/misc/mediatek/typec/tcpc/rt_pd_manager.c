				...
				unsigned long event, void *data)
{
	...
	case TCP_NOTIFY_PD_STATE:
		...
		case PD_CONNECT_PE_READY_SRC_PD30:
			...
			rpmd->tcpc[idx]->pd_port.rqc_pd_state_svid = (noti->pd_state.connected << 16) | (partner_vdos[0] & 0x0000FFFF);
			dev_info(rpmd->dev, "%s svid = %x\n",
				    __func__, partner_vdos[0] & 0x0000FFFF);
	...
	case TCP_NOTIFY_WD0_STATE:
		...
		dev_info(rpmd->dev, "%s reverse_chg_18w_enable = %d\n",
			__func__, rpmd->tcpc[idx]->pd_port.reverse_chg_18w_enable);
		if (!rpmd->tcpc[idx]->pd_port.reverse_chg_18w_enable)
