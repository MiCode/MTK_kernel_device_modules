...
//pd 4.7.2 start
int tcpci_get_fake_power_status(struct tcpc_device *tcpc, uint16_t *pw_status)
{
	int ret;

	PD_BUG_ON(tcpc->ops->get_fake_power_status == NULL);
	ret = tcpc->ops->get_fake_power_status(tcpc, pw_status);
	if (ret < 0)
		return ret;
	tcpci_vbus_level_init(tcpc, *pw_status);
...
EXPORT_SYMBOL(tcpci_get_fake_power_status);
//pd 4.7.2 end
...
int tcpci_is_sy6976(struct tcpc_device *tcpc)
{
	if(tcpc->ops->is_sy6976 == NULL) {
		TCPC_INFO("%s:not sy6976\n",__func__);
		return 0;
    } else {
		return tcpc->ops->is_sy6976();
    }
}
EXPORT_SYMBOL(tcpci_is_sy6976);
...
int tcpci_set_bg_enable(struct tcpc_device *tcpc,uint8_t enable)
{
	PD_BUG_ON(tcpc->ops->set_bg_enable == NULL);
	return tcpc->ops->set_bg_enable(tcpc,enable);
}
