		...
		uint32_t timer_id)
{
	...
	if (pd_get_rev(pd_port, sop_type) >= PD_REV30) {
		...
		if (tcpci_is_sy6976(tcpc)) {
            pr_err("pd_send_svdm_request is sy6976\n");
			if (SYCHIP_PD_VER >= 0x3116)
				ver_minor = SVDM_MINOR_REV1;
        } else {
            pr_err("pd_send_svdm_request not sy6976\n");
	...
	uint8_t reply, uint8_t cnt, uint32_t *data_obj)
{
	...
	if (pd_check_rev30(pd_port)) {
		ver_major = SVDM_MAJOR_REV20;
		if (tcpci_is_sy6976(tcpc)) {
                  	pr_err("pd_reply_svdm_request is sy6976\n");
			if (SYCHIP_PD_VER >= 0x3116)
				ver_minor = SVDM_MINOR_REV1;
		} else {
                  	pr_err("pd_reply_svdm_request not sy6976\n");
			if (SOUTHCHIP_PD_VER >= 0x3116)
				ver_minor = SVDM_MINOR_REV1;
		}
	}
