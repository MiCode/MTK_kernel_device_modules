...
int charger_get_typec_cc_orientation(void)
{
	return typec_cc_orientation;
}
EXPORT_SYMBOL(charger_get_typec_cc_orientation);
...
EXPORT_SYMBOL(tcpc_typec_handle_cc_change);
...
int tcpc_typec_handle_timeout(struct tcpc_device *tcpc, uint32_t timer_id)
{
	...
	rv = tcpci_get_chip_vid(tcpc,&chip_vid);
  	TCPC_INFO("tcpc_typec_handle_timeout rv (%d)\n", rv);
#endif /* CONFIG_SUPPORT_SOUTHCHIP_PDPHY */
...
EXPORT_SYMBOL(tcpc_typec_handle_timeout);
...
EXPORT_SYMBOL(tcpc_typec_handle_ps_change);
...
EXPORT_SYMBOL(tcpc_typec_handle_vsafe0v);
...
EXPORT_SYMBOL(tcpc_typec_swap_role);
#endif /* CONFIG_TYPEC_CAP_ROLE_SWAP */
...
EXPORT_SYMBOL(tcpc_typec_set_rp_level);
...
EXPORT_SYMBOL(tcpc_typec_disable);
...
EXPORT_SYMBOL(tcpc_typec_enable);
...
bool cid_plugin_flag = false;
EXPORT_SYMBOL(cid_plugin_flag);
int tcpc_typec_init(struct tcpc_device *tcpc, uint8_t typec_role)
{
	...
	TYPEC_INFO("typec_init: %s,%d\n", typec_role_name[typec_role],cid_plugin_flag);
...
EXPORT_SYMBOL(tcpc_typec_deinit);
#ifdef CONFIG_WATER_DETECTION
