...
int rt1711_tcpc_set_shutdownoff(struct tcpc_device *tcpc)
{
	int ret = 0;
	ret = rt1711_i2c_read8(tcpc, RT1711H_REG_IDLE_CTRL);
	pr_err("ret:%x\n",ret);
	if (ret < 0)
		return ret;
	ret &= ~(1<<5);
	rt1711_i2c_write8(tcpc, RT1711H_REG_IDLE_CTRL,ret);
	ret = rt1711_i2c_read8(tcpc, RT1711H_REG_IDLE_CTRL);
	pr_err("0x9B ret:%x\n",ret);
	rt1711_i2c_write8(tcpc, 0x1B,0x80);
	ret = rt1711_i2c_read8(tcpc, 0x1B);
	pr_err("0x1B ret:%x\n",ret);
	return 0;
}
EXPORT_SYMBOL(rt1711_tcpc_set_shutdownoff);
...
static int rt1711_tcpcdev_init(struct rt1711_chip *chip, struct device *dev)
{
	...
	if (chip->chip_id >= RT1715_DID_D || (chip->chip_id == HUSB311_DID) || ( chip->chip_vid == SOUTHCHIP_PD_VID))
...
static inline int rt1711h_check_revision(struct i2c_client *client)
{
	...
	if (vid != RICHTEK_1711_VID && vid != HUSB311_VID && vid != SOUTHCHIP_PD_VID) {
	...
	if (pid != RICHTEK_1711_PID && pid != HUSB311_PID && (pid != SC2150A_PID) && (pid != SC6601_PID)) {
