	...
	int type, unsigned long *cnt, unsigned long *block_cnt)
{
	...
	if (cnt && block_cnt)
		...
		pr_info("[name:spm&]Power/swap BLOCK FAIL (type:%d)\n"
				, type);
...
void mtk_idle_latency_profile_result(unsigned int idle_type)
{
	...
	if (pdata->count < PROFILE_LATENCY_NUMBER) {
		...
		pr_info("[name:spm&]Power/latency_profile avg %s: %u, %u, %u\n"
	...
	pr_info("[name:spm&]Power/latency_profile %s\n", plog);
}
