...
static int isp_hvc_register(unsigned long token)
{
	...
	ret = pkvm_register_el2_mod_call(kvm_nvhe_sym(isp_hyp_sethsfcam), token);
	arm_smccc_1_1_smc(SMC_ID_MTK_PKVM_ADD_HVC,
		SMC_ID_MTK_PKVM_ISP_SETHSFCAM, ret, 0, 0, 0, 0, &res);

	ret = pkvm_register_el2_mod_call(kvm_nvhe_sym(isp_hyp_sethsfcamsv), token);
	arm_smccc_1_1_smc(SMC_ID_MTK_PKVM_ADD_HVC,
		SMC_ID_MTK_PKVM_ISP_SETHSFCAMSV, ret, 0, 0, 0, 0, &res);

	ret = pkvm_register_el2_mod_call(kvm_nvhe_sym(isp_hyp_streamon), token);
	arm_smccc_1_1_smc(SMC_ID_MTK_PKVM_ADD_HVC,
		SMC_ID_MTK_PKVM_ISP_STREAMON, ret, 0, 0, 0, 0, &res);
