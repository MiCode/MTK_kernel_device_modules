...
#define LAZY_FREE 1
uint64_t mpool_va_to_pa(void *va);
void *mpool_pa_to_va(uint64_t pa);
...
static struct spinlock_t ptable_lock;

static bool mm_stage2_invalidate;
...
static struct mm_page_table *mm_page_table_from_pa(paddr_t pa)
{
	...
	return mpool_pa_to_va(pa_addr(pa));
}
			      ...
			      uint16_t id)
{
	...
	//arch_mm_invalidate_stage1_range(id, va_init(begin), va_init(end));
	// } else {
	//arch_mm_invalidate_stage2_range(id, ipa_init(begin), ipa_init(end));
	// }
	// TODO: replace with smmu_mgmt.c:smmu_tlb_invalidate
}
		    ...
		    struct mpool *ppool)
{
	...
	smmu_flush_dcache(tables, root_table_count * PAGE_SIZE);
	...
	t->root = pa_init(mpool_va_to_pa((void *)tables));
	t->id = id;
	return true;
}
			     ...
			     uint16_t id)
{
	...
	if (((flags & MM_FLAG_STAGE1) || mm_stage2_invalidate) &&
		...
		*pte = arch_mm_absent_pte(level);
		...
		smmu_flush_dcache(pte, sizeof(pte_t));
 ...
 */
static struct mm_page_table *
mm_populate_table_pte(ptable_addr_t begin, pte_t *pte, uint8_t level, int flags,
		      struct mpool *ppool, uint16_t id)
{
	...
	smmu_flush_dcache(ntable, PAGE_SIZE);
	...
	mm_replace_entry(begin, pte,
			 arch_mm_table_pte(level,
					   pa_init(mpool_va_to_pa(ntable))),
			 level, flags, ppool, id);
...
static void mm_ptable_identity_commit(struct mm_ptable *t, paddr_t pa_begin,
				      ...
				      struct mpool *ppool)
{
	mm_ptable_identity_map(t, pa_begin, pa_end, attrs,
			...
			uint32_t mode, struct mpool *ppool, ipaddr_t *ipa)
{
	int flags = mm_mode_to_flags(mode);
	bool success =
		mm_ptable_identity_update(t, begin, end,
					  arch_mm_mode_to_stage2_attrs(mode),
					  flags, ppool);
...
#define PTE_ADDR_MASK (((1UL << 48) - 1) & ~((1UL << 12) - 1))
#define PTE_ATTR_MASK (~(PTE_ADDR_MASK))

static void mm_print_s2_pte(uint64_t pte, uint64_t s_ipa, uint64_t e_ipa,
			    uint64_t s_pa, uint64_t e_pa, uint8_t level,
			    int s_idx, int e_idx)
{
}

static void mm_dump_table_recursive_s2(struct mm_page_table *table,
				       uint8_t level, int max_level,
				       uint64_t ipa_base)
{
	uint64_t i;
	uint64_t pte, attrs, pa, next_pa;
	uint64_t next_pte, next_attrs;
	uint64_t start_idx = 0, end_idx;
	uint64_t s_ipa = 0, e_ipa, s_pa = 0, e_pa;
	uint64_t in_range = 0;
	size_t entry_size = mm_entry_size(level);

	for (i = 0; i < MM_PTE_PER_PAGE; i++) {
		pte = table->entries[i];
		attrs = pte & PTE_ATTR_MASK;
		pa = pte & PTE_ADDR_MASK;
		/* check next pte to see if it's the same as me or not */
		if (i < (MM_PTE_PER_PAGE - 1)) {
			next_pte = table->entries[i + 1];
			next_attrs = next_pte & PTE_ATTR_MASK;
			next_pa = next_pte & PTE_ADDR_MASK;
			if (attrs == next_attrs &&
			    next_pa == (pa + entry_size)) {
				/* it's same attribute && it's contig pa */
				/* are we already in range? */
				if (in_range) {
					/* yes, we are already in range, and the
					 * next pte is still as same as me
					 * just do nothing
					 */
					continue;
				} else {
					/* nope, we are never being in range,
					 * now we are the start of range
					 */
					start_idx = i;
					s_ipa = ipa_base + i * entry_size;
					s_pa = pa;
					in_range = 1;
					continue;
				}
			} else if (!arch_mm_pte_is_present(pte, level) &&
				   !arch_mm_pte_is_present(next_pte, level)) {
				if (in_range)
					continue;
				start_idx = i;
				s_ipa = ipa_base + i * entry_size;
		...
		end_idx = i;
		e_ipa = ipa_base + (end_idx + 1) * entry_size;
		if (!arch_mm_pte_is_present(pte, level)) {
			mm_print_s2_pte(pte, s_ipa, e_ipa, 0, 0, level,
					start_idx, end_idx);
		} else {
			e_pa = s_pa + (end_idx - start_idx + 1) * entry_size;
			mm_print_s2_pte(pte, s_ipa, e_ipa, s_pa, e_pa, level,
					start_idx, end_idx);
		}

		if (arch_mm_pte_is_table(table->entries[i], level)) {
			mm_dump_table_recursive_s2(
				mm_page_table_from_pa(arch_mm_table_from_pte(
					table->entries[i], level)),
				level - 1, max_level, s_ipa);
		}
		in_range = 0;
		start_idx = end_idx + 1;
	}
...
void mm_ptable_dump_s2(struct mm_ptable *t)
{
	int flags = 0;
	struct mm_page_table *tables = mm_page_table_from_pa(t->root);
	uint8_t max_level = mm_max_level(flags);
	...
	for (i = 0; i < root_table_count; ++i) {
		mm_dump_table_recursive_s2(&tables[i], max_level, max_level,
					   ipa_base);
	}
}

uint64_t stage2_pte(struct mm_ptable *t, uint64_t ipa)
{
	struct mm_page_table *ptep = mm_page_table_from_pa(t->root);
	uint8_t max_level = arch_mm_stage2_max_level();
	pte_t pte;
	uint8_t level = max_level;
	uint32_t index;

	/*
	 * this traverse way is different with linux
	 * Hafnium traverse:
	 * level 2(1GB) --> level 1(2MB) --> level 0(4KB)
	 * Linux traverse:
	 * level 1(1GB) --> level 2(2MB) --> level 3(4KB)
	 */
	do {
		if (!ptep)
			return 0;

		index = mm_index(ipa, level);
		pte = ptep->entries[index];

		if (arch_mm_pte_is_block(pte, level)) {
			/* pte is valid block descriptor */
			goto found_translation;
		} else if (arch_mm_pte_is_table(ptep->entries[index], level)) {
			/*
			 * pte is valid table descriptor, go to next level page
			 * table
			 */
			ptep = mm_page_table_from_pa(arch_mm_table_from_pte(
				ptep->entries[index], level));
			level--;
		} else {
			/*
			 * pte is invalid, which means that ipa doesn't map in
			 * the s2 page table
			 */
			return 0;
		}
