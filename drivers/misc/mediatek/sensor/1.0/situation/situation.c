...
int sardetector_data_report_t(int32_t value[3], int64_t time_stamp)
{
        int err = 0, index = -1;
        struct sensor_event event;
        struct situation_context *cxt = situation_context_obj;
        memset(&event, 0, sizeof(struct sensor_event));
        index = handle_to_index(ID_SAR_DETECTOR);
        if (index < 0) {
                pr_err("[%s] invalid index\n", __func__);
                return -1;
        }
        event.time_stamp = time_stamp;
        event.handle = ID_SAR_DETECTOR;
        event.flush_action = DATA_ACTION;
        event.word[0] = value[0];
        err = sensor_input_event(situation_context_obj->mdev.minor, &event);
        if (cxt->ctl_context[index].situation_ctl.open_report_data != NULL &&
...
EXPORT_SYMBOL_GPL(sardetector_data_report_t);

int sardetector_1_data_report_t(int32_t value[3], int64_t time_stamp)
{
        int err = 0, index = -1;
        struct sensor_event event;
        struct situation_context *cxt = situation_context_obj;
        memset(&event, 0, sizeof(struct sensor_event));
        index = handle_to_index(ID_SAR_DETECTOR_1);
        if (index < 0) {
                pr_err("[%s] invalid index\n", __func__);
                return -1;
        }
        event.time_stamp = time_stamp;
        event.handle = ID_SAR_DETECTOR_1;
        event.flush_action = DATA_ACTION;
        event.word[0] = value[0];
        err = sensor_input_event(situation_context_obj->mdev.minor, &event);
        if (cxt->ctl_context[index].situation_ctl.open_report_data != NULL &&
...
EXPORT_SYMBOL_GPL(sardetector_1_data_report_t);
