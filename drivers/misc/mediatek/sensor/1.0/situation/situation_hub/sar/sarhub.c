...
static int sar_send_board_id(void);
	...
	if(NULL != chosen){
		...
		if(0 == ret){
			...
			if((NULL != strstr(bootargs, "p15aer")) || (NULL != strstr(bootargs, "p15ape"))){
				...
				err = sensor_cfg_to_hub(ID_SAR,(uint8_t *)&flags,sizeof(flags));
				if(err < 0)
					pr_err("[%s] fail\n", __func__);
			}else{
				flags = 0;
	...
	struct device_attribute *attr, char *buf)
{
	...
	sensor_enable_to_hub(ID_SAR, true);
	pr_err("device create chipinfo\n");
	msleep(50);

	res = sensor_set_cmd_to_hub(ID_SAR, CUST_ACTION_GET_SENSOR_INFO, &devinfo);
	if(res < 0) {
		pr_err("Get sar sensor info err\n");
	}

	return snprintf(buf, PAGE_SIZE, "%s\n", devinfo.name);
}

static DEVICE_ATTR_RO(chipinfo);
static int sarhub_probe(struct platform_device *pdev)
{
	...
	ret = device_create_file(dev, &dev_attr_chipinfo);
  	if(ret != 0) {
  		pr_err("chipinfo device create fail\n");
  		return ret;
  	}
...
static int sarhub_remove(struct platform_device *pdev)
{
	...
	device_remove_file(&(pdev->dev), &dev_attr_chipinfo);
	return 0;
}
