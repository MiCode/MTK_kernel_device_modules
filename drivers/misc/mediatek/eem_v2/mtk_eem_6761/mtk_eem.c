...
static unsigned int read_efuse_by_offset(__u32 offset)
{
	...
	eem_debug("%s(): offset= %d, value=%d", __func__, offset, value);
	return value;
}
...
static int get_devinfo(void)
{
	...
	val[4] = read_efuse_by_offset(DEVINFO_OFF_7);
	val[5] = read_efuse_by_offset(DEVINFO_OFF_8);
...
static int eem_buck_get(struct platform_device *pdev)
{
	...
	eem_regulator_vproc = devm_regulator_get_optional(&pdev->dev, "vproc");
