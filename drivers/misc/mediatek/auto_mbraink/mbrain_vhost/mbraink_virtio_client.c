...
#include "mbraink_virtio_client.h"
...
static int virtio_mbraink_echo_sync(struct virtio_mbraink *mbraink, u32 msg_length, char *virtio_send_buffer)
{
	int err;

	if (mbraink == NULL) {
		pr_notice("%s: mbraink is null\n", __func__);
		return -EFAULT;
	}
	...
	cmd->req.cmd = VIRTIO_MBRAINK_CMD_ECHO_SYNC;
	strscpy((char *)cmd->req.data, virtio_send_buffer, min(msg_length + 1, sizeof(cmd->req.data)));
	init_completion(&cmd->done);
...
int mbraink_hyp_sendMsg(u32 msg_length, void *msg_buffer)
{
	...
	rv = virtio_mbraink_echo_sync(g_mbraink, msg_length, msg_buffer);

	return ret;
}
EXPORT_SYMBOL(mbraink_hyp_sendMsg);

module_init(mbraink_hyp_virtio_init);
module_exit(mbraink_hyp_virtio_deinit);
