...
void mbraink_v8668_get_gnss_lp_data(struct mbraink_gnss2mbr_lp_data *gnss_lp_buffer)
{
...
void mbraink_v8668_get_gnss_mcu_data(struct mbraink_gnss2mbr_mcu_data *gnss_mcu_buffer)
{
	enum gnss2mbr_status ret;
...
static struct mbraink_gps_ops mbraink_v8668_gps_ops = {
	.get_gnss_lp_data = mbraink_v8668_get_gnss_lp_data,
	.get_gnss_mcu_data = mbraink_v8668_get_gnss_mcu_data,
};

int mbraink_v8668_gps_init(void)
{
	int ret = 0;

	ret = register_mbraink_gps_ops(&mbraink_v8668_gps_ops);

	return ret;
}

int mbraink_v8668_gps_deinit(void)
{
	int ret = 0;
