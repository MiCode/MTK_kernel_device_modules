...
static int mbraink_v8668_set_sw_count_mode(int command);
						...
						size_t count)
{
	...
	mbraink_v8668_set_sw_count_mode(command);

	return count;
}
static DEVICE_ATTR_RW(mbraink_platform_system_info);

static int mbraink_v8668_get_chipid_info(struct mbraink_chipid_info *chipid_info)
{
...
static int mbraink_v8668_set_sw_count_mode(int val)
{
	int ret = 0;
...
EXPORT_SYMBOL_GPL(mbraink_v8668_pmic_read);

int mbraink_v8668_pmic_interface_get(struct device *dev)
{
	int ret = 0;
...
int mbraink_v8668_pmic_interface_put(struct device *dev)
{
	if (mbraink_pmic_np)
		of_node_put(mbraink_pmic_np);
	if (mbraink_pmic_pdev)
...
static struct mbraink_systeminfo_ops mbraink_v8668_systeminfo_ops = {
	.get_chipid_info = mbraink_v8668_get_chipid_info,
	.set_sw_count_mode = mbraink_v8668_set_sw_count_mode,
};
...
int mbraink_v8668_systeminfo_init(struct device *dev)
{
	...
	ret = register_mbraink_systeminfo_ops(&mbraink_v8668_systeminfo_ops);
	if (ret) {
		pr_err("%s: register_mbraink_systeminfo_ops failed\n", __func__);
		goto End;
	}

	ret = mbraink_v8668_pmic_interface_get(dev);
	if (ret)
...
int mbraink_v8668_systeminfo_deinit(struct device *dev)
{
	...
	ret = mbraink_v8668_pmic_interface_put(dev);
Deinit:
	return ret;
}
