...
int mbraink_v8668_get_touch_ghost_info(struct mbraink_touch_ghost_info *touch_ghost_info)
{
	int ret = 0;
...
static struct mbraink_touch_ops mbraink_v8668_touch_ops = {
	.get_touch_ghost_info = mbraink_v8668_get_touch_ghost_info,
};

int mbraink_v8668_touch_init(void)
{
	int ret = 0;

	ret = register_mbraink_touch_ops(&mbraink_v8668_touch_ops);
	ret = register_kfifo_callback(touch_ghost_notify);
	return ret;
}

int mbraink_v8668_touch_deinit(void)
{
	int ret = 0;
