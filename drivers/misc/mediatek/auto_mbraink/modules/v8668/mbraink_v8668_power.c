...
/*spinlock for mbraink spm l1 data*/
...
static int mbraink_v8668_power_getVcoreInfo(struct mbraink_power_vcoreInfo *pmbrainkPowerVcoreInfo)
{
	int ret = 0;
	int i = 0;
...
static void mbraink_v8668_get_power_wakeup_info(
...
#if IS_ENABLED(CONFIG_MTK_SWPM_MODULE)
static int mbraink_v8668_power_get_spm_l1_info(long long *out_spm_l1_array, int spm_l1_size)
{
	int ret = -1;
	int i = 0;
	long long value = 0;
...
static int mbraink_v8668_power_get_spm_l2_all_info(
	struct mbraink_power_spm_l2_all_info *spm_l2_all_info)
{
	struct mbraink_sys_res_mbrain_dbg_ops *sys_res_mbrain_ops = NULL;
	...
	if (sys_res_mbrain_ops &&
		...
		thr[0] = spm_l2_all_info->value[0];
		thr[1] = spm_l2_all_info->value[1];
		thr[2] = spm_l2_all_info->value[2];
		thr[3] = spm_l2_all_info->value[3];

		pr_notice("spm l2 get thr(%d,%d,%d,%d)\n", thr[0], thr[1], thr[2], thr[3]);
		...
		if (sys_res_mbrain_ops->get_over_threshold_num(ptr,
			...
			if (sig_num > SPM_TOTAL_MAX_SIG_NUM)
				goto End;
			...
			if (sys_res_mbrain_ops->get_over_threshold_data(sig_tbl_ptr
				...
				memcpy(spm_l2_all_info->spm_data, ptr, SPM_L2_LS_SZ);
				if ((SPM_L2_LS_SZ + sig_num*SPM_L2_RES_SIZE) <=
					MAX_POWER_SPM_TBL_SUSPEND_ALL_SZ) {
					memcpy(spm_l2_all_info->spm_data + SPM_L2_LS_SZ,
						sig_tbl_ptr, sig_num*SPM_L2_RES_SIZE);
				}
			}
		}
	}
End:
	...
	pr_notice("spm l2 get suscess\n");

	return 0;
}

static int mbraink_v8668_power_get_spm_all_info(struct mbraink_power_spm_all_raw *spm_all_buffer)
{
	struct mbraink_sys_res_mbrain_dbg_ops *sys_res_mbrain_ops = NULL;
	unsigned char *spm_raw = NULL;
	unsigned int data_size = 0;

	if (spm_all_buffer == NULL)
		return -1;

	memset(spm_all_buffer->spm_data, 0, sizeof(spm_all_buffer->spm_data));
	spm_all_buffer->size = 0;

	sys_res_mbrain_ops = get_mbraink_dbg_ops();
	if (sys_res_mbrain_ops == NULL)
		return -1;

	if (sys_res_mbrain_ops->get_length) {
		data_size = sys_res_mbrain_ops->get_length();
		data_size += MAX_POWER_HD_SZ;
		pr_notice("spm all data_size(%d)\n", data_size);
	}

	if (data_size > 0 && data_size <= sizeof(spm_all_buffer->spm_data)) {
		if (sys_res_mbrain_ops->get_data) {
			spm_raw = vmalloc(data_size);
			if (spm_raw != NULL) {
				memset(spm_raw, 0, data_size);
				if (sys_res_mbrain_ops->get_data(spm_raw, data_size) == 0) {
					memcpy(spm_all_buffer->spm_data, spm_raw, data_size);
					spm_all_buffer->size = data_size;
				}
			}
		}
	}
...
static int mbraink_v8668_power_get_scp_info(struct mbraink_power_scp_info *scp_info)
{
...
static int mbraink_v8668_power_get_scp_task_info(struct mbraink_power_scp_task_info *scp_task_info)
{
...
static int mbraink_v8668_power_get_modem_all_info(struct mbraink_modem_all_raw *modem_all_buffer)
{
	int shm_size = 0;
...
static int mbraink_v8668_power_get_spmi_info(
...
static int mbraink_v8668_power_get_pmic_voltage_info(
...
static int mbraink_v8668_power_sys_res_init(void)
{
	int ret = 0;
...
static int mbraink_v8668_power_sys_res_deinit(void)
{
...
static void mbraink_v8668_power_send_spml1(long long last_resume_timestamp)
{
	unsigned long flags;
...
static void mbraink_v8668_power_suspend_prepare(void)
{
	unsigned long flags;
...
static void mbraink_v8668_power_post_suspend(long long last_resume_timestamp)
{
	mbraink_v8668_power_send_spml1(last_resume_timestamp);
}

static int mbraink_v8668_power_device_suspend(struct device *dev)
{
	int ret = 0;
	struct mbraink_sys_res_mbrain_dbg_ops *sys_res_mbrain_ops = NULL;
...
static int mbraink_v8668_power_device_resume(struct device *dev)
{
	unsigned long flags;
	int ret;
	...
	ret = mbraink_v8668_power_get_spm_l1_info(spm_l1_data, SPM_L1_DATA_NUM);
...
static int mbraink_v8668_power_get_mmdfvs_info(struct mbraink_mmdvfs_info *mmdvfsInfo)
{
	...
	if (res_mbrain_debug_ops &&
		...
		if (mmdvfs_data_size <= MMDVFS_TOTAL_SZ) {
			...
			ret = res_mbrain_debug_ops->get_data(mmdvfsInfo->mmdvfs_data,
				mmdvfsInfo->size);
...
static int mbraink_v8668_power_get_mmdvfs_user_info(
...
static int mbraink_v8668_power_get_power_throttle_hw_info
...
static int mbraink_v8668_power_get_lpmstate_info(struct mbraink_lpm_state_data *lpmStateInfo)
{
	int ret = 0;
...
static int mbraink_v8668_power_get_spmi_glitch_info(
...
static int mbraink_v8668_power_get_dvfsrc_info(
...
static int mbraink_v8668_power_get_smap_info
...
int mbraink_v8668_power_ccci_event_cb(enum CCCI_MBRAIN_EVENT_TYPE event_type,
...
static int mbraink_v8668_power_get_mmqos_bw_info(struct mbraink_mmqos_bw_info *mmqos_bw_info)
{
	int ret = 0;
...
static int mbraink_v8668_power_get_power_throttle_hw_oc_info
...
static struct mbraink_power_ops mbraink_v8668_power_ops = {
	.getVotingInfo = NULL,
	.getPowerInfo = NULL,
	.getVcoreInfo = mbraink_v8668_power_getVcoreInfo,
	.getWakeupInfo = mbraink_v8668_get_power_wakeup_info,
	.getSpmInfo = NULL,
	.getSpmAllInfo = mbraink_v8668_power_get_spm_all_info,
	.getSpmL1Info = mbraink_v8668_power_get_spm_l1_info,
	.getSpmL2Info = NULL,
	.getSpmL2AllInfo = mbraink_v8668_power_get_spm_l2_all_info,
	.getScpInfo = mbraink_v8668_power_get_scp_info,
	.getScpTaskInfo = mbraink_v8668_power_get_scp_task_info,
	.getModemInfo = NULL,
	.getModemAllInfo = mbraink_v8668_power_get_modem_all_info,
	.getSpmiInfo = mbraink_v8668_power_get_spmi_info,
	.getUvloInfo = NULL,
	.getPmicVoltageInfo = mbraink_v8668_power_get_pmic_voltage_info,
	.suspendprepare = mbraink_v8668_power_suspend_prepare,
	.postsuspend = mbraink_v8668_power_post_suspend,
	.getMmdvfsInfo = mbraink_v8668_power_get_mmdfvs_info,
	.getMmdvfsUserInfo = mbraink_v8668_power_get_mmdvfs_user_info,
	.getPowerThrottleHwInfo = mbraink_v8668_power_get_power_throttle_hw_info,
	.getLpmStateInfo = mbraink_v8668_power_get_lpmstate_info,
	.getSpmiGlitchInfo = mbraink_v8668_power_get_spmi_glitch_info,
	.getDvfsrcInfo = mbraink_v8668_power_get_dvfsrc_info,
	.getMMBWInfo = mbraink_v8668_power_get_mmqos_bw_info,
	.deviceSuspend = mbraink_v8668_power_device_suspend,
	.deviceResume = mbraink_v8668_power_device_resume,
	.getPowerThrottleHwOcInfo = mbraink_v8668_power_get_power_throttle_hw_oc_info,
	.getPowerSmapInfo = mbraink_v8668_power_get_smap_info,
};

int mbraink_v8668_power_init(struct device *dev)
{
	int ret = 0;

	pMBrainkPlatDev = dev;
	ret = register_mbraink_power_ops(&mbraink_v8668_power_ops);

	mbraink_v8668_power_sys_res_init();
	...
	ret = ccci_mbrain_register(mbraink_v8668_power_ccci_event_cb);
	if (ret != 0) {
...
int mbraink_v8668_power_deinit(void)
{
	int ret = 0;

	ret = unregister_mbraink_power_ops();
	mbraink_v8668_power_sys_res_deinit();
