...
#include "mbraink_v8668_memory.h"
#include "mbraink_v8668_systeminfo.h"

#include <swpm_module_psp.h>
#include <dvfsrc-mb.h>
#include <mtk_cm_mgr_mt8668.h>
...
struct device *mbraink_v8668_device;
static struct task_struct *mbraink_slbc_thread;
static struct mbraink_v8668_slbc_info *slbc_info;
unsigned int slbc_period_ms = 10000;
unsigned int slbc_upload_cnt = 1000;
static DEFINE_MUTEX(mbraink_slbc_lock);

static int mbraink_v8668_memory_getDdrInfo(struct mbraink_memory_ddrInfo *pMemoryDdrInfo)
{
	int ret = 0;
	int i, j;
...
static int mbraink_v8668_memory_getMdvInfo(struct mbraink_memory_mdvInfo  *pMemoryMdv)
{
	int ret = 0;
	int i = 0;
...
static int mbraink_v8668_get_ufs_info(struct mbraink_ufs_info  *ufs_info)
{
	int ret = 0;
#if IS_ENABLED(CONFIG_DEVICE_MODULES_SCSI_UFS_MEDIATEK)
	...
	if (mbraink_v8668_device) {
		memset(&mb_dev_info, 0, sizeof(struct ufs_mbrain_dev_info));
		phy_node = of_parse_phandle(mbraink_v8668_device->of_node, "ufsnotify", 0);
		if (phy_node) {
...
static int mbraink_v8668_memory_getEmiInfo(struct mbraink_memory_emiInfo *pMemoryEmiInfo)
{
	int i;
...
static int mbraink_v8668_memory_getCmProfileInfo
...
static int mbraink_v8668_memory_getVsmrInfo(struct mbraink_memory_vsmrInfo  *pMemoryVsmr)
{
	int ret = 0;
	int i = 0;
...
static int mbraink_v8668_memory_getCmVoteInfo(struct mbraink_memory_cmVoteInfo *pCmVoteInfo)
{
	int i, j = 0;
...
static int mbraink_v8668_memory_getCPUQosInfo
	(struct mbraink_memory_cpuQosInfo  *pMemoryCpuQos)
{
	...
	if (mbraink_v8668_pmic_read() != MT6661_RESERVED_VAL) {
...
static int mbraink_v8668_memory_getMMQosInfo
...
static int mbraink_v8668_memory_getCmDDRVoteInfo(struct mbraink_memory_cmDDRVoteInfo *pCmDDRVoteInfo)
{
	int i = 0;
...
static struct mbraink_memory_ops mbraink_v8668_memory_ops = {
	.getDdrInfo = mbraink_v8668_memory_getDdrInfo,
	.getMdvInfo = mbraink_v8668_memory_getMdvInfo,
	.get_ufs_info = mbraink_v8668_get_ufs_info,
	.getEmiInfo = mbraink_v8668_memory_getEmiInfo,
	.getCmProfileInfo = mbraink_v8668_memory_getCmProfileInfo,
	.getVsmrInfo = mbraink_v8668_memory_getVsmrInfo,
	.getCmVoteInfo = mbraink_v8668_memory_getCmVoteInfo,
	.getCpuQosInfo = mbraink_v8668_memory_getCPUQosInfo,
	.getMMQosInfo = mbraink_v8668_memory_getMMQosInfo,
};
	...
	while (!kthread_should_stop()) {
		...
		if (slbc_info) {
			...
			if (idx == slbc_upload_cnt) {
				...
				memset(slbc_info, 0,
					sizeof(struct mbraink_v8668_slbc_info) * slbc_upload_cnt);
			}
		}
	...
	if (command == 1) {
		...
		if (!slbc_info) {
			...
			pr_info("%s: Get size(%d)\n",
				...
				slbc_upload_cnt);
			slbc_info =
				vmalloc(sizeof(struct mbraink_v8668_slbc_info) * slbc_upload_cnt);
			if (!slbc_info)
		...
		if (!mbraink_slbc_thread) {
			mbraink_slbc_thread =
				kthread_run(mbraink_slbc_thread_func,
					NULL,
					"mbraink_v8668_slbc_thread");
			if (!mbraink_slbc_thread)
				pr_info("%s: mbraink_v8668_slbc_thread create fail!\n",
					__func__);
		} else
			pr_info("mbraink_v8668_slbc_thread has already existed!\n");
	} else {
		if (mbraink_slbc_thread) {
			kthread_stop(mbraink_slbc_thread);
			mbraink_slbc_thread = NULL;
		} else
			pr_notice("mbraink_v8668_slbc_thread does not exist!\n");
...
int mbraink_v8668_memory_init(struct device *dev)
{
	...
	ret = register_mbraink_memory_ops(&mbraink_v8668_memory_ops);

	if (dev) {
		mbraink_v8668_device = dev;
