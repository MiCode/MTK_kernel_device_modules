...
#if IS_ENABLED(CONFIG_MTK_MBRAINK_HOST_MODE)
#include "../../mbrain_vhost/mbraink_virtio_host.h"
#else
#include "../../mbrain_vhost/mbraink_virtio_client.h"
#endif
...
#if IS_ENABLED(CONFIG_GRT_NEBULA_TRACE)
	pr_notice("Triggering ktrace snapshot(file name: %s)\n", file_name);
	ktrace_enable_snapshot();
	ktrace_do_snapshot(file_name);
	ktrace_disable_snapshot();
#else
	pr_info("Triggering ktrace snapshot is not supported\n");
#endif

	return ret;
}

static struct mbraink_auto_hypervisor_ops mbraink_auto_v8668_hypervisor_ops = {
	.getWfeExitCountInfo = mbraink_hyp_getWfeExitCountInfo,
	.getIpiInfo = mbraink_hyp_getIpiInfo,
	.getVIrqInjectLatency = mbraink_hyp_getvIrqInjectLatency,
	.getVCpuSchedDelayInfo = mbraink_hyp_getvCpuSchedInfo,
	.getVCpuSwitchInfo = mbraink_hyp_get_vcpu_switch_info,
	.setVCpuSwitchInfo = mbraink_hyp_set_vcpu_switch_info,
	.sendMsg = mbraink_hyp_sendMsg,
	.triggerKtraceSnapshot = mbraink_hyp_trigger_ktrace_snapshot,
};
...
int mbraink_v8668_hypervisor_init(void)
{
	...
	ret = register_mbraink_auto_hypervisor_ops(&mbraink_auto_v8668_hypervisor_ops);
...
int mbraink_v8668_hypervisor_deinit(void)
{
	...
	ret = unregister_mbraink_auto_hypervisor_ops();
