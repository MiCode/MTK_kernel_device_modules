...
#include "mbraink_v8668_battery.h"

struct battery_drv_data drv_data;

static void mbraink_v8668_get_battery_info(struct mbraink_battery_data *battery_buffer,
			      long long timestamp)
{
...
static struct mbraink_battery_ops mbraink_v8668_battery_ops = {
	.getBatteryInfo = mbraink_v8668_get_battery_info,
};

int mbraink_v8668_battery_init(struct device *dev)
{
	int ret = 0;
	...
	ret = register_mbraink_battery_ops(&mbraink_v8668_battery_ops);
	return ret;
}

int mbraink_v8668_battery_deinit(void)
{
	int ret = 0;
