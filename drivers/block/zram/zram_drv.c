...
#ifdef	CONFIG_ZRAM_WRITEBACK
	for (unsigned int i = 0; i < ZRAM_WB_IDLE_MAX; i++)
		off += scnprintf(buf + off, PAGE_SIZE - off, "%lld ", 
						atomic64_read(&zram->stats.total_idle_count[i]));
	buf[off - 1] = '\n';
	ret = off;
#endif
out:
...
#if defined(CONFIG_ZRAM_WRITEBACK) && defined(CONFIG_MI_MEMORY_FREEZE)
	if (zram->mfz_enable && memcg != NULL) {
		android_oem_data1 = memcg->android_oem_data1[0];
...
static int zram_add(void)
{
	...
	if (zram->writeback_pages)
		...
		pr_err("Error allocating writeback batch pages for device %d\n",
			device_id);
	for (int count_index = 0; count_index < ZRAM_WB_IDLE_MAX; count_index++) {
...
#if defined(CONFIG_ZRAM_WRITEBACK) && defined(CONFIG_MI_MEMORY_FREEZE)
static u64
mem_cgroup_anno_writeback_enable_read(struct cgroup_subsys_state *css,
				      struct cftype *cft)
{
	struct mem_cgroup *memcg = mem_cgroup_from_css(css);

	return memcg->android_oem_data1[0];
}

static int
mem_cgroup_anno_writeback_enable_write(struct cgroup_subsys_state *css,
				       struct cftype *cft, u64 val)
{
	struct mem_cgroup *memcg = mem_cgroup_from_css(css);

	if (css->parent)
		memcg->android_oem_data1[0] = !!val;
