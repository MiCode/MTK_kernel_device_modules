...
static int mt6379_flash_mode(struct led_classdev_flash *flash, u32 brightness)
{
	...
	ret =  regmap_write(data->regmap, MT6379_REG_ISTRB(mtflash->idx),selector);
	if (ret)
		goto out_strobe_set;
    ...
    pr_info("%s selector:%u,mask:%d,state:%d,strobe_enabled:[%lu]\n",__func__,selector,mask,state,data->strobe_enabled);
	if (state) {
...
#endif
	pr_info("%s  cur:%d,brightness:%d,mask:%d,torch_enabled:[%lu]\n",__func__,cur,brightness,mask,data->torch_enabled);
...
#endif
	selector = (brightness - s->min) / s->step;
	pr_info("%s selector:%u,brightnessbrightness:%d\n",__func__,selector,brightness);
...
static const struct of_device_id mt6379_flash_dt_match[] = {
	{ .compatible = "mediatek,mt6379-pmicflash" },
	{ /* sentinel */ }
};
