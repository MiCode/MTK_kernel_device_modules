...
#define MT6379_ITOR_MAXUA		600000
...
static int mt6379_flash_mode(struct led_classdev_flash *flash, u32 brightness)
{
	...
	if (brightness <  MT6379_ISTRB_MINUA_DUAL){
		brightness = MT6379_ISTRB_MINUA_DUAL;
		pr_info("%s brightness less than MT6379_ISTRB_MINUA_DUAL,set brightness to :%d \n",__func__,brightness);
	}
	if (brightness >  s->max){
      brightness =  s->max;
	}
				       ...
				       u32 cur)
{
	...
	if (cur == LED_OFF) {
		...
		if(cur < MT6379_ITOR_MINUA_DUAL){
			cur = MT6379_ITOR_MINUA_DUAL;
			pr_info("%s cur less than MT6379_ITOR_MINUA_DUAL,set cur to :%d \n",__func__,cur);
		}
...
static int mt6379_ioctl(unsigned int cmd, unsigned long arg)
{
	...
	case XIAOMI_FLASH_GET_HWINFO_MIN:
		fl_arg->arg = MT6379_ITOR_MINUA_DUAL;
