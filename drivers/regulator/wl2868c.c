...
#define WL2868C_REG_LDO0_STATUS         0x10
#define WL2868C_REG_LDO0_OCP_CTL        0x11
...
#define WL2868C_INT_VSYS_UVLO           BIT(0)
#define WL2868C_INT_OTST2               BIT(2)
#define WL2868C_INT_OTST3               BIT(3)
#define WL2868C_INT_OCP                 BIT(4)

#define WL2868C_LDO_OCP_STAT            BIT(5)
#define WL2868C_LDO_ERR_STAT            BIT(6)

#define LDO_VSET_REG(offset) ((offset) + WL2868C_REG_LDO0)
#define LDO_STAT_REG(offset) ((offset * 2) + WL2868C_REG_LDO0_STATUS)
#define LDO_OCP_CTL_REG(offset) ((offset * 2) + WL2868C_REG_LDO0_OCP_CTL)
...
static struct regulator_data reg_data[] = {
	{ "wl2868c-l1", "wl2868c-l1", 1200000, 80000, 800000},
	{ "wl2868c-l2", "wl2868c-l2", 1200000, 80000, 800000},
	{ "wl2868c-l3", "wl2868c-l3", 2800000, 80000, 800000},
	{ "wl2868c-l4", "wl2868c-l4", 2800000, 80000, 800000},
	{ "wl2868c-l5", "wl2868c-l5", 2800000, 80000, 800000},
	{ "wl2868c-l6", "wl2868c-l6", 2800000, 80000, 800000},
	{ "wl2868c-l7", "wl2868c-l7", 2800000, 80000, 800000},
};

struct regulator_irq_type {
	u8 irq_type;
	const char *irq_name;
	unsigned long event;
};

static struct regulator_irq_type wl2868c_irq_types[] = {
	{ WL2868C_INT_VSYS_UVLO, "VSYS-UVLO", REGULATOR_EVENT_UNDER_VOLTAGE},
	{ WL2868C_INT_OTST2, "OSTS2-115C", REGULATOR_EVENT_OVER_TEMP},
	{ WL2868C_INT_OTST3, "OSTS2-155C", REGULATOR_EVENT_OVER_TEMP},
	{ WL2868C_INT_OCP, "OCP", REGULATOR_EVENT_OVER_CURRENT},
};
...
static unsigned int wl2868c_regulator_get_mode(struct regulator_dev *rdev)
{
	struct wl2868c_regulator *fan_reg = rdev_get_drvdata(rdev);
	int rc = 0;
	u8 reg = 0;

	rc = wl2868c_read(fan_reg->regmap, LDO_STAT_REG(fan_reg->offset), &reg, 1);
	if (rc < 0) {
		wl2868c_err("[%s] failed to read ldo_status reg rc = %d\n", fan_reg->rdesc.name, rc);
		return 0;
	}
	wl2868c_err("[%s] read ldo%d_status 0x%x\n", fan_reg->rdesc.name, fan_reg->offset+1, reg);

	if (reg & WL2868C_LDO_OCP_STAT) {
		regulator_notifier_call_chain(fan_reg->rdev, REGULATOR_EVENT_OVER_CURRENT, NULL);
		rc = wl2868c_masked_write(fan_reg->regmap, LDO_OCP_CTL_REG(fan_reg->offset), 1u<<1, 1u<<1);
		rc |= wl2868c_masked_write(fan_reg->regmap, LDO_OCP_CTL_REG(fan_reg->offset), 1u<<1, 0);
		if (rc < 0)
			wl2868c_err("[%s] failed to clear ocp rc = %d\n", fan_reg->rdesc.name, rc);
		else
			wl2868c_err("[%s] ocp cleared\n", fan_reg->rdesc.name);
		return 0;
	}

	return !!(reg & WL2868C_LDO_ERR_STAT);
}
...
static struct regulator_ops wl2868c_regulator_ops = {
	...
	.get_mode = wl2868c_regulator_get_mode,
};
...
static irqreturn_t wl2868c_irq_handler(int irq, void *data)
{
	struct wl2868c_regulator *wl2868c_reg = data;
	int rc = 0, i = 0;
	u8 irq_stat = 0, irq_clr = 0, ldo_stat = 0;

	if (wl2868c_reg == NULL) {
		wl2868c_err("%s wl2868c_reg is NULL\n", __func__);
		return IRQ_HANDLED;
	}
	wl2868c_err("%s INT OCCURED\n", __func__);

	rc = wl2868c_read(wl2868c_reg->regmap, WL2868C_REG_INT_PENDING_STS, &irq_stat, 1);
	if (rc < 0) {
		wl2868c_err("%s: failed to read INT_PENDING_STS rc = %d\n", __func__, rc);
		return IRQ_HANDLED;
	}

	for (i = 0; i < ARRAY_SIZE(wl2868c_irq_types); i++) {
		if (irq_stat & wl2868c_irq_types[i].irq_type) {
			wl2868c_err("%s: ldo%d %s trigger\n", __func__, wl2868c_reg->offset+1, wl2868c_irq_types[i].irq_name);
			irq_clr |= wl2868c_irq_types[i].irq_type;
			if (wl2868c_irq_types[i].irq_type != WL2868C_INT_OCP)
				regulator_notifier_call_chain(wl2868c_reg->rdev, wl2868c_irq_types[i].event, NULL);
			else {
				rc = wl2868c_read(wl2868c_reg->regmap, LDO_STAT_REG(wl2868c_reg->offset), &ldo_stat, 1);
				if (rc < 0) {
					wl2868c_err("%s: failed to read LDO_STATUS rc = %d\n", __func__, rc);
					return IRQ_HANDLED;
				}
				if (ldo_stat & WL2868C_LDO_OCP_STAT) {
					regulator_notifier_call_chain(wl2868c_reg->rdev, wl2868c_irq_types[i].event, NULL);
					rc = wl2868c_masked_write(wl2868c_reg->regmap, LDO_OCP_CTL_REG(wl2868c_reg->offset), 1u<<1, 1u<<1);
					rc |= wl2868c_masked_write(wl2868c_reg->regmap, LDO_OCP_CTL_REG(wl2868c_reg->offset), 1u<<1, 0);
					if (rc < 0) {
						wl2868c_err("%s: failed to clear LDO_OCP_CTL rc = %d\n", __func__, rc);
						return IRQ_HANDLED;
					}
				}
			}
	...
	if (irq_clr != 0)
		rc = wl2868c_write(wl2868c_reg->regmap, WL2868C_REG_INT_LATCHED_CLR, &irq_clr, 1);

	return IRQ_HANDLED;
}

static int wl2868c_register_interrupt(struct wl2868c_regulator *wl2868c_reg)
{
	int rc = 0;
	u8 irq_en = 0x1D;

	wl2868c_reg->irq_gpio = of_get_named_gpio(wl2868c_reg->of_node, "wl2868c-irq-gpio", 0);
	if (!gpio_is_valid(wl2868c_reg->irq_gpio)) {
		wl2868c_err("WL2868C failed to get irq gpio %d\n", wl2868c_reg->irq_gpio);
		return -EINVAL;
	}

	rc = gpio_request_one(wl2868c_reg->irq_gpio, GPIOF_IN,"wl2868c_irq");
	if (rc) {
		wl2868c_err("failed to request wl2868c_irq\n");
		return -EINVAL;
	}

	wl2868c_reg->irq = gpio_to_irq(wl2868c_reg->irq_gpio);
	if (wl2868c_reg->irq < 0) {
		wl2868c_err("WL2868C failed to gpio_to_irq\n");
		return -EINVAL;
	}

	if (wl2868c_reg->irq) {
		rc = wl2868c_write(wl2868c_reg->regmap, WL2868C_REG_INT_EN_SET, &irq_en, 1);
		rc |= wl2868c_masked_write(wl2868c_reg->regmap, LDO_OCP_CTL_REG(wl2868c_reg->offset), 1u<<2, 1u<<2);
		if (rc < 0) {
			wl2868c_err("failed to set interrupt reg, rc = %d\n", rc);
			return rc;
		}
		rc = devm_request_threaded_irq(wl2868c_reg->dev, wl2868c_reg->irq, NULL,
			wl2868c_irq_handler,
			IRQF_TRIGGER_HIGH | IRQF_ONESHOT,
			wl2868c_reg->rdesc.name, wl2868c_reg);

		if (rc < 0) {
			wl2868c_err("request irq for irq=%d failed, rc=%d\n", wl2868c_reg->irq, rc);
			return rc;
		}
		enable_irq_wake(wl2868c_reg->irq);
	}

	wl2868c_err("WL2868C succeed to register irq\n");
	return 0;
}
...
static int wl2868c_parse_regulator(struct regmap *regmap, struct device *dev)
{
	...
	for_each_available_child_of_node(dev->of_node, child) {
		...
		if (!strcmp(reg_data[6].name, name)) {
			rc = wl2868c_register_interrupt(wl2868c_reg);
			if (rc < 0) {
				wl2868c_err("failed to register interrupt rc = %d\n", rc);
				return rc;
			}
		}
	}
...
static ssize_t wl2868c_set_info(struct device *dev,
	struct device_attribute *attr,
	const char *buf, size_t len)
{
	unsigned int reg_tmp = 0, val_tmp = 0;
	u16 reg = 0;
	u8 val = 0;
	int ret;
	struct regmap *regmap = (struct regmap *)dev->driver_data;

	ret = sscanf(buf, "%x %x", &reg_tmp, &val_tmp);
	if (ret == 2 && (reg <= 0x25)) {
		reg = (u16)reg_tmp;
		val = (u8)val_tmp;
		wl2868c_write(regmap, reg, &val, 1);
	}
...
static DEVICE_ATTR(info, S_IWUSR|S_IRUSR, wl2868c_show_info, wl2868c_set_info);
...
static int wl2868c_driver_register(int index, struct regmap *regmap)
{
	...
	if (pwl2868c_class == NULL) {
		pwl2868c_class = class_create(WL2868C_CLASS_NAME);
