...
#define WL2868C_PRODUCT_ID      0x8233
...
#define WL2868C_CLASS_NAME       "camera_ldo_wl2868c"
	...
	int max_uv)
{
	...
	wl2868c_err("write_min_uv ==  %d, write_max_uv == %d\n", min_uv, max_uv);
	...
	wl2868c_err("fan_reg->offset ==  %d, min_uv == %d, VSET=[0x%2x]\n", fan_reg->offset, min_uv, vset);
...
static int wl2868c_regulator_probe(struct i2c_client *client)
{
	...
	rc = regmap_read(regmap, 0x00, &val);
	product_id = (val & 0xFF) << 8;
	rc |= regmap_read(regmap, 0x01, &val);
	product_id |= (val & 0xFF);
	if (rc < 0) {
		wl2868c_err("WL2868C failed to get PID\n");
		return -ENODEV;
	}
	else if(WL2868C_PRODUCT_ID != product_id){
		wl2868c_err("WL2868C product ID mismatch (0x%04x)\n", product_id);
		return -ENODEV;
	}
	wl2868c_debug("WL2868C get Product ID: [%04x]\n", product_id);
