...
static int wl2866d_write(struct regmap *regmap, u16 reg, u8*val, int count)
{
	int rc;
	u8 temp;
	wl2866d_err("[%s] Writing 0x%02x to 0x%02x\n", __func__, *val, reg);
	rc = regmap_bulk_write(regmap, reg, val, count);
	if (rc < 0)
		wl2866d_err("[%s] failed to write 0x%04x\n", __func__, reg);
	rc = wl2866d_read(regmap, reg, &temp, count);
	wl2866d_err("[%s] to write 0x%04x  exp 0x%x now 0x%x \n", __func__, reg, *val,temp);
	return rc;
}
...
static int wl2866d_driver_register(int index, struct regmap *regmap)
{
	...
	rc = sysfs_create_file(&(wl2866d_dev.pdevice->kobj), &dev_attr_status.attr);
	rc = sysfs_create_file(&(wl2866d_dev.pdevice->kobj), &dev_attr_info.attr);
	rc = sysfs_create_file(&(wl2866d_dev.pdevice->kobj), &dev_attr_enable.attr);
	rc = sysfs_create_file(&(wl2866d_dev.pdevice->kobj), &dev_attr_reset.attr);
	return 0;
}
	...
	const struct i2c_device_id *id)
{
	...
	if (rc < 0) {
		...
		wl2866d_debug("WL2866D get Product ID: [%02x]\n", val);
	if (val != 0x33) {
		wl2866d_err("WL2866D get Product ID: [%02x],probe fail\n", val);
		return -ENODEV;
	}
