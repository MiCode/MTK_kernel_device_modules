	...
	const char *buf, size_t len)
{
	...
	if(val == 1){
		...
		rc = wl2866d_write(regmap, WL2866D_REG_ENABLE, &reset_data, 1);
		if (rc < 0) {
			wl2866d_err("wl2866d write reset addr failed \n");
		}
		wl2866d_show_reg_info(regmap);
	}
	return len;
...
static int wl2866d_driver_register(int index, struct regmap *regmap)
{
	...
	if (pwl2866d_class == NULL) {
		pwl2866d_class = class_create(WL2866D_CLASS_NAME);
