...
static int rawdata_proc_open(struct inode *inode, struct file *file)
{
	return single_open_size(file, rawdata_proc_show,
			///PDE_DATA(inode), PAGE_SIZE * 10);//hc add
	...
	struct goodix_ts_board_data *board_data)
{
	...
	if (r < 0) {
		ts_err("invalid reset-gpio in dt: %d", r);//serdes touch panel not used reset pin, not need.
		//return -EINVAL;
	}
...
static int goodix_ts_gpio_setup(struct goodix_ts_core *core_data)
{
	struct goodix_ts_board_data *ts_bdata = board_data(core_data);
	int r = 0;
/*remove it , because serdes touch not used rst pin */
/*
...
#if IS_ENABLED(CONFIG_TOUCHSCREEN_GOODIX_BRL)
