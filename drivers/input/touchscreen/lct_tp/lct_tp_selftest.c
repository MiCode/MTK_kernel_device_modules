...
static ssize_t tp_selftest_proc_write(struct file *file, const char __user *buf, size_t size, loff_t *ppos)
{
    ...
    if ((strncmp(tmp_data, TP_SELF_TEST_LONGCHEER_MMI_CMD, strlen(TP_SELF_TEST_LONGCHEER_MMI_CMD)) == 0) && (!is_in_self_test))
    { // mmi

        TP_LOGW("Longcheer MMI TP self-test ...\n");
        lct_tp_selftest_cmd = TP_SELFTEST_CMD_LONGCHEER_MMI;
    }
    else if ((strncmp(tmp_data, TP_SELF_TEST_XIAOMI_I2C_CMD, strlen(TP_SELF_TEST_XIAOMI_I2C_CMD)) == 0) && (!is_in_self_test))
    { // i2c

        TP_LOGW("Xiaomi TP i2c self-test ...\n");
        lct_tp_selftest_cmd = TP_SELFTEST_CMD_XIAOMI_I2C;
    }
    else if ((strncmp(tmp_data, TP_SELF_TEST_XIAOMI_OPEN_CMD, strlen(TP_SELF_TEST_XIAOMI_OPEN_CMD)) == 0) && (!is_in_self_test))
    { // open

        TP_LOGW("Xiaomi TP open self-test ...\n");
        lct_tp_selftest_cmd = TP_SELFTEST_CMD_XIAOMI_OPEN;
    }
    else if ((strncmp(tmp_data, TP_SELF_TEST_XIAOMI_SHORT_CMD, strlen(TP_SELF_TEST_XIAOMI_SHORT_CMD)) == 0) && (!is_in_self_test))
    { // short
