...
#include "../xiaomi_usb_touch_notifier/xiaomi_usb_touch_notifier.h"
#endif
...
char novatek_firmware[35];
char novatek_mp_firmware[35];
...
static struct work_struct nvt_resume_work;
...
struct spi_delay nvt_cs_setup = {
	.value = 300,
	.unit = 1,
};
struct spi_delay nvt_cs_hold = {
	.value = 300,
	.unit = 1,
};

#if WAKEUP_GESTURE
bool nvt_wakeup_switch_flag;
...
info_retry:
	...
	NVT_LOG("fw_ver=0x%02X, fw_type=0x%02X, PID=0x%04X, T/N:%d\n", ts->fw_ver, buf[14], ts->nvt_pid, buf[18]);
	ret = 0;
out:
...
static int nvt_touchfeature_cmd_xsfer(uint8_t *touchfeature)
{
	...
	int32_t retry = 5;
	NVT_LOG("++\n");
	NVT_LOG("cmd xsfer:mode-%02x, value-%02x", touchfeature[0], touchfeature[1]);
	/* ---set xdata index to EVENT BUF ADDR--- */
...
static int nvt_set_cur_value(int nvt_mode, int nvt_value)
{
	...
	if (nvt_mode < Touch_Mode_NUM && nvt_mode >= 0) {
		...
		case Touch_Resist_RF:
			temp_value =
				xiaomi_touch_interfaces.touch_mode[Touch_Resist_RF][SET_CUR_VALUE];
			if (temp_value == 0) {
		...
		case Touch_Doubletap_Mode:
			if (nvt_value == 0) {
				nvt_wakeup_switch_flag = false;
				NVT_LOG("gesture disabled:%d", nvt_wakeup_switch_flag);
			} else if (nvt_value == 1) {
				nvt_wakeup_switch_flag = true;
				NVT_LOG("gesture enabled:%d", nvt_wakeup_switch_flag);
			}
			nvt_get_gesture_90hz_thinning_status(nvt_wakeup_switch_flag);
			nvt_get_gesture_lc_90hz_thinning_status(nvt_wakeup_switch_flag);
			nvt_get_gesture_90hz_status(nvt_wakeup_switch_flag);
			nvt_get_gesture_60hz_status(nvt_wakeup_switch_flag);
			nvt_get_gesture_lc_90hz_status(nvt_wakeup_switch_flag);
			nvt_get_gesture_lc_60hz_status(nvt_wakeup_switch_flag);
			break;
		default:
			/* Don't support */
...
int32_t nvt_enter_proximity_mode(uint8_t proximity_switch)
{
	...
	if (unlikely(i >= retry)) {
		...
		NVT_LOG("send cmd success, tried %d times\n", i);
		nvt_90hz_thinning_lc_proximity_mode(proximity_switch);
		nvt_90hz_thinning_proximity_mode(proximity_switch);
		nvt_60hz_proximity_mode(proximity_switch);
		nvt_90hz_proximity_mode(proximity_switch);
		nvt_90hz_lc_proximity_mode(proximity_switch);
		nvt_60hz_lc_proximity_mode(proximity_switch);
		ret = 0;
	}
...
{
	NVT_LOG("%s nvt_charger_flag:%d\n", __func__, nvt_charger_flag);
	nvt_charger_mode_switch();
}
void nvt_get_chg_flag_work(struct work_struct *work)
{
	int count = 0;
	while(fw_update_status == 0) {
		msleep(2*1000);
		nvt_charger_flag = get_plug_status();
		count++;
	}
	NVT_LOG("%s nvt_charger_flag:%d count:%d\n",
		__func__, nvt_charger_flag, count);
	nvt_charger_mode_switch();
	fw_update_status = 0;
}
...
#endif
void nvt_resume_work_func(struct work_struct *work)
{
...
static int32_t nvt_ts_probe(struct spi_device *client)
{
	...
	char firmware_name_boe_60hz[] = "novatek_ts_fw_boe_60hz.bin";
	char firmware_name_boe_90hz[] = "novatek_ts_fw_boe_90hz.bin";
	char firmware_name_boe_thin_90hz[] = "novatek_ts_fw_boe_thin_90hz.bin";
	char firmware_mp_name_boe_60hz[] = "novatek_mp_fw_boe_60hz.bin";
	char firmware_mp_name_boe_thin_90hz[] = "novatek_mp_fw_boe_thin_90hz.bin";
	char firmware_mp_name_boe_90hz[] = "novatek_mp_fw_boe_90hz.bin";
	char firmware_name_lc_60hz[] = "novatek_ts_fw_lc_60hz.bin";
	char firmware_name_lc_90hz[] = "novatek_ts_fw_lc_90hz.bin";
	char firmware_name_lc_thin_90hz[] = "novatek_ts_fw_lc_thin_90hz.bin";
	char firmware_mp_name_lc_60hz[] = "novatek_mp_fw_lc_60hz.bin";
	char firmware_mp_name_lc_90hz[] = "novatek_mp_fw_lc_90hz.bin";
	char firmware_mp_name_lc_thin_90hz[] = "novatek_mp_fw_lc_thin_90hz.bin";
	...
	ts->client = client;
	memcpy(&ts->client->cs_setup, &nvt_cs_setup, sizeof(struct spi_delay));
	memcpy(&ts->client->cs_hold, &nvt_cs_hold, sizeof(struct spi_delay));
	...
	if (lcm_name) {
		...
		if (strcmp("dsi_panel_n85_35_02_0a_60_vdo_lcm_drv",
				videolfb_tag->lcmname) == 0 ) {
			strncpy(novatek_firmware, firmware_name_boe_60hz,
				sizeof(firmware_name_boe_60hz));
			strncpy(novatek_mp_firmware, firmware_mp_name_boe_60hz,
				sizeof(firmware_mp_name_boe_60hz));
			sprintf(ts->disp_name, "BOE");
		} else if (strcmp("dsi_panel_n85_35_02_0a_90_vdo_lcm_drv",
				videolfb_tag->lcmname) == 0 ) { 
			strncpy(novatek_firmware, firmware_name_boe_90hz,
				sizeof(firmware_name_boe_90hz));
			strncpy(novatek_mp_firmware, firmware_mp_name_boe_90hz,
				sizeof(firmware_mp_name_boe_90hz));
			sprintf(ts->disp_name, "BOE");
		} else if (strcmp("dsi_panel_n85_35_02_0a_90_thinning_vdo_lcm_drv",
				videolfb_tag->lcmname) == 0 ) {
			strncpy(novatek_firmware, firmware_name_boe_thin_90hz,
				sizeof(firmware_name_boe_thin_90hz));
			strncpy(novatek_mp_firmware, firmware_mp_name_boe_thin_90hz,
				sizeof(firmware_mp_name_boe_thin_90hz));
			sprintf(ts->disp_name, "BOE");
		} else if (strcmp("dsi_panel_n85_47_02_0b_60_vdo_lcm_drv",
				videolfb_tag->lcmname) == 0 ) {
			strncpy(novatek_firmware, firmware_name_lc_60hz,
				sizeof(firmware_name_lc_60hz));
			strncpy(novatek_mp_firmware, firmware_mp_name_lc_60hz,
				sizeof(firmware_mp_name_lc_60hz));
			sprintf(ts->disp_name, "CSOT");
		} else if (strcmp("dsi_panel_n85_47_02_0b_90_vdo_lcm_drv",
				videolfb_tag->lcmname) == 0 ) {
			strncpy(novatek_firmware, firmware_name_lc_90hz,
				sizeof(firmware_name_lc_90hz));
			strncpy(novatek_mp_firmware, firmware_mp_name_lc_90hz,
				sizeof(firmware_mp_name_lc_90hz));
			sprintf(ts->disp_name, "CSOT");
		} else if (strcmp("dsi_panel_n85_47_02_0b_90_thinning_vdo_lcm_drv",
				videolfb_tag->lcmname) == 0 ) {
			strncpy(novatek_firmware, firmware_name_lc_thin_90hz,
				sizeof(firmware_name_lc_thin_90hz));
			strncpy(novatek_mp_firmware, firmware_mp_name_lc_thin_90hz,
				sizeof(firmware_mp_name_lc_thin_90hz));
			sprintf(ts->disp_name, "CSOT");
		} else {
			NVT_ERR("lcm name default\n");
		}
	} else {
	...
	nvt_resume_workqueue = create_singlethread_workqueue("nvt_resume");
	INIT_WORK(&nvt_resume_work, nvt_resume_work_func);
	...
	ts->nvt_get_chg_flag_workqueue =
		create_singlethread_workqueue("nvt_get_chg_flag");
	if(!ts->nvt_get_chg_flag_workqueue) {
		NVT_ERR("create get_chg_flag workqueue fail");
	} else {
		INIT_WORK(&ts->nvt_chg_flag_work, nvt_get_chg_flag_work);
		queue_work(ts->nvt_get_chg_flag_workqueue, &ts->nvt_chg_flag_work);
	}
	ts->nvt_charger_detect_workqueue =
		create_singlethread_workqueue("nvt_charger_detect");
	if(!ts->nvt_charger_detect_workqueue) {
		NVT_ERR("create charger_detect workqueue fail");
	} else {
...
static int32_t nvt_ts_suspend(struct device *dev)
{
...
#if WAKEUP_GESTURE
	if (nvt_wakeup_switch_flag == false)
		nvt_irq_enable(false);
#else
	nvt_irq_enable(false);
#endif
...
#if WAKEUP_GESTURE
	if (nvt_wakeup_switch_flag == true) {
	...
	} else if (nvt_wakeup_switch_flag == false ) {
...
extern uint8_t edge_reject_switch_status;
/*******************************************************
Description:
...
static int32_t nvt_ts_resume(struct device *dev)
{
	...
	nvt_charger_mode_switch();
	nvt_set_edge_reject_switch(edge_reject_switch_status);
	NVT_LOG("end\n");
					 ...
					 unsigned long event, void *data)
{
    ...
    if (event == MTK_DRM_EARLY_EVENT_BLANK) {
	...
	} else if (event == MTK_DRM_EVENT_BLANK) {
		if (*blank == MTK_DRM_BLANK_UNBLANK) {
			NVT_LOG("event=%lu, *blank=%d\n", event, *blank);
			queue_work(nvt_resume_workqueue, &nvt_resume_work);
		}
	}
