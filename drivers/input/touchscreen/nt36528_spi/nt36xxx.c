...
char nvt_tp_fw_name[50] = {0};
char nvt_tp_mp_name[50] = {0};
...
#if WAKEUP_GESTURE
extern void set_lcd_reset_gpio_keep_high_1st(bool en);
extern void set_lcd_reset_gpio_keep_high_3rd(bool en);
...
static int tp_nvt_ts_esd_suspend(void);
extern int (*lcd_nvt_ts_esd_suspend)(void);
...
#ifdef CONFIG_DEVICE_MODULES_SPI_MT65XX
...
#if NVT_USB_PLUGIN
static void nvt_ts_usb_plugin_handler(int usbFlag)
{
	uint8_t buf[8] = {0};
	int32_t ret = 0;
	if ( !bTouchIsAwake ) {
		NVT_ERR("TP is suspend\n");
		return;
	}
	mutex_lock(&ts->lock);
	msleep(35);
...
static void nvt_usb_online_work(struct work_struct *work)
{
	int ret;
    ...
    //NVT_LOG("last_usb_online=%d, pval.intval=%d\n", last_usb_online, pval.intval);

	if (pval.intval != last_usb_online){
        current_usb_online = pval.intval;
		NVT_LOG("usb_online state has changed, current_online(%d), last_online(%d) \n", pval.intval, last_usb_online);
		nvt_ts_usb_plugin_handler(pval.intval);
		last_usb_online = pval.intval;
	}
}

static int nvt_power_usb_notifier_callback(struct notifier_block *nb, unsigned long ev, void *v)
{
    struct nvt_ts_data *ts = container_of(nb, struct nvt_ts_data, power_usb_notifier);

    if (ev != PSY_EVENT_PROP_CHANGED)
        return -1;

    //flush_workqueue(ts->resume_wq);
    queue_work(ts->resume_wq, &ts->online_work);
...
info_retry:
	...
	if (strcmp(touch_lcm_name, "dsi_panel_p15a_42_02_0a_dsc_vdo")  == 0) {
		NVT_LOG("tp_info: [Vendor]huaxing,[FW]0x%02x,[IC]nt36528a\n",ts->fw_ver);
		sprintf(tp_info_buf, "[Vendor]huaxing,[FW]0x%02x,[IC]nt36528a\n", ts->fw_ver);
	} else if(strcmp(touch_lcm_name, "dsi_panel_p15a_43_02_0c_dsc_vdo") == 0) {
		NVT_LOG("tp_info: [Vendor]xinli,[FW]0x%02x,[IC]nt36528a\n",ts->fw_ver);
		sprintf(tp_info_buf, "[Vendor]xinli,[FW]0x%02x,[IC]nt36528a\n", ts->fw_ver);
	} else {
		NVT_ERR("unkown firmware name!!");
		sprintf(tp_info_buf, "unkown firmware name !!");
		return -1;
	}
	update_lct_tp_info(tp_info_buf);
	ret = 0;
out:
...
static void nvt_esd_check_func(struct work_struct *work)
{
	...
	if ((timer > NVT_TOUCH_ESD_CHECK_PERIOD) && esd_check) {
		...
		nvt_update_firmware(nvt_tp_fw_name);
		mutex_unlock(&ts->lock);
		/* update interrupt timer */
		irq_timer = jiffies;
...
{
	NVT_LOG("This enter nvt_ts_work_func\n");
	int32_t ret = -1;
...
#if WAKEUP_GESTURE
	if (bTouchIsAwake == 0) {
		pm_wakeup_event(&ts->input_dev->dev, 700);
	}
#endif
...
//	NVT_LOG("This is before read failed and ret = %d\n", ret);
	if (ret < 0) {
		NVT_ERR("CTP_SPI_READ failed.(%d)\n", ret);
		goto XFER_ERROR;
	}
...
*/
	...
	if (bTouchIsAwake == 0) {
		input_id = (uint8_t)(point_data[1] >> 3);
//		NVT_LOG("This is before nvt_ts_wakeup_gesture_report\n");
...
static int32_t nvt_ts_check_chip_ver_trim(struct nvt_ts_hw_reg_addr_info hw_regs)
{
	...
	for (retry = 5; retry > 0; retry--) {
		...
		NVT_LOG("buf[0]=0x%02X, buf[1]=0x%02X, buf[2]=0x%02X, buf[3]=0x%02X, buf[4]=0x%02X, buf[5]=0x%02X, buf[6]=0x%02X\n",
...
static int lct_tp_get_screen_angle_callback(void)
{
	...
	edge_reject_switch = ((tmp[1] >> 5) & 0x07);
...
//create longcheer procfs node
...
#if NVT_USB_PLUGIN
	INIT_WORK(&ts->online_work, nvt_usb_online_work);
    ts->power_usb_notifier.notifier_call = nvt_power_usb_notifier_callback;
	ret = power_supply_reg_notifier(&ts->power_usb_notifier);
	if (ret < 0) {
	...
	lcd_nvt_ts_esd_suspend = tp_nvt_ts_esd_suspend;
	NVT_LOG("end\n");
...
#if NVT_USB_PLUGIN
	power_supply_unreg_notifier(&ts->power_usb_notifier);
err_register_power_usb_notifier_failed:
	ts->power_usb_notifier.notifier_call = NULL;
	cancel_work_sync(&ts->online_work);
#endif
...
static int32_t nvt_ts_resume(struct device *dev)
{
	...
	#if NVT_USB_PLUGIN
		nvt_ts_usb_plugin_handler(current_usb_online);
	#endif
	...
	gpio_set_value(ts->reset_gpio, 1);
#endif
	if (nvt_update_firmware(nvt_tp_fw_name)) {
...
int lct_nvt_tp_gesture_callback(bool flag)
{
	...
	if (flag) {
		...
		set_lcd_reset_gpio_keep_high_1st(true);
		set_lcd_reset_gpio_keep_high_3rd(true);
		NVT_LOG("enable gesture mode\n");
	} else {
		ts->is_gesture_mode = false;
		set_lcd_reset_gpio_keep_high_1st(false);
		set_lcd_reset_gpio_keep_high_3rd(false);
...
static int32_t __init nvt_driver_init(void)
{
	...
	lct_tp_get_panel_info();
	NVT_LOG("touch_lcm_name=%s\n",touch_lcm_name);
	memset(nvt_tp_fw_name, 0, sizeof(nvt_tp_fw_name));
	memset(nvt_tp_mp_name, 0, sizeof(nvt_tp_mp_name));
	if (strcmp(touch_lcm_name, "dsi_panel_p15a_42_02_0a_dsc_vdo")  == 0)
	{
		strcpy(nvt_tp_fw_name, BOOT_UPDATE_FIRMWARE_NAME_TCL_1ST);
		strcpy(nvt_tp_mp_name, MP_UPDATE_FIRMWARE_NAME_TCL_1ST);
		NVT_LOG("nvt:huaxing+NT36528A,touch_lcm_name_success=%s\n",touch_lcm_name);
	} else if (strcmp(touch_lcm_name, "dsi_panel_p15a_43_02_0c_dsc_vdo") == 0)
	{
		strcpy(nvt_tp_fw_name, BOOT_UPDATE_FIRMWARE_NAME_XINLI_3RD);
		strcpy(nvt_tp_mp_name, MP_UPDATE_FIRMWARE_NAME_XINLI_3RD);
		NVT_LOG("nvt:xinli+NT36528A,touch_lcm_name_success=%s\n",touch_lcm_name);
	} else
	{
		return -1;
	}
