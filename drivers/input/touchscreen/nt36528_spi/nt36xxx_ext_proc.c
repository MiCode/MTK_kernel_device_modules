...
extern void set_prox_lcd_reset_gpio_keep_high1(bool en);
extern void set_prox_lcd_reset_gpio_keep_high3(bool en);
extern bool tp_sensor_probe_switch;
int32_t proximity_event = 0;
/*******************************************************
Description:
...
int32_t nvt_set_proximity_switch(uint8_t proximity_switch)
{
	...
	NVT_LOG("tp_sensor_probe_switch: %d\n", tp_sensor_probe_switch);
	if (tp_sensor_probe_switch == true) {
	if ((ts->sensor_tp_fw_status == true) && (ts->sensor_tp_probe_status == true))
	{
		NVT_LOG("sensor_tp_probe_status_switch = %d\n",ts->sensor_tp_probe_status);
		NVT_LOG("sensor_tp_fw_status_switch = %d\n",ts->sensor_tp_fw_status);
	...
	for (i = 0; i < retry; i++) {
		...
		if (proximity_switch == 0) {
			// proximity disable
			set_prox_lcd_reset_gpio_keep_high1(false);
			set_prox_lcd_reset_gpio_keep_high3(false);
			proximity_event = 0;
			buf[1] = 0x86;
		} else if (proximity_switch == 1) {
			// proximity enable
			set_prox_lcd_reset_gpio_keep_high1(true);
			set_prox_lcd_reset_gpio_keep_high3(true);
			proximity_event = 1;
			buf[1] = 0x85;
		} else {
