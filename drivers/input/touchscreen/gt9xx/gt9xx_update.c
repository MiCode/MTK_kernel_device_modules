...
static u8 gup_get_ic_fw_msg(struct i2c_client *client)
{
	...
	if (FAIL == ret) {
		dev_dbg(&client->dev, "[get_ic_fw_msg]get hw_info failed,exit");
		return FAIL;
	}
	...
	for (retry = 0; retry < 2; retry++) {
		...
		if (FAIL == ret) {
			dev_dbg(&client->dev, "Read firmware message fail.");
			return ret;
		}
	...
	if (FAIL == ret) {
		dev_dbg(&client->dev, "[get_ic_fw_msg]get pid & vid failed,exit");
		return FAIL;
	}
...
s32 gup_enter_update_mode(struct i2c_client *client)
{
	...
	if (!gpio_is_valid(ts->pdata->rst_gpio)) {
		dev_dbg(&ts->client->dev, "update failed, no rst pin\n");
		return FAIL;
	}
	...
	if (retry >= 200) {
		dev_dbg(&client->dev, "Enter update Hold ss51 failed.");
		return FAIL;
	}
				 ...
				 struct st_fw_head *fw_head)
{
	...
	if ((!memcmp(fw_head->pid, update_msg.ic_fw_msg.pid, pid_cmp_len)) ||
		...
		dev_dbg(&client->dev, "File VID == Ic VID, update aborted!");
	} else {
		dev_dbg(&client->dev, "File PID != Ic PID, update aborted!");
	}
...
static int gup_update_config(struct i2c_client *client)
{
	...
	if (!fw_cfg || !fw_cfg->data || fw_cfg->size > PAGE_SIZE) {
		dev_dbg(&client->dev, "config file illegal");
		ret = -EFAULT;
		goto cfg_fw_err;
	}
	...
	file_config = kzalloc(GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH,
			      GFP_KERNEL);
	if (!file_config) {
		dev_dbg(&ts->client->dev, "failed alloc memory");
		ret = -ENOMEM;
		goto cfg_fw_err;
	}
	...
	if (file_cfg_len < 0) {
		dev_dbg(&client->dev, "failed covert ascii to hex");
		ret = -EFAULT;
		...
		u8 **path_p)
{
	...
	if (len >= FIRMWARE_NAME_LEN_MAX) {
		dev_dbg(&client->dev, "firmware name too long!");
		return -EINVAL;
	}
				...
				struct st_fw_head *fw_head, u8 *path)
{
	...
	if (ts->pdata->auto_update_cfg) {
		...
		if (ret <= 0)
			dev_dbg(&client->dev, "Update config failed.");
	}
	...
	if (ret < 0) {
		dev_dbg(&client->dev, "Failed get firmware:%d\n", ret);
		return FAIL;
	}
	...
	if (fw_checksum & 0xFFFF) {
		dev_dbg(&client->dev, "Illegal firmware file.");
		goto invalied_fw;
	}
...
static u8 gup_load_section_file(u8 *buf, u32 offset, u16 length, u8 set_or_end)
{
	if (!update_msg.fw_data ||
		...
		dev_dbg(&i2c_connect_client->dev,
			   ...
			   u16 start_rd_addr, u16 chk_length)
{
	...
	while (recall_length < chk_length) {
		...
		if (ret <= 0) {
			dev_dbg(&client->dev, "recall i2c error,exit");
			return FAIL;
		}

		if (memcmp(&rd_buf[GTP_ADDR_LENGTH],
		&chk_src[recall_length], frame_length)) {
			dev_dbg(&client->dev, "Recall frame data fail,not equal.");
			      ...
			      u16 start_addr, u8 bank_cmd)
{
	...
	ret = gup_set_ic_msg(client, _rRW_MISCTL__SWRST_B0_, 0x0C);
	if (ret <= 0) {
		dev_dbg(&client->dev, "[burn_fw_section]hold ss51 & dsp fail.");
		return FAIL;
	}
	...
	if (ret <= 0) {
		dev_dbg(&client->dev, "[burn_fw_section]set scramble fail.");
		return FAIL;
	}
	...
	if (ret <= 0) {
		dev_dbg(&client->dev, "[burn_fw_section]send burn cmd fail.");
		return FAIL;
	}
	dev_dbg(&client->dev,
...
static u8 gup_burn_dsp_isp(struct i2c_client *client)
{
	...
	if (retry >= 5) {
		dev_dbg(&client->dev, "[burn_dsp_isp]Alloc memory fail,exit.");
		return FAIL;
	}
	...
	if (ret <= 0) {
		dev_dbg(&client->dev, "[burn_dsp_isp]disable wdt fail.");
		ret = FAIL;
		goto exit_burn_dsp_isp;
	}
	...
	ret = gup_set_ic_msg(client, _rRW_MISCTL__SWRST_B0_, 0x0C);
	if (ret <= 0) {
		dev_dbg(&client->dev, "[burn_dsp_isp]hold ss51 & dsp fail.");
		ret = FAIL;
		goto exit_burn_dsp_isp;
	}
	...
	if (ret <= 0) {
		dev_dbg(&client->dev, "[burn_dsp_isp]set boot from sram fail");
		ret = FAIL;
		goto exit_burn_dsp_isp;
	}
	...
	if (ret <= 0) {
		dev_dbg(&client->dev, "[burn_dsp_isp]software reboot fail.");
		ret = FAIL;
		goto exit_burn_dsp_isp;
	}
	...
	if (ret <= 0) {
		dev_dbg(&client->dev, "[burn_dsp_isp]select bank2 fail");
		ret = FAIL;
		goto exit_burn_dsp_isp;
	}
	...
	if (FAIL == ret) {
		dev_dbg(&client->dev, "[burn_dsp_isp]burn dsp_isp fail.");
		goto exit_burn_dsp_isp;
	}
	...
	ret = gup_set_ic_msg(client, _rRW_MISCTL__BOOT_OPT_B0_, 0x00);
	if (ret <= 0) {
		dev_dbg(&client->dev, "[burn_dsp_isp]set scramble fail.");
		ret = FAIL;
		goto exit_burn_dsp_isp;
	}
...
static u8 gup_burn_fw_ss51(struct i2c_client *client)
{
	...
	if (retry >= 5) {
		dev_dbg(&client->dev, "[burn_fw_ss51]Alloc memory fail,exit.");
		return FAIL;
	}
	...
	if (ret <= 0) {
		dev_dbg(&client->dev, "[burn_fw_ss51]clear control flag fail.");
		ret = FAIL;
...
static u8 gup_burn_fw_dsp(struct i2c_client *client)
{
	...
	if (retry >= 5) {
		dev_dbg(&client->dev, "[burn_fw_dsp]Alloc memory fail,exit.");
		return FAIL;
	}
	...
	if (FAIL == ret) {
		dev_dbg(&client->dev, "[burn_fw_dsp]load firmware dsp fail.");
	...
	if (ret <= 0) {
		dev_dbg(&client->dev, "[burn_fw_dsp]select bank3 fail.");
		ret = FAIL;
		goto exit_burn_fw_dsp;
	}
	...
	ret = gup_set_ic_msg(client, _rRW_MISCTL__SWRST_B0_, 0x0C);
	if (ret <= 0) {
		dev_dbg(&client->dev, "[burn_fw_dsp]hold ss51 & dsp fail.");
		ret = FAIL;
		goto exit_burn_fw_dsp;
	}
	...
	ret = gup_set_ic_msg(client, _rRW_MISCTL__BOOT_OPT_B0_, 0x00);
	if (ret <= 0) {
		dev_dbg(&client->dev, "[burn_fw_dsp]set scramble fail.");
		ret = FAIL;
		goto exit_burn_fw_dsp;
	}
	...
	if (ret <= 0) {
		dev_dbg(&client->dev, "[burn_fw_dsp]release ss51 & dsp fail.");
		ret = FAIL;
		goto exit_burn_fw_dsp;
	}
	/* must delay */
	...
	if (FAIL == ret) {
		dev_dbg(&client->dev, "[burn_fw_dsp]burn fw_section fail.");
		goto exit_burn_fw_dsp;
	}
	...
	if (ret <= 0) {
		dev_dbg(&client->dev, "[burn_fw_dsp]send burn cmd fail.");
		goto exit_burn_fw_dsp;
	}
	...
	do {
		ret = gup_get_ic_msg(client, _rRW_MISCTL__BOOT_CTL_, rd_buf, 1);
		if (ret <= 0) {
			dev_dbg(&client->dev, "[burn_fw_dsp]Get burn state fail");
			goto exit_burn_fw_dsp;
		}
		usleep_range(10000, 11000);
...
static u8 gup_burn_fw_boot(struct i2c_client *client)
{
	...
	if (retry >= 5) {
		dev_dbg(&client->dev, "[burn_fw_boot]Alloc memory fail,exit.");
		return FAIL;
	}
	...
	ret = gup_set_ic_msg(client, _rRW_MISCTL__SWRST_B0_, 0x0C);
	if (ret <= 0) {
		dev_dbg(&client->dev, "[burn_fw_boot]hold ss51 & dsp fail.");
		ret = FAIL;
		goto exit_burn_fw_boot;
	}
	...
	ret = gup_set_ic_msg(client, _rRW_MISCTL__BOOT_OPT_B0_, 0x00);
	if (ret <= 0) {
		dev_dbg(&client->dev, "[burn_fw_boot]set scramble fail.");
		ret = FAIL;
		goto exit_burn_fw_boot;
	}
	...
	/* 20121211 */
	if (ret <= 0) {
		dev_dbg(&client->dev, "[burn_fw_boot]release ss51 & dsp fail");
		ret = FAIL;
		goto exit_burn_fw_boot;
	}
	/* must delay */
	...
	ret = gup_set_ic_msg(client, _bRW_MISCTL__SRAM_BANK, 0x03);
	if (ret <= 0) {
		dev_dbg(&client->dev, "[burn_fw_boot]select bank3 fail.");
		ret = FAIL;
		goto exit_burn_fw_boot;
	}
	...
	if (FAIL == ret) {
		dev_dbg(&client->dev, "[burn_fw_boot]burn fw_boot fail.");
		goto exit_burn_fw_boot;
	}
	...
	if (ret <= 0) {
		dev_dbg(&client->dev, "[burn_fw_boot]send burn cmd fail.");
		goto exit_burn_fw_boot;
	}
	dev_dbg(&client->dev,
...
static u8 gup_burn_fw_boot_isp(struct i2c_client *client)
{
	...
	ret = gup_set_ic_msg(client, _rRW_MISCTL__SWRST_B0_, 0x0C);
	if (ret <= 0) {
		dev_dbg(&client->dev, "[burn_fw_boot_isp]hold ss51 & dsp fail");
		ret = FAIL;
		goto exit_burn_fw_boot_isp;
	}
	...
	ret = gup_set_ic_msg(client, _rRW_MISCTL__BOOT_OPT_B0_, 0x00);
	if (ret <= 0) {
		dev_dbg(&client->dev, "[burn_fw_boot_isp]set scramble fail.");
		ret = FAIL;
		goto exit_burn_fw_boot_isp;
	}
	...
	ret = gup_set_ic_msg(client, _bRW_MISCTL__SRAM_BANK, 0x03);
	if (ret <= 0) {
		dev_dbg(&client->dev, "[burn_fw_boot_isp]select bank3 fail.");
		ret = FAIL;
		goto exit_burn_fw_boot_isp;
	}
	...
	if (ret <= 0) {
		dev_dbg(&client->dev, "[burn_fw_boot_isp]send burn cmd fail.");
		goto exit_burn_fw_boot_isp;
	}
	dev_dbg(&client->dev,
...
static u8 gup_burn_fw_link(struct i2c_client *client)
{
	...
	if (retry >= 5) {
		dev_dbg(&client->dev, "[burn_fw_link]Alloc memory fail,exit.");
		return FAIL;
	}
...
static u8 gup_burn_fw_gwake(struct i2c_client *client)
{
	...
	if (retry >= 5) {
		dev_dbg(&client->dev, "[burn_fw_gwake]Alloc memory fail,exit.");
		return FAIL;
	}
...
s32 gup_update_proc(void *dir)
{
	...
	if (ret) {
		dev_dbg(&ts->client->dev, "Update start failed\n");
		return FAIL;
	}
	if (test_and_set_bit(FW_UPDATE_RUNNING, &ts->flags)) {
		dev_dbg(&ts->client->dev, "FW update may already running\n");
		return FAIL;
	}
	...
	if (FAIL == ret) {
		dev_dbg(&ts->client->dev, "[update_proc]get ic message fail.");
		goto file_fail;
	}
