...
#define INPUT_PHYS_NAME "mtk-tpd1/touch_input"
#define INPUT_TOUCH_NAME "mtk-tpd1"
#define STYLUS_PHYS_NAME "synaptics_dsx1/stylus"
...
static int synaptics_rmi4_gpio_setup(int gpio, bool config, int dir, int state)
{
	...
	if (config) {
		//snprintf(buf, PAGE_SIZE, "dsx_gpio_%u\n", gpio);/*'snprintf' size argument is too large; destination buffer has size 16*/
		...
		if (retval) {
			pr_info("%s: Failed to set gpio %d direction",
					__func__, gpio);
			return retval;
		}
	} else {
...
static int synaptics_rmi4_set_input_dev(struct synaptics_rmi4_data *rmi4_data)
{
	...
	rmi4_data->input_dev->name = INPUT_TOUCH_NAME;
...
static int synaptics_rmi4_free_fingers(struct synaptics_rmi4_data *rmi4_data)
{
	unsigned char ii = 0;
	mutex_lock(&(rmi4_data->rmi4_report_mutex));
pr_info("[synaptics_rmi4_free_fingers] ii = %d\n", ii);
#ifdef TYPE_B_PROTOCOL
...
void synaptics_rmi4_new_function_96851(struct synaptics_rmi4_exp_fn *exp_fn,
		bool insert)
{
	struct synaptics_rmi4_exp_fhandler *exp_fhandler;
	...
	if (insert) {
		...
		if (!exp_fhandler) {
			pr_info("%s: Failed to alloc mem for expansion function\n",
					__func__);
			goto exit;
		}
...
EXPORT_SYMBOL(synaptics_rmi4_new_function_96851);
...
static int __init synaptics_rmi4_init_96851(void)
{
	int retval;

	retval = synaptics_rmi4_bus_init_96851();
	if (retval)
		return retval;
...
static void __exit synaptics_rmi4_exit_96851(void)
{
	platform_driver_unregister(&synaptics_rmi4_driver);

	synaptics_rmi4_bus_exit_96851();
	return;
}

module_init(synaptics_rmi4_init_96851);
module_exit(synaptics_rmi4_exit_96851);
