...
static struct platform_device *synaptics_dsx_i2c_device_96851;
...
static int parse_dt(struct device *dev, struct synaptics_dsx_board_data *bdata)
{
	...
	/*bdata->irq_gpio = of_get_named_gpio_flags(np,
			...
			(enum of_gpio_flags *)&bdata->irq_flags);*///kernel-6.6 not support
	...
	if (prop && prop->length) {
		...
		retval = of_property_read_u32(np, "synaptics,power-on-state",
				&value);
		if (retval < 0) {
			dev_dbg(dev, "%s: Unable to read synaptics,power-on-state property\n",
					__func__);
			return retval;
		} else {
	...
	if (prop && prop->length) {
		retval = of_property_read_u32(np, "synaptics,power-delay-ms",
				&value);
		if (retval < 0) {
			dev_dbg(dev, "%s: Unable to read synaptics,power-delay-ms property\n",
					__func__);
			return retval;
		} else {
	...
	if (prop && prop->length) {
		...
		retval = of_property_read_u32(np, "synaptics,reset-on-state",
				&value);
		if (retval < 0) {
			dev_dbg(dev, "%s: Unable to read synaptics,reset-on-state property\n",
					__func__);
			return retval;
		} else {
		...
		retval = of_property_read_u32(np, "synaptics,reset-active-ms",
				&value);
		if (retval < 0) {
			dev_dbg(dev, "%s: Unable to read synaptics,reset-active-ms property\n",
					__func__);
			return retval;
		} else {
	...
	if (prop && prop->length) {
		retval = of_property_read_u32(np, "synaptics,reset-delay-ms",
				&value);
		if (retval < 0) {
			dev_dbg(dev, "%s: Unable to read synaptics,reset-delay-ms property\n",
					__func__);
			return retval;
		} else {
	...
	if (prop && prop->length) {
		retval = of_property_read_u32(np, "synaptics,max-y-for-2d",
				&value);
		if (retval < 0) {
			dev_dbg(dev, "%s: Unable to read synaptics,max-y-for-2d property\n",
					__func__);
			return retval;
		} else {
	...
	if (prop && prop->length) {
		retval = of_property_read_u32(np, "synaptics,ub-i2c-addr",
				&value);
		if (retval < 0) {
			dev_dbg(dev, "%s: Unable to read synaptics,ub-i2c-addr property\n",
					__func__);
			return retval;
		} else {
...
{
	if (hw_if.board_data->ub_i2c_addr == 0xFFFFFFFF)
		return;
...
{
	kfree(synaptics_dsx_i2c_device_96851);
...
static int synaptics_rmi4_i2c_probe(struct i2c_client *client)
{
	...
	synaptics_dsx_i2c_device_96851 = kzalloc(
			sizeof(struct platform_device),
			GFP_KERNEL);
	if (!synaptics_dsx_i2c_device_96851) {
		dev_dbg(&client->dev,
				"%s: Failed to allocate memory for synaptics_dsx_i2c_device_96851\n",
				__func__);
		return -ENOMEM;
	}
	...
	synaptics_dsx_i2c_device_96851->name = PLATFORM_DRIVER_NAME;
	synaptics_dsx_i2c_device_96851->id = 0;
	synaptics_dsx_i2c_device_96851->num_resources = 0;
	synaptics_dsx_i2c_device_96851->dev.parent = &client->dev;
	synaptics_dsx_i2c_device_96851->dev.platform_data = &hw_if;
	synaptics_dsx_i2c_device_96851->dev.release = synaptics_rmi4_i2c_dev_release;

	retval = platform_device_register(synaptics_dsx_i2c_device_96851);
	if (retval) {
		dev_dbg(&client->dev,
...
void synaptics_rmi4_i2c_remove(struct i2c_client *client)
{
	platform_device_unregister(synaptics_dsx_i2c_device_96851);
...
static struct of_device_id synaptics_rmi4_of_match_table[] = {
	{
		.compatible = "synaptics,dsx-i2c-96851",
	},
	{},
};
...
int synaptics_rmi4_bus_init_96851(void)
{
	return i2c_add_driver(&synaptics_rmi4_i2c_driver);
}

EXPORT_SYMBOL(synaptics_rmi4_bus_init_96851);

void synaptics_rmi4_bus_exit_96851(void)
{
	kfree(wr_buf);
...
EXPORT_SYMBOL(synaptics_rmi4_bus_exit_96851);
