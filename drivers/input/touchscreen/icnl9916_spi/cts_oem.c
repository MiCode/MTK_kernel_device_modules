...
int lct_tp_cts_selftest_all(void);
        ...
        const u16 *data, int rows, int cols)
{
    ...
    for (r = 0; r < rows; r++) {
        ...
        dump_tsdata_row_to_buffer(linebuf, sizeof(linebuf),
            data, cols, NULL, "\n", ',');
        seq_puts(m, linebuf);
    ...
    const u8 *data, int rows, int cols)
{
    ...
    for (r = 0; r < rows; r++) {
        ...
        dump_comp_cap_row_to_buffer(linebuf, sizeof(linebuf),
            data, cols, NULL, "\n", ',');
        seq_puts(m, linebuf);
...
static int save_selftest_data_to_file(struct cts_oem_data *oem_data)
{
    ...
    if (oem_data->test_noise) {
        ret = dump_tsdata_to_csv_file(OEM_NOISE_TEST_DATA_FILEPATH,
            ...
            (oem_data->noise_test_frames + 3), rows, cols);
        if (ret < 0) {
...
extern int cts_set_proximity_switch(bool prox_switch);
...
int cts_oem_init(struct chipone_ts_data *cts_data)
{
	...
	ret = init_lct_tp_proximity_switch(cts_set_proximity_switch);
	if (ret < 0) {
		cts_err("init_lct_tp_proximity_switch Failed!\n");
