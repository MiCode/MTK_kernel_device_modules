...
{
    cts_info("Deinit MTK drm notifier");
#ifdef CFG_CTS_DRM_NOTIFIER
    {
        int ret = 0;
...
#ifdef CONFIG_LCT_EARJACK_DETECT
    g_touchscreen_earphone_plugin.event_callback = cts_ts_earphone_event_callback;
#endif
...
#endif
{
    ...
    if (cts_data) {
        ...
        g_touchscreen_earphone_plugin.valid = false;
                g_touchscreen_earphone_plugin.event_callback = NULL;
    }
#endif
...
static int __init cts_driver_init(void)
{
    ...
    if (strcmp(touch_lcm_name, "dsi_panel_p15a_36_0f_0d_dsc_vdo") == 0) {
		cts_info("TP info: [Vendor]tianma [IC]icnl9916x");
	} else {
		return -1;
	}
