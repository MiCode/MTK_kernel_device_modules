...
static void cts_ts_earphone_plugin_work_func(struct work_struct *work)
{
	...
	cts_tcs_set_earjack_plug(cts_dev, g_touchscreen_earphone_plugin.earphone_plugged_in);
	cts_info("g_touchscreen_earphone_plugin.earphone_plugged_in = %d",g_touchscreen_earphone_plugin.earphone_plugged_in);
	return;
}
#endif
...
static int cts_init_fwdata(struct cts_device *cts_dev)
{
    ...
    if(strcmp(touch_lcm_name, "dsi_panel_p15a_36_0f_0d_dsc_vdo") == 0) {
        sprintf(tp_info_buf, "[Vendor]tianma,[FW]0x%02x,[IC]icnl9916x\n", (fwdata->version)&0x00ff);
    } else {
        cts_err("get_fw_ver failed");
    }
...
int cts_set_proximity_switch(bool prox_switch)
{
    int ret = 0;
    ...
    if(1 == prox_switch) {
        set_prox_lcd_reset_gpio_keep_high4(true);
        cts_proximity_en = 1;
        cts_enable_proximity_mode(&g_cts_data->cts_dev);
    }
    else if(0 == prox_switch) {
        set_prox_lcd_reset_gpio_keep_high4(false);
        cts_proximity_en = 0;
...
int cts_irq_handler(struct cts_device *cts_dev)
{
	...
	if(palm_sensor_write_value == 1)
	{
		if (CFG_CTS_PALM_ID == touch_info->vkey_state) {
		    //cts_report_palm_event(cts_dev->pdata);//input keycode # in short message
        ...
        if (cts_is_proximity_enable(cts_dev)) {
            ...
            } else if (first_proxi || (cts_dev->rtdata.proximity_status
...
#ifdef CONFIG_LCT_EARJACK_DETECT
    if (!IS_ERR_OR_NULL(g_touchscreen_earphone_plugin.event_callback)) {
        g_touchscreen_earphone_plugin.valid = true;
    }
#endif
    if (ret == 0)
