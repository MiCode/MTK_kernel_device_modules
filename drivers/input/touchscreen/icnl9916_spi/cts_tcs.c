...
#define INT_DATA_TYPE_U16_SIZ        \
    ...
    if (data_types != INT_DATA_TYPE_NONE) {
        ...
        if ((data_types & INT_DATA_TYPE_NOISE_DIFF)) {
            data_size += (INT_DATA_TYPE_LEN_SIZ + INT_DATA_TYPE_U16_SIZ/3);
        }
...
int cts_tcs_set_pocket_palm_mode(const struct cts_device *cts_dev, u8 enable)
{
    ...
    ret = cts_tcs_write(cts_dev, CMD_SYS_STS_POCKET_MODE_EN_RW, buf, sizeof(buf));
    if (ret != 0) {
...
int cts_tcs_get_pocket_palm_mode(const struct cts_device *cts_dev, u8 *enabled)
{
    ...
    cts_info("Get pocket enable:%d", enabled);
    status->pocket = *enabled;

    ret = cts_tcs_read(cts_dev, CMD_SYS_STS_POCKET_MODE_EN_RW, buf, sizeof(buf));
    if (ret != 0) {
        cts_err("Set pocket failed!");
    }
