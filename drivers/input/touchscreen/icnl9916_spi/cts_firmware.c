...
#ifdef CFG_CTS_FIRMWARE_IN_FS
if (strcmp(touch_lcm_name, "dsi_panel_p15a_36_0f_0d_dsc_vdo") == 0)
{
#ifndef CFG_CTS_FOR_GKI
    if (is_filesystem_mounted(CFG_CTS_FIRMWARE_FILEPATH))
#endif
#ifdef CFG_CTS_FW_UPDATE_FILE_LOAD
        firmware_from_file = cts_request_newer_firmware_from_fs(cts_dev,cts_dev->config_fw_name[0] ? cts_dev->config_fw_name : CFG_CTS_FIRMWARE_FILENAME,firmware_builtin ? FIRMWARE_VERSION(firmware_builtin) : curr_firmware_ver);
#else
        firmware_from_file = cts_request_newer_firmware_from_fs(cts_dev,CFG_CTS_FIRMWARE_FILENAME,firmware_builtin ? FIRMWARE_VERSION(firmware_builtin) : curr_firmware_ver);
#endif
