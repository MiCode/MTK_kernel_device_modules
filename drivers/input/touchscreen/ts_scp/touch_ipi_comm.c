...
#define pr_fmt(fmt) "touch_ipi_comm " fmt
...
static int ipi_transfer_buffer(struct ipi_transfer *t)
{
	...
	if (now_time - wakeup_time > timeout_ns)
		pr_info_ratelimited("wakeup too long, now %lld, wakeup %lld\n",
...
static int ipi_comm_notify_handler(unsigned int id, void *prdata,
		void *data, unsigned int len)
{
	WARN_ON(id != IPI_IN_TOUCH_NOTIFY);
...
int ipi_comm_init(void)
{
	...
	controller.workqueue = alloc_workqueue("touch_ipi_comm",
	...
	if (ret < 0)
		pr_info("register ipi %u fail %d\n", IPI_IN_TOUCH_CTRL, ret);
	ret = mtk_ipi_register(&scp_ipidev, IPI_IN_TOUCH_NOTIFY,
