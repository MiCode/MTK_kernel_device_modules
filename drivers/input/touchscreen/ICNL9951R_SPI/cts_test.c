...
unlock:
    ret2 = cts_set_int_data_types(cts_dev, INT_DATA_TYPE_NONE);
    if (ret2) {
        cts_err("Set INT_DATA_TYPE_NONE failed %d", ret2);
    }

    ret2 = cts_set_int_data_method(cts_dev, INT_DATA_METHOD_NONE);
    if (ret2) {
        cts_err("Set INT_DATA_METHOD_NONE failed %d", ret2);
    }
...
unlock_device:
    ...
    ret2 = cts_wait_current_mode(cts_dev, CTS_KRANG_NORMAL_MODE);
    if (ret2) {
        cts_err("Wait fw to normal work failed %d", ret2);
    }
