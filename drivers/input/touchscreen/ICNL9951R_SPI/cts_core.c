...
int cts_enable_proximity_mode(struct cts_device *cts_dev)
{
...
static void cts_handle_proximity_event(int status)
{
	//cts_info("Handler proximity '%s-%d' event", status ? "Far" : "Near", status);
    if(status == CTS_PROXIMITY_LEVEL0){
        cts_info("Handler proximity far event: %d", status);
    }
    else if (status == CTS_PROXIMITY_LEVEL1 || 
            status == CTS_PROXIMITY_LEVEL2 ||
            status == CTS_PROXIMITY_LEVEL3) {
        cts_info("Handler proximity near event: %d", status);
    }
    else {
        cts_info("Handler proximity unknown event: %d", status);
    }
}
#endif

#ifdef CFG_CTS_PALM_DETECT
bool cts_is_pocket_enable(struct cts_device *cts_dev)
{
    struct cts_firmware_status *status = (struct cts_firmware_status *)
            &cts_dev->rtdata.firmware_status;
    return !!status->pocket;
}

int cts_enable_pocket_mode(struct cts_device *cts_dev)
{
    return cts_tcs_set_pocket_enable(cts_dev, 1);
}

int cts_disable_pocket_mode(struct cts_device *cts_dev)
{
    return cts_tcs_set_pocket_enable(cts_dev, 0);
}
#endif
...
int cts_irq_handler(struct cts_device *cts_dev)
{
    ...
    if ((cts_data->pm_suspend)) {
        ret = wait_for_completion_timeout(
                  &cts_data->pm_completion,
                  msecs_to_jiffies(700));
        if (!ret) {
            cts_err("Bus don't resume from pm(deep),timeout,skip irq");
            return IRQ_HANDLED;
        }
    }
#endif
    ...
    if (touch_info->finger_or_stylus == FINGER_ONLY &&
        cts_is_pocket_enable(cts_dev)){
        if(CFG_CTS_PALM_ID == touch_info->vkey_state) {
            update_palm_sensor_value(1);
            cts_dbg("irq_handler report down event for pocket mode");
            cts_plat_release_all_touch(cts_dev->pdata);
            return 0;
        } else if(0x00 == touch_info->vkey_state) {
            update_palm_sensor_value(0);
            cts_dbg("irq_handler report up event for pocket mode");
        }
    }
#endif
        ...
        if (touch_info->finger_or_stylus == FINGER_ONLY &&
            ...
            && touch_info->vkey_state == 0x00) {
                    cts_dev->rtdata.proximity_status = PS_FAR_AWAY;
                    cts_handle_proximity_event(CTS_PROXIMITY_LEVEL0);
                    cts_plat_release_all_touch(cts_dev->pdata);
                    tpd_notifier_call_chain(cts_dev->rtdata.proximity_status, NULL);
		            cts_info("cts proximity event %s", (cts_dev->rtdata.proximity_status ? "NEAR":"FAR"));
                    return 0;
            } else if (!cts_dev->rtdata.proximity_status){  //myguo 20240531
                    
                    if(touch_info->vkey_state == CTS_CMD_PROXIMITY_STATUS1) {
                        //    cts_dev->rtdata.proximity_num = 0;
                        cts_dev->rtdata.proximity_status = PS_NEAR;
                        cts_handle_proximity_event(CTS_PROXIMITY_LEVEL1);
                        cts_plat_release_all_touch(cts_dev->pdata);
                        tpd_notifier_call_chain(CTS_PROXIMITY_LEVEL1, NULL);
		                cts_info("cts proximity event %d - %x", CTS_PROXIMITY_LEVEL1, touch_info->vkey_state);
                        return 0;
                    }
                    else if (touch_info->vkey_state == CTS_CMD_PROXIMITY_STATUS2) {
                        //    cts_dev->rtdata.proximity_num = 0;
                        cts_dev->rtdata.proximity_status = PS_NEAR;
                        cts_handle_proximity_event(CTS_PROXIMITY_LEVEL2);
                        cts_plat_release_all_touch(cts_dev->pdata);
                        tpd_notifier_call_chain(CTS_PROXIMITY_LEVEL2, NULL);
		                cts_info("cts proximity event %d - %x", CTS_PROXIMITY_LEVEL2, touch_info->vkey_state);
                        return 0;
                    }
                    else if (touch_info->vkey_state == CTS_CMD_PROXIMITY_STATUS3) {
                        //    cts_dev->rtdata.proximity_num = 0;
                        cts_dev->rtdata.proximity_status = PS_NEAR;
                        cts_handle_proximity_event(CTS_PROXIMITY_LEVEL3);
                        cts_plat_release_all_touch(cts_dev->pdata);
                        tpd_notifier_call_chain(CTS_PROXIMITY_LEVEL3, NULL);
		                cts_info("cts proximity event %d - %x", CTS_PROXIMITY_LEVEL3, touch_info->vkey_state);
                        return 0;
                    }
                    else{
                        cts_info("cts recognized touch under proximity mode ");
                        /*cts_handle_proximity_event(touch_info->vkey_state);
                        cts_plat_release_all_touch(cts_dev->pdata);
                        cts_err("cts proximity unknown event %d", touch_info->vkey_state);
                        return 0;*/
                    }
...
extern struct chipone_ts_data *g_cts_data;
struct notifier_block cts_psenable_nb;
static int cts_ps_enable_notifier_callback(struct notifier_block *self,
				unsigned long event, void *data)
{

    cts_info("jyx %s: level = %d", __func__, (int)event);
	if(event == 1) {
		cts_enable_proximity_mode(&g_cts_data->cts_dev);
		cts_info("cts_enter_proximity_mode");
	}else if(event == 0) {
        cts_exit_proximity_mode(&g_cts_data->cts_dev);
		cts_info("cts_exit_proximity_mode");
	}
    panel_60hz_check_proximity_mode((int)event);
    panel_90hz_check_proximity_mode((int)event);
    panel_90hz_check_proximity_thinning_mode((int)event);
	return 0;
}
int cts_register_psenable_callback(void)
{
	pr_info("jyx %s\n", __func__);
	memset(&cts_psenable_nb, 0, sizeof(cts_psenable_nb));
	cts_psenable_nb.notifier_call = cts_ps_enable_notifier_callback;
	return ps_enable_register_notifier(&cts_psenable_nb);
}
...
normal_exit:
    ...
    if (cts_is_proximity_enable(cts_dev)) {
        ...
        cts_info("resume to recover proximity enable mode.");
    }
#endif

#if IS_ENABLED(CONFIG_TOUCHSCREEN_XIAOMI_TOUCHFEATURE)
	cts_info("palm_sensor_switch=%d", g_cts_data->palm_sensor_switch);
	if (g_cts_data->palm_sensor_switch) {
		cts_info("palm sensor on status, switch to on\n");
		cts_enable_pocket_mode(cts_dev);
	}
#endif
...
#ifdef CONFIG_CTS_TP_PROXIMITY
    if ((cts_data->esd_check_fail_cnt > 0) && (cts_data->esd_check_fail_cnt % 2) == 0 && 
        cts_is_proximity_enable(&cts_data->cts_dev)) {
        cts_enable_proximity_mode(&cts_data->cts_dev);
        cts_err("ESD protection read normal mode failed,re-enable proximity mode!");
    }
#endif

#ifdef CFG_CTS_PALM_DETECT
if (cts_data->esd_check_fail_cnt > 0 && (cts_data->esd_check_fail_cnt % 2) == 0 && 
		cts_is_pocket_enable(&cts_data->cts_dev)) {
	    	cts_enable_pocket_mode(&cts_data->cts_dev);
			cts_err("ESD protection read normal mode failed,re-enable palm pocket mode!");
	}
#endif
...
void cts_firmware_upgrade_work(struct work_struct *work)
{
	...
	xiaomi_touch_mievent_report_str(TOUCH_EVENT_FWLOAD_ERR, 0, "TpFirmwareLoadFail", "ICNL");
#endif
        return;
    }
