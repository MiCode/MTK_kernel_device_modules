...
void get_fw_name_match_lcm_name(void)
{
    struct device_node *lcm_name;
    unsigned long size = 0;
    struct tag_videolfb *videolfb_tag = NULL;
    char firmware_name_hkc_60hz[] = "cts_ts_fw_hkc_60hz.bin";
    char firmware_name_hkc_90hz[] = "cts_ts_fw_hkc_90hz.bin";
    char firmware_name_hkc_thin_90hz[] = "cts_ts_fw_hkc_thin_90hz.bin";
    ...
    if (lcm_name) {
		...
		if (!videolfb_tag)
			cts_err("Invalid lcm name\n");

		cts_info("read lcm name : %s", videolfb_tag->lcmname);
		if (strcmp("dsi_panel_n85_46_0f_0c_60_vdo_lcm_drv",
				videolfb_tag->lcmname) == 0 ) {
			strncpy(cts_firmware, firmware_name_hkc_60hz,
				sizeof(firmware_name_hkc_60hz));
		} else if (strcmp("dsi_panel_n85_46_0f_0c_90_vdo_lcm_drv",
				videolfb_tag->lcmname) == 0) {
			strncpy(cts_firmware, firmware_name_hkc_90hz,
				sizeof(firmware_name_hkc_90hz));
		} else if (strcmp("dsi_panel_n85_46_0f_0c_90_thinning_vdo_lcm_drv",
				videolfb_tag->lcmname) == 0) {
			strncpy(cts_firmware, firmware_name_hkc_thin_90hz,
				sizeof(firmware_name_hkc_thin_90hz));
		} else {
			cts_err("lcm name default\n");
		}
	} else {
		cts_err("Can't find node: chose in dts");
	}
}
...
#endif
    cts_info("fw name : %s ", CFG_CTS_FIRMWARE_FILENAME);
#ifdef CFG_CTS_FIRMWARE_IN_FS
