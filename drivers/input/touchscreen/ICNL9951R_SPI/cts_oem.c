...
#define OEM_OF_DEF_PROPVAL_RAWDATA_MIN      1300
#define OEM_OF_DEF_PROPVAL_RAWDATA_MAX      3800
#define OEM_OF_DEF_PROPVAL_NOISE_FRAMES     16
#define OEM_OF_DEF_PROPVAL_NOISE_MAX        190
#define OEM_OF_DEF_PROPVAL_OPEN_MIN         1600
#define OEM_OF_DEF_PROPVAL_SHORT_MIN        700
...
#define CTS_TP_SELFTEST_FILENAME           "tp_selftest"
...
static struct proc_dir_entry *proc_tp_selftest_entry;
...
#endif
int test_result;
static int openshort_proc_show(struct seq_file *m, void *v)
{
    if (test_result)
        seq_printf(m, "%s\n", "result=1");
    else
...
static int openshort_test_open(struct inode *inode, struct file *file)
{
    ...
    if (cts_data == NULL) {
        cts_err("Open openshort proc with cts_data = NULL");
        return -EFAULT;
    }

    oem_data = cts_data->oem_data;
    if (oem_data == NULL) {
        cts_err("Open openshort proc with oem_data = NULL");
        return -EFAULT;
    }

    cts_info("Open '/proc/" PROC_CTP_OPENSHORT_TEST_FILE "'");
    ...
    if(oem_data->reset_pin_test_result == 0 && oem_data->rawdata_test_result == 0
	&& oem_data->noise_test_result == 0 && oem_data->open_test_result == 0
	&& oem_data->short_test_result == 0 && oem_data->comp_cap_test_result == 0) {
           test_result = 1;
    } else {
           test_result = 0;
    }
    single_open(file, openshort_proc_show, NULL);
    return 0;
}
#if LINUX_VERSION_CODE < KERNEL_VERSION(5, 6, 0)
					...
					loff_t *ppos)
{
	...
	if (!strncmp("short", temp_buf, 5) || !strncmp("open", temp_buf, 4)) {
		...
		cts_info("tp_selftest echo incorrect\n");
	}
	return count;
}
				       ...
				       size_t count, loff_t *pos)
{
	...
	if (aftersale_selftest == 1) {
        ...
        retval = cts_tcs_get_fw_ver(cts_dev, &fw_version);
        if (retval) {
            cts_err("Factory test get firmware version failed");
            fw_version = 0;
        }
		cts_info("RETVAL is %d\n", retval);
		if (!retval)
			retval = SELF_TEST_OK;
		else
			retval = SELF_TEST_NG;
...
int proc_node_init(struct chipone_ts_data *cts_data)
{
    ...
    if (proc_touchscreen_dir == NULL) {
        cts_err("Create '/proc/"PROC_TOUCHSCREEN_FOLDER"' failed");
        return -EINVAL;
    } else {
        proc_ctp_openshort_test_file =
            proc_create_data(PROC_CTP_OPENSHORT_TEST_FILE,
            (S_IWUSR | S_IRUGO), proc_touchscreen_dir, 
                &proc_ctp_openshort_test_fops, cts_data);
        if (proc_ctp_openshort_test_file == NULL) {
            cts_err("Create '/proc/touchscreen/"
                PROC_CTP_OPENSHORT_TEST_FILE"' failed");
            return -EINVAL;
        } 
    }
	return 0;
...
int proc_node_deinit(struct chipone_ts_data *cts_data)
{
    ...
    oem_data = cts_data->oem_data;
    if (proc_ctp_openshort_test_file) {
        cts_info("  Remove '/proc/touchscreen/"PROC_CTP_OPENSHORT_TEST_FILE"'");
        remove_proc_entry(PROC_CTP_OPENSHORT_TEST_FILE, NULL);
    }
    if (proc_touchscreen_dir) {
...
int cts_oem_init(struct chipone_ts_data *cts_data)
{
    ...
    proc_tp_selftest_entry = proc_create(CTS_TP_SELFTEST_FILENAME,
            (S_IWUSR | S_IRUGO), NULL, &aftersale_test_ops);
    if (proc_tp_selftest_entry == NULL) {
        cts_err("Create '/proc/"CTS_TP_SELFTEST_FILENAME"' failed");
        ret = -EFAULT;
    }
...
int cts_oem_deinit(struct chipone_ts_data *cts_data)
{
    ...
    if (proc_tp_selftest_entry) {
        cts_info("  Remove '/proc/"CTS_TP_SELFTEST_FILENAME"'");
        remove_proc_entry(CTS_TP_SELFTEST_FILENAME, NULL);
    }
