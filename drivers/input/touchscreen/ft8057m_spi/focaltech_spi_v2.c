...
int fts_bus_configure(struct fts_ts_data *ts_data, u8 *buf, u32 size)
{
    ...
    if (!strcmp(touch_lcm_name, "dsi_panel_p15a_46_03_0b_dsc_vdo")) {
        return -1;
    }
...
//extern bool foc_ts_esd_resume_probe;
/*****************************************************************************
* TP Driver
*****************************************************************************/
...
static int __init fts_ts_spi_init(void)
{
    ...
    pr_info("touch_lcm_name=%s\n",touch_lcm_name);
    if ((strcmp(touch_lcm_name, "dsi_panel_p15a_46_03_0b_dsc_vdo")) == 0) {
        FTS_INFO("TP info: 2nd [Vendor]HKC [IC]Ft8057m");
    } else {
        return -1;
    }
    FTS_FUNC_ENTER();
