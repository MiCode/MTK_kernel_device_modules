...
extern void foc_tp_notify_lcd_esd(void);
extern bool foc_ts_need_do_esd_recovery;
...
static int fts_esdcheck_tp_reset(struct fts_ts_data *ts_data)
{
    ...
    fts_release_all_finger();
    fts_tp_state_recovery(ts_data);
    ret = get_chip_id(ts_data);
    if(ret){
        foc_ts_need_do_esd_recovery = true;
        foc_tp_notify_lcd_esd();
    }
    FTS_FUNC_EXIT();
    return 0;
}
...
static int esdcheck_algorithm(struct fts_ts_data *ts_data)
{
	...
	/*
    if(foc_ts_need_do_esd_recovery == true) {
        FTS_DEBUG("In lcd esd thread, skip tp esd");
        return 0;
    }
	*/
