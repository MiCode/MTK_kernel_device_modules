...
extern int fts_set_proximity_switch_3rd(uint8_t state);
...
int lct_fts_get_tpfwver_m(const char *cmd)
{
    ...
    if ((fwver == 0xFF) || (fwver == 0x00)){
        ...
        if ((strcmp(touch_lcm_name, "dsi_panel_p15a_46_03_0b_dsc_vdo")) == 0) {
        	//num_read_chars = snprintf(buf, 64, "[Vendor]HKC_2nd,[FW]0x%02x,[IC]ft8057m\n", fwver);
            snprintf(buf, 64, "[Vendor]HKC_2nd,[FW]0x%02x,[IC]ft8057m\n", fwver);
        } else {
        	FTS_INFO("FTS:unkown firmware name!!");
        	return -1;
        }
    }
    mutex_unlock(&input_dev->mutex);
...
EXPORT_SYMBOL(lct_fts_get_tpfwver_m);

int lct_fts_tp_selftest_callback_m(unsigned char cmd)
{
    int ret = 0;
    switch (cmd) {
...
EXPORT_SYMBOL(lct_fts_tp_selftest_callback_m);
...
#if LCT_TP_WORK_EN
int lct_fts_tp_work_callback_m(bool en)
{
    if (en){
...
int lct_fts_tp_proximity_switch_callback(bool flag)
{
    int rc = 0;
    if (flag) {
        rc = fts_set_proximity_switch_3rd(1);
        FTS_INFO("fts enable proximity mode,rc = %d\n",rc);
    } else {
        rc = fts_set_proximity_switch_3rd(0);
        FTS_INFO("fts disable proximity mode,rc = %d\n",rc);
        }
    return 0;
}
int lct_create_procfs_m(struct fts_ts_data *ts_data)
{
    int ret = 0;

    if ((strcmp(touch_lcm_name, "dsi_panel_p15a_46_03_0b_dsc_vdo")) == 0)
    {
        ret = init_lct_tp_info("[Vendor]HKC_2nd,[FW]unknow,[IC]ft8057m\n");
    }
    ...
    if(ret) {
        ...
        set_lct_tp_info_callback(lct_fts_get_tpfwver_m);
        FTS_INFO("create /proc/tp_info & /proc/tp_lockdown_info succeeded");
    }

    ret = init_lct_tp_selftest(lct_fts_tp_selftest_callback_m);
    if (ret < 0) {
...
#if LCT_TP_WORK_EN
    ret = init_lct_tp_work(lct_fts_tp_work_callback_m);
    if (ret < 0) {
...
int lct_remove_procfs_m(struct fts_ts_data *ts_data)
{
#if LCT_TP_WORK_EN
    uninit_lct_tp_work();
#endif
