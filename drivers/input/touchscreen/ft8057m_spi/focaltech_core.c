...
int tp_select_proximity_3rd = 1;
int tp_proximity_3rd(void)
{
    return tp_select_proximity_3rd;
}
EXPORT_SYMBOL(tp_proximity_3rd);
extern void set_prox_lcd_reset_gpio_keep_high_2nd(bool en);
...
static int fts_enter_proximity_mode(int mode)
{
    ...
    if (mode) {
        buf_value = 0x01;
        set_prox_lcd_reset_gpio_keep_high_2nd(true);
        tp_proximity_event = 1;
    } else {
        buf_value = 0x00;
        set_prox_lcd_reset_gpio_keep_high_2nd(false);
...
int fts_proximity_recovery_3rd(struct fts_ts_data *ts_data)
{
    int ret = 0;
...
int fts_set_proximity_switch_3rd(int proximity_switch)
{
    int ret = 0;
...
EXPORT_SYMBOL_GPL(fts_set_proximity_switch_3rd);

int fts_proximity_readdata_3rd(struct fts_ts_data *ts_data)
{
...
int fts_proximity_suspend_3rd(void)
{
...
int fts_proximity_resume_3rd(void)
{
    if (fts_proximity_data.mode == ENABLE)
        return 0;
    else
...
int fts_proximity_init_3rd(void)
{
    FTS_FUNC_ENTER();
...
#if FTS_PSENSOR_ENABLE
    fts_proximity_recovery_3rd(ts_data);
#endif
#if FTS_PSENSOR_EN
...
void fts_release_all_finger(void)
{
    ...
    ts_data->key_state = 0;
    memset(ts_data->int_cnt, 0, sizeof(ts_data->int_cnt));
...
static enum hrtimer_restart hrtimer_hander(struct hrtimer *timer)
{
    ...
    spin_lock_irqsave(&ts_data->insert_spin_lock, flags);
    if (atomic_read(&ts_data->insert_flag)) {
    ...
    spin_unlock_irqrestore(&ts_data->insert_spin_lock, flags);
    return HRTIMER_NORESTART;

}

static int fts_insert_init(struct fts_ts_data *ts_data)
{
    hrtimer_init(&ts_data->timer,CLOCK_MONOTONIC,HRTIMER_MODE_REL);
    ts_data->timer.function = hrtimer_hander;
    spin_lock_init(&ts_data->insert_spin_lock);
    return 0;
}

 void fts_insert_timerexit(struct fts_ts_data *ts_data)
{
    hrtimer_cancel(&ts_data->timer);
}

static int fts_inside_insert(struct fts_ts_data *ts_data, struct ts_event *events)
{
    static struct ts_event event_insert[FTS_MAX_POINTS_SUPPORT];
    static struct ts_event event_n1[FTS_MAX_POINTS_SUPPORT];
    static struct ts_event event_n2[FTS_MAX_POINTS_SUPPORT];
    int event_num = ts_data->touch_event_num;
    int i = 0;
    struct input_dev *input_dev = ts_data->input_dev;
    int *int_cnt = ts_data->int_cnt; //Count the number of times each point is down.
    int id = 0;
    ktime_t kt;
    bool touch_down_flag = 0;

    //mutex_lock(&ts_data->report_mutex);
    for (i = 0; i < event_num; i++) {
        id = events[i].id;
        if (id >= FTS_MAX_POINTS_SUPPORT) {
            FTS_INFO("invalid id %d", id);
            continue;
        }
        if (EVENT_DOWN(events[i].flag)) {
            if (int_cnt[id] <= 3) {  //Prevent overflow
                int_cnt[id]++;
                //FTS_INFO("id(%d),int_cnt(%d)", id, int_cnt[id]);
            } else {  //Insert only after 3 frames.
                if (abs(events[i].x - event_n1[id].x) < 5) {
                    event_insert[id].x = (int)((events[i].x * 5 + event_n1[id].x * 5) / 10);
                    //FTS_INFO("id(%d),xn1(%d),xn(%d), insertx(%d)", id, event_n1[id].x, events[i].x, event_insert[id].x);
                } else {
                    event_insert[id].x = (int)((375 * events[i].x + 750 * event_n1[id].x -
                        125 * event_n2[id].x) / 1000);
                    //FTS_INFO("id(%d),xn1(%d),xn2(%d),xn(%d),insertx(%d)", id, event_n1[id].x, event_n2[id].x, events[i].x, event_insert[id].x);
                }

                if (abs(events[i].y - event_n1[id].y) < 5) {
                    event_insert[id].y = (int)((events[i].y * 5 + event_n1[id].y * 5) / 10);
                    //FTS_INFO("id(%d),yn1(%d),yn(%d), inserty(%d)", id, event_n1[id].y, events[i].y, event_insert[id].y);
                } else {
                    event_insert[id].y = (int)((375 * events[i].y + 750 * event_n1[id].y -
                        125 * event_n2[id].y) / 1000);
                    //FTS_INFO("id(%d),yn1(%d),yn2(%d),yn(%d),inserty(%d)", id, event_n1[id].y, event_n2[id].y, events[i].y,event_insert[id].y);
                }

                event_insert[id].id = id;

                input_mt_slot(input_dev, event_insert[id].id);
                input_mt_report_slot_state(input_dev, MT_TOOL_FINGER, true);
#if FTS_REPORT_PRESSURE_EN
                //input_report_abs(input_dev, ABS_MT_PRESSURE, events[i].p);
#endif
                ...
                input_report_abs(input_dev, ABS_MT_POSITION_X, event_insert[id].x);
                input_report_abs(input_dev, ABS_MT_POSITION_Y, event_insert[id].y);
                touch_down_flag = 1;
            }

        }else {
            int_cnt[id] = 0;
            FTS_INFO("id(%d)cnt clear", id);
        }
        event_n2[id] = event_n1[id];
        event_n1[id] = events[i];
   }
    if (touch_down_flag) {
        input_report_key(input_dev, BTN_TOUCH, 1);
        input_sync(input_dev);
    }
    FTS_INFO("fts lcdrate_flag(%02x)", ts_data->lcdrate_flag);
    if(ts_data->lcdrate_flag == 0x0C) {
        kt = ktime_set(0,4170000);// 0s  1000000ns  = 1ms 定时
    }
    else if(ts_data->lcdrate_flag == 0x09) {
        kt = ktime_set(0,5560000);
    }
    else {
        kt = ktime_set(0,8330000);
    }
    hrtimer_start(&ts_data->timer,kt,HRTIMER_MODE_REL);
    return 0;
}
...
static int fts_input_report_touch(struct fts_ts_data *ts_data, u8 *touch_buf)
{
    ...
    atomic_set(&ts_data->insert_flag, test_bit(7, (unsigned long *)&touch_buf[0]));
    ts_data->lcdrate_flag = ((touch_buf[0] & 0x7C)>>2);
    FTS_INFO("fts lcdrate_flag(%02x)", ts_data->lcdrate_flag);
    //if (test_bit(3, (unsigned long *)&touch_buf[0])) {
    if (atomic_read(&ts_data->insert_flag)) {
        fts_inside_insert(ts_data, events);
    } else {
        memset(ts_data->int_cnt, 0, sizeof(ts_data->int_cnt));
...
#if FTS_PSENSOR_ENABLE
    if ((fts_proximity_readdata_3rd(ts_data) != 0) && (ts_data->suspended == 1) && (fts_proximity_data.mode == ENABLE)) {
        return 1;
    }
#endif
    return 0;
...
#if FTS_PSENSOR_EN
    if (ts_data->proximity_mode) {
        if (fts_proximity_readdata_3rd(ts_data) == FTS_RETVAL_IGNORE_TOUCHES)
            return TOUCH_IGNORE;
    }
#endif
...
/* LCT charge mode work */
#if FTS_M_TP_USB_PLUGIN
static int last_usb_online = -1;
static int current_usb_online = 0;

static void fts_usb_online_work(struct work_struct *work)
{
    int ret;
    ...
    if (ret) {
        pr_err("get usb psy online state failed\n");
        return;
    }
    //FTS_INFO("last_usb_online=%d, pval.intval=%d\n", last_usb_online, pval.intval);

    if (pval.intval != last_usb_online){
        current_usb_online = pval.intval;
        FTS_INFO("usb_online state has changed, current_online(%d), last_online(%d) \n", pval.intval, last_usb_online);
        fts_ts_usb_plugin_handler(pval.intval);
        last_usb_online = pval.intval;
    }
}

static int fts_power_usb_notifier_callback(struct notifier_block *nb, unsigned long ev, void *v)
{
    struct fts_ts_data *fts_data = container_of(nb, struct fts_ts_data, power_usb_notifier);
    if (ev != PSY_EVENT_PROP_CHANGED)
        return -1;

    queue_work(fts_data->ts_workqueue, &fts_data->online_work);

    return 0;
}

void lct_usb_remove_exit(struct fts_ts_data *ts_data)
{
    FTS_FUNC_ENTER();
    power_supply_unreg_notifier(&ts_data->power_usb_notifier);
    ts_data->power_usb_notifier.notifier_call = NULL;
    FTS_FUNC_EXIT();
}
#endif
...
static int fts_ts_resume(struct device *dev)
{
    ...
    if (tp_proximity_event) {
        fts_set_proximity_switch_3rd(ENABLE);
    }

    if ((!fts_proximity_resume_3rd()) && (fts_proximity_suspend_flag == true)) {
...
#if FTS_M_TP_USB_PLUGIN
    fts_ts_usb_plugin_handler(current_usb_online);
#endif
...
extern int lcm_shutdown_symbol;
...
int fts_ts_probe_entry(struct fts_ts_data *ts_data)
{
    ...
    #if FTS_M_TP_USB_PLUGIN
        INIT_WORK(&ts_data->online_work, fts_usb_online_work);
        ts_data->power_usb_notifier.notifier_call = fts_power_usb_notifier_callback;
        ret = power_supply_reg_notifier(&ts_data->power_usb_notifier);
        if (ret < 0) {
            pr_err("Couldn't register psy notifier ret = %d\n", ret);
            goto err_power_usb_notifier_register;
        }
    #endif
    }
...
//longcheer touch procfs
    ret = lct_create_procfs_m(ts_data);
    if (ret < 0) {
    ...
    ret = fts_insert_init(ts_data);
    if (ret) {
        FTS_ERROR("request timer failed");
        goto err_irq_req;
    }
...
#if FTS_M_TP_USB_PLUGIN
    power_supply_unreg_notifier(&ts_data->power_usb_notifier);
err_power_usb_notifier_register:
    ts_data->power_usb_notifier.notifier_call = NULL;
    cancel_work_sync(&ts_data->online_work);
#endif
...
int fts_ts_remove_entry(struct fts_ts_data *ts_data)
{
    ...
    cancel_work_sync(&ts_data->resume_work);
#if FTS_M_TP_USB_PLUGIN
    lct_usb_remove_exit(ts_data);
    cancel_work_sync(&ts_data->online_work);
#endif
    fts_notifier_callback_exit(ts_data);
    free_irq(ts_data->irq, ts_data);
    fts_insert_timerexit(ts_data);
