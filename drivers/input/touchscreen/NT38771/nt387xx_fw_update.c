...
return:
	n.a.
*******************************************************/
static int32_t nvt_write_firmware(const u8 *fwdata, size_t fwsize, uint8_t skip_update_ilm)
{
	uint32_t list = 0;
	char *name;
	...
	NVT_LOG("skip_update_ilm = %d\n", skip_update_ilm);

	memset(fwbuf, 0, (NVT_TRANSFER_LEN+1));

	for (list = 0; list < partition; list++) {
		/* fast download firmware to ignore ilm (list-0) */
		if ((skip_update_ilm == true) && (list == 0))
			continue;
...
static void nvt_read_bld_hw_crc(void)
{
	...
	if (ts->bld_multi_header >= BLDMHEADER_SUPPORT) {
		...
		NVT_ERR("0x%02x 0x%02x, crc_done = %d, ilm_crc_flag = %d, dlm_crc_flag = %d\n",
				buf[1],
				buf[2],
				(buf[2] >> 3) & 0x01,
				(buf[1] >> 4) & 0x01,
		...
		NVT_ERR("0x%02x, crc_done = %d, ilm_crc_flag = %d, dlm_crc_flag = %d\n",
				buf[1],
				(buf[1] >> 2) & 0x01,
				(buf[1] >> 0) & 0x01,
				(buf[1] >> 1) & 0x01);
...
static int32_t nvt_check_dma_hw_crc(void)
{
	...
	nvt_set_bld_crc_bank(ts->mmap->BLD_DES_ADDR, bin_map[0].SRAM_addr,
			...
			ts->mmap->G_DMA_CHECKSUM_ADDR, bin_map[0].crc);
...
return:
	Executive outcomes. 0---succeed. else---fail.
*******************************************************/
static int32_t nvt_download_firmware_hw_crc(uint8_t skip_update_ilm)
{
	uint8_t retry = 0;
	int32_t ret = 0;
	...
	while (1) {
		...
		if (skip_update_ilm == true) {
			if (nvt_check_dma_hw_crc()) {
				/* if ilm crc is uncorrect. disable skip_update_ilm flag */
				skip_update_ilm = false;
				NVT_ERR("After check dma hw crc, skip_update_ilm = %d\n", skip_update_ilm);
			}
		}
		...
		if (cascade_2nd_header_info) {
			...
			ret = nvt_write_firmware(fw_entry->data, fw_entry->size, skip_update_ilm);
			if (ret) {
...
/*P16 code for HQFEAT-89149 by xiongdejun at 2024/4/25 start*/
		...
		xiaomi_touch_mievent_report_str(TOUCH_EVENT_FWLOAD_ERR, 0, "TpFirmwareLoadFail", "novatek");
#endif
/*P16 code for HQFEAT-89149 by xiongdejun at 2024/4/25 end*/
	...
	ret = nvt_download_firmware_hw_crc(skip_update_ilm);
	if (ret) {
/*P16 code for BUGP16-9011 by liuyupei at 2025/7/24 start*/
		if (retry > 0) {
