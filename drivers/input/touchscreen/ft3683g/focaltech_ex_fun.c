...
extern int factory_result;
int focal_ini_name = 0;
int fts_factory_test(void)
{
    int ret = 0;
    char fwname_tm[FILE_NAME_LENGTH] = { "ft3683g_o16_tm.ini" };
    char fwname_viox[FILE_NAME_LENGTH] = { "ft3683g_o16_viox.ini" };
    struct fts_ts_data *ts_data = fts_data;
    ...
    if (goodix_focal_panel_name() == 1) {
        FTS_INFO("fwname:%s.\n", fwname_tm);
        focal_ini_name = 1;
    } else if (goodix_focal_panel_name() == 2) {
        FTS_INFO("fwname:%s.\n", fwname_viox);
        focal_ini_name = 2;
    }
    ...
    if (ret < 0) {
        ...
        if (focal_ini_name == 1) {
            ret = fts_test_entry(fwname_tm);
            test_result = factory_result;
            FTS_INFO("ft3683g tm openshort test pass test_result = %d", test_result);
        } else if (focal_ini_name == 2) {
            ret = fts_test_entry(fwname_viox);
            test_result = factory_result;
            FTS_INFO("ft3683g viox openshort ttest pass test_result = %d", test_result);
        }
