...
int tpd1_type_cap;
static int tpd1_flag;
static int tpd1_halt;
static int tpd1_eint_mode = 1;
static int tpd1_polling_time = 50;
static DECLARE_WAIT_QUEUE_HEAD(waiter);
static DEFINE_MUTEX(i2c_access);
unsigned int tpd1_rst_gpio_number;
unsigned int tpd1_int_gpio_number;
...
const u16 touch_key_array[] = tpd1_KEYS;
...
static int tpd1_keys_local[tpd1_KEY_COUNT] = tpd1_KEYS;
struct FIRMWARE_INFO
{
	...
	char fw_version[17]; //version hc add
...
#if (defined(tpd1_WARP_START) && defined(tpd1_WARP_END))
static int tpd1_wb_start_local[tpd1_WARP_CNT] = tpd1_WARP_START;
static int tpd1_wb_end_local[tpd1_WARP_CNT]   = tpd1_WARP_END;
#endif

#if (defined(tpd1_HAVE_CALIBRATION) && !defined(tpd1_CUSTOM_CALIBRATION))
/* static int tpd1_calmat_local[8]     = tpd1_CALIBRATION_MATRIX; */
/* static int tpd1_def_calmat_local[8] = tpd1_CALIBRATION_MATRIX; */
static int tpd1_def_calmat_local_normal[8]  = tpd1_CALIBRATION_MATRIX_ROTATION_NORMAL;
//static int tpd1_def_calmat_local_factory[8] = tpd1_CALIBRATION_MATRIX_ROTATION_FACTORY;
#endif

static irqreturn_t tpd1_interrupt_handler(int irq, void *dev_id);
static int touch_event_handler(void *unused);
static int tpd1_i2c_probe(struct i2c_client *client);
static int tpd1_i2c_detect(struct i2c_client *clietpd1_CALIBRATION_MATRIX_ROTATION_NORMALnt, struct i2c_board_info *info);
///static int tpd1_i2c_remove(struct i2c_client *client);//hc add
void tpd1_i2c_remove(struct i2c_client *client);
// static void tpd1_on(void);
// static void tpd1_off(void);
...
#define tpd1_ESD_CHECK_CIRCLE        2000
...
#ifdef GTP_PROXIMITY
#define tpd1_PROXIMITY_VALID_REG                   0x814E
#define tpd1_PROXIMITY_ENABLE_REG                  0x8042
static u8 tpd1_proximity_flag;
static u8 tpd1_proximity_detect = 1;	/* 0-->close ; 1--> far away */
#endif
...
static const struct i2c_device_id tpd1_i2c_id[] = { {"ti941_one", 0}, {} };
//static unsigned short force[] = { 0x1a, 0, 0, 0 };
static unsigned short force[] = { 0x1b, 0, 0, 0 };
static const unsigned short *const forces[] = { force, NULL };
/* static struct i2c_client_address_data addr_data = { .forces = forces,}; */
static const struct of_device_id ti941_dt_match[] = {
	{.compatible = "goodix,cap_touch_one"},
	{},
};

MODULE_DEVICE_TABLE(of, ti941_dt_match);
static struct i2c_driver tpd1_i2c_driver = {
	.driver = {
		   .of_match_table = of_match_ptr(ti941_dt_match),
		   },
	.probe = tpd1_i2c_probe,
	.remove = tpd1_i2c_remove,
	.detect = tpd1_i2c_detect,
	.driver.name = "ti941_one",
	.id_table = tpd1_i2c_id,
...
static int of_get_gt9xx_platform_data(struct device *dev)
{
	...
	//tpd1_rst_gpio_number = of_get_named_gpio(dev->of_node, "rst-gpio", 0);
	tpd1_int_gpio_number = of_get_named_gpio(dev->of_node, "int-gpio", 0);
	//tpd1_rst_gpio_number = 2  ;
	//tpd1_int_gpio_number = 1  ;
	/*
	* ret = of_property_read_u32(dev->of_node, "rst-gpio", &num);
	if (!ret)
		tpd1_rst_gpio_number = num;
	ret = of_property_read_u32(dev->of_node, "int-gpio", &num);
	if (!ret)
		tpd1_int_gpio_number = num;
  */
	//GTP_ERROR("g_vproc_en_gpio_number %d\n", tpd1_rst_gpio_number);
	GTP_DEBUG("g_vproc_vsel_gpio_number %d\n", tpd1_int_gpio_number);
	return 0;
}
#else
static int of_get_gt9xx_platform_data(struct device *dev)
...
static DEVICE_ATTR(tpd1_refresh_rate, 0664, show_refresh_rate, store_refresh_rate);

static struct device_attribute *gt9xx_attrs[] = {
	&dev_attr_tpd1_refresh_rate,
};
/* ============================================================= */

static int tpd1_i2c_detect(struct i2c_client *client, struct i2c_board_info *info)
{
	//strcpy(info->type, "mtk-tpd1");
	strcpy(info->type, "mtk-tpd1-0");
	return 0;
}

#ifdef GTP_PROXIMITY
static s32 tpd1_get_ps_value(void)
{
	return tpd1_proximity_detect;
}

static s32 tpd1_enable_ps(s32 enable)
{
	u8  state;
	s32 ret = -1;
	...
	if (enable) {
		state = 1;
		tpd1_proximity_flag = 1;
		GTP_INFO("tpd1 proximity function to be on.");
	} else {
		state = 0;
		tpd1_proximity_flag = 0;
		GTP_INFO("tpd1 proximity function to be off.");
	}

	ret = ti941_i2c_write_bytes(i2c_client_point, tpd1_PROXIMITY_ENABLE_REG, &state, 1);

	if (ret < 0) {
		GTP_ERROR("tpd1 %s proximity cmd failed.", state ? "enable" : "disable");
		return ret;
	}

	GTP_INFO("tpd1 proximity function %s success.", state ? "enable" : "disable");
	return 0;
}

s32 tpd1_ps_operate(void *self, u32 command, void *buff_in, s32 size_in,
		   void *buff_out, s32 size_out, s32 *actualout)
{
	...
	case SENSOR_GET_DATA:
		if ((buff_out == NULL) || (size_out < sizeof(hwm_sensor_data))) {
			...
			sensor_data->values[0] = tpd1_get_ps_value();
...
int ti941_i2c_dma_read_bytes(struct i2c_client *client,
	...
	if(gpDMABuf_va){
		...
		pr_info("[%s-%s-%d] ti941_i2c_dma_read_bytes  ti941 dma buf alloc failed", __FILE__, __func__, __LINE__);
...
int ti941_i2c_dma_write_bytes(struct i2c_client *client,
	...
	if(gpDMABuf_va){
		...
		pr_info("[%s-%s-%d ] ti941_i2c_dma_write_bytes dma buf alloc failed ", __FILE__, __func__, __LINE__);
	}
	}
	mutex_lock(&gsMutex_ti941_drv);
	 pr_info("ti941 22222222 \n");
...
// 		tpd1_info.pid |= ((buf[i + 2] - 0x30) << ((2 - i) * 4));
// 	}
...
void gtp_int_sync(void)
{
	...
	tpd1_gpio_output(GTP_INT_PORT, 0);
	//gpio_direction_output(tpd1_int_gpio_number, 0);
	msleep(50);
	tpd1_gpio_as_int(GTP_INT_PORT);
	//gpio_direction_input(tpd1_int_gpio_number);
}



static int tpd1_power_on(struct i2c_client *client)
{
	int ret = 0;
	int reset_count = 0;
...
reset_proc:
	// tpd1_gpio_output(GTP_INT_PORT, 0);
	//gpio_direction_output(tpd1_int_gpio_number, 0);
	// tpd1_gpio_output(GTP_RST_PORT, 0);
	//gpio_direction_output(tpd1_rst_gpio_number, 0);
	// msleep(20);
	/* power on, need confirm with SA */
	// GTP_ERROR("turn on power reg-vgp6\n");
	// ret = regulator_enable(tpd1->reg);
	// if (ret != 0)
	// 	tpd1_DMESG("Failed to enable reg-vgp6: %d\n", ret);

	GTP_ERROR("tpd1_int_gpio_number:0x%x, tpd1_rst_gpio_number:0x%x", tpd1_int_gpio_number, tpd1_rst_gpio_number);
    if(0)
    {
        ...
        if (ret < 0) {
            ...
            if (reset_count < tpd1_MAX_RESET_COUNT) {
                reset_count++;
                goto reset_proc;
            } else {
                goto out;
...
//EXPORT_SYMBOL(upgrade_send_instructions);
...
static int tpd1_irq_registration(void)
{
	struct device_node *node = NULL;
	int ret = 0;
	GTP_INFO("tpd1_irq_registration start  %s, %d", __func__, __LINE__);
	//node = of_find_compatible_node(NULL, NULL, "mediatek,cap_touch");
	node = of_find_compatible_node(NULL, NULL, "goodix,cap_touch_one");
	GTP_INFO("tpd1_irq_registration  node \r\n"  );
	if (node) {
		int irqflags;
		
		tpd1_int_gpio_number = of_get_named_gpio(node, "int-gpio", 0);
		
		if (tpd1_int_gpio_number < 0 ){
			pr_info("[%s-%s-%d] tpd1_int_gpio_number:%d ", __FILE__, __func__, __LINE__,tpd1_int_gpio_number);
		}

		gpio_direction_input(tpd1_int_gpio_number);
		touch_irq = gpio_to_irq(tpd1_int_gpio_number);
		...
		pr_info("tpd1_irq_registration1 touch_irq %d , irqflags=%d\r\n",touch_irq, irqflags);
               //touch_irq  = 1;   //interrupt  1  
               //int_type = 1 ;
			ret = request_irq(touch_irq, tpd1_interrupt_handler, irqflags, tpd1_DEVICE, NULL);
//					IRQF_TRIGGER_FALLING | IRQF_TRIGGER_RISING, tpd1_DEVICE, NULL);
			gtp_eint_trigger_type = irqflags;
			if (ret > 0)
				GTP_ERROR("tpd1 request_irq IRQ LINE NOT AVAILABLE!.");
		// pr_info("%s, irq type:%d\n", __func__, irqd_get_trigger_type(irq_get_irq_data(touch_irq)));
	} else {
		GTP_ERROR("[%s] tpd1 request_irq can not find touch eint device node!.", __func__);
	}
	return 0;
}


static s32 tpd1_i2c_probe(struct i2c_client *client)
{   
	...
	client->dev.coherent_dma_mask = DMA_BIT_MASK(32);
	pr_info("ti941 i2c:0x1b----tpd1_i2c_probe dma_read_bytes: client->dev.coherent_dma_mask = DMA_BIT_MASK(32) \n");
	i2c_client_point = client;
	ret = tpd1_power_on(client);
	if (ret < 0) {
		GTP_ERROR("I2C communication ERROR!");
		// goto out;
	}
	...
	tpd1_v_magnify_x = tpd1_VELOCITY_CUSTOM_X;
	tpd1_v_magnify_y = tpd1_VELOCITY_CUSTOM_Y;
	...
	thread = kthread_run(touch_event_handler, 0, tpd1_DEVICE);

	if (IS_ERR(thread)) {
		err = PTR_ERR(thread);
		GTP_ERROR(tpd1_DEVICE " failed to create kernel thread: %d", err);
		goto out;
	}

#if 0/* #ifdef CONFIG_GTP_HAVE_TOUCH_KEY */

	for (idx = 0; idx < tpd1_KEY_COUNT; idx++)
		input_set_capability(tpd1->dev, EV_KEY, touch_key_array[idx]);
	...
	//tpd1_gpio_as_int(GTP_INT_PORT);
	msleep(50);
        pr_info("tpd1_int_gpio_number %d \n", tpd1_int_gpio_number );
	tpd1_irq_registration();
	...
	obj_ps.sensor_operate = tpd1_ps_operate;
	...
	queue_delayed_work(gtp_esd_check_workqueue, &gtp_esd_check_work, tpd1_ESD_CHECK_CIRCLE);
#endif
	...
	return 0;
out:
	//gpio_free(tpd1_rst_gpio_number);
	//gpio_free(tpd1_int_gpio_number);
	return -1;
}

static irqreturn_t tpd1_interrupt_handler(int irq, void *dev_id)
{
	tpd1_DEBUG_PRINT_INT;
	tpd1_flag = 1;
...
static void force_reset_guitar(void)
{
	...
	/* Power off TP */
	// ret = regulator_disable(tpd1->reg);
	// if (ret != 0)
	// 	tpd1_DMESG("Failed to disable reg-vgp6: %d\n", ret);
...
#endif
static int tpd1_history_x = 0, tpd1_history_y;
static void tpd1_down(s32 x, s32 y, s32 size, s32 id)
{
	if ((!size) && (!id)) {
		input_report_abs(tpd1->dev, ABS_MT_PRESSURE, 100);
		input_report_abs(tpd1->dev, ABS_MT_TOUCH_MAJOR, 100);
	} else {
		input_report_abs(tpd1->dev, ABS_MT_PRESSURE, size);
		input_report_abs(tpd1->dev, ABS_MT_TOUCH_MAJOR, size);
		/* track id Start 0 */
		input_report_abs(tpd1->dev, ABS_MT_TRACKING_ID, id);
	}
...
static void tpd1_up(s32 x, s32 y, s32 id)
{
	pr_info("tpd1_up x = %d,y =%d ,id=%d",x,y,id);
...
static void tpd1_calibrate_driver(int *x, int *y)
{
	int tx;

	GTP_DEBUG("Call tpd1_calibrate of this driver ..\n");

	tx = ((tpd1_def_calmat[0] * (*x)) + (tpd1_def_calmat[1] * (*y)) + (tpd1_def_calmat[2])) >> 12;
	*y = ((tpd1_def_calmat[3] * (*x)) + (tpd1_def_calmat[4] * (*y)) + (tpd1_def_calmat[5])) >> 12;
	*x = tx;
}
#endif 
...
{	
	...
	do {
		...
		if (tpd1_eint_mode) {
			wait_event_interruptible(waiter, tpd1_flag != 0);
			tpd1_flag = 0;
		} else {
			msleep(tpd1_polling_time);
		}
				...
				if(ext_frame_len==0xFF){
					for(i=0;i<(2);i++){
						...
						if((touch_data[i][1]&0x40)!=0){
							pretouch_status[i]=1;
							tpd1_down(touch_data[i][2], touch_data[i][3], 0,touch_data[i][0]);
						}
						else if((touch_data[i][1]&0x20)!=0){
							if(pretouch_status[i]==1){
								pretouch_status[i]=0;
								tpd1_up(0,0,touch_data[i][0]);
							}
						}
					}
				}
					...
					if(ext_frame_len==0xFF){
						for(i=0;i<3;i++){
							...
							if((touch_data[i+2][1]&0x40)!=0){
								...
								tpd1_down(touch_data[i+2][2], touch_data[i+2][3], 0,touch_data[i+2][0]);
							}
							else if((touch_data[i+2][1]&0x20)!=0){
								if(pretouch_status[2+i]==1){
									pretouch_status[2+i]=0;
									tpd1_up(0,0,touch_data[i+2][0]);
								}
							}
						}
						if (tpd1 != NULL && tpd1->dev != NULL)
...
if(0){
		...
		if (tpd1_proximity_flag == 1) {
			...
			if (proximity_status & 0x60) {
				...
				tpd1_proximity_detect = 0;
				/* sensor_data.values[0] = 0; */
			} else {
				tpd1_proximity_detect = 1;
				/* sensor_data.values[0] = 1; */
			}

			/* get raw data */
			GTP_DEBUG(" ps change");
			GTP_DEBUG("PROXIMITY STATUS:0x%02X", tpd1_proximity_detect);
		...
		if (key_value || pre_key) {
			for (i = 0; i < tpd1_KEY_COUNT; i++) {
				/* input_report_key(tpd1->dev, touch_key_array[i], key_value & (0x01 << i)); */
		...
		if (touch_num) {
			for (i = 0; i < touch_num; i++) {
				...
				input_x = tpd1_WARP_X(abs_x_max, input_x);
				input_y = tpd1_WARP_Y(abs_y_max, input_y);
				//tpd1_calibrate_driver(&input_x, &input_y);   // jhu modified 20190925
                                if(input_x<1025)
				...
				tpd1_down(input_x, input_y, input_w, id);
			}
		} else if (pre_touch) {
		...
		pre_touch = touch_num;
		/* input_report_key(tpd1->dev, BTN_TOUCH, (touch_num || key_value)); */
...
static int tpd1_local_init(void)
{   
	// int retval;

    pr_info("ti941 touch panel tpd1_local_init 2023-05-25 \r\n ");
	// tpd1->reg = regulator_get(tpd1->tpd1_dev, "vtouch");
	// retval = regulator_set_voltage(tpd1->reg, 2800000, 3300000);
	// if (retval != 0) {
	// 	tpd1_DMESG("Failed to set reg-vgp6 voltage: %d\n", retval);
	// 	return -1;
	// }
	//msleep(5000);
	if (i2c_add_driver(&tpd1_i2c_driver) != 0) {
		GTP_INFO("tpd1_local_init unable to add i2c driver.");
		return -1;
	}

	if (tpd1_load_status == 0)	{
		/* if(tpd1_load_status == 0) // disable auto load touch driver for linux3.0 porting */
		GTP_INFO("add error touch panel driver.");
		i2c_del_driver(&tpd1_i2c_driver);
		return -1;
	}
	input_set_abs_params(tpd1->dev, ABS_MT_TRACKING_ID, 0, (GTP_MAX_TOUCH-1), 0, 0);
	if (tpd1_dts_data.use_tpd1_button) {
		/*initialize tpd1 button data*/
		tpd1_button_setting(tpd1_dts_data.tpd1_key_num, tpd1_dts_data.tpd1_key_local,
		tpd1_dts_data.tpd1_key_dim_local);
	}
	tpd1_button_setting_ti941(tpd1_KEY_COUNT,tpd1_keys_local);
	// tpd1_button_init();

#if (defined(tpd1_WARP_START) && defined(tpd1_WARP_END))
	tpd1_DO_WARP = 1;
	memcpy(tpd1_wb_start, tpd1_wb_start_local, tpd1_WARP_CNT * 4);
	memcpy(tpd1_wb_end, tpd1_wb_start_local, tpd1_WARP_CNT * 4);
#endif

#if (defined(tpd1_HAVE_CALIBRATION) && !defined(tpd1_CUSTOM_CALIBRATION))
	/* memcpy(tpd1_calmat, tpd1_def_calmat_local, 8 * 4); */
	/* memcpy(tpd1_def_calmat, tpd1_def_calmat_local, 8 * 4); */

#ifdef CONFIG_MTK_BOOT
	if (get_boot_mode() == FACTORY_BOOT) {
		tpd1_DEBUG("Factory mode is detected!\n");
		memcpy(tpd1_calmat, tpd1_def_calmat_local_factory, 8 * 4);
		memcpy(tpd1_def_calmat, tpd1_def_calmat_local_factory, 8 * 4);
	} else {
#endif
		tpd1_DEBUG("Normal mode is detected!\n");
		memcpy(tpd1_calmat, tpd1_def_calmat_local_normal, 8 * 4);
		memcpy(tpd1_def_calmat, tpd1_def_calmat_local_normal, 8 * 4);
#ifdef CONFIG_MTK_BOOT
	}
#endif
#endif
	...
	/* set vendor string */
	tpd1->dev->id.vendor = 0x00;
	tpd1->dev->id.product = tpd1_info.pid;
	tpd1->dev->id.version = tpd1_info.vid;
...
// 	while (retry++ < 10) {
// 		tpd1_gpio_output(GTP_INT_PORT, 1);
// 		//gpio_direction_output(tpd1_int_gpio_number, 1);
// 		msleep(20);
// 		tpd1_gpio_output(GTP_INT_PORT, 0);
// 		//gpio_direction_output(tpd1_int_gpio_number, 0);
// 		msleep(20);
...
static void tpd1_suspend(struct device *h)
{
	// s32 ret = -1;
...
static void tpd1_resume(struct device *h)
{
	// s32 ret = -1;
...
// 	while (retry++ < 5) {
// 		ret = tpd1_power_on(i2c_client_point);
...
// 	tpd1_halt = 0;
// }
static struct tpd1_driver_t tpd1_device_driver = {

	.tpd1_device_name = "ti941",
	.tpd1_local_init = tpd1_local_init,
	.suspend = tpd1_suspend,
	.resume = tpd1_resume,
	.attrs = {
		.attr = gt9xx_attrs,
...
//static int __init tpd1_driver_init(void)
void tpd1_driver_init_one(void)
{   
	GTP_INFO("MediaTek ti941 touch panel driver init");
	pr_info("ti941 touch panel tpd1_driver_init \r\n ");
	tpd1_get_dts_info();
	if (tpd1_driver_add(&tpd1_device_driver) < 0){
		GTP_INFO("add generic driver failed");
	pr_info("ti941 tpd1_driver_add device failed \r\n ");
	}
	//return 0;
}

/* should never be called */
//static void __exit tpd1_driver_exit(void)
void tpd1_driver_exit_one(void)
{
	GTP_INFO("MediaTek ti941 touch panel driver exit");
	/* input_unregister_device(tpd1->dev); */
	tpd1_driver_remove(&tpd1_device_driver);
}
EXPORT_SYMBOL_GPL(tpd1_driver_init_one);
EXPORT_SYMBOL_GPL(tpd1_driver_exit_one);
//module_init(tpd1_driver_init);
//module_exit(tpd1_driver_exit);
