			...
			unsigned long event, void *v)
{
	...
	TPD_DEBUG("%s\n", __func__);
	pr_info("tpd_fb_notifier_callback event=[%lu]\n", event);
...
/*for some projects, we do not use mtk framebuffer*/
	...
	if (mtk_disp_notifier_register("Touch_driver", &tpd_fb_notifier))
		TPD_DMESG("register mtk_disp_notifier fail!\n");
	...
	input_abs_set_res(tpd->dev, ABS_MT_POSITION_X, TPD_RES_X);
	input_abs_set_res(tpd->dev, ABS_MT_POSITION_Y, TPD_RES_Y);
