...
#define HAPTIC_HV_DRIVER_VERSION	"v1.9.0.10"
...
static int dual_load_vib_success = 0;
#endif
#ifdef TEST_RTP
...
static int judge_cali_range(struct aw_haptic *aw_haptic)
{
	...
	if (aw_haptic->f0 < f0_cali_min) {
		...
		XM_HAP_F0_CAL_EXCEPTION(aw_haptic->f0, aw_haptic->info.f0_pre, "F0 Calibration failed, use default val");
			       ...
			       struct ff_effect *old)
{
	...
	if (aw_haptic->effect_type == FF_CONSTANT) {
		...
		/*modify for fix com.tencent.tmgp.pubgmhd High-quality vibration is incomplete */
		aw_haptic->index = aw_haptic->ram.ram_num -10;
		...
		/*modify for fix com.tencent.tmgp.pubgmhd High-quality vibration is incomplete */
		if (aw_haptic->index < aw_haptic->ram.ram_num -10) {
			...
			atomic_set(&aw_haptic->is_in_tiktap_mode, 0);
			/*modify for fix com.tencent.tmgp.pubgmhd High-quality vibration is incomplete */
			if (aw_haptic->index >= aw_haptic->ram.ram_num -10) {
...
static void tiktap_enable_ram_play(struct aw_haptic *aw_haptic, bool en_flag)
{
	if(en_flag){
	...
	aw8693x_broadcast_config(aw_haptic, en_flag, AW_RAM_MODE);
}
...
static void vibrator_work_routine(struct work_struct *work)
{
	...
	aw_info("index = %d state=%d activate_mode = %d duration = %d\n",
		aw_haptic->index, aw_haptic->state, aw_haptic->activate_mode,
		aw_haptic->duration);
	mutex_lock(&aw_haptic->lock);
	tiktap_enable_ram_play(aw_haptic, false);
	tiktap_enable_ram_play(aw_haptic, true);
	/* Enter standby mode */
	hrtimer_cancel(&aw_haptic->timer);
	aw_haptic->func->play_stop(aw_haptic);
	if (aw_haptic->state) {
		...
		aw_haptic->func->upload_lra(aw_haptic, AW_F0_CALI_LRA);
		if (aw_haptic->activate_mode == AW_RAM_MODE) {
			if(!aw_haptic->tiktap_ram_enable){
				aw_haptic_effect_strength(aw_haptic);
				ram_vbat_comp(aw_haptic, false,aw_haptic->gain);
			}
		...
		if (aw_haptic->is_custom_wave == 0) {
			...
			if ((buf_len < period_size)
				...
				if (buf_len < period_size)
					...
					aw_err("custom rtp play suspend!");
				aw_haptic->rtp_cnt = 0;
				aw_haptic->func->set_rtp_aei(aw_haptic, false);
				break;
			}
...
{
// 2025/12/25 在震动播放的时候，保证芯片的采样率为12K
	mutex_lock(&right->dual_lock);
	if (right->i2c->addr == AW8693X_BROADCAST_ADDR) {
		right->i2c->addr = right->record_i2c_addr;
		aw_haptic->func->set_pwm(right, AW_PWM_12K);
	...
	do {
		if (aw_haptic->curr_buf->status == MMAP_BUF_DATA_VALID) {
			...
			if((retry-- <= 0) || !atomic_read(&aw_haptic->richtap_rtp_mode)) {
				atomic_set(&aw_haptic->richtap_rtp_mode, false);
				aw_info("aac richtap invalid data or stop retry %d", retry);
				return;
			} else {
...
static bool richtap_rtp_start(struct aw_haptic *aw_haptic)
{
   ...
   while (cnt) {
		...
		if (!atomic_read(&aw_haptic->richtap_rtp_mode)) {
			aw_info("exit richtap_rtp_work, come stop");
			rtp_work_flag = false;
			break;
		}
...
static long richtap_file_unlocked_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd)
	{
		...
		case TIKTAP_RTP_MIX_MODE:
			...
			atomic_set(&aw_haptic->richtap_rtp_mode, false);
			atomic_set(&aw_haptic->is_in_tiktap_mode, 1);
			richtap_clean_buf(aw_haptic, MMAP_BUF_DATA_INVALID);
			...
			if (aw_haptic->is_custom_wave == 1) {
				...
				if ((buf_len < period_size) ||
					...
					if(buf_len < period_size)
						...
						aw_info("%s: custom rtp update suspend\n",__func__);
					aw_haptic->func->set_rtp_aei(aw_haptic, false);
					aw_haptic->rtp_cnt = 0;
...
static void brightness_set(struct led_classdev *cdev, enum led_brightness level)
{
	...
	aw_haptic->index = level;
	aw_haptic->tiktap_ram_enable = true;
	aw_haptic->activate_mode = AW_RAM_MODE;
	mutex_unlock(&aw_haptic->lock);
	queue_work(aw_haptic->work_queue, &aw_haptic->vibrator_work);
}
			      ...
			      const char *buf, size_t count)
{
	...
	aw_haptic->tiktap_ram_enable = false;
	mutex_unlock(&aw_haptic->lock);
	queue_work(aw_haptic->work_queue, &aw_haptic->vibrator_work);
...
static ssize_t aw_haptic_ram_init_check_show(struct device *dev,
							struct device_attribute *attr,
							char *buf)
{
	cdev_t *cdev = dev_get_drvdata(dev);
	...
	if (left->ram_init && right->ram_init ){
		...
		aw_info("ram_init = %d",aw_haptic->ram_init);
		len += snprintf(buf + len, PAGE_SIZE - len, "%d\n", 0);
	}
...
static DEVICE_ATTR(vmax_mv, S_IWUSR | S_IRUGO, vmax_mv_show,
		   vmax_mv_store);
static DEVICE_ATTR(ram_init_check, S_IRUGO, aw_haptic_ram_init_check_show, NULL);
#ifdef AW_DOUBLE
...
static int aw_i2c_probe(struct i2c_client *i2c)
{
	...
	if (of_device_is_compatible(np, "awinic,haptic_hv_l")) {
		if (dual_load_vib_success != 1) {
			pr_info("right haptic driver probe failed,exit all probe");
			return -ENODEV;
		}
	}
