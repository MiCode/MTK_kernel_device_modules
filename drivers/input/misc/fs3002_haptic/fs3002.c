...
static int fs3002_haptic_play_mode(struct fs3002 *fs3002, unsigned char play_mode);
...
static void fs3002_long_vibrate_work_routine(struct work_struct *work);
...
void fs3002_haptic_rtp_set_auto_env(struct fs3002 *fs3002, bool b_aotu_env)
{
	sprintf(str, "%s, enter,auto env=%d\n", __func__,b_aotu_env);
	fs3002_debug_message(fs3002, str);
...
static void fs3002_rtp_work_hapstream(struct work_struct *work)
{
	...
	while (true && count--)
	{
		...
		{
			fs3002_haptic_play_mode(fs3002, FS3002_HAPTIC_RTP_MODE);
...
static int fs3002_haptic_read_lra_f0(struct fs3002 *fs3002)
{
	...
	if (!f0_reg) 
	{
		...
		XM_HAP_F0_CAL_EXCEPTION(fs3002->f0, fs3002->dts_info.fs3002_f0_ref, "F0 Calibration failed");
		return -1;
	} 
	else 
	{
...
//okok
static int fs3002_haptic_cont_config(struct fs3002 *fs3002)
{
	pr_info("enter\n");

	//work mode
	fs3002_haptic_play_mode(fs3002, FS3002_HAPTIC_CONT_MODE);
	//cont play go
	fs3002_haptic_play_go(fs3002);
	return 0;
}
...
static int fs3002_haptic_get_lra_resistance(struct fs3002 *fs3002)
{
	...
	{
		usleep_range(fs3002->lra_test_sleep_time, fs3002->lra_test_sleep_time+10);//usleep_range(100, 110); //Wait > 100 us(100);
	}
	...
	{
		fs3002_i2c_write(fs3002, FS3002_SARCTRL, 0x90);//0xBB = 0x90		// SAR capture data
		if(fs3002->lra_test_sleep_time != 0)
		{
			usleep_range(fs3002->lra_test_sleep_time, fs3002->lra_test_sleep_time+10);//usleep_range(10, 20); //Wait > 10 us
		}
	...
	fs3002->lra = (fs3002->offset - ui_BCBD) * 428600 / (1024 * get_d2sgain(uc_0x2A));
...
static int fs3002_haptic_play_mode(struct fs3002 *fs3002, unsigned char play_mode)
{
	...
	switch (play_mode) 
	{
		...
		case FS3002_HAPTIC_RTP_MODE:
			...
			fs3002_i2c_write_bits(fs3002, FS3002_FDETCTRL,1,4,4);
			// set auto env
			fs3002_haptic_rtp_set_auto_env(fs3002,true);
			// set pwm
			fs3002_haptic_set_pwm(fs3002, fs3002->dts_info.fs3002_play_rtp_srate);
			break;
...
static int fs3002_haptic_get_ram_num_and_max(struct fs3002 *fs3002)
{
	...
	else
	{
		...
		else
		{
			...
			{
				ram_data[1] = fs3002_i2c_read(fs3002, FS3002_RAMRDATA, &reg_val);
				ram_data[2] = fs3002_i2c_read(fs3002, FS3002_RAMRDATA, &reg_val);
				if(i == 0)
				{
...
static int fs3002_haptic_f0_calibration(struct fs3002 *fs3002)
{
	...
	if(fs3002->dts_info.fs3002_f0_cali_mode == FS3002_F0_CALI_MODE_AUTO)
	{
		...
		fs3002_haptic_play_mode(fs3002, FS3002_HAPTIC_F0_CALI_MODE);
		fs3002_haptic_cont_get_f0(fs3002);

		fs3002_haptic_play_mode(fs3002, FS3002_HAPTIC_STANDBY_MODE);
	...
	{
		// enter DETECT_MODE
		fs3002_haptic_play_mode(fs3002, FS3002_HAPTIC_F0_DETECT_MODE);
...
static enum hrtimer_restart fs3002_vibrator_timer_func(struct hrtimer *timer)
{
	...
	fs3002->state = 0;
	queue_work(fs3002->work_queue, &fs3002->long_vibrate_work);
...
*/
	...
	INIT_WORK(&fs3002->long_vibrate_work, fs3002_long_vibrate_work_routine);
...
static void fs3002_haptic_audio_work_routine(struct work_struct *work)
{
	...
	if (fs3002->haptic_audio.ctr.cmd == FS3002_HAPTIC_CMD_ENABLE) 
	{
		if (fs3002->haptic_audio.ctr.play == FS3002_HAPTIC_PLAY_ENABLE) 
		{
			...
			mutex_lock(&fs3002->lock);
			fs3002_haptic_stop(fs3002);
			fs3002_haptic_play_mode(fs3002, FS3002_HAPTIC_RAM_MODE);
...
static void fs3002_haptic_diagnostic_sequence(struct fs3002 *fs3002)
{
	...
	pr_info( "enter\n");
	fs3002_haptic_enable_key(fs3002,true);//enable key
	fs3002_i2c_write_bits(fs3002,FS3002_INTCTRL,0,7,7);// Disable interrupt output
	...
	usleep_range(1000, 2000);//usleep_range(100, 150);//Wait > 100 us

	
	
	fs3002_i2c_write(fs3002, FS3002_SARCTRL, 0x82);// Enable SAR test mode, SAR ADC input =0.2*OUTP
	fs3002_i2c_write(fs3002, FS3002_SARCTRL, 0x92);// SAR capture data
	...
	fs3002_i2c_write(fs3002, FS3002_SARCTRL, 0x93);// SAR capture data
	fs3002_i2c_write(fs3002, FS3002_SARCTRL, 0x83);
	usleep_range(1000, 2000);//usleep_range(10, 20); //Wait > 10 us
...
static void fs3002_haptic_set_diagnostic_reg_to_default_value(struct fs3002 *fs3002)
{
	...
	fs3002_i2c_write_bits(fs3002,FS3002_INTCTRL,1,7,7);// enable interrupt output
	fs3002_haptic_enable_key(fs3002,false);//disable key
}
...
static int fs3002_haptic_offset_calibration(struct fs3002 *fs3002)
{
	...
	//outp
	fs3002_i2c_write(fs3002, FS3002_SARCTRL, 0x92);//0xBB = 0x92		// SAR capture data
	usleep_range(1000, 2000);//usleep_range(10, 20); //Wait > 5 us
	fs3002_i2c_read(fs3002, FS3002_SARTS_L, &uc_0xBC);//0xBC//low 8 offset value
	fs3002_i2c_read(fs3002, FS3002_SARTS_L, &uc_0xBC);//0xBC//low 8 offset value
	uc_0xBD = fs3002_i2c_read_bits(fs3002, FS3002_SARTS_H, 1,0);//0xBD//high 2 offset value
	...
	//outn
	fs3002_i2c_write(fs3002, FS3002_SARCTRL, 0x93);//0xBB = 0x93		// SAR capture data
	usleep_range(1000, 2000);//usleep_range(10, 20); //Wait > 5 us
	fs3002_i2c_read(fs3002, FS3002_SARTS_L, &uc_0xBC);//0xBC//low 8 offset value
	fs3002_i2c_read(fs3002, FS3002_SARTS_L, &uc_0xBC);//0xBC//low 8 offset value
	uc_0xBD = fs3002_i2c_read_bits(fs3002, FS3002_SARTS_H, 1,0);//0xBD//high 2 offset value
...
static int fs3002_haptic_play_repeat_seq(struct fs3002 *fs3002,unsigned char flag)
{
	...
	if (flag) 
	{
		fs3002_haptic_play_mode(fs3002, FS3002_HAPTIC_RAM_LOOP_MODE);
		fs3002_haptic_play_go(fs3002);
	}
	return 0;
}
...
static ssize_t fs3002_cont_store(struct device *dev,struct device_attribute *attr,const char *buf, size_t count)
{
	...
	fs3002_haptic_stop(fs3002);
	if (val)
		fs3002_haptic_cont_config(fs3002);
	return count;
}
...
int fs3002_parse_dt(struct fs3002 *fs3002, struct device *dev, struct device_node *np)
{
	...
	if (val != 0)
	{
		...
		XM_HAP_F0_PROTECT_EXCEPTION(0, "FS：haptics_get_closeloop_lra_period");
