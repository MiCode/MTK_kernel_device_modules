...
static int foursemi_i2c_probe(struct i2c_client *i2c)//, const struct i2c_device_id *id)
{
	...
	if (!foursemi->enable_pin_control) 
	{
		...
		if (gpio_is_valid(foursemi->reset_gpio)) 
		{
			...
			if (ret) 
			{
				...
				XM_HAP_REGISTER_EXCEPTION("Request reset_gpio", "devm_gpio_request_one");
	...
	if (gpio_is_valid(foursemi->irq_gpio)) 
	{
		...
		if (ret) 
		{
			...
			XM_HAP_REGISTER_EXCEPTION("Request irq_gpio", "devm_gpio_request_one");
	...
	if (ret < 0) 
	{
		...
		XM_HAP_REGISTER_EXCEPTION("Parse chipid", "foursemi_parse_chipid");
		goto err_id;
	}
	...
	if (foursemi->name == FS3002_A1 || foursemi->name == FS3002_A2 || foursemi->name == FS3002_A3) 
	{
		...
		foursemi->fs3002->fs3002_debug_enable = 1;//zzzz
		if (np) 
		{
		...
		foursemi->fs3002->work_queue = create_singlethread_workqueue("fs3002_long_vibrate_work_queue");
		if (!foursemi->fs3002->work_queue) 
		{
			pr_err("%s: Error creating fs3002_long_vibrate_work_queue\n", FSERROR);
		...
		if (rc < 0) 
		{
			XM_HAP_REGISTER_EXCEPTION("Input create", "input_ff_create");
