...
struct rb *grb;
atomic_t block_flag = ATOMIC_INIT(0);
#ifdef A2HAPTIC_SUPPORT
int32_t g_head;
EXPORT_SYMBOL_GPL(g_head);
#endif
...
int write_rb(const char *data, int32_t size)
{
	...
	while ((grb->aval_size < size) && (!atomic_read(&grb->exit))) {
		...
		atomic_set(&block_flag, 1);
		if(!hrtimer_active(&grb->timer)){
			hrtimer_start(&grb->timer, ktime_set(2,0), HRTIMER_MODE_REL);
                }
		ret = wait_event_interruptible(grb->wait_q, atomic_read(&grb->buf_condition) == 1);
		if (ret == -ERESTARTSYS) {
			 pr_err("%s wake up by signal return erro\n", __func__);
			 atomic_set(&block_flag, 0);
			 hrtimer_cancel(&grb->timer);
			 return ret;
		}
		hrtimer_cancel(&grb->timer);
		atomic_set(&block_flag, 0);
...
int read_rb(char *data, int32_t size)
{
	...
	read_bytes = XM_MIN (size, filled_size);
	if (size > filled_size)
...
static enum hrtimer_restart ringbuffer_timer_func(struct hrtimer *timer)
{
	struct rb *grb = container_of(timer, struct rb, timer);

	pr_debug("Dynamiceffect write thread timeout,atomic_read(&grb->exit) = %d,atomic_read(&grb->buf_condition) = %d",
				atomic_read(&grb->exit),atomic_read(&grb->buf_condition));
	rb_force_exit();
...
int create_rb(void)
{
	...
	hrtimer_init(&grb->timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	grb->timer.function = ringbuffer_timer_func;
