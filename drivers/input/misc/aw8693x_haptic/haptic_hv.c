			       ...
			       struct ff_effect *old)
{
	...
	case FF_CONSTANT:
		...
		break;
	case FF_PERIODIC:
		aw_info("%s: effect_type is  FF_PERIODIC!\n", __func__);
		...
		aw_info("%s: aw86938->effect_id =%d\n",__func__, aw_haptic->index);
		aw_info("aw_haptic->info.effect_max = %d", aw_haptic->info.effect_max);
		play->vmax_mv = effect->u.periodic.magnitude; /*vmax level*/
		if (aw_haptic->index < 0 || aw_haptic->index > aw_haptic->info.effect_max) {
			mutex_unlock(&aw_haptic->lock);
			return 0;
		}
		...
		if (aw_haptic->index < aw_haptic->ram.ram_num) {
			aw_info("%s: effect_type is  RAM!\n", __func__);
			aw_haptic->activate_mode = AW_RAM_MODE;
			aw_info("%s: aw_haptic->index=%d , aw_haptic->activate_mode = %d\n",__func__, aw_haptic->index,aw_haptic->activate_mode);
			/*second data*/
			data[1] = 0;
			/*millisecond data*/
			data[2] = 20;
		...
		if (aw_haptic->index == CUSTOME_WAVE_ID) {
			aw_haptic->activate_mode = AW_RTP_MODE;
			aw_info("%s: CUSTOME_WAVE_ID,aw_haptic->index=%d , aw_haptic->activate_mode = %d\n",
				__func__, aw_haptic->index,
...
int input_playback(struct input_dev *dev, int effect_id, int val)
{
	...
	if(val <= 0)
		aw_haptic->state = 0;
	hrtimer_cancel(&aw_haptic->timer);
	aw_info("aw_haptic->effect_type = %d", aw_haptic->effect_type);
	if (aw_haptic->effect_type == FF_CONSTANT &&
		...
		aw_err("Unsupported effect type: %d", aw_haptic->effect_type);
		/*other mode */
	}
	aw_info("aw_haptic->state = %d", aw_haptic->state);
	return 0;
}
...
static int rtp_play(struct aw_haptic *aw_haptic)
{
	...
	while ((!aw_haptic->func->rtp_get_fifo_afs(aw_haptic))
		...
		aw_info("aw_haptic->is_custom_wave = %d", aw_haptic->is_custom_wave);
		if (aw_haptic->is_custom_wave == 0) {
			...
			aw_info("custom rtp play\n");
			buf_len = read_rb(aw_rtp->data,  period_size);
			aw_info("custom rtp buf_len = %d", buf_len);
			aw_haptic->func->set_rtp_data(aw_haptic, aw_rtp->data, buf_len);
			glb_state_val = aw_haptic->func->get_glb_state(aw_haptic);
			aw_info("glb_state_val = 0x%02X", glb_state_val);
		...
		aw_info("Loop conditions: FIFO AFS = %d, Play Mode RTP = %d, Exit Flag = %d",
        	aw_haptic->func->rtp_get_fifo_afs(aw_haptic),
        	aw_haptic->play_mode == AW_RTP_MODE,
        	atomic_read(&aw_haptic->exit_in_rtp_loop));
	}
	enable_irq(gpio_to_irq(aw_haptic->irq_gpio));
	if (aw_haptic->play_mode == AW_RTP_MODE &&
		!atomic_read(&aw_haptic->exit_in_rtp_loop)) {
		aw_info("rtp_play: set rtp aei true\n");
