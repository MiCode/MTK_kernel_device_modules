			       ...
			       struct ff_effect *old)
{
	...
	aw_info("input_upload_effect: effect->type=0x%x,FF_CONSTANT=0x%x,FF_PERIODIC=0x%x\n",
	...
	while (atomic_read(&aw_haptic->exit_in_rtp_loop)) {
		aw_info("input_upload_effect: goint to waiting rtp  exit\n");
		mutex_unlock(&aw_haptic->lock);
		ret = wait_event_interruptible(aw_haptic->stop_wait_q,
				atomic_read(&aw_haptic->exit_in_rtp_loop) == 0);
		aw_info("input_upload_effect: wakeup\n");
		if (ret == -ERESTARTSYS) {
			mutex_unlock(&aw_haptic->lock);
			aw_err("input_upload_effect: wake up by signal return erro\n");
			return ret;
		}
		mutex_lock(&aw_haptic->lock);
	}
	...
	if (aw_haptic->effect_type == FF_CONSTANT) {
		...
		aw_info("input_upload_effect:waveform id = %d", aw_haptic->index);
#ifdef DUAL_RTP_TEST
		...
		aw_info("input_upload_effect:waveform id = %d", data[0]);
		aw_haptic->index = data[0];
		play->vmax_mv = effect->u.periodic.magnitude;
		if (left != NULL) {
			left->play.vmax_mv = play->vmax_mv;
		}
		aw_info("input_upload_effect:aw_haptic->info.effect_max = %d", aw_haptic->info.effect_max);
		...
		aw_info("input_upload_effect:aw_haptic->ram.ram_num = %d", aw_haptic->ram.ram_num);
		if (aw_haptic->index < aw_haptic->ram.ram_num) {
			aw_haptic->activate_mode = AW_RAM_MODE;
			aw_info("input_upload_effect: aw_haptic->index=%d , aw_haptic->activate_mode = %d\n",
...
int input_playback(struct input_dev *dev, int effect_id, int val)
{
	...
	aw_info("input_playback: index=%d , activate_mode = %d val = %d\n",
	...
	if (aw_haptic->effect_type == FF_CONSTANT &&
		aw_haptic->activate_mode == AW_RAM_LOOP_MODE) {
		aw_info("%s: enter cont_mode\n", __func__);
		queue_work(aw_haptic->work_queue, &aw_haptic->input_vib_work);
	} else if (aw_haptic->effect_type == FF_PERIODIC &&
		aw_haptic->activate_mode == AW_RAM_MODE) {
		aw_info("%s: enter ram_mode\n", __func__);
		queue_work(aw_haptic->work_queue, &aw_haptic->input_vib_work);
	} else if ((aw_haptic->effect_type == FF_PERIODIC) &&
		aw_haptic->activate_mode == AW_RTP_MODE) {
		aw_info("%s: enter rtp_mode\n", __func__);
...
static void rtp_work_routine(struct work_struct *work)
{
	...
	while (atomic_read(&aw_haptic->is_in_rtp_loop)) {
		aw_info("rtp_work_routine goint to waiting irq exit\n");
		mutex_unlock(&aw_haptic->lock);
		ret = wait_event_interruptible(aw_haptic->wait_q,
				atomic_read(&aw_haptic->is_in_rtp_loop) == 0);
		aw_info("rtp_work_routine wakeup\n");
		mutex_lock(&aw_haptic->lock);
		if (ret == -ERESTARTSYS) {
			atomic_set(&aw_haptic->exit_in_rtp_loop, 0);
			wake_up_interruptible(&aw_haptic->stop_wait_q);
			mutex_unlock(&aw_haptic->lock);
			aw_err("rtp_work_routine wake up by signal return erro\n");
			return;
		}
	}
	...
	if (aw_haptic->is_custom_wave == 1 && aw_haptic->state) {
		aw_err("rtp_work_routine:buffer size %d, availbe size %d\n",
		...
		while (get_rb_avalible_size() < aw_haptic->ram.base_addr &&
			...
			ret = wait_event_interruptible(aw_haptic->stop_wait_q,
							...
							rb_shoule_exit());
			aw_info("rtp_work_routine: wakeup\n");
			aw_err("rtp_work_routine: after wakeup sbuffer size %d, availbe size %d\n",
					aw_haptic->ram.base_addr >> 2, get_rb_avalible_size());
			if (ret == -ERESTARTSYS) {
				aw_err("rtp_work_routine: wake up by signal return erro\n");
					return;
	...
	if (aw_haptic->state) {
		...
		if (aw_haptic->is_custom_wave == 0) {
			...
			aw_info("rtp_work_routine: aw_haptic->rtp_file_num =%d\n",aw_haptic->rtp_file_num);
			...
			ret = request_firmware(&rtp_file,
					...
					aw_haptic->dev);
			if (ret < 0) {
				aw_err("rtp_work_routine: failed to read %s\n",aw_rtp_name[aw_haptic->rtp_file_num]);
			...
			if (!aw_haptic->aw_rtp) {
				release_firmware(rtp_file);
				aw_err("rtp_work_routine: error allocating memory\n");
				aw_haptic->rtp_routine_on = 0;
				pm_relax(aw_haptic->dev);
				mutex_unlock(&aw_haptic->lock);
				return;
			...
			aw_haptic->aw_rtp->len = rtp_file->size;
			aw_info("rtp_work_routine: rtp file:[%s] size = %dbytes\n",
