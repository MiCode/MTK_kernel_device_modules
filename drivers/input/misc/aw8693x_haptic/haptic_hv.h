...
struct aw_haptic {
	...
	atomic_t is_in_tiktap_mode;
	bool tiktap_ram_enable;
#endif
	int effect_type;
	int is_custom_wave;
...
struct aw_haptic_func {
	...
	void (*play_dual_rtp)(void);
	void (*play_dual_ram)(void);
	...
	void (*set_d2s_gain)(struct aw_haptic *aw_haptic);
	...
	uint8_t (*rtp_get_fifo_afi)(struct aw_haptic *aw_haptic);
...
extern int aw_haptic_upload_effect_to_ext(struct input_dev *dev,struct ff_effect *effect,struct ff_effect *old);
extern ssize_t aw_haptic_custom_wave_store_to_ext(struct device *dev,struct device_attribute *attr,const char *buf, size_t count);
extern int input_playback(struct input_dev *dev, int effect_id, int val);
extern int CUSTOME_WAVE_ID;
extern char awinic_rtp_name[][AWINIC_RTP_NAME_MAX];
extern int awinic_rtp_name_len;
#ifdef A2HAPTIC_SUPPORT
extern struct aw_haptic* vir_aw_haptic;
#endif
#endif
