...
static int aw8693x_get_usb_state(struct aw_haptic *aw_haptic)
{
    ...
    rc = power_supply_get_property(batt_psy, POWER_SUPPLY_PROP_STATUS, &prop);
    if (rc < 0) {
...
{
	int usb_status;
	usb_status = aw8693x_get_usb_state(aw_haptic);
	aw_info("usb_status = %d", usb_status);
	switch (play_mode) {
	...
	if (usb_status == POWER_SUPPLY_STATUS_CHARGING || usb_status == POWER_SUPPLY_STATUS_FULL) {
		aw8693x_auto_brake_mode(aw_haptic, false);
		aw_info("usb is charging or full, disable auto_brake");
	}
}
