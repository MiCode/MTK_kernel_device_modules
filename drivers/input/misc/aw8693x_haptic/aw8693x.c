...
#endif
void aw8693x_broadcast_config(struct aw_haptic *aw_haptic, bool flag, uint8_t play_mode);
/******************************************************
 *
 * aw8693x codec
 *
...
{
	pr_info("aw8693x_set_fmt fmt=0x%X\n", fmt);
...
{
	pr_info("aw8693x_set_dai_sysclk freq=%d\n", freq);
...
static const struct snd_soc_dapm_widget aw8693x_dapm_widgets[] = {
...
static const struct snd_soc_dapm_route aw8693x_dapm_routes[] = {
...
{
	pr_info("aw8693x_codec_remove enter\n");
}
#else
...
static aw_snd_soc_codec_driver_t soc_codec_dev_aw8693x = {
	...
	.dapm_widgets		= aw8693x_dapm_widgets,
	.num_dapm_widgets	= ARRAY_SIZE(aw8693x_dapm_widgets),
	.dapm_routes		= aw8693x_dapm_routes,
	.num_dapm_routes	= ARRAY_SIZE(aw8693x_dapm_routes),
};
...
static void aw8693x_set_d2s_gain(struct aw_haptic *aw_haptic)
{
	haptic_hv_i2c_write_bits(aw_haptic, AW8693X_REG_DETCFG2,
				 AW8693X_BIT_DETCFG2_D2S_GAIN_MASK, aw_haptic->info.d2s_gain);
}

static void aw8693x_get_bemf_peak(struct aw_haptic *aw_haptic, uint16_t *peak)
{
	uint8_t reg_val[AW_I2C_BYTE_FOUR] = {0};

	haptic_hv_i2c_reads(aw_haptic, 0x29, reg_val, AW_I2C_BYTE_FOUR);
	peak[0] = ((uint16_t)reg_val[1] << 2) | (reg_val[0] & 0x03);
	peak[1] = ((uint16_t)reg_val[2] << 2) | ((reg_val[0] & 0x0C) >> 2);
	peak[2] = ((uint16_t)reg_val[3] << 2) | (reg_val[0] >> 4);
}
...
static int aw8693x_wait_enter_standby(struct aw_haptic *aw_haptic)
{
	...
	while ((count > 0) && (brake_count > 0))  {
		reg_val = aw8693x_get_glb_state(aw_haptic);
		if (reg_val == AW8693X_BIT_GLBRD5_STATE_BRAKE) {
			aw_info("entered brake!");
			brake_count--;
		} else if (reg_val == AW8693X_BIT_GLBRD5_STATE_STANDBY) {
...
#ifdef DUAL_RTP_TEST
static void aw8693x_play_standby(struct aw_haptic *aw_haptic)
{
		aw_info("enter aw8693x_play_standby");
...
{
	aw_info("enter aw8693x_play_mode");
	switch (play_mode) {
	case AW_STANDBY_MODE:
		aw_info("enter standby mode");
		aw_haptic->play_mode = AW_STANDBY_MODE;
#ifdef DUAL_RTP_TEST
		mutex_lock(&right->dual_lock);
		if ((aw_haptic == right) && (right->i2c->addr == AW8693X_BROADCAST_ADDR)) {
			aw_haptic->i2c->addr = left->i2c->addr;
			aw8693x_play_standby(aw_haptic);
	...
	case AW_RTP_MODE:
		...
		if(atomic_read(&aw_haptic->is_in_tiktap_mode))
			aw8693x_auto_brake_mode(aw_haptic, false);
		else
...
#ifdef DUAL_RTP_TEST
static void aw8693x_play_dual_rtp(void)
{
	mutex_lock(&right->dual_lock);
	...
	aw8693x_broadcast_config(right, true, AW_RTP_MODE);
}

static void aw8693x_play_dual_ram(void)
{
	mutex_lock(&right->dual_lock);
	if (right->i2c->addr == AW8693X_BROADCAST_ADDR) {
		right->i2c->addr = right->record_i2c_addr;
	...
	mutex_unlock(&right->dual_lock);
	aw8693x_set_pwm(left, AW_PWM_24K);
	aw8693x_broadcast_config(right, false, AW_RAM_MODE);
	aw8693x_broadcast_config(right, true, AW_RAM_MODE);
}
#endif

#ifdef DUAL_RTP_TEST
static void aw8693x_ram_init_temp(struct aw_haptic *aw_haptic, bool flag)
{
	if (flag) {
		haptic_hv_i2c_write_bits(aw_haptic, AW8693X_REG_SYSCTRL3,
...
static void aw8693x_ram_init(struct aw_haptic *aw_haptic, bool flag)
{
	...
	if ((aw_haptic == right) && (right->i2c->addr == AW8693X_BROADCAST_ADDR)) {
		aw_haptic->i2c->addr = left->i2c->addr;
		aw8693x_ram_init_temp(aw_haptic, flag);
		aw_haptic->i2c->addr = right->record_i2c_addr;
		aw8693x_ram_init_temp(aw_haptic, flag);
		aw_haptic->i2c->addr = AW8693X_BROADCAST_ADDR;
	} else {
...
static int aw8693x_get_irq_state(struct aw_haptic *aw_haptic)
{
	...
	mutex_lock(&right->dual_lock);
	if (aw_haptic->i2c->addr == AW8693X_BROADCAST_ADDR) {
...
static int aw8693x_read_f0(struct aw_haptic *aw_haptic)
{
	...
	if (!f0_reg) {
		aw_haptic->f0 = 0;
		aw_haptic->f0_cali_status = false;
		aw_err("lra_f0 is error, f0_reg = 0");
		return -ERANGE;
	}
	aw_haptic->f0_cali_status = true;
	...
	aw_haptic->cont_f0 = AW8693X_F0_FORMULA(f0_reg);
...
static int aw8693x_get_f0(struct aw_haptic *aw_haptic)
{
	...
	cont_config[1] = aw_haptic->info.drv_width;
...
static uint8_t aw8693x_rtp_get_fifo_afi(struct aw_haptic *aw_haptic)
{
	uint8_t reg_val = 0;
	bool change_addr_flag = false;
...
#endif
	haptic_hv_i2c_reads(aw_haptic, AW8693X_REG_SYSINT, &reg_val, AW_I2C_BYTE_ONE);
	reg_val &= AW8693X_BIT_SYSINT_FF_AFI;
	reg_val >>= 3;
#ifdef AAC_RICHTAP_SUPPORT
	if (change_addr_flag)
		aw_haptic->i2c->addr = AW8693X_BROADCAST_ADDR;
...
static uint64_t aw8693x_get_theory_time(struct aw_haptic *aw_haptic)
{
	...
	if (fre_val == AW8693X_BIT_SYSCTRL4_WAVDAT_8K)
			     ...
			     struct device_node *np)
{
	...
	if (val != 0){
		...
		XM_HAP_F0_PROTECT_EXCEPTION(0, "AW：haptics_get_closeloop_lra_period");
	}
	...
	val = of_property_read_u8(np, "aw8693x_cont_drv2_lvl", &aw_haptic->info.cont_drv2_lvl);
	if (val != 0)
		aw_info("aw8693x_cont_drv2_lvl not found");

	val = of_property_read_u8(np, "aw8693x_drv_width", &aw_haptic->info.drv_width);
	if (val != 0)
		aw_info("aw8693x_drv_width not found");
	...
	val = of_property_read_u8(np, "aw8693x_algo_d2s_gain",
								&aw_haptic->info.algo_d2s_gain);
	if (val != 0)
		aw_err("aw8693x_algo_d2s_gain not found");
	...
	val = of_property_read_u32(np, "aw8693x_vib_effect_max",
				   &aw_haptic->info.effect_max);
	if (val != 0)
		aw_info("aw8693x_vib_effect_max not found");
...
void aw8693x_broadcast_config(struct aw_haptic *aw_haptic, bool flag, uint8_t play_mode)
{
	...
	mutex_lock(&right->dual_lock);
	if (flag) {
		if (right->address_mode == AW8693X_NORMAL) {
			aw_info("config broadcast mode");
			right->i2c->addr = right->record_i2c_addr;
			disable_irq_nosync(gpio_to_irq(left->irq_gpio));
			if (play_mode == AW_RTP_MODE)
			...
			right->address_mode = AW8693X_BROADCAST;
		} else {
			aw_info("already broadcast mode");
		}
	} else {
		if (right->address_mode == AW8693X_BROADCAST) {
			aw_info("restore normal mode");
			right->i2c->addr = AW8693X_BROADCAST_ADDR;
			if (play_mode == AW_RTP_MODE)
				haptic_hv_i2c_write_bits(right, AW8693X_REG_RTPCFG1,
									AW8693X_BIT_RTPCFG1_TWORTP_EN_MASK,
			...
			right->address_mode = AW8693X_NORMAL;
		} else {
			aw_info("already restore normal mode");
		}
	}
	mutex_unlock(&right->dual_lock);
}

static ssize_t rtp_dual_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	haptic_hv_i2c_reads(left, AW8693X_REG_RTPCFG1, &reg_val[0], 1);
	haptic_hv_i2c_reads(left, AW8693X_REG_SYSCTRL2, &reg_val[1], 4);
	aw_info("Left reg0x2D = 0x%02x, reg0x44 = 0x%02x, reg0x45 = 0x%02x, reg0x46 = 0x%02x, reg0x47 = 0x%02x",
								reg_val[0], reg_val[1], reg_val[2], reg_val[3], reg_val[4]);
	mutex_lock(&right->dual_lock);
	right->i2c->addr = right->record_i2c_addr;
	haptic_hv_i2c_reads(right, AW8693X_REG_RTPCFG1, &reg_val[0], 1);
	haptic_hv_i2c_reads(right, AW8693X_REG_SYSCTRL2, &reg_val[1], 4);
	mutex_unlock(&right->dual_lock);
	aw_info("Right reg0x2D = 0x%02x, reg0x44 = 0x%02x, reg0x45 = 0x%02x, reg0x46 = 0x%02x, reg0x47 = 0x%02x",
								reg_val[0], reg_val[1], reg_val[2], reg_val[3], reg_val[4]);
...
static ssize_t rtp_dual_store(struct device *dev, struct device_attribute *attr,
...
static ssize_t ram_dual_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	return 0;
}

static ssize_t ram_dual_store(struct device *dev, struct device_attribute *attr,
				  const char *buf, size_t count)
{
	cdev_t *cdev = dev_get_drvdata(dev);
	struct aw_haptic *aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);
...
#ifdef DUAL_RTP_TEST
static DEVICE_ATTR_RW(rtp_dual);
static DEVICE_ATTR_RW(ram_dual);
#endif
...
#ifdef DUAL_RTP_TEST
	&dev_attr_rtp_dual.attr,
	&dev_attr_ram_dual.attr,
#endif
	NULL
};
...
struct aw_haptic_func aw8693x_func_list = {
	...
	.get_bemf_peak = aw8693x_get_bemf_peak,
	.set_d2s_gain = aw8693x_set_d2s_gain,
	.set_pwm = aw8693x_set_pwm,
	.play_mode = aw8693x_play_mode,
	.play_dual_rtp = aw8693x_play_dual_rtp,
	.play_dual_ram = aw8693x_play_dual_ram,
	...
	.rtp_get_fifo_afi = aw8693x_rtp_get_fifo_afi,
