...
static void sih688x_set_go_disable(sih_haptic_t *sih_haptic,
	uint8_t play_mode)
{
	hp_info("%s:enter! mode = %d\n", __func__, play_mode);
	switch (play_mode) {
	case SIH_RAM_MODE:
		haptic_regmap_update_bits(sih_haptic->regmapp.regmapping,
			SIH688X_REG_GO, SIH_GO_BIT_RAM_GO_MASK, SIH_GO_BIT_RAM_GO_DISABLE);
		break;
	case SIH_RTP_MODE:
		haptic_regmap_update_bits(sih_haptic->regmapp.regmapping,
			SIH688X_REG_GO, SIH_GO_BIT_RTP_GO_MASK, SIH_GO_BIT_RTP_GO_DISABLE);
		break;
	case SIH_CONT_MODE:
		haptic_regmap_update_bits(sih_haptic->regmapp.regmapping,
			SIH688X_REG_GO, SIH_GO_BIT_F0_SEQ_GO_MASK,
...
static void sih688x_play_go(sih_haptic_t *sih_haptic, bool flag)
{
	...
	if (flag) {
		...
		sih688x_set_go_disable(sih_haptic, sih_haptic->chip_ipara.play_mode);
	}
}
