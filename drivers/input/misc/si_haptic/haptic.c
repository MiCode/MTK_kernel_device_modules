					...
					struct ff_effect *old)
{
	...
	if (sih_haptic->chip_ipara.effect_type == FF_CONSTANT) {
		...
		if (sih_haptic->chip_ipara.effect_id >= sih_haptic->chip_ipara.effect_id_boundary) {
			...
			sih_haptic->ram.action_mode = SIH_IDLE_MODE;
			/*second data*/
			data[1] = 30;
			/*millisecond data*/
			data[2] = 0;
				   ...
				   int val)
{
	...
	hp_info("%s: effect_id=%d , play_mode = %d val = %d\n",
		__func__, sih_haptic->chip_ipara.effect_id, sih_haptic->chip_ipara.play_mode, val);
	...
	struct device_attribute *attr, char *buf)
{
	...
	len += snprintf(buf + len, PAGE_SIZE - len, "osc_data = 0x%02x\n",
...
static void rtp_work_func(struct work_struct *work)
{
	...
	if ((sih_haptic->chip_ipara.effect_id < sih_haptic->chip_ipara.effect_id_boundary) &&
	    (sih_haptic->chip_ipara.effect_id > sih_haptic->chip_ipara.effect_max))
		return;
	hp_info("%s:play_mode = %d, effect_id = %d, state = %d\n", __func__,
		sih_haptic->chip_ipara.play_mode, sih_haptic->chip_ipara.effect_id,
		sih_haptic->chip_ipara.state);
	mutex_lock(&sih_haptic->lock);
	sih_haptic->hp_func->set_rtp_aei(sih_haptic, false);
