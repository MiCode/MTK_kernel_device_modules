	...
	struct device_node *np)
{
	...
	val = of_property_read_u32(np, "loop_voltage", &sih_haptic->ram.loop_voltage);
	if (val != 0){
		hp_info("%s: loop_voltage not found\n", __func__);
	}
	...
	for (i = 0; i < SIH_LP_INDEX; i++) {
		val = of_property_read_u32_index(sih_node, "lp_pvdds", i, &value);
		if (val < 0) {
			hp_info("%s: lp_pvdds[%d] value acquire failed\n", __func__, i);
			return 0;
		} else {
			sih_haptic->chip_attr.lp_pvdds[i] = value;
			hp_dbg("%s: lp_pvdds[%d] = %d\n", __func__, i, sih_haptic->chip_attr.lp_pvdds[i]);
		}

		val = of_property_read_u32_index(sih_node, "lp_ipeaks", i, &value);
		if (val < 0) {
			hp_info("%s: lp_ipeaks[%d] value acquire failed\n", __func__, i);
			return 0;
		} else {
			sih_haptic->chip_attr.lp_ipeaks[i] = value;
			hp_dbg("%s: lp_ipeaks[%d] = %d\n", __func__, i, sih_haptic->chip_attr.lp_ipeaks[i]);
		}
	}
...
void sih_set_gain(struct input_dev *dev, u16 gain)
{
	...
	queue_work(sih_haptic->work_queue, &sih_haptic->ram.gain_work);
}
