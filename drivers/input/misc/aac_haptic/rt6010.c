...
static int32_t rt6010_get_f0(struct ics_haptic_data *haptic_data)
{
    ...
    if((haptic_data->chip_config.f0 < F0_LIMIT_LOW) || (haptic_data->chip_config.f0 > F0_LIMIT_HIGH))
    {
        haptic_data->chip_config.f0 = haptic_data->chip_config.sys_f0;
        ics_err("%s: f0 out of range, set to sys_f0 %u\n", __func__, haptic_data->chip_config.sys_f0);
    }
    return ret;
}
