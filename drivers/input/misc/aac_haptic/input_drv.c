                     ...
                     const char *buf, size_t count)
{
    ...
    while (count > 0)
    {
        ...
        if (fifo_in_size == 0)
        {
            ics_dbg("%s: FIFO full, waiting for space (3s timeout)\n", __func__);
            ret = wait_event_interruptible_timeout(haptic_data->kfifo_wait_q,
                                                  atomic_read(&haptic_data->kfifo_available) == 1,
                                                  HZ * 3);  // 3秒超时

            if (ret == 0) {
                ics_err("%s: write timeout after 3 seconds\n", __func__);
                ret = -ETIMEDOUT;
                goto wave_write_exit;
            } else if (ret == -ERESTARTSYS) {
                ics_err("%s: wake up by signal\n", __func__);
                goto wave_write_exit;
            }
