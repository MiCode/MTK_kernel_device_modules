...
static int cs40l26_input_init(struct cs40l26_private *cs40l26)
{
	...
	input_set_capability(cs40l26->input, EV_FF, FF_RUMBLE);
	input_set_capability(cs40l26->input, EV_FF, FF_CONSTANT);
...
int cs40l26_probe(struct cs40l26_private *cs40l26)
{
...
#if defined(BOOTUP_VIBE_FROM_ROM)
	// mbox range (0x01800000 ~ 0x01800026) corresponding to 4.3.2.x rom waveforms
	error = cs40l26_mailbox_write(cs40l26, 0x01800008);
	if (error) {
	...
	if (!cs40l26->fw_defer) {
		error = cs40l26_fw_upload(cs40l26);
		if (error){
			dev_err(cs40l26->dev, "Failed to cs40l26_fw_upload\n");
			goto err;
		}

		error = cs40l26_request_irq(cs40l26);
		if (error){
			dev_err(cs40l26->dev, "Failed to cs40l26_request_irq\n");
			goto err;
		}
	}
...
err:
 	error = devm_mfd_add_devices(cs40l26->dev, PLATFORM_DEVID_AUTO, cs40l26_devs,
			CS40L26_NUM_MFD_DEVS, NULL, 0, NULL);
	msleep(20); // Delay 20ms or more to have cs40l26 dummy codec registered before removing input device
