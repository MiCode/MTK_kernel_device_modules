...
static int ssusb_bp_of_property_parse(struct ssusb_mtk *ssusb,
				struct device_node *dn)
{
	struct of_phandle_args args;
	struct platform_device *pdev;
	...
	/* bus protect is optional */
	if (!of_property_read_bool(dn, "mediatek,usb-bus-protect"))
		return 0;

	ret = of_parse_phandle_with_fixed_args(dn,
		"mediatek,usb-bus-protect", 5, 0, &args);
	...
	ssusb->bus_protect = device_node_to_regmap(args.np);

	if (!ssusb->bus_protect)
		return -ENODEV;

	ssusb->bus_protect_set_oft = args.args[0];
	ssusb->bus_protect_clr_oft = args.args[1];
	ssusb->bus_protect_en_oft = args.args[2];
	ssusb->bus_protect_sta_oft = args.args[3];
	ssusb->bus_protect_mask = args.args[4];

	return PTR_ERR_OR_ZERO(ssusb->bus_protect);
}
...
get_phy:
	...
	ret = ssusb_bp_of_property_parse(ssusb, node);
	if (ret)
		dev_info(dev, "failed to parse bus protect");
