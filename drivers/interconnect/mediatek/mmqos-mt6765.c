...
#include "mmqos-mtk.h"
static const struct mtk_node_desc node_descs_mt6765[] = {
	...
	DEFINE_MNODE(common0_port1, MASTER_COMMON_PORT(0, 1), 0, false, 0x0, SLAVE_COMMON(0)),
	DEFINE_MNODE(common0_port2, MASTER_COMMON_PORT(0, 2), 0, false, 0x0, SLAVE_COMMON(0)),
	DEFINE_MNODE(common0_port3, MASTER_COMMON_PORT(0, 3), 0, false, 0x0, SLAVE_COMMON(0)),
	DEFINE_MNODE(common0_port4, MASTER_COMMON_PORT(0, 4), 0, false, 0x0, SLAVE_COMMON(0)),
	/*SMI Common*/
 ...
 */
	...
	DEFINE_MNODE(l1_venc_bsdma,
		MASTER_LARB_PORT(M4U_PORT_VENC_BSDMA), 7, false, 0x0, SLAVE_LARB(1)),
	DEFINE_MNODE(l1_venc_sv_comv,
		MASTER_LARB_PORT(M4U_PORT_VENC_SV_COMV), 9, true, 0x0, SLAVE_LARB(1)),
	DEFINE_MNODE(l1_venc_rd_comv,
		MASTER_LARB_PORT(M4U_PORT_VENC_RD_COMV), 7, false, 0x0, SLAVE_LARB(1)),
	DEFINE_MNODE(l1_jpgenc_rdma,
		MASTER_LARB_PORT(M4U_PORT_JPGENC_RDMA), 7, false, 0x0, SLAVE_LARB(1)),
	DEFINE_MNODE(l1_jpgenc_bsdma,
		MASTER_LARB_PORT(M4U_PORT_JPGENC_BSDMA), 7, false, 0x0, SLAVE_LARB(1)),
	DEFINE_MNODE(l1_venc_cur_luma,
		MASTER_LARB_PORT(M4U_PORT_VENC_CUR_LUMA), 7, false, 0x0, SLAVE_LARB(1)),
	DEFINE_MNODE(l1_venc_cur_chroma,
		MASTER_LARB_PORT(M4U_PORT_VENC_CUR_CHROMA), 7, false, 0x0, SLAVE_LARB(1)),
	DEFINE_MNODE(l1_venc_ref_luma,
		MASTER_LARB_PORT(M4U_PORT_VENC_REF_LUMA), 7, false, 0x0, SLAVE_LARB(1)),
	DEFINE_MNODE(l1_venc_ref_chroma,
		MASTER_LARB_PORT(M4U_PORT_VENC_REF_CHROMA), 7, false, 0x0, SLAVE_LARB(1)),
	/*Larb 2*/
...
static const char * const comm_muxes_mt6765[] = { "mm" };
static const char * const comm_icc_path_names_mt6765[] = { "icc-bw" };
static const char * const comm_icc_hrt_path_names_mt6765[] = { "icc-hrt-bw" };
static const struct mtk_mmqos_desc mmqos_desc_mt6765 = {
	.nodes = node_descs_mt6765,
	.num_nodes = ARRAY_SIZE(node_descs_mt6765),
	.comm_muxes = comm_muxes_mt6765,
	.comm_icc_path_names = comm_icc_path_names_mt6765,
	.comm_icc_hrt_path_names = comm_icc_hrt_path_names_mt6765,
	.max_ratio = 40,
	.hrt = {
		.hrt_bw = {1850, 0, 0},
		.hrt_total_bw = 7464, /*Todo: Use DRAMC API 1866*1(channel)*4(io width)*/
		.md_speech_bw = { 1850, 1850},
...
static const struct of_device_id mtk_mmqos_mt6765_of_ids[] = {
	{
		.compatible = "mediatek,mt6765-mmqos",
		.data = &mmqos_desc_mt6765,
	},
	{}
};
MODULE_DEVICE_TABLE(of, mtk_mmqos_mt6765_of_ids);
static struct platform_driver mtk_mmqos_mt6765_driver = {
	.probe = mtk_mmqos_probe,
	.remove = mtk_mmqos_remove,
	.driver = {
		.name = "mtk-mt6765-mmqos",
		.of_match_table = mtk_mmqos_mt6765_of_ids,
	},
};
module_platform_driver(mtk_mmqos_mt6765_driver);
MODULE_LICENSE("GPL");
