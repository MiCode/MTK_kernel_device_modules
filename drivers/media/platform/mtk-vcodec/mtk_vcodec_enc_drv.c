...
int mtk_venc_ipi_dump;
static int dump_ipi_history_cb(const char *val, const struct kernel_param *kp)
{
	mtk_vcodec_dump_ipi_history(dev_ptr, 0);
	return 0;
}
static const struct kernel_param_ops venc_dump_ipi_param_ops = {
	.set = dump_ipi_history_cb,
};
module_param_cb(mtk_venc_ipi_dump, &venc_dump_ipi_param_ops, &mtk_venc_ipi_dump, 0644);
