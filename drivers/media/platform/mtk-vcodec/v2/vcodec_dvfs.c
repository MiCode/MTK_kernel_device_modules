	...
	if (inst->codec_type == MTK_INST_DECODER) {
		...
		(inst->width * inst->height <= 1280 * 736)) {
			...
			if (inst->priority > 0)
				...
				inst->op_rate = 2500;
			mtk_vcodec_dvfs_qos_log(true, "[VDVFS] VDEC w:%u x h:%u priority %d, new oprate %u",
				inst->width, inst->height, inst->priority, inst->op_rate);

			freq = (u64)inst->width * inst->height / 256 * inst->op_rate *
				perf->cy_per_mb_1;

			mtk_vcodec_dvfs_qos_log(true, "[VDVFS] VDEC priority:%d oprate:%d, set freq = %llu",
...
void update_freq(struct mtk_vcodec_dev *dev, int codec_type)
{
	...
	if (codec_type == MTK_INST_DECODER) {
		...
		if (dev->venc_dvfs_params.trans_inst && mtk_vcodec_has_active_inst(dev, MTK_INST_DECODER)) {
			...
			mtk_vcodec_dvfs_qos_log(true, "%s [VDVFS] - has trancode, set venc max freq\n", __func__);
			return;
		}
	}
