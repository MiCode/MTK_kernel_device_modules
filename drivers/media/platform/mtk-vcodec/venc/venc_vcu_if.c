				   ...
				   unsigned int *bs_size)
{
	...
	if (frm_buf) {
		...
		mtk_vcodec_debug(vcu, " num_planes = %d input (dmabuf:%lx), size %d %llx",
			...
			vsi->meta_size,
			vsi->meta_addr);
		mtk_vcodec_debug(vcu, "vsi qpmap addr %llx size %d, qprects addr %llx size %d meta_size %d",
			vsi->meta_addr, vsi->qpmap_size, vsi->qprects_addr, vsi->qprects_size, vsi->qprects_meta_size);
	} else {
