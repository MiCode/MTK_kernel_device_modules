...
static int venc_vcp_resume_request_slbc(struct mtk_vcodec_dev *dev)
{
	...
	int curr_daemon_pid = vcp_cmd_ex(VENC_FEATURE_ID, VCP_GET_GEN, "venc_srv");
	struct venc_inst *inst;
	unsigned int ret_slb;
	bool need_notify_driver = false;
	...
	unsigned long event, void *ptr)
{
	...
	case VCP_EVENT_SUSPEND:
		...
		if (need_backup) {
			...
			venc_vcp_suspend_release_slbc(dev);
			dev->has_backup = true;
		}
	...
	case VCP_EVENT_RESUME:
		...
		if (dev->has_backup) {
			...
			venc_vcp_resume_request_slbc(dev);
...
void venc_vcp_probe(struct mtk_vcodec_dev *dev)
{
	...
	dev->ipi_recv_task = kthread_run(vcp_enc_ipi_handler, dev, "venc_ipi_recv");
					  ...
					  struct venc_enc_param *enc_param)
{
	...
	case VENC_SET_PARAM_COMPATIBILITY_OPTION:
		...
		break;
	case VENC_SET_PARAM_TIMING_INFO:
		out.data_item = 1;
