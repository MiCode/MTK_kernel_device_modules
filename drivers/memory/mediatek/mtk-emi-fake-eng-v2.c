 ...
 * Copyright (c) 2024 Daniel Chang <yu-shuo.chang@mediatek.com>
 */
...
#define FAKE_ENG_HASH0_EMI          0xc8
#define FAKE_ENG_HASH1_EMI          0xcc
#define FAKE_ENG_CMD_AMOUNT         0xe4
#define FAKE_ENG_MAS_ID             0xec
...
static int fake_eng_init_normal(unsigned int chn_id)
{
	...
	fakeng->feng_arg.start_addr_wr = phy_addr >> 2;
	fakeng->feng_arg.start_addr_rd = phy_addr >> 2;
	fakeng->feng_arg.start_addr_wr_2nd = phy_addr >> 2;
	fakeng->feng_arg.start_addr_rd_2nd = phy_addr >> 2;

	fakeng->feng_arg.start_addr_wr_extend = phy_addr >> 34;
	fakeng->feng_arg.start_addr_rd_extend = phy_addr >> 34;
	fakeng->feng_arg.start_addr_wr_2nd_extend = phy_addr >> 34;
	fakeng->feng_arg.start_addr_rd_2nd_extend = phy_addr >> 34;
	...
	val = (0x0 << 24) | (0x1 << 20) | (0x1 << 16) |
			(fakeng->feng_arg.emi_disp_en << 12) |
			(fakeng->feng_arg.chn_hash_en << 8) |
			(fakeng->feng_arg.chn_number << 4) | 0x3;
	writel(val, fakeng->fake_eng_base[chn_id] + FAKE_ENG_HASH);

	/* NEW HASH */
	writel(fakeng->feng_arg.chn_hash_en,
			fakeng->fake_eng_base[chn_id] + FAKE_ENG_HASH0_EMI);
	writel(fakeng->feng_arg.emi_disp_en,
			fakeng->fake_eng_base[chn_id] + FAKE_ENG_HASH1_EMI);

	/* command amount */
	writel((fakeng->feng_arg.wr_amount << 8) |
			(fakeng->feng_arg.rd_amount << 0),
			fakeng->fake_eng_base[chn_id] + FAKE_ENG_CMD_AMOUNT);

	writel((0x2 << 4) | 0x2, fakeng->fake_eng_base[chn_id] + FAKE_ENG_MAS_ID);
								...
								unsigned int mode)
{
	...
	if(mode == 0)
		ret = fake_eng_init_normal(chn_id);
	else
		return;
	...
	(struct device_driver *driver, const char *buf, size_t count)
{
	...
	n = sscanf(buf, "%u,%u,%u,%u,%u,%u\n",
				&chn_id, &en, &wr_en, &rd_en, &latency, &mode);
	...
	pr_info("%s: %u %u %u %u %u %u\n",
				__func__, chn_id, en, wr_en, rd_en, latency, mode);
	...
	if (en)
		emi_fake_eng_start(chn_id, wr_en, rd_en, latency, mode);
	else
...
{
	pr_info("emi fake engine v2 unloaded\n");
...
static int __init emi_fake_eng_init(void)
{
	...
	pr_info("emi fake engine v2 loaded\n");

	ret = platform_driver_register(&emi_fake_eng_drv);
	if (ret) {
		pr_info("emi fake engine v2: failed to register dirver\n");
		return ret;
	}
