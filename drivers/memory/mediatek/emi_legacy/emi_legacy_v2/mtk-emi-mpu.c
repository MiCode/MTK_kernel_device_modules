...
static int emimpu_probe(struct platform_device *pdev)
{
	...
	if (ret) {
		dev_info(&pdev->dev, "No region_cnt\n");
		return -ENXIO;
	}
	...
	if (ret) {
		dev_info(&pdev->dev, "No domain_cnt\n");
		return -ENXIO;
	}
	...
	if (ret) {
		dev_info(&pdev->dev, "No addr_align\n");
		return -ENXIO;
	}
	...
	if (ret) {
		dev_info(&pdev->dev, "No dram_start\n");
		return -ENXIO;
	}
	...
	if (ret) {
		dev_info(&pdev->dev, "No dram_end fail\n");
		return -ENXIO;
	}
	...
	if (ret) {
		dev_info(&pdev->dev, "No ctrl_intf\n");
		return -ENXIO;
	}
	...
	for (i = 0; i < mpu->emi_cen_cnt; i++) {
		...
		if (IS_ERR(mpu->emi_cen_base[i])) {
			dev_info(&pdev->dev, "Failed to map EMI%d CEN base\n",
				i);
			return -EIO;
		}
		...
		if (IS_ERR(mpu->emi_mpu_base[i])) {
			dev_info(&pdev->dev, "Failed to map EMI%d MPU base\n",
				i);
			return -EIO;
		}
	}
	...
	if (ret) {
		dev_info(&pdev->dev, "Failed to request irq");
		ret = -EINVAL;
		goto free_ap_rg_info;
	}
	...
	if (!ret && slverr)
		for (i = 0; i < mpu->domain_cnt; i++) {
			arm_smccc_smc(MTK_SIP_EMIMPU_CONTROL, MTK_EMIMPU_SLVERR,
				i, 0, 0, 0, 0, 0, &smc_res);
			if (smc_res.a0) {
				dev_info(&pdev->dev, "Failed to set MPU domain%d Slave Error, ret=0x%lx\n",
					i, smc_res.a0);
				ret = -EINVAL;
				goto free_ap_rg_info;
			}
...
static __init int emimpu_init(void)
{
	...
	if (ret) {
		pr_info("emimpu: failed to register driver\n");
		return ret;
	}
