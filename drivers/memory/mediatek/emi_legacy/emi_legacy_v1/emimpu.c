...
static void (*md_handling_cb)(const char *vio_msg);
...
int mtk_emimpu_set_protection(struct emimpu_region_t *rg_info)
{
	...
	pr_info("%s: rg(%u, [%#llx, %#llx]), align_bit(%u).\n", __func__, rg_info->rg_num,
		rg_info->start, rg_info->end, emimpu_dev_ptr->addr_align);
	align_mask = (0x1u << emimpu_dev_ptr->addr_align) - 1;
	...
	(struct device_driver *driver, const char *buf, size_t count)
{
	...
	strscpy(command, buf, (size_t) MTK_EMI_MAX_CMD_LEN);
	...
	for (emi_id = 0; emi_id < emimpu_dev_ptr->emi_cen_cnt; emi_id++) {
		...
		for (i = 0; i < emimpu_dev_ptr->dump_cnt; i++) {
			...
			if (aee_msg_cnt < MTK_EMI_MAX_CMD_LEN) {
				ret_temp = snprintf(aee_msg + aee_msg_cnt,
		...
		if ((bypass == 1) && bypass_violation(emi_id, dump_reg,
					emimpu_dev_ptr->dump_cnt) == 1){
		...
		if (md_handling_cb) {
			...
			strncat(md_str, aee_msg, sizeof(md_str) - strlen(md_str) - 1);
			md_handling_cb(md_str);
		}
	}
...
static int emimpu_probe(struct platform_device *pdev)
{
	...
	ret = of_property_read_u32(emimpu_node, "bypass", &bypass);
	if (ret)
		bypass = 0;
	pr_info("bypass == %d\n ", bypass);
