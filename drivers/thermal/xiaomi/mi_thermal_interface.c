...
static ssize_t
/* P16 code for HQFEAT-95345 by p-songweijie at 2025/04/21 start */
		...
		struct device_attribute *attr, const char *buf, size_t len)
{
	...
	val = simple_strtol(buf, NULL, 10);
	ret = charger_notifier_call_chain(THERMAL_BOARD_TEMP, val/100);
	if (ret) {
		pr_err("%s: charger_notifier_call_chain error:%d\n", __func__, ret);
	}

	return len;
}
/* P16 code for HQFEAT-95345 by p-songweijie at 2025/04/21 end */
...
static ssize_t
thermal_temp_aware_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d\n", atomic_read(&temp_aware));
}
static ssize_t
thermal_temp_aware_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t len)
{
	int val = -1;
	val = simple_strtol(buf, NULL, 10);
...
static DEVICE_ATTR(temp_aware, 0664,
		thermal_temp_aware_show, thermal_temp_aware_store);
#ifdef CONFIG_MI_THERMAL_ATC_ENABLE
static ssize_t
