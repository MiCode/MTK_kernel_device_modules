...
static int lvts_read_tc_temperature(struct lvts_data *lvts_data, unsigned int tz_id, bool in_isr)
{
	...
	if(chip_id == NULL && lvts_data->mcu_sensor_id_remap) {
...
static noinline void writel_lcov(u32 value, void *addr)
{
	writel(value, addr);
}

static noinline int readl_lcov(void *addr)
{
	return readl(addr);
}

static noinline int readl_poll_timeout_lcov(void *addr,
	u32 data, bool cond, u32 delay_us, u32 iteration)
{
	return readl_poll_timeout(addr, data, cond, delay_us, iteration);
}


static noinline void lvts_write_device(struct lvts_data *lvts_data, unsigned int data,
	int tc_id)
{
	void __iomem *base;
	struct device *dev = lvts_data->dev;
	...
	writel_lcov(data, LVTS_CONFIG_0 + base);

	udelay(5);
	ret = readl_poll_timeout_lcov(LVTS_CONFIG_0 + base, data,
...
static noinline unsigned int lvts_read_device(struct lvts_data *lvts_data,
	unsigned int reg_idx, int tc_id)
{
	...
	base = GET_BASE_ADDR(tc_id);
	writel_lcov(READ_DEVICE_REG(reg_idx), LVTS_CONFIG_0 + base);
	...
	data = (readl_lcov(LVTSRDATA0_0 + base));
...
static noinline void device_identification_v1(struct lvts_data *lvts_data)
{
	...
	for (i = 0; i < lvts_data->num_tc; i++) {
		...
		writel_lcov(ENABLE_LVTS_CTRL_CLK, LVTSCLKEN_0 + base);
		...
		data = (readl_lcov(LVTS_ID_0 + base) & GENMASK(7, 0));
		if (data != (0x81 + i))
...
static noinline void disable_sensing_points(struct lvts_data *lvts_data, unsigned int tc_id)
{
	void __iomem *base;

	base = GET_BASE_ADDR(tc_id);
	writel_lcov(DISABLE_SENSING_POINT, LVTSMONCTL0_0 + base);
}

static noinline void disable_all_sensing_points(struct lvts_data *lvts_data)
{
	unsigned int i;
	void __iomem *base;
...
static noinline void enable_sensing_points(struct lvts_data *lvts_data, unsigned int tc_id)
{
	struct device *dev = lvts_data->dev;
	struct tc_settings *tc = lvts_data->tc;
	unsigned int j, num;
	...
	writel_lcov(flag, LVTSMONCTL0_0 + base);
}

static noinline void enable_all_sensing_points(struct lvts_data *lvts_data)
{
	struct device *dev = lvts_data->dev;
	struct tc_settings *tc = lvts_data->tc;
...
#ifdef DUMP_MORE_LOG
static noinline void read_controller_reg_when_error(struct lvts_data *lvts_data)
{
	unsigned int i;
	void __iomem *base;
	int temp, j;
	...
	for (i = 0; i < lvts_data->num_tc; i++) {
		...
		for (j = 0; j < NUM_LVTS_CONTROLLER_REG; j++) {
			temp = readl_lcov(g_lvts_controller_addrs[j] + base);
	...
	unsigned int dev_reg_idx, unsigned int data, int tc_id)
{
	...
	writel_lcov(config, LVTS_CONFIG_0 + base);
...
static noinline void read_device_reg_when_error(struct lvts_data *lvts_data)
{
	int i, j;
	unsigned int addr;
	void __iomem *base;
	...
	for (i = 0; i < lvts_data->num_tc; i++) {
		...
		for (j = 0; j < NUM_LVTS_DEVICE_REG; j++) {
			 ...
			 */
			cnt = 0;
			while ((readl_lcov(LVTS_CONFIG_0 + base) & BIT(24))) {
				cnt++;
			...
			g_lvts_device_value_e[i][j] = (readl_lcov(LVTSRDATA0_0 + base));
		}
	}
}
...
static int lvts_thermal_check_all_sensing_point_idle(struct lvts_data *lvts_data)
{
	...
	for (i = 0; i < lvts_data->num_tc; i++) {
		...
		base = GET_BASE_ADDR(i);
		temp = readl_lcov(base + LVTSMSRCTL1_0);
...
static noinline void lvts_wait_for_all_sensing_point_idle(struct lvts_data *lvts_data)
{
	struct device *dev = lvts_data->dev;
	...
	for (i = 0; i < lvts_data->num_tc; i++) {
		base = GET_BASE_ADDR(i);
		temp = readl_lcov(LVTSMSRCTL1_0 + base);
		if ((temp & mask) != 0) {
...
static void lvts_clear_thermal_gate(struct lvts_data *lvts_data)
{
	unsigned int i;
	void __iomem *base;
	...
	for (i = 0; i < lvts_data->num_tc; i++) {
		base = GET_BASE_ADDR(i);
		writel((LVTS_GATE_VALID|DISABLE_SENSING_POINT), LVTSMONCTL0_0 + base);
	}
...
void dump_lvts_error_info(struct lvts_data *lvts_data)
{
	...
	if (IS_ENABLE(
		FEATURE_GATING_WHEN_THERMAL_REBOOT)) {
		lvts_clear_thermal_gate(lvts_data);
	}
...
static noinline void set_polling_speed(struct lvts_data *lvts_data, int tc_id)
{
	struct tc_settings *tc = lvts_data->tc;
	 ...
	 */
	writel_lcov(lvtsMonCtl1, LVTSMONCTL1_0 + base);
	writel_lcov(lvtsMonCtl2, LVTSMONCTL2_0 + base);
}

static noinline void set_hw_filter(struct lvts_data *lvts_data, int tc_id)
{
	struct tc_settings *tc = lvts_data->tc;
	unsigned int option;
	void __iomem *base;
	...
	writel_lcov(option, LVTSMSRCTL0_0 + base);
}

static noinline int get_dominator_index(struct lvts_data *lvts_data, int tc_id)
{
	struct device *dev = lvts_data->dev;
	struct tc_settings *tc = lvts_data->tc;
	int d_index;
...
static noinline void disable_hw_reboot_interrupt(struct lvts_data *lvts_data, int tc_id)
{
	unsigned int temp;
	void __iomem *base;
	 ...
	 */
	temp = readl_lcov(LVTSPROTCTL_0 + base);
	writel_lcov(temp | 0x3FFF, LVTSPROTCTL_0 + base);

	/* Disable the interrupt of AP SW */
	temp = readl_lcov(LVTSMONINT_0 + base);
	...
	writel_lcov(temp, LVTSMONINT_0 + base);
}

static noinline void enable_hw_reboot_interrupt(struct lvts_data *lvts_data, int tc_id)
{
	unsigned int temp;
	void __iomem *base;
	...
	/* Clear the offset */
	temp = readl_lcov(LVTSPROTCTL_0 + base);
	writel_lcov(temp & ~PROTOFFSET, LVTSPROTCTL_0 + base);
}

static noinline void set_tc_hw_reboot_threshold(struct lvts_data *lvts_data,
	int trip_point, int tc_id)
{
	...
	if (d_index == ALL_SENSING_POINTS) {
		...
		config = (0x1 << 16);
		writel_lcov(config | temp, LVTSPROTCTL_0 + base);
		msr_raw = 0;
	...
	if (lvts_data->enable_dump_log) {
		/* high offset INT */
		writel_lcov(msr_raw, LVTSOFFSETH_0 + base);
		 ...
		 */
		writel_lcov(0xffff, LVTSOFFSETL_0 + base);
	} else {
		writel_lcov(msr_raw, LVTSPROTTC_0 + base);
	}

	enable_hw_reboot_interrupt(lvts_data, tc_id);
}

static noinline void set_all_tc_hw_reboot(struct lvts_data *lvts_data)
{
	struct tc_settings *tc = lvts_data->tc;
	int i, trip_point;
...
static noinline void update_all_tc_hw_reboot_point(struct lvts_data *lvts_data,
	int trip_point)
{
	struct tc_settings *tc = lvts_data->tc;
	int i;
...
static int prepare_calibration_data(struct lvts_data *lvts_data)
{
	...
	if (lvts_data->op_cali_support) {
		cal_data->op_cali = devm_kcalloc(dev, lvts_data->num_tc,
				sizeof(*cal_data->op_cali), GFP_KERNEL);
		if (!cal_data->op_cali)
			return -ENOMEM;
	...
	if (lvts_data->op_cali_support) {
		offset = snprintf(buffer, size, "[lvts_cal] num_tc:op_cali ");
		if (offset < 0)
			return -EINVAL;
		if (offset >= size)
			return -ENOMEM;
		for (i = 0; i < lvts_data->num_tc; i++) {
			ret = snprintf(buffer + offset, size - offset, "%d:0x%x ",
					i, cal_data->op_cali[i]);
			if (ret < 0) {
				dev_info(dev, "%s, %d, encode wrong\n", __func__, __LINE__);
				return -EINVAL;
			} else if (ret >= size - offset) {
				buffer[offset] = '\0';
				dev_info(dev, "%s\n", buffer);
...
static noinline enum interrupt_type interrupt_type_switch(struct lvts_data *lvts_data,
	unsigned int tc_id, unsigned int interrupt_reg_status)
{
	...
	base = GET_BASE_ADDR(tc_id);
	interrupt_reg_val = readl_lcov(LVTSMONINT_0 + base);
	...
	writel_lcov(interrupt_reg_val, LVTSMONINT_0 + base);
...
static noinline void lvts_debug_to_sysram(struct lvts_data *lvts_data, int tc_id)
{
	int i, temp;
	void __iomem *base;
	...
	for (i = 0; i < NUM_LVTS_DEBUG1_REG; i++) {
		temp = readl_lcov(g_lvts_controller_debug1_addr[i] + base);
	...
	writel_lcov( temp, (void __iomem *)(thermal_csram_base + LVTS_DEBUG1_OFFSET));
	writel_lcov(g_lvts_controller_debug1_value[2],
			(void __iomem *)(thermal_csram_base + LVTS_DEBUG1_OFFSET + 4));
	writel_lcov(g_lvts_controller_debug1_value[3],
	...
	for (i = 0; i < NUM_LVTS_DEBUG2_REG; i++) {
		temp = readl_lcov(g_lvts_controller_debug2_addr[i] + base);
	...
	writel_lcov( temp, (void __iomem *)(thermal_csram_base + debug2_offset));
	temp = (g_lvts_controller_debug2_value[3] & 0xFFFF) << 16 |
				(g_lvts_controller_debug2_value[2] & 0xFFFF);
	writel_lcov( temp, (void __iomem *)(thermal_csram_base + debug2_offset + 0x4));
	writel_lcov(g_lvts_controller_debug2_value[4],
			(void __iomem *)(thermal_csram_base + debug2_offset + 0x8));
	writel_lcov(g_lvts_controller_debug2_value[5],
			(void __iomem *)(thermal_csram_base + debug2_offset + 0xC));
	writel_lcov(g_lvts_controller_debug2_value[6],
			(void __iomem *)(thermal_csram_base + debug2_offset + 0x10));
	writel_lcov(g_lvts_controller_debug2_value[7],
			(void __iomem *)(thermal_csram_base + debug2_offset + 0x14));
	writel_lcov(tc_id, (void __iomem *)(thermal_csram_base + debug2_offset + 0x18));

	/* DEBUG3 for ATP info */
	for (i = 0; i < NUM_LVTS_DEBUG3_REG; i++) {
		temp = readl_lcov(g_lvts_controller_debug3_addr[i] + base);
		g_lvts_controller_debug3_value[i] = temp;
	}
	writel_lcov(g_lvts_controller_debug3_value[0],
			(void __iomem *)(thermal_csram_base + LVTS_DEBUG3_OFFSET));
	writel_lcov(g_lvts_controller_debug3_value[1],
	...
	writel_lcov(temp, (void __iomem *)(thermal_csram_base + LVTS_DEBUG3_OFFSET + 0x8));
	temp = (g_lvts_controller_debug3_value[4] & 0xFFFF) |
			((g_lvts_controller_debug3_value[5]&0xFFFF) << 16);
	writel_lcov(temp, (void __iomem *)(thermal_csram_base + LVTS_DEBUG3_OFFSET + 0xC));

	/* DEBUG4 for EDATA & C & LVTSPROTTC */
	if (!(lvts_data->debug_addr_v2)) {
		for (i = 0; i < NUM_LVTS_DEBUG4_REG; i++) {
			temp = readl_lcov(g_lvts_controller_debug4_addr[i] + base);
			g_lvts_controller_debug4_value[i] = temp;

			writel_lcov(g_lvts_controller_debug4_value[i],
...
static noinline void init_controller_v4(struct lvts_data *lvts_data)
{
	unsigned int i;
	void __iomem *base;
	...
	for (i = 0; i < lvts_data->num_tc; i++) {
		...
		lvts_write_device(lvts_data, SET_DEVICE_LOW_POWER_SINGLE_MODE_V4, i);
		writel_lcov(0x13121110, LVTSTSSEL_0 + base);
		writel_lcov(SET_CALC_SCALE_RULES, LVTSCALSCALE_0 + base);
...
static noinline void init_controller_v5(struct lvts_data *lvts_data)
{
	unsigned int i;
	void __iomem *base;
...
static noinline void mt6983_device_identification(struct lvts_data *lvts_data)
{
	struct device *dev = lvts_data->dev;
	unsigned int i, data;
	void __iomem *base;
...
static noinline void device_identification_v2(struct lvts_data *lvts_data)
{
	struct device *dev = lvts_data->dev;
	unsigned int i, data;
	void __iomem *base;
...
static noinline void mt6895_device_identification(struct lvts_data *lvts_data)
{
	struct device *dev = lvts_data->dev;
	unsigned int i, data;
	void __iomem *base;
 ...
 *==================================================
 */
#define MT6858_NUM_LVTS (ARRAY_SIZE(mt6858_tc_settings))

enum mt6858_lvts_domain {
	MT6858_AP_DOMAIN,
	MT6858_MCU_DOMAIN,
	MT6858_GPU_DOMAIN,
	MT6858_NUM_DOMAIN
...
enum mt6858_lvts_controller_enum {
	MT6858_LVTS_MCU_CTRL0,
	MT6858_LVTS_MCU_CTRL1,
	MT6858_LVTS_MCU_CTRL2,
	MT6858_LVTS_MCU_CTRL3,
	MT6858_LVTS_AP_CTRL0,
	MT6858_LVTS_AP_CTRL1,
	MT6858_LVTS_GPU_CTRL0,
	MT6858_LVTS_CTRL_NUM
};

static struct tc_settings mt6858_tc_settings[] = {
	[MT6858_LVTS_MCU_CTRL0] = {
		.domain_index = MT6858_MCU_DOMAIN,
		.addr_offset = 0x0,
		.num_sensor = 4,
		.sensor_map = {MT6858_TS1_0, MT6858_TS1_1, MT6858_TS1_2, MT6858_TS1_3},
		.sensor_on_off = {SEN_ON, SEN_ON, SEN_ON, SEN_ON},
		.tc_speed = SET_TC_SPEED_IN_US(10, 2460, 10, 10),
		.hw_filter = LVTS_FILTER_1,
		.dominator_sensing_point = ALL_SENSING_POINTS,
	...
	[MT6858_LVTS_MCU_CTRL1] = {
		...
		.addr_offset = 0x100,
		.num_sensor = 4,
		.sensor_map = {MT6858_TS2_0, MT6858_TS2_1, MT6858_TS2_2, MT6858_TS2_3},
		.sensor_on_off = {SEN_ON, SEN_ON, SEN_ON, SEN_ON},
		.tc_speed = SET_TC_SPEED_IN_US(10, 2460, 10, 10),
		.hw_filter = LVTS_FILTER_1,
		.dominator_sensing_point = ALL_SENSING_POINTS,
	...
	[MT6858_LVTS_MCU_CTRL2] = {
		...
		.addr_offset = 0x200,
		.num_sensor = 4,
		.sensor_map = {MT6858_TS3_0, MT6858_TS3_1, MT6858_TS3_2, MT6858_TS3_3},
		.sensor_on_off = {SEN_ON, SEN_ON, SEN_ON, SEN_ON},
		.tc_speed = SET_TC_SPEED_IN_US(10, 4490, 10, 10),
		.hw_filter = LVTS_FILTER_1,
		.dominator_sensing_point = ALL_SENSING_POINTS,
	...
	[MT6858_LVTS_MCU_CTRL3] = {
		...
		.addr_offset = 0x300,
		.num_sensor = 2,
		.sensor_map = {MT6858_TS4_0, MT6858_TS4_1},
		.sensor_on_off = {SEN_ON, SEN_ON},
		.tc_speed = SET_TC_SPEED_IN_US(10, 4490, 10, 10),
		.hw_filter = LVTS_FILTER_1,
		.dominator_sensing_point = ALL_SENSING_POINTS,
	...
	[MT6858_LVTS_AP_CTRL0] = {
		.domain_index = MT6858_AP_DOMAIN,
		.addr_offset = 0x0,
		.num_sensor = 3,
		.sensor_map = {MT6858_TS5_0, MT6858_TS5_1, MT6858_TS5_2},
		.sensor_on_off = {SEN_ON, SEN_ON, SEN_ON},
	...
	[MT6858_LVTS_AP_CTRL1] = {
		...
		.addr_offset = 0x100,
		.num_sensor = 2,
		.sensor_map = {MT6858_TS6_0, MT6858_TS6_1},
		.sensor_on_off = {SEN_ON, SEN_ON},
		.tc_speed = SET_TC_SPEED_IN_US(10, 327670, 10, 10),
		.hw_filter = LVTS_FILTER_1,
		.dominator_sensing_point = ALL_SENSING_POINTS,
	...
	[MT6858_LVTS_GPU_CTRL0] = {
		.domain_index = MT6858_GPU_DOMAIN,
		.addr_offset = 0x0,
		.num_sensor = 1,
		.sensor_map = {MT6858_TS8_0},
		.sensor_on_off = {SEN_ON},
		.tc_speed = SET_TC_SPEED_IN_US(10, 1380, 10, 10),
		.hw_filter = LVTS_FILTER_1,
		.dominator_sensing_point = ALL_SENSING_POINTS,
...
static void mt6858_efuse_to_cal_data(struct lvts_data *lvts_data)
{
	struct sensor_cal_data *cal_data = &lvts_data->cal_data;
	struct tc_settings *tc = lvts_data->tc;
	int i = 0;
	...
	if (!(cal_data->golden_temp)) {
		pr_info("efsue but golden_temp zero, set to default(%d)\n",
	...
	cal_data->count_r[MT6858_TS1_0] =  GET_CAL_DATA_BITMASK(1, 31, 16);
	cal_data->count_r[MT6858_TS1_1] =  GET_CAL_DATA_BITMASK(1, 15, 0);
	cal_data->count_r[MT6858_TS1_2] =  GET_CAL_DATA_BITMASK(2, 31, 16);
	cal_data->count_r[MT6858_TS1_3] =  GET_CAL_DATA_BITMASK(2, 15, 0);

	cal_data->count_r[MT6858_TS2_0] =  GET_CAL_DATA_BITMASK(4, 31, 16);
	cal_data->count_r[MT6858_TS2_1] =  GET_CAL_DATA_BITMASK(4, 15, 0);
	cal_data->count_r[MT6858_TS2_2] =  GET_CAL_DATA_BITMASK(5, 31, 16);
	cal_data->count_r[MT6858_TS2_3] =  GET_CAL_DATA_BITMASK(5, 15, 0);

	cal_data->count_r[MT6858_TS3_0] =  GET_CAL_DATA_BITMASK(7, 31, 16);
	cal_data->count_r[MT6858_TS3_1] =  GET_CAL_DATA_BITMASK(7, 15, 0);
	cal_data->count_r[MT6858_TS3_2] =  GET_CAL_DATA_BITMASK(8, 31, 16);
	cal_data->count_r[MT6858_TS3_3] =  GET_CAL_DATA_BITMASK(8, 15, 0);

	cal_data->count_r[MT6858_TS4_0] =  GET_CAL_DATA_BITMASK(10, 31, 16);
	cal_data->count_r[MT6858_TS4_1] =  GET_CAL_DATA_BITMASK(10, 15, 0);

	cal_data->count_r[MT6858_TS5_0] =  GET_CAL_DATA_BITMASK(12, 31, 16);
	cal_data->count_r[MT6858_TS5_1] =  GET_CAL_DATA_BITMASK(12, 15, 0);
	cal_data->count_r[MT6858_TS5_2] =  GET_CAL_DATA_BITMASK(13, 31, 16);

	cal_data->count_r[MT6858_TS6_0]  =  GET_CAL_DATA_BITMASK(15, 31, 16);
	cal_data->count_r[MT6858_TS6_1]  =  GET_CAL_DATA_BITMASK(15, 15, 0);

	cal_data->count_r[MT6858_TS8_0]  =  GET_CAL_DATA_BITMASK(17, 31, 16);

	cal_data->count_rc[MT6858_LVTS_MCU_CTRL0] =  GET_CAL_DATA_BITMASK(0, 31, 8);
	cal_data->count_rc[MT6858_LVTS_MCU_CTRL1] =  GET_CAL_DATA_BITMASK(3, 31, 8);
	cal_data->count_rc[MT6858_LVTS_MCU_CTRL2] =  GET_CAL_DATA_BITMASK(6, 31, 8);
	cal_data->count_rc[MT6858_LVTS_MCU_CTRL3] =  GET_CAL_DATA_BITMASK(9, 31, 8);

	cal_data->count_rc[MT6858_LVTS_AP_CTRL0] =  GET_CAL_DATA_BITMASK(11, 31, 8);
	cal_data->count_rc[MT6858_LVTS_AP_CTRL1] =  GET_CAL_DATA_BITMASK(14, 31, 8);

	cal_data->count_rc[MT6858_LVTS_GPU_CTRL0] =  GET_CAL_DATA_BITMASK(16, 31, 8);

}

static struct lvts_data mt6858_lvts_data = {
	.num_domain = MT6858_NUM_DOMAIN,
	.num_tc = MT6858_LVTS_CTRL_NUM,
	.tc = mt6858_tc_settings,
	.num_sensor = MT6858_NUM_TS,
	.ops = {
		.device_identification = device_identification_v2,
		.efuse_to_cal_data = mt6858_efuse_to_cal_data,
		.lvts_temp_to_raw = lvts_temp_to_raw_v1,
		.lvts_raw_to_temp = lvts_raw_to_temp_v1,
		.check_cal_data = check_cal_data_v1,
	},
...
enum mt6993_lvts_controller_enum {
	MT6993_LVTS_MCU_CTRL0,
	MT6993_LVTS_MCU_CTRL1,
	MT6993_LVTS_MCU_CTRL2,
	MT6993_LVTS_MCU_CTRL3,
	MT6993_LVTS_APU_CTRL0,
	MT6993_LVTS_GPU_CTRL0,
	MT6993_LVTS_AP_CTRL0,
	MT6993_LVTS_AP_CTRL1,
	MT6993_LVTS_AP_CTRL2,
	MT6993_LVTS_CTRL_NUM
};

static void mt6993_efuse_to_cal_data(struct lvts_data *lvts_data)
{
	struct sensor_cal_data *cal_data = &lvts_data->cal_data;
	struct tc_settings *tc = lvts_data->tc;
	int i = 0;
	...
	cal_data->count_r[MT6993_TS1_0] =  GET_CAL_DATA_BITMASK(1, 31, 16);
	cal_data->count_r[MT6993_TS1_1] =  GET_CAL_DATA_BITMASK(1, 15, 0);
	cal_data->count_r[MT6993_TS1_2] =  GET_CAL_DATA_BITMASK(2, 31, 16);
	cal_data->count_r[MT6993_TS1_3] =  GET_CAL_DATA_BITMASK(2, 15, 0);

	cal_data->count_r[MT6993_TS2_0] =  GET_CAL_DATA_BITMASK(3, 31, 16);
	cal_data->count_r[MT6993_TS2_1] =  GET_CAL_DATA_BITMASK(3, 15, 0);
	cal_data->count_r[MT6993_TS2_2] =  GET_CAL_DATA_BITMASK(4, 31, 16);
	cal_data->count_r[MT6993_TS2_3] =  GET_CAL_DATA_BITMASK(4, 15, 0);

	cal_data->count_r[MT6993_TS3_0] =  GET_CAL_DATA_BITMASK(5, 31, 16);
	cal_data->count_r[MT6993_TS3_1] =  GET_CAL_DATA_BITMASK(5, 15, 0);
	cal_data->count_r[MT6993_TS3_2] =  GET_CAL_DATA_BITMASK(6, 31, 16);
	cal_data->count_r[MT6993_TS3_3] =  GET_CAL_DATA_BITMASK(6, 15, 0);

	cal_data->count_r[MT6993_TS4_0] =  GET_CAL_DATA_BITMASK(7, 31, 16);
	cal_data->count_r[MT6993_TS4_1] =  GET_CAL_DATA_BITMASK(7, 15, 0);
	cal_data->count_r[MT6993_TS4_2] =  GET_CAL_DATA_BITMASK(8, 31, 16);
	cal_data->count_r[MT6991_TS4_3] =  GET_CAL_DATA_BITMASK(8, 15, 0);

	cal_data->count_r[MT6993_TS5_0] =  GET_CAL_DATA_BITMASK(9, 31, 16);
	cal_data->count_r[MT6993_TS5_1] =  GET_CAL_DATA_BITMASK(9, 15, 0);
	cal_data->count_r[MT6993_TS5_2] =  GET_CAL_DATA_BITMASK(10, 31, 16);
	cal_data->count_r[MT6993_TS5_3] =  GET_CAL_DATA_BITMASK(10, 15, 0);

	cal_data->count_r[MT6993_TS6_0] =  GET_CAL_DATA_BITMASK(11, 31, 16);

	cal_data->count_r[MT6993_TS7_0]  =  GET_CAL_DATA_BITMASK(12, 31, 16);
	cal_data->count_r[MT6993_TS7_1]  =  GET_CAL_DATA_BITMASK(12, 15, 0);
	cal_data->count_r[MT6993_TS7_2]  =  GET_CAL_DATA_BITMASK(13, 31, 16);
	cal_data->count_r[MT6993_TS7_3]  =  GET_CAL_DATA_BITMASK(13, 15, 0);

	cal_data->count_r[MT6993_TS8_0] =  GET_CAL_DATA_BITMASK(14, 31, 16);
	cal_data->count_r[MT6993_TS8_1] =  GET_CAL_DATA_BITMASK(14, 15, 0);
	cal_data->count_r[MT6993_TS8_2] =  GET_CAL_DATA_BITMASK(15, 31, 16);
	cal_data->count_r[MT6993_TS8_3] =  GET_CAL_DATA_BITMASK(15, 15, 0);

	cal_data->count_r[MT6993_TS9_0] =  GET_CAL_DATA_BITMASK(16, 31, 16);
	cal_data->count_r[MT6993_TS9_1] =  GET_CAL_DATA_BITMASK(16, 15, 0);

	cal_data->count_rc[MT6993_LVTS_MCU_CTRL0] =  GET_CAL_DATA_BITMASK(17, 23, 0);
	cal_data->count_rc[MT6993_LVTS_MCU_CTRL1] =  GET_CAL_DATA_BITMASK(18, 23, 0);
	cal_data->count_rc[MT6993_LVTS_MCU_CTRL2] =  GET_CAL_DATA_BITMASK(19, 23, 0);
	cal_data->count_rc[MT6993_LVTS_MCU_CTRL3] =  GET_CAL_DATA_BITMASK(20, 23, 0);

	cal_data->count_rc[MT6993_LVTS_APU_CTRL0] =  GET_CAL_DATA_BITMASK(21, 23, 0);
	cal_data->count_rc[MT6993_LVTS_GPU_CTRL0] =  GET_CAL_DATA_BITMASK(22, 23, 0);

	cal_data->count_rc[MT6993_LVTS_AP_CTRL0] =  GET_CAL_DATA_BITMASK(23, 23, 0);
	cal_data->count_rc[MT6993_LVTS_AP_CTRL1] =  GET_CAL_DATA_BITMASK(24, 23, 0);
	cal_data->count_rc[MT6993_LVTS_AP_CTRL2] =  GET_CAL_DATA_BITMASK(25, 23, 0);

	if (lvts_data->op_cali_support) {
		cal_data->op_cali[MT6993_LVTS_MCU_CTRL0] =  GET_CAL_DATA_BITMASK(17, 29, 24);
		cal_data->op_cali[MT6993_LVTS_MCU_CTRL1] =  GET_CAL_DATA_BITMASK(18, 29, 24);
		cal_data->op_cali[MT6993_LVTS_MCU_CTRL2] =  GET_CAL_DATA_BITMASK(19, 29, 24);
		cal_data->op_cali[MT6993_LVTS_MCU_CTRL3] =  GET_CAL_DATA_BITMASK(20, 29, 24);

		cal_data->op_cali[MT6993_LVTS_APU_CTRL0] =  GET_CAL_DATA_BITMASK(21, 29, 24);
		cal_data->op_cali[MT6993_LVTS_GPU_CTRL0] =  GET_CAL_DATA_BITMASK(22, 29, 24);

		cal_data->op_cali[MT6993_LVTS_AP_CTRL0] =  GET_CAL_DATA_BITMASK(23, 29, 24);
		cal_data->op_cali[MT6993_LVTS_AP_CTRL1] =  GET_CAL_DATA_BITMASK(24, 29, 24);
		cal_data->op_cali[MT6993_LVTS_AP_CTRL2] =  GET_CAL_DATA_BITMASK(25, 29, 24);
	}
}

static void mt6993_check_cal_data(struct lvts_data *lvts_data)
{
	struct device *dev = lvts_data->dev;
	struct sensor_cal_data *cal_data = &lvts_data->cal_data;
	struct tc_settings *tc = lvts_data->tc;
...
#define COF_A_T_SLP_GLD_6993 391460
#define COF_A_COUNT_R_GLD_6993 34412
#define COF_A_CONST_OFS_6993 0
#define COF_A_OFS_6993 (COF_A_T_SLP_GLD_6993 - COF_A_CONST_OFS_6993)

static void mt6993_update_coef_data(struct lvts_data *lvts_data)
{
	struct sensor_cal_data *cal_data = &lvts_data->cal_data;
	struct tc_settings *tc = lvts_data->tc;
	unsigned int i, j, s_index;
	...
	for (i = 0; i < lvts_data->num_tc; i++) {
		for  (j = 0; j < tc[i].num_sensor; j++) {
			...
			tc[i].coeff.a[j] = COF_A_OFS_6993 + (COF_A_CONST_OFS_6993 *
					cal_data->count_r[s_index] / COF_A_COUNT_R_GLD_6993);
...
static struct tc_settings mt6993_tc_settings[] = {
	[MT6993_LVTS_MCU_CTRL0] = {
		...
		.addr_offset = 0x0,
		.num_sensor = 4,
		.sensor_map = {MT6993_TS1_0, MT6993_TS1_1, MT6993_TS1_2, MT6993_TS1_3},
		.sensor_on_off = {SEN_ON, SEN_ON, SEN_ON, SEN_ON},
		.tc_speed = SET_TC_SPEED_IN_US(10, 1440, 10, 10),
		.hw_filter = LVTS_FILTER_1,
		.dominator_sensing_point = ALL_SENSING_POINTS,
	...
	[MT6993_LVTS_MCU_CTRL1] = {
		...
		.addr_offset = 0x100,
		.num_sensor = 4,
		.sensor_map = {MT6993_TS2_0, MT6993_TS2_1, MT6993_TS2_2, MT6993_TS2_3},
		.sensor_on_off = {SEN_ON, SEN_ON, SEN_ON, SEN_ON},
		.tc_speed = SET_TC_SPEED_IN_US(10, 1440, 10, 10),
		.hw_filter = LVTS_FILTER_1,
		.dominator_sensing_point = ALL_SENSING_POINTS,
	...
	[MT6993_LVTS_MCU_CTRL2] = {
		...
		.addr_offset = 0x200,
		.num_sensor = 4,
		.sensor_map = {MT6993_TS3_0, MT6993_TS3_1, MT6993_TS3_2, MT6993_TS3_3},
		.sensor_on_off = {SEN_ON, SEN_ON, SEN_ON, SEN_ON},
		.tc_speed = SET_TC_SPEED_IN_US(10, 1440, 10, 10),
		.hw_filter = LVTS_FILTER_1,
		.dominator_sensing_point = ALL_SENSING_POINTS,
	...
	[MT6993_LVTS_MCU_CTRL3] = {
		...
		.addr_offset = 0x300,
		.num_sensor = 4,
		.sensor_map = {MT6993_TS4_0, MT6993_TS4_1, MT6993_TS4_2, MT6993_TS4_3},
		.sensor_on_off = {SEN_ON, SEN_ON, SEN_ON, SEN_ON},
		.tc_speed = SET_TC_SPEED_IN_US(10, 10, 10, 10),
		.hw_filter = LVTS_FILTER_1,
		.dominator_sensing_point = ALL_SENSING_POINTS,
	...
	[MT6993_LVTS_APU_CTRL0] = {
		.domain_index = MT6993_APU_DOMAIN,
		.addr_offset = 0x0,
		.num_sensor = 4,
		.sensor_map = {MT6993_TS5_0, MT6993_TS5_1, MT6993_TS5_2, MT6993_TS5_3},
		.sensor_on_off = {SEN_ON, SEN_ON, SEN_ON, SEN_ON},
		.tc_speed = SET_TC_SPEED_IN_US(10, 1950, 10, 10),
		.hw_filter = LVTS_FILTER_1,
		.dominator_sensing_point = ALL_SENSING_POINTS,
	...
	[MT6993_LVTS_GPU_CTRL0] = {
		.domain_index = MT6993_GPU_DOMAIN,
		.addr_offset = 0x0,
		.num_sensor = 1,
		.sensor_map = {MT6993_TS6_0},
		.sensor_on_off = {SEN_ON},
		.tc_speed = SET_TC_SPEED_IN_US(10, 1230, 10, 10),
		.hw_filter = LVTS_FILTER_1,
		.dominator_sensing_point = ALL_SENSING_POINTS,
	...
	[MT6993_LVTS_AP_CTRL0] = {
		.domain_index = MT6993_AP_DOMAIN,
		.addr_offset = 0x0,
		.num_sensor = 4,
		.sensor_map = {MT6993_TS7_0, MT6993_TS7_1, MT6993_TS7_2, MT6993_TS7_3},
		.sensor_on_off = {SEN_ON, SEN_ON, SEN_ON, SEN_ON},
		.tc_speed = SET_TC_SPEED_IN_US(10, 327670, 10, 10),
		.hw_filter = LVTS_FILTER_1,
		.dominator_sensing_point = ALL_SENSING_POINTS,
	...
	[MT6993_LVTS_AP_CTRL1] = {
		...
		.addr_offset = 0x100,
		.num_sensor = 4,
		.sensor_map = {MT6993_TS8_0, MT6993_TS8_1, MT6993_TS8_2, MT6993_TS8_3},
		.sensor_on_off = {SEN_ON, SEN_ON, SEN_ON, SEN_ON},
		.tc_speed = SET_TC_SPEED_IN_US(10, 327670, 10, 10),
		.hw_filter = LVTS_FILTER_1,
		.dominator_sensing_point = ALL_SENSING_POINTS,
	...
	[MT6993_LVTS_AP_CTRL2] = {
		...
		.addr_offset = 0x200,
		.num_sensor = 2,
		.sensor_map = {MT6993_TS9_0, MT6993_TS9_1},
		.sensor_on_off = {SEN_ON, SEN_ON},
		.tc_speed = SET_TC_SPEED_IN_US(10, 327670, 10, 10),
		.hw_filter = LVTS_FILTER_1,
		.dominator_sensing_point = ALL_SENSING_POINTS,
...
static struct lvts_data mt6993_lvts_data = {
	.num_domain = MT6993_NUM_DOMAIN,
	.num_tc = MT6993_LVTS_CTRL_NUM,
	.tc = mt6993_tc_settings,
	.num_sensor = MT6993_NUM_TS,
	.ops = {
		.device_identification = NULL,
		.efuse_to_cal_data = mt6993_efuse_to_cal_data,
	...
	.cal_data = {
		.default_golden_temp = 60,
		.default_golden_temp_ht = 130,
		.default_count_r = 34412,
		.default_count_rc = 23363,
	},
	.init_done = false,
	.enable_dump_log = 0,
	.clock_gate_no_need = true,
	.reset_no_need = true,
	.op_cali_support = true,
	.is_tsfdc_n3e_ver = false,
	.dump_wo_pause = true,
	.gpu_power_ctrl_id = MT6993_LVTS_GPU_CTRL0,
...
static const struct of_device_id lvts_of_match[] = {
	...
	{
		.compatible = "mediatek,mt6993-lvts",
		.data = (void *)&mt6993_lvts_data,
	},
	{
