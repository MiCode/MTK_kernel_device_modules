...
static int cpu_isolate_set_cur_state(struct thermal_cooling_device *cdev, unsigned long state)
{
	...
	if (state == 1)
		ret = core_ctl_force_pause_request(dev->cpu_id, true, THERMAL_FORCE_PAUSE);
	else
		ret = core_ctl_force_pause_request(dev->cpu_id, false, THERMAL_FORCE_PAUSE);

	if (ret != 0)
		pr_info_ratelimited ("%s:%d set cpu%d iso %lu error %d\n", __func__, __LINE__, dev->cpu_id, state, ret);
...
static void cpu_isolate_remove(struct platform_device *pdev)
{
	int i;
	int num_cpu = MAX_CPUS;
...
static const struct of_device_id cpu_isolate_of_match[] = {
	{ .compatible = "mediatek,cpu-isolate-cooler", },
	{},
};
