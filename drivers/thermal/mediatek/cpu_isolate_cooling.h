...
#ifndef _CPU_ISOLATE_COOLING_H
#define _CPU_ISOLATE_COOLING_H
 ...
 */
/**
 * struct cpu_iso_cooling_device - data for MD cooling device
 ...
 * @node: list_head to link all cpu_iso_cooling_device.
 * @dev: device node pointer
 */
struct cpu_iso_cooling_device {
	char name[MAX_MD_COOLER_NAME_LEN];
	enum cpu_iso_cooling_type type;
	unsigned int pa_id;
	unsigned long target_state;
	unsigned long max_state;
	struct thermal_cooling_device *cdev;
	unsigned int throttle_tx_power[MAX_NUM_TX_PWR_STATE];
	int (*throttle)(struct cpu_iso_cooling_device *md_cdev, unsigned long state);
	struct list_head node;
	struct device *dev;
};

/**
 * struct cpu_iso_cooling_platform_data - MD cooler platform dependent data
 * @state_to_mutt_lv: callback function to transfer cooling state
 *			to mutt LV defined by MD
 * @max_lv: max cooler LV supported by MD
 */
struct cpu_iso_cooling_platform_data {
	unsigned long (*state_to_mutt_lv)(unsigned long state);
	unsigned long max_lv;
};

/**
 * struct cpu_iso_cooling_data - data for MD cooling driver
 ...
 */
struct cpu_iso_cooling_data {
	unsigned int pa_num;
	unsigned long mutt_state;
	enum cpu_iso_cooling_status status;
	const struct cpu_iso_cooling_platform_data *pdata;
};

extern enum cpu_iso_cooling_status get_cpu_iso_cooling_status(void);
