...
#include "devapc-mt6858.h"

static const struct mtk_device_num mtk6858_devices_num[] = {
...
static const char *mt6858_bus_id_to_master(uint32_t bus_id, uint32_t vio_addr,
		...
		uint32_t vio_addr)
{
	...
	if (slave_type == SLAVE_TYPE_INFRA) {
		for (i = 0; i < VIO_SLAVE_NUM_INFRA; i++) {
			if (vio_index == mt6858_devices_infra[i].vio_index)
				return mt6858_devices_infra[i].device;
		}
	} else if (slave_type == SLAVE_TYPE_INFRA1) {
		for (i = 0; i < VIO_SLAVE_NUM_INFRA1; i++) {
			if (vio_index == mt6858_devices_infra1[i].vio_index)
				return mt6858_devices_infra1[i].device;
		}
	} else if (slave_type == SLAVE_TYPE_PERI_PAR) {
		for (i = 0; i < VIO_SLAVE_NUM_PERI_PAR; i++) {
			if (vio_index == mt6858_devices_peri_par[i].vio_index)
				return mt6858_devices_peri_par[i].device;
		}
	} else if (slave_type == SLAVE_TYPE_VLP) {
		for (i = 0; i < VIO_SLAVE_NUM_VLP; i++) {
			if (vio_index == mt6858_devices_vlp[i].vio_index)
				return mt6858_devices_vlp[i].device;
		}
#if ENABLE_DEVAPC_ADSP
	} else if (slave_type == SLAVE_TYPE_ADSP) {
		for (i = 0; i < VIO_SLAVE_NUM_ADSP; i++) {
			if (vio_index == mt6858_devices_adsp[i].vio_index)
				return mt6858_devices_adsp[i].device;
		}
#endif
	} else if (slave_type == SLAVE_TYPE_MMINFRA) {
		for (i = 0; i < VIO_SLAVE_NUM_MMINFRA; i++) {
			if (vio_index == mt6858_devices_mminfra[i].vio_index)
				return mt6858_devices_mminfra[i].device;
		}
	} else if (slave_type == SLAVE_TYPE_MMUP) {
		for (i = 0; i < VIO_SLAVE_NUM_MMUP; i++) {
			if (vio_index == mt6858_devices_mmup[i].vio_index)
				return mt6858_devices_mmup[i].device;
		}
	} else if (slave_type == SLAVE_TYPE_GPU) {
		for (i = 0; i < VIO_SLAVE_NUM_GPU; i++) {
			if (vio_index == mt6858_devices_gpu[i].vio_index)
				return mt6858_devices_gpu[i].device;
		}
	}
...
static uint32_t mt6858_shift_group_get(int slave_type, uint32_t vio_idx)
{
	return 31;
}
...
static struct mtk_devapc_dbg_status mt6858_devapc_dbg_stat = {
...
static struct mtk_devapc_vio_info mt6858_devapc_vio_info = {
...
static const struct mtk_infra_vio_dbg_desc mt6858_vio_dbgs = {
...
static const struct mtk_sramrom_sec_vio_desc mt6858_sramrom_sec_vios = {
...
static const uint32_t mt6858_devapc_pds[] = {
	PD_VIO_MASK_OFFSET,
	PD_VIO_STA_OFFSET,
	PD_VIO_DBG0_OFFSET,
	PD_VIO_DBG1_OFFSET,
...
static struct mtk_devapc_soc mt6858_data = {
	.dbg_stat = &mt6858_devapc_dbg_stat,
	.slave_type_arr = slave_type_to_str,
	.slave_type_num = SLAVE_TYPE_NUM,
	.device_info[SLAVE_TYPE_INFRA] = mt6858_devices_infra,
	.device_info[SLAVE_TYPE_INFRA1] = mt6858_devices_infra1,
	.device_info[SLAVE_TYPE_PERI_PAR] = mt6858_devices_peri_par,
	.device_info[SLAVE_TYPE_VLP] = mt6858_devices_vlp,
#if ENABLE_DEVAPC_ADSP
	.device_info[SLAVE_TYPE_ADSP] = mt6858_devices_adsp,
#endif
	.device_info[SLAVE_TYPE_MMINFRA] = mt6858_devices_mminfra,
	.device_info[SLAVE_TYPE_MMUP] = mt6858_devices_mmup,
	.device_info[SLAVE_TYPE_GPU] = mt6858_devices_gpu,
	.ndevices = mtk6858_devices_num,
	.vio_info = &mt6858_devapc_vio_info,
	.vio_dbgs = &mt6858_vio_dbgs,
	.sramrom_sec_vios = &mt6858_sramrom_sec_vios,
	.devapc_pds = mt6858_devapc_pds,
	.irq_type_num = IRQ_TYPE_NUM,
	.subsys_get = &index_to_subsys,
	.master_get = &mt6858_bus_id_to_master,
	.mm2nd_vio_handler = &mm2nd_vio_handler,
	.shift_group_get = mt6858_shift_group_get,
};

static const struct of_device_id mt6858_devapc_dt_match[] = {
	{ .compatible = "mediatek,mt6858-devapc" },
	{},
};
...
static int mt6858_devapc_probe(struct platform_device *pdev)
{
	return mtk_devapc_probe(pdev, &mt6858_data);
}

static int mt6858_devapc_remove(struct platform_device *dev)
{
	return mtk_devapc_remove(dev);
}

static struct platform_driver mt6858_devapc_driver = {
	.probe = mt6858_devapc_probe,
	.remove = mt6858_devapc_remove,
	.driver = {
		.name = KBUILD_MODNAME,
		.of_match_table = mt6858_devapc_dt_match,
		.pm = &devapc_dev_pm_ops,
	},
};

module_platform_driver(mt6858_devapc_driver);

MODULE_DESCRIPTION("Mediatek MT6858 Device APC Driver");
