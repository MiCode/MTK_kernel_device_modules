			...
			unsigned long *handle)
{
	...
	*handle = zs_malloc_mi(NULL, pool, size, gfp);
#else
 ...
 */
#ifdef CONFIG_XRING_ZRAM_MEMCG_WRITEBACK
static struct zspage *alloc_zspage(struct zgroup_pool *zpool,
					struct zs_pool *pool,
					struct size_class *class,
					gfp_t gfp)
#else
...
{
        return zs_malloc_mi(NULL, pool, size, gfp);
}
...
#endif
{
	...
	spin_unlock(&pool->lock);
#ifdef CONFIG_XRING_ZRAM_MEMCG_WRITEBACK
	zspage = alloc_zspage(zpool, pool, class, gfp);
#else
...
#ifdef CONFIG_XRING_ZRAM_MEMCG_WRITEBACK
EXPORT_SYMBOL_GPL(zs_malloc_mi);
#else
EXPORT_SYMBOL_GPL(zs_malloc);
#endif
